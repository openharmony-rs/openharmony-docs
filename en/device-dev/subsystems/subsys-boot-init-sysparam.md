...
- Naming format
  ...
  | Category| API| Example| Description|
  | -------- | -------- | -------- | -------- |
...
| Action | System Native Process| System Application Process| Third-party Application Process|
| ----- | -------------- | ------------ | ------------ |
| get   | Permit          | Permit        | Permit        |
| watch | Permit          | Permit        | Permit        |
...
### Available APIs
  ...
  - syspara APIs
    ...
    **Table 7** Description of syspara APIs
    ...
    | const&nbsp;char\*&nbsp;GetChipType(void) | Obtains the CPU model of the device.|
    | int GetBootCount(void) | Obtains the number of times that the device has been powered on. If the operation fails, **-1** is returned.|
