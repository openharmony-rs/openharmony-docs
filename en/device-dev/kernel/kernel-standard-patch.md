...
1. Apply HDF patches.
   Apply the HDF patches based on the kernel version. For details, see [kernel.mk](https://gitcode.com/openharmony/kernel_linux_build/blob/master/kernel.mk).
   ```makefile
   $(OHOS_BUILD_HOME)/drivers/hdf_core/adapter/khdf/linux/patch_hdf.sh $(OHOS_BUILD_HOME) $(KERNEL_SRC_TMP_PATH) $(KERNEL_PATCH_PATH) $(DEVICE_NAME)
   ```

2. Apply the chip driver patch. The following uses Hi3516D V300 as an example.
   Place the chip component patches in the related directory. For details about the patch directory and naming rules, see [kernel.mk](https://gitcode.com/openharmony/kernel_linux_build/blob/master/kernel.mk).
   ```makefile
...
3. Modify the **config** file to build.
   Place the chip component **config** in the related directory. For details about the patch directory and naming rules, see [kernel.mk](https://gitcode.com/openharmony/kernel_linux_build/blob/master/kernel.mk).
   ```makefile
