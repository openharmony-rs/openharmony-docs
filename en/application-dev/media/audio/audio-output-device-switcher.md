...
<!-- md-trans-meta sourceCommit=2a52505c1ae7ab65042b3429d94aefccf312646c translatedAt=2026-09-18T02:03:13.849Z pushedAt=2026-09-18T07:30:55.267Z -->
...
Starting from API version 20, after activating an [AudioSession](../audio/audio-session-management.md), apps can use the [setDefaultOutputDevice](../../reference/apis-audio-kit/arkts-apis-audio-AudioSessionManager.md#setdefaultoutputdevice20) API of AudioSessionManager to set the default output device, and check whether the default device is set successfully through `AudioSessionManager.getDefaultOutputDevice`. The lifecycle of the default device setting follows the `AudioSession`. To cancel the default output device set by calling `setDefaultOutputDevice`, you can set the parameter to `audio.DeviceType.DEFAULT`, which returns the audio output device selection to the system.
   ...
   // ...
     ...
     // Set an appropriate audio session scenario for the application based on the business scenario. When the AudioSession is activated, the system requests the corresponding audio focus based on the audio session scenario selected by the application.
...
When the system is connected to an external device such as a Bluetooth headset or a wired headset, the system plays from the external device by default. Starting from API version 26.0.0, an app can use [setMediaOutputDevice](../../reference/apis-audio-kit/arkts-apis-audio-AudioSessionManager.md#setmediaoutputdevice) of `AudioSessionManager` to force the media output device to switch to the speaker, so that audio is still played from the speaker even when an external device is connected.
   ...
   > - On devices without a built-in speaker (such as a Vision product), calling this API to switch the output device to the speaker does not take effect, but the API call still returns success and does not throw an error or trigger an error callback.
...
| DEFAULT | 1000 | System default device. Clears the forced switch and restores the system default routing policy. |
   ...
   // ...
     ...
     // After a Bluetooth headset is connected, force the media stream output device to switch to the speaker.
     ...
     // Cancel the forced switch and return the media stream output device selection to the system default routing rules.
...
- **App-level:** Use [selectOutputDevice](../../reference/apis-audio-kit/arkts-apis-audio-AudioDeviceEnhanceManager.md#selectoutputdevice) to select a specified output device. After the setting succeeds, it takes effect on all playback streams created under the application.
  ...
  // ...
    let audioManager = audio.getAudioManager();
    let audioDeviceEnhanceManager: audio.AudioDeviceEnhanceManager = audioManager.getDeviceEnhanceManager();
    // Select an output device for the application. Here, device can be obtained as shown in the complete ArkTS example.
...
- **Audio stream level:** Use [selectOutputDeviceForAudioRenderer](../../reference/apis-audio-kit/arkts-apis-audio-AudioDeviceEnhanceManager.md#selectoutputdeviceforaudiorenderer) to select an output device for a specified audio playback stream. After the setting succeeds, it takes effect only on that playback stream.
  ...
  // ...
    let audioManager = audio.getAudioManager();
    let audioDeviceEnhanceManager: audio.AudioDeviceEnhanceManager = audioManager.getDeviceEnhanceManager();
    // Set the preferred output device for the specified audio playback stream. Here, renderer and outputDevice can be obtained as shown in the complete ArkTS example.
...
- **App-level:** Use [OH_AudioDeviceEnhanceManager_SelectOutputDevice](../../reference/apis-audio-kit/capi-native-audio-device-enhance-manager-h.md#oh_audiodeviceenhancemanager_selectoutputdevice) to select a specified output device.
