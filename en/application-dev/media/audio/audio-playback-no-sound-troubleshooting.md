...
<!-- md-trans-meta sourceCommit=c36c28945696e4e51a7cd220b9ca90518d2530d1 translatedAt=2026-09-18T02:23:33.801Z pushedAt=2026-09-18T10:39:19.036Z -->

When implementing audio playback, you may encounter issues such as failure to create or start a playback instance, or silent playback. This document describes common symptoms, related background knowledge, and troubleshooting methods for silent playback scenarios, and provides sample code and troubleshooting procedures.
...
**3. Abnormal audio data transmission from the app.**
   ...
   The system typically uses streaming to obtain audio data from the app frame by frame through the `OH_AudioRenderer_OnWriteDataCallback` callback. If the app incorrectly passes silent data or blocks the current callback function, the entire audio pipeline will play back abnormally, resulting in silent playback.
...
**5. Failure to integrate long-term tasks or concurrency control restrictions.**
   ...
   The system enforces various restrictions, such as continuous task permissions and concurrency control, to prevent apps from abusing system resources and to optimize the system audio experience. Failure to properly integrate continuous tasks or handle concurrent tasks may result in silent playback after the app moves to the background, the screen turns off, or the audio stream is interrupted by another audio stream.
...
1. Confirm the playback instance creation status.
   ...
   // Since API version 26.0.0, the audio renderer extension supports sample rates from 8000 Hz to 384000 Hz in 10 Hz increments. Supported sample rates vary by device.
   ...
   // Set the working scenario of the output audio stream.
...
3. If the `OH_AudioRenderer_OnWriteDataCallback` playback data callback is not received, check the callback registration and object lifecycle.
   ...
   ``` C++
   // Customize the data write function.
       ...
       int32_t audioDataSize)
   {
       ...
       // If you do not want to play a certain segment of audioData, return AUDIO_DATA_CALLBACK_RESULT_INVALID.
   ...
   // ...
       // Configure the audio data write callback.
...
6. If the issue is related to peripheral operations or concurrency scenarios, check the output device and concurrent playback policy.
   ...
   - Concurrency policy: Check whether other audio tasks exist on the current device. If silent playback occurs after your service runs concurrently with scenarios such as VoIP calls, cellular calls, alarms, or voice messages, refer to [Introduction to Audio Focus](audio-playback-concurrency.md) to understand the system-side audio policy, and also refer to [Handling Audio Focus Changes](audio-playback-concurrency.md#handling-audio-focus-changes) to ensure that focus callbacks are handled correctly.
