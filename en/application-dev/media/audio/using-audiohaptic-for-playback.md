...
<!-- md-trans-meta sourceCommit=afeedb9f2a6a6b573fefd666fa6085b567035af0 translatedAt=2026-09-18T03:42:16.976Z pushedAt=2026-09-18T10:19:10.798Z -->
...
1. Obtain an **AudioHapticManager** instance, and register the audio and haptic sources. A single app can register up to 128 sources at the same time. For details about the audio and haptic source formats supported by the player, see the [registerSource](../../reference/apis-audio-kit/js-apis-audioHaptic.md#registersource) documentation. You can register sources using the following two methods:
   ...
   // ...
     ...
     let audioUri = 'data/audioTest.wav'; // This is only an example. In actual use, replace the file with the URI of the target audio resource of the application.
     let hapticUri = 'data/hapticTest.json'; // This is only an example. In actual use, replace the file with the URI of the target haptic resource of the application.
     let idForUri = 0;
     ...
     // ...
     // Method 2: Register the resource by using the registerSourceFromFd API.
     // This is only an example. In actual use, replace the file with the corresponding file in the rawfile directory of the application.
