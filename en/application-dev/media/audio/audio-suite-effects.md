...
|---------|------|---------|---------|
| 0 | 31 | [-10, 10] | Ultra-low frequency, affecting the heavy bass effect. |
| 1 | 62 | [-10, 10] | Low frequency, affecting bass drums, bass, and so on. |
| 2 | 125 | [-10, 10] | Low-mid frequency, affecting male voices, guitars, and so on. |
| 3 | 250 | [-10, 10] | Mid frequency, affecting the main body of vocals and instruments. |
| 4 | 500 | [-10, 10] | High-mid frequency, affecting vocal clarity. |
| 5 | 1000 | [-10, 10] | High-mid frequency, affecting vocal brightness. |
...
|--------|------|------|
| SOUND_FIELD_FRONT_FACING = 1 | Front-facing sound field | Concentrates the sound in the front, creating a forward-focused listening experience. |
| SOUND_FIELD_GRAND = 2 | Grand sound field | Widens the sound field range, creating a grand and open sense of space. |
| SOUND_FIELD_NEAR = 3 | Near sound field | Shortens the listening distance, creating an immersive live-listening experience. |
| SOUND_FIELD_WIDE = 4 | Wide sound field | Expands the left-right sound field width, enhancing the lateral surround feeling. |
...
The voice beautification effect node [EFFECT_NODE_TYPE_VOICE_BEAUTIFIER](../../reference/apis-audio-kit/capi-native-audio-suite-base-h.md#oh_audionode_type) improves the audio quality and listening experience, delivering a better auditory experience to users.

### Beautification Types

The voice beautification effect node supports the following four beautification effect types, which are set through [OH_VoiceBeautifierType](../../reference/apis-audio-kit/capi-native-audio-suite-base-h.md#oh_voicebeautifiertype):
...
| VOICE_BEAUTIFIER_TYPE_THEATRE = 2 | Theatre effect | Creates a theatre-like sense of space and reverberation, suitable for scenarios such as drama and performances. |
...
``` C
// Set the node type to voice beautification.
OH_AudioSuiteNodeBuilder_SetNodeType(builder, OH_AudioNode_Type::EFFECT_NODE_TYPE_VOICE_BEAUTIFIER);
// Create a voice beautification node.
OH_AudioSuiteEngine_CreateNode(pipeline, builder, node);
// Set the voice beautification node effect.
...
| ENVIRONMENT_TYPE_EARPIECE = 2 | Earpiece | Simulates the narrowband listening experience of a telephone earpiece. |
| ENVIRONMENT_TYPE_UNDERWATER = 3 | Underwater | Simulates the muffled effect of sound propagating underwater. |
| ENVIRONMENT_TYPE_GRAMOPHONE = 4 | Gramophone | Simulates the vintage tone of an old gramophone. |
...
// Create an environmental effect node.
OH_AudioSuiteEngine_CreateNode(pipeline, builder, node);
...
| surroundTime | int32_t | [2, 40] | s | Time for one full revolution. |
| surroundDirection | [OH_AudioSuite_SurroundDirection](../../reference/apis-audio-kit/capi-native-audio-suite-base-h.md#oh_audiosuite_surrounddirection) | [0, 1] | - | Surround direction: 0 = counterclockwise (SPACE_RENDER_CCW), 1 = clockwise (SPACE_RENDER_CW). |
...
| Member | Type | Value Range | Unit | Description |
|------|------|---------|------|------|
| extRadius | float | [1.0, 5.0] | m | Extension radius. |
| extAngle | int32_t | (0, 360) | ° | Extension angle. |
...
// Create a spatial rendering node.
OH_AudioSuiteEngine_CreateNode(pipeline, builder, node);
// Set the spatial rendering parameters by scenario.
...
| GENERAL_VOICE_CHANGE_TYPE_SYNTH = 8 | Synthesizer | Synthesizer style voice changing. |
...
|------|------|---------|------|
| speed | float | [0.5, 10.0] | Speed parameter. 1.0 is the original speed; values less than 1.0 slow down, and values greater than 1.0 speed up. |
| pitch | float | [0.1, 5.0] | Pitch parameter. 1.0 is the original pitch; values less than 1.0 lower the pitch, and values greater than 1.0 raise the pitch. |
...
``` C
// Set the node type to pitch and speed change.
OH_AudioSuiteNodeBuilder_SetNodeType(builder, OH_AudioNode_Type::EFFECT_NODE_TYPE_TEMPO_PITCH);
// Create a pitch and speed change node.
OH_AudioSuiteEngine_CreateNode(pipeline, builder, node);
// Set the pitch and speed change node effect.
