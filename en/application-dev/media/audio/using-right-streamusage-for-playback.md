...
<!-- md-trans-meta sourceCommit=c36c28945696e4e51a7cd220b9ca90518d2530d1 translatedAt=2026-09-18T04:14:08.102Z pushedAt=2026-09-18T10:33:38.027Z -->
...
| Audio Stream Usage Type (StreamUsage) | Use Scenario |
| ---------- | ---------- |
| STREAM_USAGE_MUSIC<sup>10+</sup> | Applies to most media scenarios, mainly for playing music, and to scenarios where other stream types are not applicable, such as [Playing Short Audio with SoundPool (ArkTS)](../media/using-soundpool-for-playback.md). |
| STREAM_USAGE_MOVIE<sup>10+</sup> | Applies to playing various video content such as short videos, movies, and TV series. |
| STREAM_USAGE_AUDIOBOOK<sup>10+</sup> | Applies to playing audiobooks, news, podcasts, and the like. |
| STREAM_USAGE_GAME<sup>10+</sup> | Applies to in-game background music and voice-overs, where background music is not interrupted. For in-game voice, STREAM_USAGE_VOICE_COMMUNICATION is recommended. |
| STREAM_USAGE_NAVIGATION<sup>10+</sup> | Applies to voice announcements in navigation scenarios. |
| STREAM_USAGE_VOICE_MESSAGE<sup>10+</sup> | Applies to playing short voice messages. |
| STREAM_USAGE_VOICE_COMMUNICATION<sup>7+</sup> | Applies to two-way call scenarios such as VoIP voice calls and real-time in-game voice chat. |
| STREAM_USAGE_ALARM<sup>10+</sup> | Applies to playing alarms. |
| STREAM_USAGE_RINGTONE<sup>10+</sup> | Applies to VoIP incoming call ringtones and the like. |
| STREAM_USAGE_NOTIFICATION<sup>10+</sup> | Applies to playing notification sounds and alert tones. |
...
| Audio Stream Usage Type (StreamUsage) | Volume Type (AudioVolumeType) |
| ---------- | ---------- |
...
An audio channel is the transmission and processing channel of audio data within the system. The system obtains playback scenario information through `StreamUsage` and selects an appropriate audio channel based on parameters such as the sample rate, number of channels, and encoding format. Common audio channels are as follows:

| Audio Channel Type | Feature | Use Scenario |
| ---------- | ---------- | ---------- |
| Low-latency playback | Reduces latency by shortening buffering and minimizing audio effect algorithms. | Latency-sensitive scenarios such as game sound effects, karaoke ear return, and musical instrument performance. |
| Multichannel playback | Supports multichannel data output, avoiding downmixing multichannel audio to stereo or mono. | Multichannel playback scenarios. |
| Direct playback | Audio data is sent directly to the hardware output without changing the source format in the playback chain. | Playing high-quality lossless music, Hi-Fi audio, and the like. |
| Offload playback | Reduces interaction with hardware through a large buffer to lower power consumption. | Long-time playback of music, audiobooks, and the like. |
| Normal playback  | A general-purpose channel that outputs audio in the device's default audio format. | Video playback, notification sounds, navigation, and the like. |
...
| API | Description |
| ---------- | ---------- |
| [isFastPlaybackSupported](../../reference/apis-audio-kit/arkts-apis-audio-AudioStreamManager.md#isfastplaybacksupported) | Checks whether low-latency playback is supported.<br>**Since**: 26.0.0 |
| [isMultichannelPlaybackSupported](../../reference/apis-audio-kit/arkts-apis-audio-AudioStreamManager.md#ismultichannelplaybacksupported) | Checks whether multichannel playback is supported.<br>**Since**: 26.0.0 |
| [isDirectPlaybackSupported](../../reference/apis-audio-kit/arkts-apis-audio-AudioStreamManager.md#isdirectplaybacksupported) | Checks whether direct playback is supported.<br>**Since**: 26.0.0 |
| [isOffloadPlaybackSupported](../../reference/apis-audio-kit/arkts-apis-audio-AudioStreamManager.md#isoffloadplaybacksupported) | Checks whether offload playback is supported.<br>**Since**: 26.0.0 |
