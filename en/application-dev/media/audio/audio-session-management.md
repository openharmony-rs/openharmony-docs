...
<!-- md-trans-meta sourceCommit=5c8a6d5ce15406865f1f18928a4452d8d491334f translatedAt=2026-09-20T02:28:50.606Z pushedAt=2026-09-20T02:58:12.718Z -->
...
Audio session management provides two focus management approaches: [Independent Focus Management](#independent-focus-management) and [Unified Focus Management](#unified-focus-management). You can choose an appropriate focus management approach based on your own business requirements.
...
| Aspect | Independent Focus Management | Unified Focus Management |
|:---|:---|:---|
...
- [Independent focus management](#independent-focus-management): The system default focus strategy cannot meet the current app requirements, and only the focus strategy of the audio stream needs to be adjusted.
...
- [Unified focus management](#unified-focus-management): Multiple audio streams need to be started and the continuity of the playback process must be ensured, so as to prevent other apps' audio from resuming due to focus release when audio streams switch.
...
3. Listen for audio session deactivation events (optional).
   ...
   // ...
     ...
     // Monitor the audio session deactivation event.
...
// ...
  ...
  if (isActivated) {
    // After the audio session is activated, the application can perform audio playback, pause, stop, release, and other operations here.
  }
  // ...

  // Stop monitoring the audio session deactivation event.
...
> **NOTE**
>
> - The focus requested through AudioSession is application-level. If the app contains different modules, coordinate them properly to avoid a situation where one module requests focus through AudioSession while another module's audio stream is affected by AudioSession focus management, producing unexpected results.
> - Focus requested through AudioSession takes effect only on playback streams, and does not take effect on recording streams and some playback streams (such as STREAM_USAGE_ALARM, STREAM_USAGE_NOTIFICATION, and STREAM_USAGE_ACCESSIBILITY).
> - If AudioSessionScene is dynamically modified during AudioSession activation, call activateAudioSession again for the change to take effect.
> - After focus is requested through AudioSession, the focus is held by AudioSession. After the current playback scenario ends, you must proactively deactivate AudioSession to release the focus, so as to avoid abnormal focus holding caused by unreleased focus after the playback stream stops.
...
| AUDIO_SESSION_SCENE_VOICE_COMMUNICATION  | 2 | VoIP call audio session. |
...
[on('audioSessionStateChanged')](../../reference/apis-audio-kit/arkts-apis-audio-AudioSessionManager.md#onaudiosessionstatechanged20) includes the information of [AudioSession deactivation event](#audiosession-deactivation-event). When [unified focus management](#unified-focus-management) is used, you do not need to additionally listen for audio session deactivation events (`AudioSessionDeactivatedEvent`).

> **NOTE**
>
> If the app also registers focus event listeners for AudioRenderer, note the following two points:
> - The app receives callbacks for AudioSession focus state changes and AudioRenderer focus changes ([InterruptEvent](../../reference/apis-audio-kit/arkts-apis-audio-i.md#interruptevent9)). Handle these callbacks as needed.
> - When the AudioSession focus changes from paused to resumed, the system sends the focus resume event only to AudioSession, not to AudioRenderer. To resume playback, you must perform the following operations in order:
>    1. Call [setAudioSessionScene](../../reference/apis-audio-kit/arkts-apis-audio-AudioSessionManager.md#setaudiosessionscene20) to set the corresponding audio session scenario.
>    2. Call [activateAudioSession](../../reference/apis-audio-kit/arkts-apis-audio-AudioSessionManager.md#activateaudiosession12) to activate AudioSession.
...
1. Specify an audio session scene (AudioSessionScene) and strategy (AudioSessionStrategy) and activate the audio session.
   ...
   // ...
     ...
     // The application sets an appropriate audio session scenario based on its business scenario. When AudioSession is activated, the system requests the corresponding audio focus based on the selected audio session scenario.
...
3. Listen for AudioSession focus state change events.
   ...
   // ...
     ...
     // Monitor AudioSession focus and state change events.
     let audioSessionStateChangedCallback = (audioSessionStateChangedEvent: audio.AudioSessionStateChangedEvent) => {
       ...
       switch (audioSessionStateChangedEvent.stateChangeHint) {
         case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_PAUSE:
           // This branch indicates that the system has paused the audio stream, and the application must switch to the audio paused state.
           // Temporarily loses focus: AudioSession is deactivated and releases the focus, and playback of all audio streams of the application stops. Therefore, after the application receives the Resume callback, it must reactivate AudioSession and resume the audio streams that need to continue playing.
           break;
         case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_RESUME:
           // This branch indicates that the system releases the pause operation on the AudioSession focus.
           break;
         case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_STOP:
           // This branch indicates that the system has stopped the audio stream (permanently loses focus). To keep the state consistent, the application must switch to the audio paused state.
           // Permanently loses focus: AudioSession is deactivated and releases the focus, and playback of all audio streams of the application stops. No further audio focus events will be received, and resuming playback requires the user to trigger it proactively.
           break;
         case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_TIME_OUT_STOP:
           // This branch indicates that because no audio stream has been playing for a long time, the system has stopped AudioSession (permanently loses focus), and the application must switch to the audio stopped state.
           // Permanently loses focus: No further audio focus events will be received, and resuming playback requires the user to trigger it proactively.
           break;
         case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_DUCK:
           // This branch indicates that the system has lowered the volume of all playing audio streams of the application (to 20% of the normal volume by default).
           break;
         case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_UNDUCK:
           // This branch indicates that the system has restored the volume of all playing audio streams of the application to normal.
         case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_MUTE_SUGGESTION:
           // This branch indicates that another application starts playing non-mixing audio, and the system can decide whether to mute.
           break;
         case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_UNMUTE_SUGGESTION:
           // This branch indicates that the non-mixing audio playback of another application ends, and the system can decide whether to unmute.
           break;
         case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_MUTE:
           // This branch indicates that the system has muted all playing audio streams of the application.
           break;
         case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_UNMUTE:
           // This branch indicates that the system has unmuted all playing audio streams of the application.
           break;
         default:
           break;
       }
...
4. Deactivate the audio session.
   ...
   // ...
     ...
     // Deactivate AudioSession, that is, release the focus and deactivate all audio streams currently playing in the application.
     audioSessionManager.deactivateAudioSession().then(() => {
       console.info('Succeeded in deactivating audio session.');
       // ...
     }).catch((err: BusinessError) => {
...
// ...
  ...
  // The application sets an audio session scenario suitable for its own business scenario. When AudioSession is activated, the system requests the corresponding audio focus based on the selected audio session scenario.
  audioSessionManager.setAudioSessionScene(audio.AudioSessionScene.AUDIO_SESSION_SCENE_MEDIA);
  // ...
  ...
  let audioSessionStateChangedCallback = (audioSessionStateChangedEvent: audio.AudioSessionStateChangedEvent) => {
    ...
    switch (audioSessionStateChangedEvent.stateChangeHint) {
      case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_PAUSE:
        // This branch indicates that the system has paused the audio stream, and the application must switch to the audio paused state.
        // Temporary focus loss: AudioSession is deactivated and releases the focus, and playback of all audio streams of the application is stopped. Therefore, after the application receives the Resume callback, it must reactivate AudioSession and resume the audio streams that need to continue playing.
        break;
      case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_RESUME:
        // This branch indicates that the system releases the pause operation on the AudioSession focus.
        break;
      case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_STOP:
        // This branch indicates that the system has stopped the audio stream (permanent focus loss). To keep the state consistent, the application must switch to the audio paused state.
        // Permanent focus loss: AudioSession is deactivated and releases the focus, and playback of all audio streams of the application is stopped. No further audio focus events will be received, and resuming playback requires the user to trigger it proactively.
        break;
      case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_TIME_OUT_STOP:
        // This branch indicates that the system has stopped AudioSession (permanent focus loss) because no audio stream has been playing for a long time, and the application must switch to the audio stopped state.
        // Permanent focus loss: No further audio focus events will be received, and resuming playback requires the user to trigger it proactively.
        break;
      case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_DUCK:
        // This branch indicates that the system has lowered the volume of all playing audio streams of the application (to 20% of the normal volume by default).
        break;
      case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_UNDUCK:
        // This branch indicates that the system has restored the volume of all playing audio streams of the application to normal.
      case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_MUTE_SUGGESTION:
        // This branch indicates that another application starts playing non-mixing audio, and the system can decide whether to mute it.
        break;
      case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_UNMUTE_SUGGESTION:
        // This branch indicates that the non-mixing audio playback of another application ends, and the system can decide whether to unmute it.
        break;
      case audio.AudioSessionStateChangeHint.AUDIO_SESSION_STATE_CHANGE_HINT_MUTE:
        // This branch indicates that the system has muted all playing audio streams of the application.
        break;
  ...
  if (isActivated) {
    // After the audio session is activated, the application can perform audio playback, pause, stop, release, and other operations here normally.
    // Based on the actual business, the application can start multiple AudioRenderer audio playback streams. The audio playback streams started here no longer hold the focus and are managed by AudioSession.
    // If multiple audio streams are playing simultaneously, pay special attention to the timing of AudioSession deactivation (deactivating AudioSession also releases all audio playback streams of the application).
  }
  // ...

  // When the business ends, stop listening for AudioSession focus and state change events.
  audioSessionManager.off('audioSessionStateChanged');
  // ...

  // Deactivate AudioSession, that is, release the focus and deactivate all audio streams currently being played by the application.
  audioSessionManager.deactivateAudioSession().then(() => {
    console.info('Succeeded in deactivating audio session.');
    // ...
  }).catch((err: BusinessError) => {
...
This API can only be called when there is a running recording stream in the current audio session. Otherwise, error code `6800103` is returned. If a recording stream has both the stream-level mute hint API [AudioCapturer.setMuteHint](../../reference/apis-audio-kit/arkts-apis-audio-AudioCapturer.md#setmutehint24) and the session-level mute hint API called, the stream-level setting takes precedence. Therefore, when the mute states of multiple recording streams in the app are consistent, you can use the session-level API for unified reporting. When the mute states of different recording streams are inconsistent, use the stream-level API for specific recording streams. If you create a Mic audio source recording stream to call the session-level API, you need to request the microphone permission `ohos.permission.MICROPHONE`. No system query API is currently provided. If you need to display the mute hint state on the UI, you must maintain the most recently set state yourself. In the following example, `muteHint` set to `true` indicates reporting a mute hint, and `false` indicates canceling the mute hint.
...
// ...
  ...
  // Set an audio session scenario that suits your business scenario. When AudioSession is activated, the system requests the corresponding audio focus based on the selected audio session scenario.
  audioSessionManager.setAudioSessionScene(audio.AudioSessionScene.AUDIO_SESSION_SCENE_MEDIA);
