...
<!-- md-trans-meta sourceCommit=99c0f65ab11e80829c9fe89a70bd5d019336366b translatedAt=2026-09-20T02:43:38.532Z pushedAt=2026-09-20T03:14:00.486Z -->
...
This document describes the system's audio focus strategy and how to respond to focus changes. The system also provides an [audio session management](audio-session-management.md) mechanism, allowing apps to customize the focus policy for their audio streams.
...
It is recommended that the app proactively [handles audio focus changes](#handling-audio-focus-changes) by listening for audio focus events. If the audio focus request is rejected, the app receives an audio focus event ([InterruptEvent](../../reference/apis-audio-kit/arkts-apis-audio-i.md#interruptevent9)).
...
2. **Silent playback**: If an application starts to play audio (or video) in mute mode and intends to avoid affecting other audio during the mute phase, and then seeks audio focus with a standard strategy when unmuting, it can call APIs related to the silent concurrent playback mode. For details, see:
   ...
   When app A plays a media stream first and app B then starts a media silent stream without setting the concurrent mode described above, app B interrupts the media audio playback of app A, affecting the user experience. For this scenario, the following optimization is available since API version 26.0.0:

   When app A and app B play media streams whose [StreamUsage](../../reference/apis-audio-kit/arkts-apis-audio-e.md#streamusage) is Music, Movie, or AudioBook, application B does not immediately interrupt app A. Whether an interruption occurs is determined by the following conditions:
   - Condition 1: When app B is silent throughout, it does not interrupt the playback of app A.
   - Condition 2: App B interrupts the playback of app A only when its media stream changes to non-silent data and the volume is greater than 0.
...
If the default audio focus strategy cannot meet the requirements of specific scenarios, the app can use [audio session management](audio-session-management.md) to adjust the audio focus strategy used by its audio streams.
    ...
    if (interruptEvent.forceType === audio.InterruptForceType.INTERRUPT_FORCE) {
      ...
      switch (interruptEvent.hintType) {
        case audio.InterruptHint.INTERRUPT_HINT_PAUSE:
          ...
          isPlaying = false; // This statement is simplified and represents the operations for switching the application to the audio paused state.
          break;
      ...
      switch (interruptEvent.hintType) {
        case audio.InterruptHint.INTERRUPT_HINT_RESUME:
          // The paused audio stream can be played. It is recommended that the application continue to play the audio stream and switch to the audio playback state.
          ...
          // If calling start() is successful, the application needs to switch to the audio playback state.
          if (started) {
            isPlaying = true; // This statement is simplified and represents the operations for switching the application to the audio playback state.
          } else {
...
| Focus Mode | Description | Specifications |
|---------|----------|-------------------|
| SHARE_MODE (default, recommended) | **Shared focus mode**, in which the system does not make focus decisions and the application decides the playback, pause, ducking, and other operations of each stream on its own. | No focus decision is made between this stream and other SHARE_MODE streams within the same application; the application manages them on its own.<br>Focus decisions are made by the system between this stream and INDEPENDENT_MODE streams within the same application. |
| INDEPENDENT_MODE | **Independent focus mode**, in which the system makes focus decisions and manages streams according to the inter-application focus policy. | Focus decisions are made by the system together with all streams within the same application (whether SHARE_MODE or INDEPENDENT_MODE). |
...
async toggleStreamB() { // Toggle the playback state of stream B.
  await this.initStreams(); // Lazily initialize the two audio streams. Create the Renderer only on the first playback.
  if (this.streamBPlaying) { // Stream B is playing, and the user wants to stop it.
    // ...
    if (this.streamAPausedByApp) { // Stream A was paused by the application. After stream B stops, stream A should resume automatically.
      this.streamAPausedByApp = false; // Clear the flag. Stream A is about to resume and is no longer in the application-managed paused state.
      await this.streamA.startPlay(); // Resume the audio output of stream A to complete the pause-to-resume transition.
      this.promptAction.showToast({
        message: $r('app.string.app_control_stream_a_auto_resume'),
        duration: TOAST_DURATION
      });
    }
  } else { // Stream B is not playing, and the user wants to start it.
    if (this.appControl && this.currentMode === InterruptMode.SHARE_MODE && 
    this.streamAPlaying) { // Stream A is playing. Pause stream A.
      await this.streamA.pausePlay(); // Pause stream A proactively so that it can be resumed later, instead of letting the system stop it and make it unrecoverable.
      this.streamAPausedByApp = true; // Mark stream A as paused by the application. After stream B stops, stream A must resume automatically.
