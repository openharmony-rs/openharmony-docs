...
| Audio Stream Type (SourceType) | Applicable Scenario | Main Effect |
| ---------- | ---------- | ---------- |
| SOURCE_TYPE_MIC | Normal recording, such as a recorder. | The system configures the default input path and recording processing according to the normal recording scenario. If a more specific service type exists, preferentially select the corresponding type. |
...
| Audio Channel Type | Characteristic | Applicable Scenario |
| ---------- | ---------- | ---------- |
| Low-latency recording | Reduces latency by shortening the buffer and minimizing audio effect algorithms. | Karaoke |
| Normal recording | General-purpose channel that inputs audio in the current device's default audio format. | Recorder, camera recording, voice messages, etc. |
...
> **NOTE**
>
> - Support for audio channels depends on the device's hardware capabilities and system configuration, and varies by device.
> - The result returned by the query API only indicates whether the system supports the corresponding audio channel under the specified parameters. The channel actually used must also be determined in combination with the system's current running state and resource configuration.
