...
Audio template controller (`AVMusicTemplateController`): A class that encapsulates interactions between the Media Controller and a media application. It includes properties such as the media session ID (`sessionId`) and whether the controller has been destroyed (`isDestroy`), and provides methods for exchanging data with the media application. For media applications that integrate with the audio template, the Media Controller uses this class to obtain media data for UI presentation and send user commands to the media application.
...
1. Create an audio template controller for each media application that integrates with the audio template. Each media application requires only one controller instance. Listen for audio template creation and destruction events to create and destroy the corresponding controller as needed. The following APIs are required:
   ...
   - [getAllAVMusicTemplateDescriptors](../../reference/apis-avsession-kit/js-apis-avMusicTemplate-sys.md#avmusictemplategetallavmusictemplatedescriptors): Obtains all audio template descriptors. Since a media application may create an audio template before the Media Controller starts, the corresponding `sessionId` might not be available through the creation event. After startup, call this API to obtain existing `sessionId` values.
   ...
   export class ControllerManager {
     ...
     public unregisterListener() {
       ...
       // Unregister the listener for notifying the Media Controller to launch the specified third-party application interface.
