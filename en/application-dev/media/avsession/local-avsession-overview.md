...
2. The provider sets session metadata (such as media ID, title, and media duration) and session playback properties (such as playback state, playback speed, and playback position) through the AVSession object.

3. The controller creates an AVSessionController object through AVSessionManager.

4. Through the AVSessionController object, the controller listens for changes of the session metadata and playback properties.
