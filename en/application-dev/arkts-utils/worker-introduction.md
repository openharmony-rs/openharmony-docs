...
4. Register the callback functions in the Worker thread file.
      ...
      // Receive the message returned by the child Worker.
      workerPort.onmessage = (e: MessageEvents) => {
        let data: string = e.data;
        console.info('workerPort onmessage is: ', data);

        // Send a message to the main thread after the child Worker task is complete.
...
// Receive the message returned by the parent Worker.
parentWorker.onmessage = (e: MessageEvents) => {
  console.info('The host thread receives a message from the parent Worker' + e.data);
}

// Callback invoked when the parent Worker exits normally.
parentWorker.onexit = () => {
  console.info('The parent Worker exits');
}

// Callback invoked when an uncaught exception or running error occurs during the running of the parent Worker.
...
// Send a startup message to the parent Worker to trigger the processing logic in onmessage.
...
// Object for the parent Worker thread to communicate with the host main thread.
const workerPort: ThreadWorkerGlobalScope = worker.workerPort;

workerPort.onmessage = (e : MessageEvents) => {
  // Create a child Worker after receiving the instruction from the host main thread.
  if (e.data == 'The host thread sends a message to the parent Worker - recommended example') {
    ...
    // Receive the execution result of the child Worker.
    childWorker.onmessage = (e: MessageEvents) => {
      ...
      if (e.data == 'The child Worker sends information to the parent Worker') {
        // Notify the host thread after the child Worker task is complete.
    ...
    // Callback when an uncaught exception or running error occurs during the running of the child Worker.
    childWorker.onAllErrors = (err: ErrorEvent) => {
      console.error('An error occurred on the child Worker' + err.message);
    }

    // Send a startup message to the child Worker to trigger the processing logic in onmessage.
...
// Object for the child Worker to communicate with the parent Worker.
const workerPort: ThreadWorkerGlobalScope = worker.workerPort;
...
workerPort.onmessage = (e: MessageEvents) => {
  if (e.data == 'The parent Worker sends a message to the child Worker - recommended example') {
    ...
    // Exit after the child Worker task is complete.
    workerPort.close();
  }
}
```
...
workerPort.onmessage = (e : MessageEvents) => {
  ...
  // Callback invoked when the child Worker exits normally.
  childWorker.onexit = () => {
    console.info('The child Worker exits');
    // Send a message through the parent Worker port when the parent Worker exits or is about to exit.
    workerPort.postMessage('The parent Worker sends a message to the host thread');
  }
...
workerPort.onmessage = (e: MessageEvents) => {
  ...
  // The child Worker sends a message to the parent Worker after the parent Worker is destroyed.
...
workerPort.onmessage = (e : MessageEvents) => {
  ...
  // Create a child Worker after the parent Worker is destroyed.
  workerPort.close();
  let childWorker = new worker.ThreadWorker('entry/ets/workers/ChildWorker.ets');

  // Destroy the parent Worker before it is confirmed that the child Worker is successfully created.
  // let childWorker = new worker.ThreadWorker('entry/ets/workers/ChildWorker.ets');
  // workerPort.close();
  ...
  // Send a startup message to the child Worker.
  childWorker.postMessage('The parent Worker sends a message to the child Worker');
}
```
