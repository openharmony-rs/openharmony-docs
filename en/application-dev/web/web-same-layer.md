...
<!--Tester: @ghiker-->
<!--Adviser: @HelloShuo-->
<!-- md-trans-meta sourceCommit=bcbbbedf2234f156fe3ad4387f2cc76834ae7941 translatedAt=2026-09-22T09:26:42.756Z pushedAt=2026-09-22T11:34:45.678Z -->
...
## When to Use
### On the Web Page
To improve the performance of an mini-program, you can use the ArkUI **XComponent** component to render the map component, and use the ArkUI **TextInput** component to render the input box component to achieve an input experience consistent with system applications.
...
- On the application side, you can use the same-layer rendering event reporting API of the **Web** component to detect the lifecycle and input events of the HTML5 same-layer tags, and process the service logic of the same-layer rendered components.

- On the application side, you can use ArkUI APIs such as **NodeContainer** to construct same-layer rendered components corresponding to HTML5 same-layer tags. Common ArkUI components that support same-layer rendering include: [TextInput](../reference/apis-arkui/arkui-ts/ts-basic-components-textinput.md), [XComponent](../reference/apis-arkui/arkui-ts/ts-basic-components-xcomponent.md), [Canvas](../reference/apis-arkui/arkui-ts/ts-components-canvas-canvas.md), [Video](../reference/apis-arkui/arkui-ts/ts-media-components-video.md), [Web](../reference/apis-arkweb/arkts-basic-components-web.md). For details, see [Specifications and Constraints](#specifications-and-constraints).
...
- Imperative custom drawing nodes: [BuilderNode](../reference/apis-arkui/js-apis-arkui-builderNode.md), [ComponentContent](../reference/apis-arkui/js-apis-arkui-ComponentContent.md), [ContentSlot](../reference/apis-arkui/arkui-ts/ts-components-contentSlot.md), [FrameNode](../reference/apis-arkui/js-apis-arkui-frameNode.md), [Graphics](../reference/apis-arkui/js-apis-arkui-graphics.md), [NodeController](../reference/apis-arkui/js-apis-arkui-nodeController.md), [RenderNode](../reference/apis-arkui/js-apis-arkui-renderNode.md), [XComponentNode](../reference/apis-arkui/js-apis-arkui-xcomponentNode.md), [AttributeUpdater](../reference/apis-arkui/js-apis-arkui-AttributeUpdater.md), [ArkUI_NativeModule](../reference/apis-arkui/capi-arkui-nativemodule.md) (the range of components supporting same-layer rendering is the same as that of ArkTS)

**Supported Common Component Attributes and Events**:

- Common attributes that are not supported: [restoreId](../reference/apis-arkui/arkui-ts/ts-universal-attributes-restoreId.md) and [useEffect](../reference/apis-arkui/arkui-ts/ts-universal-attributes-use-effect.md).
...
If an application needs to use same-layer rendering on a web page loaded by the **Web** component, specify the **\<embed>** and **\<object>** tags on the web page as same-layer rendered components based on the following specifications.
...
- Creation, destruction, and changes in position, width, and height are supported.

- Web pages containing same-layer components support back-forward cache.

**Distribution and Processing of the Input Events**:
- Supports the DOWN/UP/MOVE/CANCEL events of TouchEvent. Supports [onNativeEmbedGestureEvent](../reference/apis-arkweb/arkts-basic-components-web-events.md#onnativeembedgestureevent11) (to configure the consumption result of touch events), which is consumed on the application side by default.
...
- By default, the mouse and touchpad left button events (**MousePress**/**MouseRelease**/**MouseMOVE**) are converted into touch events (**TouchDOWN**/**TouchUP**/**TouchMOVE**) for reporting.
...
- The number of same-layer tags on a web page should be kept within five. Exceeding five will degrade rendering performance.
...
- For ArkUI components that involve interface interaction (such as TextInput), you are advised to use a **Stack** component to wrap the same-layer component container and **BuilderNode** and ensure that they are in the same position. In addition, the **NodeContainer** must be aligned with the **\<embed>**/**\<object>** tag to ensure proper component interaction. If the positions of the two components are different, the following problems may occur: The position of the text selection box attached to the **TextInput**/**TextArea** component is incorrect (as shown in the following figure), and the animation start and stop of the **LoadingProgress**/**Marquee** component do not match the visibility status of the component.
...
1. Mark the HTML tags that need to be rendered at the same layer on the web page.
   ...
   - Use the \<object> tag.
     ...
     The frontend page code corresponding to registerNativeEmbedRule, where the type can be "test" or a string prefixed with "test".
...
2. Enable same-layer rendering on the application side.
...
3. Create a custom component.
   ...
   The system component is displayed in the corresponding area after same-layer rendering is enabled.
   ...
   // The specific component content of the dynamic component is in @Builder.
   @Builder
...
4. Create a node controller.
   ...
   ``` TypeScript
   // Used to control and feed back the behavior of the node on the corresponding NodeContainer. Must be used together with NodeContainer.
   class MyNodeController extends NodeController {
     ...
     // A method that must be overridden. Used to build the node tree and return the node tree to be mounted in the corresponding NodeContainer.
...
5. Listen for the lifecycle changes of the same-layer rendered tags.
   ...
   After this feature is enabled, the ArkWeb kernel triggers the callback registered by [onNativeEmbedLifecycleChange](../reference/apis-arkweb/arkts-basic-components-web-events.md#onnativeembedlifecyclechange11) when a tag supported by same-layer rendering exists on the web page.
   ...
   .onNativeEmbedLifecycleChange((embed) => {
     ...
     if (embed.status === NativeEmbedStatus.CREATE) {
       ...
       // Use the id attribute of the embed passed from Web as the key to store the nodeController in the Map.
       ...
       // Store the id attribute of the embed passed from Web in the @State state array variable for dynamically creating the nodeContainer node container. The push action must be placed after set.
...
7. Listen for mouse events of the same-layer rendered region.
   ...
   .onNativeEmbedMouseEvent((mouse) => {
     ...
     this.componentIdArr.forEach((componentId: string) => {
       let nodeController = this.nodeControllerMap.get(componentId);
       // Send the events obtained from the same-layer area to the nodeController corresponding to the embedId of the area.
      ...
      Row() {
        Column() {
          Stack() {
            ...
            Web({src: $rawfile('test2.html'), controller: this.browserTabController})
              // Register the same-layer tag as 'object' with the type prefixed by 'test'.
              .registerNativeEmbedRule('object', 'test')
              // Enable same-layer rendering.
              .enableNativeEmbedMode(true)
              // Obtain the lifecycle change data of the embed tag.
              .onNativeEmbedLifecycleChange((embed) => {
                console.info('NativeEmbed surfaceId' + embed.surfaceId);
                // If embed.info.id is used as the key for mapping the nodeController, explicitly specify the id in the H5 page.
                const componentId = embed.info?.id?.toString() as string;
                if (embed.status === NativeEmbedStatus.CREATE) {
                  console.info('NativeEmbed create' + JSON.stringify(embed.info));
                  // Create the node controller and set the parameters.
                  let nodeController = new MyNodeController();
                  // The units of embed.info.width and embed.info.height are px, which need to be converted to the default unit vp on the ets side.
                  nodeController.setRenderOption({surfaceId : embed.surfaceId as string,
                    type : embed.info?.type as string,
                    renderType : NodeRenderType.RENDER_TYPE_TEXTURE,
                    embedId : embed.embedId as string,
                  ...
                  nodeController.setDestroy(false);
                  // Use the id attribute of the embed passed from the Web as the key to store the nodeController in the Map.
                  this.nodeControllerMap.set(componentId, nodeController);
                  this.widthMap.set(componentId, this.uiContext.px2vp(embed.info?.width));
                  this.heightMap.set(componentId, this.uiContext.px2vp(embed.info?.height));
                  this.positionMap.set(componentId, this.edges);
                  // Store the id attribute of the embed passed from the Web in the @State state array variable for dynamically creating the nodeContainer node container. The push action needs to be placed after set.
                  this.componentIdArr.push(componentId);
                } else if (embed.status === NativeEmbedStatus.UPDATE) {
                  let nodeController = this.nodeControllerMap.get(componentId);
                  console.info('NativeEmbed update' + JSON.stringify(embed));
              ...
              // Obtain the touch event information of the same-layer rendered component.
              .onNativeEmbedGestureEvent((touch) => {
                console.info('NativeEmbed onNativeEmbedGestureEvent' + JSON.stringify(touch.touchEvent));
                this.componentIdArr.forEach((componentId: string) => {
                  let nodeController = this.nodeControllerMap.get(componentId);
                  // Send the obtained event of the same-layer area to the nodeController corresponding to embedId of the region.
                  if (nodeController?.getEmbedId() === touch.embedId) {
                    let ret = nodeController?.postEvent(touch.touchEvent);
                    if (ret) {
                      console.info('onNativeEmbedGestureEvent success ' + componentId);
            ...
            Web({ src: $rawfile('test3.html'), controller: this.browserTabController })
              ...
              .onNativeEmbedLifecycleChange((embed) => {
                ...
                if (embed.status === NativeEmbedStatus.CREATE) {
                  ...
                  this.heightMap.set(componentId,  this.uiContext.px2vp(embed.info?.height));
                  this.positionMap.set(componentId, this.edges);
                  // Store the id attribute of the embed passed in by Web into the @State state array variable for dynamically creating the NodeContainer node container. The push action needs to be placed after set.
                  this.componentIdArr.push(componentId);
                } else if (embed.status === NativeEmbedStatus.UPDATE) {
                  let nodeController = this.nodeControllerMap.get(componentId);
                  this.edges = {left: `${embed.info?.position?.x as number}px`, top: `${embed.info?.position?.y as number}px`};
...
- Code example of video playback on the application side. Replace the URL with the correct video URL in practice.
  ...
  export class AVPlayerDemo {
    ...
    private surfaceId: string = ''; // surfaceId is used for playback display. The specific value must be obtained through the XComponent API. For related documentation, see the XComponent creation method above.
      ...
      avPlayer.on('stateChange', async (state: string, reason: media.StateChangeReason) => {
        switch (state) {
          ...
          case 'initialized': // This state is reported when the AVPlayer sets the playback source.
            ...
            avPlayer.surfaceId = this.surfaceId; // Set the display surface. This is not required when the played resource is pure audio.
            avPlayer.prepare();
            break;
    ...
    async avPlayerLiveDemo(){
      try {
        ...
        // Replace the video link as needed when using it.
...
// Used to control and feed back the behavior of the node on the corresponding NodeContainer. It must be used together with NodeContainer.
class MyNodeController extends NodeController {
  private rootNode: BuilderNode<[Params]> | undefined | null;
  private embedId_: string = '';
  private surfaceId_: string = '';
  ...
  // Called when the corresponding NodeContainer is created, or called through the rebuild method to refresh.
  makeNode(uiContext: UIContext): FrameNode | null {
    if (this.isDestroy_) { // rootNode is null.
      return null;
    }
...
// In @Builder, this is the specific component content of the dynamic component.
@Builder
          ...
          // Enable the same-layer rendering switch.
            ...
            .onNativeEmbedLifecycleChange((embed) => {
              ...
              if (embed.status === NativeEmbedStatus.CREATE) {
                ...
                // Create the node controller and set parameters.
                let nodeController = new MyNodeController();
                // The units of embed.info.width and embed.info.height are px. They need to be converted to vp, the default unit on the ets side.
                nodeController.setRenderOption({surfaceId : embed.surfaceId as string,
                  type : embed.info?.type as string,
                  renderType : NodeRenderType.RENDER_TYPE_TEXTURE,
                  embedId : embed.embedId as string,
            ...
            .onNativeEmbedGestureEvent((touch) => {
              ...
              this.componentIdArr.forEach((componentId: string) => {
                let nodeController = this.nodeControllerMap.get(componentId);
                // Send the event of the obtained same-layer area to the nodeController corresponding to the embedId of this area.
                if (nodeController?.getEmbedId() === touch.embedId) {
                  let ret = nodeController?.postEvent(touch.touchEvent);
                  if (ret) {
                    console.info('onNativeEmbedGestureEvent success ' + componentId);
...
| object-fit:none | The texture is not stretched and aligned with the top-left corner.|
            ...
            Web({src: $rawfile('test.html'), controller: this.browserTabController})
              ...
              .onNativeEmbedLifecycleChange((embed) => {
                ...
                if (embed.status == NativeEmbedStatus.CREATE) {
                  ...
                  this.heightMap.set(componentId, this.uiContext.px2vp(embed.info?.height));
                  this.positionMap.set(componentId, this.edges);
                  // Store the id attribute of the embed passed in by Web into the @State state array variable for dynamically creating the nodeContainer node container. The push action must be placed after set.
                  this.componentIdArr.push(componentId)
                } else if (embed.status == NativeEmbedStatus.UPDATE) {
                  let nodeController = this.nodeControllerMap.get(componentId);
                  console.info('NativeEmbed update' + JSON.stringify(embed));
...
- The component height is too high.
  ...
  Due to GPU limitations, same-layer tags have a height limit of 8000px. If the height of a same-layer tag in H5 is too large, the component may be stretched. In this case, set the height of the same-layer tag to below 8000px.
...
### How do I pass events captured by a same-layer rendering component through to the web frontend?
