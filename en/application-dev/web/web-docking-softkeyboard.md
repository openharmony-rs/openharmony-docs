...
To improve user experience, the input box can automatically obtain the focus and display the soft keyboard after the page is loaded. You can call showTextInput() to set the automatic display of the soft keyboard.
...
1. Set the soft keyboard avoidance mode [setKeyboardAvoidMode()](../reference/apis-arkui/arkui-ts/ts-universal-attributes-expand-safe-area.md#setkeyboardavoidmode11) of UIContext in the application code. ArkWeb supports two modes: Resize and Offset.
...
| Offscreen component creation      | By default, the soft keyboard avoidance mode is the same as that for non-offscreen component creation. You can set another avoidance mode before adding the component to the component tree.  |
| customDialog | The **customDialog** component avoids the keyboard by itself.                       |
| Foldable device         | The soft keyboard avoidance behavior is the same as that in common scenarios. The soft keyboard on the screen is adjusted according to the screen opening and closing status.   |
...
When the [onInterceptKeyboardAttach](../reference/apis-arkweb/arkts-basic-components-web-events.md#oninterceptkeyboardattach12) tag is called, the [onInterceptKeyboardAttach](../reference/apis-arkweb/arkts-basic-components-web-events.md#oninterceptkeyboardattach12) callback is triggered. An application can use this API to control the display of the soft keyboard, including the default system soft keyboard, soft keyboard with a specific Enter key, or completely custom soft keyboard. and use any of the following options:
