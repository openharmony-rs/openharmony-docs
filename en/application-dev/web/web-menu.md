# Using Menus to Process Web Content
<!--Kit: ArkWeb-->
<!--Subsystem: Web-->
<!--Owner: @runlei-->
<!--Designer: @shulssins-->
<!--Tester: @ghiker-->
<!--Adviser: @HelloShuo-->
<!-- md-trans-meta sourceCommit=8639a0d43b773b87dfaf606ea59ba5a24af79a51 translatedAt=2026-09-21T02:19:19.037Z pushedAt=2026-09-22T01:23:15.243Z -->
...
struct WebComponent {
  ...
  onCreateMenu(menuItems: Array<TextMenuItem>): Array<TextMenuItem> {
    ...
    items.push(customItem1); // Add a new option to the end of the option list.
  ...
  onMenuItemClick(menuItem: TextMenuItem, textRange: TextRange): boolean {
    if (menuItem.id.equals(TextMenuItemId.CUT)) {
      ...
      return true; // For user-defined menu options, returning true keeps the menu open after the tap, and returning false closes the menu.
...
struct WebComponent {
  ...
  @Builder
  // Build the custom menu and the API for triggering its functions.
  MenuBuilder() {
...
<body>
  ...
  <p>Select text and right-click to open the menu</p>
</body>
</html>
```
...
After `onContextMenuShow` is triggered, the application needs to end the current menu operation. In the preceding example, the context menu result object is obtained through `event.result` in the `onContextMenuShow` callback and saved to `this.result`. Then, in the `onStateChange` callback of `bindPopup`, the popup closing is monitored, and `this.result!.closeContextMenu()` is called to close the Web context menu. Merely setting the popup state to hidden cannot replace calling [closeContextMenu](../reference/apis-arkweb/arkts-basic-components-web-WebContextMenuResult.md#closecontextmenu9). If the callback is triggered only once when you repeatedly long-press the image, first check whether this API was called when the previous menu was closed. If the menu has already been closed, check whether the web page has intercepted the `contextmenu` event.
...
struct SelectionMenuLongPress {
  ...
  @Builder PreviewBuilder($: PreviewBuilderParam){
    Column() {
      Stack(){
        Text('') // You can choose whether to display the URL.
          .padding(5)
          .width('100%')
      ...
      Web({src:$.url,controller: new webview.WebviewController()})
        ...
        .hitTestBehavior(HitTestMode.None) // Make the preview Web not respond to gestures.
...
<footer>
    <p>Note that all the URLs provided above are for demonstration purposes only.</p>
</footer>
</body>
</html>
```
...
struct WebComponent {
  ...
  onCreateMenu(menuItems: Array<TextMenuItem>): Array<TextMenuItem> {
    let items = menuItems.filter((menuItem) => {
      // Filter the system menu items that you need.
      return false;
    });
    return items;
  }
...
<div class="container">
    <div class="context">
        <span>In this digital age, text copying has become increasingly important. Whether quoting a famous saying, saving important information, or sharing interesting content, copying text is part of our daily operations.</span>
    </div>
</div>
