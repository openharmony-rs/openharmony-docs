...
<!--Tester: @ghiker-->
<!--Adviser: @HelloShuo-->
<!-- md-trans-meta sourceCommit=a40d54eb841d3251f24cd251233d70758a16c2e7 translatedAt=2026-09-21T01:56:24.481Z pushedAt=2026-09-21T09:18:24.256Z -->
...
In the following example, a Web component is created in an imperative manner when the main window Ability starts. You can use the methods and classes provided in common.ets to attach and detach the Web component. Index.ets provides an implementation for attaching and detaching the Web component. In this way, you can attach and detach the Web component on pages in different windows, that is, migrate the Web component between different windows. The following figure shows the migration process.
...
// Data is an encapsulation class for input parameters.
class Data{
  public url: string = '';    
...
// @Builder contains the specific component content of the dynamic component.
@Builder
...
// Used to control and provide feedback on the behavior of the node in the corresponding NodeContainer. It must be used together with NodeContainer.
export class MyNodeController extends NodeController {
  ...
  // A method that must be overridden, used to build the node tree and return the node to be attached in the corresponding NodeContainer.
  // Called when the corresponding NodeContainer is created or when rebuild is called to refresh the node tree.
  ...
  attachWeb() : void {
    if (this.builderNode) {
      ...
      if (frameNode?.getParent() != null) {
        // Before attaching the custom node, check whether the node has already been attached.
...
// Creates a Map to store the required BuilderNodes.
let builderNodeMap : Map<string, BuilderNode<[Data]> | undefined> = new Map();
// Creates a Map to store the required webview.WebviewControllers.
let webControllerMap : Map<string, webview.WebviewController | undefined> = new Map();

// Initialization requires a UIContext object, which can be obtained by calling getUIContext() on a window or custom component.
