...
<!--Tester: @ghiker-->
<!--Adviser: @HelloShuo-->
<!-- md-trans-meta sourceCommit=f18d113dd90e7361c74b830ab39cc318edd40714 translatedAt=2026-09-21T02:25:50.577Z pushedAt=2026-09-21T13:45:28.496Z -->
...
* Enable the relevant permissions:
    ...
    | [fileAccess](../reference/apis-arkweb/arkts-basic-components-web-attributes.md#fileaccess) | Sets whether to enable access to the file system in the application. If it is not enabled, file read and write operations are completely blocked, and modules that depend on file read and write will encounter access denied errors. | 
...
1. Check the console to confirm whether there are exceptions caused by the Mixed Content policy or the CORS policy, or JS errors. For details, see [Resolving Cross-Origin Resource Access](web-cross-origin.md). To improve security, the ArkWeb kernel prohibits cross-origin requests through the file protocol and the resource protocol. Therefore, when the Web component loads local offline resources, it intercepts cross-origin access through the file protocol and the resource protocol. When the Web component cannot access local cross-origin resources, the DevTools console displays an error message:
    ...
    **Method 1:**
    ...
    struct Index {
      ...
      // Construct the local file and the returned mimeType format.
      mimeTypeMap = new Map([
      ...
      build() {
        Row() {
          Column() {
            // For the local index.html, use HTTP or HTTPS instead of the file or resource protocol, and construct a domain name of your own.
            // In this example, www.example.com is constructed as an example.
            Web({ src: "https://www.example.com/index.html", controller: this.webviewController })
              ...
              .onInterceptRequest((event) => {
                ...
                // Match the local offline resources to load, intercept and replace them to bypass cross-origin restrictions.
        ...
        Row() {
          Web({ src: '', controller: this.controller })
            .onControllerAttached(() => {
              try {
                // Set the list of paths allowed for cross-origin access.
            ...
            function getFile() {
              ...
              if(xmlHttpReq.readyState == 4){
                if (xmlHttpReq.status == 200) {
                   // If the path list is correctly set on the ETS side, the resource can be obtained here normally.
...
4. Modify [UserAgent](../reference/apis-arkweb/arkts-apis-webview-WebviewController.md#setcustomuseragent10) and then check whether the page recovers.
