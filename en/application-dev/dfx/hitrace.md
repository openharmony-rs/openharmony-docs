...
<!--Tester: @gcw_KuLfPSbe-->
<!--Adviser: @jinqiuheng-->
<!-- md-trans-meta sourceCommit=9b41559870778475344764be8fd1231b5e38dd06 translatedAt=2026-07-30T03:06:22.164Z pushedAt=2026-07-30T08:50:23.581Z -->
...
| You have root permissions. | `whoami` | `root` |
| The init trace plugin of the init subsystem is not enabled. | `param get persist.init.trace.enabled` | Not `1` |
...
```shell
hitrace --boot_trace [parameter] tag1 [tag2 ...]
...
| -------- | -------- |
| -b N/--buffer_size N | Sets the capture buffer size, in KB. When not specified, the default value is 18432. |
| -t N/--time N | Sets the duration of a single capture (in seconds), which must be greater than 0. When not specified, the default value is 30. |
| --repeat N | Sets the number of automatic captures on subsequent boots. The value range is [1, 100]. Each time a scheduled capture is triggered, the system parameter persist.hitrace.boot_trace.count decreases by 1 until it reaches 0, after which automatic capture stops. |
| --increment | Enables incremental naming. When enabled, the output file name is `{file_prefix}_{n}.sys`, where the sequence number `n` starts from 0 and increments by 1 after each successful capture. This option must be written after `--boot_trace`. |
| --file_prefix prefix | Sets the output file name prefix. When not specified, the default value is `boot_trace`. When `--increment` is not enabled, the output file name is `{file_prefix}.sys`. |
