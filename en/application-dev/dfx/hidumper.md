...
Virtual Set Size (VSS) is the virtual memory size of the current process, including all memory areas mapped to the process address space.

Resident Set Size (RSS) is the size of memory actually resident in physical memory for the current process.
...
| Field| Description|
| -------- | -------- |
| Process | Package name of the app process holding the DMA memory block (truncated to 16 characters). |
| pid | PID of the faulty process.|
| fd | Handle held by the process.|
| size_bytes | Size of the DMA memory buffer held by the process, in bytes. |
| ino | Inode number of the file.|
| exp_pid | PID of the process that requested DMA memory from the kernel. |
| exp_task_comm | Name of the process that requested DMA memory from the kernel. |
| buf_name | Name of the DMA memory buffer. |
| exp_name | Extension name of the DMA memory buffer. |
| buf_type | Type of the DMA memory buffer. |
| leak_type | Buffer type for DMA memory leak detection. |
...
- Run the **hidumper --mem-heap pid --native** command to export the native heap memory snapshot file of a specified process. The file is named <!--RP9-->nativeheap-Process ID-Timestamp<!--RP9End-->.
  ...
  $ ls | grep nativeheap -> Run after entering the heap memory file storage directory.
