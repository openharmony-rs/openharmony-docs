...
<!--Tester: @gcw_KuLfPSbe-->
<!--Adviser: @jinqiuheng-->
<!-- md-trans-meta sourceCommit=adc4fccab9ce2d6d069c4d6bc57fbeca9e96cecb translatedAt=2026-07-30T09:21:28.591Z pushedAt=2026-07-30T09:39:02.699Z -->
...
### setEventConfig Parameter Settings
...
| API | Description |
| --- | --- |
| int [OH_HiAppEvent_SetEventConfig](../reference/apis-performance-analysis-kit/capi-hiappevent-h.md#oh_hiappevent_seteventconfig)(const char* name, HiAppEvent_Config* config) | Customizes crash log specifications and merges app logs. **Only the NativeCrash type crash is supported.** |
...
| config | HiAppEvent_Config* | Yes | App crash event configuration parameter. You can use the [OH_HiAppEvent_SetConfigItem](../reference/apis-performance-analysis-kit/capi-hiappevent-h.md#oh_hiappevent_setconfigitem) function to set the configuration items of the **config** parameter. |

| Configuration Item Name | Type | Mandatory Configuration | Description |
| --- | --- | --- | --- |
| Macro: OH_APP_CRASH_PARAM_EXTEND_PC_LR_PRINTING<br/>String: extend_pc_lr_printing | const char* | No | Whether to print the memory content of the extended byte range of the PC and LR registers.<br/>**"true"**: On a 64-bit system, prints the memory values of the 248 bytes before and 256 bytes after the PC and LR register addresses. On a 32-bit system, prints the memory values of the 124 bytes before and 128 bytes after the PC and LR register addresses.<br/>**"false"**: On a 64-bit system, prints the memory values of the 16 bytes before and 232 bytes after the PC and LR register addresses. On a 32-bit system, prints the memory values of the 8 bytes before and 116 bytes after the PC and LR register addresses.<br/>The default value is **"false"** when this parameter is omitted.<br>**Note:** This parameter is supported from API version 24. |
| Macro: OH_APP_CRASH_PARAM_LOG_FILE_CUTOFF_SZ_BYTES<br/>String: log_file_cutoff_sz_bytes | const char* | No | Whether to truncate the crash log (CPP_CRASH log and minidump), in bytes. The value range is [0, 5242880].<br/>If set, the crash log is truncated to the specified size.<br/>If not set, the default value **0** indicates that the crash log is not truncated.<br>**Note:** This parameter is supported from API version 24. |
| Macro: OH_APP_CRASH_PARAM_SIMPLIFY_VMA_PRINTING<br/>String: simplify_vma_printing | const char* | No | Whether to print the VMA (Virtual Memory Area) mapping information to which addresses appearing in the crash log belong.<br/>**"true"**: Prints only the VMA mapping information to which addresses appearing in the crash log belong, that is, the Maps section in the crash log, to reduce the log size.<br/>**"false"**: Prints all VMA mapping information.<br/>The default value is **"false"** when this parameter is omitted.<br>**Note:** This parameter is supported from API version 24. |
| Macro: OH_APP_CRASH_PARAM_MERGE_CPPCRASH_APP_LOG<br/>String: merge_cppcrash_app_log | const char* | No | Whether to merge logs from the app sandbox.<br/>**"true"**: Merges app logs in the Native Crash scenario.<br/>**"false"**: Does not merge app-generated logs.<br/>The app log path read by the framework is: sandbox path + app package name + _CppCrash_AppMerge.log, for example: /data/storage/el2/log/com.samples.eventsub_CppCrash_AppMerge.log<br/>If you choose to generate the merged log in the signal handler, the maximum generation time is 5s. If it exceeds 5s, the app-generated log cannot be merged.<br/>**Note:** The merged log generated by the app must exist in the sandbox path.<br>**Note:** This parameter is supported from API version 24. |
| Macro: OH_APP_CRASH_PARAM_COLLECT_MINIDUMP<br/>String: collect_minidump | const char* | No | Whether to enable minidump. The default value is **"false"**.<!--RP1--><br/>**"true"**: The upper limit of the total size of generated and existing fault log files, as determined by the **log_over_limit** field in [params](#params), is adjusted to 35 MB.<br/>**"false"**: The upper limit of the total size of generated and existing fault log files, as determined by the **log_over_limit** field, is restored to 5 MB.<!--RP1End--><br/>**Note:** This configuration item is persistent. The value remains unchanged until the app resets it.<br>This parameter is supported from API version 26.0.0. |
...
| API | Description |
| -------- | -------- |
| [configEventPolicy](../reference/apis-performance-analysis-kit/js-apis-hiviewdfx-hiappevent.md#hiappeventconfigeventpolicy22) (policy: EventPolicy): Promise&lt;void>| Sets crash event policy parameters. This API supports enabling page switch log collection for crash events and setting crash log configuration parameters. |
...
let policy: hiAppEvent.EventPolicy = {
    appCrashPolicy: {
      pageSwitchLogEnable: true, // Enable page switch logging. Supported from API version 24.
      ...
      collectMinidump: true // Enable minidump in native crash scenarios. Supported from API version 26.0.0.
    }
};
...
| page_switch_log | string | Page switch log path. For details about the log, see [Page Switch Logs](pageswitch-log.md).<br>**Note:** This parameter is supported since API version 24. |
