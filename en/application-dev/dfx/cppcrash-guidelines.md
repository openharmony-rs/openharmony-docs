...
| Log source | Identifies the method used to collect logs. The following methods are available: <br> **processdump:** cppcrash logs generated in user mode.<br> **pdump:** Logs generated by the kernel as compensation when user-mode cppcrash log generation fails.<br> **liteprocessdump:** cppcrash logs generated when a low-privilege process (such as render or cpu) crashes.<br> Log specifications vary depending on the collection method. For details, see [Log Specification Differences Across Collection Methods](#log-specification-differences-across-collection-methods). | 26.0.0 | Yes | - |
...
| OpenFiles | File handle informatLog sourceion held by the process when the fault occurs.| 12 | Yes| - |
