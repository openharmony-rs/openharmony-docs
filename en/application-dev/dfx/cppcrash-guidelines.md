...
<!--Tester: @gcw_KuLfPSbe-->
<!--Adviser: @jinqiuheng-->
<!-- md-trans-meta sourceCommit=4f1622755d4ef5a7ab0d626e0b0f3d75e1087530 translatedAt=2026-07-30T02:52:26.925Z pushedAt=2026-07-30T03:18:47.733Z -->
...
- **Register**
  ...
  High-speed storage unit in the CPU, which is used to store data, instruction addresses, or status information required during the execution of computer programs. In this topic, register information refers to the data, instruction addresses, or status information stored in the register when a crash occurs.
...
| ExtraCrashInfo | Additional memory information. | 23 | No | Memory information set by the app through [OH_HiDebug_SetCrashObj](hidebug-guidelines.md#adding-debugging-information-to-crash-logs). |
...
> **NOTE**
>
> Since API version 20, <!--Del-->the **cpsr** register is added to the fault site registers of the ARM32 architecture, and the AArch64 architecture <!--DelEnd-->the **pstate** and **esr** registers are added to the fault site registers.
...
Fingerprint:b2b57ac77e71434d32e015e2efd464f1ce4bbadfacf5309986d2acc12b744f53 <- Identifies the fault fingerprint
...
Device Memory(kB): Total 2001936, Free 509212, Available 1115804 <- Device memory information (optional)
...
#00 pc 000a8a94 /system/lib/ld-musl-arm.so.1(3753ef9108517347d1a03e903b3a5dbf) <- Call stack, in the order #06→#05→...→#00, where the crash ultimately occurs in the function at #00
...
Last Modified: 2026-03-18 10:10:10 <- Last modified timestamp of the merged log provided by the app
...
Fingerprint:64ededf12a18ff56e730603cd61a5c9fe7240811fdf747994184e558ecaf3f4d <- Identifies the fault characteristics
...
Log source:processdump <- Used to identify the method of log collection. **processdump** indicates generation by user mode.
...
Process Memory(kB): 5279(Rss)     <- Fault process memory usage
...
Device Memory(kB): Total 2001936, Free 311220, Available 1181516 <- Device memory information (optional).
Log source:processdump <- Identifies the log collection method. processdump indicates that the log is generated by user mode.
...
Device info:OpenHarmony 3.2       <- Device info
Build info:OpenHarmony 6.1.0.22    <- Build info
DeviceDebuggable:No <- Whether the device's system version is debuggable
...
Device Memory(kB): Total 11712088, Free 2500232, Available 5275648 <- Device memory information (optional)
Log source:processdump <- Used to identify the log collection method. processdump indicates that the logs are generated in user mode.
...
lr(/system/lib64/chipset-sdk-sp/libeventhandler.z.so): <- Memory values near the lr register address.
    0000005afeae1a80 2a1f03e0940067e8 <- When extend_pc_lr_printing is set to true, memory values are printed forward up to this point.
    ...
    0000005afeae1b68 910083e19a89b103 <- When extend_pc_lr_printing is set to false, memory values are printed forward up to this point.
    ...
    ...
    0000005afeae1c60 52801002d10243a1 <- When extend_pc_lr_printing is set to false, memory values are printed backward up to this point.
    0000005afeae1c68 ad3c83a0ad3b83a0
    0000005afeae1c70 ad3e83a0ad3d83a0
    0000005afeae1c78 94006799b9400000 <- When extend_pc_lr_printing is set to true, memory values are printed backward up to this point.
pc(/system/lib/ld-musl-aarch64.so.1): <- Memory values near the pc register address.
    0000005afb7f82f8 f9000bf3a9be7bfd <- When extend_pc_lr_printing is set to true, memory values are printed forward up to this point.
    ...
    0000005afb7f83e0 aa1f03e4aa0403e3 <- When extend_pc_lr_printing is set to false, memory values are printed forward up to this point.
    ...
    ...
    0000005afb7f84d8 aa0903e152800260 <- When extend_pc_lr_printing is set to false, memory values are printed backward up to this point.
    0000005afb7f84e0 d63f0100f9439508
    0000005afb7f84e8 a9be7bfd17ffffe6
    0000005afb7f84f0 910003fdf9000bf3 <- When extend_pc_lr_printing is set to true, memory values are printed backward up to this point.
...
...
### Log Specification for Fault Scenarios with Additional Memory Information

By using the [OH_HiDebug_SetCrashObj](hidebug-guidelines.md#adding-debugging-information-to-crash-logs) API provided by HiDebug for adding debugging information, you can add debugging information to the crash log based on your service requirements. If a program crash occurs, you can find the debugging information in the crash log.

The fault file records memory content from the specified address according to the set length. Taking a length of 64 bytes as an example:

```text
...
ExtraCrashInfo(Memory start address 0000007e3c6f2f80):
                                           ^
                            Configured memory start address
+0x000: 0706050403020100 0000000000009080 0000000000000000 0000000000000000
   ^                            ^
Relative offset       Value in memory
+0x020: 0000000000000000 0000000000000000 0000000000000000 0000000000000000
        ^                                                  ^
Value at memory offset 0x020                   Value at memory offset 0x038
...
```
...
| Build info | Version information. | Yes | Yes | Yes |
| DeviceDebuggable | Whether the system version of the device is debuggable. | Yes | Yes | Yes |
| Fingerprint | Fault fingerprint, a hash value for clustering similar issues. Identical hash values indicate the same fault cause. | Yes | Yes | Yes |
...
| Page switch history | Page switching history. | Yes | Yes | Yes |
| Timestamp | Timestamp when the fault occurred. | Yes | Yes | Yes |
| Pid | Process ID. | Yes | Yes | Yes |
| Uid | User ID. | Yes | Yes | Yes |
| HiTraceId | Unique trace identifier of HiTraceChain. | Yes | No | No |
| Process name | Name of the faulty process. | Yes | Yes | Yes |
| App running unique id | Unique ID associated with the app at runtime. | Yes | No | Yes |
| Process life time | Lifetime of the faulty process. | Yes | Yes | Yes |
| Process Memory(kB) | Memory usage of the faulty process. | Yes | Yes | Yes |
| Device Memory(kB) | System memory status. | Yes | Yes | Yes |
| Log source | Method used to collect logs. | Yes | Yes | Yes |
...
| Other thread info | Information about other threads. | Yes | Yes | Yes |
| Memory near registers | Memory values near the registers at the fault site. | Yes | Yes | Yes |
| FaultStack | Memory information of the fault thread stack. | Yes | Yes | Yes |
| Maps | Memory space of the process at the time of the fault. | Yes | Yes | Yes |
| OpenFiles | File handle information held by the process at the time of the fault. | Yes | No | Yes |
| HiLog | Logs printed before the fault, up to 1000 lines. | Yes | Yes | Yes |
...
- **SIGPIPE signal description**
  ...
  | -------- | -------- | -------- | -------- |
  | 13 | SIGPIPE | Illegal operation on a pipe or socket | - Pipe or FIFO: The write end writes to the pipe, but the read end has been closed.<br>- Socket: After the client disconnects, the server still attempts to write data to the closed connection. |
