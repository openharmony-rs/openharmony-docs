...
- **Since API version 26.0.0**, the system supports the following output prompt: Main thread is blocked by GC, which may be caused by high memory usage or system resource overload.
  ...
  - If the [Garbage Collection (GC)](../arkts-utils/gc-introduction.md) type is Shared GC, the system does not output this information.

  - If an Appfreeze event occurs during [GC](../arkts-utils/gc-introduction.md) and the GC type is not Shared GC, the system outputs this information in the NOTE. In this case, you can ignore the app freeze fault.

  - For details about the Shared GC type, see [SharedHeap Structure](../arkts-utils/gc-introduction.md#sharedheap-structure) and [SharedHeap Related Parameters](../arkts-utils/gc-introduction.md#sharedheap-related-parameters).

  - For details about the Local GC type, see [LocalHeap Structure](../arkts-utils/gc-introduction.md#localheap-structure) and [LocalHeap Related Parameters](../arkts-utils/gc-introduction.md#localheap-related-parameters).
...
| Field | Description |
| -------- | -------- |
| EVENTNAME | Composes the deadlock detection event. |
| TIMESTAMP | Time when the event is reported upon fault occurrence. You can narrow down the log viewing time range in the corresponding hilog log based on the timeout duration specified in the app not responding detection capability point. |
| PID | PID at the time of fault occurrence. |
| UID | UID at the time of fault occurrence. |
| TID | TID at the time of fault occurrence. |
| PACKAGE_NAME | Package name of the app process. |
| PROCESS_NAME | Name of the app process. |
...
| this thread has blocked 3000ms. | When a thread blocking fault occurs, the app thread task execution times out by 3 seconds (that is, the THREAD_BLOCK_3S threshold). The specific timeout duration is subject to the actual value in the log (3000ms in the example).<br>**Note**: Supported since API version 26.0.0.|
| MainHeap(bytes) | Size of the main thread LocalHeap. For details, see [Heap](../arkts-utils/gc-introduction.md#heap).<br>**Note**: Supported since API version 26.0.0. |
| SharedHeap(bytes) | Size of the main thread SharedHeap. For details, see [Heap](../arkts-utils/gc-introduction.md#heap).<br>**Note**:<br>- Supported since API version 26.0.0.<br>- The `Total` value of `SharedHeap` is a fixed size: 815792128 bytes (that is, 778 MB). For details, see the `SharedHeapSize` parameter description in [SharedHeap Related Parameters](../arkts-utils/gc-introduction.md#sharedheap-related-parameters). |
| GC Status | GC related information when an AppFreeze event occurs. For details, see [application_gc_info](hiappevent-watcher-freeze-events.md#application_gc_info).<br>**Note**: Supported since API version 26.0.0. |
| I/O(bytes) | I/O related information when an AppFreeze event occurs. For details, see [application_io_info](hiappevent-watcher-freeze-events.md#application_io_info).<br>**Note**: Supported since API version 26.0.0.|
...
| Current Running | Complete task information currently being executed. |
| History event queue information | Historical task execution information. |
...
| Idle priority event queue information | Idle task queue information. |
...
| completeTime time | Task completion time (if not printed, the task is not completed). |
...
| ------------- | ----------------------------------------------------------------------------------------------------------------- |
| has been crashed | The target process has crashed. A stack capture request is received within 10s after the crash request of the target process is received. See the crash log near the fault time point. Check whether the app's signal handling functions are used correctly or whether non-signal-safe functions are used. |
| SIGDUMP error | The target process has already exited when the stack capture request is received.|
| is dumping | The target process is dumping. For consecutive requests within a short period, see other error logs of this process near the fault time point.|
| State: S | The target process is in Sleep state.|
...
1. Load phase events, using the case where the app process is not created as an example.
   ...
   | ------------------------------------------------------------- | ---------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------- |
   | AbilityRecord::LoadAbility; the LoadAbility lifecycle starts. | - | The Ability load lifecycle starts. The system prepares the resources and process required for loading the Ability. Marks the start point of the load phase. | - | No, system-side scheduling. |
   ...
   | AppMgrService::AttachApplication | - | After the app process is created, the process sends an attach request to the server to establish a communication channel. | - | No, system-side scheduling. |
   ...
   | - | MainThread::HandleLaunchAbility | The app main thread processes the Ability load request. | - | No, system-side loading process. |
   | - | JsAbilityStage::Create | Loads [AbilityStage](../application-models/abilitystage.md). | - | No, system-side loading process. |
   | - | JsAbilityStage::OnCreate begin | The AbilityStage [onCreate](../reference/apis-ability-kit/js-apis-app-ability-abilityStage.md#oncreate) lifecycle starts. | Time-consuming operations are performed in AbilityStage onCreate. | Yes, avoid performing time-consuming operations in AbilityStage onCreate. |
   | - | JsAbilityStage::OnCreate end | The AbilityStage onCreate lifecycle ends. AbilityStage initialization is complete. The time can be calculated by comparing with begin. | AbilityStage onCreate takes too long. | Yes, optimize the AbilityStage onCreate logic. |
   | - | JsAbilityStage::OnAboutToCreateAbility begin (InsightIntent Scenario) | AbilityStage is about to create an Ability ([onAboutToCreateAbility](../reference/apis-ability-kit/js-apis-app-ability-abilityStage.md#onabouttocreateability24) callback). | Time-consuming operations are performed in onAboutToCreateAbility. | Yes, avoid performing time-consuming operations in onAboutToCreateAbility. |
   ...
   | AbilityManagerService::AttachAbilityThread; the end of load lifecycle. | - | The server confirms that AbilityThread attachment is complete. The load lifecycle ends. | - | No, system-side scheduling. |

2. Foreground phase events, with cold start of the app.

   | Server | Client | Description | Failure Cause | Requires App Handling |
   | ------------------------------------------------------------- | ---------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------- |
   | AbilityRecord::ProcessForegroundAbility; the ProcessForegroundAbility lifecycle starts. | - | The start phase of the Ability entering the foreground lifecycle. Marks the start point of the foreground phase. | - | No, system-side scheduling. |
   ...
   | - | HandleForegroundApplication | The main thread executes scheduling and distributes the app foreground task to specific modules. | - | Yes, check whether Application onForeground is time-consuming. |
   ...
   | - | AbilityThread::HandleAbilityTransaction | The main thread executes the Ability foreground scheduling and triggers the Ability lifecycle callback. | - | No, system-side recording. |
   | - | JsUIAbility::OnStart begin | The [onCreate](../reference/apis-ability-kit/js-apis-app-ability-uiAbility.md#oncreate) lifecycle starts. The Ability performs initialization. | Time-consuming operations are performed in onCreate. | Yes, avoid performing time-consuming operations in onCreate. |
   | - | JsUIAbility::OnStart end | The onCreate lifecycle ends. The initial resources of the Ability are ready. The time can be calculated by comparing with begin. | onCreate takes too long. | Yes, avoid performing time-consuming operations in onCreate. |
   | - | JsUIAbility::OnSceneCreated begin | Window scene creation starts. | Time-consuming operations are performed in [onWindowStageCreate](../reference/apis-ability-kit/js-apis-app-ability-uiAbility.md#onwindowstagecreate). | Yes, avoid performing time-consuming operations in onWindowStageCreate. |
   | - | JsUIAbility::OnSceneCreated end | Window scene creation ends. The time can be calculated by comparing with begin. | onWindowStageCreate takes too long. | Yes, avoid performing time-consuming operations in onWindowStageCreate. |
   | - | JsUIAbility::OnWillForeground begin | The [onWillForeground](../reference/apis-ability-kit/js-apis-app-ability-uiAbility.md#onwillforeground20) callback starts. | Time-consuming operations are performed in onWillForeground. | Yes, avoid performing time-consuming operations in onWillForeground. |
   | - | JsUIAbility::OnWillForeground end | The onWillForeground callback ends. The time can be calculated by comparing with begin. | onWillForeground takes too long. | Yes, avoid performing time-consuming operations in onWillForeground. |
   ...
   | - | JsUIAbility::IntentForeground execute start begin (InsightIntent Scenario) | Execution of the app foreground-related [intent](../application-models/insight-intent-overview.md) operation starts, processing the Intent task triggered by the foreground switch. | The InsightIntent executor did not return in time. | Yes, ensure that the Intent executor completes in time. |
   | - | JsUIAbility::IntentForeground end (InsightIntent Scenario) | The app foreground-related intent operation execution ends. The Intent-related logic has been processed. The time can be calculated by comparing with start. | The InsightIntent executor takes too long. | Yes, ensure that the Intent executor completes in time. |
   | - | JsUIAbility::IntentRepeat execute start (InsightIntent Scenario) | The Intent repeat execution mode starts. | The InsightIntent executor did not return in time. | Yes, ensure that the Intent executor completes in time. |
   | - | JsUIAbility::IntentRepeat execute end (InsightIntent Scenario) | The Intent repeat execution mode ends. The time can be calculated by comparing with start. | The InsightIntent executor takes too long. | Yes, ensure that the Intent executor completes in time. |
   | - | JsUIAbility::IntentPage execute start (InsightIntent Scenario) | The Intent page execution mode starts. | The InsightIntent executor did not return in time. | Yes, ensure that the Intent executor completes in time. |
   | - | JsUIAbility::IntentPage execute end (InsightIntent Scenario) | The Intent page execution mode ends. The time can be calculated by comparing with start. | The InsightIntent executor takes too long. | Yes, ensure that the Intent executor completes in time. |
   ...
   | - | JsUIAbility::OnForeground begin | The [onForeground](../reference/apis-ability-kit/js-apis-app-ability-uiAbility.md#onforeground) lifecycle starts, notifying you that the Ability is displayed in the foreground. | Time-consuming operations are performed in onForeground. | Yes, avoid performing time-consuming operations in onForeground. |
   | - | JsUIAbility::OnForeground end | The onForeground lifecycle ends. The Ability's foreground lifecycle callback is complete. The time can be calculated by comparing with begin. | onForeground takes too long. | Yes, avoid performing time-consuming operations in onForeground. |
   | - | JsUIAbility::OnDidForeground begin | The [onDidForeground](../reference/apis-ability-kit/js-apis-app-ability-uiAbility.md#ondidforeground20) callback starts. | Time-consuming operations are performed in onDidForeground. | Yes, avoid performing time-consuming operations in onDidForeground. |
   | - | JsUIAbility::OnDidForeground end | The onDidForeground callback ends. The time can be calculated by comparing with begin. | onDidForeground takes too long. | Yes, avoid performing time-consuming operations in onDidForeground. |
   ...
   | AbilityManagerService::AbilityTransitionDone; the end of foreground lifecycle. | - | The server confirms that the foreground switch is complete. The foreground lifecycle ends. | The client did not call AbilityTransitionDone in time. | No, system-side scheduling. |
...
Starting from system versions <!--__TRANSPRESERVE_0__-->OpenHarmony 6.1.0.125 and later<!--__TRANSPRESERVE_1__-->, the system merges the AppFreeze enhanced log content to the end of the log pointed to by [external_log](hiappevent-watcher-freeze-events.md#params) of the APP_FREEZE event by default.
...
At the same time, clustering helps developers categorize and count issues caused by different root causes.
...
Therefore, using fault thread information as the clustering scope provides the highest accuracy. Developers can add other fault log information based on their business clustering requirements.
