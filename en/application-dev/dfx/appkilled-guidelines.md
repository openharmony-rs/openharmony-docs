...
6. When the system manages the process, it cannot provide detailed maintenance and debugging information in some scenarios, such as **LowMemoryKiller**, RSS of the application exceeding 4 GB, and fast leaks.
...
2. Subscribe to the **APP_KILLED** event through HiAppEvent.
    ...
    | CpaKiller                     | The Digital Right Management (DRM) service applies for memory but the memory is insufficient, triggering process termination to reclaim the memory based on specific policies.| Reduce the memory usage of the application.| No                      | No            |
