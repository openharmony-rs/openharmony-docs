...
<!--Tester: @lotsof-->
<!--Adviser: @jinqiuheng-->
<!-- md-trans-meta sourceCommit=d15494bdeec2546d4ff02bd3050be81ef56e89a6 translatedAt=2026-08-24T11:36:00.106Z pushedAt=2026-08-25T06:30:58.434Z -->
...
- Sets the task name, which is useful information for printing maintenance and test information.
...
- A performance implementation similar to pthread condition variables provided by FFRT, but does not support initialization similar to `PTHREAD_COND_INITIALIZER`
...
- Note that `ffrt_cond_t` in the C API needs to be explicitly created and destroyed by calling `ffrt_cond_init` and `ffrt_cond_destroy`. However, in the C++ API, the construction and destruction are automatically completed.

- You need to set the `ffrt_cond_t` object in the C code to null and destroy the object. For the same `ffrt_cond_t` object, `ffrt_cond_destroy` can be called only once. Otherwise, undefined behavior may occur.
...
- `time_point`: 'pointer to the object specifying the waiting time limit.
...
- Starts a timer. If the timer expires and is not stopped, the callback function is executed. If `repeat` is set to `true`, the timer is set again after it expires.
