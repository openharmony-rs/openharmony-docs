...
| excludeFromRecent<sup>26+</sup> | boolean | No| Yes| Whether to hide the window from the multitasking center.<br>If this parameter is set to **true**, the current main window will not be displayed in the multitasking center. If this parameter is set to **false**, the window starts normally. <br>If this parameter is not set, the window starts normally.<br> **Model constraint**: 1. This parameter only takes effect on PC/2in1 devices. 2. This API can be used only in the stage model.<br>**Notes**: This parameter takes effect only when bound to a tray icon.|
## Callback<sup>15+</sup>
