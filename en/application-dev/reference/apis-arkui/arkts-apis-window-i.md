...
| windowMinimizedWhenStart<sup>26+</sup> | boolean | No| Yes| Whether the window starts in a minimized state.<br>If this parameter is set to **true**, the window will not be brought to the foreground. If this parameter is set to **false**, the window starts normally. <br>If this parameter is not set, the window starts normally.<br> **Model constraint**: This API can be used only in the stage model.<br>**Notes**: This parameter only takes effect on PC/2in1 devices.|
| excludeFromDock<sup>26+</sup> | boolean | No| Yes| Whether to hide the Dock icon and the hover thumbnail preview.<br>If this parameter is set to **true**, the current main window will not display its icon and hover thumbnail preview on the Dock. If this parameter is set to **false**, the window starts normally. <br>If this parameter is not set, the window starts normally.<br> **Model constraint**: This API can be used only in the stage model.<br>**Notes**: 1. This parameter only takes effect on PC/2in1 devices. 2. This parameter takes effect only when bound to a tray icon.|
| excludeFromRecent<sup>26+</sup> | boolean | No| Yes| Whether to hide the window from the multitasking center.<br>If this parameter is set to **true**, tthe current main window will not be displayed in the multitasking center. If this parameter is set to **false**, the window starts normally. <br>If this parameter is not set, the window starts normally.<br> **Model constraint**: 1. This parameter only takes effect on PC/2in1 devices. 2. This API can be used only in the stage model.<br>**Notes**: This parameter takes effect only when bound to a tray icon.|
## Callback<sup>15+</sup>
