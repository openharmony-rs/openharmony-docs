...
| windowMinimizedWhenStart<sup>26+</sup> | boolean | No| Yes| Whether the window starts in a minimized state.<br>If this parameter is set to **true**, the window will not be brought to the foreground, and only an icon will be displayed on the Dock. If this parameter is set to **false**, the window starts normally. <br>If this parameter is not set, it defaults to **undefined** and he window starts normally.
| excludeFromDock<sup>26+</sup> | boolean | No| Yes| Whether to hide the Dock icon and the hover thumbnail preview.<br>If this parameter is set to **true**, the current main window will not display its icon and hover thumbnail preview on the Dock. If this parameter is set to **false**, the window starts normally. <br>If this parameter is not set, it defaults to **undefined** and he window starts normally.<br>**Notes**: This parameter takes effect only when bound to a tray icon.
| excludeFromRecent<sup>26+</sup> | boolean | No| Yes| Whether to hide the window from the task view.<br>If this parameter is set to **true**, tthe current main window will not be displayed in the task view. If this parameter is set to **false**, the window starts normally. <br>If this parameter is not set, it defaults to **undefined** and he window starts normally.<br>**Notes**: This parameter takes effect only when bound to a tray icon.
## Callback<sup>15+</sup>
