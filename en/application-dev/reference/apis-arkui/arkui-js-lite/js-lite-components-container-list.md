...
| Name| Type| Default Value| Mandatory| Description|
| -------- | -------- | -------- | -------- | -------- |
| flex-direction | string | column | No| Main axis direction of the flex container. It specifies how items are placed in the flex container.<br>**column**: The y-axis is the main axis.<br>- **row**: Items are placed horizontally from left to right.<br>For the **list** component, the default value is **column**. For other components, the default value is **row**. Dynamic modification is not supported on lite wearables.|
