...
<!-- md-trans-meta sourceCommit=c06f3c9eaeff0c9cf0ec5be6286c056a20c6428e translatedAt=2026-08-04T10:57:24.971Z pushedAt=2026-08-05T01:18:24.397Z -->
...
Provides a collection of native-side APIs of the Node type provided by ArkUI, supporting capabilities such as component node creation and destruction, node tree operations, attribute and event management, measurement, and layout. It is suitable for scenarios where native APIs are used to build and operate ArkUI component nodes. APIs related to the Node module must be called on the main thread.
...
| [int32_t (\*registerNodeEvent)(ArkUI_NodeHandle node, ArkUI_NodeEventType eventType, int32_t targetId, void\* userData)](#registernodeevent) | Registers an event for the specified node. After the event is triggered, it can be received through the component-level callback added by [addNodeEventReceiver](#addnodeeventreceiver) or the global callback registered by [registerNodeEventReceiver](#registernodeeventreceiver). When the event is no longer needed, call [unregisterNodeEvent](#unregisternodeevent) to unregister it. |
| [void (\*unregisterNodeEvent)(ArkUI_NodeHandle node, ArkUI_NodeEventType eventType)](#unregisternodeevent) | Unregisters an event for the specified node, which is previously registered through [registerNodeEvent](#registernodeevent). After unregistration, the event no longer triggers the corresponding callback. |
| [void (\*registerNodeEventReceiver)(void (\*eventReceiver)(ArkUI_NodeEvent\* event))](#registernodeeventreceiver) | Registers a unified entry point for event callbacks. The ArkUI framework collects component events generated during processing and returns them through the registered **eventReceiver** API.<br> Repeated calls will override the previously registered API.<br> Avoid directly saving pointers to [ArkUI_NodeEvent](capi-arkui-nativemodule-arkui-nodeevent.md) objects, as the data is destroyed after the callback is complete.<br> To bind with a component instance, you can use the [addNodeEventReceiver](#addnodeeventreceiver) API.<br> When the global callback is no longer needed, call [unregisterNodeEventReceiver](#unregisternodeeventreceiver) to unregister it.<br> |
| [void (\*unregisterNodeEventReceiver)()](#unregisternodeeventreceiver) | Unregisters the unified entry point for event callbacks, which is previously registered through [registerNodeEventReceiver](#registernodeeventreceiver). After unregistration, the global entry no longer receives component events. |
| [void (\*markDirty)(ArkUI_NodeHandle node, ArkUI_NodeDirtyFlag dirtyFlag)](#markdirty) | Forcibly marks the current node for re-measurement, re-layout, or re-drawing. Regarding updates to system attributes, the ArkUI framework automatically marks nodes and re-executes measurement, layout, or drawing; you do not need to call this API actively. This API can be called to trigger the corresponding process when you modify custom measurement, layout, or rendering data that the framework cannot automatically detect. |
| [uint32_t (\*getTotalChildCount)(ArkUI_NodeHandle node)](#gettotalchildcount) | Obtains the number of child nodes.|
| [ArkUI_NodeHandle (\*getChildAt)(ArkUI_NodeHandle node, int32_t position)](#getchildat) | Obtains the child node at the specified index. |
...
| [int32_t (\*registerNodeCustomEvent)(ArkUI_NodeHandle node, ArkUI_NodeCustomEventType eventType, int32_t targetId, void\* userData)](#registernodecustomevent) | Registers a custom event for a node. After the event is triggered, it can be received through the component-level callback added by [addNodeCustomEventReceiver](#addnodecustomeventreceiver) or the global callback registered by [registerNodeCustomEventReceiver](#registernodecustomeventreceiver). When the event is no longer needed, call [unregisterNodeCustomEvent](#unregisternodecustomevent) to unregister it. |
| [void (\*unregisterNodeCustomEvent)(ArkUI_NodeHandle node, ArkUI_NodeCustomEventType eventType)](#unregisternodecustomevent) | Unregisters a custom event for a node, which is previously registered through [registerNodeCustomEvent](#registernodecustomevent). After unregistration, the event no longer triggers the corresponding callback. |
| [void (\*registerNodeCustomEventReceiver)(void (\*eventReceiver)(ArkUI_NodeCustomEvent\* event))](#registernodecustomeventreceiver) | Registers a unified entry point for custom node event callbacks. The ArkUI framework collects custom component events generated during processing and returns them through the custom event entry point function registered using **registerNodeCustomEventReceiver**.<br>Repeated calls will override the previously registered API.<br>Avoid directly saving pointers to [ArkUI_NodeCustomEvent](capi-arkui-nativemodule-arkui-nodecustomevent.md) objects, as the data will be destroyed after the callback ends.<br> To bind with a component instance, you can use the [addNodeCustomEventReceiver](#addnodecustomeventreceiver) API.<br> When the global callback is no longer needed, call [unregisterNodeCustomEventReceiver](#unregisternodecustomeventreceiver) to unregister it.<br> |
| [void (\*unregisterNodeCustomEventReceiver)()](#unregisternodecustomeventreceiver) | Unregisters the unified entry point function for custom node event callbacks, which is previously registered through [registerNodeCustomEventReceiver](#registernodecustomeventreceiver). After unregistration, the global entry no longer receives custom component events. |
| [int32_t (\*setMeasuredSize)(ArkUI_NodeHandle node, int32_t width, int32_t height)](#setmeasuredsize) | Sets the width and height of the component after measurement is complete in the measurement callback function. |
| [int32_t (\*setLayoutPosition)(ArkUI_NodeHandle node, int32_t positionX, int32_t positionY)](#setlayoutposition) | Sets the layout position of the component relative to its parent component in the layout callback function. This API has a lower priority than [NODE_POSITION](capi-native-node-h-nodeattributetype-layoutattributes.md#node_position) in **ArkUI_NodeAttributeType**. |
...
| [int32_t (\*addNodeEventReceiver)(ArkUI_NodeHandle node, void (\*eventReceiver)(ArkUI_NodeEvent\* event))](#addnodeeventreceiver) | Adds a component event callback function to a component to receive component events generated by it. Unlike the global registration function **registerNodeEventReceiver**, this API allows multiple event receivers to be added to the same component.<br> The callback added by this function is triggered before the global callback registered by **registerNodeEventReceiver**.<br>Avoid directly saving pointers to [ArkUI_NodeEvent](capi-arkui-nativemodule-arkui-nodeevent.md) objects, as the data will be destroyed after the callback is complete.<br> When the callback is no longer needed, call [removeNodeEventReceiver](#removenodeeventreceiver) to remove it.<br> |
| [int32_t (\*removeNodeEventReceiver)(ArkUI_NodeHandle node, void (\*eventReceiver)(ArkUI_NodeEvent\* event))](#removenodeeventreceiver) | Removes the event callback function previously added to this component through [addNodeEventReceiver](#addnodeeventreceiver). After removed, the callback no longer receives events generated by this component. |
| [int32_t (\*addNodeCustomEventReceiver)(ArkUI_NodeHandle node, void (\*eventReceiver)(ArkUI_NodeCustomEvent\* event))](#addnodecustomeventreceiver) | Adds a custom event callback function to a component to receive custom events (such as layout and drawing events) generated by it. Unlike the global registration function **registerNodeCustomEventReceiver**, this API allows multiple event receivers to be added to the same component.<br>The callback added by this function is triggered before the global callback registered by **registerNodeCustomEventReceiver**.<br>Avoid directly saving pointers to [ArkUI_NodeCustomEvent](capi-arkui-nativemodule-arkui-nodecustomevent.md) objects, as the data will be destroyed after the callback is complete.<br> When the callback is no longer needed, call [removeNodeCustomEventReceiver](#removenodecustomeventreceiver) to remove it.<br> |
| [int32_t (\*removeNodeCustomEventReceiver)(ArkUI_NodeHandle node, void (\*eventReceiver)(ArkUI_NodeCustomEvent\* event))](#removenodecustomeventreceiver) | Removes the custom event callback function previously added to this component through [addNodeCustomEventReceiver](#addnodecustomeventreceiver). After removed, the callback no longer receives custom events generated by this component. |
...
| [int32_t (\*setLengthMetricUnit)(ArkUI_NodeHandle node, ArkUI_LengthMetricUnit unit)](#setlengthmetricunit) | Sets the unit used when setting length-related attributes of a component through a native API of the Node type. Call this API to specify the unit when the service needs to uniformly use px, vp, or fp for setting length-related attributes of a component. |
...
| Type| Description|
| -- | -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) | Pointer to the created component object. If the component fails to be created, **NULL** is returned. When the component object is no longer needed, call [disposeNode](#disposenode) to destroy it. Improper lifecycle management may cause Use After Free, process crashes, or memory leak. |
...
| Type| Description                                                                                                                                                                                                                                                                                                                                    |
| -- |----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| int32_t | Result code.<br>             Returns [ARKUI_ERROR_CODE_NO_ERROR](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) if the operation is successful.<br>             Returns [ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) if a parameter error occurs.<br>             Returns [ARKUI_ERROR_CODE_ARKTS_NODE_NOT_SUPPORTED](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) if the operation is not supported for the node created in ArkTS.<br>             Returns [ARKUI_ERROR_CODE_NODE_IS_ADOPTED](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) if the node has been adopted as an affiliated node. This specification is supported since API version 22.|
...
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) parent | Pointer to the parent node.|
|  [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) child | Pointer to the child node.|
| int32_t position | Zero-based insertion position in the parent node's child node list. The value range is [0, Current number of child nodes]. If the value is out of range, the node is inserted at the end of the parent node by default. |
...
Registers an event for the specified node. After the event is triggered, it can be received through the component-level callback added by [addNodeEventReceiver](#addnodeeventreceiver) or the global callback registered by [registerNodeEventReceiver](#registernodeeventreceiver). When the event is no longer needed, call [unregisterNodeEvent](#unregisternodeevent) to unregister it.
...
| Type| Description|
| -- | -- |
| int32_t | Result code.<br>             Returns [ARKUI_ERROR_CODE_NO_ERROR](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) if the operation is successful.<br>             Returns [ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) if a parameter error occurs.<br>             Returns [ARKUI_ERROR_CODE_ATTRIBUTE_OR_EVENT_NOT_SUPPORTED](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) if the event is not supported.<br>             Returns [ARKUI_ERROR_CODE_ARKTS_NODE_NOT_SUPPORTED](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) if the operation is not supported for the node created in ArkTS.|
...
Unregisters an event for the specified node, which is previously registered through [registerNodeEvent](#registernodeevent). After unregistration, the event no longer triggers the corresponding callback.
...
Unregisters the unified entry point for event callbacks, which is registered through [registerNodeEventReceiver](#registernodeeventreceiver). After unregistration, the global entry no longer receives component events.
...
Forcibly marks the current node for re-measurement, re-layout, or re-drawing. Regarding updates to system attributes, the ArkUI framework automatically marks nodes and re-executes measurement, layout, or drawing; you do not need to call this API actively. This API can be called to trigger the corresponding process when you modify custom measurement, layout, or drawing data that the framework cannot automatically detect.
...
| [ArkUI_NodeDirtyFlag](capi-native-node-h.md#arkui_nodedirtyflag) dirtyFlag | Type of the process to be re-executed.  |
...
| Type| Description|
| -- | -- |
| uint32_t | Number of child nodes. If there is no child node, **0** is returned. |
...
| -- | -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) node | Target node.|
| int32_t position | Zero-based index of the target node in the child node list. The value range is [0, Number of child nodes - 1). **NULL** is returned if the index is out of range. |

**Return value**

| Type| Description|
| -- | -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) | Pointer to the child node at the specified index, or **NULL** if no child node exists at the position. |
...
| Type| Description|
| -- | -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) | Pointer to the first child node, or **NULL** if there is no child node. |
...
| Type| Description|
| -- | -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) | Pointer to the last child node, or **NULL** if there is no child node. |
...
| Type| Description|
| -- | -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) | Pointer to the previous sibling node, or **NULL** if no previous sibling node exists. |
...
| Type| Description|
| -- | -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) | Pointer to the next sibling node, or **NULL** if no next sibling node exists. |
...
Registers a custom event for a node. After the event is triggered, it can be received through the component-level callback added by [addNodeCustomEventReceiver](#addnodecustomeventreceiver) or the global callback registered by [registerNodeCustomEventReceiver](#registernodecustomeventreceiver). When the event is no longer needed, call [unregisterNodeCustomEvent](#unregisternodecustomevent) to unregister it.
...
|----------------------------------------------------------------------------------------| -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) node                       | Target node.|
| [ArkUI_NodeCustomEventType](capi-native-node-node-attributes-custom-attributes-h.md#arkui_nodecustomeventtype) eventType | Type of the event to register.|
...
Unregisters a custom event for a node, which is previously registered through [registerNodeCustomEvent](#registernodecustomevent). After unregistration, the event no longer triggers the corresponding callback.
...
|----------------------------------------------------------------------------------------|---------------|
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) node                       | Target node.|
| [ArkUI_NodeCustomEventType](capi-native-node-node-attributes-custom-attributes-h.md#arkui_nodecustomeventtype) eventType | Type of the event to unregister.             |
...
Unregisters the unified entry point function for custom node event callbacks, which is previously registered through [registerNodeCustomEventReceiver](#registernodecustomeventreceiver). After unregistration, the global entry no longer receives custom component events.
...
Sets the width and height of the component after measurement is complete in the measurement callback function.
...
| -- | -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) node | Target node.|
|  int32_t width | Width of the component after measurement, in px. If the value is less than 0, 0 is used. |
|  int32_t height | Height of the component after measurement, in px. If the value is less than 0, 0 is used. |
...
Sets the layout position of the component relative to its parent component in the layout callback function. This API has a lower priority than [NODE_POSITION](capi-native-node-h-nodeattributetype-layoutattributes.md#node_position) in **ArkUI_NodeAttributeType**.
...
| -- | -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) node | Target node.|
|  int32_t positionX | X-axis layout coordinate of the component relative to its parent component, in px. |
|  int32_t positionY | Y-axis layout coordinate of the component relative to its parent component, in px. |
...
| Type               | Description|
|-------------------| -- |
| [ArkUI_IntSize](capi-arkui-nativemodule-arkui-intsize.md) | Width and height of the component, wrapped in an **ArkUI_IntSize** struct.|
...
| Type                 | Description|
|---------------------| -- |
| [ArkUI_IntOffset](capi-arkui-nativemodule-arkui-intoffset.md) | Offset relative to the parent node after the component layout is complete, in px. |
...
|------------------------------------------------------------------| -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) node | Target node.|
| [ArkUI_LayoutConstraint](capi-arkui-nativemodule-arkui-layoutconstraint.md)* Constraint | Pointer to the size constraint used during measurement of the target component. The unit is px. |
...
Lays out a node and specifies the expected position of the node relative to its parent node.
...
| -- | -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) node | Target node.|
|  int32_t positionX | Expected X-axis coordinate of the target component relative to its parent component, in px. |
|  int32_t positionY | Expected Y-axis coordinate of the target component relative to its parent component, in px. |
...
Removes the event callback function previously added to the component through [addNodeEventReceiver](#addnodeeventreceiver). After being removed, the callback no longer receives events generated by this component.
...
| void (\*eventReceiver)([ArkUI_NodeEvent](capi-arkui-nativemodule-arkui-nodeevent.md)\* event)                    | Pointer to the component event callback function to be removed. It must be the same as the function pointer passed when [addNodeEventReceiver](#addnodeeventreceiver) is called. |
...
Removes the custom event callback function previously added to the component through [addNodeCustomEventReceiver](#addnodecustomeventreceiver). After removed, the callback no longer receives custom events generated by this component.
...
| void (\*eventReceiver)([ArkUI_NodeCustomEvent](capi-arkui-nativemodule-arkui-nodecustomevent.md)\* event)              | Pointer to the custom component event callback to be removed. It must be the same as the function pointer passed when [addNodeCustomEventReceiver](#addnodecustomeventreceiver) is called. |
...
| Type| Description|
| -- | -- |
| void* | Pointer to the user-defined data previously saved on this component through [setUserData](#setuserdata). |
...
Sets the unit used when setting length-type attributes of a component through the native API of the Node type. When the service needs to uniformly use px, vp, or fp to set length-type attributes of a component, this API can be called to specify the unit.
...
| [ArkUI_LengthMetricUnit](capi-native-type-h.md#arkui_lengthmetricunit) unit | Unit type for component length attributes. The default value is **ARKUI_LENGTH_METRIC_UNIT_DEFAULT**. When the default value is used, the unit for font-related attributes is fp, and the unit for non-font attributes is vp. |
...
| Type| Description|
| -- | -- |
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) | Pointer to the parent node, or **NULL** if no parent node exists. |
