...
<!-- md-trans-meta sourceCommit=ea6a8d62bacae30e2c1e783234c6e760940e37e1 translatedAt=2026-09-20T09:11:18.397Z pushedAt=2026-09-21T12:10:49.154Z -->
...
| Name | Description |
| -- | -- |
| [ArkUI_GestureRecognizer* (\*createTapGesture)(int32_t countNum, int32_t fingersNum)](#createtapgesture) | Creates a tap gesture. The gesture recognizer returned after the gesture is created successfully can be added to a node through **addGestureToNode()**. When the gesture is no longer used, call **removeGestureFromNode()** to remove the gesture recognizer from the node, and then call **dispose()** to release the resources. After the release, the gesture recognizer must not be used again. 1. This API is used to trigger a tap gesture with one, two, or more taps. 2. When multi-tap is configured, the timeout between the last finger up of the previous tap and the first finger down of the next tap is 300 ms. 3. If the distance between the last tapped position and the current tapped position exceeds 60 vp, gesture recognition fails. 4. When multiple fingers are configured for a gesture, the recognition fails if the number of fingers touching the screen within 300 ms of the first finger's being pressed is less than the required count, or if the number of fingers lifted from the screen within 300 ms of the first finger's being lifted is less than the required count. 5. When the number of fingers touching the screen exceeds the set value, the gesture can be recognized. |
...
| [ArkUI_GestureRecognizerType (\*getGestureType)(ArkUI_GestureRecognizer* recognizer)](#getgesturetype) | Obtains the gesture type. |
...
| [ArkUI_GestureRecognizer* (\*createTapGestureWithDistanceThreshold)(int32_t countNum, int32_t fingersNum, double distanceThreshold)](#createtapgesturewithdistancethreshold) | Creates a tap gesture with a movement range limit. The gesture recognizer returned after the gesture is created successfully can be added to a node through **addGestureToNode()**. When the gesture is no longer used, call **removeGestureFromNode()** to remove the gesture recognizer from the node, and then call **dispose()** to release the resources. After the release, the gesture recognizer must not be used again. 1. This API is used to trigger a tap gesture with one, two, or more taps. 2. If multi-tap is configured, the timeout between the last finger up of the previous tap and the first finger down of the next tap is 300 ms. 3. If the distance between the last tapped position and the current tapped position exceeds 60 vp, gesture recognition fails. 4. When multiple fingers are configured for a gesture, the recognition fails if the number of fingers touching the screen within 300 ms of the first finger's being pressed is less than the required count, or if the number of fingers lifted from the screen within 300 ms of the first finger's being lifted is less than the required count. 5. When the number of fingers touching the screen exceeds the set value, the gesture can be recognized. 6. If the finger moves beyond the preset distance limit, gesture recognition fails. |
...
Creates a tap gesture. The gesture recognizer returned after the gesture is created successfully can be added to a node through **addGestureToNode()**. When the gesture is no longer used, remove the gesture recognizer from the node through **removeGestureFromNode()**, and then call **dispose()** to release the resources. After the release, the gesture recognizer must not be used again.
...
| Name | Description |
| -- | -- |
| int32_t countNum | Number of consecutive taps to recognize. If the value is less than 1, the default value **1** is used. |
...
|  int32_t durationNum | Minimum duration that triggers the long press, in milliseconds (ms). If the value is set to less than or equal to 0, the default value **500** is used. When the component supports dragging by default, if the long press trigger time is less than 500 ms, the long press event takes precedence over the drag event; if the long press trigger time is greater than or equal to 500 ms, the drag event takes precedence over the long press event. |
...
|  double angleNum | Minimum angle change required to trigger the rotation gesture. The value range is (0, 360], in deg. Default value: **1**. If the angle change is less than or equal to 0 or greater than 360, the default value **1** is used. |
...
1. This API is used to implement a swipe gesture, which can be recognized when the swipe speed (px/s) is higher than that specified by **speedNum**.
...
| Type | Description |
| -- | -- |
| [ArkUI_GestureRecognizer](capi-arkui-nativemodule-arkui-gesturerecognizer.md)* | Pointer to the created gesture group, which can be used to add, remove, or manage child gestures in a group. |
...
| Name | Description |
| -- | -- |
| [ArkUI_GestureRecognizer](capi-arkui-nativemodule-arkui-gesturerecognizer.md)* group | Pointer to the gesture group to which the child gesture is to be associated. |
...
| void (\*targetReceiver)(ArkUI_GestureEvent* event, void* extraParams)                 | Pointer to the gesture event callback function, which is used to process the events of the registered gesture types. **event** indicates the gesture callback data, and **extraParams** indicates the context data passed during registration. No value is returned. |
...
Sets the node gesture interruption callback, for scenarios where you need to determine whether the gesture continues to be recognized based on service conditions, such as handling gesture conflicts between parent and child nodes or dynamically controlling gesture responses.
...
| Type                                                                                  | Description |
|-------------------------------------------------------------------------------------| -- |
| [ArkUI_GestureRecognizerType](capi-native-gesture-h.md#arkui_gesturerecognizertype) | Gesture type. |
...
| void* userData | Pointer to the user-defined data, which is passed through as the context data of the parallel inner gesture event callback after being set, for you to use when processing the callback. |
| ArkUI_GestureRecognizer* (\*parallelInnerGesture)(ArkUI_ParallelInnerGestureEvent* event) | Pointer to the parallel inner gesture event callback, used to return the pointer to the gesture recognizer that needs to be recognized in parallel with the inner gesture based on the parallel inner gesture event data. Here, **event** indicates the parallel inner gesture event data, and the return value is the pointer to the gesture recognizer that needs to be recognized in parallel. |
...
| Name | Description |
| -- | -- |
| int32_t countNum | Number of consecutive taps to recognize. If the value is set to less than 1 or is not set, it is converted to the default value **1**. |
|  int32_t fingersNum | Number of fingers that trigger the tap. The minimum value is 1 and the maximum value is 10. If the value is less than 1, the minimum value **1** is used. If the value is greater than 10, the maximum value **10** is used. |
