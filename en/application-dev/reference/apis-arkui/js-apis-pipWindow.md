...
| navigationId        | string                                                           | No | Yes| ID of the **Navigation** component. If no value is passed, the page does not need to be cached.<br>1. When the UIAbility (application component) uses [Navigation](arkui-ts/ts-basic-components-navigation.md) to manage pages, set the ID of the **Navigation** component for the PiP controller. This ensures that the original page can be restored from the PiP window.<br>2. When the UIAbility uses [Router](js-apis-router.md) to manage pages, you do not need to set the ID of the **Navigation** component for the PiP controller.<br>3. If the UIAbility has only one page, you do not need to set the navigation ID. The original page can be restored from the PiP window.<br>**Atomic service API**: This API can be used in atomic services since API version 12.|
| handleId<sup>22+</sup>        | number                                                                     | No | Yes| ID of the subpage under the **Navigation** component. After the full-screen window button is touched, the specified page is restored. This parameter applies only to the scenario where the UIAbility (application component) uses [Navigation](arkui-ts/ts-basic-components-navigation.md) to manage pages. You can set this parameter to the ID of a subpage under **Navigation**. The default value is **-1**, indicating that the topmost page in the Navigation stack is restored. You are advised to use [getUniqueId()](arkui-ts/ts-custom-component-api.md#getuniqueid12) to obtain the page ID. When you use page routing provided by [Navigation](arkui-ts/ts-basic-components-navigation.md), you are advised to use the [system routing table](../../ui/arkts-navigation-cross-package.md#system-routing-table). Otherwise, the page ID obtained by calling [getUniqueId()](arkui-ts/ts-custom-component-api.md#getuniqueid12) may be incorrect.<br>**Atomic service API**: This API can be used in atomic services since API version 22.|
...
| contentWidth        | number                                                                     | No | Yes| Width of the original content, in px. The value is a positive integer. It is used to determine the aspect ratio of the PiP window. When a **PiPController** is created using **typeNode** (that is, the **contentNode** parameter is passed using [PiPWindow.create()](#pipwindowcreate12)), the default value is **1920** if this parameter is not specified. When a **PiPController** is created without using **typeNode** (that is, the **contentNode** parameter is not passed using [PiPWindow.create()](#pipwindowcreate)), the default value is the width of [XComponent](arkui-ts/ts-basic-components-xcomponent.md) if this parameter is not specified.<br>**Atomic service API**: This API can be used in atomic services since API version 12.                                                            |
| contentHeight       | number                                                           | No | Yes| Height of the original content, in px. The value is a positive integer. It is used to determine the aspect ratio of the PiP window. When a **PiPController** is created using **typeNode** (that is, the **contentNode** parameter is passed using [PiPWindow.create()](#pipwindowcreate12)), the default value is **1080** if this parameter is not specified. When a **PiPController** is created without using **typeNode** (that is, the **contentNode** parameter is not passed using [PiPWindow.create()](#pipwindowcreate)), the default value is the height of [XComponent](arkui-ts/ts-basic-components-xcomponent.md) if this parameter is not specified.<br>**Atomic service API**: This API can be used in atomic services since API version 12.                                                            |
...
| defaultWindowSizeType<sup>19+</sup>| number                                                                     | No  | Yes |  Size of the PiP window that the current application starts for the first time. The value range is [0, 2] and the value must be an integer.<br>**0**: no size is set. The PiP window is started based on the size before the PiP window of the previous application is closed.<br>**1**: small window.<br>**2**: large window.<br>If no value is passed, **0** is used.<br>**Atomic service API**: This API can be used in atomic services since API version 19.                                                                |
| cornerAdsorptionEnabled<sup>22+</sup>| boolean                                                                     | No  | Yes |  Whether the PiP window automatically snaps to screen corners. When this feature is enabled, the screen is divided into four hot zones (top-left, top-right, bottom-left, and bottom-right). When the PiP window is displayed, it appears in the corresponding corner of the screen based on the position where it disappears last time. The user can drag the window freely, and once the user releases the window, it automatically snaps to the screen edge.<br>**true**: enables corner snapping.<br>**false**: disables corner snapping.<br>The default value is **true**.<br>**Device behavior differences**: This API can be properly called on phones and tablets. If it is called on other device types, it has no effect.<br>**Atomic service API**: This API can be used in atomic services since API version 22.                                                                |
...
| Name  | Type| Read-Only| Optional| Description      |
| ------ | -------- | ---- | ---- | ---------- |
| windowId  | number   | No  | No  | ID of the PiP window, which uniquely identifies the current PiP window instance.|
...
| Name                  | Value  | Description                   |
|----------------------|-----|-----------------------|
| PLAY       | 1   | Playback state. It is used for the **VIDEO_PLAY_PAUSE** component to indicate that the video is being played.         |
| PAUSE    | 0   | Paused state. It is used for the **VIDEO_PLAY_PAUSE** component to indicate that the video is paused.          |
| OPEN    | 1   | Enabled state. It is used for the **MICROPHONE_SWITCH**, **CAMERA_SWITCH**, or **MUTE_SWITCH** components to indicate that the microphone, camera, or mute feature is enabled.           |
| CLOSE       | 0   | Disabled state. It is used for the **MICROPHONE_SWITCH**, **CAMERA_SWITCH**, or **MUTE_SWITCH** components to indicate that the microphone, camera, or mute feature is disabled.         |
...
| Name   | Type    | Mandatory | Description                                    |
|--------|--------|-----|----------------------------------------|
| width  | number | Yes  | Width of the media content, in px. The value must be an integer greater than 0. This parameter is used to update the aspect ratio of the PiP window. |
| height | number | Yes  | Height of the media content, in px. The value must be an integer greater than 0. This parameter is used to update the aspect ratio of the PiP window. |
...
| Name   | Type    | Mandatory | Description                                                                                                |
|--------|--------|-----|----------------------------------------------------------------------------------------------------|
| controlType  | [PiPControlType](#pipcontroltype12)  | Yes  | Type of the PiP controller. Currently, only the **VIDEO_PLAY_PAUSE**, **MICROPHONE_SWITCH**, **CAMERA_SWITCH**, and **MUTE_SWITCH** component types are supported. If other component types are passed, they do not take effect and no error is reported.|
| status | [PiPControlStatus](#pipcontrolstatus12)  | Yes  | Status of the PiP controller.                                                                                    |
...
Sets the enabled status of the PiP controller.
...
| Name        | Type    | Mandatory | Description                                    |
|-------------|--------|-----|----------------------------------------|
| controlType | [PiPControlType](#pipcontroltype12)  | Yes  | Type of the PiP controller. |
| enabled     | boolean | Yes  | Enabled status of the PiP controller. **true** if enabled, **false** otherwise. |
...
Subscribes to PiP controller action events. To avoid potential memory leaks, you are advised to stop listening when it is no longer needed. The [on('controlEvent')](#oncontrolevent12) API is preferred.
...
Subscribes to PiP controller action events. To avoid potential memory leaks, you are advised to stop listening when it is no longer needed.
...
Unsubscribes from PiP controller action events. The [off('controlEvent')](#offcontrolevent12) API is preferred.
...
Unsubscribes from PiP controller action events.
