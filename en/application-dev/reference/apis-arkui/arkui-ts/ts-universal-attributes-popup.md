...
>
>  - The display status of the popup bubble is reported in the callback of the onStateChange attribute in [PopupOptions](#popupoptions) or [CustomPopupOptions](#custompopupoptions8). When the component bound to the popup is destroyed, the popup disappears, but the display status is not reported through onStateChange.
>
...
>
>  - By default, the PC/2in1 device has double outlines, while other devices do not.
>
...
>
>  - When the component bound to the popup is destroyed, the screen is rotated, or the app enters the background, the popup automatically disappears, and the [onWillDismiss](#popupoptions) event is not triggered.
...
| placement<sup>10+</sup>               | [Placement](ts-appendix-enums.md#placement8)                 | No  | Yes | Display position of the popup component relative to the host node. The default value is **Placement.Bottom**.<br>If both **placementOnTop** and **placement** are set, the latter prevails. If the popup component cannot be completely displayed in the specified position, the popup component automatically adjusts its position so that it can be completely displayed.<br>**Atomic service API**: This API can be used in atomic services since API version 11.|
...
| width<sup>11+</sup>                   | [Dimension](ts-types.md#dimension10)                         | No  | Yes | Width of the bubble. If this parameter is not set or the value is invalid, the bubble width is automatically adjusted to adapt to the content width.<br>Unit: vp.<br><br>**Atomic service API**: This API can be used in atomic services since API version 12.|
...
| borderWidth<sup>20+</sup>| [Dimension](ts-types.md#dimension10)  | No  | Yes | Width of the inner outline of the Popup component.<br>Default value: **1**.<br>Unit: vp.<br>**NOTE**<br>1. Percentage values are not supported. If a percentage value is set, the value **0** is used.<br>2. If no inner outline is set, this API must be used together with borderLinearGradient.<br>3. For double outlines, it is recommended that the inner outline width should not exceed 10 vp.<br>**Atomic service API**: This API can be used in atomic services since API version 20.|
...
This example shows how to configure the **keyboardAvoidMode** property in [PopupOptions](#popupoptions) or [CustomPopupOptions](#custompopupoptions8) to determine whether the popup avoids the soft keyboard.

The **keyboardAvoidMode** property is added to **PopupOptions** and CustomPopupOptions since API version 15.
...
This example configures the transition attribute in [PopupOptions](#popupoptions) or [CustomPopupOptions](#custompopupoptions8) to implement the popup animation.
...
This example shows how to use the **onWillDismiss** property in [PopupOptions](#popupoptions) to intercept popup dismissal events and execute callback functions.
...
This example demonstrates intercepting the popup dismissal event by setting the onWillDismiss property of [PopupOptions](#popupoptions) to false. In addition, configuring the **followTransformOfTarget** property in [PopupOptions](#popupoptions) enables setting whether the popup follows and displays at the corresponding position when the host component's position is transformed.
...
This example configures the outlineWidth, borderWidth, outlineLinearGradient, and borderLinearGradient attributes in [PopupOptions](#popupoptions) to set the color and direction of the linear gradient of the inner and outer strokes of the bubble.
...
### Example 8: Setting the Avoid Host Mode for Bubbles

This example configures the avoidTarget attribute of [PopupOptions](#popupoptions) to make the bubble avoid the bound component.
