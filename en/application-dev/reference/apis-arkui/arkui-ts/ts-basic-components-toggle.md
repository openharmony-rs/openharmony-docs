...
> **NOTE**
>
> The toggle type inherits from the default value of the corresponding component type and cannot be set. For example, if the toggle type is **Button**, it inherits the default value of [ButtonType](ts-basic-components-button.md#buttontype). Since API version 18, the default button type has changed from capsule to rounded rectangle. Capsule buttons do not support the [borderRadius](ts-universal-attributes-border.md#borderradius) API. Therefore, setting **borderRadius** for the **Toggle** component does not take effect.
