...
| onTipClick | ()=>void | No| - | Callback invoked when the tip text on the left is clicked. No operation is performed if this parameter is not specified.|
| onActionTextClick | ()=>void | No| - | Callback invoked when the icon on the right is clicked. No operation is performed if this parameter is not specified.|
...
| tip | [ResourceStr](ts-types.md#resourcestr)                                           | No| Yes| Text content of the exception prompt.<br>By default, the following text resources are provided:<br>1. **ohos_network_not_connected**: displayed when there is no Internet connection.<br>2. **ohos_network_connected_unstable**: displayed when the Internet connection is unstable.<br>3. **ohos_unstable_connect_server**: displayed when the server fails to be connected.<br>4. **ohos_custom_network_tips_left**: displayed when an Internet connection is available but the location fails to be obtained.<br>If this parameter is not set or is set to **undefined**, the text content is not displayed.<br>**Atomic service API**: This API can be used in atomic services since API version 12.|
