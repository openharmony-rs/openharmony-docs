...
| defaultCreator | [StorageDefaultCreator\<T\>](#storagedefaultcreatort) | No | Constructor for obtaining the default value. The default value is **undefined**. If the data is not stored in AppStorageV2 and no default constructor is passed, **undefined** is returned. |
...
Provides persistent storage for UI states. This API is inherited from [AppStorageV2](#appstoragev2). It supports persisting application state data to disks and restoring data after application restart, making it suitable for scenarios where UI state data needs to be retained. For details about the UI usage, see [PersistenceV2: Persisting UI States](../../ui/state-management/arkts-new-persistencev2.md).
...
| Name  |Type  |Mandatory  | Description                                                     |
| ------------- | ------------|-------------------|-------------------------- |
| type    |[ConnectOptions\<T\>](#connectoptionst18)    |Yes  | Configuration options of **globalConnect**, which include the specified type, key, default constructor, encryption level, and other configuration items. For details, see **ConnectOptions**. |
...
// Use key:global3 for connection and set the encryption parameter ranging from 0 to 4; otherwise, a crash occurs. In this case, EL4 is set.
...
Stores key-value pair data on the application disk. Supports the persistence of following collection types: [`Array`, `Map`, `Set`, `collections.Array`, `collections.Map`, and `collections.Set`](../../ui/state-management/arkts-new-persistencev2.md#types-supported-by-globalconnect). Note that when persisting data of the `Array<ClassA>` type, you need to call [`makeObserved`](#makeobserved) to make the returned object observable. Multi-level nested sets are not supported, for example, persisting `Array<Array<ClassA>>` is not supported.
...
| Name  | Type  | Mandatory| Description              |
| -------- | ------ | ---- | ---------------------- |
| type | [ConnectOptionsCollections\<T, S\>](#connectoptionscollectionst-s23) \| [ConnectOptions\<T\>](#connectoptionst18) | Yes | Configuration options of **globalConnect**. It supports **ConnectOptions** and **ConnectOptionsCollections** types, and includes configuration items such as the type, key, default creator, and collection item creator. For details, see the parameter descriptions of **ConnectOptions** and **ConnectOptionsCollections**.<br>If **defaultSubCreator** is provided in **ConnectOptionsCollections**, **defaultCreator** must be provided. Otherwise, the persistence fails. In addition, the collection item type **S** must be the same as the return type of **defaultSubCreator**. If the return types are inconsistent, an error will be reported during compilation.|
...
> **NOTE**
>
> Since changes to non-[\@Trace](../../ui/state-management/arkts-new-observedV2-and-trace.md) data do not trigger automatic persistence of [PersistenceV2](../../ui/state-management/arkts-new-persistencev2.md), when non-\**@Trace** data changes and needs to be persisted, you can call this API to persist the data of the corresponding key.
>
...
| Name  | Type  | Mandatory| Description              |
| -------- | ------ | ---- | ---------------------- |
| callback | [PersistenceErrorCallback](#persistenceerrorcallback) \| undefined  | Yes   | Callback called when persistence fails. The callback parameters include **key** (the key that caused the error), **reason** (the type of error cause, which can be **'quota'**, **'serialization'**, or **'unknown'**), **message** (detailed error information), and **oldValue** (optional, indicating the old data returned when deserialization fails). |
...
|areaMode      | [contextConstant.AreaMode](../apis-ability-kit/js-apis-app-ability-contextConstant.md#areamode)   |No   |Yes    |Encryption level, ranging from EL1 to EL5 (corresponding to the value from 0 to 4). For details, see [Encryption Levels](../../application-models/application-context-stage.md#obtaining-and-modifying-encryption-levels). If no value is passed in, EL2 is used by default. Different encryption levels correspond to different encryption partitions, that is, different storage paths. If the passed encryption level value is not within the range of 0 to 4, a crash will occur during running. When the same key uses different encryption levels, the encryption level in the first **globalConnect** call is used. |
...
When `StorageDefaultCreator<S>` returns `undefined` or `null`, persistence will fail. When `StorageDefaultCreator<S>` is directly set to `undefined` or `null`, the state management framework persists the data according to the original type (such as the `Object` type), but the methods in the `class` object will be lost. In the following example, when `StorageDefaultCreator<S>` is directly set to `undefined` or `null`, the `report` method in the `ClassA` object will be lost during persistence.
...
struct Comp {
  ...
  @Local arr: Array<ClassA> = PersistenceV2.globalConnect({
    ...
    // If defaultSubCreator is directly set to `undefined` or `null` (defaultSubCreator: undefined), the method in `ClassA` will be lost during the persistence.
...
| reason | string | No | No | Reason for the object's observability.<br>For the object that cannot be observed: The object itself cannot be observed.<br>For the object that can be observed:<br> 1. The V1 object is decorated by the [@Observed](./../../ui/state-management/arkts-observed-and-objectlink.md) decorator or converted by the [makeV1Observed](#makev1observed19) method. <br> 2. The V1 object is decorated by the [@Observed](./../../ui/state-management/arkts-observed-and-objectlink.md) decorator or converted by the [makeV1Observed](#makev1observed19) method, but the object is not used by a UI component. <br> 3. The V1 object is converted by the [enableV2Compatibility](#enablev2compatibility19) method and then passed to a V2 component. <br> 4. The V1 object is converted by the [enableV2Compatibility](#enablev2compatibility19) method and then passed to a V2 component, but is not used by the V2 component. <br> 5. The V2 object is decorated by the [@ObservedV2/@Trace](./../../ui/state-management/arkts-new-observedV2-and-trace.md) decorator.<br> 6. The V2 object is converted by the [makeObserved](#makeobserved) method. <br> 7. The V2 object is of the **Array**, **Map**, **Set**, or **Date** type. <br> 8. The V2 object is decorated by the [@ObservedV2/@Trace](./../../ui/state-management/arkts-new-observedV2-and-trace.md) decorator, but the object is not used by a UI component. <br> 9. The V2 object is converted by the [makeObserved](#makeobserved) method, but is not used by a UI component. <br> 10. The V2 object is of the **Array**, **Map**, **Set**, or **Date** type, but is not used by a UI component. |
...
| owningComponentOrClassName | string  | No | No   | Component or object name. For a V1 object, the component name is returned.<br> For a V1 object whose properties are decorated by the [@Track](./../../ui/state-management/arkts-track.md) decorator, the object name is returned.<br> For a V2 object, the object name is returned. |
| owningComponentId | number  | No | No   | Component ID. For a V1 object, the component ID is returned.<br> **If a V1 object has a property that uses [@Track](./../../ui/state-management/arkts-track.md), no component ID is available, and -1 is returned. In the same case, no component ID is available for a V2 object, and -1 is returned.** |
...
| Name| Type| Mandatory| Description    |
| ------ | ---- | ---- | ------------ |
| source | T | Yes | Data source object. It supports classes not decorated by **@Observed** or **@ObservedV2**, objects returned by **JSON.parse**, and classes decorated by **@Sendable**.<br>**Array**, **Map**, **Set**, and **Date** types are supported.<br>collections.Array, collections.Set, and collections.Map are supported.<br>For details about the usage rules, see [makeObserved API: Changing Unobservable Data to Observable Data](../../ui/state-management/arkts-new-makeObserved.md). |
...
| Name| Type| Mandatory| Description    |
| ------ | ---- | ---- | ------------ |
| source | T    | Yes   | Data source. Common classes, **Array**, **Map**, **Set**, and **Date** types are supported.<br>[@arkts.collections (ArkTS containers)](../apis-arkts/arkts-apis-arkts-collections.md) and classes decorated by [@Sendable](../../arkts-utils/arkts-sendable.md) are not supported.<br>**undefined** and **null** are not supported. State management V2 data and the return value of [makeObserved](#makeobserved) are not supported. |
...
| Type| Description                                            |
| ---- | ------------------------------------------------ |
| [Binding\<T\>](#bindingt20)    | Read-only one-way data binding instance with a **value** attribute, which is used to obtain the currently bound value. The value can only be read and cannot be directly modified.|
...
| Type| Description                                            |
| ---- | ------------------------------------------------ |
| [MutableBinding\<T\>](#mutablebindingt20)    | Two-way data binding instance with a **value** attribute, which allows you to read and modify data. If the value is set, the system checks whether the value type matches the generic type **T**.|
...
| Name| Type| Mandatory| Description    |
| ------ | ---- | ---- | ------------ |
| target | object | Yes | Target object. Only instances of [\@ComponentV2](../../ui/state-management/arkts-create-custom-components.md#componentv2) and [\@ObservedV2](../../ui/state-management/arkts-new-observedV2-and-trace.md) are supported.<br>For unsupported types, a runtime error is thrown. For details about the error codes, see the table below. |
| path | string \| string[] | Yes | Path of the variable name to be listened for. You can specify a path or pass a string array to specify multiple variable paths to be listened for at a time.<br>Only string and string arrays are supported. For unsupported types, a runtime error is thrown. For details about the error codes, see the table below. |
...
| ------ | ---- | ---- | ------------ |
| target | object | Yes | Target object. Only instances of [\@ComponentV2](../../ui/state-management/arkts-create-custom-components.md#componentv2) and [\@ObservedV2](../../ui/state-management/arkts-new-observedV2-and-trace.md) are supported.<br>For unsupported types, a runtime error is thrown. For details about the error codes, see the table below. |
| path | string \| string[] | Yes | Path of the variable name for which the listener is to be deleted. You can specify a single path or pass a string array to delete listeners of multiple state variables at a time.<br>Only strings and arrays are supported. For unsupported types, a runtime error is thrown. For details about the error codes, see the table below. |
| monitorCallback | [MonitorCallback](#monitorcallback20) | No | Callback to be deleted.<br>If this parameter is not passed, all listeners registered for the variable corresponding to the path are deleted.<br>For unsupported types, a runtime error is thrown. For details about the error codes, see the table below. |
...
| Parameter| Type| Read-Only| Optional| Description    |
| ------ | ---- | ---- | ---- | ------------ |
|isSynchronous|boolean|No|Yes|Whether the current callback is a synchronous callback. The value **true** indicates a synchronous callback. The default value is **false**, which indicates an asynchronous callback.<br>**Atomic service API:** This API can be used in atomic services since API version 20.|
...
| Name| Type| Mandatory| Description    |
| ------ | ---- | ---- | ------------ |
| ...args | any | No | Constructor arguments passed when creating an instance of type **T**, used to initialize the instance. |
...
// Persist the key-value pair with the key FatherSampleClass and the value as a new object of FatherSampleClass(), and assign it to source.
const source: FatherSampleClass | undefined = PersistenceV2.connect(FatherSampleClass, () => new FatherSampleClass());
...
| oldValue | string    | No   | Old serialized data stored on the disk when deserialization fails. In non-deserialization failure scenarios, the default value of this parameter is **undefined**.<br> **Since:** 26.0.0.   |
...
| Name| Type| Mandatory| Description    |
| ------ | ---- | ---- | ------------ |
| type | [TypeConstructor\<T\>](#typeconstructort)    | Yes   | Type of the class property. Only custom class types are supported. Passing other types will cause persistence failure. |

**Return value**

| Type| Description                                            |
| ---- | ------------------------------------------------ |
| PropertyDecorator    | Property decorator, used to decorate properties that belong to custom classes in nested classes. |
...
| Type| Description                                            |
| ---- | ------------------------------------------------ |
| T    | Value obtained by the callback function. |
...
| Type            | Description                                                        |
| -------------------- | ------------------------------------------------------------ |
| T | Currently bound value. The return value type is the generic parameter **T**, which is consistent with the type defined by **Binding\<T\>**. |
...
| Type            | Description                                                        |
| -------------------- | ------------------------------------------------------------ |
| T | Currently bound value. The return value type is the generic parameter **T**, which is consistent with the type defined by **MutableBinding\<T\>**. |
