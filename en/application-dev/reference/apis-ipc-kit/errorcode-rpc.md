...
1. Check whether the memory size specified when calling **Ashmem::create()** exceeds the system limit.
...
Failed to read data from **MessageSequence**. **MessageSequence** uses sequential read/write, and the read sequence must be strictly consistent with the write sequence.
...
1. Check whether the input parameter **fd** is still valid (for example, the **fd** value is greater than or equal to 0 and corresponding file descriptor has not been closed).
