...
| [OH_AVErrCode OH_VideoDecoder_SetCallback(OH_AVCodec *codec, OH_AVCodecAsyncCallback callback, void *userData)](#oh_videodecoder_setcallback) | Sets an asynchronous callback so that your application can respond to events generated by a video decoder. This function must be called prior to **OH_VideoDecoder_Prepare**. (It is deprecated from API version 11.)|
...
| [OH_AVErrCode OH_VideoDecoder_FreeOutputData(OH_AVCodec *codec, uint32_t index)](#oh_videodecoder_freeoutputdata) | Frees an output buffer of a video decoder. (It is deprecated from API version 11.)|
...
Obtains the OH_AVFormat information about the output data of a video decoder. For details, see [OH_AVFormat](capi-core-oh-avformat.md).<br> You must call **OH_AVFormat_Destroy** to release the OH_AVFormat instance when its lifecycle ends.
