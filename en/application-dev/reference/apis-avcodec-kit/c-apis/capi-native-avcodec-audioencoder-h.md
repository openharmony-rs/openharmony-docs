...
| [OH_AVCodec *OH_AudioEncoder_CreateByName(const char *name)](#oh_audioencoder_createbyname) | Creates an audio encoder instance based on an encoder name. To use this function, you must know the exact name of the encoder.(Deprecated in API11) |
...
| [OH_AVErrCode OH_AudioEncoder_Configure(OH_AVCodec *codec, OH_AVFormat *format)](#oh_audioencoder_configure) | Configures an audio encoder. Typically, you need to configure the description information about the audio track to be encoded. This function must be called prior to **Prepare**.(Deprecated in API11) |
...
| [OH_AVErrCode OH_AudioEncoder_Start(OH_AVCodec *codec)](#oh_audioencoder_start) | Starts an audio encoder after it is prepared successfully. After being started, the encoder starts to report the **OH_AVCodecOnNeedInputData** event.(Deprecated in API11) |
...
| [OH_AVFormat *OH_AudioEncoder_GetOutputDescription(OH_AVCodec *codec)](#oh_audioencoder_getoutputdescription) | Obtains the description information about the output data of an audio encoder. The caller must manually release the OH_AVFormat instance in the return value.(Deprecated in API11) |
| [OH_AVErrCode OH_AudioEncoder_SetParameter(OH_AVCodec *codec, OH_AVFormat *format)](#oh_audioencoder_setparameter) | Sets dynamic parameters for an audio encoder.<br> This function can be called only after the encoder is started. Incorrect parameter settings may cause encoding failure.(Deprecated in API11) |
| [OH_AVErrCode OH_AudioEncoder_PushInputData(OH_AVCodec *codec, uint32_t index, OH_AVCodecBufferAttr attr)](#oh_audioencoder_pushinputdata) | Notifies the audio encoder that the input data has been written to the buffer identified by **index**.<br> The {@link OH_AVCodecOnNeedInputData} callback reports the available input buffer and the index. After being<br>pushed to the encoder, a buffer is not accessible until the buffer with the same index is reported again through the<br>{@link OH_AVCodecOnNeedInputData} callback.<br> In addition, some encoders require the input of specific data to initialize the encoding process.(Deprecated in API11) |
...
| -- | -- |
| OH_AVCodec *codec | Pointer to an OH_AVCodec instance. |
| OH_AVFormat *format | Pointer to an OH_AVFormat instance, which provides the description information about the audio track to be encoded. |
...
Notifies the audio encoder that the input data has been written to the buffer identified by **index**.<br> The {@link OH_AVCodecOnNeedInputData} callback reports the available input buffer and the index. After being<br>pushed to the encoder, a buffer is not accessible until the buffer with the same index is reported again through the<br>{@link OH_AVCodecOnNeedInputData} callback.<br> In addition, some encoders require the input of specific data to initialize the encoding process.
