...
| [parseSendable](arkts-arkts-json-parsesendable-f.md) | Parses a JSON string into a Sendable object graph that can be transferred across concurrent instances (Worker or TaskPool) without copy. When parsed JSON data needs to be shared across threads, use this API instead of [parse](arkts-arkts-json-parse-f.md): the result is created directly in the shared heap and is accessible from all concurrent instances after the call returns. |
