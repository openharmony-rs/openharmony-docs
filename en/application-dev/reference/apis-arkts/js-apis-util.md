...
Converts the UUID of the string type generated by generateRandomUUID into the UUID generated by [util.generateRandomBinaryUUID](#utilgeneraterandombinaryuuid9), which complies with RFC 4122.
...
// Enable multi-thread detection.
util.ArkTSVM.setMultithreadingDetectionEnabled(true);
// Disable multi-thread detection.
...
Obtains the heap memory information of all VM threads, including the thread ID, thread name, heap type, and heap object size. This API uses a promise to return the result.

The heaps obtained by this API are classified into two types: local heap, which is the VM heap exclusive to each ArkTS thread in the application process; and shared heap, which is the VM heap shared by all ArkTS threads in the application process.

> **NOTE**
>
> When this API is called, all VM threads are suspended to obtain memory information. Because all VM threads need to be suspended, it may take a long time to call this API in heavy-load scenarios.
...
| Type| Description|
| -------- | -------- |
| Promise<[HeapMemoryInfo](js-apis-util.md#heapmemoryinfo24)[]> | Promise object, which is parsed into an array of HeapMemoryInfo objects. Each object contains the thread ID, thread name, heap type, and heap object size. This method can be used to obtain the memory information of the local and shared heaps.|
...
When the asynchronous event loop mechanism of EventHandler and libuv executes an asynchronous task, the task will jump out of the current handle scope. If you do not add a scope in the task callback, memory leakage will occur. After this API is called, tasks of the two mechanisms can be executed within the scope, preventing memory leakage.
    ...
    int ret = uv_queue_work(loop, work,
        ...
        [](uv_work_t* work, int status){
            ...
            for (int i = 0; i < 1000; i++) {
                ...
                // If no scope is added in the asynchronous mechanism provided by libuv, memory leakage will occur.
...
``` TypeScript
// Index.ets ArkTS sample code
import { hilog } from '@kit.PerformanceAnalysisKit';
import testNapi from 'libentry.so';
import { util } from '@kit.ArkTS';

try {
  // If the LocalHandle memory leak fallback mechanism is not enabled, memory overflow may occur. After this mechanism is enabled, the system automatically reclaims the napi_value created in the EventHandler and libuv asynchronous tasks.
...
If the memory leak object is still used, the system reclaims the memory leak object after the enableLocalHandleDetection API is invoked. If the object is still used, the memory leak issue will turn into a stability issue.
...
napi_value dangerous_function(napi_env env, napi_callback_info info) {
    ...
    global_js_object = js_obj; // Directly stored in the global variable and released after the LocalHandle memory leak fallback mechanism is enabled.
    return nullptr;
}
```
...
Registers a callback function. After the main thread of the VM completes garbage collection, the callback function is triggered if the heap memory usage exceeds the warning threshold.

The VM determines whether the memory warning threshold is reached by collecting statistics on the size of live objects. Due to memory fragments and floating garbage in the VM heap, it cannot be guaranteed that the callback will be triggered before an OOM occurs.
...
| Name| Type| Mandatory| Description|
| -------- | -------- | -------- | -------- |
| callback | Callback\<string\> | Yes| Callback function triggered when the memory usage reaches the warning threshold after garbage collection. The string parameter indicates the type of the memory pressure event. Currently, there are three event types: "LocalHeapMemPressure", "SharedHeapMemPressure", and "ProcessHeapMemPressure".|
| heapMemoryThreshold | [HeapMemoryThreshold](#heapmemorythreshold24) | Yes| Heap memory warning threshold, which is set in percentage. The value range is [70, 95].|

**Return value**

| Type| Description|
| -------- | -------- |
| boolean | Returns **true** if registration succeeds, and **false** if called not on the main thread or the callback has already been registered.|
...
Cancels the registered memory warning callback function.
...
Heap memory warning threshold, which is used to specify the heap memory warning threshold for triggering the callback.
...
| -------- | -------- | ---- | ---- | ---- |
| localHeapThreshold | number | No| Yes| Local heap memory warning threshold, which is set in percentage. The value range is [70, 95]. If the value is out of the range, it is automatically limited to the valid range. If it is not set, the local heap is not listened.|
| sharedHeapThreshold | number | No| Yes| Shared heap memory warning threshold, which is set in percentage. The value range is [70, 95]. If the value is out of the range, it is automatically limited to the valid range. If it is not set, the shared heap is not listened.|
| processHeapThreshold | number | No| Yes| Total VM heap memory warning threshold of the process, which is set in percentage. The value range is [70, 95]. If the value is out of the range, it is automatically limited to the valid range. If it is not set, the total VM heap size of the process is not listened.|

## HeapMemoryInfo<sup>24+</sup>

Describes the memory information of the local or shared heap, including the thread ID and heap memory size.
...
| Name| Type| Read-Only| Optional| Description|
| -------- | -------- | -------- | -------- | -------- |
| threadId | number | No| Yes| Thread ID. If the memory information describes the local heap, the value is an integer indicating the ID of the running thread. If the memory information describes the shared heap, the value is **undefined**.|
| threadName | string | No| Yes| Thread name. If the memory information describes the local heap, the value is a string indicating the name of the running thread. If the memory information describes the shared heap, the value is **undefined**.|
| heapType | string | No| No| Heap type. Currently, there are two value types: "local" indicates a local heap, and "shared" indicates a shared heap.|
| heapObjectSize | number | No| No| Heap object size, in KB (rounded up to an integer).|
...
| Name| Type| Read-Only| Optional| Description|
| -------- | -------- | ---- | ---- | -------- |
| stream | boolean | No| Yes| Whether subsequent calls to [decodeWithStream](#decodewithstreamdeprecated) are followed by additional data blocks. If data is processed in blocks, set this parameter to **true**. If this is the last data block to process or data is not divided into blocks, set this parameter to **false**. The default value is **false**.|
...
| Name| Type| Mandatory| Description|
| -------- | -------- | -------- | -------- |
| stream | boolean | No| Whether to allow data blocks in subsequent **decode()**. If data is processed in blocks, set this parameter to **true**. If data is not divided into blocks, set this parameter to **false**. The default value is **false**.|
...
| Type       | Description                          |
| ----------- | ------------------------------ |
| IterableIterator&lt;[K, V]&gt; | Two-dimensional array in key-value pairs.|
...
**ArrayBufferView** is a type representing any of the following: Int8Array, Int16Array, Int32Array, Uint8Array, Uint8ClampedArray, Uint32Array, Float32Array, **Float64Array**, and DataView.
...
**TypedArray** is a type representing any of the following: Int8Array, Int16Array, Int32Array, Uint8Array, Uint8ClampedArray, Uint16Array, Uint32Array, Float32Array, Float64Array, BigInt64Array, and BigUint64Array.
