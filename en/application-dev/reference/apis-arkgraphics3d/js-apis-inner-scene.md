...
<!--Tester: @zhangyue283-->
<!--Adviser: @ge-yafang-->
<!-- md-trans-meta sourceCommit=943c6a9d177f6e87860158abc76387727f97f346 translatedAt=2026-08-25T13:20:35.967Z pushedAt=2026-08-26T08:35:46.486Z -->
...
| ---- | ---- | ---- | ---- | ---- |
| name | string | No | No | Name of the resource to be created. It is customizable by the developer and used to identify the scene resource. |
...
| Name | Type | Mandatory | Description |
| ---- | ---- | ---- | ---- |
| params | [SceneResourceParameters](#sceneresourceparameters) | Yes | Parameters used to create the image stream. |

**Return value**

| Type | Description |
| ---- | ---- |
| Promise\<[ImageStream](js-apis-inner-scene-resources.md#imagestream)> | Promise object that returns the created image stream. |
...
function createMeshResource(): Promise<MeshResource> {
  ...
  // Create the mesh resource. The path and file name can be customized according to the actual project resources.
...
function createSamplerResource(): Promise<Sampler> {
  ...
  const renderResourceFactory: RenderResourceFactory = renderContext.getRenderResourceFactory();
  // Create the sampler resource. The path and file name can be customized according to the actual project resources.
...
Special effect parameter configuration, which is used to specify the special effect ID required for creating a special effect. It is used as the input parameter of the [createEffect](#createeffect21) API to create a special effect object.
...
| Type| Description|
| ---- | ---- |
| Promise\<[Node](js-apis-inner-scene-nodes.md#node)> | Promise object, which returns the node object.|
...
| Name| Type| Read Only| Optional| Description|
| ---- | ---- | ---- | ---- | ---- |
| name | string | No | No | Name of the scene component, which is customizable. Used to identify the scene component.|
...
| Name | Type | Read-Only | Optional | Description |
| ---- | ---- | ---- | ---- | ---- |
| shadowAlgorithmType | [ShadowAlgorithmType](js-apis-inner-scene-types.md#shadowalgorithmtype) | Yes | No | Enumeration value of the shadow algorithm. |
...
| Name | Type | Read-Only | Optional | Description |
| ---- | ---- | ---- | ---- | ---- |
| shadowSampleRadius | number \| undefined | No | Yes | Sampling radius, which determines the blur range of the shadow edge. A larger radius results in a softer shadow edge. An excessively large sampling radius causes the shadow to be overly blurred and lose its shape characteristics.<br>Default Value: 5.0.<br>Value Range: >= 0.<br>- When set to 0, PCF sampling is not performed and there is no shadow effect.<br>- When set to undefined, the default value 5.0 is restored for rendering. |
| shadowSampleCount | number \| undefined | No | Yes | Sampling count, which determines the number of times each pixel samples the shadow map. A larger count results in higher shadow quality but greater performance overhead.<br>Default Value: 16.<br>Value Range: 0 to 64.<br>- Values beyond this range are automatically clamped to the nearest valid boundary value (for example, 65 is actually processed as 64).<br>- When set to 0, PCF sampling is not performed and there is no shadow effect.<br>- When set to undefined, the default value 16 is restored for rendering. |
...
| Type| Description|
| ---- | ---- |
| [Node](js-apis-inner-scene-nodes.md#node) \| null | Returns the instance of the requested node. Returns null if not found or if the type of the found node does not match the passed parameter.|
...
| node | [Node](js-apis-inner-scene-nodes.md#node) | Yes| Node to be imported.|
...
| Type| Description|
| ---- | ---- |
| [Node](js-apis-inner-scene-nodes.md#node) | Node to be imported.|
...
| Name| Type| Mandatory| Description|
| ---- | ---- | ---- | ---- |
| node | [Node](js-apis-inner-scene-nodes.md#node) | Yes| Node to be cloned.|
| parent | [Node](js-apis-inner-scene-nodes.md#node) | Yes| Target parent node of the cloned node in the current scene. The cloned node and the target parent node must belong to the same scene.|
...
| Type| Description|
| ---- | ---- |
| [Node](js-apis-inner-scene-nodes.md#node) \| null | Returns the cloned node. If the operation fails, null is returned.|
