...
| static const char * OH_AVMETADATA_EXTRACTOR_HAS_AUDIO = "hasAudio" | Pointer to the key for obtaining the flag indicating whether the media asset contains audio. The value type is int32_t.<br>The value **0** indicates that the media asset does not contain audio, and the value **1** indicates that the media asset contains audio.|
| static const char * OH_AVMETADATA_EXTRACTOR_HAS_VIDEO = "hasVideo" | Pointer to the key for obtaining the flag indicating whether the media asset contains video. The value type is int32_t.<br>The value **0** indicates that the media asset does not contain video, and the value **1** indicates that the media asset contains video.|
...
| static const char * OH_AVMETADATA_EXTRACTOR_SAMPLE_RATE = "sampleRate" | Pointer to the key for obtaining the audio sample rate, in Hz. The value type is int32_t.|
...
| static const char * OH_AVMETADATA_EXTRACTOR_VIDEO_WIDTH = "videoWidth" | Pointer to the key for obtaining the video width, in px. The value type is int32_t.|
...
| static const char * OH_AVMETADATA_EXTRACTOR_VIDEO_IS_HDR_VIVID = "hdrType" | Pointer to the key for obtaining the flag indicating whether the video is an HDR Vivid video. The value type is int32_t.<br>        For details, see [OH_Core_HdrType](../apis-avcodec-kit/capi-media-types-h.md#oh_core_hdrtype) in **media_types.h**.|
...
| -- | -- |
| OH_AVMETADATA_EXTRACTOR_FETCH_FAILED = 0 | The frame extraction operation failed.|
| OH_AVMETADATA_EXTRACTOR_FETCH_SUCCEEDED = 1 | The frame extraction operation is successful.|
| OH_AVMETADATA_EXTRACTOR_FETCH_CANCELED = 2 | The frame extraction operation is canceled by the user.|
