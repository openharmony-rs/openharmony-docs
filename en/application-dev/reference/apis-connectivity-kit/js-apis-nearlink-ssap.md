...
Before using the methods of this class, you need to call [ssap.createClient](#ssapcreateclient) to construct an instance of this class.
...
Initiate a connection request to the server. This API uses a promise to return the result.
...
Initiates a disconnection request to the server to disconnect an existing connection or terminate a connection that is being established. This API uses a promise to return the result.
...
try {
  client = ssap.createClient(addr); // An app only needs to create one instance for a remote device.
  client.connect().then(() => {
    console.info('connect success'); // Set up a connection.
  }).catch((err: BusinessError) => {
    console.error('errCode: ' + err.code + ', errMessage: ' + err.message);
  });
  client.disconnect().then(() => {
    console.info('disconnect success'); // Disconnect the connection.
  }).catch((err: BusinessError) => {
    console.error('errCode: ' + err.code + ', errMessage: ' + err.message);
  });
} catch (err) {
...
Obtains the services supported by the server. This API uses a promise to return the result.
...
| Type| Description|
| -------- | -------- |
| Promise&lt;[Service](#service)[]&gt; | Promise used to return the list of services supported by the server.|
...
Reads a property from the server. This API uses a promise to return the result.
...
| Name| Type| Mandatory| Description|
| -------- | -------- | -------- | -------- |
| property | [Property](#property) | Yes| Property from the server.|

**Return value**

| Type| Description|
| -------- | -------- |
| Promise&lt;[Property](#property)&gt; | Promise used to return the property from the server.|
...
try {
  ...
  // Create a property. During actual development, the property needs to be obtained from the server through the getServices API.
...
| -------- | -------- | -------- | -------- |
| property | [Property](#property) | Yes| Property from the server.|
| writeType | [PropertyWriteType](#propertywritetype) | Yes| Write type. The server can respond with or without returning results.|
...
try {
  ...
  // Create a property. During actual development, the property needs to be obtained from the server through the getServices API.
  let valueBuffer = new ArrayBuffer(8);
  // Value of the property expected to be written.
  let propertyValue = new Uint8Array(valueBuffer);
  propertyValue[0] = 1;
  let property: ssap.Property = {
    serviceUuid:'FFFFFFFF-1234-5678-ABCD-000000004386',
...
Sets a [Property](#property) change notification. This method can only be used after a connection is successfully established by calling [connect](#connect). This API uses a promise to return the result.
...
| enable | boolean | Yes| Whether to enable notification. **true**: Enable notification. **false**: Disable notification.|
...
Initiates an MTU (Maximum Transmission Unit) negotiation request. This API uses a promise to return the result.
...
| Name| Type| Mandatory| Description|
| -------- | -------- | -------- | -------- |
| mtu | number | Yes| MTU, in bytes. The value range is [22, 1024]. The default value is **251**.|
...
| Name| Type| Mandatory| Description|
| -------- | -------- | -------- | -------- |
| callback | Callback&lt;[Property](#property)&gt; | Yes| Callback used to return the property from the server.|
...
Before using the methods of this class, you need to call [ssap.createServer](#ssapcreateserver) to construct an instance of this class.
...
| Name| Type| Mandatory| Description|
| -------- | -------- | -------- | -------- |
| callback | Callback&lt;[PropertyWriteRequest](#propertywriterequest)&gt; | No| Callback used to return the property write request parameters of the client.<br>If this parameter is specified, the current callback is unregistered. If this parameter is not specified, all callbacks corresponding to the event are unregistered.|
...
Define a client property read request.
