...
<!-- md-trans-meta sourceCommit=e543ca261b0af7b91138762898e609a1232aeef3 translatedAt=2026-09-14T01:43:07.712Z pushedAt=2026-09-15T01:15:53.996Z -->

This module uses the NearLink technology to provide the device ranging function for apps. It is applicable to scenarios where the accurate distance between devices needs to be obtained, such as smart anti-loss, near-field object finding, and digital keys. This feature provides the following functions:

- Supports NearLink [HADM](../../connectivity/fusion-connectivity/fusion-connectivity-terminology.md#high-accuracy-distance-measurement-hadm) ranging, enabling high-precision distance measurement.
...
- If the query is successful, a promise is used to return whether the ranging type is supported. You can call [startRanging](#rangingstartranging) to start NearLink [High Accuracy Distance Measurement (HADM)](../../connectivity/fusion-connectivity/fusion-connectivity-terminology.md#high-accuracy-distance-measurement-hadm) ranging or call [startPassiveRanging](#rangingstartpassiveranging) to start passive ranging only when the value of [nearlinkHadm](#rangingcapabilitysupported) is **true**.
...
Initiates active ranging to a specified device to obtain information such as the distance and signal strength of the target device. Typical application scenarios include smart anti-loss and locating, near-field object finding, and digital keys.
...
- If no NearLink connection has been established between the local device and the target device, this API performs the following process:
  1. The fusion ranging service attempts to establish a connection with the target device first and performs pairing and encryption upon successful connection. During pairing, the user needs to proactively authorize the operation on the device. If the user rejects authorization or the authorization times out, the ranging will be stopped. The stop status will be notified through the callback registered using the [onRangingStateChange](#rangingonrangingstatechange) API. After the ranging is stopped, the app needs to proactively call [stopRanging](#rangingstopranging) to stop the ranging and release the ranging resources.
  2. After the connection is established, the ranging service checks whether the target device supports the corresponding ranging service [UUID](../../connectivity/bluetooth/terminology.md#uuid). If supported, the ranging service automatically initiates ranging. If not, the fusion connectivity ranging service automatically disconnects from the peer device and notifies the ranging stop through a callback.
...
After ranging is successfully started, the result will be frequently reported through a callback. You are advised to call **stopRanging** to stop ranging in a timely manner after obtaining the ranging result based on the actual requirements. You can initiate ranging again when necessary to avoid unnecessary power consumption.
...
> - When using the NearLink [HADM](../../connectivity/fusion-connectivity/fusion-connectivity-terminology.md#high-accuracy-distance-measurement-hadm) to perform ranging, the local device cannot use the passive ranging mode after initiating active ranging. To use passive ranging, call [stopRanging](#rangingstopranging) to stop active ranging first.
> - If [startRanging](#rangingstartranging) is repeatedly called for the same device, a message will be displayed indicating that the device has initiated ranging, and error code 34900051 will be returned. To initiate ranging for the same device again, call [stopRanging](#rangingstopranging) to stop the previous ranging first and then call this API again.
...
| ------------- | ------------------------------------ | ---- | --------------------- |
| params        | [RangingParams](#rangingparams)            | Yes   | Ranging parameters of the target device, including the device address and ranging capability type. If the input parameters do not meet the requirements, the API returns the corresponding error code.   |
...
> - If [stopRanging](#rangingstopranging) is called without [startRanging](#rangingstartranging) being called first, error code 34900050 will be thrown, indicating that the device has not initiated ranging.
...
try {
  // Stop ranging with the specified device.
  ranging.stopRanging(rangingCallback, params);
  // Stop ranging with all devices associated with this callback.
...
Starts the passive ranging mode. The local device broadcasts ranging data packets as a ranging beacon, allowing other active ranging devices that support the corresponding ranging type to discover the local device. Typical use scenarios: The local device is deployed as a location tag, anti-loss sticker, or fixed beacon. This is applicable when the local device needs to be measured by other devices.
...
> - When using the NearLink [HADM](../../connectivity/fusion-connectivity/fusion-connectivity-terminology.md#high-accuracy-distance-measurement-hadm) to perform ranging, the local device cannot use the active ranging mode after initiating passive ranging. To use active ranging, call [stopPassiveRanging](#rangingstoppassiveranging) to stop passive ranging first.
...
| Name         | Type                         | Mandatory | Description         |
| ------------ | -------------------------- | ---- | ---------- |
| capabilityType | [RangingTypes](#rangingtypes) | Yes   | Ranging capability type. The parameter must be set to a valid value. Otherwise, error code [34900052](errorcode-fusionConnectivity.md#34900052-specified-ranging-service-unsupported) will be returned.   |

**Return value**

| Type               | Description         |
| ---------------- | ---------- |
| Promise&lt;number&gt; | Promise used to return the handle ID of a passive ranging session. The value range is [0, 2147483647).<br> The handle is used to: 1. specify the passive ranging session to be stopped in [stopPassiveRanging](#rangingstoppassiveranging). 2. identify the corresponding passive ranging session in [stateInfo.handle](#rangingstatechangeinfo) of the [onRangingStateChange](#rangingonrangingstatechange) callback.   |
...
> **NOTE**
> 
> - This API can be called to stop the passive ranging broadcast only after the [startPassiveRanging](#rangingstartpassiveranging) API is successfully called. If [stopPassiveRanging](#rangingstoppassiveranging) is called without [startPassiveRanging](#rangingstartpassiveranging) being called first, error code 34900054 will be thrown, indicating that the handle is invalid. To ensure proper resource management, [stopPassiveRanging](#rangingstoppassiveranging) must be called to stop passive ranging after [startPassiveRanging](#rangingstartpassiveranging) is called to start passive ranging, preventing ranging resource leaks.
...
| Name         | Type                         | Mandatory | Description         |
| ------------ | -------------------------- | ---- | ---------- |
| handle       | number                     | Yes   | Ranging monitoring handle, which must be a valid handle returned by [startPassiveRanging](#rangingstartpassiveranging). Otherwise, error [34900054](errorcode-fusionConnectivity.md#34900054-invalid-parameters) is thrown. After stopping, this handle is no longer valid and cannot be reused.   |
| capabilityType | [RangingTypes](#rangingtypes) | Yes   | Ranging capability type, which must be the same as **capabilityType** passed by the [startPassiveRanging](#rangingstartpassiveranging) API.   |
...
> **NOTE**
>
> - This API is valid only after [onRangingStateChange](#rangingonrangingstatechange) is called.
...
| deviceId | string           | No   | Yes   | Address of the ranging device. In active ranging scenarios, this parameter identifies the target device whose status has changed. This parameter is not applicable in passive ranging scenarios. The default value is **undefined**.  |
| handle | number           | No   | Yes   | Ranging monitoring handle, which identifies the passive ranging session whose status has changed in passive ranging scenarios. This parameter is not applicable in active ranging scenarios. The default value is **undefined**.  |
...
| angle   | [RangingMeasurement](#rangingmeasurement) | No   | No   | Azimuth output by the ranging device, in degrees. The value range is [0, 360). If **-1** is returned, angle measurement is not supported.   |
| rssi    | number                           | No   | No   | Received signal strength indicator [RSSI](../../connectivity/bluetooth/terminology.md#rssi), in dBm.    |
...
| Name         | Type     | Read-only | Optional | Description                      |
| ---------- | ------ | ---- | ---- | ----------------------- |
| nearlinkHadm | boolean | No   | No   | Whether the NearLink [HADM](../../connectivity/fusion-connectivity/fusion-connectivity-terminology.md#high-accuracy-distance-measurement-hadm) ranging type is supported. If the value is **true**, you can call [startRanging](#rangingstartranging) or [startPassiveRanging](#rangingstartpassiveranging) to start ranging. |

## RangingMeasurement

Describes the measurement result, including the measurement value and its confidence. The measurement result can be used for distance or angle measurement.
...
| ------------ | ---- | ----------------------------------------- |
| NEARLINK_HADM | 1    | NearLink [HADM](../../connectivity/fusion-connectivity/fusion-connectivity-terminology.md#high-accuracy-distance-measurement-hadm) ranging type.  |
...
| Name           | Value | Description      |
| ------------ | ---- | ------- |
| RANGING_STOPPED | 0   | Ranging stopped. For details about the stop cause, see [RangingStoppedCause](#rangingstoppedcause). |
...
| Name   | Value | Description |
| ----- | ---- | ----- |
| HIGH   | 0 | High-confidence measurement. The measurement result is highly reliable and can be directly used. |
