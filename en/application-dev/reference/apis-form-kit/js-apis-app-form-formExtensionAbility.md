...
| Name | Type  | Mandatory| Description                  |
| ------- | ------ | ---- | ---------------------- |
| newStatus  | Record\<string, number> | Yes  | ID and visibility status of the widget to be changed.<br>**Note**: The value of the **number** parameter is an integer within the range of [0, 2]. The value **0** indicates an unknown type, the value **1** indicates the visible state, and the value **2** indicates the invisible state.<br>For details, see [formInfo.VisibilityType](js-apis-app-form-formInfo.md#visibilitytype).|
...
API for the widget provider to receive widget status query notifications. By default, this API returns the initial status of the widget. (This API can be optionally overridden.)
