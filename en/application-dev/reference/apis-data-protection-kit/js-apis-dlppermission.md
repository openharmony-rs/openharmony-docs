...
> - DLP files with the same classification label are opened in the same sandbox. If multiple DLP files with the same label are opened in a sandbox, the query result returns the URIs of all opened files (including those manually closed).
...
>**NOTE**
>
> - This API can only close DLP files generated by the caller application using [generateDlpFileForEnterprise](#dlppermissiongeneratedlpfileforenterprise21).
> - If **options** is not specified, or if **classificationLabel** in **options** is not specified or is an empty string, all DLP files of all labels will be closed.
...
| Name| Type| Mandatory| Description|
| -------- | -------- | -------- | -------- |
| options | [DlpFileQueryOptions](#dlpfilequeryoptions26) | No| Query options for DLP files. If this parameter is not specified, all DLP files will be closed.|
