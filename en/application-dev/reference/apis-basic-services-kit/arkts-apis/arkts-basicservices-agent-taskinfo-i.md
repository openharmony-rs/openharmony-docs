...
Note: When [request.agent.search](arkts-basicservices-agent-search-f.md#search-1) is used for query, this value must be within the range of [after,before] for the task ID to be obtained. For details about **before** and **after**, see [Filter](arkts-basicservices-agent-filter-i.md).
