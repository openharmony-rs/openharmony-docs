...
<!--Tester: @yufeifei-->
<!--Adviser: @jinqiuheng-->
<!-- md-trans-meta sourceCommit=9bf4df179b62db5cf36fd2e478ae203ae94127f4 translatedAt=2026-09-16T11:26:42.409Z pushedAt=2026-09-20T09:01:52.281Z -->
...
| ----- | ------ | ------------------------------------------------------------ |
| DEBUG | 3      | Log level used to record detailed process records. Logs at this level allow for more detailed analysis of business processes and problem locating. |
...
| Name| Type  | Mandatory| Description                                                        |
| ------ | ------ | ---- | ------------------------------------------------------------ |
| domain | number | Yes | Domain identifier corresponding to the log, ranging from 0x0 to 0xFFFF. If out of range, the log cannot be printed.<br>You are recommended to customize the division within the application as needed. |
| tag    | string | Yes  | Log tag in the string format. You are advised to use this parameter to identify a particular service behavior or the class holding the ongoing method. A tag can contain a maximum of 31 bytes. If a tag exceeds this limit, it will be truncated. Chinese characters are not recommended because garbled characters or alignment problems may occur.|
...
| Name| Type  | Mandatory| Description                                                        |
| ------ | ------ | ---- | ------------------------------------------------------------ |
| domain | number | Yes | Domain identifier corresponding to the log, ranging from 0x0 to 0xFFFF. If it is out of range, the log cannot be printed.<br>It is recommended for developers to customize the division within the application as needed. |
| tag    | string | Yes  | Log tag in the string format. You are advised to use this parameter to identify a particular service behavior or the class holding the ongoing method. A tag can contain a maximum of 31 bytes. If a tag exceeds this limit, it will be truncated. Chinese characters are not recommended because garbled characters or alignment problems may occur.|
| format | string | Yes | Format string used for formatting output of the log. Multiple parameters can be set in the format string, and each parameter needs to include a parameter type and a privacy identifier.<br>Available parameter format specifiers include %d, %i, %s, %o, %O, etc. For details, see [Parameter Format](#parameter-format).<br>Privacy identifiers are divided into {public} and {private}, with {private} as the default. Content marked as {public} is output in plaintext, while content marked as {private} is output with \<private> filter echo. The privacy identifier mechanism helps developers protect privacy-sensitive data. |
| args   | any[]  | No | Variable-length parameter list corresponding to the format string **format**. The number and types of parameters must correspond one-to-one with the identifiers in the format string. When the format string contains no placeholders, this parameter can be omitted, and only the format string itself is output. |
...
| Name| Type  | Mandatory| Description                                                        |
| ------ | ------ | ---- | ------------------------------------------------------------ |
| domain | number | Yes   | Domain identifier corresponding to the log, ranging from 0x0 to 0xFFFF. If out of range, the log cannot be printed.<br>It is recommended for developers to customize the division within the application as needed. |
| tag    | string | Yes  | Log tag in the string format. You are advised to use this parameter to identify a particular service behavior or the class holding the ongoing method. A tag can contain a maximum of 31 bytes. If a tag exceeds this limit, it will be truncated. Chinese characters are not recommended because garbled characters or alignment problems may occur.|
| format | string | Yes   | Format string used for formatting output of the log. Multiple parameters can be set in the format string, and each parameter needs to include the parameter type and privacy identifier.<br>Available parameter format specifiers include **%d**, **%i**, **%s**, **%o**, **%O**, and so on. For details, see [Parameter Format](#parameter-format).<br>Privacy identifiers are divided into **{public}** and **{private}**, with **{private}** as the default. Content marked with **{public}** is output as plaintext, while content marked with **{private}** is output with \<private> filter echo. The privacy identifier mechanism helps developers protect privacy-sensitive data. |
| args   | any[]  | No   | Variable-length parameter list corresponding to the format string **format**. The number and types of parameters must correspond one-to-one with the identifiers in the format string. When the format string contains no placeholders, this parameter can be omitted, and only the format string itself is output. |
...
| Name| Type  | Mandatory| Description                                                        |
| ------ | ------ | ---- | ------------------------------------------------------------ |
| domain | number | Yes | Log corresponding domain identifier, ranging from 0x0 to 0xFFFF. If out of range, the log cannot be printed.<br>Recommended for developers to customize the division within the application as needed. |
| tag | string | Yes | Specify log identifier, which can be an arbitrary string. Recommended for identifying the class where the call belongs or the business behavior. The maximum length of **tag** is 31 bytes, and it is truncated after exceeding this limit. Chinese characters are not recommended, as they may cause garbled characters or alignment issues. |
| format | string | Yes | Format string used for formatting output of the log. Multiple parameters can be set in the format string, and each parameter needs to include the parameter type and privacy identifier.<br>Available parameter format specifiers include %d, %i, %s, %o, %O, etc. For details, see [Parameter Format](#parameter-format).<br>Privacy identifiers are divided into {public} and {private}, defaulting to {private}. Content marked with {public} is output as plaintext, while content marked with {private} is output with \<private> filter echo. The privacy identifier mechanism helps developers protect privacy-sensitive data. |
| args | any[] | No | Variable-length parameter list corresponding to the format string **format**. The number and types of parameters must correspond one-to-one with the identifiers in the format string. When the format string contains no placeholders, this parameter can be omitted, and only the format string itself is output. |
...
| domain | number | Yes | Domain identifier corresponding to the log, ranging from 0x0 to 0xFFFF. If it is out of range, the log cannot be printed.<br>It is recommended for developers to customize the division within the application as needed. |
| tag    | string | Yes  | Log tag in the string format. You are advised to use this parameter to identify a particular service behavior or the class holding the ongoing method. A tag can contain a maximum of 31 bytes. If a tag exceeds this limit, it will be truncated. Chinese characters are not recommended because garbled characters or alignment problems may occur.|
| format | string | Yes | Format string used for log formatting output. Multiple parameters can be set in the format string, and each parameter needs to include the parameter type and privacy identifier.<br>Available parameter format specifiers include %d, %i, %s, %o, %O, etc. For details, see [Parameter Format](#parameter-format).<br>Privacy identifiers are divided into {public} and {private}, with {private} as the default. Content marked with {public} is output as plaintext, while content marked with {private} is output with \<private> filter echo. The privacy identifier mechanism helps developers protect privacy-sensitive data. |
| args | any[] | No | Variable-length parameter list corresponding to the format string **format**. The number and types of parameters must correspond one-to-one with the identifiers in the format string. When the format string contains no placeholders, this parameter can be omitted, and only the format string itself is output. |
