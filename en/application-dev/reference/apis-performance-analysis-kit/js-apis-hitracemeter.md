...
<!--Tester: @gcw_KuLfPSbe-->
<!--Adviser: @jinqiuheng-->
<!-- md-trans-meta sourceCommit=0c0db3e174588398ce45072b7ae0a095cb0a05c5 translatedAt=2026-09-21T02:59:57.075Z pushedAt=2026-09-22T01:29:30.422Z -->
...
>
> - The total length of a [user-mode trace](../../dfx/hitracemeter-view.md#user-mode-trace-format) is limited to 512 characters. The part exceeding this limit will be truncated.
...
// Service flow...
hiTraceMeter.startTrace("myTestFunc", 2);  // The second tracing task starts while the same-name task of the first tracing has not ended yet, resulting in parallel execution. Different taskId values are required to distinguish the tasks.
// Service flow...
...
| Name  | Type                                       | Mandatory| Description            |
| -------- | ------------------------------------------- | ---- | ---------------- |
| callback | [TraceEventListener](#traceeventlistener22) | Yes | Callback invoked when the trace capture switch state of the application changes. When the trace capture switch state changes (from on to off or from off to on), this callback is triggered and the current trace state is passed in. After successful registration, the callback is executed once immediately, and it is triggered each time the trace capture switch state changes. |
...
let callback: hiTraceMeter.TraceEventListener = (traceStatus: boolean) => {
  if (traceStatus) {
    // Trace capture of the current application is enabled.
    // ...
  } else {
    // Trace capture of the current application is disabled.
    // ...
  }
};
