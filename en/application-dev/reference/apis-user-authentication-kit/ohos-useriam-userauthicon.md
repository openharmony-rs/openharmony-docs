...
Defines the embedded user authentication icon component. It provides the system-standard face and fingerprint authentication icons. Tapping the icon triggers the system-level authentication process. You only need to configure authentication parameters and callbacks to integrate the authentication entry into the application UI.
...
| iconHeight     | [Dimension](../apis-arkui/arkui-ts/ts-types.md#dimension10) | No   | Icon height, with an aspect ratio of 1:1. Percentage strings are not supported. You are advised to select an appropriate size based on the UI layout.<br>**Default value:** 64fp |
| iconColor      | [ResourceColor](../apis-arkui/arkui-ts/ts-types.md#resourcecolor) | No  | Icon color. Multiple formats are supported, such as color values and resource references. By default, the system accent color is used. You can customize the color based on the application theme, for example, using **Color.Blue** or **$r('app.color.primary')**.<br>Default value: **$r('sys.color.ohos_id_color_activated')**|
...
Errors may occur during the authentication process. For details about the error codes, see [Universal Error Codes](../errorcode-universal.md) and [User Authentication Error Codes](errorcode-useriam.md).
