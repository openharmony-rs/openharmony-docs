...
<!--Adviser: @HelloCrease-->
<!-- md-trans-meta sourceCommit=4c060e5d4f7e1d85d714e43332481f44ce6411de translatedAt=2026-09-03T10:16:00.542Z pushedAt=2026-09-05T10:47:30.384Z -->

The dialogSession module supports the dialog box feature for system applications and provides system-level dialog session management capabilities.
...
| appIndex<sup>12+</sup> | number | No | No | App clone index. The value starts from 0, where 0 indicates the main app instance and 1 or greater indicates a clone instance. |
| multiAppMode<sup>12+</sup> | [MultiAppMode](./js-apis-bundleManager-applicationInfo.md#multiappmode12) | No| No| Multi-app mode.|
| codePath<sup>23+</sup> | string | No | Yes | Installation directory of the application. Pass this parameter when the application installation path needs to be obtained; otherwise, this field is empty. |
...
Provides session information, including the requester component information, the target component information list, and other parameters.
...
| parameters | Record<string, Object> | No | Yes | Other parameters used to pass custom session-related data. Defaults to empty if not passed. |

## getDialogSessionInfo

getDialogSessionInfo(dialogSessionId: string): [DialogSessionInfo](#dialogsessioninfo)

Obtains the dialog session information based on the dialogSessionId.
...
**Parameters**
  ...
  | -------- | -------- | -------- | -------- |
  | dialogSessionId | string | Yes | ID of the user request session, automatically generated by the system when the dialog session is created. |
...
Sends the dialog result. This API uses an asynchronous callback to return the result.
...
**Parameters**
  ...
  | Name| Type| Mandatory| Description|
  | -------- | -------- | -------- | -------- |
  | dialogSessionId | string | Yes | Dialog session ID. |
  | targetWant | Want | Yes | Want information of the target ability. |
  | isAllowed | boolean | Yes| Whether the target ability can be started. **true** if allowed, **false** otherwise.|
  | callback | AsyncCallback\<void\> | Yes | Callback invoked when the dialog result is sent. If the operation is successful, err is undefined; otherwise, err is an error object. |
...
Sends the dialog result. This API uses a promise to return the result.
