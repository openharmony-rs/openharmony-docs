...
| result | Record\<string, Object> | No | Yes | Script execution result. |
| uris | Array\<string> | No | Yes | List of URIs to be authorized to the caller. |
| flags | number | No | Yes | Read/write permission of the URIs, which is the same as the flags field of [Want](js-apis-app-ability-want.md). The value can be any of the following:<br>- [wantConstant.Flags.FLAG_AUTH_READ_URI_PERMISSION](js-apis-app-ability-wantConstant.md#flags): read permission.<br>- [wantConstant.Flags.FLAG_AUTH_WRITE_URI_PERMISSION](js-apis-app-ability-wantConstant.md#flags): write permission.<br>- A combination of the two flags above: grants both read and write permissions. |
...
| context | [Context](js-apis-inner-application-context.md) | Yes | No | Context of the bound ability. |
...
export default class TotalTimeSkill {
  // The first parameter of the entry function must be scriptManager.ArkTSScriptInfo, and the subsequent parameters are developer-defined.
  public async RemoteTotalTimeModel(arkTSScriptInfo: scriptManager.ArkTSScriptInfo, ...argv: string[]): Promise<void> {
    // Example of parameter parsing (use the parsed parameters based on actual service requirements).
