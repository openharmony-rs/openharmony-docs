...
<!--Adviser: @HelloCrease-->
<!-- md-trans-meta sourceCommit=ee7c94547cc372df4b853e93ce67a995f0a13440 translatedAt=2026-09-03T10:48:46.186Z pushedAt=2026-09-05T10:47:30.493Z -->

This module provides the capability of interacting with system command-line tools (CLI), including querying tool information, invoking and executing CLI commands, and managing sessions. A session is created when the execTool API is called, and is used to track the execution status and result of a CLI tool.
...
| yieldMs    | number | No | Foreground execution duration of the task. Value range: 0 to 1000 * timeout. Default value: 0. Unit: ms. |
| timeout    | number | No | Timeout duration for task execution. Value range: 0 to 1800. Default value: 1800. Unit: s. |
...
| timeOut       | boolean | No   | Yes  | Whether the execution of the tool times out. true indicates a timeout, and false indicates no timeout. |
| executionTime | number  | No   | Yes  | Execution duration of the tool. Unit: ms. |
...
| Type                               | Description                       |
| ---------------------------------- | -------------------------- |
| Promise\<Array\<[ToolSummary](js-apis-inner-application-ToolInfo-sys.md#toolsummary)>> | Promise object used to return the list of tool summary information. |
...
try {
  // Query the summary information of all CLI tools.
...
| Type                               | Description                       |
| ---------------------------------- | -------------------------- |
| Promise\<Array\<[ToolInfo](js-apis-inner-application-ToolInfo-sys.md#toolinfo)>> | Promise object used to return the list of detailed tool information. |
...
try {
  // Query the detailed information of all CLI tools.
...
try {
  // Obtain the detailed information about the tool by its name.
...
| subCommand | string | Yes | Name of the CLI tool subcommand. If there is no subcommand, pass an empty string. |
...
| execOptions | [ExecOptions](#execoptions) | No | Optional parameters for executing the command. Default value: see the default values of the specific properties in [ExecOptions](#execoptions). |

**Return value**

| Type | Description |
| -------- | -------- |
| Promise\<[CliSessionInfo](#clisessioninfo)\> | Promise object. Returns the session information. |
...
try {
// Create an access control manager instance.
const atManager = abilityAccessCtrl.createAtManager();
let CLI_DEMO: abilityAccessCtrl.CliInfo = {
    cliName: 'ohos-timer',
    subCliName: '',
};
// Define the authorization information list.
...
// Generate the CLI authorization result.
...
> **NOTE**
>
> The session is limited to the creating process: only the process that calls `execTool` or `execCmd` to create the session can call this API. Even if other processes obtain the `sessionId`, calling this API throws error code 201 (Permission denied).
...
| callback  | [ToolEventCallback](js-apis-inner-application-toolEventCallback-sys.md) | Yes       | Callback for the CLI tool session event. |
...
// Define the callback for the CLI tool session event.
...
try {
  // Subscribe to the event of the specified session.
...
let sessionId = 'example_session_id';
try {
  // Query the status and execution result of the specified session.
...
| timeout    | number | No | Timeout duration for command execution, in seconds. Value range: 0 to 1800. Default value: 1800. Passing 0 means no timeout. |
| policy     | string | No | Security policy. The parameter is in JSON string format. |
| callback   | [ToolEventCallback](js-apis-inner-application-toolEventCallback-sys.md) | No | Event callback used to receive tool events. If this parameter is provided, session events are automatically subscribed. |
...
| execCmdOptions | [ExecCmdOptions](#execcmdoptions) | No | Optional parameters for executing the command. Default value: see the default values of the specific attributes in [ExecCmdOptions](#execcmdoptions). |

**Return value**

| Type | Description |
| -------- | -------- |
| Promise\<[CliSessionInfo](#clisessioninfo)\> | Promise object used to return the session information. |
...
| --------- | ------ | --------- | -------------------------------------------- |
| sessionId | string | Yes       | Session ID of the target CLI tool process.   |
| message   | string | Yes       | Message to send. The maximum length is 10240 characters. Error code 401 is thrown when the maximum length is exceeded. |

**Return value**

| Type           | Description                     |
| -------------- | ------------------------------- |
| Promise\<void> | Promise object, with no return value. |
