...
<!--Adviser: @ge-yafang-->
<!-- md-trans-meta sourceCommit=55b1556353cdc8a2e309705642d169ae2df47adf translatedAt=2026-09-15T11:58:57.700Z pushedAt=2026-09-16T07:50:15.720Z -->

The **cloudData** module provides APIs for implementing device-cloud sync and device-cloud sharing.

Device-cloud sync enables sync of the structured data (in RDB stores) between devices and the cloud. The cloud serves as a data hub to implement data backup in the cloud and data consistency between the devices with the same account.

Device-cloud sharing enables data sharing across accounts based on device-cloud sync. Understanding the following concepts helps you better understand the device-cloud sharing process: **sharingResource**: an identifier of the string type generated for each data record shared by an application when device-cloud sync is performed. It uniquely identifies the data record being shared.

The device-cloud sharing participant refers to all the people selected by the sharing initiator from the friend list to participate in the current data sharing.

The device-cloud sharing invitation code refers to the following: after a share is initiated, an invitation code for the current sharing operation is generated on the sharing server, and the invitation code is attached to the current sharing invitation and pushed to the devices of the invitees through push messages. The invitees can use the invitation code to confirm the invitation.

The **cloudData** module provides the following functionalities:

- [Config](#config): provides APIs for setting device-cloud sync, including enabling and disabling device-cloud sync, clearing data, and notifying data changes.
...
```ts
// accountId: ID of the cloud account opened by the user.
// bundleName: application bundle name.
// containerName: name of the database in the cloud.
// databaseScopes: type of the database in the cloud.
// recordTypes: name of the database table in the cloud.
...
| Name      | Type            | Read-Only | Optional | Description                       |
| ---------- | -------------- | ---- | ---- | -------------------------- |
| enable     | boolean           | No   | No   | Whether to enable device-cloud sync for the database. The value **true** indicates that the device-cloud sync is enabled, and the value **false** indicates the opposite. |
| tableInfo  | Record<string, boolean> | No   | Yes   | Device-cloud sync configuration of a table. The key is the table name, and the value is the switch status of the table. The value **true** indicates that device-cloud sync is enabled for the table, and the value **false** indicates the opposite. If this parameter is not set, the device-cloud sync is enabled for the database by default. |
...
Defines the database-level clear configuration information of device-cloud collaboration.
...
Defines the device-cloud sync application information.
...
Defines the database-level clear configuration of device-cloud collaboration.

**System capability**: SystemCapability.DistributedDataManager.CloudSync.Config

| Name       | Type            | Read-Only | Optional | Description                       |
| ---------- | -------------- | ---- | ---- | -------------------------- |
| dbInfo     | Record<string, [DBActionInfo](#dbactioninfo23)>    | No   | No   | Database information and clear rules for the data to be cleared. The key is the database name, and the value is the clear configuration information of the database.   |

## Config

Provides APIs for setting device-cloud sync, including enabling and disabling device-cloud sync, clearing data, and notifying data changes.
...
| switches  | Record<string, boolean>         | Yes       | Device-cloud sync switch information of each application. The value **true** means to enable the device-cloud switch of the application; the value **false** means the opposite. |
| callback  | AsyncCallback&lt;void&gt;       | Yes       | Callback used to return the result. If device-cloud sync is enabled successfully, the value of **err** is **undefined**; otherwise, the value is an error object. |
...
| --------- | ----------------------- | --------- | ------------------------------------------------------------------ |
| accountId | string                  | Yes       | ID of the logged-in cloud account.                                 |
| switches  | Record<string, boolean> | Yes       | Device-cloud sync switch settings of each application. The value **true** means to enable the device-cloud switch of the application, and **false** means to disable it. |
...
| --------- | ------------------------- | --------- | --------------------------- |
| accountId | string                    | Yes       | ID of the logged-in cloud account. |
| callback  | AsyncCallback&lt;void&gt; | Yes       | Callback used to return the result. If device-cloud sync is disabled successfully, the value of **err** is **undefined**. Otherwise, the value is an error object. |
...
| status    | boolean                        | Yes  | New device-cloud sync setting. The value **true** means to enable device-cloud sync; the value **false** means the opposite. |
| callback  | AsyncCallback&lt;void&gt;       | Yes  | Callback used to return the result. If the device-cloud sync setting for an application is successfully changed, the value of **err** is **undefined**; otherwise, the value is an error object. |
...
| bundleName| string                         | Yes       | Application bundle name. |
| status    | boolean                        | Yes       | New device-cloud sync setting. The value **true** means to enable device-cloud sync; the value **false** means the opposite. |
| config    | [SwitchConfig](#switchconfig23)   | No        | Switch configuration of a device-cloud sync database. Device-cloud sync priority: application > database > table. If this parameter is not set, the application-level device-cloud sync is used by default.|
...
Notifies the data changes of a user in the cloud. You can specify the database and tables with data changes in the **extraData** field in **extInfo**, and specify the user ID through **userId**. This API uses an asynchronous callback to return the result.
...
| -------- | ------------------------- | --------- | ------------------------------------------------------ |
| extInfo  | [ExtraData](#extradata11) | Yes       | Transparently transmitted data, including information about the application that has data changes. |
| userId   | number                    | Yes       | User ID, which corresponds to an existing user ID in the system. |
| callback | AsyncCallback&lt;void&gt; | Yes       | Callback used to return the result. If the operation is successful, **err** is **undefined**. Otherwise, **err** is an error object. |
...
Notifies the data changes in the cloud. You can specify the database and tables with data changes in the **extraData** field in **extInfo**, and specify the user ID through **userId**. This API uses a promise to return the result.
...
| Type                                                         | Description                                         |
| ------------------------------------------------------------ | -------------------------------------------- |
| Promise&lt;Record&lt;string, [SyncInfo](#syncinfo12)&gt;&gt; | Result set of the database names and the last Device-Cloud synchronization information. |
...
// Subscribe to synchronization information changes.
try {
...
// Unsubscribe from the specified callback.
try {
...
| Name     | Type                                                                     | Mandatory | Description                |
| ---------- |------------------------------------------------------------------------| ---- |-------------------|
| strategy  | [StrategyType](js-apis-data-cloudData.md#strategytype)    | Yes   | Type of the strategy to configure.          |
...
| --------- | ------------------------------------------------- | --------- | ------------------------------------------------------------------ |
| accountId  | string                                            | Yes       | ID of the logged-in cloud account.                                 |
| appActions | Record<string, [ClearAction](#clearaction)>       | Yes       | Application information and clear rules for the data to be cleared. |
...
| ---------- | --------------------------------------------------- | --------- | --------------------------------------- |
| accountId  | string                                              | Yes       | ID of the logged-in cloud account.      |
| appActions | Record<string, [ClearAction](#clearaction)>         | Yes       | Application information and clearing rules of the data to be cleared. |
...
| ---------- | --------------------------------------------------- | ---- | -------------------------------- |
| accountId  | string                                              | Yes   | ID of the logged-in cloud account.             |
| appActions | Record<string, [ClearAction](#clearaction)>         | Yes   | Application information and clearing rule of the data to be cleared. |
| config | Record<string, [ClearConfig](#clearconfig23)>         | No   | Database-level clearing configuration for device-cloud collaboration. The key is the application bundle name, and the value is the database clearing rule of the application. Clearing rule priority: table level > database level > application level. If this parameter is not configured, the application-level data clearing method is used by default.|
...
Provides APIs for device-cloud data sharing, including initiating a share, canceling a share, exiting a share, changing the privilege on the shared data, finding participants, confirming an invitation, changing a confirmed invitation, and finding shared resources.
...
Defines the return value of the device-cloud sharing result.
...
| value       | T                           | No       | Yes      | Value of the returned result. The specific type is specified by the parameter **T**. The default value is **undefined**. |
...
| Name    | Type                            | Mandatory | Description                         |
| --------- | ------------------------------- | ---- | ---------------------------- |
| storeId      | string                        | Yes   | Database name. |
...
| participants | Array&lt;[Participant](#participant11)&gt; | Yes   | Represents information about a participant of device-cloud sharing. |
...
| storeId      | string                        | Yes   | Database name. |
| predicates   | [relationalStore.RdbPredicates](arkts-apis-data-relationalStore-RdbPredicates.md) | Yes   | Predicate conditions for querying the data of the sharing resource identifier. |
| participants | Array&lt;[Participant](#participant11)&gt; | Yes   | Participants of device-cloud sharing. |
...
| Name    | Type                            | Mandatory | Description                         |
| --------- | ------------------------------- | ---- | ---------------------------- |
| sharingResource   | string                                     | Yes   | Resource identifier of the device-cloud shared data. |
| participants      | Array&lt;[Participant](#participant11)&gt;   | Yes   | Represents information about a participant of device-cloud sharing. |
...
| --------- | ------------------------------- | ---- | ---------------------------- |
| sharingResource  | string                                     | Yes   | Resource identifier of the device-cloud shared data. |
| participants     | Array&lt;[Participant](#participant11)&gt; | Yes   | Participants of the device-cloud sharing. |
| callback         | AsyncCallback&lt;[Result](#resultt11)&lt;Array&lt;[Result](#resultt11)&lt;[Participant](#participant11)&gt;&gt;&gt;&gt;  | Yes   | Callback used to return the result. |
...
| --------- | ------------------------------- | ---- | ---------------------------- |
| sharingResource  | string                | Yes   | Resource identifier of the device-cloud shared data. |
| callback         | AsyncCallback&lt;[Result](#resultt11)&lt;Array&lt;[Participant](#participant11)&gt;&gt;&gt;  | Yes   | Callback invoked to return the result of finding the sharing participants. |
