...
<!--Tester: @logic42-->
<!--Adviser: @ge-yafang-->
<!-- md-trans-meta sourceCommit=81165a532e10b3d1517ed6f395ac6f3808130ce3 translatedAt=2026-09-15T08:59:23.528Z pushedAt=2026-09-16T07:50:15.607Z -->
...
| cryptoPageSize | number | No| Yes| Page size used for database encryption and decryption. The value is an integer. Unit: byte <br>Default value: **1024**.<br>The value must be an integer within the range of 1,024 to 65,536 and must be 2<sup>n</sup>. If the specified value is not an integer, the value is rounded down.|
...
| Name| Type| Read-Only| Optional| Description|
| ---- | ---- | ---- | ---- | ---- |
| autoSync | boolean | No| No| Whether the table supports automatic device-cloud sync. If the value is **true**, the system can automatically trigger device-cloud sync. If the value is **false**, the system cannot automatically trigger device-cloud sync, and the [cloudSync](arkts-apis-data-relationalStore-RdbStore.md#cloudsync10-3) API needs to be called to trigger device-cloud sync.|
...
Defines a struct for statistics of device-cloud sync and download tasks of a database table.

**System capability**: SystemCapability.DistributedDataManager.RelationalStore.Core

| Name    | Type                     | Read-Only| Optional | Description                                      |
| -------- | ------------------------- | ---- | ---- |------------------------------------------ |
| upload   | [Statistic](arkts-apis-data-relationalStore-i.md#statistic10) | No  |   No  | Statistics of the device-cloud sync tasks.|
| download | [Statistic](arkts-apis-data-relationalStore-i.md#statistic10) | No  |   No  | Statistics of the device-cloud sync tasks.|
...
**Usage constraints** <br/>1. The number of dynamic library names is limited to a maximum of 16. If this number is exceeded, the database fails to open and error code 14800000 is returned.<br/>2. The dynamic library name must be a dynamic library in the application sandbox path or the system path. If the dynamic library cannot be loaded, the database fails to open and error code 14800010 is returned.<br/>3. The dynamic library name must be a complete path for loading by sqlite. The path consists of [context.bundleCodeDir + "/libs/arm64/" + so name], where context.bundleCodeDir is the path corresponding to the application sandbox, "libs" is a fixed directory, and "arm64" is a subdirectory determined by the system architecture. For example, when the system architecture is arm64-v8a, the subdirectory is "arm64".<br/>Example: [context.bundleCodeDir + "/libs/arm64/" + libtokenizer.so]. When this parameter is not filled, the dynamic library is not loaded by default.<br/>4. The dynamic library must include all its dependencies to avoid failure to run due to missing dependencies.<br/>For example, in an NDK project, libtokenizer.so is built with default compilation parameters, and this dynamic library depends on the C++ standard library. When this dynamic library is loaded, because the namespace is inconsistent with that at compile time, it is linked to the wrong libc++_shared.so, causing the `__emutls_get_address` symbol to be not found. To solve this problem, statically link the C++ standard library at compile time. For details, see [NDK Project Build Overview](../../napi/build-with-ndk-overview.md).<br/>
**Example of loading a developer-defined tokenizer using pluginLibs** <br/> 1. The developer needs to implement an FTS5 loadable tokenizer extension and compile it into an so file. For compilation, see [Building an NDK Project with Command-Line CMake](../../napi/build-with-ndk-cmake.md).<br/>2. Copy the generated .so file to the corresponding subdirectory under the "entry/libs/" folder of the project directory (if the corresponding directory does not exist, the user can create it). The subdirectory is determined by the system architecture. For example, when the system architecture is arm64-v8a, place it in the "entry/libs/arm64-v8a" directory; when the system architecture is armeabi-v7a, place it in the "entry/libs/armeabi-v7a" directory.<br/>3. Load the custom tokenizer.
...
export default class EntryAbility extends UIAbility {
  async onWindowStageCreate(windowStage: window.WindowStage) {
    ...
    // libdistributeddb_extension.so is the name of the .so file compiled from the implemented FTS5 loadable tokenizer extension.
    ...
    try {
      ...
      // Use the custom tokenizer to create an FTS5 virtual table. The value after tokenize is the name of the implemented tokenizer.
      ...
      await rdbStore.executeSql("INSERT INTO pages(keywords, title, body) VALUES('song', 'xxx', 'I love Beijing Tiananmen, the sun rises over Tiananmen');");
      console.info("INSERT VIRTUAL TABLE OK, body is 'I love Beijing Tiananmen, the sun rises over Tiananmen'");
