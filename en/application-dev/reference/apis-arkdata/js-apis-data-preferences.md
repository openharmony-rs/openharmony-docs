...
<!--Adviser: @ge-yafang-->
<!-- md-trans-meta sourceCommit=989f39e772bc8728b09b53c97f5ab2815c042f72 translatedAt=2026-09-15T13:03:48.701Z pushedAt=2026-09-16T07:50:15.738Z -->
...
| GSKV |  1    |[GSKV storage mode](../../database/data-persistence-by-preferences.md#gskv).<br>**Features:** Data is stored in GSKV database mode. Operations on data are persisted to disk in real time, and there is no need to call the [flush](#flush) API to persist the data to disk.      |
...
| dataGroupId | string \| null \| undefined | No  | Yes | Application group ID. <!--RP1-->Specifying **dataGroupId** to create a Preferences instance in the corresponding shared sandbox path is not supported yet.<!--RP1End--><br>This is an optional parameter. It specifies that the Preferences instance is created in the sandbox path corresponding to this  **dataGroupId**. If this parameter is not specified, the Preferences instance is created in the application sandbox directory by default.<br> **Model constraint:** This attribute is available only in the stage model.<br>**Atomic service API:** Since API version 11, this parameter is supported in atomic services. |
| storageType<sup>18+</sup> | [StorageType](#storagetype18) \| null \| undefined | No | Yes | Storage mode, which is an optional parameter. It indicates the storage mode to be used by the current Preferences instance. If this parameter is not specified, the XML storage mode is used by default. After a Preferences instance is created with a certain storage mode, switching the storage mode is not supported. <br>**Atomic service API:** Since API version 18, this parameter is supported in atomic services. |
...
> - on('change'): Subscribes to changes of all keys, suitable for scenarios where global data changes need to be perceived.
> - on('dataChange'): Precisely subscribes to changes of specified keys, suitable for scenarios focusing on specific data, and can return the specific value through the callback.
> - on('multiProcessChange'): Subscribes to multi-process data changes, suitable for scenarios where multiple processes share the same preference file.
> 
> **Selection suggestions:** For single-process applications, on('change') or on('dataChange') is recommended. Use on('multiProcessChange') for multi-process data synchronization. Use on('dataChange') when you need to precisely know the changes of specific keys and obtain the new values.
...
- If you do not need to listen for data changes of a specific key, call **off** in a timely manner to unsubscribe.
...
| callback | Callback&lt;Record&lt;string, [ValueType](#valuetype)&gt;&gt; | No   | Callback to unregister. If this parameter is not specified, all registered callbacks are unregistered; if specified, only the specified callback is unregistered. |
