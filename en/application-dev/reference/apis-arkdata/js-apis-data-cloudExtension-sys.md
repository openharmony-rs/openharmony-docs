...
| Name      | Type           | Read-Only| Optional| Description                |
| ---------- | --------------- | ---- | ---- | ------------------ |
| id         | string          | Yes  | No  | ID automatically generated by the system when data is inserted.|
...
Enumerates the types of the cloud data field. The actual type of each API parameter varies depending on its function.
...
| Uint8Array                | The value is a Uint8Array.|
...
| values     | Array&lt;Record&lt;string, [CloudType](#cloudtype)&gt;&gt; | No  | No  | Array of data to be queried, which consists of the actual data value and [ExtensionValue](#extensionvalue).|
...
Creates a [RemoteObject](../apis-ipc-kit/js-apis-rpc.md#remoteobject) object based on the instance of the [CloudService](#cloudservice) class. The system uses this object to call the implemented APIs of the [CloudService](#cloudservice) class. This API uses a promise to return the result.
...
Generates unique IDs for the data records inserted to the cloud database. This API uses a promise to return the result.
...
| Name| Type  | Mandatory| Description                |
| ------ | ------ | ---- | -------------------- |
| count  | number | Yes  | Number of IDs to generate. The value must be greater than or equal to 1.|
...
| values     | Array&lt;Record&lt;string, [CloudType](#cloudtype)&gt;&gt; | Yes  | Data to update.    |
...
| ----------- | ------------- | ---- | ------------------------ |
| table       | string        | Yes  | Table name.                  |
| fields      | Array&lt;string&gt; | Yes  | Array of names for fields to query.|
| queryCount  | number        | Yes  | Number of data records to query. The value must be greater than or equal to 1.|
...
| Name| Type  | Mandatory| Description                 |
| ------ | ------ | ---- | --------------------- |
| lockId | number | Yes  | ID of the lock whose period needs to be extended. The value is **lockId** in **LockInfo** returned by the [lock](#lock) method.|
...
| Name| Type  | Mandatory| Description         |
| ------ | ------ | ---- | ------------- |
| lockId | number | Yes  | Lock ID. The value is the same as that of **lockId** in **LockInfo** returned by the [lock](#lock) method.|
...
| Type                                                        | Description                                                        |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| Promise&lt;Record&lt;string, [AppBriefInfo](#appbriefinfo)&gt;&gt; | Promise used to return a key-value pair, where the key is **bundleName** and the value is [AppBriefInfo](#appbriefinfo).|
...
Subscribes to the cloud database change notification. This API uses a promise to return the result.
...
Unsubscribes from the cloud data change notification. This API uses a promise to return the result.
...
| Name         | Type                                  | Mandatory| Description                                                        |
| --------------- | -------------------------------------- | ---- | ------------------------------------------------------------ |
| unsubscribeInfo | Record&lt;string, Array<string&gt;&gt; | Yes  | Data to be unsubscribed from, in an array of KV pairs consisting of the application bundle name and database name.|
...
| prefix | string                                 | Yes  | Prefix of the asset download directory. |
...
Shares data. This API uses a promise to return the result. The application that initiates the share, shared resource ID, and participants of the share need to be specified.
...
Queries the participants of a share based on the invitation code. The application and the invitation code of the shared data need to be specified. This API uses a promise to return the result.
