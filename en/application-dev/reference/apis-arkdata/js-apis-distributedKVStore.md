...
<!--Tester: @logic42-->
<!--Adviser: @ge-yafang-->
<!-- md-trans-meta sourceCommit=fadd7d812e259ee0411353f410f7f8f5e48ebdff translatedAt=2026-09-15T14:32:06.152Z pushedAt=2026-09-16T07:50:15.762Z -->
...
| value | Uint8Array \| string \| number \| boolean| No    | No   | Value in the key-value pair. The length range of the Uint8Array and string types is 0 to [MAX_VALUE_LENGTH](#constants), and the value range of the number and boolean types is determined by their own types. |
...
> **NOTE**
>
> In single-device scenarios, a KV store supports upgrading its security level by modifying the **securityLevel** parameter. Note the following points for the upgrade:
> * This operation does not support databases with cross-device synchronization. Data synchronization cannot be performed between databases of different security levels. If you need to upgrade the security level of a database, you are advised to create a new database with a higher security level.
> * After closing the current database, modify the **securityLevel** parameter to reset the security level of the database, and then call [getKVStore](#getkvstore) to reopen the database.
> * This operation supports only upgrades, for example, from S2 to S3, and does not support downgrades, for example, from S3 to S2.
...
| Name  | Type              | Mandatory| Description                                                        |
| -------- | ---------------------- | ---- | ------------------------------------------------------------ |
| storeId  | string                 | Yes   | Unique identifier of the KV store. The KV store ID allows only letters, digits, and underscores (_), and its length ranges from 1 to [MAX_STORE_ID_LENGTH](#constants). |
...
try {
  const options: distributedKVStore.Options = {
    ...
    kvStoreType: distributedKVStore.KVStoreType.SINGLE_VERSION,
    securityLevel: distributedKVStore.SecurityLevel.S3,
    // Starting from API version 24, you can use rootDir to specify the database storage path.
...
| storeId  | string                    | Yes   | Unique identifier of the KV store to close. The KV store ID allows only letters, digits, and underscores (_), and its length ranges from 1 to [MAX_STORE_ID_LENGTH](#constants). |
...
const options: distributedKVStore.Options = {
  ...
  schema: undefined,
  securityLevel: distributedKVStore.SecurityLevel.S3,
  // Starting from API version 24, use rootDir to specify the database storage path.
  rootDir: "/data/storage/el2/database/entry"
}
try {
  kvManager.getKVStore<distributedKVStore.SingleKVStore>('storeId', options).then(async (store: distributedKVStore.SingleKVStore | null) => {
    ...
    store = null;
    if (kvManager != undefined) {
      // appId is the appId in createKVManager. If rootDir is not configured in options, closeKVStore does not require the options parameter.
...
| -------- | ------------------------- | ---- | ------------------------------------------------------------ |
| appId    | string                    | Yes   | Bundle name of the application. Must not be empty, with a length range of 1-256 bytes.                                      |
| storeId  | string                    | Yes   | Unique identifier of the KV store to delete. The KV store ID allows only letters, digits, and underscores (_), and its length ranges from 1 to [MAX_STORE_ID_LENGTH](#constants). |
| callback | AsyncCallback&lt;void&gt; | Yes  | Callback used to return the result. If the operation is successful, **err** is **undefined**. Otherwise, **err** is an error object.    |
...
const options: distributedKVStore.Options = {
  ...
  schema: undefined,
  securityLevel: distributedKVStore.SecurityLevel.S3,
  // Starting from API version 24, rootDir can be used to specify the database storage path.
  rootDir: "/data/storage/el2/database/entry"
}
try {
  kvManager.getKVStore<distributedKVStore.SingleKVStore>('storeId', options).then(async (store: distributedKVStore.SingleKVStore | null) => {
    ...
    store = null;
    if (kvManager != undefined) {
      // appId is the appId in createKVManager. If rootDir is not configured in options, deleteKVStore does not require the options parameter.
...
| value  | number \| string \| boolean  | Yes    | Value to match for the specified field. The value type must be consistent with the field type defined in the schema. |
...
| -----  | ------  | ----  | ----------------------- |
| field  | string  | Yes   |Field to match. It cannot contain '^'. If the value contains '^', the predicate becomes invalid and all data in the KV store will be returned. |
| value  | number \| string \| boolean  | Yes    | Value to be matched for the specified field. The value type must be consistent with the field type defined in the schema. |
...
| -----  | ------  | ----  | ----------------------- |
| field  | string  | Yes   |Field to match. It cannot contain '^'. If the value contains '^', the predicate becomes invalid and all data in the KV store will be returned. |
| value  | number \| string  | Yes    | Value to match for the specified field. The value type must be consistent with the field type defined in the schema. |
...
| -----  | ------  | ----  | ----------------------- |
| field  | string  | Yes   |Field to match. It cannot contain '^'. If the value contains '^', the predicate becomes invalid and all data in the KV store will be returned. |
| value  | number \| string  | Yes    | Value to be matched by the specified field. The value type must be consistent with the field type defined in the schema. |
...
| value  | Uint8Array \| string \| number \| boolean | Yes    | Value of the data to add. It supports Uint8Array, string, number, and boolean. The length of Uint8Array and string ranges from 0 to [MAX_VALUE_LENGTH](#constants). |
| callback | AsyncCallback&lt;void&gt; | Yes   |Callback used to return the result. If the operation is successful, **err** is **undefined**. Otherwise, **err** is an error object.  |
...
Batch inserts KV pairs into this single KV store. This API uses an asynchronous callback to return the result. If the key already exists, the corresponding value will be updated. If you have subscribed to data change notifications, the change notification callback will be triggered.
...
| callback  |AsyncCallback&lt;boolean \| string \| number \| Uint8Array&gt; | Yes    | Callback invoked to return the queried value. The value type depends on the data type used for storage.  |
...
| callback  |AsyncCallback&lt;boolean \| string \| number \| Uint8Array&gt;  | Yes    |Callback invoked to return the value matching the given condition (the value type depends on the data type at storage time) on success, or the error object on failure.    |
