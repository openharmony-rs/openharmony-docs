# OnRenderProcessNotRespondingCallback

```TypeScript
type OnRenderProcessNotRespondingCallback = (data : RenderProcessNotRespondingData) => void
```

Defines a callback invoked when the rendering process does not respond.

**Since:** 12

**System capability:** SystemCapability.Web.Webview.Core

**Parameters:**

| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| data | [RenderProcessNotRespondingData](arkts-arkweb-web-comp-renderprocessnotrespondingdata-i.md) | Yes | Detailed information about the unresponsive rendering process. |
