...
<!--Tester: @ghiker-->
<!--Adviser: @HelloShuo-->
<!-- md-trans-meta sourceCommit=ce74377d93811e6b2cc9767a2b0a616d3f69e471 translatedAt=2026-08-25T13:06:55.895Z pushedAt=2026-08-26T13:37:21.031Z -->
...
The current WebView restores the page state history from serialized data.
...
Sets whether to print the background of a web page. If the setting of this API is inconsistent with that of [PrintAttributes](../apis-basic-services-kit/js-apis-print.md#printattributes11), the setting of this API takes precedence.
...
5. Write the resource configuration information.
   ...
   export const resourceConfigs: Array<ResourceConfig> = [
     ...
     {
       localPath: "example.js",
       urlList: [ // Provide only one URL, which serves as both the source of the resource and the network request address of the resource.
    ...
    build() {
      Column() {
        ...
        Button('Cancel the trustlist.')
          .onClick(() => {
            try {
              // Pass an empty string to the trustlist to disable the trustlist mechanism, allowing all URLs to be accessed.
        ...
        Web({ src: 'http://untrust.example.com/test', controller: this.controller }).onControllerAttached(() => {
          try {
            // Set the trustlist in the onControllerAttached callback to ensure it takes effect before the URL is loaded. In this case, untrust web pages cannot be accessed and an error page is displayed.
            this.controller.setUrlTrustList(this.urltrustList);
          } catch (error) {
            console.error(`ErrorCode: ${(error as BusinessError).code},  Message: ${(error as BusinessError).message}`);
          }
...
| Type        | Description                |
| ------------ | -------------------- |
| [ControllerAttachState](./arkts-apis-webview-e.md) | Attach status of **WebViewController** and the **Web** component. |
...
| callback | Callback<[ControllerAttachState](./arkts-apis-webview-e.md)> | Yes | Callback triggered when the attach state of **WebViewController** changes. |
...
| -------- | -------- | -------- | -------- |
| type | string | Yes| Attach state event of **WebViewController**, whose value is fixed to **controllerAttachStateChange**.|
| callback | Callback<[ControllerAttachState](./arkts-apis-webview-e.md)> | No | Callback triggered when the attach state of **WebViewController** changes. By default, this parameter is left blank. If **Callback** is specified, only the specified callback is deregistered. Otherwise, all callbacks will be deregistered.<br>If **null** or **undefined** is passed, error code **401** is thrown. |
...
Asynchronously waits for the **WebViewController** to be attached to the **Web** component. If the attachment is complete or times out, a callback is triggered to return the current [ControllerAttachState](./arkts-apis-webview-e.md) through a promise.
...
| Type                          | Description                         |
| ------------------------------ | ----------------------------- |
| Promise<[ControllerAttachState](./arkts-apis-webview-e.md)> | Promise used to return the current [ControllerAttachState](./arkts-apis-webview-e.md). |
...
> **NOTE**
>
> This API is supported since API version 9 and deprecated since API version 18. You are advised to use [getLastHitTest](#getlasthittest18) instead.
...
For details about the sample code, see [setUserAgentClientHintsEnabled](#setuseragentclienthintsenabled24).
...
| metaData | [UserAgentMetadata](./arkts-apis-webview-UserAgentMetadata.md) | Yes  | **UserAgentMetadata** corresponding to the user agent. You can use [getUserAgentMetadata](#getuseragentmetadata24) to obtain the current default value and then modify it using the corresponding method.|
  ...
  build() {
    Column() {
      ...
      Button('Cancel the trustlist.')
        .onClick(() => {
          try {
            // Pass an empty string to the trustlist to disable the trustlist mechanism, allowing access to all URLs.
            this.controller.setUrlTrustList("");
          } catch (error) {
            console.error(`ErrorCode: ${(error as BusinessError).code},  Message: ${(error as BusinessError).message}`);
          }
...
> **NOTE**
>
> - When the web page is unavailable, the command cannot be executed, or no result is returned, the Promise returns an empty string.
> - When the return value is not empty, it is a JSON string. The application can parse it with `JSON.parse` before use.
...
| Type             | Description |
| ---------------- | ----------- |
| Promise\<string\> | Promise object. When the command is executed successfully, it returns the command execution result in JSON format. When the execution fails or no value is returned, it returns an empty string. |
...
| Name | Type | Mandatory | Description |
| -------- | ------- | ---- | -------------------------------------- |
| enable | boolean | Yes | Whether to enable the mainframe error page feature. The value **true** means to enable it, and **false** means to disable it. After it is enabled, an error page is displayed when the mainframe fails to load. |
| includeSubframe | boolean | Yes | Whether to enable the subframe error page feature at the same time. The value **true** means to enable it, and **false** means to disable it. After it is enabled, an error page is also displayed when the subframe fails to load. This parameter takes effect only when **enable** is **true**. |
  ...
  build() {
    Column() {
      Web({ src: $rawfile("iframe_error.html"), controller: this.controller })
        .onControllerAttached(() => {
          // Enable the error page function for mainframe and subframe.
          this.controller.setErrorPageEnabled(true, true);
          // Query whether the subframe error page function is enabled.
...
| Type                 | Description                      |
| -------------------- | ------------------------- |
| boolean | Returns whether the subframe error page function is enabled.<br>- true: The subframe error page function is enabled (that is, both enable and includeSubframe are true);<br>- false: The subframe error page function is not enabled (including two cases: the error page function is not enabled, or the error page function is enabled but the subframe error page function is not enabled).|
...
For the complete sample code, see [setErrorPageEnabled](#seterrorpageenabled).