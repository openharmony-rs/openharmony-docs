...
<!-- md-trans-meta sourceCommit=c066f5124ef720a50174e3224b1ec8b8036dfa24 translatedAt=2026-09-14T06:53:02.650Z pushedAt=2026-09-14T07:55:50.194Z -->
...
// 1. Configure the properties of a fixed-state soft keyboard panel.
let softKeyboardFixed = {
  type: PanelType.SOFT_KEYBOARD,
  flag: PanelFlag.FLAG_FIXED
};

// 2. Configure the properties of a floating-state soft keyboard panel.
let softKeyboardFloating = {
  type: PanelType.SOFT_KEYBOARD,
  flag: PanelFlag.FLAG_FLOATING
};

// 3. Configure the properties of a candidate word state panel.
let candidatePanel = {
  type: PanelType.SOFT_KEYBOARD,
...
> **NOTE**
>
> The visibility of a `FLAG_CANDIDATE` (candidate word state) panel is not controlled by the system. You need to manage the timing for showing and hiding the candidate word panel based on app scenarios.
...
| -------- | -------- | -------- | -------- | -------- |
| type | [PanelType](#paneltype) | No | No | Type of the input method panel. Determines whether the panel is a soft keyboard or a status bar. When the parameter is not specified, the default value is `SOFT_KEYBOARD`(0). |
| flag | [PanelFlag](#panelflag) | No | Yes | State type of the input method panel.<br/>- The default value is `FLAG_FIXED`(0), indicating a fixed state panel type.<br/>- Currently used only to describe the state of a soft keyboard type panel. Setting it for a `STATUS_BAR` type panel has no actual effect.<br/>- The show/hide behavior of the panel differs under different state types: `FLAG_FIXED` and `FLAG_FLOATING` are controlled by the system, while `FLAG_CANDIDATE` is controlled by you. |
