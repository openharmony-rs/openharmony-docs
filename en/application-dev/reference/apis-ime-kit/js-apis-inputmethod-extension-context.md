...
<!-- md-trans-meta sourceCommit=c066f5124ef720a50174e3224b1ec8b8036dfa24 translatedAt=2026-09-14T06:53:06.332Z pushedAt=2026-09-14T07:44:48.227Z -->
...
The **InputMethodExtensionContext** of this module must be obtained from a child class instance of **InputMethodExtensionAbility**. Its APIs are used in combination with the lifecycle callbacks of **InputMethodExtensionAbility**.

```javascript
// The following is pseudocode for illustrating the calling logic.

// 1. Define an InputMethodExtensionAbility child class.
class InputMethodExtAbility extends InputMethodExtensionAbility {
  ...
  onDestroy() {
    // Destroy itself in the lifecycle callback.
...
> **NOTE**
>
> The `InputMethodExtensionContext` instance is obtained through the `this.context` property of an `InputMethodExtensionAbility` child class and cannot be created directly. `destroy()` is usually called in the `onDestroy` lifecycle callback, and you can also proactively call it at other times to terminate the input method ExtensionAbility.
...
| -------- | -------------------- | ---- | ------------------------------------------------------------ |
| callback | [AsyncCallback](../apis-basic-services-kit/js-apis-base.md#asynccallback)&lt;void&gt;  | Yes   | Callback function. If the input method app is destroyed successfully, err is undefined; otherwise, it is an error object. |
...
| Name | Type                                                    | Mandatory | Description                                                         |
| ---- | ------------------------------------------------------- | --------- | ------------------------------------------------------------ |
| want | [Want](../apis-ability-kit/js-apis-app-ability-want.md) | Yes       | Want type information used to specify the target app, including the ability name, bundle name, and so on. |
