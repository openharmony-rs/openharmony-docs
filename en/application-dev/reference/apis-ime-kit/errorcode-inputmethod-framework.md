...
<!-- md-trans-meta sourceCommit=c066f5124ef720a50174e3224b1ec8b8036dfa24 translatedAt=2026-09-11T09:32:01.071Z pushedAt=2026-09-14T01:52:44.834Z -->
...
1. Attach the input method application to the application again: terminate the background process of the application, restart the application, and trigger the display of the input method keyboard. The issue is resolved if the keyboard is shown normally. To hide the keyboard, call the **hideTextInput** API which shall be used in pairs with **showTextInput**.
2. Switch the application to the foreground, ensure that it is not covered by other applications or windows, and trigger the keyboard to pop up.
...
This error code is thrown when the current application is not attached to an input method and operations such as **showTextInput** and **hideTextInput** are performed.
...
2. IPC fails because the data volume to transmit exceeds 1 MB or the data format is incorrect.
