...
<!-- md-trans-meta sourceCommit=11655b69cefb34a4a6c47c3aed59ee25121a8162 translatedAt=2026-09-22T13:40:27.747Z pushedAt=2026-09-23T09:56:29.560Z -->

## When to Use

Starting from API version 24, you can use HTTP global interceptors (which include read-only interceptors and modifiable interceptors) to monitor HTTP traffic in read-only interceptors for logging, or add custom logic in modifiable interceptors to modify request headers, response headers, response bodies, and more.
...
| Name | Description |
| -------- | -------- |
| OH_Http_AddReadOnlyInterceptor(struct OH_Http_Interceptor *interceptor) | Adds a global read-only HTTP interceptor. |
| OH_Http_AddWritableInterceptor(struct OH_Http_Interceptor *interceptor) | Adds a global modifiable HTTP interceptor. |
| OH_Http_RemoveInterceptor(struct OH_Http_Interceptor *interceptor) | Removes the specified HTTP global interceptor. |
| OH_Http_RemoveAllInterceptors(int32_t groupId) | Removes all HTTP global interceptors of the specified groupId. |
| OH_Http_StartAllInterceptors(int32_t groupId) | Enables all HTTP global interceptors of the specified groupId. |
| OH_Http_StopAllInterceptors(int32_t groupId) | Disables all HTTP global interceptors of the specified groupId. |
...
1. Write code in the source file to call the API, implementing the handler functions and related operations for HTTP global interceptors.
   ...
   #undef LOG_DOMAIN
   #undef LOG_TAG
   #define LOG_DOMAIN 0x3200 // Global domain macro that identifies the business domain.
   #define LOG_TAG "HttpInterceptorDemo"  // Global tag macro that identifies the module log tag.
   
   // Global read-only response interceptor instance.
   ...
   // Helper function for memory allocation and string copy.
   ...
   static void ModifyRequestMethod(OH_Http_Interceptor_Request *request)
   {
       if (request->method.buffer != nullptr) {
           // Free the original memory. Use free to release memory allocated by malloc.
   ...
   static void ModifyFirstHeaderNode(OH_Http_Interceptor_Headers **headers, const char *headerData)
   {
       ...
       if (*headers != nullptr) {
           ...
           // If there is no header node, create a new first node.
   ...
   {
       // Release the original body memory. Use free to release the memory allocated by malloc.
       if (body->buffer != nullptr) {
           free((void *)body->buffer);
       }
       
       // Reallocate memory and set the new body content. Use malloc to allocate memory.
   ...
   // Modifiable request interceptor handler (modifies the request packet of Network Kit).
       ...
       int32_t *isModified)
   {
       ...
       // Return OH_ABORT to abort the request. The request will not be sent to the server.
       return OH_CONTINUE;
   }
       ...
       int32_t *isModified)
   {
       ...
       // Return OH_ABORT to terminate the current interceptor chain.
       return OH_CONTINUE;
   }
   
   // Add a read-only response interceptor.
   ...
   static napi_value StartReadOnlyResponseInterceptors(napi_env env, napi_callback_info info)
   {
       ...
       // Enable all interceptors with groupId 1.
   ...
   static napi_value StopReadOnlyResponseInterceptors(napi_env env, napi_callback_info info)
   {
       ...
       // Disable all interceptors with groupId 1.
   ...
   // Remove the modifiable request interceptor.
   ...
   static napi_value StartModifyRequestInterceptors(napi_env env, napi_callback_info info)
   {
       ...
       // Enable all interceptors with groupId 2.
   ...
   static napi_value StopModifyRequestInterceptors(napi_env env, napi_callback_info info)
   {
       ...
       // Disable all interceptors with groupId 2.
   ...
   // Add a modifiable response interceptor (of the OH_TYPE_MODIFY_NETWORK_KIT type).
   ...
   static napi_value StartModifyResponseInterceptors(napi_env env, napi_callback_info info)
   {
       ...
       // Enable all interceptors with groupId 3.
   ...
   static napi_value StopModifyResponseInterceptors(napi_env env, napi_callback_info info)
   {
       ...
       // Disable all interceptors with groupId 3.
...
3. Register the objects successfully initialized in the previous step into `Node.js` by using the `napi_module_register` function in `RegisterEntryModule`.
...
![Initial demo screen](./figures/httpinterceptor-demo-2.png)
   ...
   - Click `Add Read Only Response Interceptor` to add a global read-only HTTP response interceptor.
...
![netmanager-2.png](./figures/httpinterceptor-result2.png)
   ...
   - Click `Add Modify Request Interceptor` to add a global modifiable HTTP request interceptor.
...
![netmanager-8.png](./figures/httpinterceptor-result8.png)
   ...
   - Click `Stop Modify Request Interceptors` to disable the modifiable request interceptor with group ID 2.

![netmanager-9.png](./figures/httpinterceptor-result9.png)

   - Click `Stop Modify Response Interceptors` to disable the modifiable response interceptor with group ID 3.
...
The following sample is provided for your reference on HTTP global interceptor development:
