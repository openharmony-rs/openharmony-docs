...
<!--Tester: @liuhaonan2-->
<!--Adviser: @jinqiuheng-->
<!-- md-trans-meta sourceCommit=2da72922843058c24813f1af5c7f3a336951e9b5 translatedAt=2026-08-26T04:27:33.230Z pushedAt=2026-08-28T03:14:37.191Z -->
...
- The system maps a dedicated application sandbox directory in the internal storage for each application. The application sandbox directory contains only the [application file directory](#application-file-directory-and-application-file-path) and the directory containing the minimum system files required for the running of the application.
...
Under the application sandbox protection mechanism, an application is not aware of the existence and location of other applications' or users' data directories, except its own application file directory. In addition, the directories visible to all applications are isolated by permission and file path mount isolation, forming an independent path view and shielding the actual physical path:
...
> **NOTE**
>
> - Do not use file paths made up of directory names before the level-4 directories in the figure above. Otherwise, incompatibility issues may occur in later app versions due to changes in application file paths.
...
3. Third-level directories **el1/** to **el5/**: different file encryption types.
    ...
    Encryption Level 2 (EL2):
     - On the basis of EL1, files are protected after the first user authentication. After the device is not powered off, users can access the files protected by EL2 only after the first authentication. Those files are always accessible as long as the device is online. This mode is recommended by default.
    ...
    Encryption Level 5 (EL5):
     ...
     - EL5 directories are not generated by default unless the permissions for access to the EL5 database are configured. For details, see [Using an EL5 Database (ArkTS)](../database/encrypted-estore-guidelines.md)

   > **NOTE**
   >
   > - Unless otherwise required, application data should
be placed in the **el2** directory for security purposes. The data that needs to be accessed before the first user authentication (such as the clock, alarm, and wallpaper data) is placed in the **el1** directory.
   >
...
1. Level 4 and level 5 directories:
