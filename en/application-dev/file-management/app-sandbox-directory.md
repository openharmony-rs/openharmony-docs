...
1. Level 1 directory **data/**: application file root directory.
...
3. There are files of different encryption levels in **el1/** and **el5/**.
    ...
    Encryption Level 5 (EL5):
     ...
     - EL5 directories are not generated by default unless the permissions for access to the EL5 database are configured. For details, see [Using an EL5 Database](../database/encrypted-estore-guidelines.md).
...
4. Level 4 and level 5 directories:
   ...
   **Table 1** Application file paths
   ...
   | distributedfiles | distributedFilesDir | Distributed file directory| Directory in **el2** for saving the application files that can be directly accessed across devices.<br>This directory is cleared when the application is uninstalled.<br>You can place the application's data used for distributed scenarios, including file sharing, file backup, and file processing across devices, in this directory. The data stored in this directory enables an application to run smoothly on multiple devices that form a Super Device.<br> **Note:**<br>The **.remote_share** directory under **distributedfiles** is created and managed by the system. Creation or modification of this directory by applications is not permitted.<br>The **distributedfiles** directory supports direct access to files on other devices. Files in this directory should not be deleted without permission. For details, see [Accessing Files Across Devices](file-access-across-devices.md).|
