...
<!--Tester: @zheng1368-->
<!--Adviser: @jinqiuheng-->
<!-- md-trans-meta sourceCommit=51a4d474cf496c221e08074148023a74c0addf3b translatedAt=2026-08-26T04:30:17.258Z pushedAt=2026-08-28T03:23:36.878Z -->
...
{
    // Obtain the input and output file paths.
    size_t argc = 2;
    ...
    // Initialize the streaming compression configuration and create a streaming compression writer.
    ...
    // Open the output file to write the compressed data.
...
3. Call **OH_Archive_StreamWrite_Update** multiple times to input compressed data.
...
static napi_value StreamCompressCancel(napi_env env, napi_callback_info info)
{
    ...
    uint64_t read = 0;
    uint64_t cancelThreshold = 2000; // Set the cancellation threshold to 2000 bytes.
    while ((read = fread(buffer, 1, bufferSize, fi)) > 0) {
        totalSize += read;
        if (totalSize > cancelThreshold) {
            // Cancel streaming compression when the accumulated data exceeds the threshold.
    ...
    (void)fclose(fi);
    // End streaming compression; do not obtain compression result information in the cancellation scenario.
...
static napi_value StreamDecompress(napi_env env, napi_callback_info info)
{
    ...
    // First generate the compressed data for testing through streaming compression.
    ...
    // Input the compressed data in segments for decompression.
    uint8_t *unzPtr = (uint8_t *)zipBuffer;
    size_t unzLeft = compressInfo.totalOutSize;

    while (unzLeft > 0) {
        size_t unzchunk = (unzLeft > 1024) ? 1024 : unzLeft; // Process up to 1024 bytes at a time.
    ...
    // End streaming decompression and obtain the decompression result information.
...
static napi_value StreamDecompressCancel(napi_env env, napi_callback_info info)
{
    ...
    // Input the compressed data in segments for decompression, and cancel decompression when the remaining data is less than the threshold.
    while (unzLeft > 0) {
        size_t unzchunk = (unzLeft > 1024) ? 1024 : unzLeft; // Process up to 1024 bytes at a time.
    ...
    // End streaming decompression. In the cancellation scenario, do not obtain the decompression result information.
