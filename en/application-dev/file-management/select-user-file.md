...
<!--Adviser: @jinqiuheng-->
<!-- md-trans-meta sourceCommit=563f325fc4197af705064cb3a103e60b86e4aa4f translatedAt=2026-08-26T04:37:45.148Z pushedAt=2026-08-28T07:17:25.885Z -->
...
2. Configure [document selection options](../reference/apis-core-file-kit/js-apis-file-picker.md#documentselectoptions) based on your service requirements. The following code only provides configuration examples for each option.
   ...
   // Document type to select. The default value is FILE (file type). Since API version 26.0.0, when the file type is FOLDER, this parameter is supported on Phone devices; when the file type is MIXED, this parameter works properly on 2-in-1 devices and has no effect on other devices.
   ...
   // Whether to authorize the specified file or directory. The value true means authorization. When it is true, defaultFilePathUri is a mandatory parameter and the file management authorization UI is displayed. The value false means no authorization (the default value is false), and the regular file management UI is displayed (optional). This parameter works properly on 2-in-1 devices and has no effect on other devices.
   documentSelectOptions.authMode = false;
   // Batch authorization mode. The default value is false (non-batch authorization mode). When multiAuthMode is true, the batch authorization mode is enabled. When multiAuthMode is true, only the multiUriArray parameter takes effect, and other parameters do not take effect. This parameter works properly on Phone devices and has no effect on other devices.
   documentSelectOptions.multiAuthMode = false;
   // URI array for batch authorization (only files are supported; folders do not take effect). Use it together with multiAuthMode. When multiAuthMode is false, configuring this parameter does not take effect. This parameter works properly on Phone devices and has no effect on other devices.
   documentSelectOptions.multiUriArray = ["file://docs/storage/Users/currentUser/test", "file://docs/storage/Users/currentUser/2test"];
   // Whether to enable the aggregate view mode, which supports displaying the aggregate view of the file management application. The default value is DEFAULT, indicating that this parameter does not take effect and the non-aggregate view is used. When this parameter is set to a value other than DEFAULT, other parameters do not take effect. Since API version 26.0.0, when this parameter is set to a value other than DEFAULT, only the fileSuffixFilters parameter takes effect, and other parameters do not take effect.
   // This parameter works properly on Phone devices and has no effect on other devices.
   documentSelectOptions.mergeMode = picker.MergeTypeMode.DEFAULT;
   // Whether encryption is supported (only files are supported; folders do not take effect). The default value is false. When this parameter is true, you can choose to encrypt files in the picker UI. (Note: This parameter is supported since API version 19.)
   documentSelectOptions.isEncryptionSupported = false;
   // Whether multiple folder selection is supported. The default value is false, indicating that multiple folder selection is not supported. It must be used together with the selectMode parameter. (Note: This parameter is supported since API version 26.0.0.)
...
4. After the application UI is returned from FilePicker, use [fileIo.openSync](../reference/apis-core-file-kit/js-apis-file-fs.md#fileioopensync) to open the file based on the URI and obtain the file descriptor (FD).
   ...
   if (uris.length > 0) {
     ...
     // Note that the API permission parameter is fileIo.OpenMode.READ_ONLY.
...
5. Use [fileIo.readSync](../reference/apis-core-file-kit/js-apis-file-fs.md#fileioreadsync) to read data from the file through the FD.
   ...
   // Close the fd after reading is complete.
...
3. Create an [AudioViewPicker](../reference/apis-core-file-kit/js-apis-file-picker.md#audioviewpicker) instance. Call [select()](../reference/apis-core-file-kit/js-apis-file-picker.md#select-5) to launch the AudioPicker app UI for file selection.
   ...
   audioViewPicker.select(audioSelectOptions).then((audioSelectResult: Array<string>) => {
     // After the file is selected successfully, the URI result set of the selected audio is returned.
...
For user file selection, the following sample is available:
