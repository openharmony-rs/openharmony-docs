...
<!-- md-trans-meta sourceCommit=deff468b8adbfa4199da5cbe7b6cbc33f2bddb1e translatedAt=2026-06-24T07:39:14.541Z pushedAt=2026-06-25T06:39:21.772Z -->
...
Generally, an app must have permission to access the system **Contacts**. When the user selects a contact, the Picker displays the contact list, directing the user to complete the operation. The app does not need to apply for the permission.
...
3. Perform the corresponding operation on the contact.
   ...
   struct Contact {
     addContactByPermissions() {
       // Obtain the context within the component and ensure that this.getUIContext().getHostContext() returns UIAbilityContext.
       ...
       abilityAccessCtrl.createAtManager().requestPermissionsFromUser(context, permissions).
         then((result: PermissionRequestResult) => {
          if (result.authResults[0] !== 0) { // 0 indicates that the permission request is successful, and any other non-zero value indicates that the request failed.
