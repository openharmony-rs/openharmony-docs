...
From API version 22, **huksExternalCrypto** provides the general query API. This API can be used to obtain the device ID, application ID, and other general attributes from an UKey. For details about the scenarios, see [General Query Overview and Specifications](huks-ukey-general-query-overview.md).
...
3. Call [getProperty](../../reference/apis-universal-keystore-kit/js-apis-huksExternalCrypto.md#huksexternalcryptogetproperty) to obtain the attribute information.
...
async function getProperty(): Promise<Array<huksExternalCrypto.HuksExternalCryptoParam>> {
  ...
  // 3. Call getProperty to obtain the attribute information.
