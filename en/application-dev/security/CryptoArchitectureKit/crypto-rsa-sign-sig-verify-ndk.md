# Signature Signing and Verification with an RSA Key Pair (C/C++)

<!--Kit: Crypto Architecture Kit-->
<!--Subsystem: Security-->
<!--Owner: @zxz--3-->
<!--Designer: @lanming-->
<!--Tester: @PAFT-->
<!--Adviser: @zengyawen-->
<!-- md-trans-meta sourceCommit=bf0ea2f25cd22c7bf410b5832735694d430dd0e4 translatedAt=2026-08-31T02:35:54.794Z pushedAt=2026-08-31T09:18:01.152Z -->

For the corresponding algorithm specifications, see [RSA Signature Signing and Verification Algorithm Specifications](crypto-sign-sig-verify-overview.md#rsa).

## Link the Related Dynamic Library in the CMake Script
```txt
target_link_libraries(entry PUBLIC libohcrypto.so)
```

## How to Develop

### Signature Signing and Verification with an RSA Key Pair (PKCS1 Mode)

**Signature development steps**

1. Call [OH_CryptoSign_Create](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_create), specify the string parameter 'RSA1024|PKCS1|SHA256', and create a Sign instance for the signature operation.

2. Call [OH_CryptoSign_Init](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_init), and initialize the Sign instance using the private key (OH_CryptoPrivKey).

3. Call [OH_CryptoSign_Update](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_update), and pass the data to be signed. The current single operation has no length limit for update, so you can determine how to call update based on the amount of data. If the data is small, you can directly call the OH_CryptoSign_Final interface once to pass it in.

4. Call [OH_CryptoSign_Final](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_final), and obtain the data after signing.

5. Call [OH_CryptoSign_Destroy](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_destroy) to release memory.

<!-- @[pkcs1_rsa_keypair_sign](https://gitcode.com/openharmony/applications_app_samples/blob/master/code/DocsSample/Security/CryptoArchitectureKit/SignatureVerification/SigningSignatureVerification/entry/src/main/cpp/types/project/rsa_pkcs1_signature.cpp) -->

``` C++
#include "CryptoArchitectureKit/crypto_common.h"
#include "CryptoArchitectureKit/crypto_signature.h"
#include "CryptoArchitectureKit/crypto_asym_key.h"

static void CleanupRsaSignResources(OH_CryptoAsymKeyGenerator *keyCtx, OH_CryptoKeyPair *keyPair,
    OH_CryptoSign *sign, Crypto_DataBlob *signData)
{
    if (sign != nullptr) {
        OH_CryptoSign_Destroy(sign);
    }
    if (keyPair != nullptr) {
        OH_CryptoKeyPair_Destroy(keyPair);
    }
    if (keyCtx != nullptr) {
        OH_CryptoAsymKeyGenerator_Destroy(keyCtx);
    }
    if (signData != nullptr && signData->data != nullptr) {
        OH_Crypto_FreeDataBlob(signData);
    }
}

bool DoTestRsaSignature()
{
    OH_CryptoAsymKeyGenerator *keyCtx = nullptr;
    OH_CryptoKeyPair *keyPair = nullptr;
    OH_CryptoSign *sign = nullptr;
    Crypto_DataBlob signData = {.data = nullptr, .len = 0};
    uint8_t plainText[] = {
        0x43, 0x31, 0x7d, 0xb5, 0x85, 0x2e, 0xd4, 0xef, 0x08, 0x7a, 0x17, 0x96, 0xbc, 0x7c, 0x8f, 0x80,
        0x8c, 0xa7, 0x63, 0x7f, 0x26, 0x89, 0x8f, 0xf0, 0xfa, 0xa7, 0x51, 0xbd, 0x9c, 0x69, 0x17, 0xf3,
        0xd1, 0xb5, 0xc7, 0x12, 0xbf, 0xcf, 0x91, 0x25, 0x82, 0x23, 0x6b, 0xd6, 0x64, 0x52, 0x77, 0x93,
        0x01, 0x9d, 0x70, 0xa3, 0xf4, 0x92, 0x16, 0xec, 0x3f, 0xa7, 0x3c, 0x83, 0x8d, 0x40, 0x41, 0xfc,
    };
    Crypto_DataBlob msgBlob = {.data = reinterpret_cast<uint8_t *>(plainText), .len = sizeof(plainText)};

    OH_Crypto_ErrCode ret = OH_CryptoAsymKeyGenerator_Create((const char *)"RSA2048", &keyCtx);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    ret = OH_CryptoAsymKeyGenerator_Generate(keyCtx, &keyPair);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    OH_CryptoPrivKey *privKey = OH_CryptoKeyPair_GetPrivKey(keyPair);
    ret = OH_CryptoSign_Create((const char *)"RSA1024|PKCS1|SHA256", &sign);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    ret = OH_CryptoSign_Init(sign, privKey);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    ret = OH_CryptoSign_Update(sign, &msgBlob);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    ret = OH_CryptoSign_Final(sign, nullptr, &signData);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    CleanupRsaSignResources(keyCtx, keyPair, sign, &signData);
    return true;
}
```

**Verification development steps**

1. Call [OH_CryptoVerify_Create](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_create), specify the string parameter 'RSA1024|PKCS1|SHA256', which must be consistent with the Sign instance used for signing. Create a Verify instance for the verification operation.

2. Call [OH_CryptoVerify_Init](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_init), and initialize the Verify instance using the public key (OH_CryptoPubKey).

3. Call [OH_CryptoVerify_Update](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_update), and pass in the data to be verified.

   There is no limit on the length of a single `update`. You can determine how to call `update` based on the amount of data. If the amount of data is small, you can directly call `OH_CryptoVerify_Final` to pass in all the data at once.

   - When the data to be verified is short, you can directly call [OH_CryptoVerify_Final](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_final) after initialization is complete.
   - When the amount of data is large, you can call `update` multiple times, that is, [segmented verification](crypto-rsa-sign-sig-verify-ndk.md#segmented-signature-signing-and-verification-using-an-rsa-key-pair-pkcs1-mode).

4. Call [OH_CryptoVerify_Final](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_final), and perform verification on the data.

<!-- @[pkcs1_verify_rsa_keypair_sign](https://gitcode.com/openharmony/applications_app_samples/blob/master/code/DocsSample/Security/CryptoArchitectureKit/SignatureVerification/SigningSignatureVerification/entry/src/main/cpp/types/project/rsa_pkcs1_signature_validator.cpp) -->

``` C++
#include "signing_signature_verification.h"

static const uint8_t g_plainText[] = {
    0x43, 0x31, 0x7d, 0xb5, 0x85, 0x2e, 0xd4, 0xef, 0x08, 0x7a, 0x17, 0x96, 0xbc, 0x7c, 0x8f, 0x80,
    0x8c, 0xa7, 0x63, 0x7f, 0x26, 0x89, 0x8f, 0xf0, 0xfa, 0xa7, 0x51, 0xbd, 0x9c, 0x69, 0x17, 0xf3,
    0xd1, 0xb5, 0xc7, 0x12, 0xbf, 0xcf, 0x91, 0x25, 0x82, 0x23, 0x6b, 0xd6, 0x64, 0x52, 0x77, 0x93,
    0x01, 0x9d, 0x70, 0xa3, 0xf4, 0x92, 0x16, 0xec, 0x3f, 0xa7, 0x3c, 0x83, 0x8d, 0x40, 0x41, 0xfc,
};

static const uint8_t g_pubKeyText[] = {
    ,
};

static const uint8_t g_signText[] = {
    ,
};

static void CleanupRsaVerifyResources(
    OH_CryptoAsymKeyGenerator *keyCtx, OH_CryptoKeyPair *keyPair, OH_CryptoVerify *verify)
{
    if (verify != nullptr) {
        OH_CryptoVerify_Destroy(verify);
    }
    if (keyPair != nullptr) {
        OH_CryptoKeyPair_Destroy(keyPair);
    }
    if (keyCtx != nullptr) {
        OH_CryptoAsymKeyGenerator_Destroy(keyCtx);
    }
}

bool DoTestRsaVerify()
{
    OH_CryptoAsymKeyGenerator *keyCtx = nullptr;
    OH_CryptoKeyPair *keyPair = nullptr;
    OH_CryptoVerify *verify = nullptr;
    Crypto_DataBlob msgBlob = {.data = const_cast<uint8_t *>(g_plainText), .len = sizeof(g_plainText)};
    Crypto_DataBlob keyBlob = {.data = const_cast<uint8_t *>(g_pubKeyText), .len = sizeof(g_pubKeyText)};
    Crypto_DataBlob signBlob = {.data = const_cast<uint8_t *>(g_signText), .len = sizeof(g_signText)};

    OH_Crypto_ErrCode ret = OH_CryptoAsymKeyGenerator_Create((const char *)"RSA2048", &keyCtx);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    ret = OH_CryptoAsymKeyGenerator_Convert(keyCtx, CRYPTO_PEM, &keyBlob, nullptr, &keyPair);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    OH_CryptoPubKey *pubKey = OH_CryptoKeyPair_GetPubKey(keyPair);
    ret = OH_CryptoVerify_Create((const char *)"RSA1024|PKCS1|SHA256", &verify);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    ret = OH_CryptoVerify_Init(verify, pubKey);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    bool res = OH_CryptoVerify_Final(verify, &msgBlob, &signBlob);
    CleanupRsaVerifyResources(keyCtx, keyPair, verify);
    return res;
}
```

### Recovering a Signature Using an RSA Key Pair (PKCS1 Mode)

**How to Develop**

1. Call [OH_CryptoVerify_Create](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_create) with the string parameter 'RSA1024|PKCS1|SHA256|Recover', which must be consistent with the Sign instance used for signing. Create a Verify instance to perform the verification operation.

2. Call [OH_CryptoVerify_Init](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_init) to initialize the Verify instance using the public key (OH_CryptoPubKey).

3. Call [OH_CryptoVerify_Recover](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_recover) to recover the signature from the data.

<!-- @[pkcs1_recover_rsa_keypair_sign](https://gitcode.com/openharmony/applications_app_samples/blob/master/code/DocsSample/Security/CryptoArchitectureKit/SignatureVerification/SigningSignatureVerification/entry/src/main/cpp/types/project/rsa_pkcs1_signature_restoration.cpp) -->

``` C++
#include "signing_signature_verification.h"

static void CleanupRsaRecoverResources(OH_CryptoAsymKeyGenerator *keyCtx,
    OH_CryptoKeyPair *keyPair, OH_CryptoVerify *verify)
{
    if (verify != nullptr) {
        OH_CryptoVerify_Destroy(verify);
    }
    if (keyPair != nullptr) {
        OH_CryptoKeyPair_Destroy(keyPair);
    }
    if (keyCtx != nullptr) {
        OH_CryptoAsymKeyGenerator_Destroy(keyCtx);
    }
}

static const uint8_t g_pubKeyText[] = {
    ,
};

static const uint8_t g_signText[] = {
    ,
};

OH_Crypto_ErrCode DoTestRsaSignatureRecover()
{
    OH_CryptoAsymKeyGenerator *keyCtx = nullptr;
    OH_CryptoKeyPair *keyPair = nullptr;
    OH_CryptoVerify *verify = nullptr;

    Crypto_DataBlob keyBlob = {.data = const_cast<uint8_t *>(g_pubKeyText), .len = sizeof(g_pubKeyText)};
    Crypto_DataBlob signBlob = {.data = const_cast<uint8_t *>(g_signText), .len = sizeof(g_signText)};
    // keypair
    OH_Crypto_ErrCode ret = CRYPTO_SUCCESS;
    ret = OH_CryptoAsymKeyGenerator_Create((const char *)"RSA2048", &keyCtx);
    if (ret != CRYPTO_SUCCESS) {
        return ret;
    }
    ret = OH_CryptoAsymKeyGenerator_Convert(keyCtx, CRYPTO_PEM, &keyBlob, nullptr, &keyPair);
    if (ret != CRYPTO_SUCCESS) {
        OH_CryptoAsymKeyGenerator_Destroy(keyCtx);
        return ret;
    }
    OH_CryptoPubKey *pubKey = OH_CryptoKeyPair_GetPubKey(keyPair);
    // verify
    ret = OH_CryptoVerify_Create((const char *)"RSA1024|PKCS1|SHA256|Recover", &verify);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaRecoverResources(keyCtx, keyPair, verify);
        return ret;
    }
    ret = OH_CryptoVerify_Init(verify, pubKey);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaRecoverResources(keyCtx, keyPair, verify);
        return ret;
    }
    Crypto_DataBlob rawSignData = {.data = nullptr, .len = 0};
    ret = OH_CryptoVerify_Recover(verify, (Crypto_DataBlob *)&signBlob, &rawSignData);
    CleanupRsaRecoverResources(keyCtx, keyPair, verify);
    return ret;
}
```

### Segmented Signature Signing and Verification Using an RSA Key Pair (PKCS1 Mode)

**Signature development steps**

1. Call [OH_CryptoSign_Create](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_create), specify the string parameter 'RSA1024|PKCS1|SHA256', and create a Sign instance for the signature operation.

2. Call [OH_CryptoSign_Init](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_init), and initialize the Sign instance using the private key (OH_CryptoPrivKey).

3. Call [OH_CryptoSign_Update](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_update), and pass the data to be signed. The current single update operation has no length restriction. You can determine how to call update based on the amount of data. If the data amount is small, they can directly call the OH_CryptoSign_Final interface once to pass it in.

4. Call [OH_CryptoSign_Final](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_final), and obtain the data after signing.

5. Call [OH_CryptoSign_Destroy](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_destroy) and other APIs to release memory.

<!-- @[pkcs1_seg_rsa_keypair_sign](https://gitcode.com/openharmony/applications_app_samples/blob/master/code/DocsSample/Security/CryptoArchitectureKit/SignatureVerification/SigningSignatureVerification/entry/src/main/cpp/types/project/rsa_pkcs1_segment_signature.cpp) -->

``` C++
#include "CryptoArchitectureKit/crypto_common.h"
#include "CryptoArchitectureKit/crypto_asym_key.h"
#include "CryptoArchitectureKit/crypto_signature.h"

static constexpr int INT_64 = 64;

static void CleanupRsaSignSegResources(OH_CryptoAsymKeyGenerator *keyCtx,
    OH_CryptoKeyPair *keyPair, OH_CryptoSign *sign, Crypto_DataBlob *signData)
{
    if (sign != nullptr) {
        OH_CryptoSign_Destroy(sign);
    }
    if (keyPair != nullptr) {
        OH_CryptoKeyPair_Destroy(keyPair);
    }
    if (keyCtx != nullptr) {
        OH_CryptoAsymKeyGenerator_Destroy(keyCtx);
    }
    if (signData != nullptr && signData->data != nullptr) {
        OH_Crypto_FreeDataBlob(signData);
    }
}

static void InitRsaSignSegTestData(Crypto_DataBlob *msgBlob)
{
    uint8_t plainText[] = {
        0x43, 0x31, 0x7d, 0xb5, 0x85, 0x2e, 0xd4, 0xef, 0x08, 0x7a, 0x17, 0x96, 0xbc, 0x7c, 0x8f, 0x80,
        0x8c, 0xa7, 0x63, 0x7f, 0x26, 0x89, 0x8f, 0xf0, 0xfa, 0xa7, 0x51, 0xbd, 0x9c, 0x69, 0x17, 0xf3,
        0xd1, 0xb5, 0xc7, 0x12, 0xbf, 0xcf, 0x91, 0x25, 0x82, 0x23, 0x6b, 0xd6, 0x64, 0x52, 0x77, 0x93,
        0x01, 0x9d, 0x70, 0xa3, 0xf4, 0x92, 0x16, 0xec, 0x3f, 0xa7, 0x3c, 0x83, 0x8d, 0x40, 0x41, 0xfc,
    };
    msgBlob->data = reinterpret_cast<uint8_t *>(plainText);
    msgBlob->len = sizeof(plainText);
}

static OH_Crypto_ErrCode RsaSignSegmented(OH_CryptoSign *sign, Crypto_DataBlob *msgBlob,
    Crypto_DataBlob *signData, int totalLen)
{
    int blockSize = 20;
    int cnt = totalLen / blockSize;
    int rem = totalLen % blockSize;
    OH_Crypto_ErrCode ret = CRYPTO_SUCCESS;
    for (int i = 0; i < cnt; i++) {
        msgBlob->len = blockSize;
        ret = OH_CryptoSign_Update(sign, msgBlob);
        if (ret != CRYPTO_SUCCESS) {
            return ret;
        }
        msgBlob->data += blockSize;
    }
    if (rem > 0) {
        msgBlob->len = rem;
        ret = OH_CryptoSign_Final(sign, msgBlob, signData);
    }
    msgBlob->data -= totalLen - rem;
    msgBlob->len = totalLen;
    return ret;
}

bool DoTestRsaSignatureSeg()
{
    OH_CryptoAsymKeyGenerator *keyCtx = nullptr;
    OH_CryptoKeyPair *keyPair = nullptr;
    OH_CryptoSign *sign = nullptr;
    Crypto_DataBlob signData = {.data = nullptr, .len = 0};
    Crypto_DataBlob msgBlob = {nullptr, 0};
    InitRsaSignSegTestData(&msgBlob);
    OH_Crypto_ErrCode ret = OH_CryptoAsymKeyGenerator_Create((const char *)"RSA2048", &keyCtx);
    if (ret != CRYPTO_SUCCESS) {
        return false;
    }
    ret = OH_CryptoAsymKeyGenerator_Generate(keyCtx, &keyPair);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSignSegResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    OH_CryptoPrivKey *privKey = OH_CryptoKeyPair_GetPrivKey(keyPair);
    ret = OH_CryptoSign_Create((const char *)"RSA1024|PKCS1|SHA256", &sign);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSignSegResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    ret = OH_CryptoSign_Init(sign, privKey);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSignSegResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    ret = RsaSignSegmented(sign, &msgBlob, &signData, INT_64);
    CleanupRsaSignSegResources(keyCtx, keyPair, sign, &signData);
    return ret == CRYPTO_SUCCESS;
}
```


**Verification development steps**

1. Call [OH_CryptoVerify_Create](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_create), specify the string parameter 'RSA1024|PKCS1|SHA256', and keep it consistent with the Sign instance used for signing. Create a Verify instance for the verification operation.

2. Call [OH_CryptoVerify_Init](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_init), and initialize the Verify instance using the public key (OH_CryptoPubKey).

3. Call [OH_CryptoVerify_Update](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_update) to pass the data to be verified. There is no limit on the length of a single `update` operation. You can determine how to call `update` based on the amount of data. If the amount of data is small, you can pass it all at once by calling `OH_CryptoVerify_Final` directly.

4. Call [OH_CryptoVerify_Final](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_final) to verify the data.

<!-- @[pkcs1_seg_verify_rsa_keypair_sign](https://gitcode.com/openharmony/applications_app_samples/blob/master/code/DocsSample/Security/CryptoArchitectureKit/SignatureVerification/SigningSignatureVerification/entry/src/main/cpp/types/project/rsa_pkcs1_segment_signature_verify.cpp) -->

``` C++
#include "signing_signature_verification.h"

static constexpr int INT_64 = 64;

static const uint8_t g_plainText[] = {
    0x43, 0x31, 0x7d, 0xb5, 0x85, 0x2e, 0xd4, 0xef, 0x08, 0x7a, 0x17, 0x96, 0xbc, 0x7c, 0x8f, 0x80,
    0x8c, 0xa7, 0x63, 0x7f, 0x26, 0x89, 0x8f, 0xf0, 0xfa, 0xa7, 0x51, 0xbd, 0x9c, 0x69, 0x17, 0xf3,
    0xd1, 0xb5, 0xc7, 0x12, 0xbf, 0xcf, 0x91, 0x25, 0x82, 0x23, 0x6b, 0xd6, 0x64, 0x52, 0x77, 0x93,
    0x01, 0x9d, 0x70, 0xa3, 0xf4, 0x92, 0x16, 0xec, 0x3f, 0xa7, 0x3c, 0x83, 0x8d, 0x40, 0x41, 0xfc,
};

static const uint8_t g_pubKeyText[] = {
    ,
};

static const uint8_t g_signText[] = {
    ,
};

static void CleanupRsaSegVerifyResources(
    OH_CryptoAsymKeyGenerator *keyCtx, OH_CryptoKeyPair *keyPair, OH_CryptoVerify *verify)
{
    if (verify != nullptr) {
        OH_CryptoVerify_Destroy(verify);
    }
    if (keyPair != nullptr) {
        OH_CryptoKeyPair_Destroy(keyPair);
    }
    if (keyCtx != nullptr) {
        OH_CryptoAsymKeyGenerator_Destroy(keyCtx);
    }
}

static bool VerifySegmented(OH_CryptoVerify *verify, Crypto_DataBlob *msgBlob, Crypto_DataBlob *signBlob)
{
    int blockSize = 20;
    int cntS = INT_64 / blockSize;
    int remS = INT_64 % blockSize;
    bool res = false;
    for (int i = 0; i < cntS; i++) {
        msgBlob->len = blockSize;
        OH_Crypto_ErrCode ret = OH_CryptoVerify_Update(verify, (Crypto_DataBlob *)msgBlob);
        if (ret != CRYPTO_SUCCESS) {
            return false;
        }
        msgBlob->data += blockSize;
    }
    if (remS > 0) {
        msgBlob->len = remS;
        res = OH_CryptoVerify_Final(verify, (Crypto_DataBlob *)msgBlob, (Crypto_DataBlob *)signBlob);
        if (res != true) {
            return false;
        }
    }
    msgBlob->data -= INT_64 - remS;
    msgBlob->len = INT_64;
    return res;
}

bool DoTestRsaSignatureVerifySeg()
{
    OH_CryptoAsymKeyGenerator *keyCtx = nullptr;
    OH_CryptoKeyPair *keyPair = nullptr;
    OH_CryptoVerify *verify = nullptr;
    Crypto_DataBlob msgBlob = {.data = const_cast<uint8_t *>(g_plainText), .len = sizeof(g_plainText)};
    Crypto_DataBlob keyBlob = {.data = const_cast<uint8_t *>(g_pubKeyText), .len = sizeof(g_pubKeyText)};
    Crypto_DataBlob signBlob = {.data = const_cast<uint8_t *>(g_signText), .len = sizeof(g_signText)};

    OH_Crypto_ErrCode ret = OH_CryptoAsymKeyGenerator_Create((const char *)"RSA2048", &keyCtx);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSegVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    ret = OH_CryptoAsymKeyGenerator_Convert(keyCtx, CRYPTO_PEM, &keyBlob, nullptr, &keyPair);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSegVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    OH_CryptoPubKey *pubKey = OH_CryptoKeyPair_GetPubKey(keyPair);
    ret = OH_CryptoVerify_Create((const char *)"RSA1024|PKCS1|SHA256", &verify);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSegVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    ret = OH_CryptoVerify_Init(verify, pubKey);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaSegVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    bool res = VerifySegmented(verify, &msgBlob, &signBlob);
    CleanupRsaSegVerifyResources(keyCtx, keyPair, verify);
    return res;
}
```

### Signature Signing and Verification with an RSA Key Pair (PSS Mode)

**Signature development steps**

1. Call [OH_CryptoSign_Create](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_create), specify the string parameter 'RSA2048|PSS|SHA256|MGF1_SHA256', and create a Sign instance with the asymmetric key type RSA2048, padding mode PSS, digest algorithm SHA256, and mask algorithm MGF1_SHA256 for the signature operation.
2. Call [OH_CryptoSign_Init](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_init), and initialize the Sign instance using the private key (OH_CryptoPrivKey).
3. Call [OH_CryptoSign_Update](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_update), and pass the data to be signed. The current single update operation has no length limit. You can determine how to call update based on the amount of data. If the data is small, you can directly call OH_CryptoSign_Final to pass it in at once.
4. Call [OH_CryptoSign_Final](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_final), and obtain the data after signing.
5. Call [OH_CryptoSign_Destroy](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptosign_destroy) to release memory.

<!-- @[pss_rsa_keypair_sign](https://gitcode.com/openharmony/applications_app_samples/blob/master/code/DocsSample/Security/CryptoArchitectureKit/SignatureVerification/SigningSignatureVerification/entry/src/main/cpp/types/project/rsa_pss_signature_tool.cpp) -->

``` C++
#include "CryptoArchitectureKit/crypto_common.h"
#include "CryptoArchitectureKit/crypto_signature.h"
#include "CryptoArchitectureKit/crypto_asym_key.h"

static void CleanupRsaPssSignResources(OH_CryptoAsymKeyGenerator *keyCtx, OH_CryptoKeyPair *keyPair,
    OH_CryptoSign *sign, Crypto_DataBlob *signData)
{
    if (sign != nullptr) {
        OH_CryptoSign_Destroy(sign);
    }
    if (keyPair != nullptr) {
        OH_CryptoKeyPair_Destroy(keyPair);
    }
    if (keyCtx != nullptr) {
        OH_CryptoAsymKeyGenerator_Destroy(keyCtx);
    }
    if (signData != nullptr && signData->data != nullptr) {
        OH_Crypto_FreeDataBlob(signData);
    }
}

bool DoTestRsaPssSignSeg()
{
    OH_CryptoAsymKeyGenerator *keyCtx = nullptr;
    OH_CryptoKeyPair *keyPair = nullptr;
    OH_CryptoSign *sign = nullptr;
    Crypto_DataBlob signData = {.data = nullptr, .len = 0};
    uint8_t plainText[] = {
        0x13, 0xa7, 0x73, 0xe8, 0xb8, 0x22, 0x99, 0x72, 0x98, 0x29, 0xae, 0x74, 0xa8, 0x4a, 0xea, 0xa9,
    };
    Crypto_DataBlob msgBlob = {.data = reinterpret_cast<uint8_t *>(plainText), .len = sizeof(plainText)};

    OH_Crypto_ErrCode ret = OH_CryptoAsymKeyGenerator_Create((const char *)"RSA2048", &keyCtx);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaPssSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    ret = OH_CryptoAsymKeyGenerator_Generate(keyCtx, &keyPair);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaPssSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }

    OH_CryptoPrivKey *privKey = OH_CryptoKeyPair_GetPrivKey(keyPair);
    ret = OH_CryptoSign_Create((const char *)"RSA2048|PSS|SHA256|MGF1_SHA256", &sign);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaPssSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }

    ret = OH_CryptoSign_Init(sign, privKey);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaPssSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    ret = OH_CryptoSign_Update(sign, &msgBlob);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaPssSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }
    ret = OH_CryptoSign_Final(sign, nullptr, &signData);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaPssSignResources(keyCtx, keyPair, sign, nullptr);
        return false;
    }

    CleanupRsaPssSignResources(keyCtx, keyPair, sign, &signData);
    return true;
}
```


**Verification development steps**

1. Call [OH_CryptoVerify_Create](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_create), specify the string parameter 'RSA2048|PSS|SHA256|MGF1_SHA256', and create a Verify instance with the asymmetric key type RSA2048, padding mode PSS, digest algorithm SHA256, and mask algorithm MGF1_SHA256 for the verification operation.

2. Call [OH_CryptoVerify_Init](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_init), and initialize the Verify instance using the public key (OH_CryptoPubKey).

3. Call [OH_CryptoVerify_Update](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_update), and pass the data to be verified. The current single operation has no length restriction on update. You can determine how to call update based on the amount of data. If the amount of data is small, you can directly call OH_CryptoVerify_Final to pass it in at once.

4. Call [OH_CryptoVerify_Final](../../reference/apis-crypto-architecture-kit/capi-crypto-signature-h.md#oh_cryptoverify_final), and perform verification on the data.

<!-- @[pss_verify_rsa_keypair_sign](https://gitcode.com/openharmony/applications_app_samples/blob/master/code/DocsSample/Security/CryptoArchitectureKit/SignatureVerification/SigningSignatureVerification/entry/src/main/cpp/types/project/rsa_pss_verification_tool.cpp) -->

``` C++
#include "signing_signature_verification.h"

static const uint8_t g_pubKeyText[] = {
    ,
};

static const uint8_t g_signText[] = {
    ,
};

static void CleanupRsaPssVerifyResources(
    OH_CryptoAsymKeyGenerator *keyCtx, OH_CryptoKeyPair *keyPair, OH_CryptoVerify *verify)
{
    if (verify != nullptr) {
        OH_CryptoVerify_Destroy(verify);
    }
    if (keyPair != nullptr) {
        OH_CryptoKeyPair_Destroy(keyPair);
    }
    if (keyCtx != nullptr) {
        OH_CryptoAsymKeyGenerator_Destroy(keyCtx);
    }
}

bool DoTestRsaPssVerifySeg()
{
    OH_CryptoAsymKeyGenerator *keyCtx = nullptr;
    OH_CryptoKeyPair *keyPair = nullptr;
    OH_CryptoVerify *verify = nullptr;

    uint8_t plainText[] = {
        0x13, 0xa7, 0x73, 0xe8, 0xb8, 0x22, 0x99, 0x72, 0x98, 0x29, 0xae, 0x74, 0xa8, 0x4a, 0xea, 0xa9,
    };
    Crypto_DataBlob msgBlob = {.data = reinterpret_cast<uint8_t *>(plainText), .len = sizeof(plainText)};
    Crypto_DataBlob keyBlob = {.data = const_cast<uint8_t *>(g_pubKeyText), .len = sizeof(g_pubKeyText)};
    Crypto_DataBlob signBlob = {.data = const_cast<uint8_t *>(g_signText), .len = sizeof(g_signText)};

    OH_Crypto_ErrCode ret = OH_CryptoAsymKeyGenerator_Create((const char *)"RSA2048", &keyCtx);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaPssVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    ret = OH_CryptoAsymKeyGenerator_Convert(keyCtx, CRYPTO_PEM, &keyBlob, nullptr, &keyPair);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaPssVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    OH_CryptoPubKey *pubKey = OH_CryptoKeyPair_GetPubKey(keyPair);
    ret = OH_CryptoVerify_Create((const char *)"RSA2048|PSS|SHA256|MGF1_SHA256", &verify);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaPssVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    ret = OH_CryptoVerify_Init(verify, pubKey);
    if (ret != CRYPTO_SUCCESS) {
        CleanupRsaPssVerifyResources(keyCtx, keyPair, verify);
        return false;
    }
    bool res = OH_CryptoVerify_Final(verify, &msgBlob, &signBlob);
    CleanupRsaPssVerifyResources(keyCtx, keyPair, verify);
    return res;
}
```