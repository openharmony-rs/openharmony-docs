...
- Frequent pop-up windows may disturb user experience and are not recommended. If the user denies the permission, the window for requesting user authorization shall not be displayed again. The application shall provide information to guide the user to manually grant the permission in **Settings**. Alternatively, it shall call [requestPermissionOnSetting](request-user-authorization-second.md) to display the permission settings dialog box.
