...
The SE emulates an NFC card and communicates with an NFC card reader to complete the NFC card transaction. The card to be emulated is configured in the SE through the application. When the device taps the NFC card reader, data is not processed by the device CPU, but is directly sent to the SE to complete the card transaction.
