...
<!-- md-trans-meta sourceCommit=2dfae07aab20af8fdc02150f7d49baaef283b81a translatedAt=2026-05-30T02:49:57.042Z pushedAt=2026-06-01T12:14:45.778Z -->
...
2. In the **entryability** directory, right-click and choose **New > File** to create two files, **EntryAbility.ets** and **PartnerAgentAbility.ets**. The directory structure is as follows:
   ...
   /src/main/
   ├── ets/entryability
   │       └──EntryAbility.ets               # Displays the button-based interaction UI.
   │       └──PartnerAgentAbility.ets        # Defines a custom class that inherits from PartnerAgentExtensionAbility and adds required lifecycle callbacks.
   │   ├───pages
   │       └── Index.ets                     # Draws the button-based interaction page and implements device registration and unregistration.
...
// Capabilities supported by the registered device.
let capability: partnerAgent.DeviceCapability = {
  "supportBR": true,
  "supportBleAdvertiser": true,
};
// Service features of the device registered by the application, including media control and call control.
let businessCap: partnerAgent.BusinessCapability = {
  "supportMediaControl": true,
  "supportTelephonyControl": false,
};
if (isEnabled == false) {
  // PartnerAgentAbility is the name of the Ability to start when the device is discovered.
...
```ts
// Obtain all devices currently registered by the application.
...
// Check whether the current application has registered the device.
...
2. **EntryAbility.ets** file.
    ...
    export default class EntryAbility extends UIAbility {
      ...
      onBackground(): void {
        // Ability is brought to background.
    ...
    struct Index {
      ...
      build() {
        Column() {
          ...
          TextInput({ placeholder: 'Enter the address of the device to be bound', text: this.perBtAddress })
            .margin(1)
            .width('90%')
            .height(50)
