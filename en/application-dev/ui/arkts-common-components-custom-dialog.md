...
<!-- md-trans-meta sourceCommit=c2d83cda3725307f9b8811ef7b48416f8771d3ed translatedAt=2026-09-21T02:40:25.223Z pushedAt=2026-09-21T09:44:51.349Z -->
...
By default, the dialog is modal with a mask, and interaction with controls beneath the mask is not allowed (tap and gesture events cannot pass through to the underlying layer). You can configure the **isModal** attribute in [CustomDialogControllerOptions](../reference/apis-arkui/arkui-ts/ts-methods-custom-dialog-box.md#customdialogcontrolleroptions) to implement modal and non-modal dialogs. For details, see [Dialog Box Types](arkts-dialog-overview.md#types-of-popup-windows).
...
export struct DialogAnimationNew {
  ...
  // Set dialogController to null before the custom component is destroyed.
  aboutToDisappear() {
    this.dialogController = null; // Set dialogController to null.
  }
...
When you perform interactive operations such as tapping the mask to close, swiping left or right, pressing the three-key Back button, or pressing ESC to close the dialog, if the onWillDismiss callback in [CustomDialogControllerOptions](../reference/apis-arkui/arkui-ts/ts-methods-custom-dialog-box.md#customdialogcontrolleroptions) is registered, the dialog is not closed immediately. In the callback, you can obtain the type of operation that prevented the dialog from closing through the reason property in [DismissDialogAction](../reference/apis-arkui/arkui-ts/ts-methods-custom-dialog-box.md#dismissdialogaction12), and decide whether to close the dialog based on the reason.
...
struct CustomDialogExample {
  ...
  build() {
    Column() {
      ...
      // Replace $r('app.string.close_windows') with the actual resource file. In this example, the value of the resource file is "Click me to close the dialog".
