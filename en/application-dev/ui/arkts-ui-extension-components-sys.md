...
**UIExtension** provides a cross-process component sharing mechanism. A host application (such as a system application) can embed the provider's UI into its own UI via the **UIExtensionComponent** component. When this component is initialized, the Application Management Service (AMS) in the system schedules and launches the provider application in an independent process to handle UI rendering and interaction.
