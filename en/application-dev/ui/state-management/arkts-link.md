...
- When the decorated variable is of the class or Object type, its value change and value changes of all its attributes, that is, the attributes that **Object.keys(observedObject)** returns, can be observed. For details, see [Using \@Link with Primitive and Class Types](#using-link-with-primitive-and-class-types). @Link can only observe the changes of objects and their layer-1 attributes, but cannot observe the changes of inner data in nested scenarios (such as nested objects and object arrays). For details, see [When to Use](arkts-observed-and-objectlink.md#when-to-use).
...
1. You are not advised to use the \@Link decorator in a custom component decorated by [\@Entry](./arkts-create-custom-components.md#entry). Otherwise, a warning will be reported during compilation. If the custom component is used as the root node of the page, a runtime error will be reported.
...
3. The type of the variable decorated by \@Link must be the same as the data source type. Otherwise, an error is reported during compilation. In addition, the data source must be a state variable. Otherwise, the framework throws a runtime error.

    > **NOTE**
    >
    > Starting from API version 23, validation of the \@Link data source is enhanced. A runtime error that previously occurred is now detected as a compilation error. For details, see [Common UI-Related Application Crash Issues](../arkts-stability-crash-issues.md).
    ...
    **Incorrect Usage**
    ...
    @Component
    struct Child {
      // Incorrect format 1: The type of the variable decorated by @Link is different from that of the variable decorated by @State.
      @Link test: Cousin;
      // Incorrect format 2: The data source is not a state variable.
      @Link testStr: string;
    ...
    **Correct Usage**
    ...
    @Component
    struct LinkChild {
      // In the subcomponent, use @Link to decorate the test variable of the LinkInfo type.
      @Link test: LinkInfo;
    ...
    struct LinkExample {
      ...
      build() {
        Column() {
          // In the parent component, use the info variable decorated by @State to initialize the test variable of the LinkChild component.
...
5. \@Link does not support variables of type **Function**. In versions earlier than API version 23, the application would encounter a runtime error.

   Starting from API version 23, relevant compile-time validation has been added. Decorating a **Function** type variable with \@Link will result in an **ERROR** message. You should remove the \@Link decorator from variables of the **Function** type in your code.
...
struct ArrayTypesChild {
  ...
  build() {
    Column() {
      ...
      // Array data in the child component can be synchronized back to the parent component.
...
struct MapSampleChild {
  ...
  build() {
    Column() {
      ...
      // The Map type in the child component can be synchronized back to the parent component.
...
struct DateComponent {
  ...
  build() {
    Column() {
      // The Date type in the child component can be synchronized back to the parent component.
...
struct ChangeVariables {
  ...
  build() {
    Column() {
      ...
      // Modifying sourceNumber does not affect the change of the variable in the parent component.
...
The \@Link decorator supports union types, including **undefined** and **null**. In the following example, the **name** variable is declared with type **string | undefined**. After the button in the parent **UnionTypes** component is clicked to modify either the value or type of **name**, the **UnionChild** component will automatically update to reflect these changes.
...
@Component
struct UnionChild {
  // Union types are supported by @Link.
