...
> - The component structure should remain unchanged before and after reuse. For scenarios where component structures differ, [reuseId](../../../application-dev/reference/apis-arkui/arkui-ts/ts-universal-attributes-reuse-id.md#reuseid) can be used to distinguish between reusable components with different structures.
> - If a custom component decorated with \@Reusable has no child components, recycling and reuse will not be triggered.
