...
<!--Tester: @test_lzz-->
<!--Adviser: @k1ngqaquuu-->
<!-- md-trans-meta sourceCommit=e7d54c65a024645f8f688ed024b0b4059342e5b3 translatedAt=2026-09-01T02:43:19.612Z pushedAt=2026-09-02T06:43:54.867Z -->
...
- Wasm function: a function defined in a Wasm module. The functions in a Wasm module can be used by external code after being exported. You can use **OH_JSVM_CompileWasmFunction** to convert Wasm bytecode into the format that JSVM can execute efficiently.
...
| API                         | Description                                                                                |
| --------------------------- | ------------------------------------------------------------------------------------ |
| OH_JSVM_CompileWasmModule   | Synchronously compiles Wasm bytecode into a Wasm module. If the `cache` parameter is provided, the API first attempts to deserialize the cache into a Wasm module. If deserialization fails, it compiles the Wasm bytecode instead. If JIT permission is not granted, a log message is printed to notify the developer. |
...
static JSVM_Value WasmDemo(JSVM_Env env, JSVM_CallbackInfo info)
{
    ...
    // Once the cache is saved, explicitly release it to avoid memory leaks.
