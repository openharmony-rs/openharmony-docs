...
| beforeEachIt | @since1.0.25 在测试套内定义一个单元预置条件，在每条测试用例开始前执行，支持一个参数：预置动作函数。 外层测试套定义的beforeEachIt会在内部测试套中的测试用例执行前执行。 |
|  afterEach         | 在测试套内定义一个单元清理条件，在每条测试用例结束后执行，执行次数与it定义的测试用例数一致。          |
| afterEachIt | @since1.0.25 在测试套内定义一个单元预置条件，在每条测试用例结束后执行，支持一个参数：预置动作函数。<br/>外层测试套定义的afterEachIt会在内部测试套中的测试用例执行结束后执行。 |
...
| mockPrivateFunc | Mock某个类的实例上的私有方法，支持使用异步函数。 |
| mockProperty | Mock某个类的实例上的属性，将其值设置为预期值，支持私有属性。 |
