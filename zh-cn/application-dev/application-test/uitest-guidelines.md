...
export default function abilityTest(): void {
  describe('componentOperationTest', (): void => {
    /**
     * 查找类型为'Button'的控件，并进行控件点击操作
     */
    it('componentSearchAndOperation', TestType.FUNCTION, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      await driver.delayMs(1000);
...
// 导入测试依赖kit
import { Component, Driver, ON, PointerMatrix, Point, UiDirection } from '@kit.TestKit';

export default function abilityTest(): void {
  describe('touchScreen_sample', (): void => {
    /**
     * 基于坐标的触摸屏手指操作
     */
    it('touchScreenOperation', TestType.FUNCTION, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      // 单击
      await driver.click(100, 100);
      // 指定屏幕id进行单击，从API version 20开始支持
...
export default function abilityTest(): void {
  describe('inputTextTest', (): void => {
    /**
     * 基于控件的文本输入，调用接口会默认清空文本框中内容后输入指定文本
     * 当输入文本中不包含中文、特殊字符，且文本长度不超过200字符时默认为逐字键入
     */
    it('componentInputText', TestType.FUNCTION, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      let input: Component | null = await driver.findComponent(ON.type('TextInput'));
      if (input) {
        await input.inputText('abc');
      }
    })
     ...
     * 指定以追加方式输入，即在输入文本签不清空原有内容
     */
    it('componentInputTextAddition', TestType.FUNCTION, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      let input: Component | null = await driver.findComponent(ON.type('TextInput'));
      if (input) {
        // 该接口从API version 20开始支持
     ...
     * 当输入文本中不包含中文、特殊字符，且文本长度不超过200字符时默认为逐字键入
     */
    it('pointInputText', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      let input: Component | null = await driver.findComponent(ON.type('TextInput'));
      if (input) {
        let center: Point = await input.getBoundsCenter();
        await driver.inputText(center, 'abc');
      }
    })
     ...
     * 指定以追加方式输入，即点击指定位置使输入框获焦后将光标移动至原有文本末尾后输入
     */
    it('pointInputTextAddition', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      let input: Component | null = await driver.findComponent(ON.type('TextInput'));
      if (input) {
        let center: Point = await input.getBoundsCenter();
     ...
     * 当输入内容包含中文或特殊字符时，仅支持以复制粘贴方式输入文本，'paste'字段不生效
     */
    it('pointInputTextChinese', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      let input: Component | null = await driver.findComponent(ON.type('TextInput'));
      if (input) {
        let center: Point = await input.getBoundsCenter();
...
export default function abilityTest(): void {
  describe('screenCap_sample', (): void => {
    /**
     * 截取指定区域的屏幕，并保存到指定路径
     */
    it('screenCapture', TestType.FUNCTION, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      // 应用沙箱路径，el2为用户级加密区，base为应用在本设备上存放持久化数据的子目录
      // 请开发者使用时替换为实际的路径
      let savePath: string = '/data/storage/el2/base/cache/1.png';
      let res: boolean = await driver.screenCapture(savePath, {
        left: 0 as int,
        top: 0 as int,
        right: 100 as int,
        bottom: 100 as int
     ...
     * 截取指定屏幕id的屏幕全屏，并保存到指定路径
     */
    it('screenCapWithId', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      // 获取默认屏幕对象
      let disPlay = display.getDefaultDisplaySync();
      let savePath: string = '/data/storage/el2/base/cache/1.png';
      // 从API version 20开始支持
      let displayId: int = disPlay.id.toInt();
      let res: boolean = await driver.screenCap(savePath, displayId); // 获取默认屏幕ID属性
    })
  })
}
```
...
// 设备不支持时的错误代码
const DeviceErrorCode: int = 17000005;

export default function abilityTest(): void {
  describe('touchPadOp_sample', (): void => {
    // PC/2in1场景，模拟触摸板三指上滑（界面返回桌面），三指下滑（界面恢复窗口）操作
    it('touchPadOperation', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      try {
        // 触摸板三指上滑返回桌面
        await driver.touchPadMultiFingerSwipe(3, UiDirection.UP);
...
export default function abilityTest(): void {
  describe('penOp_sample', (): void => {
    // 模拟手写笔单击、双击、长按、滑动操作
    it('penOperation', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      // 手写笔单击
      await driver.penClick({ x: 100, y: 100 });
      // 手写笔双击
...
export default function abilityTest(): void {
  describe('watchOp_sample', (): void => {
    // 手表场景，模拟表冠顺/逆时针旋转，从API version 20开始支持
    it('crownRotate', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      try {
        // 顺时针旋转50格，旋转速度为30格/秒
        await driver.crownRotate(50, 30);
        ...
        let err: BusinessError = error as BusinessError;
        // driver.crownRotate接口仅在智能表设备上生效，其他设备调用时将抛出801错误码
        expect(err.code).assertEqual(CapabilityCode);
      }
    })
  })
}
...
export default function abilityTest(): void {
  describe('displayOp_sample', (): void => {
    // 屏幕属性获取和屏幕操作
    it('displayOperation', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (): Promise<void> => {
      let driver: Driver = Driver.create();
      // 获取屏幕大小
      let size: Point = await driver.getDisplaySize();
      // 获取屏幕清晰度
