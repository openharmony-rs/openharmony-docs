...
C/C++场景下读取和编辑图片的旋转角度信息，可参考：[使用Image_NativeModule读取和编辑图片Exif信息](../image-tool-c.md)，对应的key需要设置为：OHOS_IMAGE_PROPERTY_ORIENTATION。
...
在调用图片编码接口时，需要设置[packingOptionapis-image-kit/arkts-apis-image-i.md#packingoption)的needsPackProperties属性为true（该属性的默认值为false）。
