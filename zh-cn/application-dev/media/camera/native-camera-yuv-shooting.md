...
3. 创建并打开相机设备，参考设备输入(C/C++)中的步骤3-4。
...
6. 注册单段式(PhotoAvailable)或分段式(PhotoAssetAvailable)拍照回调，若应用希望快速得到回图，推荐使用分段式拍照(PhotoAssetAvailable)回调。
...
7. 创建拍照类型会话（参考会话管理(C/C++)），开启会话，准备拍照。
