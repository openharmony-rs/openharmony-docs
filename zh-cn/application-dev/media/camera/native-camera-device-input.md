...
3. 通过OH_CameraManager_CreateCameraInput()方法，获取cameraInput对象。
   ```c++
   // 监听cameraInput错误信息。
...
4. 通过OH_CameraManager_GetSupportedSceneModes()方法，获取当前相机设备支持的模式列表，列表中存储了相机设备支持的所有模式Camera_SceneMode。
...
5. 通过OH_CameraManager_GetSupportedCameraOutputCapabilityWithSceneMode()方法，获取当前设备在当前模式下支持的所有输出流，如预览流、拍照流等。输出流在Camera_OutputCapability中的各个profile字段中。根据相机设备指定模式Camera_SceneMode的不同，需要向Session中添加对应类型的输出流，请参考会话管理开发步骤中的第6步。
