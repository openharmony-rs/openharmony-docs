...
从API version 20开始，支持设置白平衡效果。白平衡是相机的色彩校正技术，通过设置白平衡，使照片产生不同的效果。目前白平衡效果支持：拍照（[PhotoSession/apis-camera-kit/arkts-apis-camera-PhotoSession.md)）、录像（[VideoSession/apis-camera-kit/arkts-apis-camera-VideoSession.md)）和安全相机模式（[SecureSession/apis-camera-kit/arkts-apis-camera-SecureSession.md)）。
...
2. 白平衡的设置提供两种方法。
   ...
   - 方法一：通过[isWhiteBalanceModeSupported/apis-camera-kit/arkts-apis-camera-WhiteBalanceQuery.md#iswhitebalancemodesupported20)判断是否支持该白平衡模式。再通过[setWhiteBalanceMode/apis-camera-kit/arkts-apis-camera-WhiteBalance.md#setwhitebalancemode20)和[getWhiteBalanceMode/apis-camera-kit/arkts-apis-camera-WhiteBalance.md#getwhitebalancemode20)分别设置和查看白平衡模式（只能查看当前已设置的白平衡模式）。该接口提供白平衡模式设置，目前包括：自动，手动，阴天、白炽光、荧光和日光。当同时设置白平衡模式和设置白平衡值时，仅可生效一种，默认白平衡模式优先生效。
      ```ts
   ...
   - 方法二：通过[getWhiteBalanceRange/apis-camera-kit/arkts-apis-camera-WhiteBalanceQuery.md#getwhitebalancerange20)接口，获取当前设备支持的白平衡值范围。再通过[setWhiteBalance/apis-camera-kit/arkts-apis-camera-WhiteBalance.md#setwhitebalance20)和[getWhiteBalance/apis-camera-kit/arkts-apis-camera-WhiteBalance.md#getwhitebalance20)分别设置和查看白平衡值（只能查看当前已设置的白平衡值）。
      ```ts
