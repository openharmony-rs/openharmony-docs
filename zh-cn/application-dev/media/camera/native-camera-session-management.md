...
- 添加闪光灯、调整焦距等配置。具体支持的配置及接口说明请参考[OH_Camera/apis-camera-kit/capi-oh-camera.md)。
...
3. 调用[OH_CameraManager_CreateCaptureSession()/apis-camera-kit/capi-camera-manager-h.md#oh_cameramanager_createcapturesession)方法创建一个会话。
...
4. 调用[OH_CaptureSession_SetSessionMode()/apis-camera-kit/capi-capture-session-h.md#oh_capturesession_setsessionmode)方法配置会话模式。
...
5. 调用[OH_CaptureSession_BeginConfig()/apis-camera-kit/capi-capture-session-h.md#oh_capturesession_beginconfig)方法配置会话。
...
6. 使能。向会话中添加相机的输入流和输出流，调用[OH_CaptureSession_AddInput()/apis-camera-kit/capi-capture-session-h.md#oh_capturesession_addinput)添加相机的输入流；调用[OH_CaptureSession_AddPreviewOutput()/apis-camera-kit/capi-capture-session-h.md#oh_capturesession_addpreviewoutput)和[OH_CaptureSession_AddPhotoOutput()/apis-camera-kit/capi-capture-session-h.md#oh_capturesession_addphotooutput)添加相机的输出流。以下示例代码以添加预览流previewOutput和拍照流photoOutput为例，即当前模式支持拍照和预览。

   调用[OH_CaptureSession_CommitConfig()/apis-camera-kit/capi-capture-session-h.md#oh_capturesession_commitconfig)和[OH_CaptureSession_Start()/apis-camera-kit/capi-capture-session-h.md#oh_capturesession_start)方法提交相关配置，并启动会话。
...
7. 会话控制。调用[OH_CaptureSession_Stop()/apis-camera-kit/capi-capture-session-h.md#oh_capturesession_stop)方法可以停止当前会话。调用[OH_CaptureSession_RemovePhotoOutput()/apis-camera-kit/capi-capture-session-h.md#oh_capturesession_removephotooutput)和[OH_CaptureSession_AddVideoOutput()/apis-camera-kit/capi-capture-session-h.md#oh_capturesession_addvideooutput)方法可以完成会话切换控制。以下示例代码以移除拍照流photoOutput，添加视频流videoOutput为例，完成了拍照到录像的切换。
