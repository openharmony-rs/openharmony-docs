...
3. 创建拍照输出流。
   ...
   通过CameraOutputCapability中的photoProfiles属性，可获取当前设备支持的拍照输出流。

   通过createPhotoOutput方法传入支持的某一个输出流Profile，创建拍照输出流。

   通过getSupportedFullOutputCapability获取相机支持的完整输出能力cameraOutputCapability，参考步骤2。在cameraOutputCapability的photoProfiles中选择支持YUV格式的Profile，作为创建拍照输出流的参数photoProfile。
...
4. 设置拍照输出流的回调。
   ...
   如果需要在图库中看到所保存的图片、视频资源，需要先将其保存到媒体库，保存方式请参考：保存媒体库资源。

   如果需要在onCapturePhotoAvailable接口获取到buffer，先将PixelMap数据在安全控件中保存到媒体库。
...
5. 触发拍照。
   ...
   通过photoOutput的capture方法，执行拍照任务。该方法有两个参数，第一个参数为拍照设置setting，setting中可以设置图片质量，图片旋转角度等信息。第二参数为异步回调函数，用于获取结果。接口调用失败会返回相应错误码。

   通过PhotoOutput中的getPhotoRotation方法，可以获取拍照旋转角度。
