...
1. 移动端设备的视频送显帧率通常受限于整机功耗管控（常为60fps），远低于高倍速下的理论帧率。此外，在高速动态画面下，受限于人眼的视觉暂留效应与动态视锐度下降，强行解码并渲染超出分辨能力的微小运动帧，会产生无效的算力消耗。
...
本能力需主动下发配置才可以生效。通过配置`OH_AVFormat`中的以下键值对来精确控制行为。
