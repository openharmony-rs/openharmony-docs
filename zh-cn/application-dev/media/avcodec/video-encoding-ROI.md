...
3. Native层解析TS层传递的人脸位置信息。
   ...
   static napi_value UpdateFaceRect(napi_env env, napi_callback_info info)
   {
       ...
       for (uint32_t i = 0; i < arrayLength; i++) {
           ...
           napi_get_named_property(env, faceRectObj, "height", &propValue);
...
4. 转换成ROI信息格式字符串并保存。
   ...
   { 
       if (faceRectVec == nullptr || faceRectVec->empty()) {
           return;
       }
   
       // 核心容器1：用于接收底层 realloc 的 C 风格字符串指针
       char *mergedRoiStrC = nullptr; 
       
       // 核心容器2：复用同一个 AVFormat，避免在循环中频繁 Create/Destroy
       OH_AVFormat *roiFormat = OH_AVFormat_Create();
       if (roiFormat == nullptr) {
           // 系统资源不足，打错误日志并退出
           return;
       }
   
       // 遍历所有 faceRect (修复了原代码的指针遍历语法，使用 *faceRectVec)
       for (const auto& faceRect : *faceRectVec) {
           // 归一化坐标转像素坐标。
           int32_t left = static_cast<int32_t>(faceRect.topLeftX * width);
           int32_t top = static_cast<int32_t>(faceRect.topLeftY * height);
           int32_t right = static_cast<int32_t>(faceRect.width * width) + left;
           int32_t bottom = static_cast<int32_t>(faceRect.height * height) + top;
   
           // 1. 将坐标与参数安全地写入字典，杜绝手动拼接字符串 
           OH_AVFormat_SetIntValue(roiFormat, OH_MD_KEY_VIDEO_ENCODER_ROI_TOP, top);
           OH_AVFormat_SetIntValue(roiFormat, OH_MD_KEY_VIDEO_ENCODER_ROI_LEFT, left);
           OH_AVFormat_SetIntValue(roiFormat, OH_MD_KEY_VIDEO_ENCODER_ROI_BOTTOM, bottom);
           OH_AVFormat_SetIntValue(roiFormat, OH_MD_KEY_VIDEO_ENCODER_ROI_RIGHT, right);
           OH_AVFormat_SetIntValue(roiFormat, OH_MD_KEY_VIDEO_ENCODER_ROI_DELTA_QP, qpOffset);
   
           // 2. 调用新接口追加字符串，底层会自动处理扩容
           OH_AVErrCode ret = OH_VideoMetadata_AppendRoiString(&mergedRoiStrC, roiFormat);
           if (ret != AV_ERR_OK) {
               // 发生异常（如扩容失败），跳出循环，底层会保证已有 mergedRoiStrC 不被破坏
               break; 
           }
       }
       ...
       // 3. 收尾转换与安全的内存释放
       if (mergedRoiStrC != nullptr) {
           // 将 C 风格字符串转换为 C++ 的 std::string，此时发生了深拷贝
           std::string finalMergedStr(mergedRoiStrC);
   
           // 根据我们之前的架构共识：底层 C 接口分配的内存，由外部业务侧负责 free [cite: 144, 157]
           free(mergedRoiStrC);
   
           // 存入 Map
           std::lock_guard<std::mutex> lock(g_roiMutex);
           g_roiStrMap[timestamp] = finalMergedStr;
       }
   }
   ```
...
6. 将ROI信息设置到视频帧NativeBuffer元数据中。
   ...
   绘制之前获取最新的NativeBuffer，并配置ROI信息。绘制完成后可通过`eglSwapBuffers`把绘制好的数据送到编码器进行编码。
   ```c++
   int fenceFd = -1;
