...
**依赖ROI检测识别能力：** 编码器不具备ROI的检测识别能力，所以ROI编码技术生效依赖于开发者输入的ROI信息。开发者可根据业务场景自行设计并实现ROI识别能力，或通过调用系统相机模块原生提供的人脸区域信息（写入帧的NativeBuffer元数据中），降低开发成本，具体参考[相机人脸ROI获取示例](../camera/native-camera-metadata.md#状态监听)。
...
1. 在CMakeList.txt中链接动态库。
   ...
   ```txt
   set(BASE_LIBRARY
       libace_napi.z.so libhilog_ndk.z.so libnative_media_codecbase.so libnative_media_core.so
       libnative_media_venc.so libnative_media_avmuxer.so libnative_window.so
...
编码器在接收到视频帧时触发参数回调，由于参数回调不包含帧时间戳信息，开发者需要使用以PTS为键的同步队列（RoiQueue）将帧处理线程中的ROI数据按时间顺序传递到编码回调。回调中从队列弹出最老的ROI条目，通过`OH_AVFormat_SetStringValue`配置到参数格式中。
...
4. 实现PTS同步队列与回调用户数据结构。
   ...
   由于参数回调不包含帧时间戳信息，无法直接将ROI数据与编码帧对齐。建议实现一个以PTS为键的同步队列，将帧处理线程中的ROI数据按PTS顺序传递到编码回调。编码回调通过用户数据结构中的队列指针访问队列，Pop最老条目获取ROI配置字符串。
...
6. 在编码输入参数回调中配置ROI信息。
   ...
   当编码器接收到一帧输入数据时，会触发`OnNeedInputParameter`回调。在回调中，从RoiQueue按PTS顺序弹出最老的ROI条目，并通过`OH_AVFormat_SetStringValue`配置到参数格式中。即使弹出空字符串也显式设置，以明确表达"此帧无ROI"。
...
7. 将组装好的ROI字符串连同帧PTS传递到VideoEncoder的RoiQueue。
   ...
   在帧处理线程中提取并组装ROI字符串后，需要将帧PTS和ROI字符串一同推入VideoEncoder的RoiQueue。RoiQueue以PTS为键排序，确保编码回调按帧顺序获取ROI数据，避免错位。
...
> **说明：**
>
> RoiQueue以PTS为键排序存储ROI条目。编码回调通过`Pop`方法取出最老条目（PTS最小），保证帧与ROI的顺序一致性。若队列空，`Pop`会等待最多3ms再返回空串。队列会自动清理超过2秒的陈旧条目，防止无限增长。每帧均应Push（包括空ROI串），以显式表达"此帧无ROI"；关闭ROI时调用`ClearRoiQueue`清空队列。
