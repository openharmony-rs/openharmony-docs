...
**依赖ROI检测识别能力：** 编码器不具备ROI的检测识别能力，所以ROI编码技术生效依赖于开发者输入的ROI信息。开发者可根据业务场景自行设计并实现ROI识别能力，或通过调用系统相机模块原生提供的人脸区域信息，降低开发成本，具体参考[元数据(C/C++)](../camera/native-camera-metadata.md#元数据(C/C++))。
...
- dqp为负表示ROI区域编码画质优于非ROI区域，绝对值越大质量差异越大。纯数值格式中省略"=DeltaQp"时使用默认参数（=-3）。
...
2. 从视频帧NativeBuffer元数据提取ROI信息。
   ...
   > **说明：**
   >
   > `InBuffer`是通过OH_NativeImage_AcquireNativeWindowBuffer从OH_NativeImage获取的相机帧，相机输出的ROI信息已写入该Buffer的元数据中。关于NativeImage的创建和帧接收机制，请参考[视频编码](video-encoding.md)开发指南中Surface模式的相关说明。
...
4. 实现PTS同步队列与回调用户数据结构。
   ...
   由于参数回调不包含帧时间戳信息，无法直接将ROI数据与编码帧对齐。建议实现一个以PTS为键的同步队列，将帧处理线程中的ROI数据按PTS顺序传递到编码回调。编码回调通过用户数据结构中的队列指针访问队列，Pop弹出PTS最小的条目获取ROI配置字符串。
...
6. 在编码输入参数回调中配置ROI信息。
   ...
   当编码器接收到输入数据时，会触发`OnNeedInputParameter`回调。在回调中，从RoiQueue中弹出PTS最小的ROI条目，并通过`OH_AVFormat_SetStringValue`配置到参数格式中。弹出空字符串时仍应显式配置，以明确标识该帧未配置ROI区域。
...
> **说明：**
>
> RoiQueue以PTS为键排序存储ROI条目。编码回调通过`Pop`方法取出PTS最小的条目，保证帧与ROI的顺序一致性。如果队列为空，`Pop`最多等待3ms再返回空字符串。队列会自动清理超过2秒的陈旧条目，防止无限增长。每帧均应Push入队（包括空ROI字符串），空ROI字符串用于明确标识该帧未配置ROI区域；关闭ROI时调用`ClearRoiQueue`清空队列。
