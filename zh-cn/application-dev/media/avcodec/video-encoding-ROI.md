...
感兴趣区域视频编码(Region Of Interest Video Encoding)，简称ROI视频编码，是基于硬件 H.264/H.265 编码能力扩展的高级优化技术。其核心逻辑为对画面中指定的重点区域分配更多编码资源实现高编画质，最终在有限带宽条件下保障 ROI 区域内容清晰呈现，显著提升整体视觉体验。

开发者可自主定义视频画面中的 ROI 区域（如直播中的人脸、监控中的车牌等），并通过设定质量偏移参数，调节 ROI 区域与非 ROI 区域的编码质量差异，实现编码资源的差异化分配。

## 适用场景

ROI 适用于因带宽约束（码率参数）不能满足视频画质要求的场景，比如视频通话，视频直播等，而且该类场景下，业务可以定义明确的画面内容重要性。
...
基于以上两大核心条件，选择三种编码场景提供ROI编码开发示例供大家参考。
...
| **开发示例** | [开发示例1](#开发示例1surface模式下随nativebuffer接口配置roi) | [开发示例2](#开发示例2surface模式下通过随编码输入回调接口配置roi) | [开发示例3](#开发示例3buffer模式下配置roi) |
...
- **使用依赖**：编码器不具备 ROI 的检测识别能力，ROI 编码技术生效依赖于调用者输入ROI 信息，ROI 信息的获取方式。

    - 自主研发：开发者可根据业务场景自行设计并实现 ROI 识别能力（如商品、人脸区域识别、文字区域检测等）；
...
- ROI是一个矩形区域，Top,Left和Bottom,Right分别定义了ROI的区域在图像中的左上角和右下角的坐标位置，如下图所示。
...
## 开发示例1：surface模式下随NativeBuffer接口配置ROI

通过系统相机能力获取ROI信息和通过相机获得的视频帧是独立的两个回调接口，需要通过获取视频时间戳与ROI信息时间戳进行数据同步匹配，在编码之前完成对应编码帧的ROI配置。

在具体业务场景中，相机获取的视频帧会经过一系列的图像处理，如美颜，滤波，增强等前处理，流程架构图如下，开发者可以根据实际的业务需求进行模块增减。
...
## 开发示例2：surface模式下通过随编码输入回调接口配置ROI
...
*注意：上述ROI信息和相机视频帧信息需对齐同步，在实际使用中，因为两个接口处理不同步，在时间信息上可能存在偏差，或者ROI连续帧的时间戳相同，遇到该情况不影响编码功能；对编码画质的影响，开发可以进行评估后使用。*

## 开发示例3：buffer模式下配置ROI
