...
以下示例以图片选择为例，媒体文件类型请参见[PhotoViewMIMETypes/apis-media-library-kit/arkts-apis-photoAccessHelper-e.md#photoviewmimetypes)。
...
4. 创建图库选择器实例，调用[PhotoViewPicker.select/apis-media-library-kit/arkts-apis-photoAccessHelper-PhotoViewPicker.md#select)接口拉起图库界面进行文件选择。文件选择成功后，返回[PhotoSelectResult/apis-media-library-kit/arkts-apis-photoAccessHelper-class.md#photoselectresult)结果集。
   ...
   如有获取元数据需求，可以通过[文件管理接口/apis-core-file-kit/js-apis-file-fs.md)和[文件URI/apis-core-file-kit/js-apis-file-fileuri.md)根据uri获取部分文件属性信息，比如文件大小、访问时间、修改时间、文件名、文件路径等。

## 指定URI读取文件数据

1. 待界面从图库返回后，再通过一个类似按钮的组件去调用其他函数，使用[fileIo.openSync/apis-core-file-kit/js-apis-file-fs.md#fileioopensync)接口，通过[媒体文件uri](../../file-management/user-file-uri-intro.md#媒体文件uri)打开这个文件得到fd。这里需要注意接口权限参数是fileIo.OpenMode.READ_ONLY。
...
2. 通过fd使用[fileIo.readSync/apis-core-file-kit/js-apis-file-fs.md#readsync)接口读取这个文件内的数据，读取完成后关闭fd。
...
1. 定义媒体资源处理器[MediaAssetDataHandler/apis-media-library-kit/arkts-apis-photoAccessHelper-MediaAssetDataHandler.md)，系统在资源准备就绪时向应用回调onDataPrepared。
...
2. 使用[getAssets/apis-media-library-kit/arkts-apis-photoAccessHelper-PhotoAccessHelper.md#getassets-1)接口获取要访问的资产，并通过[requestImageData/apis-media-library-kit/arkts-apis-photoAccessHelper-MediaAssetManager.md#requestimagedata11)获取对应资源。
