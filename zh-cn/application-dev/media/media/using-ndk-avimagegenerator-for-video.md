...
使用OH_PixelmapNative_ConvertPixelmapNativeToNapi()接口将OH_PixelmapNative对象转换为PixelMapnapi对象、OH_PixelmapNative_Release()接口释放OH_PixelmapNative对象资源，需引入如下头文件。
```c
...
1. 使用OH_AVImageGenerator_Create()创建实例。
   ```c
...
2. 设置视频资源的文件描述符：调用OH_AVImageGenerator_SetFDSource()。
   ```c
...
4. 获取指定时间的视频帧：调用OH_AVImageGenerator_FetchFrameByTime()，可以获取到一个OH_PixelmapNative对象，该对象可用于图片显示。

   使用完成需要调用OH_PixelmapNative_Release释放OH_PixelmapNative对象资源，详细使用方法请参阅OH_PixelmapNative_Release()。
   ```c
   #include "napi/native_api.h"
   #include <multimedia/image_framework/image/pixelmap_native.h>
   #include <multimedia/player_framework/avimage_generator.h>
   ...
   ```
5. 释放资源：调用OH_AVImageGenerator_Release()销毁实例，释放资源。
   ```c
   // 释放OH_AVImageGenerator资源。
   OH_AVImageGenerator_Release(generator);
   ```
