...
Dynamic Adaptive Streaming over HTTP，一种基于HTTP的自适应流媒体传输协议，通过MPD（Media Presentation Description）文件描述多个不同码率的媒体片段，播放器可根据网络状况动态切换码率，适用于网络点播场景。
...
HTTP Live Streaming，一种基于HTTP的自适应流媒体传输协议，通过m3u8索引文件描述媒体片段，支持直播和点播，播放器可根据网络状况自动切换不同码率的视频流，适用于网络直播和点播场景。
