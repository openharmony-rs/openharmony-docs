...
2. 设置回调监听函数：使用OH_AVPlayer_SetOnInfoCallback()、OH_AVPlayer_SetOnErrorCallback()接口设置信息监听回调函数和错误监听回调函数，搭配全流程场景使用。支持的监听回调函数包括：
   | 回调函数类型 | 说明 |
   | -------- | -------- |
...
6. 设置播放画面窗口：调用OH_AVPlayer_SetVideoSurface()设置播放画面窗口。此函数必须在OH_AVPlayer_SetURLSource()之后，OH_AVPlayer_Prepare()之前调用。
