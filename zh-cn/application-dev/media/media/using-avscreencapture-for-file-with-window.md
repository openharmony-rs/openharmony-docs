...
- 如果配置了采集麦克风音频数据，需向用户申请授权麦克风权限**ohos.permission.MICROPHONE**和申请长时任务（ArkTS）。
- 从API version 22开始，在PC/2in1设备上对应用进行录屏时，可通过申请权限**ohos.permission.TIMEOUT_SCREENOFF_DISABLE_LOCK**，实现在屏幕熄灭但不锁屏的场景下，继续保持录屏的效果。配置方式请参见声明权限。
- 从API version 22开始，在PC/2in1设备上对应用进行录屏时，可通过申请权限**ohos.permission.CUSTOM_SCREEN_RECORDING**，实现在录屏时不再弹出隐私警告弹窗。配置方式请参见受限开放权限。
...
3. 调用OH_AVScreenCapture_Init方法配置屏幕录屏参数。
