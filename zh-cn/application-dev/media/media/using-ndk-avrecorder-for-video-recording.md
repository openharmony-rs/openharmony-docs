...
开发者通过引入[avrecorder.h/apis-media-kit/capi-avrecorder-h.md)、[avrecorder_base.h/apis-media-kit/capi-avrecorder-base-h.md)和[native_averrors.h/apis-avcodec-kit/capi-native-averrors-h.md)头文件，使用视频录制相关API。

AVRecorder详细的API说明请参考[AVRecorder API参考/apis-media-kit/capi-avrecorder.md)。
...
使用[native_avformat.h/apis-avcodec-kit/capi-native-avformat-h.md)相关接口时，需引入如下头文件。
```C++
