...
- 从API version 22开始，在PC/2in1设备上对应用进行录屏时，可通过申请权限**ohos.permission.CUSTOM_SCREEN_RECORDING**，实现在录制屏幕时不再弹出隐私告警弹窗。配置方式请参见受限开放权限。
> **说明：**
>
> 仅应用需要克隆、备份或同步用户公共目录的图片、视频类文件时，可申请ohos.permission.READ_IMAGEVIDEO、ohos.permission.WRITE_IMAGEVIDEO权限来读写图片和视频类文件，申请方式请参考<!--RP1-->申请受控权限<!--RP1End-->。
...
5. 基于预先配置的屏幕录制参数，调用init方法初始化screenCapture。

   <!-- @screenCapture_arkts_init --> 
   
   ``` TypeScript
   await this.screenCapture?.init(this.captureConfig);
   ```

6. 创建豁免隐私窗口，这里填写的是子窗口id和主窗口id，具体开发步骤可参见窗口APIWindowProperties。

   <!-- @screenCapture_arkts_skipPrivacyMode --> 
...
7. 调用startRecording方法开始进行屏幕录制，并通过监听函数监听状态。

   <!-- @screenCapture_arkts_startRecording --> 
...
8. 停止录屏。
   ...
   - 应用主动调用stopRecording方法，停止录屏。
     ...
     <!-- @screenCapture_arkts_stopRecording --> 
     
     ``` TypeScript
     await this.screenCapture?.stopRecording();
     ```

9. 调用release方法销毁实例，释放资源。

   <!-- @screenCapture_arkts_release --> 
