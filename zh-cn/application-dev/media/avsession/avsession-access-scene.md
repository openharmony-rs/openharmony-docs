...
AVSession在构造方法中支持不同的类型参数，由 [AVSessionType/apis-avsession-kit/arkts-apis-avsession-t.md#avsessiontype10) 定义，不同的类型代表了不同场景的控制能力，对于播控中心来说，会展示不同的控制模版。
...
对媒体类播放来说，需要申请[AUDIO_PLAYBACK BackgroundMode/apis-backgroundtasks-kit/js-apis-resourceschedule-backgroundTaskManager.md#backgroundmode)的长时任务。
...
应用可以通过[setAVPlaybackState/apis-avsession-kit/arkts-apis-avsession-AVSession.md#setavplaybackstate10)。把当前的播放状态设置给系统，以在播控中心界面进行展示。
...
具体的接口参考[接口注册/apis-avsession-kit/arkts-apis-avsession-AVSession.md#onplay10)。
...
音乐类应用实现收藏功能，那么需要注册收藏的控制响应[on('toggleFavorite')/apis-avsession-kit/arkts-apis-avsession-AVSession.md#ontogglefavorite10)。
...
针对音乐类应用，系统的播控中心界面会默认展示循环模式的控制操作，目前系统支持四种固定的循环模式控制，参考: [LoopMode/apis-avsession-kit/arkts-apis-avsession-e.md#loopmode10)。
...
- 方式二：
  ...
  通过AVSession注册[HandleMediaKeyEvent/apis-avsession-kit/arkts-apis-avsession-AVSession.md#onhandlekeyevent10)指令。该回调接口会直接转发媒体按键事件[KeyEvent/apis-input-kit/js-apis-keyevent.md)。应用需要自行识别按键事件的类型，并响应事件实现对应的功能。目前支持转发的按键事件类型如下：

  | 按键类型([KeyCode/apis-input-kit/js-apis-keycode.md#keycode)) | 功能说明   |
  | ------  | -------------------------|
