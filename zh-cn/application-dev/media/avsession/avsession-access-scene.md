...
2. 按需[创建后台任务](#创建后台任务)。
3. [设置必要的元数据（Metadata）](#设置元数据)，以在播控中心展示相应的信息，包括但不限于：当前媒体的ID（assetId），上一首媒体的ID（previousAssetId），下一首媒体的ID（nextAssetId），标题（title），专辑作者（author），专辑名称（album），词作者（writer），媒体时长（duration）等属性。
...
1. 歌曲支持试听
   ...
   （1）应用不需要设置完整的歌曲时长，只需要设置歌曲的试听时长。当应用仅设置歌曲的试听时长而不是完整时长，用户在播控中心触发进度控制时，应用接收到的时间戳位置也是VIP试听时长内的相对时间戳位置，而不是完整歌曲的绝对时间戳位置，应用需要重新计算歌曲从零开始的绝对时间戳进行实际响应处理。
...
对于系统支持但应用不支持的控制命令（例如应用不支持“上一首”命令），只需使用off接口注销对应的控制命令，系统播控中心会相应地对该控制按钮进行置灰处理，明确告知用户此控制命令不可用。
