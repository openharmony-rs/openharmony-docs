...
应用若需实现桌面歌词功能，需遵循以下流程：创建会话并设置歌词元数据、通过接口控制歌词的使能与显示状态、监听系统侧的状态变化以同步UI，以及在退出时正确销毁会话。

### 具体步骤：
1. 先创建 AVSession 实例，并在 AVMetadata 中填入符合 LRC 格式的歌词内容。系统播控中心将解析该内容进行同步展示。歌词内容需包含时间标签和歌词文本。

```ts
import { BusinessError } fomr '@kit.BasicServicesKit';
import { avSession as AVSessionManager } from '@kit.AVSessionKit';
...
  // 例如：[00:25.44]xxx\r\n[00:26.44]xxx\r\n
  //lyric: "lrc格式歌词内容",
  lyric: "[00:25.44]Looking out the window\r\n[00:26.44]Feeling so low\r\n" 
};
...
```
 
2. 应用根据自身设置，调用接口开关歌词、显示歌词、锁定歌词。
> 应用需提供入口供用户手动开启/关闭桌面歌词，或设置锁定状态。调用以下接口进行控制：
> - **使能桌面歌词**：调用 `enableDesktopLyric(true/false)` 接口。
> - **设置可见性**：调用 `setDesktopLyricVisible(true/false)` 控制歌词窗口的显示与隐藏。
> - **设置锁定状态**：调用 `setDesktopLyricState` 设置歌词窗口是否可被拖动或关闭。
...
  console.error(`enableDesktopLyricBusinessError: code: ${err.code}, message: ${err.message}`);
});
 
// 显示桌面歌词
...
 
// 设置桌面歌词锁定状态（true为锁定，false为解锁）
let lyricState: AVSessionManager.DesktopLyricState = {
   lock: true,
}；
currentAVSession.setDesktopLyricState(lyricState).then(() => {
...
// 监听桌面歌词状态
currentAVSession.onDesktopLyricEnabled(state: boolean) => {
  console.info('lyric state ${state}`');
   // 刷新应用内界面
}).catch((err: BusinessError) => {
  console.error(`onDesktopLyricEnabled  BusinessError: code: ${err.code}, message: ${err.message}`);
});
```
4. 应用退出，销毁会话
当应用退出或不再需要播放媒体时，必须主动销毁 AVSession。销毁后，桌面歌词组件将自动跟随退出。再次启动时需重新初始化。

```ts
import { BusinessError } fomr '@kit.BasicServicesKit';
// 主动销毁已创建的session。
currentAVSession.destroy((err) => {
  if (err) {
...
```
> **说明：**
> - **元数据必填：**必须先成功设置包含 lyric 字段的 AVMetadata，否则桌面歌词功能可能无法生效。
> - **生命周期管理：**确保 AVSession 对象在后台播放期间保持存活，避免被系统回收导致歌词中断。
> - **状态同步：**务必处理 desktopLyricEnabled 回调，保证应用内UI与系统实际状态一致。
