...
从API version 23开始，音频模板提供统一的UI界面以及交互逻辑，供媒体应用接入使用，音频模板支持播放列表、收藏列表、媒体详情等界面的展示以及媒体播放、暂停、搜索、收藏等能力的交互。媒体应用只需完成接口接入，便可在音频模板上运行，无需开发UI以及流程交互代码，只需专注于接口接入并提供业务数据，可极大减少开发工作量。该文档介绍系统音频模板的能力，包括监听音频模板创建，查询媒体信息，下发操作指令等。媒体应用接入请参考文档[使用音频模板](using-avsession-AVMusicTemplate.md)。
