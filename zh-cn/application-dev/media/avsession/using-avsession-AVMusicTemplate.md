...
从API version 23开始，媒体应用通过实现音频模板的提供公共接口，可接入至媒体播控中心（系统应用），无需再开发UI以及流程交互代码，只需专注于接口接入并提供业务数据，便于播控中心进行统一的界面显示和播控管理，可极大减少开发工作量。对于接入播控中心的媒体应用，可通过播控中心内提供的应用切换功能，切换至不同的媒体应用并对其进行播控管理。该文档介绍音频模板公共接口能力及开发基本流程，包括通过音模板接入播控中心、上报媒体相关信息（标题，作者，播放状态等）至播控中心、响应播控中心下发的操作（播放、暂停、搜索、收藏）指令等。。
