...
从API version 23开始，当媒体应用接入音频模板使用统一的界面风格时，需要提供媒体相关信息（标题，作者，播放状态等）给音频模板操作方（媒体中心是OpenHarmony系统预置的音频模板操作方，以下统称为媒体中心），并响应媒体中心下发的操作（搜索，播放，暂停等）命令。

媒体中心同时支持音频和视频内容，且两者的接入方式相同。本文档以音频场景为例进行说明。

## 基本概念

音频模板（AVMusicTemplate）：用于描述音频模板相关属性，包含标识当前媒体会话的ID（sessionId）、会话标签（sessionTag）等属性，与媒体中心数据交互的操作方法。
