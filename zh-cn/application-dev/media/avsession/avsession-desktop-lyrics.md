...
2. 创建AVSession实例，通过设置元数据信息填入LRC格式的歌词数据，包含时间标签及对应的歌词文本。不符合LRC格式的歌词数据，系统可能存在解析异常导致无法展示歌词内容。

3. 调用enableDesktopLyric接口启用歌词组件，需传入参数true打开歌词组件。
...
5. 歌词组件启用后默认是非锁定状态，应用可以通过接口主动锁定/解锁歌词组件，具体接口如下：
   ...
   **查询锁定状态：** 调用getDesktopLyricState接口，查询当前歌词组件锁定状态。

6. 应用可以通过系统提供的回调监听歌词组件可见性、锁定状态的变化，具体接口如下：

   **监听歌词组件是否可见：** 监听onDesktopLyricVisibilityChanged，回调返回false，表示当前歌词组件不可见；回调返回true，表示当前歌词组件可见。如需取消该监听，使用offDesktopLyricVisibilityChanged接口。

   **监听歌词组件是否锁定：** 监听onDesktopLyricStateChanged，回调返回false，表示当前歌词组件未锁定；回调返回true，表示当前歌词组件锁定。如需取消该监听，使用offDesktopLyricStateChanged接口。
