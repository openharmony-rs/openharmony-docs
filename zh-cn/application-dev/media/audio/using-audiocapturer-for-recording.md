...
2. 调用[on('readData')](../../reference/apis-audio-kit/arkts-apis-audio-AudioCapturer.md#onreaddata11)方法，订阅监听音频数据读入回调。
   ...
   > - **高负载阻塞风险**：承载回调的ArkTS执行上下文在高负载场景下，可能因为其他业务任务持续占用而调度延迟。此类场景会产生和主线程阻塞类似的录音数据缺失、卡顿或杂音问题。处理高负载时，应降低同一执行上下文中非必要任务的执行频率，避免多个定时任务同时运行，必要时暂停其他非关键业务。
   > - **阻塞风险确认**：录音过程中，音频框架服务端将音频输入数据写入与应用侧音频客户端共享的录音缓冲区，应用侧音频客户端从该缓冲区取走数据。应用侧取走数据的速率低于服务端写入速率时，未处理的数据会持续积压。当共享缓冲区可写空间不足一个采集跨度时，框架判定发生过载。本次输入数据不会写入共享缓冲区，因而本次数据可能丢失。过载计数用于记录检测到的过载情况，ArkTS应用可调用[getOverflowCount()](../../reference/apis-audio-kit/arkts-apis-audio-AudioCapturer.md#getoverflowcount12)或[getOverflowCountSync()](../../reference/apis-audio-kit/arkts-apis-audio-AudioCapturer.md#getoverflowcountsync12)查询过载音频帧数量。API版本26及以上，使用[printCapturerInfo](../../reference/apis-audio-kit/arkts-apis-audio-AudioDebuggingManager.md#printcapturerinfo)输出录音快照时，也可查看其中的`overflowCount`，用于录音结束后的问题定位。高负载或任务队列积压期间，如果过载数持续增加，应优先降低任务竞争、限制队列长度或主动停止录音流。
   ...
   > - **业务静音处理**：当前可通过以下方式实现录音静音。若静音期间仍需保持录音流运行，应用应在`readData`回调中复制采集数据，再将副本设置为静音值后保存或发送。对于设备和录音流实际支持的有符号PCM格式，静音值为`0`；如果录音流使用`SAMPLE_FORMAT_U8`，静音值为`0x80`。如果业务允许静音期间不再采集数据，可以由应用主动停止录音流，后续需要录音时再重新启动，以停止采集替代静音处理。
