...
2. 调用[on('readData')](../../reference/apis-audio-kit/arkts-apis-audio-AudioCapturer.md#onreaddata11)方法，订阅监听音频数据读入回调。
   ...
   >   1. 静音期间仍需保持录音流运行：应用在`readData`回调中复制采集数据，将副本中的采样数据全部设置为静音值后保存或发送。对于设备和录音流实际支持的有符号PCM格式，静音值为`0`；如果录音流使用`SAMPLE_FORMAT_U8`，静音值为`0x80`。
