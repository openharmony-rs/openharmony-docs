...
应用使用麦克风录音前，需要声明并动态申请`ohos.permission.MICROPHONE`权限。权限申请方式请参考[向用户申请授权](../../security/AccessToken/request-user-authorization.md)。
...
后台录音不属于播控能力。播控主要面向音频播放状态展示、播放控制和媒体会话交互；后台录音面向麦克风采集和录音任务持续运行。

如果应用只需要在后台播放音频或展示播放控制，应使用后台播放及播控相关能力；如果应用需要在退到后台后继续录音，应按后台录音模式进行声明、实现和管控。
