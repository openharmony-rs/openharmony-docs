...
从API version 20开始，支持移动全景声相关状态和能力的查询、设置。
...
创建AudioCollaborativeManager实例。在使用AudioCollaborativeManager的API前，需要使用[getCollaborativeManager](../../reference/apis-audio-kit/js-apis-audio-sys.md#getcollaborativemanager20)创建一个AudioCollaborativeManager实例。
...
系统应用开发者可以通过[setCollaborativePlaybackEnabledForDevice](../../reference/apis-audio-kit/js-apis-audio-sys.md#setcollaborativeplaybackenabledfordevice20)接口开启/关闭指定设备的移动全景声效果，该接口需要传递两个参数：AudioDeviceDescriptor和enabled。
