...
1. 获取设备数量：调用OH_MIDIClient_GetDeviceCount接口获取当前连接的设备数量。

2. 获取设备信息：分配足够大的缓冲区，调用OH_MIDIClient_GetDeviceInfos接口填充设备详细信息。
...
2. 缓冲区未及时处理：考虑使用OH_MIDIDevice_FlushOutputPort接口清空缓冲区。
