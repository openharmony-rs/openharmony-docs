...
> **说明：**
>
> 本模块从API version 24开始支持。后续版本如有新增内容，则采用上标版本标记。

OH_MIDI是系统提供的Native MIDI API，用于在C/C++层实现MIDI功能。通过OH_MIDI，开发者可以：
...
使用MIDI进行开发前，先调用[canIUse](../../reference/common/init.md#caniuse)接口判断当前设备是否支持MIDI能力。当canIUse("SystemCapability.Multimedia.Audio.MIDI")返回值为true时，表示可以使用MIDI能力。
...
> **说明：**
>
> 完整的可运行示例请参考[快速体验：最小MIDI客户端](#快速体验最小midi客户端)。本节重点讲解API细节。
...
async sendSysExMessage(): Promise<void> {
  ...
  const sysexData = new Uint8Array([
    0xF0, 0x7E, 0x7F, 0x09, 0x01, 0xF7  // GM系统开启。
  ]);
...
3. 内存安全：`OnMIDIReceived`回调中的`events`数组及其中所有数据指针是临时的，仅在此回调期间有效。必须在回调返回前复制任何需要保留的数据。

   > **注意：**
   >
   > 访问过期指针会导致未定义行为（崩溃、内存损坏）。
...
5. 性能优化：在回调函数中禁止执行超过1毫秒的操作或阻塞I/O，以确保MIDI消息的实时处理不受影响。
   ...
   > **注意：**
   >
   > 在回调中执行耗时操作或阻塞I/O会严重影响MIDI消息的实时处理，可能导致音频延迟或丢帧。
