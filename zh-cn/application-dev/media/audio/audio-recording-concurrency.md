...
- 录音并存是指一个或多个应用创建的不同录音客户端（[AudioCapturer/apis-audio-kit/arkts-apis-audio-AudioCapturer.md)）在同一时间段内同时存在的状态。
...
- 不同的录音流类型对应着不同的系统优化处理策略，建议开发者根据需求选择合适的[SourceType/apis-audio-kit/arkts-apis-audio-e.md#sourcetype8)，具体可参考指南：[使用合适的音频流类型](using-right-streamusage-and-sourcetype.md)。
