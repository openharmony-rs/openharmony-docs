...
应用开发者在使用AudioSpatializationManager的接口前，需要先调用[getSpatializationManager/apis-audio-kit/arkts-apis-audio-AudioManager.md#getspatializationmanager18)创建AudioSpatializationManager实例。
...
应用开发者可以使用[AudioDeviceDescriptor/apis-audio-kit/arkts-apis-audio-i.md#audiodevicedescriptor)的spatializationSupported属性获取指定设备是否具有空间音频渲染的能力，需要通过音频框架中其他接口来获取当前已连接设备或当前发声设备的AudioDeviceDescriptor。例如，可以使用[getRoutingManager/apis-audio-kit/arkts-apis-audio-AudioManager.md#getroutingmanager9)创建的AudioRoutingManager实例的[getDevicesSync/apis-audio-kit/arkts-apis-audio-AudioRoutingManager.md#getdevicessync10)接口获取。
...
应用开发者可以通过[isSpatializationEnabledForCurrentDevice/apis-audio-kit/arkts-apis-audio-AudioSpatializationManager.md#isspatializationenabledforcurrentdevice18)接口查询当前设备的空间音频渲染效果开关状态。
...
应用开发者可以通过[on('spatializationEnabledChangeForCurrentDevice')/apis-audio-kit/arkts-apis-audio-AudioSpatializationManager.md#onspatializationenabledchangeforcurrentdevice18)接口订阅当前空间音频渲染效果的开关状态变化事件。
...
应用开发者可以通过[off('spatializationEnabledChangeForCurrentDevice')/apis-audio-kit/arkts-apis-audio-AudioSpatializationManager.md#offspatializationenabledchangeforcurrentdevice18)接口取消订阅空间音频渲染效果的开关状态变化事件。
