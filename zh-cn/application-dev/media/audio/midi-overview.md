...
MIDI设备是指物理或虚拟的MIDI接口设备，包括USB MIDI键盘、蓝牙MIDI控制器等。每个设备都有唯一的标识符（[OH_MIDIDeviceInformation](../../reference/apis-audio-kit/capi-native-midi-base-h.md#oh_midideviceinformation)中的`midiDeviceId`）用于区分和管理。
...
数据流向说明：应用向设备发送数据，应用调用[OH_MIDIDevice_Send](../../reference/apis-audio-kit/capi-native-midi-h.md#oh_mididevice_send)接口发送MIDI消息。
...
每个设备可以有多个输入和输出端口。每个端口在打开时，可以通过[OH_MIDIPortDescriptor](../../reference/apis-audio-kit/capi-native-midi-base-h.md#oh_midiportdescriptor)结构配置期望的协议行为。
...
* 自动转换说明：系统服务层会在接收方向自动将底层协议数据转换为UMP格式供应用处理。但在发送方向，应用需要按照UMP规范自行构造[OH_MIDIEvent](../../reference/apis-audio-kit/capi-native-midi-base-h.md#oh_midievent)数据。
...
* 数据长度（length）：表示data数组中32位字（Word）的数量。
...
| MT值 | 名称 | 大小（bit） | 说明 |
|------|------|------|------|
| 0x0 | Utility Messages | 32 | 实用消息，如[JR时间戳](#术语表)。 |
...
- 资源管理顺序：关闭客户端时会自动关闭所有设备和端口。建议应用按相反顺序关闭资源，以保证代码逻辑清晰。
  1. 关闭所有端口（[OH_MIDIDevice_CloseInputPort](../../reference/apis-audio-kit/capi-native-midi-h.md#oh_mididevice_closeinputport)/[OH_MIDIDevice_CloseOutputPort](../../reference/apis-audio-kit/capi-native-midi-h.md#oh_mididevice_closeoutputport)）。
  2. 关闭所有设备（[OH_MIDIClient_CloseDevice](../../reference/apis-audio-kit/capi-native-midi-h.md#oh_midiclient_closedevice)）。
