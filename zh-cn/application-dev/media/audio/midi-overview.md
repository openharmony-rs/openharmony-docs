...
- MIDI 1.0：1983年发布的经典协议，使用7位（bit）分辨率（取值范围0~127），单向通信，广泛兼容现有设备。
...
MIDI设备是指物理或虚拟的MIDI接口设备，包括USB MIDI键盘、蓝牙MIDI控制器等。设备连接后，MIDI服务会为其分配标识符（[OH_MIDIDeviceInformation](../../reference/apis-audio-kit/capi-native-midi-base-h.md#oh_midideviceinformation)中的`midiDeviceId`），用于在当前会话中区分和管理不同设备。
...
每个设备可以有多个输入和输出端口。当每个端口打开时，可以通过[OH_MIDIPortDescriptor](../../reference/apis-audio-kit/capi-native-midi-base-h.md#oh_midiportdescriptor)结构配置期望的协议行为。
...
* 自动转换说明：系统服务层会在接收方向自动将底层协议数据转换为UMP格式供应用处理。但在发送方向上，应用需要按照UMP规范自行构造[OH_MIDIEvent](../../reference/apis-audio-kit/capi-native-midi-base-h.md#oh_midievent)数据。
...
* 时间戳（timestamp）：纳秒级精度。取值范围0~2^63-1纳秒。0表示立即发送。
...
| MT值 | 名称 | 大小（bit） | 说明 |
|------|------|------|------|
| 0x0 | Utility Messages | 32 | 实用消息，如[JR时间戳（Jitter Reduction）](#术语表)。 |
