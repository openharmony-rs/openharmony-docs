...
Taihe在ANI场景下提供按需开启的Debug模式，用于在桥接调用过程中对关键ANI调用结果执行额外校验。该模式默认关闭，开发者在构建承载Taihe ANI生成代码的native target时定义`TH_ANI_ENABLE_CHECKED_CALL`后，即可在运行时对调用失败进行即时检测。
...
Taihe Debug模式自1.14版本起引入，该Debug模式主要用于Taihe生成的ANI桥接代码中，适用于所有通过Taihe IDL定义并生成ANI代码的接口调用。
...
- Taihe C++对象被转换为ArkTS对象时失败。例如C++侧传递了一个不合法的枚举值等。
  ...
  ```
- Taihe未能获取到ANI VM，这通常需要排查在调用Taihe ANI相关接口时，所在动态链接库中的ANIRegister是否已被调用过。
  ```text
...
此处以CMake为例，说明如何在构建承载Taihe ANI生成代码的native target时定义`TH_ANI_ENABLE_CHECKED_CALL`。
...
修改完成后重新构建native target，即可在运行时启用Debug模式。
