...
- **禁止空联合体**：联合体必须至少包含一个成员。
- **ArkTS投影规则**：为了能够将对象从ArkTS-STA中的联合类型正确转换到Taihe union，联合体的成员顺序和类型有严格要求，以确保正确的类型判断：
    1. **泛型容器限制**：联合体中不允许包含具有不同类型参数的**同一种泛型容器**。例如，`Array<i32>`和`Array<String>`不能同时存在。
    2. **成员顺序**：
        - 如果有`undefined`对应的成员，必须放在**第一位**。
        - 如果有`null`对应的成员，建议放在**第二位**（或紧随`undefined`之后）。
        - 如果存在继承关系的类型，**子类应放在父类之前**。
        - `Object`类型（如果使用）必须放在**最后**。
