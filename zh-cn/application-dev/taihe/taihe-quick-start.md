...
| `ani-bridge` | 生成 ANI 用户侧的桥接代码 | `cpp-user` |
...
*注：DevEco 用户必须指定 `arkts:module-prefix` 和 `arkts:path-prefix`*

## 使用示例

以下是一个 `taihec` 的基本使用示例：
```sh
taihec test/ani_test/idl/*.ohidl -Otest/ani_test/generated -Gani-bridge -Gcpp-author -Carkts:module-prefix=<module_name> -Carkts:path-prefix=<pkg_name> -Bcmake  # 生成用户自己在 IDL 中定义的接口的 ANI 桥接代码，以及 C++ 实现模板等
```
