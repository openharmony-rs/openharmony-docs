...
这里以Grid懒加载组件复用场景为例，在aboutToReuse中加入测试日志，观察在滑动Grid时aboutToReuse的调用情况。
```ts
...
下面通过在组件属性中使用耗时的函数入参调用作为反例，使用@ohos.taskpool (启动任务池)处理耗时操作后返回结果给Row的高度rowHeight作为正例，来对比组件属性刷新时的性能差异。
