...
当应用在运行时出现明显的延迟情况，会影响用户体验，开发者需要定位、分析、解决应用时延问题。本文先简单介绍应用流畅度评测指标，然后基于Trace数据，介绍时延问题分析思路，并结合案例实践，实操定位分析并优化时延问题。
...
在可滑动页面中，从滚动停止开始计算，到屏幕内占位符（一般是图片）加载完成。
...
滑动操作占位符完成时延类问题的通用定位思路如下：首先确认时延起止点，然后判断起止点时延是否超标准的40ms。若未超过则视为达标，若超过则需要进一步分析Trace耗时，重点关注网络耗时和帧渲染耗时，最后确定是系统问题还是三方问题。
...
1. 在FLING结束点，查看ArkTS CallStack调用栈中的耗时任务。如果发现耗时任务，则继续分析其原因，通常需要结合应用进程UI主线程查看；如未发现耗时任务（比如idle状态），则查看此时Frame应用侧是否有渲染任务及其对应的Component组件情况，通常idle状态的应用送帧情况为动画。
