...
本文主要是以Trace数据为切入点进行分析，相应的工具可以使用SmartPerf Host或DevEco Studio内置的Frame等。若开发者需要补充SmartPerf Host工具和Trace相关知识，可以分别参考《性能优化工具SmartPerf-Host》和《常用Trace使用指导》等应用开发文档。
...
如果依据RSMainThread::DoComposition标签，从GPU合成图层的角度，去模拟最终呈现给用户的帧数，平均帧率为32.5FPS。


如果依据Repaint标签，从硬件合成器合成绘制的角度，去模拟最终呈现给用户的帧数，平均帧率为29.27FPS。
...
| 启动应用 | OHOS::AppExecFwk::MainThread::HandleLaunchApplication() |
| 启动UI Ability | OHOS::AppExecFwk::MainThread::HandleLaunchAbility() |
| 创建窗口 | AbilityMonitor::OnWindowStageCreate() |
| 绘制应用UI界面首帧 | JsRuntime::RunScript() RSMainThread::SendCommands() |

而实际分析时，开发者可以借助SmartPerf Host提供的AppStartup模板，将各启动阶段自动拆解出来，如图14所示。关于AppStartup模板详细使用方法，请参考《性能优化工具SmartPerf-Host》中相关章节。
