...
主接口文件引用了其他文件中的类型，使用 -C 参数来引用公共数据类型。带 -C 参数的生成命令会将公共数据类型生成到单独的 ExampleServiceIpcTypes.cpp 和 ExampleServiceIpcTypes.h 文件当中。
```bash
taihec
-G modobj-ipc
...
<!-- @[easy](https://gitcode.com/openharmony/applications_app_samples/blob/7aee34bd505c74f144d08d6f36a79328c96fecce/code/DocsSample/Ability/ModularObjectExtensionAbilityIDL/exampleone/example/Easy.ohidl) -->
```rust
@!namespace("OHOS", "IPC")
...
首先用户需在自定义路径下创建idl文件
 ...
 <!-- @[example](https://gitcode.com/openharmony/applications_app_samples/blob/1976fa07f8045c9ffdd6ec80fc78037b254b9383/code/DocsSample/Ability/ModularObjectExtensionAbilityIDL/examplethree/example/ITestInterface.ohidl) -->
```rust
@!namespace("OHOS", "IPC")
