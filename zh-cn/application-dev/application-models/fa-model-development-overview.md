...
- FA模型：从API 7开始支持的模型，已经不再主推。FA模型中每个应用组件独享一个ArkTS引擎实例，适用于简单应用的开发。
    
- Stage模型：从API 9开始新增的模型，是目前主推且会长期演进的模型。Stage模型支持多个应用组件共享同一个ArkTS引擎实例，以及应用组件间的状态共享与对象调用，可以降低内存开销、提升开发效率，适用于复杂应用的开发。
    
FA模型当前主要用于Lite Wearable设备。其他场景推荐使用Stage模型，详见应用模型概述。
...
| 任务 | 简介 | 相关指导 |
| -------- | -------- | -------- |
| 应用组件开发 | 本章节介绍了如何使用FA模型的PageAbility、ServiceAbility、DataAbility以及服务卡片进行应用开发。 | 应用/组件级配置<br/>PageAbility组件概述<br/>ServiceAbility组件概述<br/>DataAbility组件概述<br/>服务卡片开发指导<br/>FA模型的Context<br/>信息传递载体Want |
