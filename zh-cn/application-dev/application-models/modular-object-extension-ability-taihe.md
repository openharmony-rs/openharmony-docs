...
通过Taihe可自动生成ModularObjectExtensionAbility（[modular_object_extension_ability.h](../reference/apis-ability-kit/capi-modular-object-extension-ability-h.md)）所需的Proxy/Stub代码及类型库文件，屏蔽IPC通信底层细节（如参数序列化/反序列化、消息码分发等），使开发者专注于接口设计与业务逻辑，大幅简化ModularObjectExtensionAbility的开发流程。
...
Taihe提供了核心编译器工具`taihec`，用于解析`.ohidl`文件并编译为目标语言代码。本章节介绍如何使用`taihec`生成用于ModularObjectExtensionAbility IPC通信所需的Proxy、Stub代码。

```bash
taihec [taihe_files ...] [options ...]
```

- `taihe_files`：可以是一个或多个`.idl`文件，主接口文件和公共类型文件需一并列出。
...
| `--codegen <namespace>:<config>[=<value>]` | `-C <namespace>:<config>[=<value>]` | 额外的代码生成配置项，ModularObjectExtensionAbility的IPC通信场景下仅支持`modobj:ipc-common=common.ohidl`，用于引入公共类型文件。 |
...
**参数与返回值约定**
- 方法参数统一视为输入参数，代表数据从Proxy发送到Stub。
- 方法返回值统一视为输出参数，代表数据从Stub返回到Proxy。
- 需要返回多个字段时，用`struct`作为返回值封装。

**基础数据类型**

| Taihe     | C++ 类型   | Parcel 写入/读取规则                                         |
| :-------  | :------    | :--------------------------------------------------------   |
...
| 注解                             | 作用范围   |说明                                    | 示例                               |                   
| :---                             | :---       | :---                                  | :---                               |                     
| `@main_service(version="x.y.z")` | interface     | 声明主服务接口，有且只有一个，生成的Stub用于在[OH_AbilityRuntime_ModObjExtensionAbility_OnConnectFunc](../reference/apis-ability-kit/capi-modular-object-extension-ability-h.md#oh_abilityruntime_modobjextensionability_onconnectfunc)中返回。  | @main_service(version="1.0.0")    |
| `@callback`                      | 方法       | 用于客户端向服务端注册回调接口。回调接口被调用时在客户端执行，不跟随服务端的线程模型。 | @callback interface ICallback {}  |
| `@oneway`                        | 方法       | 异步单向IPC调用，仅支持void类型返回值。       | @oneway Notify(...): void;        |
| `@!namespace("A","B")`           | ohidl文件   | 设置生成的代码所在命名空间，以及IPC接口描述符字符串前缀。 | @!namespace("OHOS","NativeApp")   |
| `@size(N)`                       | 参数       | 定长数组的大小，该注解仅供Array类型参数使用。  | @size(4) Array<i32>; |
...
- 为保证兼容性，多个版本间interface中已有方法不能改变顺序，否则会导致IpcCode不一致。
...
| 方法              | 默认行为                                                                         | 常量                        |
| :---              | :---                                                                            |:-----                       |
| `GetTypeLibInfo`  | 类型库信息文件，供系统调用，开发者无需关注。                                        | COMMAND_GET_TYPE_LIB_INFO = 1 |
...
请参考[使用ModularObjectExtensionAbility实现模块化对象 (C/C++)](./modular-object-extension-development.md)部分。