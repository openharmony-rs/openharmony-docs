...
从API version 24开始，提供应用快启机制。该机制提前持久化保存应用启动的中间状态，启动应用从中间状态直接加载。当用户启动应用时，由于会跳过部分的启动流程，所需的启动时间会缩短，有助于提升用户的启动体验和应用竞争力。开发者可以通过如下两方面判断，应用是否适合接入快启特性
...
快照制作中，禁止有状态的进程间通讯，比如保存应用状态和数据持久化，否则会触发保护极致导致快照制作失败。
...
举例四：类静态变量初始化中存在快照风险操作下面的例子中存在两类静态执行，一个是静态变量data的初始化，会直接进行文件读取；另一个是静态域会直接。//检查下这个代码是不是对的
...
* 快照内，AbilityStage.onCreate中读取本地配置，并重新调用接口注：这种设计有两个收益：
  ...
  export class AppAbilityStage extends AbilityStage {
    onCreate() {
  	...
  	readConfigFromDB() {
       //read local value of "setHyperSnapEnbaled" from DB
     }
  }
  ```
...
特别地，不建议将启动之后的import动作前置到启动流程甚至快照流程中去，因为这样会导致两方面问题：（1）劣化普通冷启动性能（2)导致启动流程的内存发生不必要的膨胀
