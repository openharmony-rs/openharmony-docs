...
| ------------| ---------------- | ----------------------- |
| 应用主动销毁 | 开发者显式调用terminateSelf()。具体场景例如：1.应用检测到致命错误，主动终止进程以避免数据损坏；2.完成核心任务后（如应用内一键清理缓存），主动结束自己。      |   是   |
| 用户返回键退出 | 用户侧滑返回或按返回键退出。 |  是   |
| 用户主动清理UIAbility | 用户在多任务管理界面中上滑清理单个卡片，或点击"清除全部"按钮。 |  是 |
| 应用异常退出 | 应用进程异常崩溃或被系统强制终止。具体场景例如：应用内部发生jscrash或出现指针异常等。 |   否 |
| 系统清理    | 系统因资源回收等原因自动清理UIAbility。具体场景例如：1.系统内存极度紧张，回收后台进程以释放资源；2.系统因电量优化、权限变更等策略终止进程。 |  否 |
