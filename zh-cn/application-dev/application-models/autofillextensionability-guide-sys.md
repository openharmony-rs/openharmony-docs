...
AutoFillExtensionAbility是AUTO_FILL_PASSWORD/AUTO_FILL_SMART类型的ExtensionAbility组件，提供自动填充能力。
...
自动填充功能主要接口如下。其他接口介绍详情参见AutoFillRequest、AutoFillExtensionAbility。
...
AutoFillExtensionAbility提供了onCreate、onSessionDestroy、onForeground、onBackground、onDestroy、onSaveRequest和onFillRequest生命周期回调，根据需要重写对应的回调方法。
...
1. 设定AutoFillExtensionAbility应用的包名。
   ...
   在AppScope/app.json5配置文件中把bundleName设定为："com.ohos.passwordbox"，例如：
...
2. 配置extensionAbilities信息。
   ...
   在entry/src/main/module.json5配置文件中配置AutoFillAbility，例如：
...
情景化自动填充的具体类型可参考AutoFillType。

开发者在实现情景化自动填充服务时，需要在DevEco Studio工程中手动新建一个SmartAutoFillExtensionAbility，具体步骤如下。

1. 设定AutoFillExtensionAbility应用的包名。

   在AppScope/app.json5配置文件中把bundleName设定为："com.ohos.textautofill"，例如：
...
3. 情景化自动填充与自动填充服务的实现基本一致。请参考实现账号密码自动填充功能。
