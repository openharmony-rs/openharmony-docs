...
- multiton：多实例模式，每次调用[startAbility()/apis-ability-kit/js-apis-inner-application-uiAbilityContext.md#startability)方法，都会在应用进程中创建一个该UIAbility实例。
  
  **图2** 任务与multiton模式  
  ![mission-and-multiton](figures/mission-and-multiton.png)
  
- specified：指定实例模式，由[AbilityStage](abilitystage.md)的[onAcceptWant()/apis-ability-kit/js-apis-app-ability-abilityStage.md#onacceptwant)决定是否创建新的UIAbility实例。
