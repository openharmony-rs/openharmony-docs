...
[AbilityStage/apis-ability-kit/js-apis-app-ability-abilityStage.md)是一个[Module](../quick-start/application-package-overview.md#应用的多module设计机制)级别的组件管理器。
...
[意图提供方管理能力/apis-ability-kit/js-apis-app-ability-insightIntentProvider.md)指通过声明标准意图或自定义意图，将业务功能接入意图框架的应用主体。
...
开发者可以设置[onNewProcessRequest/apis-ability-kit/js-apis-app-ability-abilityStage.md#onnewprocessrequest11)的返回值，让主控进程通过接收该接口的回调信息，决定新的Ability实例运行在哪个进程中。
...
显式Want启动是指在启动目标应用组件时，调用方传入的[want/apis-ability-kit/js-apis-app-ability-want.md)参数中指定了abilityName和bundleName，称为显式Want启动。
...
隐式Want启动是指在启动目标应用组件时，调用方传入的[want/apis-ability-kit/js-apis-app-ability-want.md)参数中未指定abilityName，称为隐式Want启动。
