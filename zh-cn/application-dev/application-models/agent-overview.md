...
随着智能化的深入发展，越来越多的应用需要具备智能交互和跨应用协作的能力。传统应用之间的交互通常需要预先定义紧耦合的接口，开发成本高、扩展性差。从API version 24开始，鸿蒙端侧智能体框架（端侧A2A）为智能体（Agent）提供了一种标准化的通信与交互机制，该框架是HMAF（Harmony Agent Framework）框架在端侧能力的延伸，支持统一A2A协议规范和端云互调。
