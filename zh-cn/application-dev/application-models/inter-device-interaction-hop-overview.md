...
跨多设备的分布式操作可以统称为**流转**；根据使用场景的不同，流转又分为跨端迁移和多端协同两种具体场景。
...
- **多端协同**
  ...
  - 场景二：设备A上正在和客户进行聊天，客户需要的资料在设备B上，可以通过聊天软件打开设备B上的文档应用选择到想要的资料回传到设备A上，然后通过聊天软件发送给客户。在应用开发层面，多端协同指多端上的不同[UIAbility/apis-ability-kit/js-apis-app-ability-uiAbility.md)/[ServiceExtensionAbility/apis-ability-kit/js-apis-app-ability-serviceExtensionAbility-sys.md)同时运行、或者交替运行实现完整的业务；或者多端上的相同[UIAbility/apis-ability-kit/js-apis-app-ability-uiAbility.md)/[ServiceExtensionAbility/apis-ability-kit/js-apis-app-ability-serviceExtensionAbility-sys.md)同时运行实现完整的业务。
