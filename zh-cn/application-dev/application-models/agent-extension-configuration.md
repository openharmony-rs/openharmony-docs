...
| type | Agent卡片的类型。支持数值枚举和字符串枚举名称：<br>- 0（APP）表示应用型，<br>- 1（ATOMIC_SERVICE）表示原子化服务型<!--Del-->，<br>- 2（LOW_CODE）表示低代码型<!--DelEnd-->。<!--Del-->LOW_CODE类型为系统接口，仅系统应用可使用。<!--DelEnd--> <br>如果未指定，默认为APP类型。<br>**说明**：从API版本26.0.0开始支持。 | 整数或字符串 | 可缺省，缺省为0（APP）。 |
