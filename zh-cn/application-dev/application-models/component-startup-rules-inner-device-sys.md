...
为了保障系统安全与用户体验，系统限制了应用在后台状态时任意弹窗、相互唤醒以及前台应用任意跳转的行为，相关行为表现请参考[设备内组件启动规则（Stage模型）](./component-startup-rules-inner-device.md)。本文主要介绍系统应用在设备内启动[UIAbility/apis-ability-kit/js-apis-app-ability-uiAbility.md)和[ExtensionAbility/apis-ability-kit/js-apis-app-ability-extensionAbility.md)的约束规则。
...
- [DataShareExtensionAbility/apis-arkdata/js-apis-application-dataShareExtensionAbility-sys.md)组件启动规则：

   通过[createDataShareHelper/apis-arkdata/js-apis-data-dataShare-sys.md#datasharecreatedatasharehelper)接口可以启动DataShareExtensionAbility组件，具体操作和限制请参考[通过DataShareExtensionAbility实现数据共享](../database/share-data-by-datashareextensionability-sys.md)。
