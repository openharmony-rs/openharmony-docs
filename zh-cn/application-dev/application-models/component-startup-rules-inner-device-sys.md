...
为了保障系统安全与用户体验，系统限制了应用在后台状态时任意弹窗、相互唤醒以及前台应用任意跳转的行为，相关行为表现请参考设备内组件启动规则。本文主要介绍系统应用在设备内启动UIAbility和ExtensionAbility的约束规则。
...
### 应用内启动UIAbility组件的规则
   ...
   位于后台状态的UIAbility应用，默认不允许再启动UIAbility组件。可申请ohos.permission.START_ABILITIES_FROM_BACKGROUND（下文简称BACKGROUND）权限启动UIAbility组件，权限的申请方式请参考声明权限。
...
### 跨应用启动UIAbility组件的规则
   ...
   通过startAbility()/openLink()等跨应用启动UIAbility组件时，只允许拉起exported为true的目标组件。若申请ohos.permission.START_INVISIBLE_ABILITY（下文简称INVISIBLE）权限，可不受该条规则约束。位于后台状态的UIAbility应用，默认不允许跨应用启动UIAbility组件，需申请BACKGROUND权限启动UIAbility组件。权限的申请方式请参考声明权限。
...
所有类型的ExtensionAbility组件（ServiceExtensionAbility、DataShareExtensionAbility除外）是由相应的系统管理服务拉起，以确保其生命周期受系统管控。ExtensionAbility组件在使用时被拉起，使用完则销毁。

- ServiceExtensionAbility组件启动规则：

   通过startServiceExtensionAbility()跨应用启动或使用connectServiceExtensionAbility()跨应用连接ServiceExtensionAbility组件时，只允许拉起exported为true的目标组件。若申请INVISIBLE权限，可不受该条规则约束。
...
- DataShareExtensionAbility组件启动规则：
   ...
   通过createDataShareHelper()接口可以启动DataShareExtensionAbility组件，具体操作和限制请参考通过DataShareExtensionAbility实现数据共享。

- 其他ExtensionAbility组件启动规则：
   
   其他ExtensionAbility组件的启动规则请参考ExtensionAbility组件。
