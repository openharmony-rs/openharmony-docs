...
从ArkTS-Dyn API version 20、ArkTS-Sta API version 23开始，支持使用机械体设备控制器，提供更丰富的拍摄体验，如控制设备转动（仅限系统应用）。
...
|getMaxRotationTime(mechId: number): number | 获取机械体设备的最大连续旋转持续时间。|
|getMaxRotationSpeed(mechId: number): RotationSpeed | 获取机械体设备的最大旋转速度。|
|stopMoving(mechId: number): Promise\<void> | 停止机械体设备的移动。|
|getCurrentAngles(mechId: number): EulerAngles | 获取机械体设备的当前角度。|
|getRotationLimits(mechId: number): RotationLimits | 获取指定机械体设备相对于参考点的最大旋转角度。|
|getRotationAxesStatus(mechId: number): RotationAxesStatus | 获取旋转轴的状态。|
...
2. 执行相对角度的旋转控制，以调整设备的位置。
    ...
    try {
        ...
        const currentYaw = (currentAngles.yaw ?? 0) as number;
        const currentPitch = (currentAngles.pitch ?? 0) as number;
        const currentRoll = (currentAngles.roll ?? 0) as number;

        const negYawMax = (rotationLimits.negativeYawMax ?? 0) as number;
        const posYawMax = (rotationLimits.positiveYawMax ?? 0) as number;
        const negPitchMax = (rotationLimits.negativePitchMax ?? 0) as number;
        const posPitchMax = (rotationLimits.positivePitchMax ?? 0) as number;
        const negRollMax = (rotationLimits.negativeRollMax ?? 0) as number;
        const posRollMax = (rotationLimits.positiveRollMax ?? 0) as number;

        if (
            currentYaw + (angles.yaw ?? 0) > negYawMax ||
                currentYaw + (angles.yaw ?? 0) < posYawMax ||
                currentPitch + (angles.pitch ?? 0) > negPitchMax ||
                currentPitch + (angles.pitch ?? 0) < posPitchMax ||
                currentRoll + (angles.roll ?? 0) > negRollMax ||
                currentRoll + (angles.roll ?? 0) < posRollMax
...
3. 以指定速度持续转动，直至任务完成。
    ...
    try {
        ...
        // 获取最大旋转速度
        let maxSpeed = mechanicManager.getMaxRotationSpeed(mechId);
        if (!maxSpeed || maxSpeed.yawSpeed === undefined || maxSpeed.pitchSpeed === undefined ||
            maxSpeed.rollSpeed === undefined) {
            console.error('Failed to retrieve maximum rotation speed or speed values are undefined.');
            return;
        ...
        // 定义旋转速度和持续时间
        const speed: mechanicManager.RotationSpeed = {
            yawSpeed: (maxSpeed.yawSpeed ?? 0) as int / 2, // 偏航速度：最大速度的一半
            pitchSpeed: (maxSpeed.pitchSpeed ?? 0) as int / 2, // 俯仰速度：最大速度的一半
            rollSpeed: (maxSpeed.rollSpeed ?? 0) as int / 2    // 横滚速度：最大速度的一半
        }

        const duration = Math.floor(Math.min(maxTime, 5000)) as int; // 持续时间：最多5秒
...
4. 监听旋转轴状态变化。
    ...
    // 注册监听
    mechanicManager.onRotationAxesStatusChange(rotationAxesCallback);
    ```
