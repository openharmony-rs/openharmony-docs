...
NFC卡模拟完整的API说明以及实例代码请参考：[NFC卡模拟接口/apis-connectivity-kit/js-apis-cardEmulation.md)。
...
- HCE应用前台刷卡<br>
1. 在配置文件module.json5中，不需要静态声明NFC读卡器选择的应用ID（AID，参考ISO/IEC 7816-4规范），而是通过[start/apis-connectivity-kit/js-apis-cardEmulation.md#start9)来动态注册。
2. HCE应用的刷卡页面退出时，需要显式调用[stop/apis-connectivity-kit/js-apis-cardEmulation.md#stop9)来释放动态注册的AID刷卡配置项。
- HCE应用后台刷卡<br>
...
5. HCE应用后台刷卡建议使用单独专用的HceAbility实现，以减少[onCreate/apis-ability-kit/js-apis-app-ability-uiAbility.md#oncreate)函数耗时。
6. HceAbility的OnCreate函数中除了[hceService.on/apis-connectivity-kit/js-apis-cardEmulation.md#on8)外，尽量减少其它操作。
...
> - HCE示例代码的提供，全部按照[Stage模型](../../application-models/ability-terminology.md#stage模型)来说明。
