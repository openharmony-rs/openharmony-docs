...
星闪基于SSAP（SparkLink Service Access Protocol，星闪服务交互协议）实现设备间的服务管理。应用可分别以服务端或客户端角色参与：服务端是服务的承载方，创建服务并声明属性，接收和响应客户端的读写请求；客户端是服务的使用方，连接服务端后读取、写入属性并订阅属性变化通知。
...
4. 添加服务端支持的服务。服务与属性使用自定义UUID（禁止使用标准UUID），参见[星闪常见问题 > 标准 UUID 与自定义 UUID 有什么区别](nearlink-faq-guide.md#标准-uuid-与自定义-uuid-有什么区别)；支持通知的属性需声明客户端属性值配置描述符，参见[星闪常见问题 > SSAP 属性描述符的作用](nearlink-faq-guide.md#ssap-属性描述符的作用)。
...
5. 订阅连接状态变化事件，并在回调中保存客户端地址，供后续通知使用。
...
7. 订阅客户端写属性请求事件，将写入的值保存到对应属性。
    ...
    ``` TypeScript
    try {
      // Property write request: save the written value to the matching property
...
8. 通知客户端属性值更新。其中connectedAddress为已连接客户端的地址，在步骤5的连接状态回调中赋值保存。
    ...
    ``` TypeScript
    try {
      // Update the property with a fixed demo value; replace with business data in real development
      propertyValue1 = 0x4E;
...
8. 设置属性变化通知。仅当服务端对应属性支持通知（NOTIFY）操作并声明了客户端属性值配置描述符时，通知才能生效，参见[星闪常见问题 > SSAP 属性描述符的作用](nearlink-faq-guide.md#ssap-属性描述符的作用)。
