...
支持功能键列表：从API version 16开始支持音量加按键和音量减按键。从API version 21开始额外支持多媒体播放/暂停、多媒体下一首和多媒体上一首按键。
约束：应用只有处于焦点时，优先响应才生效。
注意：应用选择高于系统优先响应指定的系统功能键后，功能键的默认行为将失效，所以应用需要确保只有在确定响应时才激活该功能。
