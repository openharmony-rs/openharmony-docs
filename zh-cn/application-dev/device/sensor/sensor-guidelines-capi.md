...
2. 配置加速度传感器权限，具体配置方式请参考声明权限。
   ...
   <!-- @sensor_capi_permission_example --> 
...
4. 在oh_sensor_capi.cpp文件中编码，首先导入模块。
   ...
   <!-- @sensor_capi_development_dependency_import_example --> 
...
5. 定义常量。
   ...
   <!-- @sensor_capi_define_variables_example --> 
...
6. 定义一个回调函数用来接收传感器数据。
   ...
   <!-- @sensor_capi_define_callback_example -->
...
7. 获取设备上所有传感器的信息。  
   ...
   <!-- @sensor_capi_get_sensors_info_example --> 
...
8. 订阅和取消订阅传感器数据。
   ...
   <!-- @sensor_capi_subscriber_example --> 
...
10. 在types/libentry路径下index.d.ts文件中引入Napi接口。
    ...
    <!-- @sensor_capi_dependency_napi_example --> 
