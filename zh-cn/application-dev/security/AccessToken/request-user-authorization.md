...
2. 校验当前是否已经授权。
   ...
   async function checkPermissionGrant(permission: Permissions): Promise<abilityAccessCtrl.GrantStatus> {
     ...
     try {
       ...
       tokenId = appInfo?.accessTokenId as long as int;
     } catch (error) {
       const err: BusinessError = error as BusinessError;
       console.error(`Failed to get bundle info for self. Code is ${err.code}, message is ${err.message}`);
     }
...
3. 动态向用户申请授权。
   ...
   - 在UIAbility中向用户申请授权。
      ...
      ArkTS-Sta示例：
      ```ts
      import { abilityAccessCtrl, common, Permissions, UIAbility, PermissionRequestResult } from '@kit.AbilityKit';
      ...
      function reqPermissionsFromUser(permissions: Array<Permissions>, context: common.UIAbilityContext): void {
        ...
        atManager.requestPermissionsFromUser(context, permissions).then((data: PermissionRequestResult): void => {
          ...
          let length: int = grantStatus.length;
          for (let i = 0; i < length; i++) {
            if (grantStatus[i] === 0) {
              // 用户授权，可以继续访问目标操作。
            } else {
