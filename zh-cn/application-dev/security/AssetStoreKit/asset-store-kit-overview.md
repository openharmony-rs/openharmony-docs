...
- **安全环境执行：** 关键资产的加解密操作和访问控制校验均在安全环境（如可信执行环境）中完成，使用AES256-GCM算法进行加密，密钥存储在通用密钥库系统中，保证用户敏感数据不发生泄露。
- **身份隔离：** 关键资产存储服务会对应用身份（应用的appId）校验，确保应用只能读取或修改本应用的数据，无法读取或修改其他应用数据。相同群组的应用之间数据可以互相访问，详情请参见管理群组关键资产(ArkTS)/管理群组关键资产(C/C++)。
...
- **基于属主的访问控制：** 所有的关键资产都受属主访问控制保护，业务无需设置。
  ...
  - 关键资产属主身份（应用的appId）由ASSET从系统服务中获取，即使业务身份被仿冒，仿冒者也无法获取到其他业务的数据。
...
- 使用限制
  ...
  ASSET以安全存储为核心设计目标，提供用户短敏感数据的安全存储能力，并非通用存储服务。使用时需遵循以下限制：

  - **不适用于大量数据存储：** ASSET面向密码、Token、银行卡号等长度较短、数量有限的关键敏感信息，并非为大数据量场景设计。对于较长的数据，请考虑使用通用密钥库系统或加解密算法库框架保护数据。

  - **不适用于高性能场景：** 关键资产的加解密操作和访问控制校验均在安全环境（如可信执行环境）中完成，相较于常规存储存在更高的性能开销，不适合高频读写、低时延等高性能场景。对性能敏感的业务请勿将ASSET作为常规存储使用。

  - **非短敏感数据不推荐使用ASSET存储：** ASSET侧重安全而非通用存储，将非敏感的常规业务数据存入ASSET既无法发挥其安全优势，又会占用本就有限的存储资源。对于非短敏感数据，请使用系统提供的常规存储能力。

- 基于别名的访问

  关键资产以密文的形式存储在ASSET数据库中，以业务身份（应用的appId） + 别名作为唯一索引。故业务需要保证每条关键资产的别名唯一。
...
关键资产有且仅有以下删除时机：
- 业务主动调用remove删除关键资产时，删除符合条件的数据。详见删除关键资产(ArkTS)/删除关键资产(C/C++)开发指导。
- 应用卸载时，关键资产存储服务会清除对应的数据。IS_PERSISTENT属性设置为true的数据将保留。
    - 对于API版本21及之后版本：清除存储在ASSET中的非群组数据。群组数据仅在群组内所有应用卸载时清除。
    - 对于API版本20及之前版本：清除存储在ASSET中的非群组和群组数据。
