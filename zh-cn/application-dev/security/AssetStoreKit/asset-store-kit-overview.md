...
- 基于群组的访问控制
  ...
  - [IS_PERSISTENT/apis-asset-store-kit/js-apis-asset.md#tag)属性设置为True的关键资产，不允许设置为群组共享。

- 关键资产删除时机

  关键资产有且仅有以下删除时机：
  - 业务主动调用remove删除关键资产时，删除符合条件的数据。详见删除关键资产[ArkTS](asset-js-remove.md)、[C/C++](asset-native-remove.md)开发指导。
  - 应用卸载时，会清除对应的数据。[IS_PERSISTENT/apis-asset-store-kit/js-apis-asset.md#tag)属性设置为true的数据将保留。
