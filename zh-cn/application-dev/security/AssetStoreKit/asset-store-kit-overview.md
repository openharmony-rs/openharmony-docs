...
关键资产的安全存储，依赖底层的[通用密钥库系统](../UniversalKeystoreKit/huks-overview.md)。具体来说，关键资产的加/解密操作以及访问控制校验，都由通用密钥库系统在安全环境（如可信执行环境）中完成，即使系统被攻破，也能保证用户敏感数据不发生泄露。
...
### 使用限制
  ...
  - **不适用于大量数据存储：** ASSET面向密码、Token、银行卡号等长度较短、数量有限的关键敏感信息，并非为大数据量场景设计。对于较长的数据，请考虑使用[通用密钥库系统](../UniversalKeystoreKit/huks-overview.md)或[加解密算法库框架](../CryptoArchitectureKit/crypto-architecture-kit-intro.md)保护数据。
