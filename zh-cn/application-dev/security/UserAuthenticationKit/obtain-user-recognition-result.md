# 查询和订阅用户识别结果

<!--Kit: User Authentication Kit-->
<!--Subsystem: UserIAM-->
<!--Owner: @WALL_EYE-->
<!--Designer: @lichangting518-->
<!--Tester: @jane_lz-->
<!--Adviser: @zengyawen-->

系统持续识别当前使用者。应用可查询最新识别结果或订阅识别结果变化，用于判断当前使用者是否与已登录的系统用户匹配。

## 接口说明

具体参数、返回值、错误码等描述，请参考对应的[userAuth.getUserRecognitionMgr](../../reference/apis-user-authentication-kit/js-apis-useriam-userauth.md#userauthgetuserrecognitionmgr2610)。

| 接口名称 | 功能描述 |
| -------- | -------- |
| getUserRecognitionMgr(): UserRecognitionMgr \| null | 获取UserRecognitionMgr实例，用于查询和订阅用户识别结果。 |
| getUserRecognitionResult(): Promise\<UserRecognitionResult\> | 获取最新的用户识别结果。 |
| onUserRecognitionChange(callback: UserRecognitionResultCallback): void | 订阅用户识别结果变化事件。 |
| offUserRecognitionChange(callback?: UserRecognitionResultCallback): void | 取消订阅用户识别结果变化事件。 |

## 开发步骤

1. [申请权限](prerequisites.md#申请权限)：ohos.permission.ACCESS_USER_PASSIVE_RECOGNITION。

   该权限授权方式为system_grant，在module.json5中声明即可获取。

2. 调用[getUserRecognitionMgr](../../reference/apis-user-authentication-kit/js-apis-useriam-userauth.md#userauthgetuserrecognitionmgr2610)获取实例。

3. 调用[getUserRecognitionResult](../../reference/apis-user-authentication-kit/js-apis-useriam-userauth.md#getuserrecognitionresult2610)获取最新识别结果，或调用[onUserRecognitionChange](../../reference/apis-user-authentication-kit/js-apis-useriam-userauth.md#onuserrecognitionchange2610)订阅变化事件。

```ts
import { userAuth } from '@kit.UserAuthenticationKit';

// 获取实例
let mgr = userAuth.getUserRecognitionMgr();

// 获取最新识别结果
async function getRecognitionResult() {
  try {
    let result = await mgr.getUserRecognitionResult();
    console.info(`status: ${result.status}, userId: ${result.userId}`);
  } catch (err) {
    console.error(`getUserRecognitionResult failed, Code: ${err?.code}, message: ${err?.message}`);
  }
}

// 订阅识别结果变化
let callback: userAuth.UserRecognitionResultCallback = (result: userAuth.UserRecognitionResult) => {
  console.info(`status: ${result.status}, userId: ${result.userId}`);
};
mgr.onUserRecognitionChange(callback);

// 取消订阅
mgr.offUserRecognitionChange(callback);
```

## 示例代码

  - [查询和订阅用户识别结果](https://gitcode.com/openharmony/applications_app_samples/tree/master/code/DocsSample/UserAuthentication)
