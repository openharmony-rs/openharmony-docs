...
SM4（CBC模式）解密失败返回错误码17630001可参考使用SM4-ECB/CBC算法解密时调用doFinal失败
...
2. 调用cryptoFramework.createCipher，指定字符串参数'SM4_128|GCM'，创建对称密钥类型为SM4_128、分组模式为GCM的Cipher实例，用于完成加密操作。
...
1. 调用cryptoFramework.createCipher，指定字符串参数'SM4_128|GCM'，创建对称密钥类型为SM4_128、分组模式为GCM的Cipher实例，用于完成解密操作。
...
SM4（GCM模式）解密失败返回错误码17630001可参考使用SM4-GCM算法解密时调用doFinal失败
