...
3. 设置指定字节长度，调用Random.generateRandom或Random.generateRandomSync，生成安全随机数。

   指定字节长度范围为1~INT_MAX。

- 通过await返回异步结果：
  <!-- @secure_hard_ware_random_number_generation_await -->
...
- 同步返回结果：
  <!-- @secure_hard_ware_random_number_generation_sync -->
