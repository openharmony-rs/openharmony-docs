...
1. 指定uint8_t类型的ECC非压缩公钥数据，封装成Crypto_DataBlob。

   公钥和私钥可单独传入，此处示例传入非压缩公钥。

2. 调用OH_CryptoAsymKeyGenerator_Create，指定字符串参数'ECC_BrainPoolP256r1'，创建密钥算法为ECC、密钥长度为256位的非对称密钥生成器（OH_CryptoAsymKeyGenerator）。
