...
2. 调用OH_CryptoAsymKeyGenerator_Convert，传入私钥数据，生成非对称密钥对象（OH_CryptoKeyPair）。

3. 调用OH_CryptoKeyPair_GetPubKey获取公钥对象 (OH_CryptoPubKey)。

4. 调用OH_CryptoPubKey_Encode获取公钥对象的二进制数据。
