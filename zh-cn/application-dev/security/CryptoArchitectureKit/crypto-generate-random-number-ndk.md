...
1. 调用OH_CryptoRand_Create，创建随机数生成器。

2. （可选）调用OH_CryptoRand_SetSeed，为随机数生成器设置种子。

3. 调用OH_CryptoRand_GenerateRandom，生成指定长度的安全随机数。指定字节长度范围为1~INT_MAX。

4. 调用OH_CryptoRand_GetAlgoName，获取随机数生成器使用的算法名称。
