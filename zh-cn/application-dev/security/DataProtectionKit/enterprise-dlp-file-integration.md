...
2. 输入指定的文件标签，查询当前已打开过的拥有该标签的DLP文件。
    ...
    2.2 设置企业DLP文件的查询选项DlpFileQueryOptions。当需要按分类标签筛选查询特定企业DLP文件时传入此参数，当需要查询所有企业DLP文件时可不传此参数。
...
3. 输入指定的文件标签，关闭当前正打开的拥有该标签的DLP文件。
    ...
    3.2 设置企业DLP文件的查询选项DlpFileQueryOptions。当需要按分类标签筛选关闭特定企业DLP文件时传入此参数，当需要关闭所有企业DLP文件时可不传此参数。
...
4. 实现SA调用的API。
    ...
    4.2 DlpConnPlugin被用于registerPlugin接口中，将回调能力注册到SA（System Ability）中。
...
5. 注册通信插件。
    ...
    5.2 DlpConnPlugin被用于registerPlugin接口中，将回调能力注册到SA中。
