...
1. 构造注册参数，需要传入[HUKS_EXT_CRYPTO_TAG_ABILITY_NAME/apis-universal-keystore-kit/js-apis-huksExternalCrypto.md#huksexternalcryptotag)。

2. 调用注册接口[registerProvider/apis-universal-keystore-kit/js-apis-huksExternalCrypto.md#huksexternalcryptoregisterprovider)。
...
2. 构造UI注册参数，需要传入[HUKS_EXT_CRYPTO_TAG_ABILITY_INFO/apis-universal-keystore-kit/js-apis-huksExternalCrypto.md#huksexternalcryptotag)，该值为JSON字符串，包含abilityName和index两个字段，其中abilityName为该UIExtensionAbility中module.json5中的name字段，其长度不得大于128字节，index值的长度不得大于512字节，其值应是resourceId，1个CryptoExtension最多注册10个UIExtensionAbility。

3. 调用注册接口[registerProvider/apis-universal-keystore-kit/js-apis-huksExternalCrypto.md#huksexternalcryptoregisterprovider)。
...
1. 构造注销参数，注销单个ability需要传入[HUKS_EXT_CRYPTO_TAG_ABILITY_NAME/apis-universal-keystore-kit/js-apis-huksExternalCrypto.md#huksexternalcryptotag)参数。批量注销不需要传入[HUKS_EXT_CRYPTO_TAG_ABILITY_NAME/apis-universal-keystore-kit/js-apis-huksExternalCrypto.md#huksexternalcryptotag)参数。

2. 调用注销接口[unregisterProvider/apis-universal-keystore-kit/js-apis-huksExternalCrypto.md#huksexternalcryptounregisterprovider)。
