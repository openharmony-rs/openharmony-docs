...
- **导出到应用：** 封装或解封装时不指定sharedKeyAlias和sharedKeyParams，共享密钥通过封装和解封装返回结果中的sharedSecret字段导出给应用，由应用自行管理共享密钥的存储和使用。此方式下，应用需自行确保共享密钥的安全存储。具体开发步骤请参考[密钥封装(ArkTS)](./huks-kem-arkts.md)中共享密钥不托管到HUKS的示例。
