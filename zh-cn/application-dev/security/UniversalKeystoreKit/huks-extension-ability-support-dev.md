...
5. 在CryptoAbility.ets文件中，增加导入CryptoExtensionAbility的依赖包，自定义类继承CryptoExtensionAbility组件并实现其中的接口函数。导入CryptoExtensionAbility需要实现在[CryptoExtensionAbility](../../reference/apis-universal-keystore-kit/js-apis-CryptoExtensionAbility.md)中给出的所有函数，此处给出实现参考，与底层驱动的调用对应关系见下文。
   ...
   > 2. 句柄资源和PIN认证状态需要做基于UID的隔离，在onOpenResource/onCloseResource/onAuthUkeyPin/onGetUkeyPinAuthState/onClearUkeyPinAuthState接口的入参params中会包含业务的UID信息（通过[HUKS_EXT_CRYPTO_TAG_UID](../../reference/apis-universal-keystore-kit/js-apis-huksExternalCrypto.md#huksexternalcryptotag)可获取业务身份），可基于此做句柄资源和PIN认证状态的隔离。
