...
首先，需继承CryptoExtensionAbility并完成相关的接口实现。其次，通过Provider注册接口[registerProvider/apis-universal-keystore-kit/js-apis-huksExternalCrypto.md#huksexternalcryptoregisterprovider)完成能力注册。最后，由HUKS和证书管理将对应的密钥管理扩展能力开放给应用。
...
2. 句柄管理，针对同一个Ukey资源（例如容器下的密钥），支持应用维度句柄资源管理，通过[onOpenResource/apis-universal-keystore-kit/js-apis-CryptoExtensionAbility.md#onopenresource)打开资源，通过[onCloseResource/apis-universal-keystore-kit/js-apis-CryptoExtensionAbility.md#oncloseresource)关闭资源。
...
3. 密钥会话管理，支持三段式密钥管理操作，单次签名验签需通过[onInitSession/apis-universal-keystore-kit/js-apis-CryptoExtensionAbility.md#oninitsession)/[onUpdateSession/apis-universal-keystore-kit/js-apis-CryptoExtensionAbility.md#onupdatesession)/[onFinishSession/apis-universal-keystore-kit/js-apis-CryptoExtensionAbility.md#onfinishsession)三个函数三步配合完成，需支持会话管理，缓存密钥会话状态。
