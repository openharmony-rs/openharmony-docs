...
2. 初始化密钥属性集。需要添加群组密钥标签OH_HUKS_TAG_KEY_ACCESS_GROUP、OH_HUKS_TAG_KEY_OVERRIDE，避免密钥被覆盖。
...
3. 调用OH_Huks_InitParamSet指定算法参数配置。需要添加群组密钥标签OH_HUKS_TAG_KEY_ACCESS_GROUP。
...
2. 调用OH_Huks_InitParamSet指定算法参数配置。需要添加群组密钥标签OH_HUKS_TAG_KEY_ACCESS_GROUP。

3. 调用OH_Huks_DeleteKeyItem删除密钥，具体请参考密钥删除。
...
2. 密钥生成时，指定参数OH_HUKS_TAG_KEY_ACCESS_GROUP，用于生成群组密钥。
...
1. 获取密钥别名，指定对应的属性参数HuksOptions，添加参数OH_HUKS_TAG_KEY_ACCESS_GROUP，用于派生群组密钥。

2. 调用OH_Huks_InitParamSet指定算法参数配置。需要添加群组密钥标签OH_HUKS_TAG_KEY_ACCESS_GROUP。

3. 调用OH_Huks_InitSession初始化密钥会话，并获取会话的句柄handle。

4. 调用OH_Huks_UpdateSession更新密钥会话。

5. 调用OH_Huks_FinishSession结束密钥会话，完成派生。
