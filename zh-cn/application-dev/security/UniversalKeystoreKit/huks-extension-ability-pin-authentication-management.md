...
1. 防暴力破解：连续错误输入达到一定次数（与密钥管理扩展服务实现相关）后自动锁定，通过onAuthUkeyPin返回值中的retryCount字段查询剩余重试次数。
...
- **查询认证状态**：从API版本22开始，可通过getUkeyPinAuthState查询当前PIN码认证状态。

- **清除认证状态**：从API版本26.0.0开始，可通过clearUkeyPinAuthState清除指定资源的PIN码认证状态。
<!--Del-->
      
- **PIN认证**：从API版本22开始，可通过authUkeyPin发起验证PIN码请求。
<!--DelEnd-->
...
2. 调用查询认证状态接口getUkeyPinAuthState验证PIN码。
...
2. 通过获取资源ID中的证书管理途径获取resourceId，并传入该resourceId进行打开资源，可参考打开关闭资源。

3. 调用OH_Huks_InitExternalCryptoParamSet指定参数配置。

4. 调用OH_Huks_GetUkeyPinAuthState获取PIN码认证状态。

5. 操作执行完后，关闭资源，可参考打开关闭资源。
...
应用在用户输入PIN码后，可调用对应接口完成PIN码认证。在发起PIN认证流程前，调用方需要先获取密钥管理扩展服务中的公钥用于加密用户输入的PIN码。调用getProperty接口并传入SKF_ExportPublicKey作为propertyId。

### 开发步骤

1. 通过获取资源ID操作，得到resourceId，并传入该resourceId进行打开资源，可参考打开关闭资源。

2. 调用getProperty并传入SKF_ExportPublicKey作为propertyId，导出用于PIN码加密传输的公钥。

3. 构造参数，必传HUKS_EXT_CRYPTO_TAG_UID和HUKS_EXT_CRYPTO_TAG_UKEY_PIN。

4. 调用接口authUkeyPin验证PIN码。
