...
签名/验签操作通过HUKS的三段式接口实现，通过指定HUKS_TAG_KEY_CLASS为HUKS_KEY_CLASS_EXTENSION，表示指定密钥由密钥管理扩展服务中的密钥。

具体使用可参考签名验签示例。

| 步骤 | 接口 | 说明 |
|------|------|------|
| 1 | initSession | 初始化密钥会话，返回会话handle。 |
| 2 | updateSession | （可选）传入分段数据，执行中间密码运算。 |
| 3 | finishSession | 结束会话，返回最终结果（签名/验签结果）。 |

> **说明：**
>
> 1. 通过HUKS_TAG_KEY_CLASS指定密钥类别为HUKS_KEY_CLASS_EXTENSION，表示该密钥由外部密钥管理扩展管理。
...
| 参数 | 取值 | 必填 | 说明 |
|------|------|------|------|
| HUKS_TAG_KEY_CLASS | `HUKS_KEY_CLASS_EXTENSION` | 是 | 表示由密钥管理扩展管理 |
| HUKS_TAG_PURPOSE | 用途枚举值 | 否 | 指定用途，未传入则使用密钥管理扩展提供的默认值 |
| HUKS_TAG_ALGORITHM | 算法枚举值 | 否 | 指定签名/验签算法 |
| HUKS_TAG_KEY_SIZE | 密钥长度 | 否 | 如 2048、3072、4096 |
...
1. 通过获取资源ID操作，得到resourceId，并传入该resourceId进行打开资源，可参考打开关闭资源。

2. 参考PIN码访问控制完成PIN认证。

3. 设置属性参数HuksOptions，properties传入算法参数配置。

4. 调用initSession初始化密钥会话，不传入inData，获取sessionHandle。

5. （可选）调用updateSession更新密钥会话，inData传入明文数据。

6. 调用finishSession结束会话，inData传入明文数据，从返回值的outData中获取signature。

7. 操作执行完后，关闭资源，可参考打开关闭资源。

**验签：**

1. 通过获取资源ID操作，得到resourceId，并传入该resourceId进行打开资源，可参考打开关闭资源。

2. 设置属性参数HuksOptions，properties传入算法参数配置。

3. 调用initSession初始化密钥会话，不传入inData，获取sessionHandle。

4. （可选）调用updateSession更新会话，inData传入明文数据。

5. 调用finishSession结束会话，inData传入签名。

6. 操作执行完后，关闭资源，可参考打开关闭资源。

**C++接口**

**签名：**

1. 通过证书管理系统能力提供的openAuthorizeDialog获取keyUri作为resourceId，并作为密钥别名，打开资源后完成PIN码认证，可参考打开关闭资源。

2. 指定待签名的明文数据。

3. 调用OH_Huks_InitParamSet指定算法参数配置，并指定KeyClass参数，tag为OH_HUKS_TAG_KEY_CLASS，值为OH_HUKS_KEY_CLASS_EXTENSION。
...
1. 通过证书管理系统能力提供的openAuthorizeDialog获取keyUri作为resourceId，并作为密钥别名，然后打开资源，可参考打开关闭资源。
