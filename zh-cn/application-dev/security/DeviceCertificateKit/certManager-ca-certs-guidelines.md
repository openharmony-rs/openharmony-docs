...
Device Certificate Kit的CA证书管理功能包含如下能力：
 - 系统CA证书管理：由操作系统预安装的CA证书，包括国密算法（SM算法）和国际算法（RSA和ECC算法）的CA证书。只有通过[《华为根证书计划》](https://www.huawei.com/cn/root-certification-program)审核的CA证书才能预安装到操作系统中。
...
|用户CA证书<br>的作用范围（Scope）   | 作用范围说明 | 证书管理方式 | 设备用户可执行的操作| 应用可执行的操作 | 典型应用场景|
|-----|-------------------|-------|-------------------------------|----|---|
...
> **说明**
>
> 本开发指导需使用API version 18及以上版本SDK。<br>
