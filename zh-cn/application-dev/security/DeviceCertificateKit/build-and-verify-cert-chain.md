...
从API版本26.0.0开始，证书链校验器提供证书链构建和校验能力，支持通过[CertValidationParams/apis-device-certificate-kit/js-apis-cert.md#certvalidationparams)参数配置构建和校验行为，包括信任锚设置、证书吊销检查、日期校验等。

## 开发步骤

1. 导入[@ohos.security.cert/apis-device-certificate-kit/js-apis-cert.md)。
   ```ts
   import { cert } from '@kit.DeviceCertificateKit';
   ```

2. 基于已有的证书数据，创建待校验[X509Cert/apis-device-certificate-kit/js-apis-cert.md#x509cert)证书对象。

3. 调用[cert.createCertChainValidator/apis-device-certificate-kit/js-apis-cert.md#certcreatecertchainvalidator)接口创建证书链校验器对象。

4. 创建[CertValidationParams/apis-device-certificate-kit/js-apis-cert.md#certvalidationparams)校验参数对象。

5. 调用[validateCert(cert: X509Cert, params: CertValidationParams): Promise&lt;CertValidationResult&gt;/apis-device-certificate-kit/js-apis-cert.md#validatecert)接口构建并校验证书链，返回校验成功的证书链[CertValidationResult/apis-device-certificate-kit/js-apis-cert.md#certvalidationresult)。

## 场景一：指定信任证书

当应用有自定义的信任锚证书时，可以通过[trustedCerts/apis-device-certificate-kit/js-apis-cert.md#certvalidationparams)参数指定信任的CA证书。
...
当应用需要验证互联网公开证书（如HTTPS网站证书）时，可以使用系统预置的CA证书作为信任锚。通过设置[trustSystemCa/apis-device-certificate-kit/js-apis-cert.md#certvalidationparams)为`true`，校验器会使用系统预置CA证书库构建并验证证书链。
...
可以通过[crls/apis-device-certificate-kit/js-apis-cert.md#x509certrevokedparams)参数指定CRL，用来检查证书是否被吊销。
...
国密SM2证书链校验，通常需要设置[userId/apis-device-certificate-kit/js-apis-cert.md#certvalidationparams)参数。
