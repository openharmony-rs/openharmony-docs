...
从API 23开始，支持证书链校验时允许下载缺失的中间证书。

以校验证书链为例，完成证书链对象的创建，校验过程允许下载缺失的中间证书。
...
2. 基于已有的证书数据，调用[cert.createX509CertChain](../../reference/apis-device-certificate-kit/js-apis-cert.md#certcreatex509certchain11-2)创建X509证书链对象，并返回结果。

3. 调用[x509CertChain.validate](../../reference/apis-device-certificate-kit/js-apis-cert.md#validate11)设置校验参数allowDownloadIntermediateCa为true，允许校验过程中从网络下载缺失的中间CA。
...
async function createX509CertChain(): Promise<cert.X509CertChain> {
  ...
  try {
    x509CertChain = cert.createX509CertChain([leaf]);
  } catch (error) {
    let e: BusinessError = error as BusinessError;
