...
签名工具仅支持以下2种签名算法：SHA256withECDSA、SHA384withECDSA（无其他可选）。
...
生成密钥对时，keystoreFile参数指定的密钥库文件已存在与keyAlias参数值同名的密钥。

**处理步骤**

检查keystoreFile参数指定的密钥库文件的条目信息。
<!--DelEnd-->
...
keyAlias参数指定的密钥算法与signAlg指定的签名算法不匹配。例如：keyAlias指定的密钥为RSA密钥，signAlg指定的签名算法为SHA256withECDSA。

**处理步骤**

检查keyAlias参数指定的密钥算法与signAlg指定的签名算法是否匹配。
...
待签名的HAP/HSP/HNP软件包不符合zip规范：① 大小超过4GB；② 条目数（文件+目录）超过65535。这两种情况会被打包为zip64格式，而签名工具不支持该格式。
