...
5. 日历账户创建之后，针对搭载HarmonyOS 7.0.0及以上版本的设备，日历账户颜色默认为黑色。针对搭载HarmonyOS 7.0.0以下版本的设备，日历账户颜色默认为蓝色。不指定日历账户颜色可能导致部分版本/设备深色模式下显示效果不佳。开发者需要调用setConfig()接口设置日历配置信息，包括是否打开日历账户下的日程提醒能力、设置日历账户颜色。
