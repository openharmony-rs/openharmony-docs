...
- 应用可以在“应用文件目录”下保存和处理自己的应用文件；系统文件及其目录对于应用是只读的；而应用若需访问[用户文件](user-file-overview.md)，则查看后文[用户文件](user-file-overview.md)的详细指导。
...
3. 三级目录el1/~el5/：不同文件加密类型。
    ...
    EL5(Encryption Level 5):
     ...
     - 默认情况下不会生成EL5的相关目录，应用若需要使用EL5目录，则需要配置访问E类加密数据库的权限。具体配置方法详见[E类加密数据库的使用](../database/encrypted_estore_guidelines.md)。
