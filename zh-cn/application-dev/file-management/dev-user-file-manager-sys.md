...
开发用户文件管理器的相关API详细介绍请参见[API参考/apis-core-file-kit/js-apis-fileAccess-sys.md)。
...
5. 操作文件或目录。
   ...
   开发者可以集成文件访问框架的接口，完成一些用户行为，比如删除文件（目录）、重命名文件（目录）、新建文件（目录）、移动文件（目录）等。以下示例展示了如何创建一个文件，其他接口请参见[API参考/apis-core-file-kit/js-apis-fileAccess-sys.md)。
...
开发设备上下线的相关API详细介绍请参见[API参考/apis-core-file-kit/js-apis-fileAccess-sys.md)。
...
4. 注册监听设备和取消设备监听。
   ...
   开发者可以根据提供的[DEVICES_URI/apis-core-file-kit/js-apis-fileAccess-sys.md#常量)，传入方法registerObserver()中，就能监听设备上下线状态。传入方法unregisterObserver()中，就能取消设备上线，下线状态。
