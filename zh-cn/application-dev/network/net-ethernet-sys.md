...
> **说明：**
> 为了保证应用的运行效率，大部分API调用都是异步的，对于异步调用的API均提供了callback和Promise两种方式，以下示例均采用promise函数，更多方式可以查阅[@ohos.net.ethernet (以太网连接管理)/apis-network-kit/js-apis-net-ethernet-sys.md)。
...
完整的JS API说明以及实例代码请参考：[以太网连接/apis-network-kit/js-apis-net-ethernet-sys.md)。
