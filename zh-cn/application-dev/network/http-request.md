...
2. 根证书可信性验证
   ...
   - 存储库来源包括：
     ...
     - 应用信任证书
     - 本次请求指定的CA证书
   - 可通过相关API指定应用级和请求级信任证书

3. 证书内容有效性验证
   - 证书有效期检查
   - 域名匹配验证（SAN或CN与访问域名一致）
   - 证书吊销状态检查（CRL/OCSP）
...
2. 配置请求级CA证书：
- 通过[httprequestoptions](../../js-apis-http.md#httprequestoptions)的caPath和caData字段配置HTTPS请求CA证书
