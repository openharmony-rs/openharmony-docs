...
> **说明：**
> 为了保证应用的运行效率，大部分 API 调用都是异步的，对于异步调用的 API 均提供了 callback 和 Promise 两种方式，以下示例均采用 Promise 函数，更多方式可以查阅[@ohos.net.statistics (流量管理)/apis-network-kit/js-apis-net-statistics.md)。

以下分别介绍具体开发方式。

## 开发步骤

1. 导入[statistics/apis-network-kit/js-apis-net-statistics.md)、[socket/apis-network-kit/js-apis-socket.md)以及错误码模块。
...
2. 获取指定网卡实时流量数据
    ...
    调用[getIfaceRxBytes/apis-network-kit/js-apis-net-statistics.md#statisticsgetifacerxbytes-1)接口传入网卡名获取实时下行流量数据。
...
3. 获取蜂窝实时流量数据
    ...
    调用[getCellularRxBytes/apis-network-kit/js-apis-net-statistics.md#statisticsgetcellularrxbytes-1)接口获取蜂窝实时上下行流量数据。
...
4. 获取所有网卡实时流量数据
    ...
    调用[getAllRxBytes/apis-network-kit/js-apis-net-statistics.md#statisticsgetallrxbytes-1)接口获取所有网卡实时上下行流量数据。
...
5. 获取指定应用实时流量数据
    ...
    调用[getUidRxBytes/apis-network-kit/js-apis-net-statistics.md#statisticsgetuidrxbytes-1)接口，传入UID获取指定应用实时上下行流量数据。
   ```ts
    let UID = 20010038;
   ```
...
6. 获取指定socket实时流量数据
    ...
    调用[getSockfdRxBytes/apis-network-kit/js-apis-net-statistics.md#statisticsgetsockfdrxbytes11-1)接口，传入指定的sockFd获取指定socket实时上下行流量数据。
