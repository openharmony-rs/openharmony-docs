...
目前系统模板仅支持进度条模板，通知模板[NotificationTemplate/apis-notification-kit/js-apis-inner-notification-notificationTemplate.md)中的data参数为用户自定义数据，用于显示与模块相关的数据。

## 接口说明

[`isSupportTemplate()`/apis-notification-kit/js-apis-notificationManager.md#notificationmanagerissupporttemplate)是查询是否支持对应的通知模板，目前仅支持进度条模板。
