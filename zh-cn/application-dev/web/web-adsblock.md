...
```
ArkTS-Sta示例：
<!-- @[app_select_list_rule_file_for_ad_filter](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkWeb-Sta/ManageWebCompSecPriv/entry/src/main/ets/pages/EnablingAdsBlocking.ets) -->

``` TypeScript
import { Button, Column, State, Component, Flex, Row, Entry, Web } from '@kit.ArkUI';
...
struct WebComponent {
  ...
  build() {
    Column() {
      Row() {
        Flex() {
          Button('setAdsBlockRules')
            .onClick(() => {
              try {
                let documentSelectionOptions: picker.DocumentSelectOptions = {};
                let documentPicker: picker.DocumentViewPicker = {};
                documentPicker.select(documentSelectionOptions).then((documentSelectResult: Array<string>) => {
                  if (documentSelectResult && documentSelectResult.length > 0) {
                    let fileRealPath = new fileUri.FileUri(documentSelectResult[0]);
                    console.info('DocumentViewPicker.select successfully, uri: ' + fileRealPath);
                    webview.AdsBlockManager.setAdsBlockRules(fileRealPath.path!, true);
...
```
ArkTS-Sta示例：
<!-- @[turn_off_ad_filtering_for_specific_domain_pages](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkWeb-Sta/ManageWebCompSecPriv/entry/src/main/ets/pages/DisAdsBlockSpecDomPages_one.ets) -->

``` TypeScript
import { Button, ButtonType, Color, Column, Component, Entry, Flex, Row, State, Text, TextInput, TextInputController, Web } from '@kit.ArkUI';
import { webview } from '@kit.ArkWeb';
...
```
ArkTS-Sta示例：
<!-- @[set_up_page_level_ad_filtering_switch](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkWeb-Sta/ManageWebCompSecPriv/entry/src/main/ets/pages/DisAdsBlockSpecDomPages_two.ets) -->
...
```
ArkTS-Sta示例：
<!-- @[collect_information_about_ad_filtering](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkWeb-Sta/ManageWebCompSecPriv/entry/src/main/ets/pages/CollectingAdsBlockingInformation.ets) -->

``` TypeScript
import { Column, Component, Entry, State, Web } from '@kit.ArkUI';
import { AdsBlockedDetails } from '@ohos.arkui.component';
import { webview } from '@kit.ArkWeb';
    ...
    Column() {
      Web({ src: 'https://www.example.com', controller: this.controller })
        .onAdsBlocked((details: AdsBlockedDetails) => {
          if (details) {
            console.info(' Blocked ' + details.adsBlocked.length + ' in ' + details.url);
            let adList: Array<string> = Array.from(new Set<string>(details.adsBlocked));
            this.totalAdsBlockCounts += adList.length;
            console.info('Total blocked counts :' + this.totalAdsBlockCounts);
          }
        })
