...
2. 建议配置[overScrollMode](../reference/apis-arkweb/arkts-basic-components-web-attributes.md#overscrollmode11)（过滚动模式）为关闭状态。当过滚动模式开启时，用户在Web界面上滑动到边缘时，Web会通过弹性动画弹回界面，会与Scroll组件的回弹相互冲突，影响体验。
...
import { $rawfile, Entry, Component, Navigation, Text, Web, Column, RenderMode, WebLayoutMode, OverScrollMode, Scroll, Scroller } from '@kit.ArkUI';
import { webview } from '@kit.ArkWeb';
...
CSS样式`height：<number> vh`和Web组件大小自适应页面布局存在计算冲突，请检查`height：<number> vh`是由body节点以内的第一个高度CSS样式。如以下结构，id为2的DOM节点高度将为0。
