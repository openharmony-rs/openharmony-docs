...
跨域错误、ServiceWorker请求失败一般会报ERR_FAILED，如果出现ERR_FAILED需要通过使用DevTools工具调试前端页面定位具体报错类型（跨域、资源404、JS异常）。
1. 检查控制台，确认是否存在因MixedContent策略或CORS策略导致的异常，或JS错误等。可参考解决Web组件本地资源跨域问题。为了提高安全性，ArkWeb内核禁止file协议和resource协议访问跨域请求。因此，在使用Web组件加载本地离线资源的时候，Web组件会拦截file协议和resource协议的跨域访问。Web组件无法访问本地跨域资源时，DevTools控制台会显示报错信息：
    ```txt
...
* 排查putAcceptThirdPartyCookieEnabled是否使能。若未使能，需设置putAcceptThirdPartyCookieEnabled为true，使webCookieManager实例允许发送和接收第三方Cookie。Cookie每30s周期性保存到磁盘中，设置Cookie后，30s内退出应用可能会导致Cookie没有及时落盘而丢失。此时可以使用接口saveCookieAsync进行强制落盘（PC/2in1和Tablet设备不会持久化session cookie，即使调用saveCookieAsync，也不会将session cookie写入磁盘）。