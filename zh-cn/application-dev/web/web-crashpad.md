...
2. 参考[Native访问应用沙箱](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-file-native-side)实现访问应用沙箱dmp文件。还有另一种获取dmp文件的方法，因为存放dmp文件的沙箱路径无法在DevEco Studio的设备文件浏览器直接查看，可以考虑将其移动到可以查看的路径，demo如下
  ...
  ```
可以搜索hilog日志“.dmp”得到dmp文件名，这样只需知道文件名，就可以将dmp文件移动到另一个沙箱路径下了，具体的路径为

  ```c
  /data/app/el2/100/base/应用包名/haps/entry/files/
  ```

而这个路径可以利用DevEco Studio查看。
