...
1. 当接收到创建NativeMessaging连接时，先通过[get](../reference/apis-arkdata/js-apis-data-dataShare.md#get)获取目标应用的extension配置。然后读取WebNativeMessagingExtensionAbility名称和允许访问的扩展列表。最后校验是否允许访问。
