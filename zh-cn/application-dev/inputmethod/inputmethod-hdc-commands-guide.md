...
Ime为开发者提供在开发者模式下管理系统输入法应用的能力，包括启用、禁用输入法，切换输入法等。

> **说明：**
>
> Ime工具从API version 20开始支持。
> Ime工具在防作弊模式下切换输入法命令无效。
...
| -e | bundle [-b /-f] | 启用指定输入法到指定模式。未设置-b/-f选项时，默认-b为基础模式，-f为完整体验模式。<br/>**注意：** 系统预置的默认输入法不支持通过此命令更改其使能状态。 |
