...
2. 当用户点击子窗中的输入框组件时，可以先通过setWindowFocusable将子窗设置为可获焦，然后通过shiftAppWindowFocus将焦点窗口从主窗切换为子窗，即可在子窗的输入框中正常使用输入法。
...
3. 当用户重新点击子窗中的非输入框组件时，可以通过setWindowFocusable将子窗重新设置为不可获焦，焦点窗口即可恢复至主窗。
