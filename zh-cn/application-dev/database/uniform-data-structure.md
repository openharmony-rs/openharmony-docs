...
7. 遍历每条记录，判断该记录的数据类型，转换为子类对象并得到原数据记录。
    ...
    **示例：**
    ...
    // 3. 创建文件URI数据类型记录，将其添加到刚才创建的UnifiedData对象。
    ...
    let hyperlinkRecord =
      new unifiedDataChannel.UnifiedRecord(uniformTypeDescriptor.UniformDataType.HYPERLINK, hyperlink);
    let fileUriRecord =
      new unifiedDataChannel.UnifiedRecord(uniformTypeDescriptor.UniformDataType.FILE_URI, fileUri);

    // 5. 添加hyperlink和fileUri数据记录。
    unifiedData.addRecord(hyperlinkRecord);
    unifiedData.addRecord(fileUriRecord);
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (unifiedDataRecord.getType() == uniformTypeDescriptor.UniformDataType.FILE_URI.toString()) {
        ...
        if (fileUriRead != undefined) {
          hilog.info(0xFF00, '[Sample_Udmf]',
            `fileUriRead.uniformDataType: ${fileUriRead.uniformDataType} + , fileUri.uniformDataType: ${fileUri.uniformDataType}`);
        }
      }
    }
    ```
