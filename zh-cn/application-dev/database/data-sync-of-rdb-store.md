...
- 分布式表：支持组网内多设备间数据同步的数据库表。来自其他设备的数据将同步至本地，并通过与设备ID关联的表名进行存储。默认使用设备协同表模式，即每个设备中同步的数据存储在独立的表中，不会直接写入本地表。eg.设备A同步至设备B，设备B创建分布式表名为`table_deviceA`，同步数据写入设备B分布式表中；设备B同步至设备A，设备A创建分布式表名为`table_deviceB`，同步数据写入设备A分布式表中。
