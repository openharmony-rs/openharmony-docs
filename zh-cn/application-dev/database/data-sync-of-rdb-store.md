...
5. 创建关系型数据库，创建数据表，并将需要进行跨设备同步的数据表设置为分布式表。需要将[DistributedConfig](../reference/apis-arkdata/arkts-apis-data-relationalStore-i.md#distributedconfig10)中[DistributedTableType](../reference/apis-arkdata/arkts-apis-data-relationalStore-e.md#distributedtabletype23)配置为SINGLE_VERSION。
...
6. 创建关系型数据库，创建数据表，并将需要进行跨设备同步的数据表设置为分布式表，单版本表模式。需要将[DistributedConfig](../reference/apis-arkdata/arkts-apis-data-relationalStore-i.md#distributedconfig10)中[DistributedTableType](../reference/apis-arkdata/arkts-apis-data-relationalStore-e.md#distributedtabletype23)配置为SINGLE_VERSION。
   <!--@[setSingleDistributedTables](https://gitcode.com/openharmony/applications_app_samples/blob/master/code/DocsSample/ArkData/RelationalStore/DataSyncAndPersistence/entry/src/main/ets/pages/datasync/RdbDataSync.ets)--> 
...
10. 当数据未完成同步，或未触发数据同步时，可使用RdbStore的[remoteQuery](../reference/apis-arkdata/arkts-apis-data-relationalStore-RdbStore.md#remotequery-1)方法查询组网内指定设备上分布式表中的数据。
