...
**设备协同表模式**
- 分布式表设备协同表模式，同步数据时，本端新增一个分布式表，表名为原有表名前拼接对端设备的DeviceID标识符，对端同步过来的数据写入改分布式表中，不会写入本地表，这样能保证每个设备产生的数据严格隔离。数据以设备的维度管理，不存在冲突；支持按照设备的维度查询数据。
- 底层按照设备的维度管理数据，设备协同模式支持以设备的维度查询分布式数据，但是不支持修改远端设备同步过来的数据。
