...
针对[UTD标准化数据类型/apis-arkdata/capi-utd-h.md)中的部分常见类型，为了方便业务使用，提供了标准化数据结构。例如，系统定义的桌面图标类型（标准化数据类型标识为'OH_UdsAppItem'）明确定义了相关描述信息。
...
详细的接口说明请参考[标准化数据结构相关接口/apis-arkdata/capi-uds-h.md)。
