...
以[FileUri](../reference/apis-arkdata/js-apis-data-uniformDataStruct.md#fileUri)、[HTML](../reference/apis-arkdata/js-apis-data-uniformDataStruct.md#html)、[PixelMap](../reference/apis-arkdata/js-apis-data-uniformDataStruct.md#pixelmap15)三种数据进行多对多数据共享的过程为例说明开发步骤，数据提供方可以通过UDMF提供的insertData接口将数据写入公共数据通路，获取到的返回值（生成的数据的唯一标识符），可用于对其插入的数据进行更新和删除操作。数据访问方则可以通过UDMF提供的查询接口获取当前公共数据通路的全量数据。
      ...
      try {
        unifiedDataChannel.insertData(options, unifiedData, (err, key) => {
          ...
          if (error === undefined) {
            ...
            hilog.error(0xFF00, '[Sample_Udmf]', `Failed to insert data. code is ${error.code},message is ${error.message}`);
          }
        });
      } catch (e) {
...
3. 更新上一步插入的统一数据对象。
    ...
    let fileUri : uniformDataStruct.FileUri = {
      uniformDataType: "general.file-uri",
      oriUri: 'file://data/1.png',
      fileType: 'general.png'
    };
    let recordUpdate =
      new unifiedDataChannel.UnifiedRecord(uniformTypeDescriptor.UniformDataType.FILE_URI, fileUri);
    ...
    try {
      unifiedDataChannel.updateData(optionsUpdate, unifiedDataUpdate, (err) => {
        ...
        if (error === undefined) {
          ...
          hilog.error(0xFF00, '[Sample_Udmf]', `Failed to update data. code is ${error.code},message is ${error.message}`);
        }
      });
    } catch (e) {
      let error: BusinessError = e as BusinessError;
    ...
    try {
      unifiedDataChannel.deleteData(optionsDelete, (err, data) => {
        ...
        if (error === undefined) {
          for (let i = 0; i < data!.length; i++) {
            ...
            for (let j = 0; j < records.length; j++) {
              ...
              if (types.includes(uniformTypeDescriptor.UniformDataType.FILE_URI)) {
                let text =
                  records[j].getEntry(uniformTypeDescriptor.UniformDataType.FILE_URI) as uniformDataStruct.FileUri;
                hilog.info(0xFF00, '[Sample_Udmf]', `${i + 1}.${text.oriUri}`);
              }
          ...
          hilog.error(0xFF00, '[Sample_Udmf]', `Failed to delete data. code is ${error.code},message is ${error.message}`);
        }
      });
    } catch (e) {
      let error: BusinessError = e as BusinessError;
    ...
    try {
      unifiedDataChannel.queryData(options, (err, data) => {
        ...
        if (error === undefined) {
          ...
          hilog.error(0xFF00, '[Sample_Udmf]', `Failed to query data. code is ${error.code},message is ${error.message}`);
        }
      });
    } catch (e) {
      let error: BusinessError = e as BusinessError;
