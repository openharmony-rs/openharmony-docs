...
IPC/RPC通信模型中的客户端侧远程对象代理，由元能力连接服务接口返回，用于向远端Stub读写数据和发起请求。Proxy不可在本设备内二次跨进程传递，也不可把跨设备的Proxy回传到其指向的Stub所在设备。

## R

### Remote Procedure Call (RPC)；远程过程调用

基于DSoftBus实现的跨设备进程间通信机制，采用与IPC相同的客户端-服务端模型和Proxy-Stub对应关系。典型使用场景为多端协同，提供跨设备远端接口调用与数据传递能力。与IPC的关键区别在于通信范围跨设备而非设备内，底层驱动为DSoftBus而非Binder。

## S

### Stub

IPC/RPC通信模型中的服务端侧对象，接收并处理Proxy发起的请求并应答结果。Stub定义具体的请求处理逻辑，Proxy实现对应的请求发起方法。