...
1. 新建一个ArkTS应用工程，编辑工程中的“entry > src > main > ets  > entryability > EntryAbility.ets”文件，导入所需的依赖模块：
   ...
   ```
   ArkTS-Sta：
   <!-- @[AppEvent_Crash_Click_ArkTS_Header](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/ets/entryability/EntryAbility.ets) -->
...
2. 编辑工程中的“entry > src > main > ets  > entryability > EntryAbility.ets” 文件，在onCreate函数中添加对崩溃事件、按钮点击事件的订阅。
   ...
   ```
   ArkTS-Sta：
   <!-- @[AppEvent_Crash_ArkTS_Add_Watcher](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/ets/entryability/EntryAbility.ets) -->
   ...
   ```
   ArkTS-Sta：
   <!-- @[AppEvent_Click_ArkTS_Add_Watcher](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/ets/entryability/EntryAbility.ets) -->
...
3. 编辑工程中的“entry > src > main > ets  > pages > Index.ets” 文件，导入依赖模块：
   ...
   ```
   ArkTS-Sta：
   <!-- @[EventSub_Header](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/ets/pages/Index.ets) -->
   
   ``` TypeScript
   import { hiAppEvent } from '@kit.PerformanceAnalysisKit';
   import { ButtonType, Row } from '@ohos.arkui.component';
...
4. 编辑工程中的“entry > src > main > ets  > pages > Index.ets” 文件，新增“WatchAppCrash ArkTS&C++”按钮触发崩溃事件；新增“writeEvent ArkTS”按钮，在按钮点击的函数中进行事件打点。示例代码如下：
   ...
   ```
   ArkTS-Sta：
   <!-- @[AppEvent_Click_ArkTS_Button](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/ets/pages/Index.ets) -->
   ...
   Button('writeEvent ArkTS')
     ...
     .onClick(() => {
       // 在按钮点击函数中进行事件打点，以记录按钮点击事件
       let eventParams: Record<string, int> = { 'clickTime': 100 };
       let eventInfo: hiAppEvent.AppEventInfo = {
         // 事件领域定义
         domain: 'button',
         // 事件名称定义
       ...
       hiAppEvent.write(eventInfo).then(() => {
         ...
         hilog.error(0x0000, 'testTag',
           `AppEvents HiAppEvent err.code: ${bErr.code}, err.message: ${bErr.message}`);
       });
     })
   ```
