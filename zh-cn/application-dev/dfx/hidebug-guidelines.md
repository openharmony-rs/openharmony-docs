...
| hidebug.getRssInfo | 获取应用进程使用的物理内存信息（与hidebug.getAppNativeMemInfo相比，该接口开销小更加轻量）。读取/proc/{pid}/status节点的数据。<br/>**说明**：从API version 24开始，支持该接口。 |
...
| OH_HiDebug_GetRssInfo | 获取应用进程使用的物理内存信息。读取/proc/{pid}/status节点的数据。<br/>**说明**：从API version 24开始，支持该接口。 |
