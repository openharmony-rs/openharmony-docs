...
| OH_HiCollie_SetFreezeCallback | 将冻屏回调设置进系统，系统将在冻屏发生时回调此函数。 |
| OH_HiCollie_AssociateProcessReport | 主动报告一个进程的卡死事件。此时会生成[APP_HICOLLIE](hiappevent-watcher-apphicollie-events.md)事件。 |
...
7. 在DevEco Studio的底部，切换到“Log”窗口，过滤自定义的LOG_TAG。
   ...
   （5）先点击“testHiCollieSetFreezeCallback”按钮，再持续点击“testHiCollieAssociateProcessReport”按钮。

   此时窗口将显示通过OH_HiCollie_SetFreezeCallback接口，设置自定义日志函数，再通过OH_HiCollie_AssociateProcessReport接口上报自定义卡死事件