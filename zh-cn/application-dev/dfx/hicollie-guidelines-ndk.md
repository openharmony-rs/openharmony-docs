...
| OH_HiCollie_AssociateProcessReport | 主动报告一个进程的卡死事件。此时会生成[APP_HICOLLIE](hiappevent-watcher-apphicollie-events.md)事件。<br/>**说明**：从API version 24开始，支持该接口。 |
...
3. 编辑“napi_init.cpp”文件，导入依赖的文件，定义LOG_TAG，下述代码步骤用于模拟卡死卡顿场景，具体使用请结合业务需要。示例代码如下：
   ...
   （5）**应用线程卡死检测，三方框架生成自定义日志**：OH_HiCollie_SetFreezeCallback，示例代码如下：
...
4. 将TestHiCollieNdk注册为ArkTS接口。
   ...
   ```
    ...
    （5）OH_HiCollie_SetFreezeCallbac、OH_HiCollie_AssociateProcessReport示例，编辑“index.d.ts”文件，定义ArkTS接口：
...
7. 在DevEco Studio的底部，切换到“Log”窗口，过滤自定义的LOG_TAG。
   ...
   （5）先点击“testHiCollieSetFreezeCallback”按钮，再点击“testHiCollieAssociateProcessReport”按钮。

   此时窗口将显示通过OH_HiCollie_SetFreezeCallback接口，设置自定义日志函数，再通过OH_HiCollie_AssociateProcessReport接口上报[APP_HICOLLIE](hiappevent-watcher-apphicollie-events.md)事件，其中包含回调日志字段。