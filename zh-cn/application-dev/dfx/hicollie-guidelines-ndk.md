...
| OH_HiCollie_SetFreezeCallback | 将冻屏回调设置为系统，系统将在冻屏发生时回调此函数。 |
| OH_HiCollie_AssociateProcessReport | 报告一个从非主应用进程的事件卡死。此时会生成APP_HICOLLIE事件，调用者进程可能不会被杀死。 |
...
3. 编辑“napi_init.cpp”文件，导入依赖的文件，定义LOG_TAG，下述代码步骤用于模拟卡死卡顿场景，具体使用请结合业务需要。示例代码如下：
   ...
   （4）**应用线程卡死检测，应用主动上报输入无响应故障**：OH_HiCollie_ReportInputBlock，示例代码如下：
   ...
   （5）**应用线程卡死检测，三方框架生成字定义日志**：OH_HiCollie_SetFreezeCallback，示例代码如下：
   ...
   static napi_value TestSetFreezeCallback(napi_env env, napi_callback_info info)
   {
      // 设置用户回调
   ...
   {
       napi_property_descriptor desc[] = {
           { "testSetFreezeCallback", nullptr, TestSetFreezeCallback, nullptr, nullptr, nullptr, napi_default, nullptr }
       };
       napi_define_properties(env, exports, sizeof(desc) / sizeof(desc[0]), desc);
       return exports;
   }
   ...
   ```
   （6）**应用线程卡死检测，三方框架上报自定义freeze事件**：OH_HiCollie_AssociateProcessReport，示例代码如下：
   ...
   {
       napi_property_descriptor desc[] = {
           { "testAssociateProcessReport", nullptr, TestAssociateProcessReport, nullptr, nullptr, nullptr, napi_default, nullptr }
       };
       napi_define_properties(env, exports, sizeof(desc) / sizeof(desc[0]), desc);
       return exports;
   }
...
4. 将TestHiCollieNdk注册为ArkTS接口。
   ...
   ```
    ...
    （4）OH_HiCollie_ReportInputBlock示例，编辑“index.d.ts”文件，定义ArkTS接口：
   ...
   ```
    ...
    （5）OH_HiCollie_AssociateProcessReport示例，编辑“index.d.ts”文件，定义ArkTS接口：
...
5. 编辑“Index.ets”文件：
   ...
   ```
    ...
    （4）添加点击事件，触发OH_HiCollie_Init_StuckDetectionWithTimeout方法和OH_HiCollie_ReportInputBlock方法。
   ...
   ```
    （5）添加点击事件，触发OH_HiCollie_SetFreezeCallback方法和OH_HiCollie_AssociateProcessReport方法。
   ...
   Column() {
     Button("testHiCollieSetFreezeCallback", { stateEffect:true, type: ButtonType.Capsule})
       ...
       .fontSize(20)
       .fontWeight(FontWeight.Bold)
       .onClick(testNapi.testSetFreezeCallback);
...
7. 在DevEco Studio的底部，切换到“Log”窗口，过滤自定义的LOG_TAG。
   ...
   （4）先点击“testHiCollieStuckNdk”按钮，再持续点击“testHiCollieInputBlock”按钮。
   ...
   （5）先点击“testSetFreezeCallback”按钮，再持续点击“testHiCollieAssociateProcessReport”按钮。

   此时窗口将显示通过testHiCollieAssociateProcessReport接口，设置自定义日志函数，再上报自定义卡死事件