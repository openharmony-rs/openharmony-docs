...
2. 通过HiAppEvent订阅APP_KILLED事件。
    ...
    | IllegalAudioCapturerBySuspend      | 应用录音未申请合理的后台任务，其退至到后台后仍进行录音。           | 应用退至后台时，应避免不必要的后台录音，或者合理使用后台任务。                                                                        | 是             | 否        |
    ...
    | JsHeapSleepCleanKill               | sleep状态（夜间充电）进行js heapsize检测，超限后查杀。                     | 归类为整机后台内存清理，尝试降优化应用js heap的使用率。                                                                                         | 否             | 否        |
    ...
    | CPUHighload                        | 应用后台CPU高负载，系统自动杀死。                                            | 尝试降低应用自身的CPU负载。                                                                                        | 否             | 否        |
    ...
    | DmaKiller                          | 整机低内存，单进程Dma占用达到阈值。尝试通过HiAppEvent订阅RESOURCE_OVERLIMIT获取更多的线程泄漏日志。                             | 尝试降低应用自身的Dma内存占用。尝试通过HiAppEvent订阅RESOURCE_OVERLIMIT获取更多的Dma占用达到阈值日志。                        | 是             | 是        |
