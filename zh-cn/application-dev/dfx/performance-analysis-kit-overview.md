...
- 性能跟踪与分布式调用链跟踪，提供开发者通过进行性能追踪和跨线程、跨进程的分布式跟踪能力。
...
- 错误管理与应用恢复，提供开发者捕获应用运行时异常能力，以及应用非预期退出后启动状态和数据的自恢复能力。

- 应用灰度采集，提供开发者应用灰度采集能力，可以选取少量设备打开全量日志采集获取更详细的故障日志，帮助开发者更好的定位故障。

- 调试命令提供包含hdc、hilog、hidumper、hitrace、hiperf等工具用于调试系统和应用。
...
- 提供接口（HiDebug、HiAppEvent、HiLog），可用于自建端侧APM（Application Performance Management，应用性能管理平台）SDK，与厂商自研APM对接。

- 基于HiAppEvent系统事件和HiAppEvent应用事件，订阅应用运行过程中所触发的事件（如崩溃、应用冻屏），快捷地记录、收集运维和运营所需的事件信息。
