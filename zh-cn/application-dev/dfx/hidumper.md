...
| <!--RP13-->[--mem-heap pid ARG [--leakobj]](#查询虚拟机堆内存) | 导出指定类型的内存快照信息。ARG用于指定快照类型，当前仅有--native一种类型。<br />指定--leakobj，只触发打印可疑内存泄漏点不做快照导出，仅配合--native参数使用。<br />**说明**：从API版本26.0.0开始，支持--leakobj参数。<!--RP13End--> |
