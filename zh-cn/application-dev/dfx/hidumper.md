...
| --mem-jsheap pid [-T tid] [--gc] [--leakobj] [--clean] | pid 必选参数。命令触发所有线程gc和快照导出。如果指定线程的tid，只触发该线程gc和快照导出；如果指定--gc，只触发gc不做快照导出；如果指定--leakobj，则获取泄露对象的列表。 <br />指定--clean时，快照导出后触发清理nodeId节点信息。<br />**说明**：<br />从API version 24开始，支持--clean参数，只支持ArkTS动态语言。 |
