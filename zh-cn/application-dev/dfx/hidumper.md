...
| [--mem-heap pid ARG [--leakobj] [--raw] [-T tid]](#查询虚拟机堆内存) | 导出指定类型的内存快照信息。ARG用于指定快照类型，对于--native一种类型。<br />如果指定--leakobj，只触发打印可疑内存泄漏点不做快照导出，仅配合--native参数使用。<br />--jsvm类型，可以-T导出指定线程虚拟机快照，可以--raw导出raw heap类型快照**说明**：从API版本26.0.0开始，支持该参数。 |
...
- 可使用hidumper --mem-heap pid --jsvm命令导出指定进程的jsvm堆内存快照文件，文件命名为：<!--RP9-->jsvmheap-进程号-JS线程号-时间戳，如果有多个JS线程会生成多个文件。<!--RP9End-->
...
- 可使用hidumper --mem-heap pid --jsvm [-T tid] --raw命令导出指定进程的jsvm堆内存快照文件，生成的堆快照文件为rawheap格式，文件命名为：<!--RP9-->jsvmheap-进程号-JS线程号-时间戳.rawheap，如果有多个JS线程会生成多个文件<!--RP9End-->
