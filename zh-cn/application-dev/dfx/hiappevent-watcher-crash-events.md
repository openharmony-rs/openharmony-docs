...
| caught_by_errormanager | boolean | 是否注册异常管理捕获。true：已注册异常管理捕获；false：未注册异常管理捕获。<br>**说明**：从API version 26开始，支持该字段。 |
| uncatchable_fault | boolean | 是否为不可捕获的异常类型，当前仅包含OutOfMemoryError。true：不可捕获的异常类型；false：可捕获的异常类型。<br>**说明**：从API version 26开始，支持该字段。 |
