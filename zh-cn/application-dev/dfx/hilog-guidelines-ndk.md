...
HiLog中定义了DEBUG、INFO、WARN、ERROR、FATAL五种日志级别，并提供了对应的方法输出不同级别的日志，接口如下表所示，具体说明可查阅[log.h/apis-performance-analysis-kit/capi-log-h.md)。
...
- level：用于指定日志级别。取值见[LogLevel/apis-performance-analysis-kit/capi-log-h.md#loglevel)。

- prefer：用于指定偏好策略。取值见[PreferStrategy/apis-performance-analysis-kit/capi-log-h.md#preferstrategy)。
