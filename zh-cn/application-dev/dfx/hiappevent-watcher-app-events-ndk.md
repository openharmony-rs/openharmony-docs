...
2. 编辑“CMakeLists.txt”文件，添加所需的源文件和动态库。
   ...
   target_link_libraries(entry PUBLIC libhiappevent_ndk.z.so libhilog_ndk.z.so)
   # 新增三方库依赖libjsoncpp.so(解析订阅事件中的json字符串)
   target_link_libraries(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/jsoncpp/${OHOS_ARCH}/lib/libjsoncpp.so)
   target_include_directories(entry PRIVATE ${DEST_DIR}/jsoncpp-1.9.6/include/json)
   ```
...
3. 编辑“napi_init.cpp”文件，导入依赖的文件并定义LOG_TAG：
   ...
   ```
   ArkTS-Sta：
   <!-- @[EventSub_napi_Header](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/cpp/ani_init.cpp) -->
...
1. 订阅事件。分别使用OnReceive类型观察者、OnTrigger类型观察者的订阅方式。
   - 订阅崩溃事件（系统事件），采用OnReceive类型观察者的订阅方式，观察者接收到事件后会立即触发OnReceive()回调。定义OnReceive类型观察者相关方法：
    ...
    编辑“ani_init.cpp”文件。
    <!-- @[AppEvent_Crash_C++_Add_Watcher](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/cpp/ani_init.cpp) -->
   ...
   - 订阅按钮点击事件（应用事件），采用OnTrigger类型观察者的订阅方式。需满足OH_HiAppEvent_SetTriggerCondition()设置的条件，才能触发OnTrigger()回调。定义OnTrigger类型观察者相关方法：
    ...
    编辑“ani_init.cpp”文件。
    <!-- @[AppEvent_Click_C++_Add_Watcher](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/cpp/ani_init.cpp) -->
...
2. 添加按钮点击事件的打点接口：
   ...
   编辑“ani_init.cpp”文件。
   <!-- @[AppEvent_Click_C++_WriteAppEvent](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/cpp/ani_init.cpp) -->  
...
3. 注册RegisterWatcherCrash()(订阅崩溃事件)、RegisterWatcherClick()（订阅按钮点击事件）、WriteAppEvent()(按钮点击事件打点接口)为ArkTS接口：
   ...
   编辑“ani_init.cpp”文件。
   <!-- @[AppEvent_C++_Init](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/cpp/ani_init.cpp) -->    
...
4. 声明ArkTS接口：
   ...
   编辑“EntryAbility.ets”文件。
   <!-- @[AppEvent_Declare_C_Function](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/ets/entryability/EntryAbility.ets) -->
   ...
   ```
   编辑“Index.ets”文件。
   <!-- @[AppEvent_Declare_C_Function](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/ets/pages/Index.ets) -->
   
   ``` TypeScript
   class Calc {
     native writeAppEvent(): void;
   }
   ```
...
5. 编辑“EntryAbility.ets”文件，在onCreate()函数中添加接口调用：
   ...
   ```
   ArkTS-Sta：
   <!-- @[AppEvent_Call_Capi_Function](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/ets/entryability/EntryAbility.ets) -->
...
```
ArkTS-Sta：
<!-- @[AppEvent_Crash_Button](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/ets/pages/Index.ets) -->
...
Button('WatchAppCrash ArkTS&C++')
  ...
  .height('5%')
  .onClick(() => {
    // 在按钮点击函数中构造一个crash事件，触发事件回调
    throw new Error('This is a jscrash triggering test.');
  })
```
<!-- @[AppEvent_CPP_Button](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/ets/pages/Index.ets) -->
...
Button('writeEvent C++')
  ...
  .onClick((e: ClickEvent) => {
        hilog.info(0x0000, 'HiAppEvent', 'Click writeEvent C++ Button');
        try {
          loadLibrary("entry")
        } catch (e) {
          hilog.error(0x0000, 'HiAppEvent', 'loadFailed when click writeEvent C++', e.message);
        }

        let cls = new Calc();
        let result = cls.writeAppEvent();
  })
```
...
4. 移除应用的事件观察者：
   ...
   ```
   ArkTS-Sta：
   <!-- @[AppEvent_C++_RemoveWatcher](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/cpp/ani_init.cpp) -->
...
5. 销毁应用的事件观察者：
   ...
   ```
   ArkTS-Sta：
   <!-- @[AppEvent_C++_DestroyWatcher](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/PerformanceAnalysisKit/HiAppEvent/EventSub_Sta/entry/src/main/cpp/ani_init.cpp) -->    
