...
随着分布式协同场景的发展，设备间的协同唤醒需求日益增长。OpenHarmony 基于分布式软总线，提供设备感知能力（softbusBase），支持系统应用通过自定义负载广播唤醒周边协同设备，并通过扫描发现周边感知设备。
...
| 接口名 | 功能描述 |
| --- | --- |
| startPerceptionAdv(type: PerceptionType, customData?: ArrayBuffer) | 启动感知广播或更新当前所有者的自定义负载。 |
| setPerceptionAdvHighFreq(type: PerceptionType, customData?: ArrayBuffer) | 将活跃感知广播切换为高频 10 秒，可同时更新自定义负载。 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| deviceType | int | 设备类型，具体值以系统定义为准。 |
| deviceId | ArrayBuffer | 设备 ID，最大长度 6 字节，网络字节序（大端）。 |
| customData | ArrayBuffer | 广播携带的自定义数据，最大长度 5 字节。 |

## 设备感知开发指导

- 广播端调用 startPerceptionAdv() 携带自定义负载启动广播；如需短时高频，调用 setPerceptionAdvHighFreq()。
