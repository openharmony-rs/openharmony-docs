...
## List/Grid并行加载子节点的方案和注意事项(API 22)

在应用开发中，当List/Grid的列表结构复杂、每个列表项包含组件较多时，会因嵌套层级较深导致组件负载加重、绘制耗时增长，而List/Grid并行加载子节点的方案本质是基于分帧的实现——其核心是为解决转场或列表滑动时，列表项需一次性加载大量数据的性能问题，通过将一帧内原本要加载的全部数据拆分到多帧逐步加载来优化

List/Grid并行加载子节点的注意事项：分帧渲染需要开发者手动计算每帧加载的数据量，操作较为复杂，仅推荐在必要场景下使用；同时需注意，若该分帧方案的帧间插入大任务（即主线程存在长时任务占用），会直接导致List/Grid的item显示时间延后，进而造成页面整体完成时间的推迟。
