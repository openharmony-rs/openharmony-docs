...
## List/Grid并行化创建子组件的方案和注意事项(API 22)

在应用开发中，当List&Grid的列表结构复杂、每个列表项包含组件较多时，会因嵌套层级较深导致组件负载加重、绘制耗时增长。
List&Grid并行化创建子组件的方案本质是基于分帧并行的实现——其核心是为解决转场或列表滑动时，列表项需一次性创建全部组件的性能问题，通过将主线程内原本要创建的全部组件拆分到子线程并行创建来优化。

List&Grid并行化创建子组件的注意事项：List&Grid使用并行化加载子组件时，子组件在子线程完成创建后，需要在主线程完成挂载及显示。应避免在此过程中在主线程插入大任务，否则会直接导致List&Grid的子组件显示时间延后，进而造成页面整体完成时间的推迟。
