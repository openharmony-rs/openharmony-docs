...
1. 创建OE文档：客户端创建OE文档有三种方式：基于OEID创建OE文档，基于文件创建OE文档，基于OE格式文件加载OE文档。
2. 创建客户端OE对象：客户端基于OE文档创建客户端OE对象，实现OE文档与客户端OE对象的关联，并用于与服务端通信。创建客户端OE对象完成后需注册该对象回调以响应OE服务端通知。
3. 启动OE Extension：在嵌入或者编辑OE文档时，OE客户端应用需通过调用OE框架层接口，通知OE服务端启动OE Extension组件。
4. 注册OE Extension回调：当OE服务端应用的OE Extension组件启动后，需在OE Extension组件的入口函数中注册OE Extension回调，以响应客户端请求。
5. 注册服务端OE对象回调：当OE服务端应用的OE Extension组件启动后，客户端OE对象将会与服务端OE对象绑定，此时需注册服务端OE对象回调函数，以响应OE客户端的请求。
