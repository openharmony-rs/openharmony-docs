...
若需基于本Demo适配自有模型，请优先选择静态Shape模型。由于ArkTS暂不支持动态Shape，如确有相关需求，请参考[使用MindSpore Lite实现图像分类（C/C++）](mindspore-guidelines-based-native.md)，通过Native侧的[OH_AI_ModelResizeapis-mindspore-lite-kit/capi-model-h.md#oh_ai_modelresize)接口对模型inputs进行动态调整。
