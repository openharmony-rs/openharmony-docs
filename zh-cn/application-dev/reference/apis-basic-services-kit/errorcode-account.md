...
该错误码表示在指定的逻辑屏（displayId）上未找到前台运行的系统账号，可能原因如下。
1. 指定的逻辑屏上没有前台运行的系统账号（未激活或未切换至前台）。
2. 指定的displayId无效或对应的逻辑屏不存在。
3. 指定的localId无效或对应的系统账号不存在。

**处理步骤**

请确认传入的displayId/localId是否正确并存在；需确保逻辑屏上已有账号并处于前台，再重试。
...
该错误码表示指定的逻辑屏ID无法被识别或不存在，可能原因如下。
1. 指定的displayId无效或对应的逻辑屏不存在。
2. 目标逻辑屏未连接或已断开。

**处理步骤**

请确认displayId是否正确，检查逻辑屏设备是否已连接并可用；使用有效的逻辑屏ID重试。
...
该错误码表示目标设备或当前系统配置不支持在不同逻辑屏之间激活账号，可能原因如下：

当前系统策略或配置限制了跨逻辑屏激活操作。

**处理步骤**

请在同一逻辑屏上进行激活操作，或在指定逻辑屏上退出账号后再激活。
...
1. 系统账号认证时，认证总数超过5个。
2. 应用账号认证时，三方应用的认证器服务忙碌（根据实际情况决定）。
...
目标服务器配置信息已成功添加，请勿重复操作。

## 12300214 服务器配置信息已与账号绑定
...
服务器配置信息已与账号绑定，无法操作服务器配置信息。

**处理步骤**

请更换服务器配置信息或删除绑定的账号后再进行操作。

## 12300215 服务器配置信息数量已达上限
...
当前设备上的服务器配置信息数量已达到预设的上限，无法增加新的服务器配置信息。
