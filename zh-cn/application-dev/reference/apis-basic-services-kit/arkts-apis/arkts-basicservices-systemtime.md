...
| 名称 | 说明 |
| --- | --- |
| getCurrentTime | 获取自Unix纪元以来经过的时间，使用callback异步回调。 |
...
| getDate | 获取当前系统日期，使用callback异步回调。 |
| [getDate](arkts-basicservices-systemtime-getdate-f.md#getdate-1) | 获取当前系统日期，使用Promise异步回调。 |
| getRealActiveTime | 获取自系统启动以来经过的时间，不包括深度睡眠时间，使用callback异步回调。 |
...
| getRealTime | 获取自系统启动以来经过的时间，包括深度睡眠时间，使用callback异步回调。 |
