...
| 名称   | 类型     | 只读 | 可选 | 说明                            |
|------|--------|----|----|-------------------------------|
| headers | Record\<string, string\> | 否  | 是 | 缓存下载任务在HTTP传输时使用的请求头。<br/>**ArkTS-Dyn起始版本：**  18 <br/>**ArkTS-Sta起始版本：**  23 |
...
| 名称   | 类型     | 只读 | 可选 | 说明                            |
|------|--------|----|----|-------------------------------|
| size | ArkTS-Dyn: number <br/>ArkTS-Sta: long | 是  | 否  | 预下载资源解压后的大小。当值为正整数时表示资源下载成功，-1表示下载失败。 |
...
| message     | string         | 是  | 否  | 返回[通用错误码](../../reference/errorcode-universal.md)的错误描述或[HTTP错误码](../../reference/apis-network-kit/errorcode-net-http.md)的错误描述。 |
...
| callback | Callback&lt;void&gt; | 是 | 回调函数。返回为空 |
