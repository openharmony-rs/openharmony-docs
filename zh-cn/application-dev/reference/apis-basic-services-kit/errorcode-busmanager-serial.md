...
>
> 本文档提供串口通信接口调用的错误码说明，帮助开发者快速定位和解决串口通信问题。适用于开发串口通信应用、调试串口通信异常等场景。以下仅介绍本模块特有错误码，通用错误码请参考通用错误码说明文档。
...
**状态转换说明：**
- 调用open()后，串口处于打开状态，可以调用onDataRead、write等接口。onDataRead需与offDataRead配对调用，onDisconnect需与offDisconnect配对调用。在close()前应先调用offDataRead和offDisconnect取消订阅。
