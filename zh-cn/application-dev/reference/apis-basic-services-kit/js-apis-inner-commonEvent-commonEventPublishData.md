...
| code                  | number               | 否  | 是  | 表示发布方传递的公共事件数据（number类型）。默认值为0。<br>**原子化服务API（仅ArkTS-Dyn）：** 从API version 11开始，该接口支持在原子化服务中使用。       |
...
| parameters            | {[key: string]: any} | 否  | 是  | 表示发布方传递的公共事件的附加信息。<br>**原子化服务API（仅ArkTS-Dyn）：** 从API version 11开始，该接口支持在原子化服务中使用。       |