...
| 名称                  | 类型                 | 只读 | 可选 | 说明                         |
| --------------------- | -------------------- | ---- | ---- | ---------------------------- |
| bundleName            | string               | 否  | 是  | 表示订阅者包名，用于限定将公共事件发布给指定包名的订阅者。<br>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。 |
...
| subscriberPermissions | Array\<string>       | 否  | 是  | 表示订阅者的权限，只有具备该权限的订阅者才能收到该公共事件。<br>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。             |
...
| parameters            | {[key: string]: any} | 否  | 是  | 表示发布方传递的公共事件的附加信息，以键值对形式携带自定义参数。<br>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。       |
