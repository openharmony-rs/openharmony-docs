...
2. 若数据内容不支持复制（如只读类型数据），不对此类数据内容进行复制操作。如DLP只读数据，参见[数据防泄漏服务简介](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides-V5/dlp-overview-V5)
...
用系统默认进度显示（当未自定义进度显示时，系统会自动使用默认进度条），创建进度显示失败时，系统会报此错误码。
