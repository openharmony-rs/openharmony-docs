...
| deviceId  | string   | 是   | 串口设备ID，用于指定需要授权访问的串口设备。可通过接口[getSerialPortList](./js-apis-busmanager-serial.md#serialgetserialportlist)获取串口设备列表获得。板载串口取值为portName；USB虚拟串口取值为VID+PID+SN的组合或设备路径（如/dev/ttyUSB0）。设置后，应用将获得对指定串口设备的访问权限。 |
