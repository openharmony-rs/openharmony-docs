...
从给定的未压缩字节序列初始化解压缩字典。使用Promise异步回调。
...
这个函数相当于先调用inflateEnd再调用inflateInit，但是并不会释放和重新分配内部解压缩状态。使用Promise异步回调。
