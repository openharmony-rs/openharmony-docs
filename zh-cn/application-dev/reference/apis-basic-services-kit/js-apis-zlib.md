...
| ERRNO        <sup>22+</sup>| -1 | 函数调用失败，表示文件操作错误。|
| STREAM_ERROR <sup>22+</sup>| -2 | 函数调用失败，表示压缩或解压缩流错误。|
| DATA_ERROR   <sup>22+</sup>| -3 | 函数调用失败，表示输入数据不正确。|
| MEM_ERROR    <sup>22+</sup>| -4 | 函数调用失败，表示内存分配失败。|
| BUF_ERROR    <sup>22+</sup>| -5 | 函数调用失败，表示输入缓冲区不正确。|
