...
| ERRNO        | -1 | 函数调用失败，表示文件操作错误。**起始版本：** 22 |
| STREAM_ERROR | -2 | 函数调用失败，表示压缩或解压缩流错误。**起始版本：** 22 |
| DATA_ERROR   | -3 | 函数调用失败，表示输入数据不正确。**起始版本：** 22 |
| MEM_ERROR    | -4 | 函数调用失败，表示内存分配失败。**起始版本：** 22 |
| BUF_ERROR    | -5 | 函数调用失败，表示输入缓冲区不正确。**起始版本：** 22 |
