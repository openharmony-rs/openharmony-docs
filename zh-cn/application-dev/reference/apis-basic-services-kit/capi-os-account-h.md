...
| 类型 | 说明 |
| -- | -- |
| [OsAccount_ErrCode](capi-os-account-common-h.md#osaccount_errcode) | OS_ACCOUNT_ERR_OK：表示成功；<br>         OS_ACCOUNT_ERR_PERMISSION_DENIED：表示权限被拒绝；<br>         OS_ACCOUNT_ERR_INTERNAL_ERROR：表示内部错误；<br>         OS_ACCOUNT_ERR_INVALID_PARAMETER：表示name为空指针或名称长度（包括结束符'\0'）大于name_size；<br>         OS_ACCOUNT_ERR_ACCOUNT_NOT_FOUND：表示未找到账号；<br>         OS_ACCOUNT_ERR_RESTRICTED_ACCOUNT：表示账号受限，无法查询； |
