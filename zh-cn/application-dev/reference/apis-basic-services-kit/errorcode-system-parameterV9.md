# 系统参数错误码
<!--Kit: Basic Services Kit-->
<!--Subsystem: Startup-->
<!--Owner: @chenjinxiang3-->
<!--Designer: @chenjinxiang3-->
<!--Tester: @liuhaonan2-->
<!--Adviser: @fang-jinxu-->

系统参数错误码用于标识系统参数操作中的错误情况，包括参数查找失败、值无效、权限不足、系统内部错误等场景。通过明确的错误码和描述，帮助开发者快速定位和解决问题。

> **说明：**
>
> 以下仅介绍本模块特有错误码，通用错误码请参考通用错误码说明文档。

## 14700101 系统参数查找失败

**错误信息**

System parameter not found.

**错误描述**

内存中没有设置key值，系统会报此错误码。

**可能原因**

参数没有设置，或设置失败。

**处理步骤**

设置合法的参数。关于参数的验证机制，请参见系统参数。

## 14700102 系统参数值无效

**错误信息**

Invalid system parameter value.

**错误描述**

当系统参数value值为空字符、非法字符或长度超出范围时，系统会报此错误码。关于参数值的验证机制，请参见系统参数。

**可能原因**

接口的参数值超出设置范围或者含有特殊字符，比如，"const.param.xxx"。

**处理步骤**

修改value值为合法字符串。关于参数值的验证机制，请参见系统参数。

## 14700103 系统操作因权限被拒绝

**错误信息**

The operation on the system permission is denied.

**错误描述**

系统参数没有DAC或MAC权限时，系统会报此错误码。关于权限配置的实现机制和方法，请参见系统参数。

**可能原因**

没有配置DAC或MAC权限。

**处理步骤**

添加相应的DAC或MAC权限。关于权限配置的实现机制和方法，请参见系统参数。

## 14700104 系统内部错误，包括内存不足，死锁等

**错误信息**

System internal error such as out memory or deadlock.

**错误描述**

当试图修改const属性参数（赋值后不可更改的参数）、socket连接失败、内存拷贝失败等错误时，系统会报此错误码。关于const属性参数的设计机制，请参见系统参数。

**可能原因**

当socket连接异常，添加节点或获取节点失败，试图修改const属性参数，或内存拷贝失败。

**处理步骤**

1. 内存不足，需要分析整个进程的内存占用情况，是否有内存泄漏的情况。
2. 死锁，多出现在多线程场景下，需要通过错误堆栈查看是否有死锁的场景。