...
| fail | (data: any, code: number) => void | 否 | 接口调用失败的回调函数，在接口调用失败时执行。data 为错误信息，code 为错误码。code:200，表示返回结果中存在无法获得的信息，可能原因：设备信息获取权限不足或部分设备信息不可用。处理步骤：请检查应用是否具有获取设备信息的权限，或使用其他接口获取设备信息。不传入时无法处理错误情况，建议设置此回调。|
