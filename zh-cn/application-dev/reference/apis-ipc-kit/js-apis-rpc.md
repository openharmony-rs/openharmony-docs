...
> ArkTS-Dyn不支持函数重写，无法同时重写带有CallingInfo和不带CallingInfo的onRemoteMessageRequest方法。
...
class TestRemoteObject extends rpc.RemoteObject {
  ...
  onRemoteMessageRequest(code: int, data: rpc.MessageSequence, reply: rpc.MessageSequence,
    ...
    if (code === 1) {
      hilog.info(0x0000, 'testTag', 'Succeed in processing a synchronous remote message request');
      let pid = callingInfo.callerPid;
      return true;
    } else {
      hilog.error(0x0000, 'testTag', 'Failed to process the remote message request, unknown request code: ' + code);
      return false;
    }
  }
}
...
class TestRemoteObject extends rpc.RemoteObject {
  ...
  async onRemoteMessageRequest(code: int, data: rpc.MessageSequence, reply: rpc.MessageSequence,
    option: rpc.MessageOption, callingInfo: rpc.CallingInfo): Promise<boolean> {
    if (code === 1) {
      hilog.info(0x0000, 'testTag', 'Succeed in processing an asynchronous remote message request');
      let pid = callingInfo?.callerPid;
    } else {
      hilog.error(0x0000, 'testTag', 'Failed to process the remote message request, unknown request code: ' + code);
      return false;
    }
    await new Promise<int>((resolve: (data: int) => void) => {
        setTimeout(() => {
          resolve(100);
        }, 100);
      })
...
ArkTS-Sta示例：
```ts
// 同时重写带有CallingInfo和不带CallingInfo的onRemoteMessageRequest方法同步处理请求
import rpc from '@ohos.rpc';
import { hilog } from '@kit.PerformanceAnalysisKit';
...
ArkTS-Sta示例：
```ts
// 同时重写带有CallingInfo和不带CallingInfo的onRemoteMessageRequest方法异步处理请求
import rpc from '@ohos.rpc';
import { hilog } from '@kit.PerformanceAnalysisKit';
