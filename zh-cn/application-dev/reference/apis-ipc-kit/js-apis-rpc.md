...
| 名称    | 类型            | 可读 | 可写 | 说明                                  |
| ------- | --------------- | ---- | ---- |-------------------------------------- |
| errCode | ArkTS-Dyn: number<br>ArkTS-Sta: int          | 是   | 否   | 错误码。                              |
| code    | ArkTS-Dyn: number<br>ArkTS-Sta: int          | 是   | 否   | 消息代码。                            |
...
**参数：**
  ...
  | 参数名    | 类型   | 必填 | 说明                                          |
  | --------- | ------ | ---- | --------------------------------------------- |
  | syncFlags | ArkTS-Dyn: number<br>ArkTS-Sta: int | 否   | 同步调用或异步调用标志。默认同步调用。        |
...
**参数：**
  ...
  | callingInfo | [CallingInfo](#callinginfo23)      | 否   | 获取调用方PID、UID等IPC上下文信息。                  |
...
**同时重载带有CallingInfo的onRemoteMessageRequest方法和onRemoteRequest方法异步处理请求示例：**
...
**参数：**
  ...
  | size   | ArkTS-Dyn: number<br>ArkTS-Sta: int | 是   | Ashmem的大小，以字节为单位。 |
