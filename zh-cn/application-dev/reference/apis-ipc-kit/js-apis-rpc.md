...
**参数：**
  ...
  | --------- | --- | ---- | --------------------------------------------- |
  | syncFlags | int | 是   | 同步调用或异步调用标志，同步调用标志：0；异步调用标志：1。默认同步调用。|
  | waitTime  | int | 是   | 调用rpc最长等待时间（单位：秒）。默认TF_WAIT_TIME。|
...
**返回值：**
  ...
  | 类型   | 说明              |
  | ------ | ----------------- |
  | ArkTS-Dyn: number<br>ArkTS-Sta: int | rpc最长等待时间（单位：秒）。默认TF_WAIT_TIME。|
...
**参数：**
  ...
  | 参数名   | 类型   | 必填 | 说明                  |
  | -------- | ------ | ---- | --------------------- |
  | waitTime | ArkTS-Dyn: number<br>ArkTS-Sta: int | 是   | rpc调用最长等待时间（单位：秒），上限为3000秒。|
