...
**返回值：**
  ...
  | 类型  | 说明  |
  | ----- | ----- |
  | ArkTS-Sta: [OnRemoteDiedFunc](#onremotediedfunc) | 接收到远程对象的死亡通知时执行后续操作。 |
...
getLocalInterface(interfaceDe: string): IRemoteBroker
...
**参数：**
  ...
  | 参数名    | 类型   | 必填 | 说明                   |
  | --------- | ------ | ---- | ---------------------- |
  | interfaceDe | string | 是   | 需要查询的接口描述符。 |
...
**参数：**
  ...
  | --------- | --- | ---- | --------------------------------------------- |
  | syncFlags | int | 是   | 同步调用或异步调用标志，同步调用标志：0；异步调用标志：1。默认同步调用。|
  | waitTime  | int | 是   | 调用rpc最长等待时间。默认TF_WAIT_TIME。|
...
onRemoteMessageRequest(code: int, data: MessageSequence, reply: MessageSequence, options: MessageOption, callingInfo: CallingInfo): boolean
