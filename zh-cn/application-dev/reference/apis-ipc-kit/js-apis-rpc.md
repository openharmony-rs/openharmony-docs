...
ArkTS-Dyn: onRemoteDied(): void

ArkTS-Sta: onRemoteDied: OnRemoteDiedFunc
...
**返回值：**
  ...
  | 类型  | 说明  |
  | ----- | ----- |
  | ArkTS-Sta: [OnRemoteDiedFunc](#onremotediedfunc23) | 接收到远程对象的死亡通知时执行后续操作。 |
...
**参数：**
  ...
  | waitTime  | number | 否   | 调用rpc最长等待时间。默认TF_WAIT_TIME，调用rpc最长等待时间（单位：秒）。|
...
**参数：**
  ...
  | --------- | --- | ---- | --------------------------------------------- |
  | syncFlags | int | 是   | 同步调用或异步调用标志，同步调用标志：0；异步调用标志：1。默认同步调用。|
  | waitTime  | int | 是   | 调用rpc最长等待时间。默认TF_WAIT_TIME，调用rpc最长等待时间（单位：秒）。|
...
ArkTS-Sta: onRemoteMessageRequest(code: int, data: MessageSequence, reply: MessageSequence, options: MessageOption, callingInfo?: CallingInfo): boolean | Promise\<boolean>
