...
abilityConnectionManager模块提供了应用协同接口管理能力。设备组网成功（需登录同账号、双端打开蓝牙）后，系统应用和三方应用可以跨设备拉起同应用的一个 [UIAbility]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_，拉起并连接成功后可实现跨设备数据传输（文本信息）。
