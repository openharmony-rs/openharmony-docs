...
| 名称 | 类型   | 只读 | 可选 | 说明                                     |
| ---- | ------ | ---- | ---- | ---------------------------------------- |
| hookId   | ArkTS-Dyn: number<br/>ArkTS-Sta: long | 否   | 是   | 段落对应的hook ID，每个主要段落的标识。 |
| chapterId   | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否   | 是   | 段落对应的chapter ID，每个子章节的标识。 |
...
| 名称 | 类型   | 只读 | 可选 | 说明                                     |
| ---- | ------ | ---- | ---- | ---------------------------------------- |
| windowId   | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否   | 是   | 需要获取内容的窗口ID，不赋值或赋值undefined则默认获取全屏窗口。 |
...
| 名称 | 类型   | 只读 | 可选 | 说明                                     |
| ---- | ------ | ---- | ---- | ---------------------------------------- |
| windowId   | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否   | 否   | 获取到的屏上内容的窗口ID |
| sessionId   | ArkTS-Dyn: number<br/>ArkTS-Sta: long | 否   | 否   | 此次调用该接口的session ID，标识当次调用动作。 |
...
| 名称 | 类型   | 只读 | 可选 | 说明                                     |
| ---- | ------ | ---- | ---- | ---------------------------------------- |
| windowId   | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否   | 否   | 控制事件要操作的窗口的window ID。 |
| sessionId   | ArkTS-Dyn: number<br/>ArkTS-Sta: long | 否   | 否   | 控制事件要操作的session ID。控制事件要操作的hook ID和该次会话对应的session ID都由某次会话获取的[PageContent](#pagecontent)提供。 |
| eventType    | [EventType](#eventtype) | 否   | 否   | 控制事件类型。 |
| hookId    | ArkTS-Dyn: number<br/>ArkTS-Sta: long | 否   | 是   | 控制事件对应的hook ID。控制事件要操作的hook ID和该次会话对应的session ID都由某次会话获取的[PageContent](#pagecontent)提供。 |
