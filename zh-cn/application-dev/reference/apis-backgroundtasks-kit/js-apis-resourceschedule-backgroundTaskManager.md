...
申请长时任务，一个UIAbility（FA模型则为ServiceAbility）下支持通过本接口申请多个长时任务，使用Promise异步回调。通过本接口申请长时任务时，支持与已存在的长时任务合并通知，具体请参考[ContinuousTaskRequest](#continuoustaskrequest21)。</br>同一时间最多可存在10个长时任务，长时任务申请成功后，会有通知栏消息，没有提示音。</br>如果申请多类型长时任务，其他包含上传下载类型，则发送2个长时任务通知，上传下载类型单独一个通知，其他类型合发一个通知，返回的通知Id为上传下载类型通知用于更新，任意一个通知被移除时，长时任务取消。
...
3. 3. 如果待更新的长时任务或指定的更新类型中均包含数据传输类型，直接返回失败。
