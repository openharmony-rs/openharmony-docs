...
```ts
  ...
  backgroundLoader.getTaskInfo(taskId).then((taskInfo: backgroundLoader.TaskInfo) => {
    ...
    console.error(`workschedulerLog getTaskInfo failed. code is ${e.code} message is ${e.message}`);
  })
``` 
...
应用须实现后台加载任务[常量](#常量)的主业务回调方法。业务加载完成后，须在异步回调或独立线程中调用finishTask方法，以通知系统任务已结束。
...
```ts
  ...
  // 02.定义后台加载的onStop回调方法
  function BackgroundPrefetchOnStop(pdata: rpc.MessageSequence): rpc.Parcelable {
    let taskStopInfo: backgroundLoader.TaskStopInfo() = {
        taskId: pdata.readInt(),
        abilityName: pdata.readString(),
        stopCode: pdata.readInt(),
...
| 名称             | 类型                                | 只读   | 可选   | 说明               |
| --------------- | --------------------------------- | ---- | ---- | ---------------- |
| taskId          | ArkTs-Dyn: number <br> ArkTs-Sta: int        | 否    | 否    |应用指定标识任务ID。          |
