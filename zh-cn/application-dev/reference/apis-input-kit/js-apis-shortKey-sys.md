...
ArkTS-Dyn：setKeyDownDuration(businessKey: string, delay: number): Promise&lt;void&gt;

ArkTS-Sta：setKeyDownDuration(businessKey: string, delay: int): Promise&lt;void&gt;
...
| distanceX | ArkTS-Dyn: number<br/>ArkTS-Sta: double      | 否    |  否   | 相对于侧边指纹器件短轴偏移量（正数表示向右移动，负数表示向左移动），单位为px。 |
| distanceY | ArkTS-Dyn: number<br/>ArkTS-Sta: double     | 否    |  否   | 相对于侧边指纹器件长轴偏移量（正数表示向上移动，负数表示向下移动），单位为px。 |