...
查询最近的触屏输入事件，最多支持查询100条事件，从API version 26.0.0开始，最多支持查询60条事件，使用Promise异步回调。
