...
| screenX        | number      | 否    | 否    | 该鼠标事件以指定屏幕左上角为原点的相对坐标系的X坐标。当前仅支持整数，单位为px。 |
| screenY        | number      | 否    | 否    | 该鼠标事件以指定屏幕左上角为原点的相对坐标系的Y坐标。当前仅支持整数，单位为px。 |
| windowX        | number      | 否    | 否    | 鼠标所在窗口左上角为原点的相对坐标系的X坐标。当前仅支持整数，单位为px。 |
| windowY        | number      | 否    | 否    | 鼠标所在窗口左上角为原点的相对坐标系的Y坐标。当前仅支持整数，单位为px。|
| rawDeltaX      | number      | 否    | 否    | 鼠标当前事件相对于上次事件的X坐标偏移值。当前仅支持整数，单位为px。 |
| rawDeltaY      | number      | 否    | 否    | 鼠标当前事件相对于上次事件的Y坐标偏移值。当前仅支持整数，单位为px。 |
