...
使用该接口取消某个事件订阅后，已通过[emit](arkts-basicservices-emitter-emit-f.md#emit-3)接口发布但尚未被执行的事件将被取消。
...
取消事件ID为eventId且回调处理函数为callback的订阅。仅当已使用[on](arkts-basicservices-emitter-on-f.md#on-1)或
[once](arkts-basicservices-emitter-once-f.md#once-1)接口订阅callback时，该接口才生效。
...
取消事件ID为eventId且回调处理函数为callback的订阅。仅当已使用[on](arkts-basicservices-emitter-on-f.md#on-2)或
[once](arkts-basicservices-emitter-once-f.md#once-2)接口订阅callback时，该接口才生效。
...
[on](arkts-basicservices-emitter-on-f.md#on-3)或
[once](arkts-basicservices-emitter-once-f.md#once-3)接口订阅callback时，该接口才生效。
