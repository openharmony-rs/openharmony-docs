...
/*
 ...
 能入口（如翻译、搜索等）；主面板（MAIN_PANEL）为二级面板，在用户点击菜单面板中的功能按钮后弹出，展示具体的功能结果。本模块提供划词面板的属性信息和类型，开发者可通过[PanelInfo](arkts-basicservices-selectioninput-selectionpanel-panelinfo-i-sys.md#panelinfo系统接口)设定
 面板的位置和尺寸，通过[PanelType](arkts-basicservices-selectioninput-selectionpanel-paneltype-e-sys.md#paneltype系统接口)指定面板类型。划词面板的创建与显示接口请参见
