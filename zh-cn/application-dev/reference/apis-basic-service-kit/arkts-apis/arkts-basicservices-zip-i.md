...
| dest | ArrayBuffer | 是 | 目标缓冲区。 |
| source | ArrayBuffer | 是 | 源数据缓冲区。 |
| level | CompressLevel | 是 | 参考[CompressLevel枚举定义](arkts-basicservices-compresslevel-e.md)。 |
| sourceLen | number | 否 | 源数据长度。默认值为0。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| strm | ZStream | 是 | 参考[ZStream定义](arkts-basicservices-zstream-i.md)。 |
| flush | CompressFlushMode | 是 | 参考[CompressFlushMode定义](arkts-basicservices-compressflushmode-e.md)。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| source | Zip | 是 | 参考[Zip定义](arkts-basicservices-zip-i.md)。 |
...
| strm | ZStream | 是 | 参考[ZStream定义](arkts-basicservices-zstream-i.md)。 |
| level | CompressLevel | 是 | 参考[CompressLevel枚举定义](arkts-basicservices-compresslevel-e.md)。 |
| method | CompressMethod | 是 | 参考[CompressMethod枚举定义](arkts-basicservices-compressmethod-e.md)。 |
| windowBits | number | 是 | 控制内存窗口的大小，并指定数据的格式（zlib、gzip、raw deflate）。取值如下：<br />zlib格式：[1, 15]。<br />gzip格式：大于15。<br />raw deflate格式：[-15, -1]。 |
| memLevel | MemLevel | 是 | 参考[MemLevel枚举定义](arkts-basicservices-memlevel-e.md)。 |
| strategy | CompressStrategy | 是 | 参考[CompressStrategy枚举定义](arkts-basicservices-compressstrategy-e.md)。 |
