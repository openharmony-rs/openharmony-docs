...
查询当前DLP沙箱的权限信息。仅支持在DLP沙箱应用中调用。使用Promise方式异步返回结果。
...
监听打开DLP文件。仅支持在非DLP沙箱应用中调用。在当前应用的沙箱应用打开DLP文件时，通知当前应用。
...
取消监听打开DLP文件。仅支持在非DLP沙箱应用中调用。在当前应用的沙箱应用打开DLP文件时，取消通知当前应用。
...
打开DLP文件时自动安装沙箱，关闭DLP文件时自动卸载沙箱。设置沙箱保留状态时DLP文件关闭时自动卸载暂时失效。仅支持在DLP沙箱应用中调用。使用Promise方式异步返回结果。
...
打开DLP文件时自动安装沙箱，关闭DLP文件时自动卸载沙箱。设置沙箱保留状态时DLP文件关闭时自动卸载暂时失效。仅支持在DLP沙箱应用中调用。使用callback方式异步返回结果。
...
查询最近访问的DLP文件列表。仅支持在非DLP沙箱应用中调用。使用Promise方式异步返回结果。
...
查询当前系统是否提供加密保护特性，仅支持企业设备且需[MDM](../../mdm/mdm-kit-intro.md)配置使能。使用Promise方式异步返回结果。
...
获取DLPFile管理对象，仅支持企业账号调用。使用Promise异步回调。
