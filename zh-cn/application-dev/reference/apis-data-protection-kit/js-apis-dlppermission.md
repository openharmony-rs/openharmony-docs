...
根据文件的fd，查询该文件是否是DLP文件。调用成功后返回查询结果，true表示是DLP文件，false表示非DLP文件。使用Promise方式异步返回结果。**使用场景：** 在文件处理流程中，需要先判断文件是否为DLP文件，再决定后续处理策略（如是否需要通过DLP沙箱打开）。
...
根据文件的fd，查询该文件是否是DLP文件。调用成功后返回查询结果，true表示是DLP文件，false表示非DLP文件。使用callback方式异步返回结果。**使用场景：** 在文件处理流程中，需要先判断文件是否为DLP文件，再决定后续处理策略（如是否需要通过DLP沙箱打开）。
...
查询当前DLP沙箱的权限信息。仅支持在DLP沙箱应用中调用。返回的权限信息包括文件的授权类型和可执行的操作权限(如查看、编辑、复制等)。**使用场景:** 在DLP沙箱中处理文件时,可根据权限信息判断当前用户可以执行哪些操作,避免调用无权限的功能。使用Promise方式异步返回结果。
...
查询当前DLP沙箱的权限信息。返回的权限信息包括文件的授权类型和可执行的操作权限(如查看、编辑、复制等)。使用callback方式异步返回结果。**使用场景:** 在DLP沙箱中处理文件时,可根据权限信息判断当前用户可以执行哪些操作,避免调用无权限的功能。
...
获取指定DLP文件名的原始文件名。该接口为同步接口。**使用场景:** 根据原始文件名后缀判断文件类型，选择对应的应用打开。
...
获取DLP文件扩展名。调用成功后返回DLP文件扩展名（如'.dlp'）。接口为同步接口。**使用场景:** 用于获取DLP文件的标准扩展名，便于构建DLP文件名或进行文件类型判断。
...
监听打开DLP文件。调用成功后，当DLP文件被打开时会触发回调通知当前应用。仅支持在非DLP沙箱应用中调用。**使用场景:**  当应用需要在DLP文件打开后执行特定操作(如记录日志、更新界面)时,可注册该监听。
...
取消监听打开DLP文件。仅支持在非DLP沙箱应用中调用。调用成功后，将不再接收DLP文件打开事件的通知。**使用场景:** 通常在页面销毁或不再需要监听时调用以释放资源。
...
查询当前应用是否运行在DLP沙箱环境。调用成功后返回boolean值，true表示运行在沙箱中，false表示不在沙箱中。使用Promise方式异步返回结果。**使用场景:** 用于判断当前应用是否处于DLP沙箱环境，以便决定是否执行沙箱相关的操作或调用沙箱专用接口。
...
查询当前应用是否运行在DLP沙箱环境。使用callback方式异步返回结果。**使用场景:** 用于判断当前应用是否处于DLP沙箱环境，以便决定是否执行沙箱相关的操作或调用沙箱专用接口。
...
查询当前可支持权限设置和校验的文件扩展名类型列表。调用成功后返回支持的文件类型列表，用于判断哪些文件类型可进行DLP权限管理。使用Promise方式异步返回结果。**使用场景:** 用于获取支持DLP权限管理的文件类型列表，以便决定当前文件是否可以进行加密。
...
查询当前可支持权限设置和校验的文件扩展名类型列表。调用成功后返回支持的文件类型列表，用于判断哪些文件类型可进行DLP权限管理。使用callback方式异步返回结果。**使用场景:** 用于获取支持DLP权限管理的文件类型列表，以便决定当前文件是否可以进行加密。
...
设置DLP沙箱的保留状态。默认情况下,打开DLP文件时系统会自动创建沙箱环境,关闭文件后自动销毁沙箱。设置保留状态后,即使关闭DLP文件,沙箱环境也会保留,便于快速重新打开相同DLP文件。适用于需要频繁操作同一DLP文件的场景,可提升文件打开效率。使用Promise方式异步返回结果。**使用场景:** 用于设置DLP沙箱的保留状态，避免频繁打开同一DLP文件时重复创建沙箱环境，提升文件访问效率。
...
设置DLP沙箱的保留状态。默认情况下,打开DLP文件时系统会自动创建沙箱环境,关闭文件后自动销毁沙箱。设置保留状态后,即使关闭DLP文件,沙箱环境也会保留,便于快速重新打开相同DLP文件。适用于需要频繁操作同一DLP文件的场景,可提升文件打开效率。**使用场景:** 用于设置DLP沙箱的保留状态，避免频繁打开同一DLP文件时重复创建沙箱环境，提升文件访问效率。
...
取消沙箱保留状态即恢复DLP文件关闭时自动卸载沙箱策略。使用Promise方式异步返回结果。**使用场景:** 用于取消沙箱保留状态，恢复默认行为以释放系统资源，适用于不再频繁访问DLP文件的场景。
...
取消沙箱保留状态即恢复DLP文件关闭时自动卸载沙箱策略。使用callback方式异步返回结果。**使用场景:** 用于取消沙箱保留状态，恢复默认行为以释放系统资源，适用于不再频繁访问DLP文件的场景。
...
查询指定应用的保留沙箱信息列表。仅支持在非DLP沙箱应用中调用。使用Promise方式异步返回结果。**使用场景:** 用于查询指定应用的保留沙箱列表，以便查看或管理当前处于保留状态的沙箱环境。
...
查询指定应用的保留沙箱信息列表。使用callback方式异步返回结果。**使用场景:** 用于查询指定应用的保留沙箱列表，以便查看或管理当前处于保留状态的沙箱环境。
...
查询当前应用的保留沙箱信息列表。使用callback异步回调。**使用场景:** 用于查询指定应用的保留沙箱列表，以便查看或管理当前处于保留状态的沙箱环境。
...
查询最近访问的DLP文件列表。调用成功后返回文件访问记录，用于追踪和管理DLP文件的使用情况。仅支持在非DLP沙箱应用中调用。使用Promise方式异步返回结果。**使用场景:** 用于获取最近访问的DLP文件记录列表，便于审计追踪和文件使用情况管理。
...
查询最近访问的DLP文件列表。调用成功后返回文件访问记录，用于追踪和管理DLP文件的使用情况。使用callback方式异步返回结果。**使用场景:** 用于获取最近访问的DLP文件记录列表，便于审计追踪和文件使用情况管理。
...
在当前[UIAbility](../apis-ability-kit/js-apis-app-ability-uiAbility.md#uiability)界面以无边框形式打开DLP权限管理应用。使用Promise方式异步返回结果。**使用场景:** 用于拉起DLP权限管理应用配置文件权限，并将用户操作结果返回给调用方。
...
设置沙箱应用配置信息，配置信息为JSON字符串格式，具体内容由应用自行设置。调用成功后，沙箱应用将按照配置信息运行。使用Promise方式异步返回结果。**使用场景:**用于设置沙箱应用的配置信息，以便应用按需传递自定义参数。
...
清理沙箱应用配置信息。调用成功后，沙箱应用配置将被清除，恢复默认状态。使用Promise方式异步返回结果。**使用场景:** 用于清理沙箱应用的配置信息，恢复默认状态以防止配置残留影响后续使用。
...
获取沙箱应用配置信息，使用Promise方式异步返回结果。**使用场景:** 用于获取沙箱应用的配置信息，便于读取或验证当前的配置状态。
...
查询当前系统是否提供加密保护特性，仅支持企业设备且需[MDM（Mobile Device Management，移动设备管理）](../../mdm/mdm-kit-intro.md)配置使能。调用成功后返回查询结果，用于判断系统是否支持DLP加密功能。使用Promise方式异步返回结果。**使用场景:** 用于判断当前系统是否支持DLP加密功能，以便在不支持的设备上做兼容处理或功能降级。
...
设置企业应用防护策略。调用成功后，企业应用的DLP防护将按照设置的策略执行。**使用场景**：可用于企业管理员配置DLP安全策略，以统一管理企业数据安全防护规则。
...
将明文文件加密生成企业账号DLP文件，仅支持企业账号调用。使用Promise异步回调。**使用场景:** 用于将明文文件加密生成企业账号的DLP权限受控文件，实现企业级的文件权限管理。
...
将DLP文件解密生成明文文件，仅支持企业账号调用。使用Promise异步回调。**使用场景:** 用于将DLP加密文件解密为明文文件，适用于拥有者权限用户导出或迁移文件。
> **说明：**
>
...
在DLP文件中解析文件头，获取DLP明文策略。返回的策略JSON字符串包含[DLPProperty](#dlpproperty21)和[CustomProperty](#customproperty21)信息。**使用场景**：可用于在查看DLP文件权限配置等场景中，获取文件的策略信息以便进行分析。使用Promise异步回调。
...
该函数提供给SA（System Ability）侧调用，处理完连接云端服务的请求后，通过callback将结果返回给SA（System Ability）。**使用场景**：可用于企业账号认证、云端权限验证等场景，实现SA与云服务器的通信能力，完成权限校验或账号验证流程。
...
提供将回调从SA（System Ability）侧注销的能力。**使用场景**：可用于应用退出时注销回调释放资源，确保回调能力正确释放。
...
查询已打开且符合指定选项的企业DLP文件的URI列表。使用Promise异步回调。**使用场景：** 在需要管理或追踪当前应用已打开的企业DLP文件时调用该接口，可用于文件状态检查、资源管理等场景。
...
| 参数名 | 类型 | 必填 | 说明 |
| -------- | -------- | -------- | -------- |
| options | [DlpFileQueryOptions](#dlpfilequeryoptions26) | 否 | 企业DLP文件的查询选项。当需要按分类标签筛选查询特定企业DLP文件时传入此参数，当需要查询所有企业DLP文件时可不传此参数。不传入或传入空字符串时，查询所有企业DLP文件。|
...
关闭当前打开的所有符合指定选项的企业DLP文件。使用Promise异步回调。**使用场景:** 在需要批量关闭企业DLP文件、清理文件资源或应用退出前释放文件句柄时调用该接口。
...
| 参数名 | 类型 | 必填 | 说明 |
| -------- | -------- | -------- | -------- |
| options | [DlpFileQueryOptions](#dlpfilequeryoptions26) | 否 | 企业DLP文件的查询选项。当需要按分类标签筛选关闭特定企业DLP文件时传入此参数，当需要关闭所有企业DLP文件时可不传此参数。不传入或传入空字符串时，关闭所有企业DLP文件。|
