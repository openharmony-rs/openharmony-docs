...
根据文件的fd，查询该文件是否是DLP文件。调用成功后返回查询结果，true表示是DLP文件，false表示非DLP文件。使用Promise方式异步返回结果。

**使用场景：** 在文件处理流程中，需要先判断文件是否为DLP文件，再决定后续处理策略（如是否需要通过DLP沙箱打开）。
...
**使用场景:** 在DLP沙箱中处理文件时,可根据权限信息判断当前用户可以执行哪些操作,避免调用无权限的功能。使用Promise方式异步返回结果。
...
查询当前DLP沙箱的权限信息。返回的权限信息包括文件的授权类型和可执行的操作权限(如查看、编辑、复制等)。使用callback方式异步返回结果。

**使用场景:** 在DLP沙箱中处理文件时,可根据权限信息判断当前用户可以执行哪些操作,避免调用无权限的功能。
...
**使用场景:** 根据原始文件名后缀判断文件类型，选择对应的应用打开。
...
**使用场景:**  当应用需要在DLP文件打开后执行特定操作(如记录日志、更新界面)时,可注册该监听。
...
**使用场景:** 用于获取支持DLP权限管理的文件类型列表，以便决定当前文件是否可以进行加密。
...
设置DLP沙箱的保留状态。默认情况下,打开DLP文件时系统会自动创建沙箱环境,关闭文件后自动销毁沙箱。设置保留状态后,即使关闭DLP文件,沙箱环境也会保留,便于快速重新打开相同DLP文件。适用于需要频繁操作同一DLP文件的场景,可提升文件打开效率。使用Promise方式异步返回结果。

**使用场景:** 用于设置DLP沙箱的保留状态，避免频繁打开同一DLP文件时重复创建沙箱环境，提升文件访问效率。
...
**使用场景:** 用于取消沙箱保留状态，恢复默认行为以释放系统资源，适用于不再频繁访问DLP文件的场景。
...
**使用场景:** 用于查询指定应用的保留沙箱列表，以便查看或管理当前处于保留状态的沙箱环境。
...
**使用场景:** 用于获取最近访问的DLP文件记录列表，便于审计追踪和文件使用情况管理。
...
**使用场景:** 用于拉起DLP权限管理应用配置文件权限，并将用户操作结果返回给调用方。
...
设置沙箱应用配置信息，配置信息为JSON字符串格式，具体内容由应用自行设置。调用成功后，沙箱应用将按照配置信息运行。使用Promise方式异步返回结果。

**使用场景:**用于设置沙箱应用的配置信息，以便应用按需传递自定义参数。
...
**使用场景:** 用于清理沙箱应用的配置信息，恢复默认状态以防止配置残留影响后续使用。
...
**使用场景:** 用于获取沙箱应用的配置信息，便于读取或验证当前的配置状态。
...
**使用场景:** 用于判断当前系统是否支持DLP加密功能，以便在不支持的设备上做兼容处理或功能降级。
...
**使用场景:** 用于将明文文件加密生成企业账号的DLP权限受控文件，实现企业级的文件权限管理。
...
**使用场景:** 用于将DLP加密文件解密为明文文件，适用于拥有者权限用户导出或迁移文件。
