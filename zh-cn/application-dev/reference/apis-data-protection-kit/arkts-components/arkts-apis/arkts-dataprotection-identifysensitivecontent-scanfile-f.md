...
| identifyPolicies | Array&lt;[Policy](arkts-dataprotection-identifysensitivecontent-policy-i.md)&gt; | 是 | 用于识别敏感内容的策略数组。每个Policy定义识别规则（标签、关键字、正则表达式），系统将根据这些规则扫描文件内容并返回匹配结果。 |
...
| [801](../../errorcode-universal.md#801-该设备不支持此api) | Capability not supported. Possible causes: The device type does not support the capability. |
