...
用于返回沙箱应用配置信息。 调用成功后，返回的配置信息为JSON格式字符串。该JSON格式字符串包含应用在DLP沙箱中的配置信息，调用者需使用free()释放内存。DLP沙箱是DLP技术为受保护文件创建的安全隔离环境，应用在打开DLP保护的文件时会自动运行在DLP沙箱中。该接口只能在DLP沙箱环境中调用。
