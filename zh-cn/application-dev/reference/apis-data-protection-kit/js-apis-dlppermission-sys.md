...
| uri | string | 是 | DLP文件的URI。不超过4095字节。 超出范围时抛出错误码19100001。|
| callback | AsyncCallback&lt;[DLPSandboxInfo](#dlpsandboxinfo)&gt; | 是 | 回调函数。当installDLPSandbox成功，err为undefined，DLPSandboxInfo为获取到的沙箱信息；否则为错误对象。 |
...
| callback | AsyncCallback&lt;void&gt; | 是 | 回调函数。当uninstallDLPSandbox成功，err为undefined，否则为错误对象。|
...
| ciphertextFd | number | 是 | 目标加密文件的fd。取值范围为[0, 2<sup>31</sup>-1]。超出范围时该数值会被截断。 |
| property | [DLPProperty](js-apis-dlppermission.md#dlpproperty21) | 是 | 授权用户信息：授权用户列表、owner账号、联系人账号。 |
| callback | AsyncCallback&lt;[DLPFile](#dlpfile)&gt; | 是 |回调函数。当generateDLPFile成功，err为undefined，DLPFile为获取到的DLP文件信息；否则为错误对象。 |
