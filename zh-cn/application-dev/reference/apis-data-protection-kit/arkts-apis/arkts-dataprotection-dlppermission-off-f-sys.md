...
必须在调用[on](arkts-dataprotection-dlppermission-on-f-sys.md#on-2)注册监听后才能调用此方法取消监听。
...
| listener | [Callback](../../apis-arkui/arkts-components/arkts-arkui-common-callback-i.md)<DLPSandboxState> | 否 | 沙箱应用卸载事件的回调。默认为空，表示取消该类型事件的所有回调。 |
