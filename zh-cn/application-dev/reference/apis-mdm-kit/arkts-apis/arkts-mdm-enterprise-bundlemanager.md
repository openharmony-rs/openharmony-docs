...
| getAllowedInstallBundlesSync | 获取当前/指定用户下的应用程序包安装允许名单。 |
...
| getDisallowedInstallBundlesSync | 获取当前/指定用户下的应用程序包安装禁止名单。 |
...
| getDisallowedUninstallBundlesSync | 获取当前/指定用户下包卸载禁止名单。 |
...
| getInstallationAllowedAppDistributionTypes | 获取可安装的应用程序签名证书的分发类型。 |
...
| getInstalledBundleList | 获取设备指定用户下已安装应用列表。使用Promise异步回调。 |
...
| [install](arkts-mdm-bundlemanager-install-f.md#install-2) | 安装指定路径下的应用包。使用Promise异步回调。 |
