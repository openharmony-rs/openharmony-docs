...
安装指定路径下的应用包，并返回安装结果。使用Promise异步回调。 此接口只能安装分发类型为enterprise\_mdm（MDM应用）和enterprise\_normal（普通企业应用）类型的应用，可以通过 [getBundleInfoForSelf]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口查询应用自身的 [BundleInfo]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_，其中BundleInfo.appInfo.appDistributionType为应用的分发类型。 > **说明：** > > 该接口比较耗时，当调用此接口后，后续如果在应用主线程调用其他同步接口时需要等待该接口异步返回。
