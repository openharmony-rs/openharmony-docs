...
| [isWifiActive](arkts-mdm-wifimanager-iswifiactive-f-sys.md#isWifiActive（系统接口）) | 查询当前设备的Wi-Fi开启状态。使用Promise异步回调。 |
...
| [setWifiProfile](arkts-mdm-wifimanager-setwifiprofile-f-sys.md#setWifiProfile（系统接口）) | 为当前设备配置Wi-Fi，使连接到指定网络。使用Promise异步回调。 |
<!--DelEnd-->
