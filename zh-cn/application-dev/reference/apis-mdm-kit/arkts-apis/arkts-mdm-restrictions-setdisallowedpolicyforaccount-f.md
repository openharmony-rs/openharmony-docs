...
| disallow | boolean | 是 | true表示禁用，false表示启用。 |
| accountId | number | 是 | 用户ID<br>取值应为≥0的整数。   - 用户ID，取值范围：大于等于0。<br>accountId可以通过getOsAccountLocalId等接口来获取。<br>当feature值为SUPER_HUB时，accountId仅支持传入当前用户的用户ID，不支持跨用户设置。否则会抛出9200012错误码。 |
