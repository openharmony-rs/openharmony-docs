...
若已经通过[setDisallowedPolicyForAccount](./js-apis-enterprise-restrictions.md#restrictionssetdisallowedpolicyforaccount)接口禁用了某用户下的中转站，再通过本接口添加中转站为该用户的不可关停应用，会发生策略冲突，抛出9200010错误码。可以通过[setDisallowedPolicyForAccount](./js-apis-enterprise-restrictions.md#restrictionssetdisallowedpolicyforaccount)接口解除禁用该用户下的中转站来解决冲突。
