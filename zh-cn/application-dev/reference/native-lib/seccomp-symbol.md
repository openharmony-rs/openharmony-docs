...
Seccomp（Secure Computing Mode）是 Linux 内核提供的轻量级系统调用过滤机制，自 Linux 2.6.12 首次引入后，已从仅支持「严格模式（Strict）」的极简形态，演进为以 Seccomp-BPF（Berkeley Packet Filter） 为核心的主流形态 ——OpenHarmony OS 中采用的正是 Seccomp-BPF 模式，其核心目标是通过 BPF 程序自定义系统调用过滤规则，精准限制进程可调用的系统调用范围，降低恶意代码利用内核漏洞的风险，提升进程 / 容器 / 应用的安全隔离性。
