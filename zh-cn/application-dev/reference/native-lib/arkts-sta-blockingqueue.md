...
```typescript
let queueInt: containers.BlockingQueue<int> = new containers.LinkedBlockingQueue<int>(10); //阻塞队列容量设置为10
for (let i = 0; i < 10; i++) {
  queueInt.push(i);
}
queueInt.push(10); //阻塞
...
```typescript
let queueInt: containers.BlockingQueue<int> = new containers.LinkedBlockingQueue<int>();
for (let i = 0; i < 10; i++) {
  queueInt.push(i);
}
for (let i = 0; i < 9; i++) {
...
```typescript
let queueInt: containers.BlockingQueue<int> = new containers.LinkedBlockingQueue<int>(10);
for (let i = 0; i < 10; i++) {
...
当前队列在不阻塞的情况下还可以容纳的元素数量，即剩余容量，等于队列的容量减去当前的元素个数。
...
```typescript
let queueInt: containers.BlockingQueue<int> = new containers.ArrayBlockingQueue<int>(10);
for (let i = 0; i < 5; i++) {
  queueInt.push(i);
}
...
arr[0] = 0;
let queueInt: containers.BlockingQueue<int> = new containers.LinkedBlockingQueue<int>(arr);
...
>
>- capacity大于等于iterable中有效元素（非undefined）个数时，可正常编译运行，iterable中的有效元素会全部入队。
>
>- capacity小于iterable中有效元素（非undefined）个数时，可正常编译运行，仅前capacity个有效元素入队，超出容量的元素不会入队。
...
arr[0] = 0;
let iter = arr.values();
let queueInt: containers.BlockingQueue<int> = new containers.LinkedBlockingQueue<int>(10, iter);
...
```typescript
let queueInt: containers.LinkedBlockingQueue<int> = new containers.LinkedBlockingQueue<int>(10);
for (let i: int = 0; i < 10; ++i) {
  queueInt.push(i);
}
...
let queueInt: containers.BlockingQueue<number> = new containers.ArrayBlockingQueue<number>(200, set);
...
```typescript
let queueInt: containers.ArrayBlockingQueue<int> = new containers.ArrayBlockingQueue<int>(10);
for (let i: int = 0; i < 10; ++i) {
  queueInt.push(i);
}
let res = queueInt.getEnd(); // 9
