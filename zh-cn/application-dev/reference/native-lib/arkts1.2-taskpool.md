...
创建的同一优先级任务的执行顺序时可以自行决定。任务的实际执行顺序与调用任务池API提供的任务执行接口的顺序一致。任务的默认优先级为MEDIUM。
...
execute(func: Function, ...args: Any[]): Promise\<Any>

将待执行的函数放入taskpool内部任务队列，函数不会立即执行，而是等待分发到工作线程执行。当前执行模式下任务无法取消。使用Promise异步回调。
...
| args   | Any[] | 否   | 执行逻辑的函数所需要的入参。默认值为undefined。 |
...
taskpool.execute(printArgs, 100.0).then((value: Any) => {
...
将创建好的任务放入taskpool内部任务队列，任务不会立即执行，而是等待分发到工作线程执行。当前执行模式可以设置任务优先级，并支持尝试调用cancel取消任务。该任务不能是任务组任务、串行队列任务或已执行的任务。该任务仅支持执行一次。使用Promise异步回调。
...
taskpool.execute(task1, taskpool.Priority.LOW).then((value: Any) => {
  hilog.info(0x0000, "testTag", "taskpool result1: " + value);
});
taskpool.execute(task2, taskpool.Priority.MEDIUM).then((value: Any) => {
  hilog.info(0x0000, "testTag", "taskpool result2: " + value);
});
taskpool.execute(task3, taskpool.Priority.HIGH).then((value: Any) => {
...
taskpool.execute(taskGroup1).then((res: Array<Any>) => {
...
executeDelayed(delayTime: int, task: Task, priority?: Priority): Promise\<Any>
...
| 参数名       | 类型          | 必填  | 说明                 |
| ----------- | ------------- | ---- | -------------------- |
| delayTime   | int           | 是   | 延时时间。单位为ms。不可以为负值。 |
...
taskpool.executeDelayed(1000, task).then((res: Any) => {
...
executePeriodically(period: int, task: Task, priority?: Priority): void

周期执行任务，每隔period时长执行一次任务。当前执行模式支持设置任务优先级和调用cancel取消任务周期执行。周期任务不能是任务组任务、串行队列任务或已执行的任务，不能再次调用执行接口，不能拥有依赖关系。

**参数：**

| 参数名        | 类型          | 必填  | 说明                 |
| -----------  | ------------- | ----- | -------------------- |
| period       | int        | 是    | 周期时长，单位为毫秒，不可以为负值。  |
| task         | [Task](#task) | 是    | 需要周期执行的任务。 |
| priority     | [Priority](#priority) | 否   | 周期执行的任务的优先级，该参数的默认值为taskpool.Priority.MEDIUM。|
...
| the task with dependency cannot executePeriodically. | 有依赖关系的任务不能周期执行。<br>可能原因：调用接口时传入了有依赖关系的任务。<br>处理步骤：执行任务前确保任务无依赖关系。无法保证时，需捕获异常。 |
...
function concurrentFunc() {
  ...
  taskpool.execute(task1).then((res: Any) => {
...
取消任务池中的任务组。如果任务组的任务未全部执行结束时取消任务组，返回undefined作为任务组结果。
...
使用constructor方法构造任务（Task）。任务可以单次执行、放入任务组执行、放入串行队列执行或添加依赖关系执行。

### 属性

| 名称              | 类型           | 只读 | 可选 | 说明                                                         |
| ---------------- | -------------- | ---- | ---- | ------------------------------------------------------------ |
| f                | () => Any  | 否   | 否   | 创建任务时需要传入的函数。 |
| arguments        | Any[]  | 否   | 是   | 创建任务传入函数所需的参数。默认值为undefined。 |
| name             | string         | 否   | 否   | 创建任务时指定的任务名称。无长度限制和其他格式要求。 |
| totalDuration    | int         | 否   | 否   | 执行任务总耗时。单位为ms。当任务未执行时值为0，当任务执行后为任务执行的实际总耗时。 |
| ioDuration       | int         | 否   | 否   | 执行任务异步IO耗时。单位为ms。当任务未执行时值为0，当任务执行后为任务执行的实际异步IO耗时。 |
| cpuDuration      | int         | 否   | 否   | 执行任务CPU耗时。单位为ms。当任务未执行时值为0，当任务执行后为任务执行的实际异步CPU耗时。 |

### constructor

constructor(func: Function, ...args: Any[])

Task类的构造函数，用于初始化Task对象。
...
检查当前正在运行的任务是否已取消。在使用该方法前，需要先构造一个Task对象。

**返回值：**

| 类型    | 说明                                 |
| ------- | ------------------------------------ |
| boolean | 如果任务被取消，返回true；否则，返回false。|
...
在任务执行过程中，向宿主线程发送消息并触发回调。使用此方法前，需要先构造Task。
...
为任务注册回调函数，接收并处理任务池工作线程的数据。使用此方法前需构造Task。
...
为当前任务添加对其他任务的依赖。使用该方法前需要先构造Task。任务和被依赖的任务不能是任务组任务、串行队列任务、已执行任务或周期任务。存在依赖关系的任务（依赖其他任务的任务或被依赖的任务）执行后，不能再次执行。
...
| groupTask cannot addDependency. | 任务组任务增加依赖关系。<br>可能原因：任务组任务调用该函数。<br>处理步骤：调用时传入任务，无法保证时，捕获异常。 |
...
taskpool.execute(task3).then((res: Any) => {
...
删除当前任务对其他任务的依赖。在使用该方法前，需要先创建Task。
...
| cannot removeDependency on a dependent and executed task. | 已执行的任务不能移除依赖关系。<br>可能原因：调用该函数。传参是已执行的任务。<br>处理步骤：调用时，确保传参不是已执行的任务。无法保证时，需要捕获异常。 |
...
| ------ | --------- | ---- | ---------------------------------------------------------------------- |
| func   | Function  | 是   | 执行的逻辑需要传入函数。     |
| args   | Any[] | 否   | 任务执行函数所需要的入参，默认值为undefined。 |
...
| task has been executed.  | 添加已执行的任务。<br>可能原因：任务已被执行。<br>处理步骤：添加任务前确保任务未执行。无法保证时，需捕获异常。  |
...
SequenceRunner的构造函数用于构造一个全局串行队列，如果名字相同，将返回同一个串行队列。
...
| 参数名    | 类型                  | 必填 | 说明                                                     |
| -------- | --------------------- | ---- | ------------------------------------------------------- |
| name     | string                | 是   | 串行队列的名字。无长度限制，但须符合字符集要求。 |
| priority | [Priority](#priority) | 否   | 指定任务的优先级，该参数默认值为taskpool.Priority.MEDIUM。 |
...
执行串行任务前，需要先构造SequenceRunner。串行队列不支持执行任务组任务、其他串行队列任务、有依赖关系的任务和已执行的任务。使用Promise异步回调。

> **说明：**
>
> - 不支持加入存在依赖的任务。
> - 前面任务执行失败或取消不影响后续任务执行。
...
| SequenceRunner cannot execute the periodicTask. | 周期任务不能再次执行。<br>可能原因：串行队列不能执行周期任务。<br>处理步骤：执行任务前确保任务未被执行过。无法保证时，需捕获异常。  |
...
async function seqRunner() {
  ...
  runner.execute(task2).then((res: Any) => {
    finalString += 'b';
    hilog.info(0x0000, "testTag", "seqrunner: task2 done");
  });
  runner.execute(task3).then((res: Any) => {
    finalString += 'c';
...
| 参数名 | 类型          | 必填 | 说明                             |
| ------ | ------------- | ---- | -------------------------------- |
| runningCapacity   | int | 是   | 同时允许执行的最大任务数量（并发度）。该参数应为正整数，传入非正整数时报错。|
| waitingCapacity   | int | 否   | 等待队列的最大容量。默认值为0。如果不传入或者传入值为0，表示等待任务的处理模式为排队模式，等待任务的容量为无限。如果传入值大于0，表示等待队列的处理模式为丢弃队头模式，如果等待队列中的任务数目超出了等待队列的容量，则丢弃等待队列的队头，并将新任务添加至等待队列的队尾。如果传入负数报错。|
...
| 参数名 | 类型          | 必填 | 说明                             |
| ------ | ------------- | ---- | -------------------------------- |
| name   | string\|undefined | 是 | 异步队列名称，为undefined时等同于匿名构造函数。|
...
>
> - 支持并发执行任务。
> - 不支持执行已执行过的任务、任务组任务、依赖型任务、串行队列中的任务、周期性任务、延迟任务和其他异步队列的任务。
...
| 名称      | 类型               | 只读 | 可选 | 说明                                                           |
| -------- | ------------------ | ---- | ---- | ------------------------------------------------------------- |
| taskId   | int             | 否   | 否   | 任务的标识符。无单位。值为标识任务的一个正值。 |
| state    | [State](#state)    | 否   | 否   | 任务的状态。 |
| duration | int             | 否   | 是   | 任务执行至当前所用的时间，单位为ms。当返回为0时，表示任务未执行；返回为空时，表示没有任务执行。默认值为0。  |
...
| 名称     | 类型                    | 只读 | 可选 | 说明                                                      |
| -------- | ---------------------- | ---- | ---- | -------------------------------------------------------- |
| tid      | int                 | 否   | 否   | 工作线程的标识符。无单位。值为标识工作线程的一个正值。返回为空时，代表没有任务执行。   |
| taskIds  | int[]               | 否   | 是   | 在当前线程上运行的任务ID列表。无单位。值为运行任务ID的一个正值数组。返回为空时，代表没有任务执行。默认值为undefined。   |
...
export async function func2(): Promise<void> {
  let strArray: Array<string> = ['c test string', 'b test string', 'a test string'];
  taskpool.execute(strSort, strArray).then((result: Any) => {
...
for (let i = 0; i < allCount; i++) {
  ...
  taskpool.execute(task1, taskpool.Priority.LOW).then((res: Any) => {
    lowCount++;
  }).catch((e) => {
    hilog.info(0x0000, "testTag", "low task error: " + e);
  })
  taskpool.execute(task2, taskpool.Priority.MEDIUM).then((res: Any) => {
    mediumCount++;
  }).catch((e) => {
    hilog.info(0x0000, "testTag", "medium task error: " + e);
  })
  taskpool.execute(task3, taskpool.Priority.HIGH).then((res: Any) => {
    highCount++;
  }).catch((e) => {
...
for (let threadInfo of threadIS) {
  tid = threadInfo.tid;
  let taskIdsInfo = threadInfo.taskIds;
  let priorityInfo = threadInfo.priority;
  if (taskIdsInfo != undefined && priorityInfo != undefined) {
    taskIds = taskIdsInfo;
    priority = priorityInfo as int;
  }
...
for (let taskInfo of taskIS) {
  ...
  let durationInfo = taskInfo.duration;
  if (durationInfo != undefined ) {
