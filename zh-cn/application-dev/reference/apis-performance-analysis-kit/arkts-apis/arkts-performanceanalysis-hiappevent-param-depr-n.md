# Param

此接口提供了所有预定义参数的参数名称常量。 | 名称 | 类型 | 可读 | 可写 | 说明 | | ------------------------------- | ------ | ---- | ---- | ------------------ | | USER\_ID | string | 是 | 否 | 用户自定义ID。 | | DISTRIBUTED\_SERVICE\_NAME | string | 是 | 否 | 分布式服务名称。 | | DISTRIBUTED\_SERVICE\_INSTANCE\_ID | string | 是 | 否 | 分布式服务实例ID。 |

**起始版本：** 7

**ArkTS模式：** 仅支持ArkTS-Dyn，起始版本为7。

**废弃版本：** 9

**替代接口：** ohos.hiviewdfx.hiAppEvent/hiAppEvent.Param

<!--Device-hiAppEvent-namespace Param--><!--Device-hiAppEvent-namespace Param-End-->

**系统能力：** SystemCapability.HiviewDFX.HiAppEvent

## 汇总

### 常量

| 名称 | 说明 |
| --- | --- |
| [USER_ID](arkts-performanceanalysis-param-depr-con.md#user_id) | 用户自定义ID。 |
| [DISTRIBUTED_SERVICE_NAME](arkts-performanceanalysis-param-depr-con.md#distributed_service_name) | 分布式服务名称。 |
| [DISTRIBUTED_SERVICE_INSTANCE_ID](arkts-performanceanalysis-param-depr-con.md#distributed_service_instance_id) | 分布式服务实例ID。 |

