...
设置传递堆快照规格。 "event"：应用发生OOM时，不传递堆快照。 "event_rawheap"：应用发生OOM时，系统生成并传递堆快照。 **说明：** - 当前仅接收以上二值，如果传入其他内容，方法将调用失败，不会产生任何效果。 - 参数值为"event_rawheap"，无法确保成功生成堆快照文件。原因是生成堆快照时，应用可能因性能问题触发冻屏而提前退出。 - 应用每次使能行为只在应用当前生命周期生效，在同一生命周期内，以最后一次成功调用的使能状态为准。应用重启后，需要重新设置使能状态。 **起始版本**：26.0.0 **原子化服务API：** 从API版本26.0.0开始，该接口支持在原子化服务中使用。
