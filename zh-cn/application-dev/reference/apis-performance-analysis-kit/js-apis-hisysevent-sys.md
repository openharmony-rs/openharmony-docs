...
| onEvent | (info: [SysEventInfo](#syseventinfo)) => void | 否 | 否 | 订阅事件的回调方法。当订阅的事件发生时此回调方法会被触发，回调函数接收订阅到的[SysEventInfo](#syseventinfo)，用于处理订阅到的事件数据。|
| onServiceDied | () => void | 否 | 否 | 系统事件服务关闭的回调方法。当系统事件服务异常关闭或停止时触发，用于处理服务异常情况（如记录日志、重新订阅事件等）。 |
...
| condition<sup>10+</sup> | string | 否 | 是 | 事件的额外参数条件，格式：{"version":"V1","condition":{"and":[{"param":"参数","op":"操作符","value":"比较值"}]}}。<br>参数：指定事件参数的键值，类型为string。<br>操作符支持：=、!=、<、<=、>和>=。<br>value：比较值，类型可为string、number等。<br>支持在“and”数组中配置多个条件，查询结果取交集。建议避免配置过多复杂条件以优化查询性能。默认不传该参数时不进行额外参数过滤。<br>**ArkTS-Dyn起始版本**：10<br>**ArkTS-Sta起始版本**：23 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| -------- | -------- | -------- | -------- | -------- |
| onQuery | (infos: [SysEventInfo](#syseventinfo)[]) => void | 否 | 否 | 返回查询到的系统事件的回调方法。当查询到系统事件时触发（可能调用多次），回调函数接收查询到的[SysEventInfo](#syseventinfo)数组，用于处理查询结果。 |
| onComplete | ArkTS-Dyn: (reason: number, total: number) => void<br> ArkTS-Sta: (reason: int, total: int) => void | 否 | 否 | 查询结果统计的回调方法。当查询完成时触发（只调用一次），回调函数接收查询结果统计信息：reason表示查询完成的原因（如查询成功、查询超时等），total表示查询到的总事件数。 |
