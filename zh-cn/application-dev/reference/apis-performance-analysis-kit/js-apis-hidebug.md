...
为当前线程所在进程转储虚拟机的原始堆快照，并支持清除nodeId缓存。生成的文件为rawheap格式，使用Promise异步回调完成。该文件可通过[rawheap-translator工具](../../tools/rawheap-translator.md)转化为heapsnapshot格式文件进行解析。
...
| processDump      | boolean | 是  | 是否转储当前线程所在进程的原始堆快照。true：转储进程级堆快照；false：转储当前线程的原始堆快照。|

**返回值**：

| 类型  | 说明                                                                                                   |
| ------ |------------------------------------------------------------------------------------------------------|
| Promise&lt;Array&lt;string&gt;&gt; | Promise对象，返回生成的快照文件路径（[应用沙箱内路径](../../file-management/app-sandbox-directory.md#应用沙箱路径和真实物理路径的对应关系)）。 |
