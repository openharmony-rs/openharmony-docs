...
| [typedef  EventType](#typedef  eventtype) | - | 事件类型。建议开发者根据不同的使用场景选择不同的事件类型。 |
| [OH_HiAppEvent_FrameworkType](#oh_hiappevent_frameworktype) | OH_HiAppEvent_FrameworkType | 框架类型。建议根据不同的使用场景选择不同的框架类型。 |
...
| [HiAppEvent_Watcher* OH_HiAppEvent_CreateWatcher(const char* name)](#oh_hiappevent_createwatcher) | - | 创建一个用于监听app事件的监听器。 |
...
| [HiAppEvent_Processor* OH_HiAppEvent_CreateProcessor(const char* name)](#oh_hiappevent_createprocessor) | - | 创建一个用于处理app事件上报的处理者。 |
...
| [int OH_HiAppEvent_ReportFrameworkMemAnomaly(enum OH_HiAppEvent_FrameworkType frameworkType, const char* frameworkVersion, const char* description)](#oh_hiappevent_reportframeworkmemanomaly) | - | 当框架发生内存泄漏上报事件。 |
...
| HIAPPEVENT_INVALID_PARAM_VALUE_LENGTH = 4 | 无效的参数值长度<br>**起始版本：** 18 |
| HIAPPEVENT_PROCESSOR_IS_NULL = -7 | 处理器为空<br>**起始版本：** 18 |
| HIAPPEVENT_PROCESSOR_NOT_FOUND = -8 | 找不到处理器<br>**起始版本：** 18 |
| HIAPPEVENT_INVALID_PARAM_VALUE = -9 | 参数值非法。<br>**起始版本：** 15 |
| HIAPPEVENT_EVENT_CONFIG_IS_NULL = -10 | 事件配置为空。<br>**起始版本：** 15 |
| HIAPPEVENT_OPERATE_FAILED = -100 | 操作失败<br>**起始版本：** 18 |
| HIAPPEVENT_INVALID_UID = -200 | 无效的uid<br>**起始版本：** 18 |
| HIAPPEVENT_REPORT_FREQUENCY_EXCEEDED = -300 | 报告频率超过。<br>**起始版本：** 26.0.0 |

### typedef  EventType

```c
enum typedef  EventType
```
...
| enum EventType type | 事件类型，在{@link EventType}中定义。 |
...
| int timeOut | 单位为秒，实际生效值为timeOut * 30秒。timeOut>0时，每timeOut * 30秒检查新事件，有则触发onTrigger，触发后重新计时；触发onTrigger后，经过timeOut秒后将再次检查是否存在新接收到的事件。timeOut≤0时不启用超时触发。 |
...
用于设置监听器onTrigger回调的接口。如果未设置OnReceive回调或已将其设置为nullptr，则将保存观察者接收到的应用事件。当保存的应用事件满足onTrigger回调的触发条件时，将调用onTrigger回调。
...
| 参数项 | 描述 |
| -- | -- |
| enum OH_HiAppEvent_FrameworkType frameworkType | 框架类型。 |
| const char* frameworkVersion | 框架版本号。 |
| const char* description | 框架内存泄漏事件的描述。 |
