...
ArkTS-Dyn: setOutputTypeByDomainID(type: OutputType, domainIDs: Array&lt;number&gt;, isExclude: boolean): OutputType

ArkTS-Sta: setOutputTypeByDomainID(type: OutputType, domainIDs: Array&lt;int&gt;, isExclude: boolean): OutputType
...
| ------ | --------------------- | ---- | ------------------------------------------------------------ |
| type  | [OutputType](#outputtype) | 是   | hilog的输出类型。                                                   |
| domainIDs  | ArkTS-Dyn: Array&lt;number&gt; <br/>ArkTS-Sta: Array&lt;int&gt; | 是   | domainID列表，每个domainID取值范围为0x0000~0xFFFF，仅对应用domain生效。                                                   |
...
ArkTS-Dyn: getLogFile(latestSeconds: number): Array&lt;string&gt;

ArkTS-Sta: getLogFile(latestSeconds: int): Array&lt;string&gt;
...
| 参数名 | 类型                  | 必填 | 说明                                                         |
| ------ | --------------------- | ---- | ------------------------------------------------------------ |
| latestSeconds  | ArkTS-Dyn: number <br/>ArkTS-Sta: int | 是   | 距离现在的时间间隔，以秒为单位。<br>若传入的值小于0，则为无效值，返回值为空。             |
