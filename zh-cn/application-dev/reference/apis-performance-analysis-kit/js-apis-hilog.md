...
hilog模块是HarmonyOS提供的日志打印子系统，属于Performance Analysis Kit，允许应用或服务按照指定的日志类型、日志级别和格式字符串输出日志。开发者可利用hilog在应用运行过程中记录关键流程信息，异常情况和错误事件，从而了解应用的运行状态并更好的调试程序。
...
- **日志输出管理**：通过setOutputType设置日志输出到控制台、私有沙箱或共享沙箱，通过setOutputTypeByDomain按域ID列表精细化控制不同域的输出方式，并提供日志目录查询、日志文件获取、日志刷新与清理等管理能力。

hilog适用于应用和服务在开发调试阶段记录运行日志的场景，也适用于正式发布版本中通过级别控制减少日志输出和保护隐私数据的场景。DEBUG级别在正式版本中默认不记录，仅在调试版本或开启调试功能的正式版本中可用。
> **说明：**
>
...
**原子化服务API**：从API version 11开始，该接口支持在原子化服务中使用。原子化服务是HarmonyOS提供的一种面向未来的服务提供方式。
...
| domain | number | 是   | 日志对应的领域标识，范围是0x0~0xFFFF，超出范围则日志无法打印。<br/>建议开发者在应用内根据需要自定义划分。 |
| tag    | string | 是   | 指定日志标识，可以为任意字符串，建议用于标识调用所在的类或者业务行为。tag长度最多为31字节，超出后会截断，不建议使用中文字符，可能出现乱码或者对齐问题。 |
| format | string | 是   | 格式字符串，用于日志的格式化输出。格式字符串中可以设置多个参数，参数需要包含参数类型、隐私标识。<br/>可用的参数格式符包括%d、%i、%s、%o、%O等，详见[参数格式符](#参数格式符)。<br>隐私标识分为{public}和{private}，缺省为{private}。标识{public}的内容明文输出，标识{private}的内容以\<private>过滤回显。 隐私标识机制帮助开发者保护用户敏感数据。|
...
可通过prefer参数配置不同的偏好策略。UNSET_LOGLEVEL策略表示清除设置、系统控制级别生效；PREFER_CLOSE_LOG策略表示取新设置的级别和系统控制级别两者较大值，适用于需要严格限制日志输出的场景；PREFER_OPEN_LOG策略取两者较小值，适用于需要尽可能打开日志输出的场景。如果选择策略PREFER_CLOSE_LOG，等同于调用setMinLogLevel函数。
...
| PREFER_OPEN_LOG | 2 | 实际生效的最低日志级别是新设置的级别和系统控制的最低级别两个值的较小值，适用于需要尽可能打开日志输出的场景。 |
