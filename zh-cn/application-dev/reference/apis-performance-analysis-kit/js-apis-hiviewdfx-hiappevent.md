...
- MAIN_THREAD_JANK（参数配置详见[主线程超时事件检测](../../dfx/hiappevent-watcher-mainthreadjank-events.md#seteventconfig参数设置说明)）
...
> **说明：**
>
> 系统事件中：地址越界事件和任务执行超时事件不支持在元服务中订阅。CPU高负载事件、24h功耗器件分解统计事件和音频卡顿事件均不支持在元服务和分身应用中订阅。启动耗时事件和滑动丢帧事件不支持在分身应用中订阅。
