...
| [OH_HiDebug_ProfilerOptions* OH_HiDebug_CreateProfilerOptions(void)](#oh_hidebug_createprofileroptions) | - | 创建资源Profiler配置对象。须与[OH_HiDebug_DestroyProfilerOptions](#oh_hidebug_destroyprofileroptions)配对使用。 |
...
| [HiDebug_ErrorCode OH_HiDebug_StartProfilerWithOptions(OH_HiDebug_ResourceType type, OH_HiDebug_ProfilerOptions *opts, OH_HiDebug_ProfilingCallback callback)](#oh_hidebug_startprofilerwithoptions) | - | 按指定类型和配置启动资源采集。须与[OH_HiDebug_StopProfiler](#oh_hidebug_stopprofiler)配对使用。 |
...
1. 采集配额限制：<br>
   ...
   - 进程（单进程独享）：同一时刻，仅支持启动1个采集任务。
2. 系统负载熔断机制：
   当系统触发以下任一负载保护条件时，API采集请求将被拒绝，并返回相应错误码：<br>
   - CPU限制：整机系统CPU占用率超过70%；<br>
   - 内存限制：整机系统可用内存不足20%；<br>
   - 存储限制：整机系统可用存储空间不足20%。<br>
3. 并发冲突约束：
   - 当API采集与命令行工具或系统采集任务发生冲突时，API采集请求将被拒绝，并返回相应错误码。<br>
4. 采集结果保存：<br>
   - 存储路径：采集到的调用栈日志文件将自动保存在应用沙箱/data/storage/el2/base/files/目录下。<br>
   - 文件名规则：”资源采集类型-进程名-进程号-时间戳.htrace”。<br>
5. 权限约束：<br>
   - 采集资源的目标进程仅支持调用接口进程本身，不支持跨进程采集。<br>

> **注意：**
>
> 1. 受到上述采集规格与约束限制，本API不保证采集请求一定成功。
> 2. 调用本API会对系统性能产生影响，请谨慎使用；影响类别包括但不限于：CPU占用率、内存占用、存储占用、系统响应速度等。开发者在集成本API时，必须谨慎评估其性能功耗开销，并建立严格的条件触发策略（如仅在特定灰度范围内、应用资源超基线阈值时触发，同时须配置合理的采集参数以兼顾性能功耗开销平衡），再决策是否开启，严禁在线上环境盲目或高频次触发。
...
| 类型 | 说明 |
| -- | -- |
| [HiDebug_ErrorCode](capi-hidebug-type-h.md#hidebug_errorcode) | 返回结果码：<br>        HIDEBUG_RES_PROF_SUCCESS：启动资源采集成功。<br>        HIDEBUG_RES_PROF_INVALID_ARG：资源采集参数无效。<br>        HIDEBUG_RES_PROF_INVALID_MAX_DURATION：资源采集最大持续时间参数无效。<br>        HIDEBUG_RES_PROF_INVALID_FILTER_SIZE：资源采集过滤大小参数无效。<br>        HIDEBUG_RES_PROF_INVALID_MAX_STACK_DEPTH：资源采集最大回栈深度参数无效。<br>        HIDEBUG_RES_PROF_INVALID_STATISTICS_INTERVAL：资源采集统计间隔参数无效。<br>        HIDEBUG_RES_PROF_INVALID_SAMPLE_INTERVAL：资源采集采样大小参数无效。<br>        HIDEBUG_RES_PROF_INVALID_RESOURCE_TYPE：资源采集类型参数无效。<br>        HIDEBUG_RES_PROF_PERMISSION_DENIED：资源采集权限不足，采集资源的目标进程仅支持调用接口进程本身。<br>        HIDEBUG_RES_PROF_ALREADY_STARTED：资源采集重复启动。<br>        HIDEBUG_RES_PROF_PROCESS_OVERLIMIT：资源采集进程数超出限制，详见[规格和约束](#oh_hidebug_startprofiler)。<br>        HIDEBUG_RES_PROF_CONFLICT：资源采集与命令行工具或系统采集任务冲突，详见[规格和约束](#oh_hidebug_startprofiler)。<br>        HIDEBUG_RES_PROF_DAILY_QUOTA_EXCEEDED：资源采集每日配额超出限制，详见[规格和约束](#oh_hidebug_startprofiler)。<br>        HIDEBUG_RES_PROF_CPU_OVERLOADED：系统 CPU 处于高负载状态，详见[规格和约束](#oh_hidebug_startprofiler)。<br>        HIDEBUG_RES_PROF_MEM_PRESSURE_CRITICAL：内存可用空间紧张，详见[规格和约束](#oh_hidebug_startprofiler)。<br>        HIDEBUG_RES_PROF_STORAGE_PRESSURE_CRITICAL：存储可用空间紧张，详见[规格和约束](#oh_hidebug_startprofiler)。<br>        HIDEBUG_RES_PROF_FAILURE：启动资源采集失败。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_HiDebug_ProfilerOptions* | 指向OH_HiDebug_ProfilerOptions结构体的指针。 |
...
释放通过[OH_HiDebug_CreateProfilerOptions](#oh_hidebug_createprofileroptions)创建的配置对象。
...
| 类型 | 说明 |
| -- | -- |
| [HiDebug_ErrorCode](capi-hidebug-type-h.md#hidebug_errorcode) | 返回结果码：<br>        HIDEBUG_SUCCESS：操作成功。<br>        HIDEBUG_RES_PROF_INVALID_ARG：opts为空指针。 |
...
| 类型 | 说明 |
| -- | -- |
| [HiDebug_ErrorCode](capi-hidebug-type-h.md#hidebug_errorcode) | 返回结果码：<br>        HIDEBUG_SUCCESS：操作成功。<br>        HIDEBUG_RES_PROF_INVALID_ARG：opts为空指针。<br>        HIDEBUG_RES_PROF_INVALID_MAX_ASYNC_NESTING_DEPTH：异步嵌套深度无效。 |
...
| 类型 | 说明 |
| -- | -- |
| [HiDebug_ErrorCode](capi-hidebug-type-h.md#hidebug_errorcode) | 返回结果码：<br>        HIDEBUG_SUCCESS：操作成功。<br>        HIDEBUG_RES_PROF_INVALID_ARG：opts为空指针。<br>        HIDEBUG_RES_PROF_INVALID_MAX_ASYNC_TASK_STACK_DEPTH：资源采集异步任务回栈深度参数无效。 |
...
| 类型 | 说明 |
| -- | -- |
| [HiDebug_ErrorCode](capi-hidebug-type-h.md#hidebug_errorcode) | 返回结果码：<br>        HIDEBUG_SUCCESS：操作成功。<br>        HIDEBUG_RES_PROF_INVALID_ARG：opts为空指针。<br>        HIDEBUG_RES_PROF_INVALID_SAMPLE_INTERVAL：采样间隔无效。 |
...
| 类型 | 说明 |
| -- | -- |
| [HiDebug_ErrorCode](capi-hidebug-type-h.md#hidebug_errorcode) | 返回结果码：<br>        HIDEBUG_SUCCESS：操作成功。<br>        HIDEBUG_RES_PROF_INVALID_ARG：opts为空指针。<br>        HIDEBUG_RES_PROF_INVALID_STATISTICS_INTERVAL：统计间隔无效。 |
...
| 类型 | 说明 |
| -- | -- |
| [HiDebug_ErrorCode](capi-hidebug-type-h.md#hidebug_errorcode) | 返回结果码：<br>        HIDEBUG_SUCCESS：操作成功。<br>        HIDEBUG_RES_PROF_INVALID_ARG：opts为空指针。<br>        HIDEBUG_RES_PROF_INVALID_MAX_STACK_DEPTH：最大回栈深度无效。 |
...
| 类型 | 说明 |
| -- | -- |
| [HiDebug_ErrorCode](capi-hidebug-type-h.md#hidebug_errorcode) | 返回结果码：<br>        HIDEBUG_SUCCESS：操作成功。<br>        HIDEBUG_RES_PROF_INVALID_ARG：opts为空指针。<br>        HIDEBUG_RES_PROF_INVALID_FILTER_SIZE：过滤大小无效。 |
...
| 类型 | 说明 |
| -- | -- |
| [HiDebug_ErrorCode](capi-hidebug-type-h.md#hidebug_errorcode) | 返回结果码：<br>        HIDEBUG_SUCCESS：操作成功。<br>        HIDEBUG_RES_PROF_INVALID_ARG：opts为空指针。<br>        HIDEBUG_RES_PROF_INVALID_MAX_DURATION：最大持续采集时间无效。 |
...
按指定类型和配置启动当前进程资源分配栈信息采集功能。须与[OH_HiDebug_StopProfiler](#oh_hidebug_stopprofiler)配对使用。
...
| 参数项 | 描述 |
| -- | -- |
| [OH_HiDebug_ResourceType](capi-hidebug-type-h.md#oh_hidebug_resourcetype) type | 资源采集类型（OH_HiDebug_ResourceType）。 |
| OH_HiDebug_ProfilerOptions* opts | 指向已配置的OH_HiDebug_ProfilerOptions结构体的指针，不能为NULL。 |
| [OH_HiDebug_ProfilingCallback](capi-hidebug-type-h.md#oh_hidebug_profilingcallback) callback | 接收采集结果的回调函数。 |

**返回：**

| 类型 | 说明 |
| -- | -- |
| [HiDebug_ErrorCode](capi-hidebug-type-h.md#hidebug_errorcode) | 返回结果码：<br>        HIDEBUG_RES_PROF_SUCCESS：启动资源采集成功。<br>        HIDEBUG_RES_PROF_INVALID_ARG：opts或callback为空指针。<br>        HIDEBUG_RES_PROF_INVALID_MAX_DURATION：最大持续采集时间参数无效。<br>        HIDEBUG_RES_PROF_INVALID_FILTER_SIZE：过滤大小参数无效。<br>        HIDEBUG_RES_PROF_INVALID_MAX_STACK_DEPTH：最大回栈深度参数无效。<br>        HIDEBUG_RES_PROF_INVALID_STATISTICS_INTERVAL：统计间隔参数无效。<br>        HIDEBUG_RES_PROF_INVALID_SAMPLE_INTERVAL：采样大小参数无效。<br>        HIDEBUG_RES_PROF_INVALID_MAX_ASYNC_NESTING_DEPTH：异步嵌套深度参数无效。<br>        HIDEBUG_RES_PROF_INVALID_MAX_ASYNC_TASK_STACK_DEPTH：异步任务回栈深度参数无效。<br>        HIDEBUG_RES_PROF_INVALID_RESOURCE_TYPE：资源采集类型参数无效。<br>        HIDEBUG_RES_PROF_PERMISSION_DENIED：资源采集权限不足。<br>        HIDEBUG_RES_PROF_ALREADY_STARTED：资源采集重复启动。<br>        HIDEBUG_RES_PROF_PROCESS_OVERLIMIT：资源采集进程数超出限制。<br>        HIDEBUG_RES_PROF_CONFLICT：资源采集与命令行工具或系统采集任务冲突。<br>        HIDEBUG_RES_PROF_DAILY_QUOTA_EXCEEDED：资源采集每日配额超出限制。<br>        HIDEBUG_RES_PROF_CPU_OVERLOADED：系统CPU处于高负载状态。<br>        HIDEBUG_RES_PROF_MEM_PRESSURE_CRITICAL：内存可用空间紧张。<br>        HIDEBUG_RES_PROF_STORAGE_PRESSURE_CRITICAL：存储可用空间紧张。<br>        HIDEBUG_RES_PROF_FAILURE：启动资源采集失败。 |
