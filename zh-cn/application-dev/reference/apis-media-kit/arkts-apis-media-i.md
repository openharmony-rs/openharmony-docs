...
| frameWidth        | number                                                       | 否   | 是   | 录屏的视频宽度。<br>默认屏幕宽度，根据不同屏幕默认值不同。<br>取值范围为[0, 10240]，单位为像素（px）。 |
| frameHeight       | number                                                       | 否   | 是   | 录屏的视频高度。<br>默认屏幕高度，根据不同屏幕默认值不同。<br>取值范围为[0, 4320]，单位为像素（px）。 |
| videoBitrate      | number                                                       | 否   | 是   | 录屏的视频比特率。<br>取值范围为[1, 30000000]，默认为10000000。                            |
| audioSampleRate   | number                                                       | 否   | 是   | 录屏的音频采样率。<br>内录的系统音和外录的麦克风都是用此采样率，取值范围为[12000, 192000]，默认48000，仅支持设置48000或16000。 |
| audioChannelCount | number                                                       | 否   | 是   | 录屏的音频通道数，内录的系统音和外录的麦克风都是用此通道数，取值范围为[1, 16]，默认2声道，仅支持设置1或2声道。 |
