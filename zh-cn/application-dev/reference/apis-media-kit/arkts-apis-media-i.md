...
| preferredBufferDuration | number | 否   | 是   | 播放策略首选缓冲持续时间，单位为秒（s），取值范围[1, 20]。超出范围时自动修正为边界值或使用默认值。<br>具体使用方式可参考[在线视频播放卡顿优化实践](<!--RP1-->https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-online-video-playback-lags-practice<!--RP1END-->)。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。 |
