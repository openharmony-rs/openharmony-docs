...
| 名称    | 类型                                 | 只读 | 可选 | 说明               |
| ------- | ------------------------------------ | ---- | ---- | ------------------ |
| state<sup>9+</sup> | [AVRecorderState](arkts-apis-media-t.md#avrecorderstate9) | 是   | 否   | 音视频录制的状态。<br>**原子化服务API（仅ArkTS-Dyn）：** 从API version 12开始，该接口支持在原子化服务中使用。 |
...
| 类型           | 说明                                       |
| -------------- | ------------------------------------------ |
| ArkTS-Dyn: Promise\<number>  <br/> ArkTS-Sta: Promise\<int>  | Promise对象，返回所添加水印的编号，取值范围[1, 5]。 |
...
| 类型             | 说明                             |
| ---------------- | -------------------------------- |
| ArkTS-Dyn: Promise\<string><br>ArkTS-Sta: Promise\<string \| undefined> | Promise对象，返回获取的surfaceId。 |
...
| 参数名   | 类型                 | 必填 | 说明                        |
| -------- | -------------------- | ---- | --------------------------- |
| rotation | ArkTS-Dyn: number<br>ArkTS-Sta: int | 是   | 旋转角度，单位为度（°）。取值仅支持0°、90°、180°和270°。传入不支持的角度值时，返回错误码401。|
