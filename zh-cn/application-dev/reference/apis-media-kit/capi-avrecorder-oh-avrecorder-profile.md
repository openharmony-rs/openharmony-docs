...
| bool enableTemporalScale | 是否支持时域分层编码功能。<br> true表示编码输出的码流中部分帧可以支持跳过不编码，false表示编码输出的码流中部分帧不支持跳过不编码，[详情见](/zh-cn/application-dev/media/avcodec/video-encoding-temporal-scalability.md)。<br> 默认是false。 |
