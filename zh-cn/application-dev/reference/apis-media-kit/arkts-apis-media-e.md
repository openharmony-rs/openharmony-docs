...
| LIP_ASYNC_COUNT                    | 'lip_async_count'                    | 表示总的音画不同步的次数。<br> **模型约束：** 此接口仅可在Stage模型下使用。       |
| TOTAL_LIP_ASYNC_TIME               | 'total_lip_async_time'               | 表示总的音画不同时长，单位为毫秒（ms）。<br> **模型约束：** 此接口仅可在Stage模型下使用。       |
