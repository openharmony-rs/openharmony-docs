...
<<<<<<< HEAD
| MD_KEY_BITRATE           | 'bitrate'       | 表示比特率，其对应键值类型为number，单位为比特率（bps），返回值为0表示异常。<br>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。|
=======
| MD_KEY_BITRATE           | 'bitrate'       | 表示比特率，其对应键值类型为number，单位为比特率（bps），返回值为0表示异常（从API version 21开始支持）。<br>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。|
>>>>>>> 2b6bbb0cf2c (修复视频轨码率获取_1117)
