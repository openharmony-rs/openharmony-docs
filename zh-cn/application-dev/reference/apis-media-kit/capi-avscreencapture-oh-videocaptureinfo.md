...
| int32_t *missionIDs | 指定窗口ID数组，设置后录制指定窗口内容。适用于仅录制特定应用窗口内容的场景，如录制单个应用操作演示、避免录制桌面背景和隐私信息等。使用该参数需要在captureMode为OH_CAPTURE_SPECIFIED_WINDOW模式下使用，其他模式下此参数不生效。可通过窗口接口[getWindowProperties](../../reference/apis-arkui/arkts-apis-window-Window.md#getwindowproperties9)获取有效的missionID值。列表长度需与missionIDsLen匹配，ID取值为整数。传入无效ID时录制失败。 |
