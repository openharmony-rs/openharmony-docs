...
| const int32_t VIDEO_PROCESSING_TYPE_AUTOEFFECT_AISR | 表示在XComponent中使用的视频AISR（Artificial Intelligence Super Resolution）显示效果增强类型。<br>调用[OH_VideoProcessing_IsAutoEffectSupported](capi-video-processing-h.md#oh_videoprocessing_isautoeffectsupported)查询是否支持AISR显示增强效果。<br>**起始版本：** 26.1.0 |
| const char *VIDEO_AUTOEFFECT_ENABLE | 设置启用或禁用显示增强效果的关键字。<br>调用[OH_AVFormat_SetIntValue](../apis-avcodec-kit/capi-native-avformat-h.md#oh_avformat_setintvalue)将启用值（0为false，1为true）设置到AVFormat参数中。调用[OH_VideoProcessing_SetAutoEffectParam](capi-video-processing-h.md#oh_videoprocessing_setautoeffectparam)将参数设置到视频处理实例中。<br>**起始版本：** 26.1.0 |
