...
| const int32_t VIDEO_PROCESSING_TYPE_AUTOEFFECT_AISR | 表示在XComponent中使用的视频AISR（Artificial Intelligence Super Resolution）显示效果增强类型。<br>调用[OH_VideoProcessing_IsAutoEffectSupported](capi-video-processing-h.md#oh_videoprocessing_isautoeffectsupported)查询是否支持AISR自动效果。<br>**起始版本：** 26.1.0 |
