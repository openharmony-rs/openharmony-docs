...
| const int32_t VIDEO_PROCESSING_TYPE_AUTOEFFECT_AISR | 表示在XComponent中使用的视频AISR显示效果增强类型。AISR在科技和影音领域最常指AI超分辨率（AI Super Resolution），是一种利用人工智能技术提升图像和视频清晰度的画质增强技术。<br>调用[OH_VideoProcessing_IsAutoEffectSupported](capi-video-processing-h.md#oh_videoprocessing_isautoeffectsupported)查询是否支持AISR自动效果。<br>**起始版本：** 26.1.0 |
| const char *VIDEO_AUTOEFFECT_ENABLE | 设置启用或禁用自动效果的关键字。<br>调用[OH_AVFormat_SetIntValue](../apis-avcodec-kit/capi-native-avformat-h.md#oh_avformat_setintvalue)将启用值（0为false，1为true）设置到AVFormat参数中。调用[OH_VideoProcessing_SetAutoEffectParam](capi-video-processing-h.md#oh_videoprocessing_setautoeffectparam)将参数设置到视频处理实例中。<br>**起始版本：** 26.1.0 |
