...
| 类型 | 说明 |
| --- | --- |
| number | 当前服务端接受的字节数。<br/><br/>- 返回值小于0表示操作失败。<br/><br/>- 返回值为-2时，表示播放器不再需要当前数据，客户端应停止当前读取过程。<br/><br/>- 返回值为-3时，表示播放器的缓冲区已满，客户端应等待下一次读取。 |
...
[respondData](media.MediaSourceLoadingRequest.respondData(uuid: number, offset: number, buffer: ArrayBuffer))
方法之前调用。
...
| --- | --- | --- | --- |
| uuid | number | 是 | 资源句柄的标识。来源是[SourceOpenCallback](@ohos.multimedia.media:media.SourceOpenCallback)。 |
| header | Record&lt;string, string&gt; | 否 | HTTP响应中的头部信息。应用可将头部信息字段与底层支持解析字段取交集传递或直接传入对应的所有头部信息。<br/>- 底层播放需要解析的<br/>字段包括Transfer-Encoding、Location、Content-Type、Content-Range、Content-Encode、Accept-Ranges、content-length。 |
