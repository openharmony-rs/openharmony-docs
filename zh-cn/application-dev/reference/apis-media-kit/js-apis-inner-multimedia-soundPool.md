...
| leftVolume  | number | 否 | 是  | 设置左声道音量。设置范围（0.0, 1.0），默认值为1.0。<br> 当音量超过边界值时自动设置为边界值。                       |

| rightVolume | number  | 否 | 是  | 设置右声道音量（当前不支持左右分别设置，将以左声道音量为准）。设置范围（0.0, 1.0），默认值为1.0。<br> 当音量超过边界值时自动设置为边界值。 |
