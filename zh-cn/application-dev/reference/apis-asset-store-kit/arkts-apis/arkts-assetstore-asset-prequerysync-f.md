...
查询的预处理，用于需要用户认证的关键资产。在用户认证成功后，应当随后调用[asset.querySync](arkts-assetstore-asset-querysync-f.md#querySync-1)、
[asset.postQuerySync](arkts-assetstore-asset-postquerysync-f.md#postQuerySync-1)。使用同步方式返回结果。
...
| 类型 | 说明 |
| --- | --- |
| Uint8Array | 挑战值。<br/><br/>**说明：** 挑战值用于后续用户认证。 |
