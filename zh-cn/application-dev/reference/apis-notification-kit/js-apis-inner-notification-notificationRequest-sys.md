...
| 名称                 | 类型                 | 只读 | 可选 | 说明                                     |
| -------------------- | -------------------- | ---- | ---- | ---------------------------------------- |
| isGroupIcon | boolean | 否 | 是 | 是否用该通知的左图标作为通知成组后的组图标。假如该通知是通知组里面最新的一条通知，且开发者传入smallIcon, 是否使用smallIcon作为组通知的图标。true：使用smallIcon作为组通知的图标； false：不使用smallIcon作为组通知的图标。|
