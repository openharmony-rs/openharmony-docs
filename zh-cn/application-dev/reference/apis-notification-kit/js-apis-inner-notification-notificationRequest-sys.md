...
| 名称                   | 类型            | 可填 | 说明                               |
| ---------------------- | -------------- | ---- | ---------------------------------- |
| remindType             | ArkTS-Dyn: number<br/>ArkTS-Sta: int  | 是   | 通知的提醒方式。<br>**系统接口**：此接口为系统接口。  |
...
| 名称            | 类型                                   | 可选 | 说明                               |
| ----------------| ------------------------------------- | ---- | ---------------------------------- |
| bundle          | [BundleOption](js-apis-inner-notification-notificationCommonDef.md#bundleoption) | 否   | 实况通知的包信息。|
| notificationKey | [NotificationKey](js-apis-notificationSubscribe-sys.md#notificationkey) | 否   | 通知信息，包含通知ID和通知标签。   |
| extraInfoKeys   | Array\<string>                        | 是   | 筛选附加信息的键值列表。不填表示查询所有的附加信息。|
...
| 名称          | 类型                                                       | 可选 | 说明              |
| --------------| --------------------------------------------------------- | ---- | ----------------- |
| contentType   | [ContentType](js-apis-notificationManager.md#contenttype) | 否   | 通知类型。         |
| slotType      | [SlotType](js-apis-notificationManager.md#slottype)       | 否   | 渠道类型。         |
| extraInfoKeys | Array\<string>                                            | 否   | 实况通知的附加信息。|
...
| ---------------------- | -------------- | ---- | ---------------------------------- |
| key          | string        | 是   | 聚合组ID。                   |
| title  | string | 是   | 聚合组标题。            |
| content  | string | 是   | 聚合组摘要正文。              |
| sceneName          | string        |  是   | 聚合场景名称。                   |
| extraInfo  | ArkTS-Dyn: {[key: string]: any} <br/>ArkTS-Sta: Record<string, RecordData> | 是   | 其他聚合信息。            |
