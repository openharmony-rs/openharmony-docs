...
| appIndex             | ArkTS-Dyn: number<br>ArkTS-Sta: int | 是   | 否   | 创建通知的应用包的分身索引标识，仅在分身应用中生效。|
| appName              | string | 是   | 是   | 创建通知的应用程序名称。                          |
| deliveryTime         | ArkTS-Dyn: number<br>ArkTS-Sta: long | 是   | 是   | 通知发布的时间戳。<br>数据格式：时间戳。<br>单位：ms。|
