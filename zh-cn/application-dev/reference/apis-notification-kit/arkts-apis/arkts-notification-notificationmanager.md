...
| cancel | 根据指定的通知ID取消已发布的通知。使用callback异步回调。 |
...
| getActiveNotifications | 获取当前应用未删除的通知列表。使用callback异步回调。 |
...
| isSupportTemplate | 在使用[通知模板](arkts-notification-notificationtemplate-notificationtemplate-i.md)发布通知前，可以通过该接口查询是否支持对应的通知模板。使用callback异步回调。 |
...
| setBadgeNumber | 设定角标个数，在应用的桌面图标上呈现。使用callback异步回调。 |
