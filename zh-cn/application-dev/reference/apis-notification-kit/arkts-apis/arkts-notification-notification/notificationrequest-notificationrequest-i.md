...
应用图标上显示的通知数，该数量累计展示，默认值为0。 当\_\_\_INLINE\_CODE\_DESC\_USD\_0\_\_\_取值小于或等于0时，将忽略本次角标设定。 当角标累加设定个数取值大于99时，通知角标将显示99+。 例如，应用发布3条通知，\_\_\_INLINE\_CODE\_DESC\_USD\_1\_\_\_依次设置为2、0、3，应用将依次展示为2、2、5。
...
扩展参数。为应用提供定制服务。默认为空。 以下Key由系统赋值，开发者手动修改也不会生效，系统在数据传递时会自动修改为实际值。 - 'ohos.notificationManager.wantUri'：用户点击通知时传递给应用的[Want]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ 中的uri字段，使用 [getActiveNotifications]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ 接口获取该信息。
...
发布或更新该通知时，是否只进行一次通知提醒，默认值为false。 - true：仅首次发布通知时进行提醒，后续更新该通知时，提醒方式变更为[LEVEL\_LOW]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_。 - false：每次均按照配置的通知提醒方式进行提醒。
...
通知大图标，默认为空。图标像素的总字节数不超过192KB（图标像素的总字节数通过 [getPixelBytesNumber]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_获取），超出后设置不生效。 未设置\_\_\_INLINE\_CODE\_DESC\_USD\_0\_\_\_时，通知将不展示大图标。建议图标像素长宽为128*128。 实际显示效果依赖于设备能力和通知中心UI样式。
...
通知标志位设置，默认为空。从API version 23开始成为可写参数，设置该参数可削减通知的提醒方式，当通知渠道类型为 [LIVE\_VIEW]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_时，该参数设置不生效。
...
通知重叠图标，默认为空。图标像素的总字节数不超过192KB（图标像素的总字节数通过 [getPixelBytesNumber]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_获取），超出后设置不生效。 未设置\_\_\_INLINE\_CODE\_DESC\_USD\_0\_\_\_时，通知将不展示重叠图标。 此接口只在[notificationSlotType]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_类型设置为SOCIAL\_COMMUNICATION时生效。 建议图标像素长宽为128*128。实际显示效果依赖于设备能力和通知中心UI样式。
...
封装了应用的行为意图，移除通知时触发该行为，默认为空。 当前不支持跳转UIAbility，只支持发布公共事件（即[WantAgentInfo]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_的actionType字段取值为4）。
...
通知渠道类型，默认值为OTHER\_TYPES。 从API version 7开始支持，从API version 11开始废弃，建议使用notificationSlotType替代。
...
通知小图标，默认为空。图标像素的总字节数不超过192KB（图标像素的总字节数通过 [getPixelBytesNumber]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_获取），超出后设置不生效。 未设置\_\_\_INLINE\_CODE\_DESC\_USD\_0\_\_\_时，通知将展示应用默认图标。建议图标像素长宽为128*128。 实际显示效果依赖于设备能力和通知中心UI样式。
