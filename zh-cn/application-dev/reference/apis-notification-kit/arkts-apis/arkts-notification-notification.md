...
| 名称 | 说明 |
| --- | --- |
| addSlot | 创建指定类型的通知通道（callback形式）。 |
| addSlot | 创建指定类型的通知通道（Promise形式）。 |
| cancel | 取消与指定通知ID相匹配的已发布通知（callback形式）。 |
| cancel | 通过通知ID和通知标签取消已发布的通知（callback形式）。 |
| cancel | 取消与指定通知ID相匹配的已发布通知，label可以指定也可以不指定（Promise形式）。 |
| cancelAll | 取消所有已发布的通知（callback形式）。 |
| cancelAll | 取消所有已发布的通知（Promise形式）。 |
| cancelGroup | 取消本应用指定组下的通知（Callback形式）。 |
| cancelGroup | 取消本应用指定组下的通知（Promise形式）。 |
| getActiveNotificationCount | 获取当前应用未删除的通知数（Callback形式）。 |
| getActiveNotificationCount | 获取当前应用未删除的通知数（Promise形式）。 |
| getActiveNotifications | 获取当前应用未删除的通知列表（Callback形式）。 |
| getActiveNotifications | 获取当前应用未删除的通知列表（Promise形式）。 |
| getSlot | 获取一个指定类型的通知通道（callback形式）。 |
| getSlot | 获取一个指定类型的通知通道（Promise形式）。 |
| getSlots | 获取此应用程序的所有通知通道（callback形式）。 |
| getSlots | 获取此应用程序的所有通知通道（Promise形式）。 |
| isDistributedEnabled | 查询设备是否支持分布式通知（Callback形式）。 |
| isDistributedEnabled | 查询设备是否支持分布式通知（Promise形式）。 |
...
| isSupportTemplate | 在使用[通知模板](arkts-notification-notificationtemplate-notificationtemplate-i.md)发布通知前，可以通过该接口查询是否支持对应的通知模板。使用Promise异步回调。 |
| publish | 发布通知（callback形式）。 |
| publish | 发布通知（Promise形式）。 |
| removeAllSlots | 删除所有通知通道（callback形式）。 |
| removeAllSlots | 删除所有通知通道（Promise形式）。 |
| removeSlot | 删除指定类型的通知通道（callback形式）。 |
| removeSlot | 删除指定类型的通知通道（Promise形式）。 |
| requestEnableNotification | 应用请求通知使能（Callback形式）。 |
| requestEnableNotification | 应用请求通知使能（Promise形式）。 |
