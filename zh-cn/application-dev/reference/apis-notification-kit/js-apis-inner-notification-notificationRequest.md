...
| notificationFlags<sup>8+</sup> | [NotificationFlags](js-apis-inner-notification-notificationFlags.md) |   否  | 是  | 设置或获取NotificationFlags，默认为空。从API version 23开始成为可写参数，设置该参数可削减通知的提醒方式，但当通知渠道类型为[LIVE_VIEW](js-apis-notificationManager.md#slottype)时，该参数设置不生效。 |
...
| sound<sup>12+</sup>           | string                                                   |   否  | 是  | 应用通知自定义铃声文件名，默认为空。该文件必须放在resources/rawfile目录下，支持m4a、aac、mp3、ogg、wav、flac、amr等格式。 |
