...
| extraInfo                     | {[key: string]: any}                                     |   否  | 是  | 扩展参数。为应用提供定制服务。<br/>以下Key均由系统赋值，开发者手动修改也不会生效，系统在数据传递时会自动修改为实际值。<br/>ohos.notificationManager.wantUri：用户点击通知时传递给app的wantAgent的wants字段；使用getActiveNotifications接口获取该信息。 |
