...
表示提供自动填充类型的枚举。该模块定义了多种自动填充类型（如地址、姓名、手机号、邮箱、银行卡号、身份证号等），支持应用在用户输入场景中快速识别和自动填充常用信息，减少用户手动输入的工作量，提升用户体验。

> **说明：**
> 
> 本模块首批接口从API version 11开始支持。后续版本的新增接口，采用上角标单独标记接口的起始版本。
>
> 当前页面仅包含本模块的系统接口，其他公开接口参见[AutoFillType (自动填充类型)](js-apis-inner-application-autoFillType.md)。
