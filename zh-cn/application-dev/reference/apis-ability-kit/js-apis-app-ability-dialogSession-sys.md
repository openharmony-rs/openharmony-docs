...
| appIndex<sup>12+</sup> | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否 | 否 | 表示应用的分身索引。取值从0开始，0表示主应用实例，1及以上表示分身实例。<br>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 23 |
...
| codePath<sup>23+</sup> | string | 否 | 是 | 表示应用程序的安装目录。当需要获取应用安装路径时传入，不传入时此字段为空。<br>**ArkTS-Dyn起始版本：** 23<br/>**ArkTS-Sta起始版本：** 23 |
