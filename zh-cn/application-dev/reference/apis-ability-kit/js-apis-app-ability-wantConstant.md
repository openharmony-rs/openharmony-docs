...
| 名称                    | 值                                 | 说明                                                                           |
| ----------------------- | ---------------------------------- | ------------------------------------------------------------------------------ |
| ABILITY_BACK_TO_OTHER_MISSION_STACK   | ability.params.backToOtherMissionStack     | 表示是否支持跨任务栈返回。<br>该参数用于控制跨应用的UIAbility返回逻辑，其核心作用是改变用户执行返回键时的应用跳转行为。例如现有UIAbility A和UIAbility B，当前前台显示的是UIAbility A，随后系统服务又拉起UIAbility B（同时在Want的Params字段配置该参数为true），那么，当UIAbility B退出时，会返回到UIAbility A（即返回到最近一次的访问任务）。如果未配置该参数，则默认直接返回桌面。需要注意的是，该字段仅支持系统设置，三方应用传入该字段不生效。<br>**原子化服务API**：从API version 11开始，该接口支持在原子化服务中使用。  |
...
| APP_CLONE_INDEX_KEY<sup>12+</sup>  | ohos.extra.param.key.appCloneIndex  | 表示分身应用索引。分身应用是指同一应用的多个独立实例，系统为每个实例分配唯一的索引值，用于区分不同的应用实例。在应用跳转场景中，开发者可通过设置该参数指定目标分身应用。<br>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。 |
...
| CALLER_APP_CLONE_INDEX<sup>14+</sup>  | ohos.param.callerAppCloneIndex  | 表示拉起方应用的分身索引，参考[创建应用分身](../../quick-start/app-clone.md)。|
...
| DESTINATION_PLUGIN_ABILITY<sup>19+</sup>  | ohos.params.pluginAbility  | 表示目标Ability是插件Ability，设置该Flag后，系统将目标Ability识别为插件Ability，并以插件模式进行拉起。在拉起插件Ability时，开发者需要在Want中将该字段设置为true，以此标识目标Ability的类型。插件Ability具备专属的生命周期与运行模式，和普通Ability存在区别。 |
| ATOMIC_SERVICE_SHARE_ROUTER<sup>20+</sup> | ohos.params.atomicservice.shareRouter | 表示被拉起的原子化服务的页面栈信息，用于指定跳转的目标页面。值为string类型的页面栈信息，开发者可以通过[UIAbilityContext (UIAbility上下文)](js-apis-inner-application-uiAbilityContext.md)获取当前页面栈信息并传递。仅当拉起方为UIAbilityContext，被拉起方为原子化服务时生效。<br>例如，某原子化服务中包含首页和第2页，如果希望直接拉起原子化服务的第2页，可以在拉起原子化服务时通过该字段传递第2页的页面栈信息。<br>**说明**：从API version 26.0.0开始，如果拉起方具有ohos.permission.START_ABILITY_TO_PAGE权限，目标方不是原子化服务也能生效。<br>**原子化服务API**：从API version 20开始，该接口支持在原子化服务中使用。 |
