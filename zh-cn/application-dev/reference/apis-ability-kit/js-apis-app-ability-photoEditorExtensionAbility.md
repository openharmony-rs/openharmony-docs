...
| 参数名 |  类型 | 必填  | 说明  |
| ------------ | ------------ | ------------ | ------------ |
|  uri |  string |  是 | 待编辑的原始图片[uri](../apis-core-file-kit/js-apis-file-fileuri.md)，格式为file://\<bundleName>/\<sandboxPath>，其中bundleName表示应用包名，sandboxPath表示应用沙箱路径。  |
| want  | [Want](./js-apis-app-ability-want.md)  | 是  | 当前PhotoEditorExtensionAbility的Want类型信息，包括ability名称、bundle名称等，用于传递启动图片编辑扩展页面所需的配置信息。 |
|  session |  [UIExtensionContentSession](./js-apis-app-ability-uiExtensionContentSession.md) | 是  |  PhotoEditorExtensionAbility界面内容相关信息，用于管理与调用方的会话交互，包括发送数据、终止会话等能力。|
...
当PhotoEditorExtensionAbility从后台转到前台时，系统会触发该回调。开发者可以在该回调内执行页面刷新、资源加载等前台相关的业务逻辑操作。
...
当PhotoEditorExtensionAbility从前台转到后台时，系统会触发该回调。开发者可以在该回调内执行资源释放、状态保存等后台相关的业务逻辑操作。
