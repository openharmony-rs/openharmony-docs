...
| [notifySaveAsResult](arkts-ability-abilitymanager-notifysaveasresult-f-sys.md) | 该接口仅供[DLP](../../apis-data-protection-kit/arkts-apis/arkts-dataprotection-dlppermission.md)（Data Loss Prevention, 数据丢失防护）管理应用使用，其他应用禁止使用，DLP管理应用通过该接口通知沙箱应用另存为结果。使用Promise异步回调。 |
