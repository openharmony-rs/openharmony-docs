...
| uris | Array\<[SkillUri](#skilluri)> | 是   | 否   | Want匹配的Uri集合。 |
...
| port            | ArkTS-Dyn: number<br>ArkTS-Sta: string | 是   | 否   | 标识 URI 端口，仅当 scheme 和 host 同时存在时才生效。   |
...
| type            | string | 是   | 否   | 标识与Want相匹配的数据类型，使用MIME（Multipurpose&nbsp;Internet&nbsp;Mail&nbsp;Extensions）类型规范和[UniformDataType](../../reference/apis-arkdata/js-apis-data-uniformTypeDescriptor.md#uniformdatatype)类型规范。 |
