...
与[ErrorObserver.onException](#errorobserveronexception)的差异在于：onUnhandledException仅捕获未处理的异常，参数仅包含错误消息字符串；而onException会捕获所有上报到JS层的异常，参数为完整的Error对象，包含name、message、stack等更多信息。建议在需要完整错误信息时使用onException，仅需简单错误消息时使用onUnhandledException。二者可组合使用。
