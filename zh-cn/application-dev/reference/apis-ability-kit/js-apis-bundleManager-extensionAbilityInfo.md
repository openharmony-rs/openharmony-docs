...
| labelId              | ArkTS-Dyn: number<br>ArkTS-Sta: long | 是   | 否   | ExtensionAbility的标签资源ID。                       |
| descriptionId        | ArkTS-Dyn: number<br>ArkTS-Sta: long | 是   | 否   | ExtensionAbility的描述资源ID。                       |
| iconId               | ArkTS-Dyn: number<br>ArkTS-Sta: long | 是   | 否   | ExtensionAbility的图标资源ID。                       |
...
| applicationInfo      | ArkTS-Dyn: [ApplicationInfo](js-apis-bundleManager-applicationInfo.md)<br>ArkTS-Sta: [ApplicationInfo](js-apis-bundleManager-applicationInfo.md) \| null | 是   | 否   | 应用程序的配置信息。通过调用[getBundleInfoForSelf](js-apis-bundleManager.md#bundlemanagergetbundleinfoforself)接口，bundleFlags参数传入GET_BUNDLE_INFO_WITH_HAP_MODULE、GET_BUNDLE_INFO_WITH_EXTENSION_ABILITY和GET_BUNDLE_INFO_WITH_APPLICATION获取。 |
