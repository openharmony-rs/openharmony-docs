...
| -------- | -------- | -------- | -------- |
| want | [Want](js-apis-app-ability-want.md) | 是 | 启动Ability的Want信息。 |
| accountId | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 是 | 系统账号的账号ID，详情参考[getCreatedOsAccountsCount](../apis-basic-services-kit/js-apis-osAccount.md#getcreatedosaccountscountdeprecated)。 |
...
| -------- | -------- | -------- | -------- |
| want | [Want](js-apis-app-ability-want.md) | 是 | 启动Ability的Want信息。 |
| accountId | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 是 | 系统账号的账号ID，详情参考[getCreatedOsAccountsCount](../apis-basic-services-kit/js-apis-osAccount.md#getcreatedosaccountscountdeprecated-1)。 |
| options | [StartOptions](js-apis-app-ability-startOptions.md) | 否 | 启动Ability所携带的参数。 |
...
| -------- | -------- | -------- | -------- |
| want | [Want](js-apis-app-ability-want.md) | 是 | 停止Ability的Want信息。 |
| accountId | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 是 | 需要停止的系统账号的账号ID，详情参考[getCreatedOsAccountsCount](../apis-basic-services-kit/js-apis-osAccount.md#getcreatedosaccountscountdeprecated)。 |
...
| -------- | -------- | -------- | -------- |
| want | [Want](js-apis-app-ability-want.md) | 是 | 停止Ability的Want信息。 |
| accountId | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 是 | 需要停止的系统账号的账号ID，详情参考[getCreatedOsAccountsCount](../apis-basic-services-kit/js-apis-osAccount.md#getcreatedosaccountscountdeprecated-1)。 |
...
| 类型 | 说明 |
| -------- | -------- |
| ArkTS-Dyn: number<br/>ArkTS-Sta: long | 返回一个数值，后续根据这个数值去断开连接。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| -------- | -------- | -------- | -------- |
| connection | ArkTS-Dyn: number<br/>ArkTS-Sta: long | 是 | connectServiceExtensionAbility中的返回值。 |
...
ArkTS-Dyn: startAbilityByCallWithAccount(want: Want, accountId: number): Promise&lt;Caller&gt;

ArkTS-Sta: startAbilityByCallWithAccount(want: Want, accountId: int): Promise&lt;Caller&gt;
...
| accountId | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 是 | 系统账号的账号ID，-1表示当前活动用户，详情参考[getCreatedOsAccountsCount](../apis-basic-services-kit/js-apis-osAccount.md#getcreatedosaccountscountdeprecated-1)。 |
...
export default class ServiceExtAbility extends ServiceExtensionAbility {
  ...
  onRequest(want: Want, startId: number) {
    ...
    try {
      ...
      log(`exception occured, ${JSON.stringify(e)}`);
    }
  }
  ...
  onRequest(want: Want, startId: number) {
    ...
    try {
      ...
      if (!bundleName || !abilityName || !moduleName) {
        log(`Missing required parameters: bundleName=${bundleName}, abilityName=${abilityName}, abilityName=${moduleName}`);
        return;
      }
      ...
      this.context.preStartMission(
        ...
        log(`pre-start mission failed, errCode ${JSON.stringify(err)}`);
      });
    } catch (e) {
      log(`exception occured, ${JSON.stringify(e)}`);
    }
