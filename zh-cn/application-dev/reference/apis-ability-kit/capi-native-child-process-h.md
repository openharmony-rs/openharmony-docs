...
| [bool OH_Ability_IsChildProcessSupported()](#oh_ability_ischildprocesssupported) | - | 查询是否允许创建子进程。 |
...
**设备行为差异：** 对于API 13及之前版本，该接口在PC/2in1设备中可正常使用，在其他设备类型中返回[NCP_ERR_NOT_SUPPORTED](#ability_nativechildprocess_errcode)错误码。对于API 14开始，该接口在PC/2in1、Tablet中可正常使用，在其他设备类型中返回[NCP_ERR_NOT_SUPPORTED](#ability_nativechildprocess_errcode)错误码。对于API 23开始，该接口在PC/2in1、Tablet、car中可正常使用，在其他设备类型中返回[NCP_ERR_NOT_SUPPORTED](#ability_nativechildprocess_errcode)错误码。
...
### OH_Ability_IsChildProcessSupported()

```
bool OH_Ability_IsChildProcessSupported()
```

**描述**

检查是否允许调用者在此设备上创建本机子进程。
...
| 类型 | 说明 |
| -- | -- |
| boolean | 如果允许调用者创建本机子进程,则为true,否则为false。 |