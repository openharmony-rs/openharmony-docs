...
场景说明：
- 系统应用访问敏感权限时，需要在使用开始时调用startUsingPermission，在使用结束时调用stopUsingPermission，以便系统感知对应的权限使用状态变化；
- 系统应用访问敏感权限后，可以调用addPermissionUsedRecord，以便系统记录对应的敏感权限访问事件。
...
场景说明：系统应用需要感知敏感权限当前是否处于使用中，或在权限使用状态变化时联动业务流程，可组合使用[checkPermissionInUse](#privacymanagercheckpermissioninuse)、[on](#privacymanageron)和[off](#privacymanageroff)。
...
| 参数名   | 类型                 | 必填 | 说明                                       |
| -------- | -------------------  | ---- | ------------------------------------------ |
| tokenID   | ArkTS-Dyn: number <br> ArkTS-Sta: int | 是   | 调用方的应用身份标识。可通过应用[BundleInfo](js-apis-bundleManager-bundleInfo.md)中的[ApplicationInfo](js-apis-bundleManager-applicationInfo.md#applicationinfo-1)的accessTokenId字段获取。该参数必须为大于0的整数，传入0时返回错误码12100001。|
| permissionName | [Permissions](../../security/AccessToken/app-permissions.md) | 是   | 需要记录的权限名称。权限名长度不能超过256个字符，传入无效值时返回错误码12100001。 |
| successCount | ArkTS-Dyn: number <br> ArkTS-Sta: int | 是   | 访问成功的次数，取值必须为非负整数。传入无效值时返回错误码12100001。 |
| failCount | ArkTS-Dyn: number <br> ArkTS-Sta: int | 是   | 访问失败的次数，取值必须为非负整数。传入无效值时返回错误码12100001。 |
| options<sup>12+</sup> | [AddPermissionUsedRecordOptions](#addpermissionusedrecordoptions12) | 否   | 添加权限使用记录可选参数，用于指定敏感权限使用类型和扩展身份。当需要区分权限访问方式（如通过Picker或安全控件访问）或标识调用方扩展身份时传入此参数。<br>usedType：从API version 12开始支持。<br>默认值：NORMAL_TYPE。<br>enhancedIdentity：从API版本26.0.0开始支持，长度不超过48个字符。<br>默认值：""。<br>**ArkTS-Dyn起始版本：** 12<br>**ArkTS-Sta起始版本：** 23 |
...
| 202 | Not system app. Interface caller is not a system app. |
| 401 | Parameter error. Possible causes: 1.Mandatory parameters are left unspecified; 2.Incorrect parameter types. |
| 12100006 | Operation not allowed. The toggle status of the specified permission has already been set by [setPermissionUsedRecordToggleStatus](#privacymanagersetpermissionusedrecordtogglestatus).<br>适用版本：26.0.1+ |
| 12100007 | Service exception. |
...
| 201 | Permission denied. Interface caller does not have permission "ohos.permission.PERMISSION_USED_STATS". |
| 202 | Not system app. Interface caller is not a system app. |
| 12100004 | This API must be used together with [setPermissionUsedRecordToggleStatus](#privacymanagersetpermissionusedrecordtogglestatus18).<br>适用版本：26.0.1+ |
| 12100007 | Service exception. |
...
| permissionName | [Permissions](../../security/AccessToken/app-permissions.md) | 是   | 需要使用的权限名称。权限名长度不能超过256个字符，传入无效值时返回错误码12100001。|
| pid            | ArkTS-Dyn: number <br> ArkTS-Sta: int | 否   | 调用方的进程pid，用于根据进程生命周期管理权限使用状态。当需要精确控制特定进程的权限使用状态（例如进程退出时自动停止权限使用）时传入此参数。<br>默认值：-1，表示不根据进程生命周期响应。|
...
ArkTS-Dyn: startUsingPermission(tokenID: number, permissionName: Permissions, pid?: number, usedType?: PermissionUsedType, options?: PermissionUsingOptions): Promise&lt;void&gt;

ArkTS-Sta: startUsingPermission(tokenID: int, permissionName: Permissions, pid?: int, usedType?: PermissionUsedType, options?: PermissionUsingOptions): Promise&lt;void&gt;
...
ArkTS-Dyn: stopUsingPermission(tokenID: number, permissionName: Permissions, pid?: number, options?: PermissionUsingOptions): Promise&lt;void&gt;

ArkTS-Sta: stopUsingPermission(tokenID: int, permissionName: Permissions, pid?: int, options?: PermissionUsingOptions): Promise&lt;void&gt;
...
| tokenID        | ArkTS-Dyn: number <br> ArkTS-Sta: int | 是   | 目标应用的身份标识。可通过应用[BundleInfo](js-apis-bundleManager-bundleInfo.md)中的[ApplicationInfo](js-apis-bundleManager-applicationInfo.md#applicationinfo-1)的accessTokenId字段获取。该参数必须为大于0的整数，传入0时返回错误码12100001。|
| permissionName | [Permissions](../../security/AccessToken/app-permissions.md) | 是   | 需要停止使用的权限名称。权限名长度不能超过256个字符，传入无效值时返回错误码12100001。|
| pid            | ArkTS-Dyn: number <br> ArkTS-Sta: int | 否   | 调用方的进程pid，需与[startUsingPermission](#privacymanagerstartusingpermission-1)传入的pid相同。不满足配套关系可能导致API调用失败（错误码12100004）。<br>默认值：-1，表示不根据进程生命周期响应。 |
| options        | [PermissionUsingOptions](#permissionusingoptions)| 否 | 权限使用可选参数，用于指定扩展身份。当需要标识调用方的扩展身份信息时传入此参数。<br>默认值：enhancedIdentity为空字符串。 |
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[onActiveStateChange](#privacymanageronactivestatechange23)。
...
| permissionList | Array&lt;[Permissions](../../security/AccessToken/app-permissions.md)&gt;   | 是   | 订阅的权限名列表，数组长度不能超过1024。为空时表示订阅所有的权限使用状态变化，合法的权限名取值可在[应用权限列表](../../security/AccessToken/app-permissions.md)中查询。传入无效值时返回错误码12100001。|
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[offActiveStateChange](#privacymanageroffactivestatechange23)。
...
## privacyManager.onActiveStateChange<sup>23+</sup>
...
## privacyManager.offActiveStateChange<sup>23+</sup>
...
| 参数名             | 类型                   | 必填 | 说明                                                          |
| ------------------ | --------------------- | ---- | ------------------------------------------------------------ |
| permissionList | Array&lt;[Permissions](../../security/AccessToken/app-permissions.md)&gt; | 是   | 取消订阅的权限名列表，为空时表示订阅所有的权限状态变化，必须与[onActiveStateChange](#privacymanageronactivestatechange23)的输入一致。|
...
| 参数名             | 类型                   | 必填 | 说明                                                          |
| ------------------ | --------------------- | ---- | ------------------------------------------------------------ |
| tokenId            | ArkTS-Dyn: number \| null <br> ArkTS-Sta: int \| null | 否   | 访问敏感权限的应用身份标识。可通过应用[BundleInfo](js-apis-bundleManager-bundleInfo.md)中的[ApplicationInfo](js-apis-bundleManager-applicationInfo.md#applicationinfo-1)的accessTokenId字段获取。当需要查询特定应用的敏感权限访问类型信息时传入具体的tokenId；为0或null时表示查询所有应用的敏感权限访问类型信息。从API version 20开始，新增支持null类型。<br>默认值：0。   |
...
| 名称       | 类型             | 只读 | 可选 | 说明                                       |
| -------- | -------------- | ---- | ---- | ---------------------------------------- |
| tokenId  | ArkTS-Dyn: number <br> ArkTS-Sta: int | 否    | 是    | 目标应用的身份标识。可通过应用[BundleInfo](js-apis-bundleManager-bundleInfo.md)中的[ApplicationInfo](js-apis-bundleManager-applicationInfo.md#applicationinfo-1)的accessTokenId字段获取。<br/>默认值：0，查询所有应用。         |
...
| beginTime | ArkTS-Dyn: number <br> ArkTS-Sta: long | 否    | 是    | 查询的起始时间，单位：ms。<br>默认值：0，表示不限制起始时间。 |
| endTime | ArkTS-Dyn: number <br> ArkTS-Sta: long | 否    | 是    | 查询的终止时间，单位：ms。应不早于beginTime。<br>默认值：0，表示不限制终止时间。 |
...
| 名称       | 类型             | 只读 | 可选 | 说明                                       |
| -------- | -------------- | ---- | ---- | ---------------------------------------- |
| tokenId  | ArkTS-Dyn: number <br> ArkTS-Sta: int | 否    | 否    | 使用权限的应用身份标识。分布式场景下该字段无效，需结合deviceId和deviceName识别来源设备。                                 |
...
| 名称       | 类型             | 只读 | 可选 | 说明                                       |
| -------- | -------------- | ---- | ---- | ---------------------------------------- |
| permissionName  | [Permissions](../../security/AccessToken/app-permissions.md)         | 否    | 否    | 权限名，用于标识当前统计记录对应的敏感权限。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| accessCount | ArkTS-Dyn: number <br> ArkTS-Sta: int | 否    | 否    | 该权限访问总次数，表示在查询时间窗口内成功使用该权限的累计次数。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| rejectCount | ArkTS-Dyn: number <br> ArkTS-Sta: int | 否    | 否    | 该权限拒绝总次数，表示在查询时间窗口内权限访问失败或被拒绝的累计次数。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| lastAccessTime | ArkTS-Dyn: number <br> ArkTS-Sta: long | 否    | 否    | 最后一次访问时间，单位：ms。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| lastRejectTime | ArkTS-Dyn: number <br> ArkTS-Sta: long | 否    | 否    | 最后一次拒绝时间，单位：ms。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| lastAccessDuration | ArkTS-Dyn: number <br> ArkTS-Sta: long | 否    | 否    | 最后一次访问时长，单位：ms。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| accessRecords  | Array&lt;[UsedRecordDetail](#usedrecorddetail)&gt;         | 否    | 否    | 访问记录集合，仅当flag为FLAG_PERMISSION_USAGE_DETAIL时生效。<br>默认值：查询最近10条成功访问记录。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| rejectRecords  | Array&lt;[UsedRecordDetail](#usedrecorddetail)&gt;         | 否    | 否    | 拒绝记录集合，仅当flag为FLAG_PERMISSION_USAGE_DETAIL时生效。<br>默认值：查询最近10条失败或拒绝记录。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| enhancedIdentity| string | 否 | 是 | 扩展身份，长度不超过48个字符。<br>**ArkTS-Dyn起始版本：** 26.0.0<br>**ArkTS-Sta起始版本：** 26.0.0<br>**模型约束：** 此接口仅可在Stage模型下使用。 |
...
| 名称       | 类型             | 只读 | 可选 | 说明                                       |
| -------- | -------------- | ---- | ---- | ---------------------------------------- |
| status  | ArkTS-Dyn: number <br> ArkTS-Sta: int | 否    | 否    | 访问状态。0表示停止使用，1表示前台使用，2表示后台使用。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| lockScreenStatus<sup>11+</sup>  | ArkTS-Dyn: number <br> ArkTS-Sta: int | 否    | 是    | 访问时的锁屏状态。<br> - 1，表示非锁屏场景使用权限。<br> - 2，表示锁屏场景使用权限。<br>默认值：1。<br>**ArkTS-Dyn起始版本：** 11<br>**ArkTS-Sta起始版本：** 23 |
| timestamp | nArkTS-Dyn: number <br> ArkTS-Sta: long | 否    | 否    | 访问时的时间戳，单位：ms。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| accessDuration  | ArkTS-Dyn: number <br> ArkTS-Sta: long | 否    | 否    | 访问时长，单位：ms。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| count<sup>11+</sup> | ArkTS-Dyn: number <br> ArkTS-Sta: int | 否 | 是    | 访问次数。在accessRecords中表示成功访问次数，在rejectRecords中表示失败或拒绝次数。<br>默认值：0。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| usedType<sup>12+</sup> | [PermissionUsedType](#permissionusedtype12) | 否 | 是    | 敏感权限访问方式。<br>默认值：NORMAL_TYPE。<br>**ArkTS-Dyn起始版本：** 12<br>**ArkTS-Sta起始版本：** 23 |
...
| 名称           | 类型                    | 只读 | 可选 | 说明                   |
| -------------- | ---------------------- | ---- | ---- | --------------------- |
| callingTokenId<sup>18+</sup> | ArkTS-Dyn: number <br> ArkTS-Sta: int| 否   | 是   | 接口调用方的应用身份标识，当activeStatus为INACTIVE时该值无效。<br>默认值：0。<br>**ArkTS-Dyn起始版本：** 18<br>**ArkTS-Sta起始版本：** 23 |
| tokenId        | ArkTS-Dyn: number <br> ArkTS-Sta: int | 否   | 否   | 被订阅的应用身份标识。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| permissionName | [Permissions](../../security/AccessToken/app-permissions.md)            | 否   | 否   | 权限使用状态发生变化的权限名。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| deviceId       | string                 | 否   | 否   | 权限使用状态发生变化时所在设备的ID。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| activeStatus   | [PermissionActiveStatus](#permissionactivestatus) | 否   | 否   | 权限使用状态变化类型。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| usedType<sup>18+</sup> | [PermissionUsedType](#permissionusedtype12) | 否   | 是   | 敏感权限使用类型，当activeStatus为INACTIVE时该值无效。<br>默认值：NORMAL_TYPE。<br>**ArkTS-Dyn起始版本：** 18<br>**ArkTS-Sta起始版本：** 23 |
| enhancedIdentity | string | 否 | 是 | 扩展身份，用于标识调用方的附加身份信息。当需要区分同一应用下不同调用来源的权限使用记录时返回此字段。长度不超过48个字符。<br>默认值：空字符串。<br>**ArkTS-Dyn起始版本：** 26.0.0<br>**ArkTS-Sta起始版本：** 26.0.0<br>**模型约束：** 此接口仅可在Stage模型下使用。 |
...
| 名称           | 类型                    | 只读 | 可选 | 说明                   |
| -------------- | ---------------------- | ---- | ---- | --------------------- |
| usedType | [PermissionUsedType](#permissionusedtype12) | 否 | 是    | 敏感权限使用类型。<br>默认值：NORMAL_TYPE。<br>**ArkTS-Dyn起始版本：** 12<br>**ArkTS-Sta起始版本：** 23 |
| enhancedIdentity | string | 否 | 是 | 扩展身份，用于标识调用方的附加身份信息。当需要区分同一应用下不同调用来源的权限使用记录时传入此字段。长度不超过48个字符，调用[addPermissionUsedRecord](#privacymanageraddpermissionusedrecord)时传入超长值会返回错误码12100001。<br>默认值：空字符串。<br>**ArkTS-Dyn起始版本：** 26.0.0<br>**ArkTS-Sta起始版本：** 26.0.0<br>**模型约束：** 此接口仅可在Stage模型下使用。 |
