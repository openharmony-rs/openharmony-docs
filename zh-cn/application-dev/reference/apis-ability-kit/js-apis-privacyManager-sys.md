...
- 系统访问敏感权限后，需要调用可调用addPermissionUsedRecord，以便系统记录对应的敏感权限访问事件。
...
场景说明：系统应用需要查询敏感权限历史使用情况时，可构造[PermissionUsedRequest](#permissionusedrequest)指定查询范围，再调用[getPermissionUsedRecord](#privacymanagergetpermissionusedrecord)或[getPermissionUsedTypeInfos](#privacymanagergetpermissionusedtypeinfos12)获取记录和使用类型信息。
...
// 1. 查询历史权限使用记录。
let response: privacyManager.PermissionUsedResponse = await privacyManager.getPermissionUsedRecord(request);

// 2. 查询敏感权限访问类型信息。
let typeInfos: Array<privacyManager.PermissionUsedTypeInfo> =
...
// 1. 查询当前权限是否正在使用中。
let inUse: boolean = privacyManager.checkPermissionInUse(permissionName);
...
受权限保护的应用在被其他服务、应用调用时，可以使用该接口增加一条权限使用记录。使用Promise异步回调。
...
| permissionName | [Permissions](../../security/AccessToken/app-permissions.md) | 是   | 需要记录的权限名称。权限名长度不能超过256个字符，超出限制时返回错误码12100001。 |
| successCount | number | 是   | 访问成功的次数。 |
| failCount | number | 是   | 访问失败的次数。 |
| options<sup>12+</sup> | [AddPermissionUsedRecordOptions](#addpermissionusedrecordoptions12) | 否   | 添加权限使用记录可选参数。<br>usedType：从API version 12开始支持。默认值：NORMAL_TYPE。<br>enhancedIdentity：从API版本26.0.0开始支持。默认值：""。 |
...
| permissionName | [Permissions](../../security/AccessToken/app-permissions.md) | 是   | 需要使用的权限名称。权限名长度不能超过256个字符，超出限制时返回错误码12100001。|
...
| -------------- | ------ | ---- | ------------------------------------ |
| tokenID        | number | 是   | 目标应用的身份标识。可通过应用[BundleInfo](js-apis-bundleManager-bundleInfo.md)中的[ApplicationInfo](js-apis-bundleManager-applicationInfo.md#applicationinfo-1)的accessTokenId字段获取。该参数必须为大于0的整数，传入0时返回错误码12100001。|
| permissionName | [Permissions](../../security/AccessToken/app-permissions.md) | 是   | 需要停止使用的权限名称。权限名长度不能超过256个字符，超出限制时返回错误码12100001。|
...
| tokenID        | number | 是   | 目标应用的身份标识。可通过应用[BundleInfo](js-apis-bundleManager-bundleInfo.md)中的[ApplicationInfo](js-apis-bundleManager-applicationInfo.md#applicationinfo-1)的accessTokenId字段获取。该参数必须为大于0的整数，传入0时返回错误码12100001。|
| permissionName | [Permissions](../../security/AccessToken/app-permissions.md) | 是   | 需要停止使用的权限名称。权限名长度不能超过256个字符，超出限制时返回错误码12100001。|
| pid            | number | 否   | 与startUsingPermission传入的pid相同。不满足配套关系可能导致API调用失败(错误码12100004)。<br>默认值：-1，表示不根据进程生命周期响应。|
...
| tokenID        | number | 是   | 目标应用的身份标识。可通过应用[BundleInfo](js-apis-bundleManager-bundleInfo.md)中的[ApplicationInfo](js-apis-bundleManager-applicationInfo.md#applicationinfo-1)的accessTokenId字段获取。该参数必须为大于0的整数，传入0时返回错误码12100001。|
| permissionName | [Permissions](../../security/AccessToken/app-permissions.md) | 是   | 需要停止使用的权限名称。权限名长度不能超过256个字符，超出限制时返回错误码12100001。|
| pid            | number | 否   | 调用方的进程pid。<br>默认值：-1，表示不根据进程生命周期响应。 |
| options        | [PermissionUsingOptions](#permissionusingoptions)| 否 | 权限使用可选参数。 |
...
| 参数名          | 类型   | 必填 | 说明                                  |
| -------------- | ------ | ---- | ------------------------------------ |
| permissionName | [Permissions](../../security/AccessToken/app-permissions.md)                | 是   | 需要查询的权限名称。权限名不能为空，且长度不能超过256个字符，超出限制时返回错误码12100001。|
...
订阅指定权限列表的权限使用状态变更事件。订阅成功后，当权限使用状态变更时，callback会被触发，返回[ActiveChangeResponse](#activechangeresponse)对象，包含权限使用状态变化的详情。使用callback异步回调。
...
| permissionName     | [Permissions](../../security/AccessToken/app-permissions.md)           | 否   | 被访问的敏感权限名称。当需要查询特定敏感权限的访问类型信息时传入具体的权限名；为空时表示查询所有敏感权限的访问类型信息。权限名长度不能超过256个字符。<br>默认值：空。   |
...
| deviceId  | string         | 否    | 是    | 目标应用所在设备的ID。查询分布式场景下的权限使用记录时，建议明确传入目标设备ID，避免因默认值导致只返回本端数据。<br>默认值：本端设备ID。 |
| bundleName | string         | 否    | 是    | 目标应用的包名。若同时设置tokenId和bundleName，建议两者指向同一应用，便于缩小查询范围并减少歧义。<br>默认值：查询所有应用。 |
| permissionNames  | Array&lt;[Permissions](../../security/AccessToken/app-permissions.md)&gt;         | 否    | 是    | 需要查询的权限集合。建议仅传入当前审计所关注的敏感权限，避免在历史数据量较大时返回过多无关记录。<br>默认值：查询所有权限的使用记录。 |
| beginTime | number         | 否    | 是    | 查询的起始时间，单位：ms。建议与endTime成对使用，明确限定时间窗口，以提升查询效率并减少结果规模。<br>默认值：0，表示不限制起始时间。 |
| endTime | number         | 否    | 是    | 查询的终止时间，单位：ms。设置后应保证不早于beginTime，用于界定审计时间范围。<br>默认值：0，表示不限制终止时间。 |
...
| bundleRecords  | Array&lt;[BundleUsedRecord](#bundleusedrecord)&gt;         | 否    | 否    | 应用或设备的权限使用记录集合。每个元素表示一个应用维度下的权限访问记录，开发者可进一步遍历permissionRecords获取具体权限使用详情。                                 |
...
| 名称       | 类型             | 只读 | 可选 | 说明                                       |
| -------- | -------------- | ---- | ---- | ---------------------------------------- |
| tokenId  | number         | 否    | 否    | 使用权限的应用身份标识。在本地应用记录场景下可用于唯一定位目标应用；在分布式场景下该字段无效，应结合deviceId和deviceName识别来源设备。                                 |
...
| deviceName<sup>24+</sup>  | string         | 否    | 是    | 使用权限的应用所在设备名称，仅用于分布式场景。可用于在界面中展示更易理解的设备标识。                                 |
...
| accessCount | number         | 否    | 否    | 该权限访问总次数，表示在查询时间窗口内成功使用该权限的累计次数。可用于衡量权限实际使用频率。 |
| rejectCount | number         | 否    | 否    | 该权限拒绝总次数，表示在查询时间窗口内权限访问失败或被拒绝的累计次数。可用于分析授权失败或访问受阻情况。 |
| lastAccessTime | number         | 否    | 否    | 最后一次访问时间，单位：ms。可用于定位最近一次成功使用该权限的时间点。 |
| lastRejectTime | number         | 否    | 否    | 最后一次拒绝时间，单位：ms。可用于定位最近一次权限被拒绝或访问失败的时间点。 |
| lastAccessDuration | number         | 否    | 否    | 最后一次访问时长，单位：ms。可用于分析最近一次权限使用持续时间。 |
