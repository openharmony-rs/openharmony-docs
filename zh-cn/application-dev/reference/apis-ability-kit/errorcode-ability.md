...
3. 调用connectAgentExtensionAbility时，入参指定的abilityName或moduleName与agentId对应AgentCard的appInfo内配置的abilityName或moduleName不匹配。<!--DelEnd-->
...
入参ExecuteParam中的deviceId不为空且无效。
...
2. 调用connectAgentExtensionAbility时，入参bundleName与入参agentId关联的AgentCard中appInfo所配置的bundleName不匹配。<!--DelEnd-->

**处理步骤**

1. 检查目标应用的静态配置信息，重新传入正确的agentId。<!--Del-->
2. 调用connectAgentExtensionAbility时，确保入参bundleName与入参agentId关联的AgentCard中appInfo所配置的bundleName保持一致。<!--DelEnd-->
