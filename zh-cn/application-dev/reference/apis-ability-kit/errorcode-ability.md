...
1. 启动Ability时传入的[Want](./js-apis-app-ability-want.md)数据过大。
2. 设备解锁前拉起非系统应用。
3. 隐式拉起时未安装应用APP。
...
1. 对于启动Ability失败时，可以检查传入的[Want](./js-apis-app-ability-want.md)数据是否过大。
2. 确保在设备解锁前只拉起系统应用，或者延迟拉起非系统应用直到设备解锁。
3. 确保设备上已安装应用市场APP，或者在拉起应用前检查应用市场是否已安装。
