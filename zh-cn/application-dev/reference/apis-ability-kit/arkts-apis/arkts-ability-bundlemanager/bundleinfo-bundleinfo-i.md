...
应用程序的配置信息，通过调用 [getBundleInfoForSelf]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接 口，bundleFlags参数传入GET\_BUNDLE\_INFO\_WITH\_APPLICATION获取。
...
应用包的构建版本号，用于标识相同发布版本下的不同构建版本包，对应\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_中配置的buildVersion字段。 **模型约束：** 此接口仅可在Stage模型下使用。
...
模块的配置信息，通过调用 [getBundleInfoForSelf]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接 口，bundleFlags参数传入GET\_BUNDLE\_INFO\_WITH\_HAP\_MODULE获取。
...
申请权限的授予状态，通过调用 [getBundleInfoForSelf]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接 口，bundleFlags参数传入GET\_BUNDLE\_INFO\_WITH\_REQUESTED\_PERMISSION获取。reqPermissionDetails数组和permissionGrantStates数组的索引顺序一一对 应，即reqPermissionDetails[2]的授权状态为permissionGrantStates[2]。
...
应用运行时需向系统申请的权限集合的详细信息，通过调用 [getBundleInfoForSelf]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接 口，bundleFlags参数传入GET\_BUNDLE\_INFO\_WITH\_REQUESTED\_PERMISSION获取。reqPermissionDetails数组和permissionGrantStates数组的索引顺序一一对 应，即reqPermissionDetails[2]的授权状态为permissionGrantStates[2]。
...
应用的路由表配置，由hapModulesInfo下的routerMap信息，根据RouterItem中的name字段进行去重后合并得到。通过调用 [getBundleInfoForSelf]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接 口，bundleFlags参数传入GET\_BUNDLE\_INFO\_WITH\_HAP\_MODULE和GET\_BUNDLE\_INFO\_WITH\_ROUTER\_MAP获取。
...
应用包的签名信息，通过调用 [getBundleInfoForSelf]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接 口，bundleFlags参数传入GET\_BUNDLE\_INFO\_WITH\_SIGNATURE\_INFO获取。
