...
| 名称 | 说明 |
| --- | --- |
| getBundleName | 获取WantAgent实例的Bundle名称。使用callback异步回调。 |
| getBundleName | 获取WantAgent实例的Bundle名称。使用Promise异步回调。 |
| getUid | 获取WantAgent实例的用户ID。使用callback异步回调。 |
| getUid | 获取WantAgent实例的用户ID。使用Promise异步回调。 |
| cancel | 取消WantAgent实例。使用callback异步回调。 |
| cancel | 取消WantAgent实例。使用Promise异步回调。 |
| trigger | 主动激发WantAgent实例。使用callback异步回调。 |
| equal | 判断两个WantAgent实例是否相等，以此来判断是否是来自同一应用的相同操作。使用callback异步回调。 |
| equal | 判断两个WantAgent实例是否相等，以此来判断是否是来自同一应用的相同操作。使用Promise异步回调。 |
| getWantAgent | 创建WantAgent。创建失败返回的WantAgent为空值。使用callback异步回调。 |
| getWantAgent | 创建WantAgent。创建失败返回的WantAgent为空值。使用Promise异步回调。 |
