...
| disconnectAbility | 断开与指定ServiceAbility的连接。使用callback异步回调。 |
...
| terminateSelf | 停止当前的Ability。使用callback异步回调。 |
