...
撤销授权指定应用的URI。使用callback异步回调。 该接口仅在Phone、PC/2in1、Tablet设备中可正常调用，在其他设备可以调用但是不生效。 > **说明：** > > - 允许应用撤销自身获得的其他应用URI权限，或授权给其他应用的URI权限。 > > - 因URI处理涉及编解码，传入的URI需要使用[getUriFromPath]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口获取。对于应用自行拼接的URI，系统无法保证 > 其功能。
...
| 错误码ID | 错误信息 |
| --- | --- |
| [201](../../errorcode-universal.md#201-权限校验失败) | Permission denied.\_\_\_HTML\_TAG\_USD\_0\_\_\_**适用版本：** 10 - 11 |
...
撤销授权指定应用的URI。使用Promise异步回调。 该接口仅在Phone、PC/2in1、Tablet设备中可正常调用，在其他设备可以调用但是不生效。 > **说明：** > > - 允许应用撤销自身获得的其他应用URI权限，或授权给其他应用的URI权限。 > > - 因URI处理涉及编解码，传入的URI需要使用[getUriFromPath]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口获取。对于应用自行拼接的URI，系统无法保证 > 其功能。
...
撤销授权指定应用的URI。使用Promise异步回调。 该接口仅在Phone、PC/2in1、Tablet设备中可正常调用，在其他设备可以调用但是不生效。 > **说明：** > > - 允许应用撤销自身获得的其他应用URI权限，或授权给其他应用的URI权限。 > > - 该接口支持撤销授权给分身应用的URI权限，需要指定目标应用的应用包名和分身索引。 > > - 因URI处理涉及编解码，传入的URI需要使用[getUriFromPath]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口获取。对于应用自行拼接的URI，系统无法保证 > 其功能。
