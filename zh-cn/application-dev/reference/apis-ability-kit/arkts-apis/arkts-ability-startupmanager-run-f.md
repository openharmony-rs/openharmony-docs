...
执行启动框架启动任务或加载so文件。 > **说明：** > > 本接口不支持执行feature类型HAP中的启动任务，如需要使用相关能力请调用 > [startupManager.run]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ > 接口。
...
执行启动框架启动任务或加载so文件。支持指定[AbilityStageContext]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_用于启动任务的加载。使 用Promise异步回调。
