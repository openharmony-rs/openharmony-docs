...
表示传递的公共事件数据，仅当WantAgent实例的OperationType类型是'SEND_COMMON_EVENT'时有效。该字段与发布者使用commonEventManager.publish发布公共事件时，传递CommonEventPublishData公共事件数据中的`code`字段含义一致。
...
表示公共事件订阅者的权限。仅当WantAgent实例的OperationType类型是'SEND_COMMON_EVENT'时，该字段生效。
