...
销毁设备。必须与[OH_Hid_CreateDevice](#oh_hid_createdevice)成对使用，只能销毁由当前调用者创建的设备。
...
初始化HID DDK。不再使用任何HID DDK功能时，必须调用[OH_Hid_Release](#oh_hid_release)释放资源。
...
打开deviceId和interfaceIndex指定的设备。在使用完毕后调用[OH_Hid_Close](#oh_hid_close)释放设备资源。
