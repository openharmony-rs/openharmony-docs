...
声声明主机侧访问输入设备的HID DDK（Driver Development Kit，驱动开发工具包）接口。该模块提供了创建、打开、读写HID设备的能力，支持设备管理和事件注入，适用于需要模拟或管理HID输入设备的场景。
...
| [int32_t OH_Hid_Close(Hid_DeviceHandle **dev)](#oh_hid_close) | 关闭设备。必须与OH_Hid_Open配对使用，适用于不再需要与HID设备通信时，释放设备资源的场景。 |
...
| uint16_t length | 发送事件列表长度（一次发送事件个数），最大值不超过7。 |
...
关闭设备。必须与OH_Hid_Open配对使用，适用于不再需要与HID设备通信时，释放设备资源的场景。
