...
| [int32_t OH_Hid_Release(void)](#oh_hid_release) | 释放HID DDK。必须与[OH_Hid_Init](#oh_hid_init)配对使用。 |
...
| [int32_t OH_Hid_Close(Hid_DeviceHandle **dev)](#oh_hid_close) | 关闭设备。必须与[OH_Hid_Open](#oh_hid_open)配对使用，适用于不再需要与HID设备通信时，释放设备资源的场景。 |
...
关闭设备。必须与[OH_Hid_Open](#oh_hid_open)配对使用，适用于不再需要与HID设备通信时，释放设备资源的场景。
