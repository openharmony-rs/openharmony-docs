...
| [void OH_Usb_Release(void)](#oh_usb_release) | 释放USB DDK。需在调用[OH_Usb_Init](#oh_usb_init)完成初始化后使用。 |
| [int32_t OH_Usb_ReleaseResource(void)](#oh_usb_releaseresource) | 释放USB DDK。需在调用[OH_Usb_Init](#oh_usb_init)完成初始化后使用。 |
...
| [int32_t OH_Usb_ReleaseInterface(uint64_t interfaceHandle)](#oh_usb_releaseinterface) | 释放USB接口，用于释放对USB设备接口的独占访问权。需要先调用[OH_Usb_ClaimInterface](#oh_usb_claiminterface)声明接口获取interfaceHandle后，在使用完毕后才能调用此方法释放接口。 |
...
释放USB DDK。需在调用[OH_Usb_Init](#oh_usb_init)完成初始化后使用。
...
释放USB接口，用于释放对USB设备接口的独占访问权。需要先调用[OH_Usb_ClaimInterface](#oh_usb_claiminterface)声明接口获取interfaceHandle后，在使用完毕后才能调用此方法释放接口。
