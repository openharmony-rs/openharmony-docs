...
如果生成返回值RemoteObject依赖一个异步接口，可以使用异步的生命周期回调函数：
...
在执行完onDisconnect生命周期回调后，应用可能会退出，从而可能导致onDisconnect中的异步函数未能正确执行，比如异步写入数据库。可以使用异步的生命周期回调函数，以确保异步onDisconnect完成后再继续后续的生命周期。
