...
| 枚举项 | 描述 |
| -- | -- |
| PROCESS_BACKGROUND = 1 | 该档位相较PROCESS_INACTIVE压制效果更显著，获取到的CPU资源更少。推荐执行处于后台的图文页面等用户无感知业务的后台子进程时设置该档位。<br>**起始版本：** 17 |
| PROCESS_INACTIVE = 2 | 推荐正在执行播放音频、导航等用户可感知业务的后台子进程时设置该档位。<br>**起始版本：** 17 |
...
| 参数项 | 描述 |
| -- | -- |
| int pid | 需要被压制子进程的进程号，{@link OH_Ability_StartNativeChildProcess}接口创建子进程后的pid参数，即为子进程进程号。 |
...
| 参数项 | 描述 |
| -- | -- |
| int pid | 子进程的进程号，{@link OH_Ability_StartNativeChildProcess}接口创建子进程后的pid参数，即为子进程进程号。 |
