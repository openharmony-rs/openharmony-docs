...
4. API version 20及之前版本，只有PC/2in1设备才能申请长时任务TASK_KEEPING。
...
16. 应用申请MODE_SPECIAL_SCENARIO_PROCESSING类型的长时任务时，未申请ACL授权。
...
1. 调用requestSuspendDelay方法传递的callback对象已存在。
2. 调用cancelSuspendDelay方法传递的callback对象不存在。
...
1. 请检查调用requestSuspendDelay时，是否传递了已存在的callback对象。
2. 请检查调用cancelSuspendDelay时，是否传递了不存在的callback对象。
