...
| 名称 | 说明 |
| --- | --- |
| addNotificationSlot | 添加一个NotificationSlot，使用回调的方式实现异步调用。 |
...
| cancelAllReminders | 取消当前应用所有的提醒，使用回调的方式实现异步调用。 |
...
| publishReminder | 发布一个后台代理提醒，使用回调的方式实现异步调用，该方法需要申请通知弹窗权限Notification.requestEnableNotification后才能调用。 |
| [publishReminder](arkts-backgroundtasks-reminderagent-publishreminder-f.md#publishreminder-1) | 发布一个后台代理提醒，使用Promise方式实现异步调用，该方法需要申请通知弹窗权限Notification.requestEnableNotification后才能调用。 |
| removeNotificationSlot | 删除目标NotificationSlot，使用callback方式实现异步调用。 |
