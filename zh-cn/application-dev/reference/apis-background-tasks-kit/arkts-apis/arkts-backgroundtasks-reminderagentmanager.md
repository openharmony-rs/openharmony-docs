...
| addNotificationSlot | 添加通知渠道。使用callback异步回调。 |
...
| cancelAllReminders | 取消当前应用设置的所有代理提醒。使用callback异步回调。 |
...
| cancelReminder | 取消指定id的代理提醒。使用callback异步回调。 |
...
| getValidReminders | 获取当前应用设置的所有[有效（未过期）的代理提醒](../../../task-management/agent-powered-reminder.md#约束与限制)。使用callback异步回调。 |
