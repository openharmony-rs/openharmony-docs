...
2. 更新进度的通知类型必须为实况窗，具体实现可参考[startBackgroundRunning()](arkts-backgroundtasks-startbackgroundrunning-f.md#startbackgroundrunning-3)中的示例。
...
1. 如果应用需要在后台长时间运行，可以通过[requestAuthFromUser](arkts-backgroundtasks-continuoustaskrequest-c.md#requestauthfromuser-1)接口请求用户授权、通过[checkSpecialScenarioAuth](arkts-backgroundtasks-continuoustaskrequest-c.md#checkspecialscenarioauth-1)接口查询用户授权结果。
