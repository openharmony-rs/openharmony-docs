...
| [void OH_Drawing_ShadowLayerDestroy(OH_Drawing_ShadowLayer* shadowLayer)](#oh_drawing_shadowlayerdestroy) | 销毁由[OH_Drawing_ShadowLayerCreate](#oh_drawing_shadowlayercreate)创建的阴影层对象，并回收该对象占用的内存。 |
...
销毁由[OH_Drawing_ShadowLayerCreate](#oh_drawing_shadowlayercreate)创建的阴影层对象，并回收该对象占用的内存。调用此接口后，不应再使用该阴影层对象，否则可能出现未定义行为。
