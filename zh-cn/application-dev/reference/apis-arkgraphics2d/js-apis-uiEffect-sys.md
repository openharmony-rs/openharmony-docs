...
扭曲环遮罩参数，用于指定光环的半径、宽度、变化量、旋转、3D朝向和噪声演化。
...
| 名称               | 类型   | 只读 | 可选 | 说明                                                                                                                           |
| ------------------ | ------ | ---- | ---- | ------------------------------------------------------------------------------------------------------------------------------ |
| radius             | number | 否   | 否   | 定义光环的半径，从光环中心到其厚度中点测量。该值无限制，推荐范围为[0.0, 1.0]。小于0.0的值无实际效果。设为1.0时，光环的直径等于组件宽度和高度中的较小值。                           |
| baseHalfWidth      | number | 否   | 否   | 定义光环厚度的一半，从中心线到任一侧边缘测量。该值无限制，推荐范围为[0.0, 0.5]。小于0.0的值无实际效果。调整该效果时，建议使用0.01的步长以获得更好的效果。             |
...
| options | [SweepRefractionMaskOptions](#sweeprefractionmaskoptions) | 否   | 扫光折射的可选参数，用于配置棱镜形状、尺寸和扫光中心等属性。不传入时SweepRefractionMaskOptions的各字段使用默认值。 |
...
| 类型            | 说明                                   |
| --------------- | -------------------------------------- |
| [Mask](#mask20) | 返回带有扭曲环遮罩效果的Mask实例。 |
...
创建一个双目蒙版。生成一个左右对称的双椭圆弧形蒙版形状，与[maskDispersion](#maskdispersion20)滤镜配合使用，用于控制色散效果的作用区域和方向。
...
| 参数名    | 类型   | 必填 | 说明                                                                                                    |
| --------- | ------ | ---- | ------------------------------------------------------------------------------------------------------- |
| radiusX   | number | 是   |  双目椭圆的半长轴。取值范围为[0.0, 1.0]；超出范围的值将在内部被截断。 |
| radiusY   | number | 是   | 双目椭圆的半短轴。取值范围为[0.0, 1.0]；超出范围的值将在内部被截断。 |
| gap       | number | 是   | 每个椭圆中心相对于原点的水平偏移。取值范围为[0.0, 1.0]；超出范围的值将在内部被截断。 |
| softness  | number | 是   | 双目蒙版形状的边缘柔和度。取值范围为[0.0, 1.0]；超出范围的值将在内部被截断。 |
