...
| [OH_Drawing_ImageFilter* OH_Drawing_ImageFilterCreateFromShaderEffect(OH_Drawing_ShaderEffect* shaderEffect)](#oh_drawing_imagefiltercreatefromshadereffect) | 基于着色器创建一个图像滤波器。本接口会产生错误码，可以通过[OH_Drawing_ErrorCodeGet](capi-drawing-error-code-h.md#oh_drawing_errorcodeget)查看错误码的取值。shaderEffect为NULL时返回OH_DRAWING_ERROR_INVALID_PARAMETER。 |
...
| 参数项 | 描述 |
| -- | -- |
| float sigmaX | 表示沿x轴方向上高斯模糊的标准差，单位：px。传入小于等于0的值时不生效。 |
| float sigmaY | 表示沿y轴方向上高斯模糊的标准差，单位：px。传入小于等于0的值时不生效。 |
| [OH_Drawing_TileMode](capi-drawing-shader-effect-h.md#oh_drawing_tilemode) tileMode | 用于控制图像滤波器效果在图像边界处的平铺方式，支持的可选模式详见[OH_Drawing_TileMode](capi-drawing-shader-effect-h.md#oh_drawing_tilemode)枚举。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_Drawing_ImageFilter](capi-drawing-oh-drawing-imagefilter.md)* | 函数会返回一个指针，指针指向创建的图像滤波器对象[OH_Drawing_ImageFilter](capi-drawing-oh-drawing-imagefilter.md)。如果返回NULL，表示创建失败；可能的原因是可用内存不足，或者是sigmaX、sigmaY小于等于0。 |
...
| 参数项 | 描述 |
| -- | -- |
| float sigmaX | 表示沿x轴方向上高斯模糊的标准差，单位：px。必须大于0.0，传入小于等于0的值时不生效。 |
| float sigmaY | 表示沿y轴方向上高斯模糊的标准差，单位：px。必须大于0.0，传入小于等于0的值时不生效。 |
| [OH_Drawing_TileMode](capi-drawing-shader-effect-h.md#oh_drawing_tilemode) tileMode | 用于控制图像滤波器效果在图像边界处的平铺方式，支持的可选模式详见[OH_Drawing_TileMode](capi-drawing-shader-effect-h.md#oh_drawing_tilemode)枚举。 |
...
| 参数项 | 描述 |
| -- | -- |
| float x | 表示沿x轴方向的平移距离，单位：px。 |
| float y | 表示沿y轴方向的平移距离，单位：px。 |
...
基于着色器创建一个图像滤波器。本接口会产生错误码，可以通过[OH_Drawing_ErrorCodeGet](capi-drawing-error-code-h.md#oh_drawing_errorcodeget)查看错误码的取值。shaderEffect为NULL时返回OH_DRAWING_ERROR_INVALID_PARAMETER。使用本函数创建的图像滤波器对象，在使用完毕后必须调用[OH_Drawing_ImageFilterDestroy](#oh_drawing_imagefilterdestroy)进行销毁，否则会导致内存泄漏。
...
| 参数项 | 描述 |
| -- | -- |
| [OH_Drawing_ShaderEffect](capi-drawing-oh-drawing-shadereffect.md)* shaderEffect | 表示要应用于图像的着色器效果。如果为NULL，返回OH_DRAWING_ERROR_INVALID_PARAMETER。 |
...
销毁通过OH_Drawing_ImageFilterCreateBlur等Create函数创建的图像滤波器对象并回收该对象占用的内存。这些对象在使用完毕后必须调用本函数进行销毁，否则会导致内存泄漏。
