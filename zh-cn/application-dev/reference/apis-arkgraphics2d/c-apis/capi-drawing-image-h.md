...
| 参数项 | 描述 |
| -- | -- |
| [OH_Drawing_Image](capi-drawing-oh-drawing-image.md)* image | 指向由[OH_Drawing_ImageCreate](capi-drawing-image-h.md#oh_drawing_imagecreate)创建的图片对象[OH_Drawing_Image](capi-drawing-oh-drawing-image.md)的指针，用于销毁该图片对象并回收内存。销毁后不应再使用该指针，否则可能导致未定义行为。 |
