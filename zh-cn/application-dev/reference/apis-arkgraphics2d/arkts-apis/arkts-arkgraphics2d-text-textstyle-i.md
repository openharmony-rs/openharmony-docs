...
行高上限，单位为物理像素px。若同时应用行高缩放，行高上限在[TextStyle](arkts-arkgraphics2d-text-textstyle-i.md#TextStyle).heightScale大于0时生效。取值为正数浮点数，默认值为Number.MAX_VALUE。
...
行高下限，单位为物理像素px。若同时应用行高缩放，行高下限在[TextStyle](arkts-arkgraphics2d-text-textstyle-i.md#TextStyle).heightScale大于0时生效。取值范围为非负浮点数，默认值为0。
