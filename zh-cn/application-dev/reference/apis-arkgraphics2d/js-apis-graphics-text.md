...
| compressHeadPunctuation<sup>23+</sup>   | boolean | 否   | 是   | 设置文本排版时是否使能行首标点压缩。true表示使能行首标点压缩，false表示不使能行首标点压缩，默认值为false。说明：1、需要字体文件支持[FontFeature](#fontfeature)中的"ss08"特性，否则无法压缩。2、在[行首标点压缩范围](#行首压缩的标点范围)内的标点才在本特性作用范围内。 |
