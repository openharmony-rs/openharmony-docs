...
通过绘制两条水平线和两条垂直线将图像分割成9个部分：四个边，四个角和中心。使用此接口时，再次调用[brush](./arkts-apis-graphics-drawing-Brush.md)设置抗锯齿，抗锯齿效果不生效。<br>
...
将图像按照矩形网格对象的设置划分为多个网格，并把图像的每个部分按照网格对象的设置绘制到画布上的目标矩形区域。使用此接口时，再次调用[brush](./arkts-apis-graphics-drawing-Brush.md)设置抗锯齿，抗锯齿效果不生效。<br>
