...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| -------- | -------- | -------- | -------- | -------- |
| length | ArkTS-Dyn: number <br> ArkTS-Sta: int | 是 | 否 | List的元素个数。 |
...
| -------- | -------- | -------- | -------- |
| element | T | 是 | 插入元素。 |
| index | ArkTS-Dyn: number <br> ArkTS-Sta: int | 是 | 插入的位置索引。需要小于等于int32_max即2147483647。 |
...
```ts
let list: List<string | int | boolean> = new List<string | int | boolean>();
list.insert("A", 0);
list.insert(0, 1);
list.insert(true, 2);
```
...
ArkTS-Dyn: getSubList(fromIndex: number, toIndex: number): List&lt;T&gt;

ArkTS-Sta: getSubList(fromIndex: int, toIndex: int): List&lt;T&gt;
...
```ts
let list: List<int | string> = new List();
list.add(2);
list.add(4);
list.add(5);
list.add(4);
...
ArkTS-Sta: type ListComparatorFn\<T\> = (firstValue: T, secondValue: T) => double
