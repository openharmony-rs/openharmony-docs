...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| -------- | -------- | -------- | -------- | -------- |
| length | ArkTS-Dyn: number <br> ArkTS-Sta: int | 是 | 否 | TreeMap的元素个数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| -------- | -------- | -------- | -------- |
| comparator | [TreeMapComparator\<K\>](#treemapcomparatork23) | 否 | 用户自定义的比较函数，可通过比较关系对元素进行排序。默认值为hole（一个空白占位符），表示不提供比较函数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| -------- | -------- | -------- | -------- |
| callbackFn | [TreeMapForEachCb\<K, V\>](#treemapforeachcbk-v23) | 是 | 回调函数。 |
...
| map | [TreeMap<K, V>](#treemap) | 是 | 当前调用[forEach](#foreach23)方法的实例对象。 |

### TreeMapComparator\<K\><sup>23+</sup>

type TreeMapComparator\<K\> = (firstValue: K, secondValue: K) => double

TreeMap中[constructor](#constructor23)方法的回调函数。
...
| 类型 | 说明 |
| -------- | -------- |
| double | 通过回调函数返回的double，TreeMap能够根据自定义的比较规则维护元素的顺序。 |