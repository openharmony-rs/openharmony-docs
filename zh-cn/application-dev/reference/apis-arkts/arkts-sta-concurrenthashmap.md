...
> - 本模块仅适用于ArkTS-Sta。
>
> - 泛型参数`K`和`V`需为非空类型，不能将`null`或`undefined`作为key或value。通过构造函数初始化时，传入的键和值也不能为`null`或`undefined`。
...
向哈希表中添加或更新一个键值对，返回this即调用该方法的[ConcurrentHashMap](./arkts-sta-concurrenthashmap.md)对象，支持链式调用。
...
| 参数名 | 类型 | 必填 | 说明                  |
| :----- | :--- | :--- | :-------------------- |
| key    | K    | 是   | 待增添/更新元素的键，不能为`null`或`undefined`。 |
| val    | V    | 是   | 待增添/更新元素的值，不能为`null`或`undefined`。 |

**返回值：**

| 类型 | 说明                                                                     |
| :--- | :----------------------------------------------------------------------- |
| this | 返回调用该方法的[ConcurrentHashMap](./arkts-sta-concurrenthashmap.md)对象。 |
...
| :------- | :--- | :--- | :-------------------- |
| key      | K    | 是   | 待替换元素指定key值。 |
| newValue | V    | 是   | 待替换的新值，不能为`null`或`undefined`。 |
...
while (!v.done) {
  let value = v.value;
  console.info("key: " + value![0] + "  value: " + value![1]);
  v = iter.next();
}
/*
key: 0  value: zero
...
| key    | K                       | 否   | 当前遍历到的键值对的键。                                             |
| map    | ConcurrentHashMap<K, V> | 否   | 调用该方法的[ConcurrentHashMap](arkts-sta-concurrenthashmap.md)对象。 |
