...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| -------- | -------- | -------- | -------- | -------- |
| length | ArkTS-Dyn: number <br> ArkTS-Sta: int | 是 | 否 | LightWeightSet的元素个数。 |
...
```ts
let lightWeightSet: LightWeightSet<int | string> = new LightWeightSet<int | string>();
```
...
```ts
const lightWeightSet: LightWeightSet<int> = new LightWeightSet<int>();
let result = lightWeightSet.isEmpty();
```
...
```ts
let lightWeightSet: LightWeightSet<number> = new LightWeightSet<number>();
...
```ts
let lightWeightSet: LightWeightSet<int> = new LightWeightSet<int>();
lightWeightSet.add(123);
let result = lightWeightSet.has(123);
```
...
ArkTS-Dyn: getIndexOf(key: T): number

ArkTS-Sta: getIndexOf(key: T): int
