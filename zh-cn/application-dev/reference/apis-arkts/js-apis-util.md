...
| 参数名 | 类型 | 必填 | 说明 |
| -------- | -------- | -------- | -------- |
| original | ArkTS-Dyn: (err: Object, value: Object) => void <br> ArkTS-Sta: Function | 是 | Function为带有两个参数的函数，其中第一个参数err是拒绝原因（如果&nbsp;Promise&nbsp;已解决，则为&nbsp;null），第二个参数value是已解决的值。  |

**返回值：**

| 类型 | 说明 |
| -------- | -------- |
| ArkTS-Dyn: Function <br> ArkTS-Sta: [PromisifiedFunc](#promisifiedfunc20) | 返回一个Promise的函数。 |
...
| 类型 | 说明 |
| -------- | -------- |
| ArkTS-Dyn: number <br> ArkTS-Sta: long | 返回Hash值 |
...
如果之前内存泄漏的对象被继续使用，使用enableLocalHandleDetection接口后，系统会回收内存泄漏对象。继续使用该对象会导致内存泄漏问题变为稳定性问题。
...
class ChildLRUCache<K, V> extends util.LRUCache<K, V> {
  ...
  afterRemoval(isEvict: boolean, key: K, value: V, newValue: V): void {
    if (isEvict === true) {
      ...
      // 输出结果：value = 1
      console.info('newValue = ' + newValue);
      // 输出结果：newValue = undefined
    }
  }
}
let lru = new ChildLRUCache<int, int>(2);
lru.put(1, 1);
lru.put(2, 2);
lru.put(3, 3);
```
...
| 名称   |值| 说明               |
| ----- |---| ----------------- |
| BASIC | 0 | 表示BASIC编码格式。**原子化服务API**：从API version 11开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 10 <br> **ArkTS-Sta起始版本：** 23|
| MIME  | 1 | 表示MIME编码格式。**原子化服务API**：从API version 11开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 10 <br> **ArkTS-Sta起始版本：** 23|
| BASIC_URL_SAFE<sup>12+</sup> | 2 | 表示BASIC_URL_SAFE编码格式。<br/>从API version 12开始支持此枚举。**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 12 <br> **ArkTS-Sta起始版本：** 23|
| MIME_URL_SAFE<sup>12+</sup> | 3 | 表示MIME_URL_SAFE编码格式。<br/>从API version 12开始支持此枚举。**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。 <br>**ArkTS-Dyn起始版本：** 12 <br> **ArkTS-Sta起始版本：** 23|
