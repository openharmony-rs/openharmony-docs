...
| 参数名  | 类型          | 必填 | 说明                 |
| ------ | ------------- | ---- | -------------------- |
| taskId   | int      | 是   | 任务的标识符。无单位。可通过[task](#task)的属性taskId获取。 |
...
通过任务ID取消任务池中的任务。如果任务在taskpool等待队列中，取消后任务将不再执行，并返回任务取消的异常。如果任务已在taskpool工作线程中执行，取消操作不会影响任务的继续执行。可以通过[isCanceled](#iscanceled)检查任务是否被取消并作出相应处理。取消后的任务结果根据实际任务结果在then或者catch中获取。

**参数：**

| 参数名  | 类型          | 必填 | 说明                 |
| ------ | ------------- | ---- | -------------------- |
| taskId   | int | 是   | 任务的标识符。可通过[task](#task)的属性taskId获取。 |
...
取消任务池中的任务。当任务在taskpool等待队列中，取消该任务后该任务将不再执行，并返回任务被取消的异常。如果任务已经在工作线程中执行，取消操作不会影响任务的继续执行。可以通过[isCanceled](#iscanceled)检查任务是否被取消并作出相应处理。取消后的任务结果根据实际任务结果在then或者catch中获取。
...
取消任务池中的任务组。当任务组中处在等待队列中的任务，取消后这些任务将不再执行。正在执行的任务会继续完成。

如果所有任务都在执行中且正常完成，可调用then获取结果数组。如果存在等待任务被取消，可以调用catch获取取消异常。正在执行的任务可通过[isCanceled](#iscanceled)检查取消状态并作出相应处理。
...
>
> - 该接口在taskpool的线程中调用。
> - 避免在回调函数中使用该方法，否则可能导致报错。
> - 避免在异步函数中使用该方法，否则可能导致报错。
> - 调用该接口时确保处理数据的回调函数在宿主线程已注册。
> - 在上述不支持场景下使用该方法，会与ArkTS-Dyn的[sendData](js-apis-taskpool.md#senddata11)行为产生差异，请避免使用。
