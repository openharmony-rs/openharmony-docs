...
ArkTS-Dyn: type Value = string | number | Decimal

ArkTS-Sta: type Value = string | double | Decimal
...
ArkTS-Dyn: type Rounding = 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8

ArkTS-Sta: type Rounding = int
...
ArkTS-Dyn: type Modulo = Rounding | 9

ArkTS-Sta: type Modulo = int
...
| s    | ArkTS-Dyn: number <br> ArkTS-Sta: double   | 是   | 否   | sign：表示Decimal数的符号位。|
...
| 名称               | 类型   | 值   | 说明                                                         |
| ------------------ | ------ | ---- | ------------------------------------------------------------ |
| ROUND_UP           | ArkTS-Dyn: number<br>ArkTS-Sta: int | 0    | 向远离零的方向舍入。模运算下，如果被除数为负，则余数为正，否则为负。 |
| ROUND_DOWN         | ArkTS-Dyn: number<br>ArkTS-Sta: int | 1    | 向靠近零的方向舍入。模运算下，余数与被除数的符号相同，使用截断除法。 |
| ROUND_CEILING      | ArkTS-Dyn: number<br>ArkTS-Sta: int | 2    | 向正无穷方向舍入。 |
| ROUND_FLOOR        | ArkTS-Dyn: number<br>ArkTS-Sta: int | 3    | 向负无穷方向舍入。模运算下，余数与除数的符号相同。 |
| ROUND_HALF_UP      | ArkTS-Dyn: number<br>ArkTS-Sta: int | 4    | 向最近的邻值舍入。如果距离相等，则向远离零的方向舍入。 |
| ROUND_HALF_DOWN    | ArkTS-Dyn: number<br>ArkTS-Sta: int | 5    | 向最近的邻值舍入。如果距离相等，则向靠近零方向舍入。 |
| ROUND_HALF_EVEN    | ArkTS-Dyn: number<br>ArkTS-Sta: int | 6    | 向最近的邻值舍入。如果距离相等，则向偶数邻值舍入。模运算下，IEEE 754 求余函数。 |
| ROUND_HALF_CEILING | ArkTS-Dyn: number<br>ArkTS-Sta: int | 7    | 向最近的邻值舍入。如果距离相等，则向正无穷方向舍入。 |
| ROUND_HALF_FLOOR   | ArkTS-Dyn: number<br>ArkTS-Sta: int | 8    | 向最近的邻值舍入。如果距离相等，则向负无穷方向舍入。 |
| EUCLIDEAN          | ArkTS-Dyn: number<br>ArkTS-Sta: int | 9    | 模运算下，余数始终为正。使用欧几里得除法：q = sign(x) * floor(a / abs(x))。 |
...
ArkTS-Dyn: comparedTo(n: Value): number

ArkTS-Sta: comparedTo(n: Value): double
...
let data: Decimal = new Decimal(Infinity);
let data1: Decimal = new Decimal(5);
let data2: double = data.comparedTo(data1);
console.info("test Decimal comparedTo:" + data2); // 'test Decimal comparedTo:1'

let data3: double = data1.comparedTo(10.5);
console.info("test Decimal comparedTo:" + data3); // 'test Decimal comparedTo:-1'
```
...
ArkTS-Dyn: toBinary(significantDigits: number): string

ArkTS-Sta: toBinary(significantDigits: double): string
...
ArkTS-Dyn: toBinary(significantDigits: number, rounding: Rounding): string

ArkTS-Sta: toBinary(significantDigits: double, rounding: Rounding): string
...
ArkTS-Dyn: toOctal(significantDigits: number): string

ArkTS-Sta: toOctal(significantDigits: double): string
...
ArkTS-Dyn: toOctal(significantDigits: number, rounding: Rounding): string

ArkTS-Sta: toOctal(significantDigits: double, rounding: Rounding): string
...
ArkTS-Dyn: toHexadecimal(significantDigits: number): string

ArkTS-Sta: toHexadecimal(significantDigits: double): string
...
ArkTS-Dyn: toHexadecimal(significantDigits: number, rounding: Rounding): string

ArkTS-Sta: toHexadecimal(significantDigits: double, rounding: Rounding): string
...
ArkTS-Dyn: toDecimalPlaces(decimalPlaces: number): Decimal

ArkTS-Sta: toDecimalPlaces(decimalPlaces: double): Decimal
...
ArkTS-Dyn: toDecimalPlaces(decimalPlaces: number, rounding: Rounding): Decimal

ArkTS-Sta: toDecimalPlaces(decimalPlaces: double, rounding: Rounding): Decimal
...
ArkTS-Dyn: toExponential(decimalPlaces: number): string

ArkTS-Sta: toExponential(decimalPlaces: double): string
...
ArkTS-Dyn: toExponential(decimalPlaces: number, rounding: Rounding): string

ArkTS-Sta: toExponential(decimalPlaces: double, rounding: Rounding): string
...
ArkTS-Dyn: toFixed(decimalPlaces: number): string
...
ArkTS-Dyn: toFixed(decimalPlaces: number, rounding: Rounding): string

ArkTS-Sta: toFixed(decimalPlaces: double, rounding: Rounding): string
...
ArkTS-Dyn: toPrecision(significantDigits: number): string

ArkTS-Sta: toPrecision(significantDigits: double): string
...
ArkTS-Dyn: toPrecision(significantDigits: number, rounding: Rounding): string

ArkTS-Sta: toPrecision(significantDigits: double, rounding: Rounding): string
...
ArkTS-Dyn: toSignificantDigits(significantDigits: number): Decimal

ArkTS-Sta: toSignificantDigits(significantDigits: double): Decimal
...
ArkTS-Dyn: toSignificantDigits(significantDigits: number, rounding: Rounding): Decimal

ArkTS-Sta: toSignificantDigits(significantDigits: double, rounding: Rounding): Decimal
...
ArkTS-Dyn: toNumber(): number

ArkTS-Sta: toNumber(): double

ArkTS-Dyn: 将值转换为number类型。

ArkTS-Sta: 将值转换为double类型。
...
| 类型   | 说明                            |
| ------ | ------------------------------- |
| ArkTS-Dyn: number<br>ArkTS-Sta: double | ArkTS-Dyn: 返回一个表示Decimal的number值。<br>ArkTS-Sta: 返回一个表示Decimal的double值。 |
...
```ts
let data: Decimal = new Decimal(456.789);
let data1: double = data.toNumber();
console.info("test Decimal toNumber:" + data1.toString()); // 'test Decimal toNumber:456.789'
```
...
ArkTS-Dyn: decimalPlaces(): number

ArkTS-Sta: decimalPlaces(): double
...
| 类型   | 说明                        |
| ------ | --------------------------- |
| ArkTS-Dyn: number<br>ArkTS-Sta: double | 返回Decimal对象的小数位数。 |
...
```ts
let data: Decimal = new Decimal(1.234);
let data1: double = data.decimalPlaces();
console.info("test Decimal decimalPlaces:" + data1); // 'test Decimal decimalPlaces:3'
```

### precision

ArkTS-Dyn: precision(): number

ArkTS-Sta: precision(): double
...
| 类型   | 说明                        |
| ------ | --------------------------- |
| ArkTS-Dyn: number<br>ArkTS-Sta: double | 返回Decimal对象的有效位数。 |
...
```ts
let data: Decimal = new Decimal(1.234);
let data1: double = data.precision();
console.info("test Decimal precision:" + data1); // 'test Decimal precision:4'
```

### precision

ArkTS-Dyn: precision(includeZeros: boolean | number): number

ArkTS-Sta: precision(includeZeros: boolean | int): double
...
| 参数名       | 类型    | 必填 | 说明                                                         |
| ------------ | ------- | ---- | ------------------------------------------------------------ |
| includeZeros | ArkTS-Dyn: boolean \| number<br>ArkTS-Sta: boolean \| int | 是   | 是否计算整数部分尾随零。true或1表示计算整数部分尾随零，false或0表示不计算整数部分尾随零。 |
...
| 参数名 | 类型              | 必填 | 说明         |
| ------ | ----------------- | ---- | ------------ |
| ...n      | [Value](#value)[] | 否   | 加数的序列。<br>此参数不填时会抛出401错误码。 |
...
| 参数名 | 类型              | 必填 | 说明                 |
| ------ | ----------------- | ---- | -------------------- |
| ...n      | [Value](#value)[] | 否   | 需要求平方和的序列。<br>此参数不填时会抛出401错误码。 |
...
| 参数名 | 类型              | 必填 | 说明                 |
| ------ | ----------------- | ---- | -------------------- |
| ...n      | [Value](#value)[] | 否   | 需要求最大值的序列。<br>此参数不填时会抛出401错误码。 |
...
| 参数名 | 类型            | 必填 | 说明                 |
| ------ | --------------- | ---- | -------------------- |
| ...n      | [Value](#value)[] | 否   | 需要求最小值的序列。<br>此参数不填时会抛出401错误码。 |
...
ArkTS-Dyn: static random(significantDigits: number): Decimal

ArkTS-Sta: static random(significantDigits: double): Decimal
...
| 参数名            | 类型   | 必填 | 说明                   |
| ----------------- | ------ | ---- | ---------------------- |
| significantDigits | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 随机值保留的有效数字。 |
...
ArkTS-Dyn: static sign(n: Value): number

ArkTS-Sta: static sign(n: Value): double
...
ArkTS-Sta示例：
```ts
let data: double = Decimal.sign(2);
console.info("test Decimal sign:" + data); // 'test Decimal sign:1'
```
