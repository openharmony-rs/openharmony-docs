...
| listener | [RejectedObjectListener](#rejectedobjectlistener) | 否 | 要移除的特定监听器回调函数。<br> - 当type为'unhandledPromiseRejection'时：如果提供，则移除指定的监听器；如果未提供，则移除该类型的所有监听器。<br> - 当type为'uncaughtError'时：无论是否提供，效果均为恢复默认的uncaught error行为。|
