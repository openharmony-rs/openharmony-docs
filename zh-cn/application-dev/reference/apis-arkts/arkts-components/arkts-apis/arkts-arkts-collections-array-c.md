...
> **说明：** 
> 
> - 本模块仅支持在ArkTS文件（文件后缀为.ets）中导入使用。本节使用以下标识来表示泛型的使用：
...
| --- | --- | --- | --- |
| searchElement | T | 是 | 要搜索的值。 |
| fromIndex | number | 否 | 开始搜索的索引。默认值为ArkTS Array长度减1（即从末尾开始）。如果索引大于或等于ArkTS Array的长度，则返回**-1**。如果传入负数，则指代**fromIndex + array.length**位置的下标。 |
