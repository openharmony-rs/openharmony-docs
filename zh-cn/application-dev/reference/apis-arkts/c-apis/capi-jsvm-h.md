...
| #ifdef JSVM_EXPERIMENTAL JSVM_VERSION JSVM_VERSION_EXPERIMENTAL#else JSVM_VERSION 8 | JSVM实验阶段标识。<br>**起始版本：** 11 |
...
| EXTERN_C_END }#else EXTERN_C_START EXTERN_C_END | 用于告知编译器按C Code编译以下代码段的段终止标识：当使用预处理指令__cplusplus检查到C++编译器正在编译时：EXTERN_C_START被赋值为"}" ，表示C代码到此为止。当预处理指令__cplusplus检查到不是C++编译器时，无须标记。<br>**起始版本：** 11 |
...
| [JSVM_CompileOptions](capi-jsvm-jsvm-compileoptions.md) options[] | option 数组，存放所有的编译选项。 |
...
| 参数项 | 描述 |
| -- | -- |
| [JSVM_VM](capi-jsvm-jsvm-vm--8h.md) vm | 将被监控堆使用量的虚拟机。 |
...
| 参数项 | 描述 |
| -- | -- |
| [JSVM_VM](capi-jsvm-jsvm-vm--8h.md) vm | 将被清除堆阈值回调的虚拟机。 |
...
| -- | -- |
| [JSVM_Env](capi-jsvm-jsvm-env--8h.md) env | 调用该API的环境。 |
| [JSVM_Value](capi-jsvm-jsvm-callbackstruct.md#jsvm_value) value | 要检查的JavaScript值。 |
...
| [JSVM_DeserializeResult](capi-jsvm-jsvm-deserializeresult--8h.md)* result | 后台反序列化结果。 |
...
| 参数项 | 描述 |
| -- | -- |
| [JSVM_DeserializeResult](capi-jsvm-jsvm-deserializeresult--8h.md) result | 需要进行释放的后台反序列化结果。 |
...
| 参数项 | 描述 |
| -- | -- |
| [JSVM_Env](capi-jsvm-jsvm-env--8h.md) env | 调用该 JSVM-API 的环境。 |
...
| [JSVM_Value](capi-jsvm-jsvm-callbackstruct.md#jsvm_value)* result | 表示创建的 JavaScript ArrayBuffer。 |
