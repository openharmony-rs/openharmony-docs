...
该接口当前主要为兼容保留，不再作为推荐用法。新开发应优先使用更符合ArkTS-Sta设计规范的实例化原子类型接口。
...
> - 基于typedArray的静态Atomics接口主要为兼容保留，不再作为推荐用法，应更优先使用实例化原子类型接口，参见[Atomics (实例化原子类型接口)](arkts-sta-atomics.md)。
