...
- [CompletableJob](#completablejob)：继承自Job，可手动设置任务执行结果。

## Job
执行任务返回值的句柄，用于表征一个任务，可以通过[Await](#await)等待获取任务的返回值。
...
CompletableJob的构造器，用于构造一个开发者能够设置任务成功与失败的[CompletableJob](#completablejob)实例。
...
```
通过Await获取失败状态的CompletableJob的值。
```ts
import hilog from '@ohos.hilog'
