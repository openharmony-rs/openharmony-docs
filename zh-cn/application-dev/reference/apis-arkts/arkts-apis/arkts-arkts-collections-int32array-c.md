...
| 错误码ID | 错误信息 |
| --- | --- |
| [10200012](../../errorcode-universal.md#10200012-The) | The Int32Array's constructor cannot be directly invoked. |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| array | ArrayLike&lt;number&gt; \| ArrayBuffer | 是 | 用于构造ArkTS Int32Array的对象。当参数类型是ArrayBuffer时，<br/>buffer所占的字节数须是4的整数倍。 |
...
| end | number | 否 | 源终止位置的下标（不包含end位置的元素）。如果传入负数，则表示`end + Int32Array.length`<br/>位置的下标。默认值为ArkTS Int32Array的长度。 |
...
| end | number | 否 | 结束填充的索引（不包括该元素）。如果传入负数，则表示`end + Int32Array.length`<br/>位置的索引。默认值为ArkTS Int32Array的长度。 |
...
| fromIndex | number | 否 | 搜索的起始下标。默认值为**0**。如果下标大于等于ArkTS Float32Array的长度，<br/>则返回**-1**。如果传入负数，则从ArkTS Float32Array末尾开始搜索。 |
...
| --- | --- | --- | --- |
| searchElement | number | 是 | 待索引的值。 |
| fromIndex | number | 否 | 搜索的起始下标。默认值为**0**。如果下标大于等于ArkTS Int32Array的长度，<br/>则返回**-1**。如果传入负数，则从ArkTS Int32Array末尾开始搜索。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callbackFn | TypedArrayMapCallback&lt;number, Int32Array&gt; | 是 | 一个最多接受三个参数的函数。<br/>map方法对数组中的每个元素调用一次callbackfn函数。 |
...
| 类型 | 说明 |
| --- | --- |
| Int32Array | 新的ArkTS Int32Array实例。可能原因：1.必填参数未指定；<br/><br/>2.参数类型不正确；3.参数校验失败。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callbackFn | TypedArrayReduceCallback&lt;number, number, Int32Array&gt; | 是 | 一个最多接受四个参数的函数。<br/>reduce方法对数组中的每个元素调用一次callbackfn函数。 |
...
| --- | --- | --- | --- |
| callbackFn | TypedArrayReduceCallback&lt;number, number, Int32Array&gt; | 是 | 一个最多接受四个参数的函数。<br/>reduce方法对数组中的每个元素调用一次callbackfn函数。 |
| initialValue | number | 是 | 如果指定了initialValue，则将其作为开始累加的初始值。<br/>首次调用callbackfn函数时以此值作为参数，而不是使用数组元素值。 |
...
| initialValue | U | 是 | 作为回调函数首次调用的第一个参数的初始值。<br/><br/>如果未提供初始值，则使用Int32Array的最后一个元素作为初始值，<br/><br/>并且回调从倒数第二个元素开始执行。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| U | 由归约函数最后一次调用返回的结果。可能原因：<br/>1.必填参数未指定。<br/>2.参数类型不正确。 |
...
| 类型 | 说明 |
| --- | --- |
| number | 由归约函数最后一次调用返回的结果。可能原因：<br/>1.必填参数未指定。<br/>2.参数类型不正确。 |
...
| end | number | 否 | 结束索引（不包括该元素）。如果传入负数，则表示`end + Int32Array.length`位置<br/>的索引。默认值为ArkTS Int32Array的长度。 |
