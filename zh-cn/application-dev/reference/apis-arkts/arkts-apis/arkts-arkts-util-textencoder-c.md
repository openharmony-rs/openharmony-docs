...
| 类型 | 说明 |
| --- | --- |
| EncodeIntoUint8ArrayInfo | 返回该对象，其中 read 表示已编码的字符数，written 表示已编码字符所占用的字节数。[since 9 - 10]<br/>@returns { EncodeIntoUint8ArrayInfo } 获取到的对象。**read** 表示已编码的字符数，**written** 表示已编码字符所占用的<br/>字节数。[since 11] |
