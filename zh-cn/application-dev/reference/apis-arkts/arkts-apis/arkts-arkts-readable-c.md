...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | ReadableOptions | 是 | **Readable**构造函数中的选项。 |
...
需要由开发者实现此API。在可读流首次调用[on](arkts-arkts-writable-c.md#on-1)时调用此API。使用异步回调返回结果。
...
检查可读流是否已暂停。流在调用[pause()](arkts-arkts-readable-c.md#pause-1)后暂停，在调用[resume()](arkts-arkts-readable-c.md#resume-1)后从暂停状态恢复。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| destination | Writable | 否 | 待分离的可写流。默认值为**undefined**。 |
