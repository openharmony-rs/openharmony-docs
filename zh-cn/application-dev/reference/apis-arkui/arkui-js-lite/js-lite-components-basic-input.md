...
- 当input类型为checkbox、radio时，支持如下事件：
  ...
  | click | - | 点击动作触发该事件。 |
  | longpress | - | 长按动作触发该事件。 |
  | swipe<sup>5+</sup> | [SwipeEvent](js-lite-common-events.md#swipeevent) | 组件上快速滑动后触发的事件对象。 |
...
| font-family | string | SourceHanSansSC-Regular | 否 | 字体。从API版本26.2.0开始，支持任意字体。API版本26.2.0之前，仅支持SourceHanSansSC-Regular字体。 |
