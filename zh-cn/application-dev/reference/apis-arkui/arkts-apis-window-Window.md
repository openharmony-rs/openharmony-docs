...
从OpenHarmony 5.1.1开始，该接口在支持并处于[电脑模式](../../windowmanager/window-terminology.md#free-windows电脑模式)的设备上调用不生效也不报错；在支持并处于[自由多窗模式](../../windowmanager/window-terminology.md#free-windows自由多窗模式)的设备、支持但不处于自由窗口状态的设备及不支持自由窗口状态的设备上可正常调用。
