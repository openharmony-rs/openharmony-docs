...
开启本窗口所处屏幕变化事件的监听。比如，当前窗口移动到其他屏幕时，可以从此接口监听到这个行为（非实时通知，如跨屏拖拽时需要松手后才会触发回调通知）。
...
窗口亮度生效时只会影响当前设备[内置屏](../../displaymanager/display-terminology.md#实体屏)的屏幕亮度，无法修改[虚拟屏](../../displaymanager/display-terminology.md#虚拟屏)（如投屏所在的屏幕）的屏幕亮度。
