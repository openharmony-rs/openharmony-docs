...
```ts
import { Entry, Text, Column, Component, Button, ClickEvent, GestureShortcut, Scroll, State, ColumnOptions } from '@kit.ArkUI'
import { SmartGestureController } from '@ohos.arkui.UIContext'
...
import { SmartGestureController, BaseGestureHandlingProposal, GestureHandlingResolution } from '@ohos.arkui.UIContext'

@Entry
@Component
struct SmartGestureControllerExample {
  private controller: SmartGestureController = this.getUIContext().getSmartGestureController();
  private smartGestureMonitor: Callback<BaseGestureHandlingProposal, GestureHandlingResolution> =
    (proposal: BaseGestureHandlingProposal) => {
...
> **说明：**
> 
> - 仅当目标组件满足以下全部条件时，请求才会生效：组件可以响应智慧手势，且组件在屏幕内可见。
...
ArkTS-Dyn: constructor(node: FrameNode, pageCount: number)

ArkTS-Sta: constructor(node: FrameNode, pageCount: int)
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| ---- | ---- | ---- | ---- | ---- |
| pageCount | int | 否 | 否 | 智慧手势翻页数量。<br/>取值范围：[0, +∞)，小于0时按0处理。<br/>单位为页。 |
...
ArkTS-Dyn: constructor(node: FrameNode, distance: number)

ArkTS-Sta: constructor(node: FrameNode, distance: double)
...
struct SmartGestureControllerExample {
  private controller: SmartGestureController = this.getUIContext().getSmartGestureController();
  private arrayList: Array<int> = [0, 1, 2, 3, 4, 5, 6];
  private smartGestureMonitor: Callback<BaseGestureHandlingProposal, GestureHandlingResolution> =
    (proposal: BaseGestureHandlingProposal) => {
      let result = new GestureHandlingResolution(true);
  ...
  build() {
    Scroll() {
      Column({ space: 12 } as ColumnOptions) {
        List({ space: 8 }) {
          this.arrayList.forEach((item: int) => {
            ListItem() {
