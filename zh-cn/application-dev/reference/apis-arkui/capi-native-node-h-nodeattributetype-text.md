...
| 参数项 | 描述 |
| -- | -- |
| .value[i].i32 | 表示第i行文本的尾部缩进值，单位为fp。取值范围：[0, +∞)。当.size大小为1时，所有行共享相同的尾部缩进.value[0].i32；当.size大于1时，第i行使用.value[i].i32；当文本行数超过.size，则超出部分复用.value[.size - 1].i32。 |
