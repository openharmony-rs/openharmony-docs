...
遮罩类型枚举。遮罩是一种用于限制组件显示区域的技术，通过指定形状来裁剪组件内容，只有遮罩形状内的部分才会显示。
...
在动画中定义[onFinish](../../apis-arkui/arkui-ts/ts-transition-animation-component.md#transition)回调的类型。
...
| ARKUI_ANIMATION_FILL_MODE_BACKWARDS = 2 | 动画将在应用于目标时立即应用第一个关键帧中定义的值，并在[delay](../../apis-arkui/arkui-ts/ts-keyframeAnimateTo.md#KeyframeAnimateParam对象说明)期间保留此值。 |
