...
定义[ComponentContent](#componentcontent-1)和[ReactiveComponentContent](#reactivecomponentcontent22)的基类。
...
ArkTS-Dyn: 继承自[Content](js-apis-arkui-Content.md#content-1)。

ArkTS-Sta: 继承自[ComponentContentBase](#componentcontentbase)。
...
ArkTS-Dyn: reuse(param?: Object): void

ArkTS-Sta: reuse(param?: RecordData): void
...
| 参数名 | 类型   | 必填 | 说明                                                                     |
| ------ | ------ | ---- | ------------------------------------------------------------------------ |
| param  | ArkTS-Dyn: Object<br>ArkTS-Sta: [RecordData](../apis-arkdata/js-apis-data-preferences.md#recorddata23) | 否   | 用于复用ComponentContent的参数。该参数将直接用于ComponentContent中所有顶层自定义组件的复用，应该包含每个自定义组件的构造函数参数所需内容，否则会导致未定义行为。调用此方法将同步触发内部自定义组件的[aboutToReuse](../../reference/apis-arkui/arkui-ts/ts-custom-component-lifecycle.md#abouttoreuse10)生命周期回调，并将该参数作为回调的入参。默认值为undefined，此时ComponentContent中的自定义组件将直接使用构造时的数据源。 |
...
ArkTS-Dyn: ReactiveComponentContent继承自[Content](js-apis-arkui-Content.md#content-1)，是一个用于动态承载和复用UI内容的容器组件。它通过@Builder函数构建UI，并利用[ReactiveBuilderNode](./js-apis-arkui-builderNode.md#reactivebuildernode22)生成和管理组件树。该组件的核心价值在于为动态内容提供完整的生命周期管理，使其能够融入ArkUI的组件复用体系，特别适用于长列表等需要高性能渲染的场景。

ArkTS-Sta: ReactiveComponentContent继承自[ComponentContentBase](#componentcontentbase)，是一个用于动态承载和复用UI内容的容器组件。它通过@Builder函数构建UI，并利用[ReactiveBuilderNode](./js-apis-arkui-builderNode.md#reactivebuildernode22)生成和管理组件树。该组件的核心价值在于为动态内容提供完整的生命周期管理，使其能够融入ArkUI的组件复用体系，特别适用于长列表等需要高性能渲染的场景。
...
ReactiveComponentContent的构造函数。用于按照带参数的[CustomBuilder](./arkui-ts/ts-types.md#custombuilder8)创建组件树。
...
| 参数名    | 类型                                      | 必填 | 说明                               |
| --------- | ----------------------------------------- | ---- | ---------------------------------- |
| uiContext | [UIContext](./arkts-apis-uicontext-uicontext.md) | 是   | 用于创建对应节点的UI上下文。 |
| builder  | [CustomBuilder](./arkui-ts/ts-types.md#custombuilder8)  | 是   |   CustomBuilder函数。 |
| options | [BuildOptions](./js-apis-arkui-builderNode.md#buildoptions12)                                                    | 否  |  build的配置参数。<br/>默认值：参考[BuildOptions](./js-apis-arkui-builderNode.md#buildoptions12)各个成员的默认值。 |
...
| 参数名 | 类型   | 必填 | 说明                                                                     |
| ------ | ------ | ---- | ------------------------------------------------------------------------ |
| param  | ArkTS-Dyn: Object<br>ArkTS-Sta: [RecordData](../apis-arkdata/js-apis-data-preferences.md#recorddata23) | 否   | 用于复用[ReactiveComponentContent](#reactivecomponentcontent22)的参数。该参数将直接用于ReactiveComponentContent中所有顶层自定义组件的复用，应该包含每个自定义组件的构造函数参数所需内容，否则会导致未定义行为。调用此方法将同步触发内部自定义组件的[aboutToReuse](../../reference/apis-arkui/arkui-ts/ts-custom-component-lifecycle.md#abouttoreuse10)生命周期回调，并将该参数作为回调的入参。默认值为undefined，此时ReactiveComponentContent中的自定义组件将直接使用构造时的数据源。 |
...
判断ComponentContent是否通过transfer.transferStatic或transfer.transferDynamic方法创建。如果通过上述两个接口创建，则不支持以下方法：[update](#update)，[dispose](#dispose)，[updateConfiguration](#updateconfiguration)，[inheritFreezeOptions](#inheritfreezeoptions20)。
...
| 类型                | 说明                                                                    |
| ------------------ | ----------------------------------------------------------------------- |
| boolean | 返回ReactiveComponentContent是否通过transfer.transferStatic或transfer.transferDynamic方法创建。<br/>true：ComponentContent通过transfer.transferStatic或transfer.transferDynamic方法创建。<br/>false：ComponentContent不通过transfer.transferStatic或transfer.transferDynamic方法创建。|