...
1. 应用浮窗正在启动或已启动时，再次启动。

2. 应用浮窗正在停止或已停止时，再次停止。

3. 重复注册应用浮窗回调。

**处理步骤**

1. 启动应用浮窗前，检查应用浮窗是否处于已启动/正在启动状态。

2. 停止应用浮窗前，检查应用浮窗是否处于已停止/正在停止状态。

3. 注册应用浮窗回调前，确保回调未注册。

## 1300031 应用浮窗状态不支持该操作
...
1. 应用浮窗已启动但未停止时，执行绑定/解绑等要求停止态的操作。

2. 应用浮窗未启动时，执行需要启动态的操作（例如停止、恢复主窗口或获取窗口属性）。

3. 应用浮窗处于正在停止状态时，执行启动操作。

**处理步骤**

1. 操作前检查应用浮窗当前状态，确保其满足接口的状态要求。

2. 如需停止应用浮窗，请先确保应用浮窗已启动。

3. 等待应用浮窗停止完成（通过状态变化回调确认）后再执行后续操作。
...
2. 启动应用浮窗前，请确保应用存在前台窗口。

## 1300034 应用浮窗与其他悬浮窗口操作冲突
