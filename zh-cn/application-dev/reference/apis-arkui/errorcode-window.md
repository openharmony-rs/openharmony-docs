...
1. 标准悬浮窗正在启动中或处于已启动的状态时，重复启动标准悬浮窗。

2. 标准悬浮窗正在停止中或处于已停止的状态时，重复停止标准悬浮窗。

3. 重复注册标准悬浮窗回调。

**处理步骤**

1. 建议通过[onStateChange](js-apis-floatView.md#onstatechange)获取当前状态变化。启动标准悬浮窗前，检查标准悬浮窗是否处于已启动的状态。

2. 建议通过[onStateChange](js-apis-floatView.md#onstatechange)获取当前状态变化。停止标准悬浮窗前，检查标准悬浮窗是否处于已停止的状态。

3. 注册标准悬浮窗回调前，确保回调未注册。

## 1300031 标准悬浮窗状态不支持该操作
...
1. 标准悬浮窗已启动但未停止时，执行要求停止态的操作（例如绑定、解绑等）。

2. 标准悬浮窗未启动时，执行需要启动态的操作（例如停止、恢复主窗口或获取窗口属性）。

3. 标准悬浮窗处于正在停止状态时，执行启动操作。

**处理步骤**

1. 操作前检查标准悬浮窗当前状态，确保其满足接口的状态要求。

2. 如需停止标准悬浮窗，请先确保标准悬浮窗已启动。

3. 等待标准悬浮窗停止完成（通过状态变化回调确认）后再执行后续操作。
...
2. 启动标准悬浮窗时，context关联的主窗不在前台。

**处理步骤**

1. 同一应用只能启动一个标准悬浮窗，请避免同时启动多个标准悬浮窗。

2. 启动标准悬浮窗前，请确保传入context的应用窗口处于前台。

## 1300034 标准悬浮窗与其他悬浮窗口操作冲突
