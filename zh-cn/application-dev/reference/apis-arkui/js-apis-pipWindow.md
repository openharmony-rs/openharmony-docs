...
该模块提供画中画基础功能，包括判断当前系统是否支持画中画功能，以及创建画中画控制器用于启动或停止画中画等。适用于视频播放、视频通话、视频会议或视频直播场景下，以小窗（画中画）模式呈现内容，支持用户在处理其他任务的同时继续观看视频内容，提升多任务处理效率和用户体验，保持视频播放的连续性。
...
| 参数名          | 类型                                       | 必填        | 说明                                                                                                                                                                                                                                     |
|--------------|------------------------------------------|-----------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| config       | [PiPConfiguration](#pipconfiguration)    | 是         | 创建画中画控制器的参数。该参数不能为空，并且构造该参数的context和componentController不能为空。当不传入contentNode参数时，构造该参数的componentController不能为空。构造该参数时，如果指定了templateType，需保证templateType是[PiPTemplateType](#piptemplatetype)类型；如果指定了controlGroups，需保证controlGroups与templateType匹配，详见[PiPControlGroup](#pipcontrolgroup12)。 |
...
| defaultWindowSizeType<sup>19+</sup>| number                                                                     | 否   | 是  |  当前应用第一次拉起画中画的窗口大小，取值范围为[0, 2]。<br/>0：代表不设置大小。按照上个应用的画中画关闭前的大小启动；<br/>1：代表小窗；<br/>2：代表大窗；<br/>不传值则为默认值0。<br/>**原子化服务API：** 从API version 19开始，该接口支持在原子化服务中使用。                                                                 |
...
| scale  | number   | 否   | 否   | 窗口缩放比，显示大小相对于width和height的缩放比，该参数为浮点数，取值范围大于0.0，小于等于1.0。等于1表示与width和height一样大。传入0、负数或超过屏幕高度的值时抛出异常。 |
...
| OPEN    | 1   | 打开状态。用于MICROPHONE_SWITCH、CAMERA_SWITCH、MUTE_SWITCH等控件，表示麦克风、摄像头或静音功能处于打开状态。            |
| CLOSE       | 0   | 关闭状态。用于MICROPHONE_SWITCH、CAMERA_SWITCH、MUTE_SWITCH等控件，表示麦克风、摄像头或静音功能处于关闭状态。          |
...
应用调用此方法停止画中画，使用Promise异步回调。调用该方法前需确保已通过[PiPWindow.create()](#pipwindowcreate)创建PiPController实例且实例未被销毁。
...
应用调用此方法设置是否在返回桌面时自动启动画中画，默认不自动拉起。调用该方法前需确保已通过[PiPWindow.create()](#pipwindowcreate)创建PiPController实例且实例未被销毁。
...
| 参数名      | 类型        | 必填    | 说明                              |
|----------|-----------|-------|---------------------------------|
| enable   | boolean   | 是     | 是否在返回桌面时自动启动画中画。true表示自动启动画中画，false表示不自动启动画中画。当设置-系统-智慧多窗-自动启动画中画开关为关闭状态时，就算该参数配置为true，应用返回桌面时也不会自动启动画中画窗口。  |
...
当媒体源切换时，开发者调用此接口向画中画控制器更新媒体源尺寸信息。
...
| 参数名    | 类型     | 必填  | 说明                                     |
|--------|--------|-----|----------------------------------------|
| width  | number | 是   | 表示媒体内容宽度，必须为大于0的整数，单位为px，传入0或负数时抛出异常，用于更新画中画窗口比例。调用前需确保PiPController已创建且未销毁。  |
| height | number | 是   | 表示媒体内容高度，必须为大于0的整数，单位为px，传入0或负数时抛出异常，用于更新画中画窗口比例。调用前需确保PiPController已创建且未销毁。  |
...
| 错误码ID | 错误信息                                                                                                        |
|-------|-------------------------------------------------------------------------------------------------------------|
| 401   | Params error. Possible causes: 1.The PiPController is not created or destroyed. |
...
应用调用此方法更新画中画控制面板控件功能状态，用于同步控件当前状态。
...
| 参数名    | 类型     | 必填  | 说明                                                                                                 |
|--------|--------|-----|----------------------------------------------------------------------------------------------------|
| controlType  | [PiPControlType](#pipcontroltype12)  | 是   | 表示画中画控制面板控件类型。仅支持VIDEO_PLAY_PAUSE、MICROPHONE_SWITCH、CAMERA_SWITCH和MUTE_SWITCH这几种控件类型，传入其他控件类型不生效也不报错，建议在调用前检查控件类型是否在支持范围内。 |
...
| 参数名         | 类型     | 必填  | 说明                                     |
|-------------|--------|-----|----------------------------------------|
| controlType | [PiPControlType](#pipcontroltype12)  | 是   | 表示画中画控制面板控件类型。仅支持VIDEO_PLAY_PAUSE、MICROPHONE_SWITCH、CAMERA_SWITCH和MUTE_SWITCH这几种控件类型，传入其他控件类型不生效。  |
...
获取画中画窗口信息，使用Promise异步回调。调用该方法前需确保已通过[PiPWindow.create()](#pipwindowcreate)创建PiPController实例且实例未被销毁。
