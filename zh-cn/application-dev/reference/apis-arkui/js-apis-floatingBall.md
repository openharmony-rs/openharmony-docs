...
| titleColor | string | 否 | 是 | 闪控球标题文字颜色，为不带透明度的十六进制颜色格式（例如'#008EF5'或'#FF008EF5'），不传入时根据背景色反色处理。配置此属性时，必须配置背景色backgroundColor。<br>**起始版本**：26.0.0 <br>**模型约束：** 此接口仅可在Stage模型下使用。 |
| contentColor | string | 否 | 是 | 闪控球内容文字颜色，为不带透明度的十六进制颜色格式（例如'#008EF5'或'#FF008EF5'），不传入时根据背景色反色处理。配置此属性时，必须配置背景色backgroundColor。<br>**起始版本**：26.0.0 <br>**模型约束：** 此接口仅可在Stage模型下使用。 |
