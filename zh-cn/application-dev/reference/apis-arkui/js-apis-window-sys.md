...
对于主屏幕与虚拟屏之间的窗口迁移，主窗及其子窗（全局悬浮窗除外）会一起被迁移到对应屏幕上且被抬升，如果存在子窗，最上层可获焦子窗会获取焦点，否则主窗口获焦；对于主屏与扩展屏之间的窗口迁移（处于同一全局坐标系下），只会将主窗口迁移到对应屏幕，抬升并获取焦点。

若指定的主窗口处于分屏或全景多窗状态，会先退出分屏或全景多窗再进行迁移。
