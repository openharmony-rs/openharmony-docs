...
| [OH_ArkUI_FontConfigs](capi-arkui-nativemodule-oh-arkui-fontconfigs.md) | OH_ArkUI_FontConfigs | 定义文本的字体配置，当前支持通过相关接口设置和获取字体粗细配置，适用于需要自定义字体粗细显示效果的场景。可以通过{@link OH_ArkUI_FontConfigs_Create}接口创建字体配置对象，通过<br>{@link OH_ArkUI_FontConfigs_Destroy}接口销毁字体配置对象。配置创建后，可通过以下接口进行设置和查询：通过<br>{@link OH_ArkUI_FontConfigs_SetFontWeightConfigs}接口设置字体粗细配置，通过{@link OH_ArkUI_FontConfigs_GetFontWeightConfigs} 接口获取字体粗细配置。 |
| [OH_ArkUI_NativeModule_LineSpacingOptions](capi-arkui-nativemodule-oh-arkui-nativemodule-linespacingoptions.md) | OH_ArkUI_NativeModule_LineSpacingOptions | 定义文本行间距选项对象，用于配置文本行间距是否仅在行与行之间生效。可以通过{@link OH_ArkUI_NativeModule_LineSpacingOptions_Create}接口创建行间距选项对象，<br>创建后必须在使用完毕后调用{@link OH_ArkUI_NativeModule_LineSpacingOptions_Destroy}接口销毁对象以释放资源。二者必须成对使用，否则会导致内存泄漏。创建对象后，可使用<br>{@link OH_ArkUI_NativeModule_LineSpacingOptions_SetOnlyBetweenLines}接口设置行间距是否仅在行与行之间生效，通过<br>{@link OH_ArkUI_NativeModule_LineSpacingOptions_GetOnlyBetweenLines}接口获取行间距配置。适用于需要精确控制文本行间距显示效果的场景， 如排版要求首尾行不加行间距的文本显示。 |
...
| 名称 | 描述 |
| -- | -- |
| [ArkUI_TextMarqueeOptions* OH_ArkUI_TextMarqueeOptions_Create()](#oh_arkui_textmarqueeoptions_create) | 创建文本跑马灯模式配置项。当该对象不再使用时，请调用[OH_ArkUI_TextMarqueeOptions_Dispose](capi-text-h.md#oh_arkui_textmarqueeoptions_dispose)销毁，以释放资源避免内存泄漏。 |
| [void OH_ArkUI_TextMarqueeOptions_Dispose(ArkUI_TextMarqueeOptions* option)](#oh_arkui_textmarqueeoptions_dispose) | 销毁文本跑马灯模式配置项对象。与[OH_ArkUI_TextMarqueeOptions_Create](capi-text-h.md#oh_arkui_textmarqueeoptions_create)成对使用，否则会导致内存泄漏。 |
...
| [OH_ArkUI_TextDataDetectorConfig* OH_ArkUI_TextDataDetectorConfig_Create()](#oh_arkui_textdatadetectorconfig_create) | 创建一个文本实体识别配置对象。当该对象不再使用时，请调用[OH_ArkUI_TextDataDetectorConfig_Destroy](capi-text-h.md#oh_arkui_textdatadetectorconfig_destroy)销毁，以释放资源避免内存泄漏。 |
...
| [void OH_ArkUI_TextController_Destroy(OH_ArkUI_TextController* controller)](#oh_arkui_textcontroller_destroy) | 销毁文本控制器对象。与[OH_ArkUI_TextController_Create](capi-text-h.md#oh_arkui_textcontroller_create)成对使用，否则会导致内存泄漏。 |
...
| [void OH_ArkUI_FontWeightConfigs_Destroy(OH_ArkUI_FontWeightConfigs* option)](#oh_arkui_fontweightconfigs_destroy) | 销毁文本字体粗细配置对象。与[OH_ArkUI_FontWeightConfigs_Create](capi-text-h.md#oh_arkui_fontweightconfigs_create)成对使用，否则会导致内存泄漏。 |
...
| [void OH_ArkUI_FontConfigs_Destroy(OH_ArkUI_FontConfigs* option)](#oh_arkui_fontconfigs_destroy) | 销毁文本字体配置对象。与[OH_ArkUI_FontConfigs_Create](capi-text-h.md#oh_arkui_fontconfigs_create)成对使用，否则会导致内存泄漏。 |
...
| ARKUI_FONT_WEIGHT_NORMAL | 字体粗细正常，与ARKUI_FONT_WEIGHT_NORMAL相同。 |
| ARKUI_FONT_WEIGHT_BOLDER | 字体非常粗。 |
| ARKUI_FONT_WEIGHT_LIGHTER | 字体较细。 |
| ARKUI_FONT_WEIGHT_MEDIUM | 字体粗细适中。 |
| ARKUI_FONT_WEIGHT_REGULAR | 字体粗细正常，与ARKUI_FONT_WEIGHT_NORMAL相同。 |
...
| ARKUI_TEXT_HEIGHT_ADAPTIVE_POLICY_MIN_FONT_SIZE_FIRST | 设置文本高度自适应方式为以缩小字体优先。当文本内容超出显示区域时，优先通过缩小字体大小来适应高度限制，确保文本完整显示，字体大小会在允许范围内自动调整。 |
| ARKUI_TEXT_HEIGHT_ADAPTIVE_POLICY_LAYOUT_CONSTRAINT_FIRST | 设置文本高度自适应方式为以布局约束（高度）优先。严格遵循布局约束的高度设置，不自动调整文本高度或字体大小，超出部分可能被截断或通过其他方式处理。 |
...
| 类型 | 说明 |
| -- | -- |
| [ArkUI_TextMarqueeOptions*](capi-arkui-nativemodule-arkui-textmarqueeoptions.md) | 返回指向文本跑马灯模式配置项对象的指针。如果创建失败，返回空指针。使用后需调用[OH_ArkUI_TextMarqueeOptions_Dispose](capi-text-h.md#oh_arkui_textmarqueeoptions_dispose)销毁。 |
...
销毁文本跑马灯模式配置项对象。与[OH_ArkUI_TextMarqueeOptions_Create](capi-text-h.md#oh_arkui_textmarqueeoptions_create)成对使用，否则会导致内存泄漏。
...
| 类型 | 说明 |
| -- | -- |
| [OH_ArkUI_TextDataDetectorConfig*](capi-arkui-nativemodule-oh-arkui-textdatadetectorconfig.md) | 指向[OH_ArkUI_TextDataDetectorConfig](capi-arkui-nativemodule-oh-arkui-textdatadetectorconfig.md)对象的指针。如果创建失败，返回空指针。使用后需调用      [OH_ArkUI_TextDataDetectorConfig_Destroy](capi-text-h.md#oh_arkui_textdatadetectorconfig_destroy)销毁。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_ArkUI_TextController*](capi-arkui-nativemodule-oh-arkui-textcontroller.md) | 指向文本组件控制器对象的指针。如果创建失败，返回空指针。使用后需调用[OH_ArkUI_TextController_Destroy](capi-text-h.md#oh_arkui_textcontroller_destroy)销毁。 |
...
销毁文本控制器对象。与[OH_ArkUI_TextController_Create](capi-text-h.md#oh_arkui_textcontroller_create)成对使用，否则会导致内存泄漏。
...
| 类型 | 说明 |
| -- | -- |
| [OH_ArkUI_FontWeightConfigs*](capi-arkui-nativemodule-oh-arkui-fontweightconfigs.md) | 指向文本字体粗细配置对象的指针。如果创建失败，返回空指针。使用后需调用[OH_ArkUI_FontWeightConfigs_Destroy](capi-text-h.md#oh_arkui_fontweightconfigs_destroy)销毁。当配置对象为空指针时，不应用默认值，      文本字体粗细行为与父组件保持一致。 |
...
销毁文本字体粗细配置对象。与[OH_ArkUI_FontWeightConfigs_Create](capi-text-h.md#oh_arkui_fontweightconfigs_create)成对使用，否则会导致内存泄漏。
...
| 类型 | 说明 |
| -- | -- |
| [OH_ArkUI_FontConfigs*](capi-arkui-nativemodule-oh-arkui-fontconfigs.md) | 返回指向文本字体配置对象的指针。如果创建失败，返回空指针。使用后需调用[OH_ArkUI_FontConfigs_Destroy](capi-text-h.md#oh_arkui_fontconfigs_destroy)销毁。 |
...
销毁文本字体配置对象。与[OH_ArkUI_FontConfigs_Create](capi-text-h.md#oh_arkui_fontconfigs_create)成对使用，否则会导致内存泄漏。
...
| 类型 | 说明 |
| -- | -- |
| ArkUI_ErrorCode | {@link ARKUI_ERROR_CODE_NO_ERROR} 表示操作成功。{@link ARKUI_ERROR_CODE_PARAM_INVALID} 表示options参数为空。 |
...
| 类型 | 说明 |
| -- | -- |
| ArkUI_ErrorCode | {@link ARKUI_ERROR_CODE_NO_ERROR} 表示操作成功。{@link ARKUI_ERROR_CODE_PARAM_INVALID} 表示任意参数为空。 |
