...
| -- | -- |
| ArkUI_NativeDialogHandle handle | 指向自定义弹窗控制器的指针。 |
|  int32_t top | 上边框的样式。参数类型{@link ArkUI_BorderStyle}，默认值为ARKUI_BORDER_STYLE_SOLID。 |
|  int32_t right | 右边框的样式。参数类型{@link ArkUI_BorderStyle}，默认值为ARKUI_BORDER_STYLE_SOLID。 |
