...
无障碍事件信息。用于承载应用向无障碍服务上报的无障碍事件内容，可通过OH_ArkUI_SendAccessibilityAsyncEvent接口上报：既适用于无障碍服务或辅助应用要求控件执行操作后，发送执行结果事件以通知操作结果，也适用于组件状态变化（如点击、选中、文本更新、页面切换、焦点变化等）、主动播报、请求聚焦等需要向无障碍服务上报事件的场景。
