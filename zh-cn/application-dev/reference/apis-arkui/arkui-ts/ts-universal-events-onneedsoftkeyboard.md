...
ArkTS-Dyn: onNeedSoftkeyboard(onNeedSoftkeyboardCallback: OnNeedSoftkeyboardCallback | undefined): T

ArkTS-Sta: onNeedSoftkeyboard(onNeedSoftkeyboardCallback: OnNeedSoftkeyboardCallback | undefined): this
...
XComponent组件使用该方法时，如果返回值为`true`且XComponent组件使用OH_NativeXComponent_SetNeedSoftKeyboard设置了需要键盘，才会保留键盘，如果返回值为`false`，无论组件如何设置，键盘都不会保留。
...
```ts
import { Entry, Column, Component, Button, TextInput } from '@kit.ArkUI';
@Entry
@Component
struct Index {
  build() {
