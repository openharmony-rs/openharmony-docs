...
| zFar | ArkTS-Dyn: number</br>ArkTS-Sta: double | 否 | 否 | 远裁剪面距离。无单位。必须为正数。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| -------- | -------- | -------- | -------- | -------- |
| x | ArkTS-Dyn: number</br>ArkTS-Sta: int | 否 | 否 | 水平方向偏移量，单位为像素。 |
| y | ArkTS-Dyn: number</br>ArkTS-Sta: int | 否 | 否 | 垂直方向偏移量，单位为像素。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| -------- | -------- | -------- | -------- | -------- |
| componentWidth | ArkTS-Dyn: number</br>ArkTS-Sta: double | 是 | 否 | 组件宽度，单位为vp。 |
| componentHeight | ArkTS-Dyn: number</br>ArkTS-Sta: double | 是 | 否 | 组件高度，单位为vp。 |
...
@Entry
@Component
struct DepthComponentStaticExample {
  build() {
    Column() {
      // 请开发者替换为实际的资源文件
      DepthComponent($r('app.media.background')) {
