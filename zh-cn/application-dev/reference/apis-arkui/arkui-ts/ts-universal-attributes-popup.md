...
>  **说明：**
>
>  - 不支持在输入法类型窗口中使用子窗（showInSubWindow为true）的bindPopup，详情见输入法框架的约束与限制说明createPanel。
>
...
| levelMode | LevelMode | 否 | 是 | 设置气泡的显示层级模式。<br/>默认值：LevelMode.OVERLAY<br/>**起始版本：** 26.0.0<br/>**原子化服务API：** 从API版本26.0.0开始，该接口支持在原子化服务中使用。<br/>**模型约束：** 此接口仅可在Stage模型下使用。 |
...
配置弹出气泡的参数。使用UIContext中的getPromptAction()方法获取到PromptAction对象，再通过该对象调用openPopup和updatePopup时传入的options参数。
...
| showInSubWindow | boolean                                  | 否    | 是   | 气泡是否显示在创建的子窗里。<br/>true：气泡会显示在创建的子窗里；false：气泡会显示在对应的主窗中。<br />默认值：false<br />**说明：**<br />不支持通过updatePopup进行更新。<br/>**原子化服务API：** 从API version 18开始，该接口支持在原子化服务中使用。 |
