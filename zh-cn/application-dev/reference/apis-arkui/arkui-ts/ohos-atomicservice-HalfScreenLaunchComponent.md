...
>
> 如果需要在该组件中实现一个可嵌入式运行的原子化服务时，原子化服务必须继承自EmbeddableUIAbility。若不继承自EmbeddableUIAbility，系统无法确保原子化服务正常运行。
...
| onTerminated | Callback\<[TerminationInfo](ts-container-embedded-component.md#terminationinfo)> | 否 | - | 被拉起的嵌入式运行原子化服务通过点击原子化服务退出按钮、手势侧滑、调用terminateSelfWithResult或者terminateSelf正常退出时，触发本回调函数。 |
| onReceive<sup>20+</sup> | Callback\<Record<string, Object>> | 否 | - | 被拉起的嵌入式运行原子化服务通过@ohos.window (窗口)调用API时，触发本回调。<br/>**原子化服务API：** 从API version 20开始，该接口支持在原子化服务中使用。|
