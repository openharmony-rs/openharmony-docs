...
| poolAccepts | string[] | 否 | 是 | 自定义组件全局复用池接纳的自定义组件名称，reusePool参数被设置时，poolAccepts必须为非空数组。poolAccepts和reusePool都没有赋值时，全局复用不生效。<br>**起始版本：** 26.0.0<br>**卡片能力（仅ArkTS-Dyn）：** 从API版本26.0.0开始，该接口支持在ArkTS卡片中使用。<br>**原子化服务API（仅ArkTS-Dyn）：** 从API版本26.0.0开始，该接口支持在原子化服务中使用。 |
