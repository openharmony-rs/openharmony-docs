...
>  **说明：**
>
>  在复杂界面中使用多组件嵌套时，若布局组件的嵌套层数过深或嵌套的组件数量过多，将会产生额外开销。建议通过移除冗余节点、利用布局边界减少布局计算、合理采用渲染控制语法及布局组件方法来优化性能。
...
| 参数名 | 类型 | 必填 | 说明 |
| -------- | -------- | -------- | -------- |
| options | ArkTS-Dyn: [ColumnOptions](#columnoptions18对象说明) \| [ColumnOptionsV2](#columnoptionsv218对象说明)<br>ArkTS-Sta: [ColumnOptions](#columnoptions18对象说明) \| [ColumnOptions](#columnoptions18对象说明) \| [ColumnOptionsV2](#columnoptionsv218对象说明) | 否 | Column组件的间距配置选项。通过space属性设置纵向布局元素垂直方向间距，space支持设置number、string或Resource类型。当需要为子组件设置固定垂直间距时传入此参数；省略时不设置子组件间距。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| -------- | -------- | -------- | -------- | -------- |
| space | [SpaceType](#spacetype18) | 否 | 是 | 设置纵向布局元素垂直方向间距。<br>space为负数或者[justifyContent](#justifycontent8)设置为FlexAlign.SpaceBetween、FlexAlign.SpaceAround、FlexAlign.SpaceEvenly时，space不生效。<br>取值范围：[0, +∞)<br>默认值：0<br>单位：vp<br>非法值：按默认值处理。<br>**说明：**<br>space取值是大于等于0的数字，或者可以转换为非负数字的字符串，或者可以转换为数字的Resource类型数据，不支持设置百分比。 |
...
|ArkTS-Dyn: number<br/>ArkTS-Sta: double|表示类型为数字，取值为大于等于0的数字。取负数或非法值时，按默认值0处理。|
...
| 参数名 | 类型                                        | 必填 | 说明                                                       |
| ------ | ------------------------------------------- | ---- | ---------------------------------------------------------- |
| value  | ArkTS-Dyn: [FlexAlign](ts-appendix-enums.md#flexalign)<br/>ArkTS-Sta: [FlexAlign](ts-appendix-enums.md#flexalign) \| undefined | 是   | 子组件在垂直方向上的对齐格式。<br/>默认值：FlexAlign.Start<br/>取值为undefined时，按默认值处理。> <br>**说明：** 若子组件不设置[flexShrink](ts-universal-attributes-flex-layout.md#flexshrink)，FlexAlign.Center和FlexAlign.End可能失效，详见下方说明。设置为FlexAlign.SpaceBetween、FlexAlign.SpaceAround、FlexAlign.SpaceEvenly时，[space](#columnoptions18对象说明)属性不生效。 |
