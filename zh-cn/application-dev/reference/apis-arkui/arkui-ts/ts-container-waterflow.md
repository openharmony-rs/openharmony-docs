...
普通模板串（非repeat形式）中每项仅支持'数字+fr'、'数字+px'、'数字+%'三种格式，不支持vp（如'100vp 100vp'）。需要按固定vp尺寸自动计算列数时，应使用repeat(auto-fill, track-size)形式。
...
普通模板串（非repeat形式）中每项仅支持'数字+fr'、'数字+px'、'数字+%'三种格式，不支持vp（如'100vp 100vp'）。需要按固定vp尺寸自动计算行数时，应使用repeat(auto-fill, track-size)形式。
