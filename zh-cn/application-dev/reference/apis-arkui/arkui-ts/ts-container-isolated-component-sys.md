...
IsolatedComponent用于支持在本页面内嵌入显示独立Abc（方舟字节码，.abc文件）提供的UI，展示的内容在受限[worker](../../apis-arkts/js-apis-worker.md)线程中运行。
...
IsolatedComponent加载的Abc（以[Ability](../../apis-ability-kit/js-apis-app-ability-ability.md)扩展形式运行）在运行过程中发生异常时触发本回调。可通过回调参数中的code、name和message获取错误信息并做处理。
