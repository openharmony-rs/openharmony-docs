...
build函数用于构造ComposeTitleBarV2高级组件。
...
ArkTS-Sta示例：
```ts
import { Entry, ComponentV2, $r, Row, Column, Divider } from '@kit.ArkUI';
...
struct Index01 {
  // 定义右侧菜单项目列表
  private menuItems: Array<ComposeTitleBarV2MenuItem> = [
    new ComposeTitleBarV2MenuItem({
      // 系统菜单图片资源
      value: $r('sys.media.ohos_save_button_filled'),
      // 启用图标
  ...
  build() {
    Row() {
      Column() {
        ...
        // 定义带头像的标题栏
        ComposeTitleBarV2({
          menuItems: [new ComposeTitleBarV2MenuItem({
            // 系统保存图片资源
          ...
          subtitle: '副标题',
          // 系统app图标资源
          item: new ComposeTitleBarV2MenuItem({ isEnabled: true, value: $r('sys.media.ohos_app_icon') })
        })
        Divider().height(2).color(0xCCCCCC)
      }
    }.height('100%')
...
```ts
import { Entry, ComponentV2, $r, Row, Column, Divider } from '@kit.ArkUI';
import { ComposeTitleBarV2, ComposeTitleBarV2MenuItem } from '@ohos.arkui.advanced.ComposeTitleBarV2';
...
struct Index {
  ...
  @Local menuItems: Array<ComposeTitleBarV2MenuItem> = [
    ...
    new ComposeTitleBarV2MenuItem({
      value: $r('sys.symbol.house_circle'),
      isEnabled: true,
...
ArkTS-Sta示例：
```ts
import { Entry, ComponentV2, $r, Row, Column, Divider, SymbolGlyphModifier, Color } from '@kit.ArkUI';
import { ComposeTitleBarV2, ComposeTitleBarV2MenuItem } from '@ohos.arkui.advanced.ComposeTitleBarV2';
...
![示例3](figures/image-composetitlebarv2-example-030.png)