...
> - 支持通过[env](#env)设置的系统环境变量键，例如[WritableEnvKey.FONT_SCALE](ts-env-system-property.md#writableenvkey)、[WritableEnvKey.DIRECTION](ts-env-system-property.md#writableenvkey)。
...
> **说明：**
>
> - `WithEnv.env(WritableEnvKey.FONT_SCALE, value)`用于为后代组件提供局部字体缩放比例，`value`为number类型，表示字体缩放倍数。
> - 后代组件实际生效的字体缩放值同时受[WritableEnvKey.FONT_SCALE](ts-env-system-property.md#writablesystemenvkeyt)设置值与组件自身的字体缩放限制共同作用。该限制可通过组件的`minFontScale`和`maxFontScale`属性设置，也可通过应用配置中的[fontSizeMaxScale](../../../quick-start/app-configuration-file.md)等全局配置生效。最终生效值为[WritableEnvKey.FONT_SCALE](ts-env-system-property.md#writableenvkey)设置值在各限制范围内的取值。
...
| 参数名 | 类型 | 必填 | 说明 |
| ----- | ----- | ---- | ---- |
| key | [WritableSystemEnvKey&lt;T&gt;](ts-env-system-property.md#writablesystemenvkeyt) | 是 | 系统环境变量键。当前正式支持[WritableEnvKey.FONT_SCALE](ts-env-system-property.md#writableenvkey)和[WritableEnvKey.DIRECTION](ts-env-system-property.md#writableenvkey)。 |
| value | T | 是 | 系统环境变量值。当`key`为`WritableEnvKey.FONT_SCALE`时，`value`类型为number。当`key`为`WritableEnvKey.DIRECTION`时，`value`类型为Direction。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| ----- | ----- | ---- | ---- |
| key | [CustomEnvKey&lt;T&gt;](ts-custom-env-property.md#customenvkeys) | 是 | 自定义环境变量的键。 |
| value | T | 是 | 自定义环境变量的值。 |
...
从API版本26.0.0开始，新增env属性和WritableEnvKey.FONT_SCALE枚举值。
...
从API版本26.0.0开始，新增env属性和WritableEnvKey.DIRECTION枚举值。
