...
>
> - 如果FoldSplitContainer设置[通用属性](ts-component-general-attributes.md)或[通用事件](ts-component-general-events.md)，编译工具链会额外生成节点__Common__，并将通用属性或通用事件挂载在__Common__上，而不是直接应用到FoldSplitContainer本身。这可能导致开发者设置的通用属性或通用事件不生效或不符合预期，因此，不建议FoldSplitContainer设置通用属性和通用事件。
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| -------- | -------- | -------- | -------- | -------- |
| isExtraRegionPerpendicular | boolean | 否 | 是 | 设置为true时，扩展区域从上到下贯穿整个组件；设置为false时，扩展区域不贯穿整个组件。此字段仅在FoldSplitContainer组件传入了extra参数时生效。<br>默认值：true |
| verticalSplitRatio | ArkTS-Dyn: number<br/>ArkTS-Sta: [PresetSplitRatio](#presetsplitratio) | 否 | 是 | 主要区域高度与次要区域高度的比值。取值可使用PresetSplitRatio预设值或自定义数值，取值范围为(0, +∞)，传入小于等于0的值时使用默认值。例如：取值为1.5时，表示主要区域高度是次要区域高度的1.5倍（即3:2比例）。<br>默认值：[PresetSplitRatio](#presetsplitratio).LAYOUT_1V1 |
| horizontalSplitRatio | ArkTS-Dyn: number<br/>ArkTS-Sta: [PresetSplitRatio](#presetsplitratio) | 否 | 是 | 主要区域宽度与扩展区域宽度的比值。取值可使用PresetSplitRatio预设值或自定义数值，取值范围为(0, +∞)，传入小于等于0的值时使用默认值。此字段仅在FoldSplitContainer组件传入了extra参数时生效。<br>默认值：[PresetSplitRatio](#presetsplitratio).LAYOUT_3V2 |
| extraRegionPosition | [ExtraRegionPosition](#extraregionposition) | 否 | 是 | 扩展区域的位置信息，可选值为TOP（上半区域）或BOTTOM（下半区域）。当isExtraRegionPerpendicular设置为false且FoldSplitContainer组件传入了extra参数时此字段生效。<br>默认值：`ExtraRegionPosition.TOP` |
...
| horizontalSplitRatio | ArkTS-Dyn: number<br/>ArkTS-Sta: [PresetSplitRatio](#presetsplitratio) | 否 | 是 | 主要区域宽度与扩展区域宽度的比值。取值可使用PresetSplitRatio预设值或自定义数值，取值范围为(0, +∞)，传入小于等于0的值时使用默认值。此字段仅在FoldSplitContainer组件传入了extra参数且showExtraRegion设置为true时生效。<br>默认值：[PresetSplitRatio](#presetsplitratio).LAYOUT_3V2 |
| extraRegionPosition | [ExtraRegionPosition](#extraregionposition) | 否 | 是 | 扩展区域的位置信息，可选值为TOP（上半区域）或BOTTOM（下半区域）。此字段仅在FoldSplitContainer组件传入了extra参数且showExtraRegion设置为true时生效。<br>默认值：`ExtraRegionPosition.TOP` |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| -------- | -------- | -------- | -------- | -------- |
| verticalSplitRatio | ArkTS-Dyn: number<br/>ArkTS-Sta: [PresetSplitRatio](#presetsplitratio) | 否 | 是 | 主要区域高度与次要区域高度的比值。取值可使用PresetSplitRatio预设值或自定义数值，取值范围为(0, +∞)，传入小于等于0的值时使用默认值。此字段仅在折叠态布局下生效。例如：取值为1.5时，表示主要区域高度是次要区域高度的1.5倍（即3:2比例）。<br>默认值：[PresetSplitRatio](#presetsplitratio).LAYOUT_1V1 |
...
| appRotation | ArkTS-Dyn: number<br/>ArkTS-Sta: double | 否 | 否 | 应用旋转角度，单位为度（degree）。 |
...
| 名称 | 值 | 说明 |
| -------- | -------- | -------- |
| LAYOUT_1V1 | ArkTS-Dyn: 1<br/>ArkTS-Sta: 1.0 | 1:1比例，表示主要区域与次要区域尺寸相等。用于verticalSplitRatio时表示上下区域高度比为1:1，用于horizontalSplitRatio时表示左右区域宽度比为1:1。 |
