...
| 名称           | 值   | 说明                                                             |
| ---------------| ------ | ---------------------------------------------------------------- |
| OFFSCREEN_WITH_BACKGROUND<sup>23+</sup> | 2 |创建离屏画布时，先拷贝一份背景初始化画布，再将此组件和子组件内容绘制到离屏画布上，然后整体进行混合。<br> **卡片能力：** 从API version 23开始，该接口支持在ArkTS卡片中使用。 <br> **系统接口：** 此接口为系统接口。<br>**ArkTS-Dyn起始版本：** 23 <br>**ArkTS-Sta起始版本：** 23 |
...
从API version 23开始，advancedBlendMode中的type参数，新增取值为BlendApplyType.OFFSCREEN_WITH_BACKGROUND枚举。
该示例主要演示如何通过设置BlendApplyType.OFFSCREEN_WITH_BACKGROUND，给组件同时添加提亮和渐隐效果。

ArkTS-Dyn示例：

```ts
// xxx.ets
import {Entry,Text,Column,Component,Button,FontStyle,Stack,BlendApplyType,BlendMode,GradientDirection,Color} from '@ohos.arkui.component';
