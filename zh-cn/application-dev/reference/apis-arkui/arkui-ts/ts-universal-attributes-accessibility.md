...
| 参数名 | 类型    | 必填 | 说明                                                         |
| ------ | ------- | ---- | ------------------------------------------------------------ |
| value  | ArkTS-Dyn: boolean <br/>ArkTS-Sta: boolean \| undefined | 是   | 无障碍分组，设置为true时表示该组件及其所有子组件为一整个可以选中的组件，无障碍服务将不再关注其子组件内容；设置为false表示不启用无障碍分组。<br/>默认值：false |
...
ArkTS-Sta: accessibilityText(text: Resource | undefined): this
...
ArkTS-Sta: accessibilityDescription(value: string | undefined): this
...
ArkTS-Sta: accessibilityDescription(description: Resource | undefined): this
...
| 参数名 | 类型   | 必填 | 说明                                                         |
| ------ | ------ | ---- | ------------------------------------------------------------ |
| nextId | ArkTS-Dyn: string <br/>ArkTS-Sta: string \| undefined | 是   | 下一个被指定聚焦组件的[唯一标识id](ts-universal-attributes-component-id.md#id)。若唯一标识id无对应组件，则设置的accessibilityNextFocusId不存在，设置无效。 |
...
| 参数名 | 类型    | 必填 | 说明                                                         |
| ------ | ------- | ---- | ------------------------------------------------------------ |
| focus  | ArkTS-Dyn: boolean <br/>ArkTS-Sta: boolean \| undefined | 是   | 设置屏幕朗读当前页默认首焦点。值为true则表示该组件为当前页默认首焦点，值为false或其他值无效。 |
...
| 参数名   | 类型                                                         | 必填 | 说明                                             |
| -------- | ------------------------------------------------------------ | ---- | ------------------------------------------------ |
| pageMode | ArkTS-Dyn: [AccessibilitySamePageMode](#accessibilitysamepagemode18枚举说明) <br/>ArkTS-Sta: [AccessibilitySamePageMode](#accessibilitysamepagemode18枚举说明) \| undefined | 是   | 当前跨进程嵌入式显示的组件和宿主应用的同page模式。 |
...
从API version 23开始，新增[accessibilityStateDescription](ts-universal-attributes-accessibility.md#accessibilitystatedescription23)接口。
...
struct Demo {
  ...
  build() {
    Column(undefined) {
      Button('accessibilityStep').backgroundColor('#FFFF00FF')
        .accessibilityStateDescription(this.isSelected ? "已点赞" : "未点赞")
        .onClick((e: ClickEvent) => {
          this.isSelected = !this.isSelected;
        })
...
从API version 23开始，新增[accessibilityActionOptions](ts-types.md#accessibilityactionoptions23对象说明)接口。
...
``` ts
// xxx.ets
import { Entry, Component, Column, Row, SliderStyle, Slider} from '@ohos.arkui.component';
