...
| exposeInnerGesture   | ArkTS-Dyn: boolean<br/>ArkTS-Sta: boolean&nbsp;\|&nbsp;undefined         | 是    | 暴露内部手势标识。<br/>**说明：** <br/>如果是组合组件，此参数设置true，回调中的current参数则会包含组合组件内部的手势识别器。<br>当前仅支持[Tabs](ts-container-tabs.md)，其他组件请不要设置此参数为true。<br/>设置为false时，功能与原接口[onGestureRecognizerJudgeBegin](#ongesturerecognizerjudgebegin)相同。 |
...
ArkTS-Dyn: onGestureCollectIntercept(callback: GestureCollectInterceptCallback): T

ArkTS-Sta: onGestureCollectIntercept(callback: GestureCollectInterceptCallback): this
...
ArkTS-Sta示例：
```ts
import { Entry, Row, Stack, Text, Column, Component, Button, ClickEvent, GestureRecognizer, TouchRecognizer,
  AttributeModifier, CommonAttribute, GestureCollectIntervention, TapGesture, HitTestMode, TouchEvent, RowOptions,
  Color, GestureCollectInterceptCallback, BaseGestureEvent, TouchTestDoneCallback, GestureJudgeResult,
  GestureRecognizerJudgeBeginCallback, FlexAlign, State} from '@kit.ArkUI';
