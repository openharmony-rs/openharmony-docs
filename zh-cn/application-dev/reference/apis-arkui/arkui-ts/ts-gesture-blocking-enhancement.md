...
| exposeInnerGesture   | boolean         | 是    | 暴露内部手势标识。<br>**说明：** <br>如果目标组件是组合组件，将此参数设置为true时，回调中的current参数会包含组合组件内部的手势识别器。<br>当前仅支持[Tabs](ts-container-tabs.md)，其他组件请不要设置此参数为true。<br>设置为false时，功能与原接口[onGestureRecognizerJudgeBegin](#ongesturerecognizerjudgebegin)相同。 |
