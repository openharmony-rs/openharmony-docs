...
该接口在CAPI场景使用时默认关闭。可以在工程的module.json5中配置metadata字段控制是否启用预上屏，配置如下：
```json
"metadata": [
  {
...
| style  | [PlaceholderStyle](#placeholderstyle12) | 否   | 提示文本的字体样式。<br/>当需要自定义placeholder的颜色、字体大小等样式时传入此参数；不传入或缺省时，默认跟随主题样式。 |
...
| 参数名 | 类型   | 必填 | 说明                                                         |
| ------ | ------ | ---- | ------------------------------------------------------------ |
| maxLength  | [Optional](ts-universal-attributes-custom-property.md#optionalt)\<number> | 是   | 内容的最大输入长度。当内容（包含文本、图片、Symbol和Builder）的总长度达到此值时，将无法继续添加内容。<br/>默认值：Infinity，可以无限输入，支持undefined类型。<br/>**说明：** <br/>取值范围：[0, +∞)。当不设置该属性或设置为undefined、0或负数时，取默认值Infinity；设置小数时，取整数部分。 |
...
当controller未与组件绑定时，该接口返回false，设置不成功。
...
关闭自定义选择菜单或系统默认选择菜单。<br>当controller未与组件绑定时，该接口调用无效。
...
| 类型                                       | 说明      |
| ---------------------------------------- | ------- |
| [RichEditorTextStyle](#richeditortextstyle) | 用户预设的文本输入样式对象，包含字体颜色、大小、粗细等样式属性，可用于查询当前组件的输入文本样式配置。<br/>当controller未绑定组件或绑定controller的组件被销毁时，返回undefined。 |
...
当controller未绑定组件或绑定controller的组件被销毁时，该接口调用无效。
...
设置用户预设的段落样式。仅在组件内容为空或组件末尾换行后，输入文本生效。当controller未绑定组件或绑定controller的组件被销毁时，该接口调用无效。
...
获取当前富文本的编辑状态。当controller未绑定组件或绑定controller的组件被销毁时，返回false。
...
添加文本内容。如果组件光标闪烁，插入后光标位置更新为新插入文本的后面。当controller未与组件绑定时，该接口调用无效。
...
添加图片内容。如果组件光标闪烁，插入后光标位置更新为新插入图片的后面。当controller未与组件绑定时，该接口调用无效。
...
更新文本、图片或SymbolSpan样式。<br/>若只更新了一个Span的部分内容，则会根据更新部分、未更新部分将该Span拆分为多个Span。当controller未与组件绑定时，该接口调用无效。
...
删除指定范围内的文本和图片。当controller未与组件绑定时，该接口调用无效。
...
| paragraphSpacing<sup>19+</sup> | number | 否    | 是 | 设置段落间距大小。<br/>单位：fp<br/>取值范围：[0, +∞)。传入负值时，按默认值0处理。<br/>段落间距默认大小为0。<br/>**原子化服务API：** 从API version 19开始，该接口支持在原子化服务中使用。|
...
| fontSize                |  [Length](ts-types.md#length) \| number  | 否 | 是    | 设置字体大小，Length为number类型时，使用fp单位。number类型取值范围：(0, +∞)。设置为0时显示默认字体大小；设置为负值时，按默认值处理。字体默认大小为16fp。不支持设置百分比字符串。|
...
| 名称     | 类型      | 只读 | 可选   | 说明                                    |
| ------ | ------ | ---- | ----------|--------------------------- |
| offset | number | 否 | 是    | 添加builder的位置。取值范围：[0, contentLength]。省略时，添加到所有内容的末尾。当值小于0时，添加到所有内容最前面；当值大于所有内容长度时，添加到所有内容最后面。<br/>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。|
