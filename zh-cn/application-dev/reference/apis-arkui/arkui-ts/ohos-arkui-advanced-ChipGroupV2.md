...
| selectedIndexes | ArkTS-Dyn: Array&lt;number&gt; <br/> ArkTS-Sta: Array&lt;int&gt; | 否 | @Param | 被选中ChipV2的索引。<br>取值范围：索引值必须为大于等于0且小于items数组长度的整数。传入不在取值范围内的索引时，该索引不生效。<br>默认值：[0]<br>值为undefined时，按默认值处理。<br>当multiple等于false时，如果没有传入selectedIndexes，默认是第一个ChipV2被选中，如果传入的selectedIndexes有多个元素时，默认第一个索引的ChipV2被选中。<br>当multiple等于true时，如果没有传入selectedIndexes，默认第一个ChipV2被选中；如果传入的selectedIndexes有多个元素时，所有对应索引的ChipV2均被选中。 |
| $selectedIndexes | ArkTS-Dyn: [Callback](ts-types.md#callback12)&lt;Array&lt;number&gt;&gt; <br/> ArkTS-Sta: [Callback](ts-types.md#callback12)&lt;Array&lt;int&gt;&gt; |否   | @Event | 被选中ChipV2索引的双向绑定回调方法，当需要监听或修改选中的ChipV2索引时传入此回调。<br>默认值：undefined，不触发回调。 |
...
| onChange        | ArkTS-Dyn: [Callback](ts-types.md#callback12)\<Array\<number>> <br/> ArkTS-Sta: [Callback](ts-types.md#callback12)\<Array\<int>>  | 否   | @Event  | ChipV2状态改变时的回调方法，用于响应ChipV2选中状态变化。当需要监听ChipV2选中状态改变时传入此回调。<br>触发场景：用户点击ChipV2改变选中状态时触发，返回当前选中ChipV2的索引数组。<br>默认值：undefined，不触发事件。 |
| suffix          | ArkTS-Dyn: [Callback](ts-types.md#callback12)\<void\> <br/> ArkTS-Sta: [ChipGroupV2SuffixBuilder](#chipgroupv2suffixbuilder)    | 否   | @BuilderParam | 支持开发者自定义builder，如需在组件最右侧显示自定义内容可配置suffix属性，使用属性suffix需引用[ChipGroupV2IconGroupSuffix](#chipgroupv2icongroupsuffix)接口。<br/>默认值：undefined，不在最右侧显示自定义内容。 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| ---- | ---- | ---- | ---- | ---- |
| itemSpace | string \| number | 否 | 是 | ChipV2与ChipV2之间的间距（不支持百分比）。<br/>取值范围：<br/>- number类型：[0, +∞)，如0、8、16、24.5。<br/>- string类型：单位为fp\|vp\|px\| lpx且数值部分大于等于0的字符串，如"8vp"、"16fp"、"12px"、"10lpx"。<br>- 不支持：负数、百分比单位、无效字符串格式。<br>传入不在取值范围内或不支持格式的值时，按默认值处理。<br>默认值：8<br>单位：vp<br>值为undefined时，按默认值处理。<br>**装饰器类型：** @Trace |
...
```ts
import { Entry, Column, ComponentV2, Color, $r, ColumnOptions, Local, ChipV2Size, ChipGroupV2, ChipGroupV2Items,
  ChipGroupV2ItemStyle, ChipGroupV2Space, ChipGroupV2Padding, ColorMetrics, ChipV2PrefixImageIconConfig, ChipV2SuffixImageIconConfig } from '@kit.ArkUI';

@Entry
@ComponentV2
struct Index {
  @Local selected_index: Array<int> = [0, 1, 2, 3, 4, 5, 6];
    ...
    Column() {
      ChipGroupV2({
        ...
        items: new ChipGroupV2Items([
          {
            // $r('app.media.icon')需要替换为开发者所需的图像资源文件。
            prefixIcon: { src: $r('app.media.icon') } as ChipV2PrefixImageIconConfig,
            label: { text: '操作块1' },
            suffixIcon: { src: $r('sys.media.ohos_ic_public_cut') } as ChipV2SuffixImageIconConfig,
            allowClose: false
          },
          {
            prefixIcon: { src: $r('sys.media.ohos_ic_public_copy') } as ChipV2PrefixImageIconConfig,
            label: { text: '操作块2' },
            allowClose: true
          },
          {
            prefixIcon: { src: $r('sys.media.ohos_ic_public_clock') } as ChipV2PrefixImageIconConfig,
            label: { text: '操作块3' },
            allowClose: true
          },
          {
            prefixIcon: { src: $r('sys.media.ohos_ic_public_cast_stream') } as ChipV2PrefixImageIconConfig,
            label: { text: '操作块4' },
            allowClose: true
          },
          {
            prefixIcon: { src: $r('sys.media.ohos_ic_public_cast_mirror') } as ChipV2PrefixImageIconConfig,
            label: { text: '操作块5' },
            allowClose: true
          },
          {
...
```ts
import { Entry, Column, ComponentV2, Color, $r, ColumnOptions, Local, ChipV2Size, ChipGroupV2, ChipGroupV2Items,
  ChipGroupV2IconGroupSuffix, ChipGroupV2ItemStyle, ChipGroupV2Space, ChipGroupV2Padding, ColorMetrics, LengthMetrics,
  ChipV2PrefixImageIconConfig, ChipV2SuffixImageIconConfig } from '@kit.ArkUI';
...
```ts
import { Entry, Column, ComponentV2, Color, $r, ColumnOptions, Local, ChipV2Size, ChipGroupV2, ChipGroupV2Items,
  ChipGroupV2IconGroupSuffix, SymbolGlyphModifier, ChipGroupV2ItemStyle, ChipGroupV2Space, ChipGroupV2Padding,
  ColorMetrics, ChipV2PrefixImageIconConfig, ChipV2PrefixSymbolIconConfig, ChipV2SuffixSymbolIconConfig } from '@kit.ArkUI';
  ...
  build() {
    Column() {
      ChipGroupV2({
        ...
        items: new ChipGroupV2Items([
          {
            prefixSymbolIcon: { normal: this.prefixModifierNormal, activated: this.prefixModifierActivated } as ChipV2PrefixSymbolIconConfig,
            label: { text: '操作块1' },
            suffixSymbolIcon: { normal: this.suffixModifierNormal, activated: this.suffixModifierActivated } as ChipV2SuffixSymbolIconConfig,
            allowClose: false,
          },
          {
            prefixSymbolIcon: { normal: this.prefixModifierNormal, activated: this.prefixModifierActivated } as ChipV2PrefixSymbolIconConfig,
