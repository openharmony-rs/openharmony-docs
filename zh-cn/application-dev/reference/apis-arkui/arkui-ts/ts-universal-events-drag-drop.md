...
其他支持拖出能力的组件需要开发者将[draggable](ts-universal-attributes-drag-drop.md#draggable)属性设置为true，并在[onDragStart](#ondragstart)等接口中实现数据传输相关内容，才能正确处理拖拽能力。
<!--RP1--><!--RP1End-->
...
| 参数名      | 类型                            | 必填 | 说明                           |
| ----------- | ------------------------------- | ---- | ------------------------------ |
| callback    | Callback<[PreDragStatus](#predragstatus12枚举说明)>     | 是   | 拖拽发起前状态变化的回调函数，用于接收当前拖拽手势触发前所处阶段。回调参数为PreDragStatus，表示拖拽发起前的各阶段状态。|
