...
- 不适用场景：
  ...
  RichText组件不适用于对HTML字符串的显示效果进行较多自定义的应用场景。例如RichText组件不支持通过设置属性与事件，来修改背景颜色、字体颜色、字体大小、动态改变内容等。在这种情况下，推荐使用[Web组件](../../apis-arkweb/arkts-basic-components-web.md)。
...
> - 该组件不再更新和维护，推荐使用[Web组件](../../apis-arkweb/arkts-basic-components-web.md)。
...
```ts
// xxx.ets
import { RichText, FlexAlign, ItemAlign, FlexDirection, Flex, State, Component, Entry } from '@kit.ArkUI';
...
ArkTS-Sta示例：
  ```ts
  // xxx.ets
  import { $rawfile, RichText, Column, Component, Entry } from '@kit.ArkUI';
...
ArkTS-Sta示例：
  ```ts
  // xxx.ets
  import { RichText, Column, Component, Entry } from '@kit.ArkUI';
