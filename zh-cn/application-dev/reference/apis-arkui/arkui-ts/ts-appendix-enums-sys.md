...
| 名称     | 值 | 说明              |
| ------ | --- | --------------- |
 | EDGELIGHT_AUTO | 0 | 流光效果的生效与控件类型相关：需设置[IMMERSIVE](../arkts-apis-uimaterial.md#immersivematerial)类型的材质方可生效；EDGELIGHT_AUTO的自动生效规则因控件而异，以各控件接口处的说明为准。对于在控件接口处未做特殊说明的控件，依据设备算力档位和系统设置中的沉浸光感配置自动生效流光效果：高算力设备在沉浸光感配置为强或均衡时生效流光动画，为弱时不生效；中算力设备在沉浸光感配置为强时生效流光动画，为均衡或弱时不生效；低算力设备不生效。 |
