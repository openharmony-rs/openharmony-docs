...
build函数用于构造ExceptionPromptV2高级组件。
...
ArkTS-Sta示例：
```ts
import { Entry, ComponentV2, Column, $r, State, Local, Text } from '@kit.ArkUI';
import {
  ExceptionPromptV2,
  PromptOptionsV2,
  MarginTypeV2,
...
@ComponentV2
struct Index01 {
  @Local config: PromptOptionsV2Config = {
    // 系统异常图片资源
    icon: $r('sys.media.ohos_ic_public_fail'),
    tip: '异常提示',
    marginType: MarginTypeV2.DEFAULT_MARGIN,
  ...
  @Local options: PromptOptionsV2 = new PromptOptionsV2(this.config);
...
struct Index02 {
  ...
  composeTitleBarDialogBuilder() {
    Column() {
      ...
      TextInput({ placeholder: '', text: this.textValue }).margin({ top: 70 } as Margin).height(60).width('90%')
        .onChange((value: string) => {
          this.textValue = value;
        })
      Text('Whether to change a text?').fontSize(16).margin({ bottom: 10 } as Margin)
      Flex({ justifyContent: FlexAlign.SpaceAround }) {
        Button('cancel')
          .onClick(() => {
...
ArkTS-Sta示例：
```ts
import { Entry, ComponentV2, Column, $r, List, ListItem, Color } from '@ohos.arkui.component';
