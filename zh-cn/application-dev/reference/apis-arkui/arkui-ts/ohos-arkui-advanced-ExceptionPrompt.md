...
| isShown | boolean                                                                          | 否 | 是 | 指定当前异常提示的显隐状态。<br />true：显示状态。<br />false：隐藏状态。<br/>默认值：false<br/>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br/> **ArkTS-Dyn起始版本：** 11 <br/>**ArkTS-Sta起始版本：** 23 |
...
ArkTS-Sta示例：
```ts
import { Entry, Component, Column, $r, State } from '@kit.ArkUI';
...
```ts
import {
  Entry, Component, Column, $r, Button, CustomDialogController, CustomDialog, TextInput, Margin, Text, Flex, FlexAlign,
  Color, TextInputController, DialogAlignment, State, Link
} from '@kit.ArkUI';
import { ExceptionPrompt, PromptOptions, MarginType } from '@ohos.arkui.advanced.ExceptionPrompt';
...
ArkTS-Sta示例：
```ts
import { Entry, Component, Column, $r, SymbolGlyphModifier, Color } from '@kit.ArkUI';
