...
| 参数名    | 类型                                             | 必填  | 说明                       |
|--------|---------------------------------------------------|-----|--------------------------|
| callback  | ArkTS-Dyn: Callback\<[MarqueeState](#marqueestate18枚举说明)\> <br> ArkTS-Sta: Callback\<[MarqueeState](#marqueestate18枚举说明)\> \| undefined | 是   | 通过callback参数指定触发回调的状态，状态由MarqueeState枚举定义，例如开始滚动、完成一次滚动、滚动完成。取值为undefined时，不使用回调函数。 |
...
| 类型                                       | 说明      |
| ---------------------------------------- | ------- |
| ArkTS-Dyn: [LayoutManager](ts-text-common.md#layoutmanager12) <br> ArkTS-Sta: [LayoutManager](ts-text-common.md#layoutmanager12) \| undefined | 布局管理器对象，用于获取文本布局信息，包括行数、字形位置、行信息、字符绘制区域等。 |
