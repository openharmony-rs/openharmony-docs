...
| params      | Object   | 否 | 是   | 当前action携带的额外参数，内容使用JSON格式的键值对形式。当action类型为call时，必须包含参数'method'，其值类型应为string用于指定UIAbility中需要触发的方法名称。 <br/>**说明：** <br/>不建议通过params传递卡片内部的状态变量。<br/>**卡片能力：** 从API version 10开始，该接口支持在ArkTS卡片中使用。 |  
