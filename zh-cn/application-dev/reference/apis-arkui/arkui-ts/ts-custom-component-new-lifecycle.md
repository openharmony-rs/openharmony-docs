...
> **说明：**
>
> -  在状态管理V1的组件里，\@ComponentReuse装饰的函数允许有一个入参或者无参。入参params建议为[ReuseObject](./ts-custom-component-lifecycle.md#reuseobject23)类型。
>
...
| 参数名  | 类型     | 必填   | 说明                                       |
| ---- | ------ | ---- | ------- |
| params   | [ReuseObject](./ts-custom-component-lifecycle.md#reuseobject23) | 否    | 当params存在时，表示V1组件的复用回调。不填默认值为undefined |
...
CustomComponentLifecycle用于监控自定义组件生命周期的变化，开发者可以通过[UIUtils.getLifecycle](../js-apis-stateManagement-static.md#getlifecycle24)获取CustomComponentLifecycle实例。
...
import { UIUtils, ComponentBuilt, Entry, Component, Column, Text } from '@kit.ArkUI';

@Entry
@Component
struct Index {
  // 组件生命周期ComponentBuilt，在Index首次渲染触发的build函数执行完成之后回调myBuilt
  @ComponentBuilt
  myBuilt() {
    console.info(`Index Lifecycle is ${UIUtils.getLifecycle(this).getCurrentState()}.`);
  }
  build() {
    Column() {
      Text(`HelloWorld`)
...
import { ComponentInit, ComponentDisappear, UIUtils, CustomComponentLifecycleObserver, CustomComponentLifecycle, Entry, Component, Column, Button, State, FontWeight, Reusable, Text, ReuseObject } from '@kit.ArkUI';
...
import { CustomComponentLifecycleState, ComponentBuilt, Entry, Component, Column, Text } from '@kit.ArkUI';
...
struct Index {
  ...
  @ComponentBuilt
  myBuilt() {
    // CustomComponentLifecycleState.BUILT的枚举值为2
    console.info(`Index Lifecycle is ${CustomComponentLifecycleState.BUILT}.`)
  }
  build() {
    Column() {
      Text(`HelloWorld`)
...
import { ComponentInit, ComponentAppear, ComponentBuilt, ComponentDisappear, ComponentReuse, ComponentRecycle, Entry, Component, State, Column, Button, FontWeight, Reusable, Text } from '@kit.ArkUI';
import { hilog } from '@kit.PerformanceAnalysisKit';
