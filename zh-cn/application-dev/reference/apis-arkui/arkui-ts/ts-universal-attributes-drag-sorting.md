...
| 参数名 | 类型      | 必填 | 说明       |
| ------ | --------- | ---- | ---------- |
| handler  | Optional\<[OnMoveHandler](#onmovehandler)\> | 是   | 拖拽排序数据移动回调。当数据位置因拖拽发生变化时触发，需在回调中根据起始索引号和目标索引号修改数据源。 |
...
| 名称 | 类型   | 只读 | 可选 | 说明                 |
| ------ | ------ | ---- | ---- | -------------------- |
| onLongPress  |  Callback\<number\> | 否  | 是 | 长按时触发的回调，不设置时不触发该回调。<br>- 参数index为长按时当前目标的索引号，取值范围是[0, 数据源长度-1]。 |
| onDragStart  | Callback\<number\> | 否   | 是 | 拖拽开始时触发的回调，不设置时不触发该回调。<br>- 参数index为拖拽开始时当前目标的索引号，取值范围是[0, 数据源长度-1]。 |
...
| onDrop  | Callback\<number\> | 否   | 是 | 拖拽结束时触发的回调，不设置时不触发该回调。<br>- 参数index为拖拽结束时当前目标的索引号，取值范围是[0, 数据源长度-1]。 |
