...
| 名称              | 类型                                       | 只读 | 可选   | 说明              |
| --------------- | ------------------------------- | --------- | ---- | --------------- |
| offset<sup>14+</sup>       | [Position](ts-types.md#position) | 否 | 是    | 设置半模态弹窗偏移量。仅当半模态为底部弹窗时，支持设置底部间距。不支持设置半模态的[SheetOptions](ts-universal-attributes-sheet-transition.md#sheetoptions)中的detents属性。y轴设置为负数时不生效，将回退至默认值0vp。<br> 默认值：x轴坐标为0vp，y轴坐标为0vp。<br>**系统接口：** 此接口为系统接口。|
