...
ArkTS-Dyn: geometryTransition(id: string, options?: GeometryTransitionOptions): T

ArkTS-Sta: geometryTransition(id: string | undefined, options?: GeometryTransitionOptions): this
...
| 参数名  | 类型                 | 必填 | 说明                                                     |
| ------- | ------------------------ | ---- | ------------------------------------------------------------ |
| id      | ArkTS-Dyn: string<br > ArkTS-Sta: string \| undefined                   | 是   | 用于设置绑定关系，id置空字符串清除绑定关系避免参与共享行为，id可更换重新建立绑定关系。同一个id只能有两个组件绑定且是in/out不同类型角色，不能多个组件绑定同一个id。当id的值为undefined时，组件内隐式共享元素转场不生效。 |
