...
| selectedIndexes | ArkTS-Dyn: [Array](../../apis-arkts/arkts-apis-arkts-collections-Array.md)&lt;number&gt; <br/> ArkTS-Sta: Array&lt;int&gt; | 否   | ArkTS-Dyn: @Prop<br/> ArkTS-Sta: @PropRef  | 被选中Chip的索引，索引从0开始计数。<br/>默认值：[0] <br>值为undefined时，按默认值处理。<br>**原子化服务API（仅ArkTS-Dyn）：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 12<br>**ArkTS-Sta起始版本：** 23 |
...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| ---- | ---- | --- | ---- | ---- |
| action | [VoidCallback](ts-types.md#voidcallback12) | 否 | 是 | 后缀图标响应事件，用户点击后缀图标时触发回调。当需要为后缀图标添加自定义交互功能时设置此参数，如执行特定操作、打开界面等。<br>值为undefined时，无后缀图标响应事件。 |
