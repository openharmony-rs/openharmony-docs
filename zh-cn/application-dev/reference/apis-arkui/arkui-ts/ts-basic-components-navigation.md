...
ArkTS-Sta: navBarWidth(value: Length | undefined)
...
| 参数名 | 类型                                                         | 必填 | 说明                                      |
| ------ | ------------------------------------------------------------ | ---- | ----------------------------------------- |
| value  | ArkTS-Dyn: [Length](ts-types.md#length)<br/>ArkTS-Sta: [Length](ts-types.md#length) \| undefined | 是   | 导航栏宽度。<br/>默认值：240<br/>单位：vp<br/>undefined：行为不做处理，导航栏宽度与默认值保持一致。 |
...
ArkTS-Sta: splitPlaceholder(placeholder: ComponentContent | undefined)
...
| 参数名 | 类型                                                         | 必填 | 说明               |
| ------ | ------------------------------------------------------------ | ---- | ------------------ |
| placeholder  | ArkTS-Dyn: [ComponentContent](../js-apis-arkui-ComponentContent.md#componentcontent-1)<br/>ArkTS-Sta: [ComponentContent](../js-apis-arkui-ComponentContent.md#componentcontent-1) \| undefined | 是   |设置Navigation双栏模式下右侧的默认占位页。|
...
| 类型                                                         | 说明                         |
| ------------------------------------------------------------ | -------------------------- |
| ArkTS-Dyn: unknown<br/>ArkTS-Sta: Object \| null \| undefined | 返回对应NavDestination页面的参数信息，unkown可以是用户自定义的类型。 |
...
| param | ArkTS-Dyn: unknown<br/>ArkTS-Sta: Object \| null \| undefined | 否    | 开发者设置的NavDestination页面详细参数，unkown可以是用户自定义的类型。<br/>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。 |
...
| isEntry<sup>12+</sup> | boolean | 否 | 标记NavDestination是否为入口页面。<br/>默认值：false。<br/>true：NavDestination是入口页面。<br/>false：NavDestination不是入口页面。 <br/>标记清理时机：1、在当前navDestination页面触发一次全局返回事件。2、应用退至后台。<br/>**说明**：<br/>入口NavDestination不响应应用内的全局back事件，直接触发应用间的全局back事件。<br/>**原子化服务API：** 从API version 12开始，该接口。 |
...
| index | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 是 | NavDestination在NavPathStack中的序号， 如果为根视图(NavBar)，则返回值为 -1。<br/>取值范围：[-1, +∞)。 |
...
| 名称 | 类型 | 必填 | 说明 |
|------|-----|-----|------|
| timeout | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否 | 动画超时结束时间。<br> 单位：ms。<br/>取值范围：[0, +∞)。<br> 默认值：可交互动画无默认值，不可交互动画默认超时时间为1000ms。|
...
ArkTS-Dyn: updateTransition?(progress: number): void;

ArkTS-Sta: updateTransition?(progress: double): void;
...
| 参数名 | 类型 | 必填 | 说明 |
|------|------|------|-----|
| progress | ArkTS-Dyn: number<br/>ArkTS-Sta: double | 是 | 设置交互转场动画进度百分比。取值范围 0-1。|
...
| 名称     | 类型            | 必填   | 说明              |
| ------ | ------------- | ---- | --------------- |
| value  | string \| [Resource<sup>14+<sup>](ts-types.md#resource)       | 是    | API version 9：显示菜单栏单个选项的文本。<br> 从API version 10开始，不显示菜单栏单个选项的文本。  <br/>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 7<br/>**ArkTS-Sta起始版本：** 23 |
| icon   | string \| [Resource<sup>14+<sup>](ts-types.md#resource)       | 否    | 菜单栏单个选项的图标资源路径。 <br/>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 7<br/>**ArkTS-Sta起始版本：** 23 |
...
| action | () =&gt; void | 否    | 当前选项被选中的事件回调。   <br/>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 7<br/>**ArkTS-Sta起始版本：** 23 |
