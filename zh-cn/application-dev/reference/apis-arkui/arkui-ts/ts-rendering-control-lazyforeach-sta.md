...
开发者指南见：[ArkTS-Dyn LazyForEach开发者指南](../../../ui/rendering-control/arkts-rendering-control-lazyforeach.md)。
...
| index  | MoveIndex     | 是   | 移动位置。取值范围是[0, 数据源长度-1]。|
...
| index  | ExchangeIndex           | 是   | 交换位置。取值范围是[0, 数据源长度-1]。|
| key    | ExchangeKey           | 否   | 分配新的键值，默认使用原键值。 |
