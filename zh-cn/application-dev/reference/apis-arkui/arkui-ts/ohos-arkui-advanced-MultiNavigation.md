...
| navDestination | ArkTS-Dyn: [NavDestinationBuildFunction](#navdestinationbuildfunction) <br/>ArkTS-Sta: [PageMapBuilder](./ts-types.md#pagemapbuilder23) \| undefined| 是 | @BuilderParam | 设置加载目标页面的路由规则。 |
...
|  参数名   | 类型   | 必填 | 说明           |
| ----- | ------ | ---- | ---------------------- |
| index | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 是    | NavDestination页面的位置索引。 |
| result | Object | 是 | 页面自定义处理结果。 |
| animated | boolean | 否    | 是否支持转场动画。<br/>默认值：true<br/>true：支持转场动画。<br/>false：不支持转场动画。 |
