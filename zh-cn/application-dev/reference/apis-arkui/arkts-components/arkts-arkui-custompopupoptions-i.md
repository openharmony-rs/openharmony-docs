...
设置箭头宽度。若所设置的箭头宽度超过所在边的长度减去两倍的气泡圆角大小，则不绘制气泡箭头。默认值：16单位：vp  
**说明：**不支持设置百分比。
...
**说明：**如果要实现点击气泡内消失需要在builder中先放一个布局组件，然后再将Popup高级组件放在布局组件里面，再在布局组件的onClick事件中修改控制显隐的状态变量show为false。
...
3. 该构造器的builder仅支持定义在UI组件中，例如可以定义在Builder函数、方法或者[build](arkts-arkui-basecustomcomponent-c.md#build)方法里。
...
设置气泡遮罩层颜色。  
**说明：**从 API version 10 开始废弃，建议使用`mask`替代。
...
设置Popup组件相对于placement设置的显示位置的偏移。默认值：{ x: 0, y: 0 }单位：vp  
**说明：**不支持设置百分比。
...
**说明：**在onWillDismiss回调中，不能再做onWillDismiss拦截。
...
设置Popup与宿主节点的间距。不支持设置百分比。默认值：8单位：vp
