...
设置文本最大显示字号，与minFontSize配合使用。当需要限制文本的最大显示尺寸以避免文本过大或需要实现字号自适应时传入此参数。 > **说明**：当设置minFontSize和maxFontSize时，font中的size将不生效。详细规则请参考Text组件的 > [maxFontSize]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_属性。
...
设置文本最小显示字号，与maxFontSize配合使用。当需要限制文本的最小显示尺寸以避免文本过小或需要实现字号自适应时传入此参数。 > **说明**：当设置minFontSize和maxFontSize时，font中的size将不生效。默认最大行数为1，自适应高度方式为MIN\_FONT\_SIZE\_FIRST。详细 > 规则请参考Text组件的[minFontSize]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_属性。
