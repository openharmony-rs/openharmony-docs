...
> [NavigationOption](arkts-arkui-navigation-comp-navigationoptions-i.md)中的launchMode属性值为LaunchMode.NEW_INSTANCE模式。
> 
...
| 类型 | 说明 |
| --- | --- |
| Array&lt;[NavPathInfo](arkts-arkui-navigation-comp-navpathinfo-c.md)&gt; | 当前路由栈中的路由页面信息数组。 |
...
将info指定的NavDestination页面信息入栈，使用Promise异步回调返回接口调用结果，具体根据options中指定不同的[LaunchMode](arkts-arkui-navigation-comp-launchmode-e.md)，来实现不同的行为。
...
将info指定的NavDestination页面信息入栈，具体根据options中指定不同的[LaunchMode](arkts-arkui-navigation-comp-launchmode-e.md)，来实现不同的行为。
...
将路由栈内指定navDestinationId的NavDestination页面删除。navDestinationId可以在NavDestination的[onReady](arkts-arkui-navdestination-comp-attribute.md#onready)回调中获取，也可以在[NavDestinationInfo](../arkts-apis/arkts-arkui-uiobserver-navdestinationinfo-i.md)中获取。
...
替换路由栈操作。使用Promise异步回调返回接口调用结果，具体根据options中指定不同的[LaunchMode](arkts-arkui-navigation-comp-launchmode-e.md)，来实现不同的行为。
...
替换路由栈操作，具体根据options中指定不同的[LaunchMode](arkts-arkui-navigation-comp-launchmode-e.md)，来实现不同的行为。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| interception | [NavigationInterception](arkts-arkui-navigation-comp-navigationinterception-i.md) | 是 | 设置Navigation跳转拦截对象。设置后，可在页面跳转前/后执行自定义回调，允许操作栈或拦截跳转。 |
...
> 
> 3. 开发者可以通过[NavPathInfo](arkts-arkui-navigation-comp-navpathinfo-c.md)中的页面唯一标识符navDestinationId来操作已有页面，该id由系统默认生成且全局唯一（可以通过[getPathStack](#getpathstack)接口获取，不可主动赋新值）。若该id在当前路由栈中不存在，则表示新增页面，若在当前路由栈中存在，同时对应的name相同，则表示复用已有页面。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| pathStack | Array&lt;[NavPathInfo](arkts-arkui-navigation-comp-navpathinfo-c.md)&gt; | 是 | 设置当前路由栈中的路由页面信息数组。设置后，将当前路由栈更新为指定内容，并实现路由转场。开发者可在原有栈的基础上批量添加或删除页面。<br>**说明：** <br>数组长度无限制。 |
