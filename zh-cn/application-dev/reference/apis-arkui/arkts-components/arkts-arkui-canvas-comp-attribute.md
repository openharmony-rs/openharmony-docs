...
需要搭配[CanvasRenderingContext2D](arkts-arkui-canvasrenderingcontext2d-c.md)中的StartImageAnalyzer和StopImageAnalyzer一起使用。
...
当该事件被触发时画布被清空，该事件之后Canvas组件宽高确定且可获取，可使用Canvas相关API进行绘制。当Canvas组件仅发生位置变化时，只触发[onAreaChange](arkts-arkui-commonmethod-c.md#onareachange)事件，不触发onReady事件。[onAreaChange](arkts-arkui-commonmethod-c.md#onareachange)事件在onReady事件后触发。
