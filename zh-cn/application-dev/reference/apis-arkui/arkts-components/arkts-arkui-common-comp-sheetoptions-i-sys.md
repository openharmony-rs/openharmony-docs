...
继承自[BindOptions](arkts-arkui-common-comp-bindoptions-i.md)。

半模态页面内容选项。

**继承/实现关系：** SheetOptions extends [BindOptions](arkts-arkui-common-comp-bindoptions-i.md)
...
设置半模态弹窗偏移量。仅当半模态为底部弹窗时，支持设置底部间距。不支持设置半模态的[SheetOptions](arkts-arkui-common-comp-sheetoptions-i.md)中的detents属性。y轴设置为正数时不生效，将回退至默认值0vp。
