...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| drawingContext | [DrawingRenderingContext](arkts-arkui-canvas-drawingrenderingcontext-c.md) | 是 | 一个DrawingRenderingContext类型的对象。<br>异常值undefined或null按无效值处理。 |
| options | [RenderingContextOptions](arkts-arkui-canvas-renderingcontextoptions-i.md) | 否 | 渲染上下文的配置选项。<br>默认值：{ antialias: false } |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| config | [ImageAnalyzerConfig](../arkts-apis/arkts-arkui-image-common-imageanalyzerconfig-i.md) | 是 | 执行AI分析所需要的入参，用于配置AI分析功能。<br>异常值undefined或null按无效值处理。 |
