...
颜色设置方式可参考：[ResourceColor](arkts-arkui-resourcecolor-t.md)，非[ResourceColor](arkts-arkui-resourcecolor-t.md)范围内的颜色值即为非法颜色。

数组内颜色设置为undefined或者null时，默认为黑色。

colors参数的约束：

[ResourceColor](arkts-arkui-resourcecolor-t.md)表示填充的颜色，number表示指定颜色所处的位置，取值范围为[0,1.0]，0表示需要设置渐变色的容器的起始位置，1.0表示容器的结束位置。为实现多个颜色渐
