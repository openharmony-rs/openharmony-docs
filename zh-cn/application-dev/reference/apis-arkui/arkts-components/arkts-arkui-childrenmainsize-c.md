...
| childrenSize | Array &lt;number&gt; | 否 | 要在start位置插入的所有子组件的主轴方向的大小。Array中各个数值单位：vp    **说明：** 1.数组中数值如 果是有限的非负值，则认为是指定的大小，后续不随默认大小的变化而变化。 2. 数组中数值如果不是有限的非负值，会被处理成默认大小，后续会随默认大小的变化而变化。 默认值为空数组。 取值范围： [0, +∞) |
