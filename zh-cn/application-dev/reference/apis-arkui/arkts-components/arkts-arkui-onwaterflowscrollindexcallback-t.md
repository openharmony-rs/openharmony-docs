...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| first | number | 是 | 当前显示的瀑布流起始位置的索引值。<br/>正常取值范围：[0, 子组件总数-1]。列表为空时存在特殊取值，详见 onScrollIndex事件说明。 |
| last | number | 是 | 当前显示的瀑布流终止位置的索引值。<br/>正常取值范围：[0, 子组件总数-1]。列表为空时存在特殊取值，详见 onScrollIndex事件说明。 |
