...
根据指定的目标索引，计算Grid滚动到该位置时页面内的起始行，用于支持[scrollToIndex](arkts-arkui-scroll-comp-scroller-c.md#scrolltoindex)等操作。不设置时不启用该回调，需与onGetStartIndexByOffset同时设置才能生效。
