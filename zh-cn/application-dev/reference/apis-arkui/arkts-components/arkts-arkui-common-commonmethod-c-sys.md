...
将当前组件的内容（包含子节点内容）与下方画布（可能为离屏画布）已有内容进行混合。不能与[blendMode](arkts-arkui-common-commonmethod-c.md#blendmode-1)接口同时使用。
...
设置当前组件和其子组件是否从祖先组件的节点组中剔除。需搭配祖先组件设置节点组[renderGroup](arkts-arkui-common-commonmethod-c.md#rendergroup-2)属性使用，单独使用无效果。
...
> [translate](arkts-arkui-common-commonmethod-c.md#translate-1)或  
> [scale](arkts-arkui-common-commonmethod-c.md#scale-1)等属性导致子组件超出当前组件范围，或当前组件上有  
> [shadow](arkts-arkui-common-commonmethod-c.md#shadow-1)、  
> [pixelStretchEffect](arkts-arkui-common-commonmethod-c.md#pixelstretcheffect-1)等属性导致当前组件的绘制内容超出组件  
...
| options | [GravityCenterOptions](arkts-arkui-common-gravitycenteroptions-i-sys.md) | 否 | 引力中心参数。 需要配合UnionMode.GRAVITY_UNION使用。 |
