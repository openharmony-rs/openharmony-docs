...
**错误描述**<br>
@ObjectLink装饰的变量类型需要是在ets文件中定义的被@Observed装饰的类，或使用makeV1Observed返回值初始化的类型。

**可能原因**<br>
用未被@Observed装饰的类、未使用makeV1Observed返回值初始化的类型或其他不兼容类型初始化@ObjectLink装饰的变量导致报错。

**处理步骤**<br>
需要用@ObjectLink装饰器装饰由@Observed装饰的类实例，或使用makeV1Observed返回值初始化的实例。
...
**错误描述**<br>
build()方法内有多个根节点，或根节点不是容器组件。

**可能原因**<br>
开发者在build()方法中定义了多个根节点，或根节点未使用容器组件。

**处理步骤**<br>
删除多余根节点，并保证build()方法中仅有一个根节点且该根节点为容器组件。
...
**可能原因**<br>
开发者在定义@Styles装饰器装饰的函数或方法时包含了参数。

**处理步骤**<br>
删除@Styles装饰器装饰的函数或方法的参数即可。
...
## 10905321 @Require装饰器装饰的变量需要在构造传参时初始化
**错误信息**<br>
\'@Require\' decorated \'xxx\' must be initialized through the component constructor.

**错误描述**<br>
@Require装饰器装饰的变量需要在构造传参时初始化。

**可能原因**<br>
开发者用在父组件初始化子组件时没有给子组件中@Require装饰的变量传参导致报错。
...
**错误描述**<br>
V2组件中，@Require只能与@Param一起使用。

**可能原因**<br>
开发者在V2场景下使用@Require装饰器搭配了@Param以外的V2装饰器一起使用导致报错。

**处理步骤**<br>
将@Require与@Param一起搭配使用。
...
**可能原因**<br>
开发者在@Styles装饰器装饰的函数或方法中包含了参数导致报错。
...
**错误描述**<br>
@Track等装饰器不能用于@ObservedV2装饰的类中，请使用@Trace装饰器替代。

**可能原因**<br>
开发者在@ObservedV2装饰的类中使用了@Track等装饰器导致报错。

**处理步骤**<br>
避免在@ObservedV2装饰的类中使用@Track等装饰器，请使用@Trace装饰器替代。
...
**错误描述**<br>
@Track等装饰器只能装饰class中的成员变量。

**可能原因**<br>
开发者使用@Track等装饰器装饰了非class成员变量导致报错。

**处理步骤**<br>
避免使用@Track等装饰器装饰非class成员变量。
...
**错误描述**<br>
@Concurrent不能装饰方法，请将其用于函数声明。

**可能原因**<br>
开发者使用@Concurrent装饰成员方法，而不是函数声明，导致报错。

**处理步骤**<br>
避免使用@Concurrent装饰器装饰方法，请将其用于函数声明。
...
**错误描述**<br>
在main_pages.json或build-profile.json5中配置的页面必须有且仅有一个@Entry装饰器。

**可能原因**<br>
开发者在main_pages.json或build-profile.json5中配置的页面中没有设置唯一生效的@Entry装饰器导致报错。

**处理步骤**<br>
确保在main_pages.json或build-profile.json5中配置的页面中有且仅有一个@Entry装饰器生效。
...
**错误描述**<br>
@Styles装饰的函数不应添加返回类型声明。

**可能原因**<br>
开发者为@Styles装饰的函数添加了返回类型声明导致报错。

**处理步骤**<br>
避免为@Styles装饰的函数添加返回类型声明。
...
**错误描述**<br>
@Extend装饰的函数不应添加返回类型声明。

**可能原因**<br>
开发者为@Extend装饰的函数添加了返回类型声明导致报错。

**处理步骤**<br>
避免为@Extend装饰的函数添加返回类型声明。
