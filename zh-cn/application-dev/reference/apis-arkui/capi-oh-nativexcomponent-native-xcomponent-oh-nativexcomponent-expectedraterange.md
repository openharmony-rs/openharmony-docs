...
| 名称 | 描述 |
| -- | -- |
| int32_t min | 期望帧率范围最小值。取值需大于等于0且小于等于max。单位为帧/秒。传入无效值时不生效。取值范围：[0, 设备支持的最大帧率]。 |
| int32_t max | 期望帧率范围最大值。取值需大于等于min且不超过设备支持的最大帧率。单位为帧/秒。传入无效值时不生效。取值范围：[0, 设备支持的最大帧率]。 |
| int32_t expected | 期望帧率。单位为帧/秒。取值需满足 min ≤ expected ≤ max，传入无效值时不生效。 |
