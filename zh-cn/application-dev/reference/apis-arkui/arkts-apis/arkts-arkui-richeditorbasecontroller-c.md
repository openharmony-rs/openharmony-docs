...
**继承/实现关系：** RichEditorBaseController implements [TextEditControllerEx](arkts-arkui-text-common-texteditcontrollerex-i.md#TextEditControllerEx)
...
| 类型 | 说明 |
| --- | --- |
| LayoutManager | 布局管理器对象。<br/><br/>当controller未绑定组件或绑定controller的组件被释放时，返回undefined。 |
...
| 类型 | 说明 |
| --- | --- |
| PreviewText | 预上屏信息。<br/><br/>当controller未绑定组件或绑定controller的组件被释放时，返回undefined。 |
...
| 类型 | 说明 |
| --- | --- |
| RichEditorTextStyle | 用户预设样式。<br/><br/>当controller未绑定组件或绑定controller的组件被释放时，返回undefined。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| range | TextRange | 否 | 可视范围。<br/>如果参数无效，该方法将不产生效果 |
...
| options | SelectionOptions | 否 | 选择项配置。 [since 12] |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| styledString | StyledString | 是 | 设置属性字符串样式的提示文本，其优先级高于[placeholder](RichEditorAttribute.placeholder)属性设<br/>置的提示文本。<br/>提示文本不支持触发属性字符串[GestureStyle](arkts-arkui-styled-string-gesturestyle-c.md#GestureStyle)样式绑定的手势事件，以及<br/>[UrlStyle](arkts-arkui-styled-string-urlstyle-c.md#UrlStyle)样式的超链接跳转能力。 |
