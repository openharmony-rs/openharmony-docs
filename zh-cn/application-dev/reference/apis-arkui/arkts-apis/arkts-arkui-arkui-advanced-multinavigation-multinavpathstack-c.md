...
当前，MultiNavigation的路由栈仅支持由使用方自行创建，不支持通过回调方式获取。请勿使用[NavDestination]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_的 \_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_等类似事件或接口来获取 NavPathStack并进行栈操作，因为这可能会导致不可预知的问题。
...
清除栈中所有页面。 > **说明：** > > 当调用[keepBottomPage]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口并设置为true时，会保留栈底页面。
...
| 类型 | 说明 |
| --- | --- |
| Object | Returns the detailed parameter of the NavDestination |
...
弹出路由栈栈顶元素，并触发onPop回调传入页面处理结果。 > **说明：** > > 当调用[keepBottomPage]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口并设置为true时，会保留栈底页面。
...
| 类型 | 说明 |
| --- | --- |
| \_\_\_MD\_LINK\_USD\_0\_\_\_ | Returns the top NavPathInfo if the stack is not empty, |
