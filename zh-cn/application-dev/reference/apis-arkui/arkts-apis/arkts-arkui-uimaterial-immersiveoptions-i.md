...
自动反色能力仅对部分属性接口设置特殊资源值时生效，生效的属性接口包括：Text组件的fontColor，Button组件的fontColor，SymbolGlyph组件的fontColor)，Image组件的fillColor)，Search组件的placeholderColor、fontColor、searchIcon中的图标颜色、cancelButton中的图标颜色、caretStyle中的光标颜色，TabContent组件的tabBar)属性使用BottomTabBarStyle样式时其中的文本和图标颜色。
