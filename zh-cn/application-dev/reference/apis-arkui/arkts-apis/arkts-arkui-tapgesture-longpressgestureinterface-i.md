...
创建长按手势对象。继承自[GestureInterface<T>](arkts-arkui-tapgesture-gestureinterface-i.md#GestureInterface)。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | { fingers?: number; repeat?: boolean; duration?: number } | 否 | 设置长按手势参数。<br/>- fingers：触发长按的最少手指数，最小值为1，&nbsp;最大值为10。<br/>默认值：1<br/><br/>- repeat：是否连续触发事件回调。true表示连续触发事件回调，false表示不连续触发事件回调。<br/>默认值：false<br/><br/>- duration：触发长按的最短时间，单位为毫秒（ms）。<br/>默认值：500 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| LongPressGestureInterface | @syscap SystemCapability.ArkUI.ArkUI.Full<br/>@crossplatform [since 10]<br/>@atomicservice [since 11] |
...
| 类型 | 说明 |
| --- | --- |
| LongPressGestureInterface | @syscap SystemCapability.ArkUI.ArkUI.Full<br/>@stagemodelonly<br/>@crossplatform<br/>@atomicservice |
