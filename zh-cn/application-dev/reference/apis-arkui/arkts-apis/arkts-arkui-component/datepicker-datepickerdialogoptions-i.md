...
弹窗背板模糊材质。 默认值：BlurStyle.COMPONENT\_ULTRA\_THICK **说明：** - 设置为BlurStyle.NONE关闭背景虚化。设置backgroundBlurStyle为非NONE值时，不要设置backgroundColor， 否则显示的颜色将不符合预期效果。
...
设置取消按钮显示样式、样式和重要程度、角色、背景色、圆角、文本颜色、字号、字体粗细、字体样式、字体列表、按钮是否默认响应Enter键。 **说明：** 1. acceptButtonStyle与cancelButtonStyle中最多只能有一个primary字段配置为true，如果同时设置为true，则primary字段不生效， 保持默认值false。 2. 按钮高度默认40vp，在关怀模式-大字体场景下高度不变，即使按钮样式设置为圆角矩形 \_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_，呈现效果 依然是胶囊型按钮\_\_\_MD\_LINK\_DESC\_USD\_1\_\_\_。
...
设置农历开关的颜色样式。 默认值：{ selectedColor: \_\_\_INLINE\_CODE\_DESC\_USD\_0\_\_\_, unselectedColor: \_\_\_INLINE\_CODE\_DESC\_USD\_1\_\_\_, strokeColor: Color.White }
