...
选中项背景的颜色。 默认值：'sys.color.comp\_background\_tertiary' **说明：** 当type为PickerIndicatorType.BACKGROUND时生效。
...
选中项背景的边框圆角半径。 默认值：{ value:12, unit:LengthUnit.vp }，即四个圆角半径均为12vp。 取值范围：取选中项的宽和高之中较小的边长为x，最大不超过x的一半。当取值小于0时，使用默认值；当取值大于最大值时，使用最大值。 **说明：** 1. 当type为PickerIndicatorType.BACKGROUND时生效。 2. [LengthMetrics]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_：统一设置四个圆角半径的大小和单位。 3. [BorderRadiuses]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_：单独设置四个圆角半径的大小（单位为vp）。 4. [LocalizedBorderRadiuses]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_：单独设置四个圆角半径的大小和单位。
...
分割线的颜色。 默认值：'sys.color.comp\_divider' **说明：** 当type为PickerIndicatorType.DIVIDER时生效。
