...
选中项的索引值。\_\_\_HTML\_TAG\_DESC\_USD\_0\_\_\_取值范围：[0, 子组件的个数-1]内的整数。不在取值范围内时，使用默认值；设置小数时，使用向下取整后的整数。 \_\_\_HTML\_TAG\_DESC\_USD\_1\_\_\_默认值：0 **说明：** 统计子组件的个数时，不包含Row容器内的子组件，Row容器及其子组件共同视为1个子组件。
