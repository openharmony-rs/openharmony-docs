...
在router路由页面（即[\@Entry](../../../../ui/state-management/arkts-create-custom-components.md#entry)装饰的自定义组件）生效，当用户点击返回按钮时
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| selfLayoutInfo | GeometryInfo | 是 | 计算自定义组件大小后的自身布局信息。<br/>**说明：**<br/>第一次布局时以自身设置的属性为准。 |
| children | Array&lt;Measurable&gt; | 是 | 计算子组件大小后的子组件布局信息。<br/>**说明：**<br/><br/>如果没有设置子组件的布局信息，子组件会维持上一次的布局信息，当子组件从来没有设置过尺寸时，尺寸默认为0。 |
...
该回调仅生效于由[\@Entry](../../../../ui/state-management/arkts-create-custom-components.md#entry)装饰的、作为[router](**起始版本：** 19
...
router路由页面（即[\@Entry](../../../../ui/state-management/arkts-create-custom-components.md#entry)装饰的自定义组件）每次隐藏时触发一次，包括路由
跳转、应用进入后台等场景。
> **说明：**
...
router路由页面（即[\@Entry](../../../../ui/state-management/arkts-create-custom-components.md#entry)装饰的自定义组件）每次显示时触发一次，包括路由
跳转、应用进入前台等场景。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| selfLayoutInfo | GeometryInfo | 是 | 计算自定义组件大小后的自身布局信息。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| isInner | Optional&lt;boolean&gt; | 是 | -<br/>true：向内查询最近的，且在栈内的NavDestinationInfo的详细信息。<br/>false：向外查询最近的，且在栈内的NavDestinationInfo的详细信息。 |
