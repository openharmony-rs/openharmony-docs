...
| surfaceRegion | [Rect](arkts-arkui-display-rect-i.md) | 否 | surface显示的虚拟屏的矩形区域。如果虚拟屏幕未通过[setVirtualScreenSurface()](arkts-arkui-display-setvirtualscreensurface-f.md) 或[addVirtualScreenSurface()](arkts-arkui-display-addvirtualscreensurface-f-sys.md)绑定过surface，surfaceRegion无效，默认全屏。在镜像模式下，surfaceRegion无效，默认全屏。在异源模式下，surfaceRegion有效。 |
...
| [801](../../errorcode-universal.md#801-api功能在部分设备不支持) | Capability not supported. Function addVirtualScreenSurface can not work correctly due to limited device capabilities. |
