...
| 名称 | 说明 |
| --- | --- |
| [convertGlobalToRelativeCoordinate](arkts-arkui-convertglobaltorelativecoordinate-f.md#convertglobaltorelativecoordinate-1) | 将主屏左上角为原点的全局坐标转换成displayId指定屏幕左上角为原点的相对坐标。若未传入displayId，默认转换为全局坐标所在屏幕的相对坐标系。若全局坐标不在任何屏幕上，默认转换成主屏的相对坐标。 |
| [convertRelativeToGlobalCoordinate](arkts-arkui-convertrelativetoglobalcoordinate-f.md#convertrelativetoglobalcoordinate-1) | 将指定屏幕左上角为原点的相对坐标转换成主屏左上角为原点的全局坐标，仅支持主屏和扩展屏的坐标转换。 |
| [createVirtualScreen](arkts-arkui-createvirtualscreen-f.md#createvirtualscreen-1) | 创建虚拟屏幕，使用Promise异步回调。 |
| [destroyVirtualScreen](arkts-arkui-destroyvirtualscreen-f.md#destroyvirtualscreen-1) | 销毁虚拟屏幕，使用Promise异步回调。 |
| [getAllDisplay](arkts-arkui-getalldisplay-f.md#getalldisplay-1) | 获取当前所有的Display对象，使用callback异步回调。 |
| [getAllDisplay](arkts-arkui-getalldisplay-f.md#getalldisplay-2) | 获取当前所有的Display对象，使用Promise异步回调。 |
...
| [getDefaultDisplaySync](arkts-arkui-getdefaultdisplaysync-f.md#getdefaultdisplaysync-1) | 返回应用所在屏幕的Display对象。若应用内多个Ability在不同屏幕，返回主屏的Display对象，若应用内多个Ability在同一屏幕，返回所在屏幕的Display对象。 |
| [getDisplayByIdSync](arkts-arkui-getdisplaybyidsync-f.md#getdisplaybyidsync-1) | 根据displayId获取对应的Display对象。 |
| [getFoldDisplayMode](arkts-arkui-getfolddisplaymode-f.md#getfolddisplaymode-1) | 获取可折叠设备当前的显示模式。 |
| [getFoldStatus](arkts-arkui-getfoldstatus-f.md#getfoldstatus-1) | 获取可折叠设备当前的折叠状态。 |
| [getPrimaryDisplaySync](arkts-arkui-getprimarydisplaysync-f.md#getprimarydisplaysync-1) | 获取主屏信息。除2in1之外的设备获取的是设备自带屏幕的Display对象；2in1设备外接屏幕时获取的是当前主屏幕的Display对象；2in1设备没有外接屏幕时获取的是自带屏幕的Display对象。 |
| [isCaptured](arkts-arkui-iscaptured-f.md#iscaptured-1) | 检查设备的屏幕显示信息是否被获取。 |
| [isCaptured](arkts-arkui-iscaptured-f.md#iscaptured-2) | 检查该设备是否被bundle名称列表中的任何应用抓拍、投影或录制。 |
| [isFoldable](arkts-arkui-isfoldable-f.md#isfoldable-1) | 判断设备是否可折叠。 |
| [makeUnique](arkts-arkui-makeunique-f.md#makeunique-1) | 将屏幕设置为异源模式，使用Promise异步回调。 |
| [off](arkts-arkui-off-f.md#off-1) | 关闭显示设备变化的监听。 |
| [off](arkts-arkui-off-f.md#off-2) | 关闭显示设备变化的监听。 |
| [off](arkts-arkui-off-f.md#off-3) | 关闭显示设备变化的监听。 |
| [off](arkts-arkui-off-f.md#off-5) | 关闭折叠设备折叠状态变化的监听。 |
| [off](arkts-arkui-off-f.md#off-6) | 关闭折叠设备折叠角度变化的监听。 |
| [off](arkts-arkui-off-f.md#off-7) | 关闭设备的屏幕显示信息是否被获取的监听。 |
| [off](arkts-arkui-off-f.md#off-8) | 关闭折叠设备屏幕显示模式变化的监听。 |
| [off](arkts-arkui-off-f.md#off-9) | 关闭所有屏幕亮度信息状态变化的监听。 |
...
| [Rect](arkts-arkui-rect-i.md) | 矩形区域。 |
| [RelativePosition](arkts-arkui-relativeposition-i.md) | 相对坐标系下的坐标位置，以displayId对应的屏幕左上角为原点。 |
| [RoundedCorner](arkts-arkui-roundedcorner-i.md) | 屏幕圆角定义。 |
| [VirtualScreenConfig](arkts-arkui-virtualscreenconfig-i.md) | 创建虚拟屏幕的参数。 |
| [WaterfallDisplayAreaRects](arkts-arkui-waterfalldisplayarearects-i.md) | 瀑布屏曲面部分显示区域。 |
...
| 名称 | 说明 |
| --- | --- |
| [CornerType](arkts-arkui-cornertype-e.md) | 屏幕圆角类型枚举。 |
| [DisplaySourceMode](arkts-arkui-displaysourcemode-e.md) | 屏幕显示内容的显示模式枚举。 |
| [DisplayState](arkts-arkui-displaystate-e.md) | 显示设备的状态枚举。 |
| [FoldDisplayMode](arkts-arkui-folddisplaymode-e.md) | 可折叠设备的显示模式枚举。 |
| [FoldStatus](arkts-arkui-foldstatus-e.md) | 当前可折叠设备的折叠状态枚举。如果是双折轴设备，则在充电口朝下的状态下，从右到左分别是折轴一和折轴二。 |
| [Orientation](arkts-arkui-orientation-e.md) | 显示设备当前显示的方向枚举。 |
| [ScreenShape](arkts-arkui-screenshape-e.md) | 显示设备的屏幕形状枚举。 |

### 类型

| 名称 | 说明 |
| --- | --- |
| [BrightnessCallback](arkts-arkui-brightnesscallback-t.md) | 监听屏幕亮度信息时使用的回调函数类型。 |
