...
默认情况下，系统以[STANDARD](arkts-arkui-image-dynamicrangemode-e.md#DynamicRangeMode)模式进行截图。如果知道被截图组件使用的动态范围模式，可通过`dynamicRangeMode`字段指定具体的动态范围模式，并将
...
3. 当配置[色彩空间](arkts-arkui-componentsnapshot-colormodeoptions-i.md#ColorModeOptions)为SRGB或DISPLAY_P3时，截图实际采用的动态范围为SDR。
4. 如果被截图组件同时包含SDR和HDR两种动态范围的子组件时，则当作HDR处理。
5. 如果3和4的条件同时被满足，则截图实际采用的动态范围为SDR。

取值范围：[DynamicRangeMode](arkts-arkui-image-dynamicrangemode-e.md#DynamicRangeMode) 枚举值。
