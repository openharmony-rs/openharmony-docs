...
继承于[BaseEvent](arkts-arkui-common-baseevent-i.md#BaseEvent)。
...
该接口仅能在[MouseEvent](arkts-arkui-common-mouseevent-i.md#MouseEvent)中调用，用于获取触发[onMouse](arkts-arkui-commonmethod-c.md#onMouse-1)时当前帧历史点的相关信息，不同设备每帧的鼠标事件上报频率不同，一
帧通常只会上报一个鼠标事件，如果当前帧收到的[MouseEvent](arkts-arkui-common-mouseevent-i.md#MouseEvent)数目大于1，会将该帧最后一个点通过[onMouse](arkts-arkui-commonmethod-c.md#onMouse-1)返回，其余点作为历
史点。
...
**说明：** 在使用[postInputEventWithStrategy](arkts-arkui-buildernode-c.md#postInputEventWithStrategy-1)接口分发事件时会使用该字段，事件每分
发一次字段会增加100000。
