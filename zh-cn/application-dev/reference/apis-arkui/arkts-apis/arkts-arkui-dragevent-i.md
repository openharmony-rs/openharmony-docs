...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| customDropAnimation | Callback&lt;void&gt; | 是 | 在此回调函数中实现自定义落位动效。<br/>**说明：**<br/>1. 该接口仅在onDrop回调中使用有效。<br/>2.<br/>使用前需设置useCustomDropAnimation为true，否则该接口不生效。<br/>3. 不要在动画callback中实现与动效无关的逻辑，避免影响执行效率。 |
...
| 错误码ID | 错误信息 |
| --- | --- |
| [190001](../../errorcode-universal.md#190001-Data) | Data not found. |
| [190002](../../errorcode-universal.md#190002-Data) | Data error. |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| keys | Array&lt;string&gt; | 是 | 获取功能键按压状态。报错信息请参考以下错误码。支持功能键 'Ctrl' \| 'Alt' \| 'Shift'。<br/>**说明：**<br/>此接口不支持在手写笔场景<br/>下使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | DataSyncOptions | 是 | 获取拖拽数据时的参数，包含目标路径、文件冲突选项、进度条类型等。数据传输过程中可使用<br/>[cancelDataLoading](arkts-arkui-dragcontroller-c.md#cancelDataLoading-1)接口取消数据加载。 |
