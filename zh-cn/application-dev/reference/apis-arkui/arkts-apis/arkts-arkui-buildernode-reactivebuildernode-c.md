...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| uiContext | UIContext | 是 | UI上下文，获取方式可参考UIContext获取方法。uiContext需要为一个有效的值，即UI上下文正确，如果传入非法值或者未设置，会导致创建失败。 |
...
> onTouch。  
>  
> 注入事件为轴事件（AxisEvent）时，由于轴事件中缺少旋转轴信息，因此注入的事件无法触发  
> RotationGesture。  
>  
...
> TouchType中DOWN和UP字段都要有，防止出现未定义行为。  
>  
...
> 不建议同一个事件转发多次。  
>  
> postInputEvent的参数不支持UIExtensionComponent。
...
> TouchType不会发生变化，规格可查看  
> onTouch。  
>  
> - 注入事件为轴事件AxisEvent时，由于轴事件中缺少旋转轴信息，因此注入的事件无法触发旋转手势  
> RotationGesture。  
>  
...
>  
> - 支持同一个事件转发多次，不支持UIExtensionComponent调用本接  
> 口。
...
OffsetA为buildNode相对于父组件的偏移量，可以通过FrameNode中的[getPositionToParent](arkts-arkui-framenode-c.md#getpositiontoparent)获取。OffsetB为point点相对于buildNode的偏移量，可以通过TouchEvent获取。OffsetC为OffsetA与OffsetB的和，是传给postTouchEvent的最终结果。
...
>  
> postTouchEvent的参数不支持UIExtensionComponent。
