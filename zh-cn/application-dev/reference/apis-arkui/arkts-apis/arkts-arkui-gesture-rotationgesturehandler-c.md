...
| 类型 | 说明 |
| --- | --- |
| [RotationGestureHandler](arkts-arkui-gesture-rotationgesturehandler-c.md) | 返回当前旋转手势处理器对象。 |
...
设置旋转手势处理器取消回调。旋转手势处理器识别成功后，接收到触摸取消事件时触发回调。与[onActionCancel](arkts-arkui-gesture-rotationgesturehandler-c.md#onactioncancel-1)相比，此接口返回手势事件信息。
