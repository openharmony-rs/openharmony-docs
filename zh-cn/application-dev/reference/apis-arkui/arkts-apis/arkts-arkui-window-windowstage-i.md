...
调用该接口前，建议先通过loadContent方法或者setUIContent方法完成页面加载。
...
根据指定路由页面名称为当前窗口加载命名路由页面，使用callback异步回调。
