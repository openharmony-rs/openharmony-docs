...
提供ArkUI在Native侧的输入事件定义，支持触摸、鼠标、轴和按键事件，以及事件属性获取、事件冒泡控制和事件克隆处理，用于Native组件识别和处理用户输入。
...
| 名称 | 描述                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -- |----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [int32_t OH_ArkUI_UIInputEvent_GetType(const ArkUI_UIInputEvent* event)](#oh_arkui_uiinputevent_gettype) | 获取UI输入事件的类型。在访问一个ArkUI_UIInputEvent指针对象之前，推荐使用该方法判断该输入事件的类型，该接口会返回[ArkUI_UIInputEvent_Type](#arkui_uiinputevent_type)枚举中的一种。比如，当事件是一个触控类型时，由于触控类型是指向性事件，那么使用OH_ArkUI_UIInputEvent_GetXXX及OH_ArkUI_PointerEvent_GetXXX系列接口，均可以正常访问；而如果使用OH_ArkUI_KeyEvent_GetXXX相关接口去访问它，则会有无法预期的结果。对于还未支持的事件类型，接口返回默认值0。                                                                                                                                                                                                                                                       |
| [int32_t OH_ArkUI_UIInputEvent_GetAction(const ArkUI_UIInputEvent* event)](#oh_arkui_uiinputevent_getaction) | 获取输入事件的action类型。action类型为基础事件在不同阶段的类型定义，通常代表事件的特点，并表征事件的开始与结束，如touch down、touch up。触控事件的action类型为UI_TOUCH_EVENT_ACTION_XXX，鼠标事件的action类型为UI_MOUSE_EVENT_ACTION_XXX。                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| [int32_t OH_ArkUI_UIInputEvent_GetToolType(const ArkUI_UIInputEvent* event)](#oh_arkui_uiinputevent_gettooltype) | 获取UI输入事件的工具类型。输入工具是操作输入源设备以产生事件的工具，如手指、触控笔。它们自身不直接产生事件，但可以驱动输入源设备不断产生事件。返回的类型由UI_INPUT_EVENT_TOOL_TYPE_XXX枚举值定义。                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| [float OH_ArkUI_PointerEvent_GetGlobalDisplayXByIndex(const ArkUI_UIInputEvent* event, uint32_t pointerIndex)](#oh_arkui_pointerevent_getglobaldisplayxbyindex) | 从指向性输入事件（如触摸事件、鼠标事件、轴事件）中获取相对于全局显示的X坐标。只能从指向性输入事件中获取位置信息，对于鼠标和轴事件，当给定的pointerIndex大于0时，始终返回默认值0.0f。 |
...
| [float OH_ArkUI_PointerEvent_GetGlobalDisplayYByIndex(const ArkUI_UIInputEvent* event, uint32_t pointerIndex)](#oh_arkui_pointerevent_getglobaldisplayybyindex) | 从指向性输入事件（如触摸事件、鼠标事件、轴事件）中获取相对于全局显示的Y坐标。只能从指向性输入事件中获取位置信息，对于鼠标和轴事件，当给定的pointerIndex大于0时，始终返回默认值0.0f。 |
...
| [float OH_ArkUI_PointerEvent_GetTouchAreaWidth(const ArkUI_UIInputEvent* event, uint32_t pointerIndex)](#oh_arkui_pointerevent_gettouchareawidth) | 从指向性输入事件（如触摸事件）中获取触屏区域的宽度。仅适用于手指操作产生的触控事件。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| [float OH_ArkUI_PointerEvent_GetTouchAreaHeight(const ArkUI_UIInputEvent* event, uint32_t pointerIndex)](#oh_arkui_pointerevent_gettouchareaheight) | 从指向性输入事件（如触摸事件）中获取触屏区域的高度。仅适用于手指操作产生的触控事件。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| [int32_t OH_ArkUI_PointerEvent_GetInteractionHand(const ArkUI_UIInputEvent *event, ArkUI_InteractionHand *hand)](#oh_arkui_pointerevent_getinteractionhand) | 获取当前触摸事件是左手触发还是右手触发。仅在部分支持触控交互手别识别的产品上有效。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| [double OH_ArkUI_AxisEvent_GetHorizontalAxisValue(const ArkUI_UIInputEvent* event)](#oh_arkui_axisevent_gethorizontalaxisvalue) | 获取当前轴事件的水平滚动轴的值，该值通过在触控板上双指横向滑动产生。 1. 上报的数值单位为px，表示单次滚动增量，非滚动总量。 2. 上报的数值不受用户配置的放大系数[OH_ArkUI_AxisEvent_GetScrollStep](#oh_arkui_axisevent_getscrollstep)影响。 3. 数值的正负代表方向，双指从左往右滑动时上报数值为负数，双指从右往左滑动时上报数值为正数。 4. 方向会受系统设置中“自然滚动”配置的影响。 |
| [double OH_ArkUI_AxisEvent_GetPinchAxisScaleValue(const ArkUI_UIInputEvent* event)](#oh_arkui_axisevent_getpinchaxisscalevalue) | 获取当前轴事件的捏合轴缩放的值。通过触控板双指缩放/捏合操作产生。上报的scale数值，为相对于初始状态时的当前scale值。初始状态为系统识别到用户通过触控板发生了捏合操作时的两指位置状态，此时的scale数值为1.0。在手指抬起前的本次捏合操作过程中，所上报的scale数值均将初始状态作为参考系，从初始状态往中心捏合，则上报的scale会从1.0逐步往0.0缩小；当从初始状态往外扩大双指距离时，会从1.0逐步变大。 |
...
| [int32_t OH_ArkUI_UIInputEvent_GetModifierKeyStates(const ArkUI_UIInputEvent* event, uint64_t* keys)](#oh_arkui_uiinputevent_getmodifierkeystates) | 获取UI输入事件的修饰键状态。该接口会通过keys传出当前事件发生时所有修饰键的状态，应用可将keys与[ArkUI_ModifierKeyName](capi-ui-input-event-h.md#arkui_modifierkeyname)中定义的修饰键类型进行位运算，以获取处于按下状态的修饰键。                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| [int32_t OH_ArkUI_AxisEvent_SetPropagation(const ArkUI_UIInputEvent* event, bool propagation)](#oh_arkui_axisevent_setpropagation) | 设置是否启用轴事件冒泡。默认不会进行冒泡传递，仅发送给第一个可响应轴事件的控件。可在接收到轴事件时，主动启用冒泡传递，以便当前事件可以继续传递给响应链上的下一个可响应轴事件的祖先组件处理。不支持对从手势事件中获取到的轴事件进行设置。 |
| [int32_t OH_ArkUI_AxisEvent_GetScrollStep(const ArkUI_UIInputEvent* event)](#oh_arkui_axisevent_getscrollstep) | 获取滚动轴事件的滚动步长系数，适用于鼠标滚轮产生的轴事件。该值表示用户配置的滚动放大系数。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| [int32_t OH_ArkUI_PointerEvent_SetClonedEventActionType(const ArkUI_UIInputEvent* event, int32_t actionType)](#oh_arkui_pointerevent_setclonedeventactiontype) | 设置当前带有指向性的克隆输入事件的动作类型。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| [ArkUI_ErrorCode OH_ArkUI_ClonedEvent_SetPressedKeys(const ArkUI_UIInputEvent* event, int32_t* pressedKeyCodes, int32_t length)](#oh_arkui_clonedevent_setpressedkeys) | 设置克隆事件中所有按下的按键。该接口仅适用于按键事件。只有通过[OH_ArkUI_PointerEvent_CreateClonedPointerEvent](#oh_arkui_pointerevent_createclonedpointerevent)和[OH_ArkUI_PointerEvent_CreatePointerEvent](#oh_arkui_pointerevent_createpointerevent)接口创建的ArkUI_UIInputEvent对象才能使用此接口。 |
...
| [ArkUI_ErrorCode OH_ArkUI_ClonedEvent_SetMouseButton(const ArkUI_UIInputEvent* event, int32_t button)](#oh_arkui_clonedevent_setmousebutton) | 设置克隆事件的按键类型值。该接口仅适用于鼠标事件。只有通过[OH_ArkUI_PointerEvent_CreateClonedPointerEvent](#oh_arkui_pointerevent_createclonedpointerevent)和[OH_ArkUI_PointerEvent_CreatePointerEvent](#oh_arkui_pointerevent_createpointerevent)接口创建的ArkUI_UIInputEvent对象才能使用此接口。 |
...
获取UI输入事件的触发源类型。输入源为产生输入事件的真实物理设备，如触摸屏、鼠标等，由[UI_INPUT_EVENT_SOURCE_TYPE](#anonymous3)定义，而输入工具为操作输入源设备来产生事件的工具，如手指、触控笔。当用户操作鼠标时，鼠标既是输入源，也是输入工具。对于按键事件，并不支持获取输入源类型，返回unknown。
...
获取UI输入事件的工具类型。输入工具是操作输入源设备以产生事件的工具，如手指、触控笔。它们自身不直接产生事件，但可以驱动输入源设备不断产生事件。返回类型由[UI_INPUT_EVENT_TOOL_TYPE](#anonymous2)枚举定义。对于按键事件，不支持获取输入工具类型，返回 unknown。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t pointerIndex | 表示多点触控数据列表中目标触控点的索引，有效取值范围为[0, OH_ArkUI_PointerEvent_GetPointerCount(event)-1]。超出范围时返回参数异常结果。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t* pointerIndex | 输出参数，用于接收触发当前事件的触点在多点触控数据列表中的索引。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t pointerIndex | 表示多点触控数据列表中的序号；对于鼠标和轴事件，该参数大于0时返回默认值0.0f。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t pointerIndex | 表示多点触控数据列表中的序号；对于鼠标和轴事件，仅支持索引0，传入大于0的索引时返回0.0f；对于触摸事件，传入超出有效范围的索引时返回0.0f。 |
...
pointerIndex为uint32_t，不存在小于0的取值；当索引超出当前事件支持的触点范围时为非法值。
...
| -- | -- |
| const [ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t pointerIndex | 表示多点触控数据列表中目标触控点的索引。对于鼠标和轴事件，该参数必须为0。 |
...
从指向性输入事件（如触摸事件、鼠标事件、轴事件）中获取相对于全局显示的X坐标。只能从指向性输入事件中获取位置信息，对于鼠标和轴事件，当给定的pointerIndex大于0时，始终返回默认值0.0f。
...
| -- |---------------------------------------------------------------------------------------------------------------------|
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。                                                                                                    |
| uint32_t pointerIndex | 表示多点触控数据列表中目标触控点的索引。有效值范围[0, [OH_ArkUI_PointerEvent_GetPointerCount()](#oh_arkui_pointerevent_getpointercount) - 1]；对于鼠标和轴事件，该参数必须为0。 |
...
从指向性输入事件（如触摸事件、鼠标事件、轴事件）中获取相对于全局显示的Y坐标。只能从指向性输入事件中获取位置信息，对于鼠标和轴事件，当给定的pointerIndex大于0时，始终返回默认值0.0f。
...
从指向性输入事件（如触摸事件）中获取触屏压力，适用于绘图、书写等需要根据触控压力调整笔刷粗细或交互效果的场景。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t pointerIndex | 表示多点触控数据列表中目标触控点的索引，有效值范围为[0, OH_ArkUI_PointerEvent_GetPointerCount(event) - 1]。超出范围时返回默认值0.0f。 |

**返回：**

| 类型 | 说明 |
| -- | -- |
| float | 返回当前指向性输入事件产生的触屏压力。取值范围为[0, 1]，压感大小与数值正相关。如果参数异常则返回默认值0.0f。不同设备的硬件参数配置可能存在差异，部分设备可能返回大于1的值。 |
...
从指向性输入事件（如触摸事件）中获取相对YZ平面的角度，适用于绘图、书写等需要根据触控笔倾斜方向调整笔刷效果的场景。取值的范围[-90, 90]，单位为deg，其中正值是向右倾斜。仅适用于支持倾角上报的触控笔操作产生的触控事件。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t pointerIndex | 表示多点触控数据列表中目标触控点的索引，有效值范围为[0, OH_ArkUI_PointerEvent_GetPointerCount(event) - 1]。超出范围时返回参数异常结果。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t pointerIndex | 表示多点触控数据列表中目标触控点的索引，有效取值范围为[0, OH_ArkUI_PointerEvent_GetPointerCount(event)-1]。 |
...
获取触控笔绕Z轴旋转的角度，适用于绘图、书写等需要根据触控笔旋转方向调整笔刷方向或交互效果的场景。
...
| 类型 | 说明 |
| -- | -- |
| int32_t | 错误码。<br>         Returns [ARKUI_ERROR_CODE_NO_ERROR](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) 成功。<br>         Returns [ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) 函数参数异常，可能是事件指针或rollAngle输出参数无效；请检查并传入有效参数。 |
...
从指向性输入事件（如触摸事件）中获取触屏区域的宽度。仅适用于手指操作产生的触控事件。
...
从指向性输入事件（如触摸事件）中获取触屏区域的高度。仅适用于手指操作产生的触控事件。
...
获取当前触摸事件是左手点击触发还是右手点击触发。仅在支持触控交互手别识别的产品上有效，请通过设备能力查询接口判断当前设备是否支持该功能。该值并非在按下时就能实时获取，在系统完成结果推断之前，皆默认返回NONE，因此请不要将按下阶段的返回值作为最终识别结果。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md) *event | 表示指向当前UI输入事件的指针。 |
| [ArkUI_InteractionHand](#arkui_interactionhand) *hand | 输出参数，用于接收触摸点的交互手信息，返回值为ARKUI_EVENT_HAND_LEFT、ARKUI_EVENT_HAND_RIGHT或ARKUI_EVENT_HAND_NONE。 |

**返回：**

| 类型 | 说明 |
| -- | -- |
| int32_t | 返回结果。<br> 如果操作成功，则返回[ARKUI_ERROR_CODE_NO_ERROR](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br> 如果发生参数异常，例如事件指针或hand输出参数无效，则返回[ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)；请检查并传入有效参数。 |
...
获取当前触摸事件是左手点击触发还是右手点击触发。仅在支持触控交互手别识别的产品上有效。该值并非在按下时就能实时获取，在系统完成结果推断之前，皆默认返回NONE，因此请不要将按下阶段的返回值作为最终识别结果。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md) *event | 表示指向当前UI输入事件的指针。 |
| int32_t pointerIndex | 表示多点触控数据列表中目标触控点的索引，有效值范围为[0, OH_ArkUI_PointerEvent_GetPointerCount(event) - 1]，不支持负索引；超出范围时返回[ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。 |
| [ArkUI_InteractionHand](#arkui_interactionhand) *hand | 表示触摸点是左手还是右手。 |

**返回：**

| 类型 | 说明 |
| -- | -- |
| int32_t | 返回结果。<br> 如果操作成功，则返回[ARKUI_ERROR_CODE_NO_ERROR](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br> 如果发生参数异常，例如事件指针、hand输出参数或pointerIndex无效，则返回[ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)；请检查并传入有效参数。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t historyIndex | 表示历史事件数据列表的序号，必须小于[OH_ArkUI_PointerEvent_GetHistorySize](#oh_arkui_pointerevent_gethistorysize)。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t pointerIndex | 表示特定历史事件中多点触控数据列表的序号，有效值范围为[0, OH_ArkUI_PointerEvent_GetHistoryPointerCount(event, historyIndex) - 1]。超出范围时返回参数异常结果。 |
| uint32_t historyIndex | 表示历史事件数据列表的序号，必须小于[OH_ArkUI_PointerEvent_GetHistorySize](#oh_arkui_pointerevent_gethistorysize)。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t pointerIndex | 表示多点触控数据列表中的序号，有效值范围为[0, OH_ArkUI_PointerEvent_GetHistoryPointerCount(event, historyIndex) - 1]。对于鼠标事件，该参数大于0时始终返回默认值0.0f。 |
| uint32_t historyIndex | 表示历史事件数据列表的序号，必须小于[OH_ArkUI_PointerEvent_GetHistorySize](#oh_arkui_pointerevent_gethistorysize)。 |
...
从给定指针索引和历史记录索引的指向性输入事件中获取历史事件中相对于全局显示的特定触摸点的X坐标。本接口支持的指向性输入事件仅包含触摸事件、鼠标事件。只能从指向性输入事件中获取位置信息，对于鼠标事件，当给定的pointerIndex大于0时，始终返回默认值0.0f。从API version 20开始支持触摸事件，从API version 26.0.0开始支持鼠标事件。
...
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。                                                                                                     |
| uint32_t pointerIndex | 表示多点触控数据列表中的序号，有效值范围为[0, OH_ArkUI_PointerEvent_GetHistoryPointerCount(event, historyIndex) - 1]。对于鼠标事件，该参数大于0时始终返回默认值0.0f。 |
| uint32_t historyIndex | 表示要返回的历史值，必须小于[OH_ArkUI_PointerEvent_GetHistorySize](#oh_arkui_pointerevent_gethistorysize)。 |
...
从给定指针索引和历史记录索引的指向性输入事件中获取历史事件中相对于全局显示的特定触摸点的Y坐标。本接口支持的指向性输入事件仅包含触摸事件、鼠标事件。只能从指向性输入事件中获取位置信息，对于鼠标事件，当给定的pointerIndex大于0时，始终返回默认值0.0f。从API version 20开始支持触摸事件，从API version 26.0.0开始支持鼠标事件。
...
从指向性输入事件（如触摸事件）中获取特定历史事件中的触屏压力。本接口支持的指向性输入事件仅包含触摸事件，historyIndex必须小于OH_ArkUI_PointerEvent_GetHistorySize返回的历史事件数量。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t pointerIndex | 表示特定历史事件中多点触控数据列表的序号，有效值范围为[0, OH_ArkUI_PointerEvent_GetHistoryPointerCount(event, historyIndex) - 1]。对于鼠标事件，该参数大于0时始终返回默认值0.0f。 |
...
从指向性输入事件（如触摸事件）中获取特定历史事件中的相对YZ平面的角度，取值的范围[-90, 90]，单位为deg，其中正值是向右倾斜。仅适用于支持倾角上报的触控笔操作产生的触控事件，且historyIndex必须小于OH_ArkUI_PointerEvent_GetHistorySize返回的历史事件数量。
...
从指向性输入事件（如触摸事件）中获取特定历史事件中的相对XZ平面的角度，取值的范围[-90, 90]，单位为deg，其中正值是向下倾斜。仅适用于支持倾角上报的触控笔操作产生的触控事件，且historyIndex必须小于OH_ArkUI_PointerEvent_GetHistorySize返回的历史事件数量。
...
从指向性输入事件（如触摸事件）中获取特定历史事件中的触屏区域的宽度。仅适用于手指操作产生的触控事件，且historyIndex必须小于OH_ArkUI_PointerEvent_GetHistorySize返回的历史事件数量。
...
从指向性输入事件（如触摸事件）中获取特定历史事件中的触屏区域的高度。仅适用于手指操作产生的触控事件，且historyIndex必须小于OH_ArkUI_PointerEvent_GetHistorySize返回的历史事件数量。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| uint32_t pointerIndex | 表示特定历史事件中多点触控数据列表的序号，必须小于[OH_ArkUI_PointerEvent_GetHistoryPointerCount](#oh_arkui_pointerevent_gethistorypointercount)返回的触点数量。 |
| uint32_t historyIndex | 表示历史事件数据列表的序号，必须小于[OH_ArkUI_PointerEvent_GetHistorySize](capi-ui-input-event-h.md#oh_arkui_pointerevent_gethistorysize)。 |
...
通常情况下，垂直滚动轴事件只能驱动竖向的滑动手势响应。但当鼠标指针下命中的可滑动手势的可响应方向与垂直滚动轴事件的方向一致时，垂直滚动轴事件可以驱动这些滑动手势得到响应，即使这些手势定义的滑动方向是横向的。
...
获取当前轴事件的捏合轴缩放的值。该值由触控板双指缩放或捏合操作产生。系统识别到捏合操作时，将两指位置作为初始状态，scale值为1.0。在手指抬起前的本次捏合操作过程中，所上报的scale数值均将初始状态作为参考系，从初始状态往中心捏合，则上报的scale会从1.0逐步往0.0缩小；当从初始状态往外扩大双指距离时，会从1.0逐步变大。
...
配置触摸测试模式，用于在接收基础触摸事件时控制组件自身、子组件、兄弟组件和祖先组件是否参与触摸命中测试，例如在需要调整触摸事件分发范围的场景中使用。仅适用于接收基础事件的场景，如使用NODE_ON_TOUCH接收触摸事件场景。对于通过[OH_ArkUI_GestureEvent_GetRawInputEvent](capi-native-gesture-h.md#oh_arkui_gestureevent_getrawinputevent)接口从一个手势事件中获取到的ArkUI_UIInputEvent对象，无法使用该接口。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | 表示指向当前UI输入事件的指针。 |
| [HitTestMode](capi-ui-input-event-h.md#hittestmode) mode | 指定触摸测试模式。HTM_DEFAULT适用于自身及子节点响应触摸测试且阻塞兄弟节点的场景；HTM_BLOCK适用于仅自身响应并阻塞子节点、兄弟节点和祖先节点的场景；HTM_TRANSPARENT适用于自身和子节点响应且不阻塞兄弟节点和祖先节点的场景；HTM_NONE适用于自身不响应触摸测试且不影响其他节点的场景；HTM_BLOCK_HIERARCHY适用于自身和子节点响应并阻止低优先级兄弟节点及父节点参与触摸测试的场景；HTM_BLOCK_DESCENDANTS适用于自身及所有后代均不响应触摸测试且不影响祖先节点的场景。根据所需的节点响应范围和阻塞关系选择对应模式。 |
...
设置是否阻止事件冒泡，适用于当前组件已处理基础触摸事件且不希望事件继续传递给父组件或其他祖先组件的场景。仅适用于接收基础事件的场景，如使用NODE_ON_TOUCH接收触摸事件场景，不适用于轴事件。对于通过[OH_ArkUI_GestureEvent_GetRawInputEvent](capi-native-gesture-h.md#oh_arkui_gestureevent_getrawinputevent)接口从一个手势事件中获取到的ArkUI_UIInputEvent对象，无法使用该接口。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| int32_t* pressedKeyCodes | 输出参数，指向由调用者分配的按键编码数组，用于接收所有按下按键的键值；数组容量由输入参数length指定，实际按键数量通过length输出。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| int32_t axis | 焦点轴事件的轴类型。需要读取游戏手柄横向、纵向或深度控制量时分别选择UI_FOCUS_AXIS_EVENT_ABS_X、UI_FOCUS_AXIS_EVENT_ABS_Y或UI_FOCUS_AXIS_EVENT_ABS_Z；需要读取旋转控制量时选择UI_FOCUS_AXIS_EVENT_ABS_RZ、UI_FOCUS_AXIS_EVENT_ABS_RX或UI_FOCUS_AXIS_EVENT_ABS_RY；需要读取油门、刹车、方向舵或方向盘控制量时分别选择UI_FOCUS_AXIS_EVENT_ABS_GAS、UI_FOCUS_AXIS_EVENT_ABS_BRAKE、UI_FOCUS_AXIS_EVENT_ABS_RUDDER或UI_FOCUS_AXIS_EVENT_ABS_WHEEL；需要读取方向帽控制量时选择UI_FOCUS_AXIS_EVENT_ABS_HAT0X、UI_FOCUS_AXIS_EVENT_ABS_HAT0Y、UI_FOCUS_AXIS_EVENT_ABS_HAT1X、UI_FOCUS_AXIS_EVENT_ABS_HAT1Y、UI_FOCUS_AXIS_EVENT_ABS_HAT2X、UI_FOCUS_AXIS_EVENT_ABS_HAT2Y、UI_FOCUS_AXIS_EVENT_ABS_HAT3X或UI_FOCUS_AXIS_EVENT_ABS_HAT3Y。根据要读取的游戏手柄控制轴选择对应枚举值。 |
...
获取UI输入事件的修饰键状态。该接口会通过keys传出当前事件发生时所有修饰键的状态，应用可将keys与[ArkUI_ModifierKeyName](capi-ui-input-event-h.md#arkui_modifierkeyname)中定义的修饰键类型进行位运算，以获取处于按下状态的修饰键。
...
| 类型 | 说明 |
| -- | -- |
| int32_t | 错误码。<br>         [ARKUI_ERROR_CODE_NO_ERROR](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) 成功。<br>         [ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) 函数参数异常，可能是事件指针或输出参数无效；请检查并传入有效参数。 |
...
设置是否启用轴事件冒泡。默认不会进行冒泡传递，仅发送给第一个可响应轴事件的控件。可在接收到轴事件时，主动启用冒泡传递，以便当前事件可以继续传递给响应链上的下一个可响应轴事件的祖先组件处理。不支持对从手势事件中获取到的轴事件进行设置。
...
获取滚动轴事件的滚动步长系数，适用于鼠标滚轮产生的轴事件。该值表示用户配置的滚动放大系数。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| uint32_t pointerIndex | 指示多点触控数据列表中目标触控点的索引，有效值范围为[0, OH_ArkUI_PointerEvent_GetPointerCount(event) - 1]。超出范围时返回0。 |
...
| 类型 | 说明 |
| -- | -- |
| float | 返回鼠标设备在二维平面Y轴的移动增量，使用物理世界中鼠标移动的距离单位进行表示；如果发生任何参数错误，则返回0.0f。<br>**说明：** API版本26.0.0之前，返回值并非鼠标硬件的原始移动数据，而是原始数据缩小了Y倍，Y为系统的显示大小比例。API版本26.0.0开始，返回值为鼠标硬件的原始移动数据。 |
...
| int32_t* length | 作为输入参数表示传入的pressedButtons数组长度，作为输出参数表示实际按下按键的个数。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| [ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)** clonedEvent | 输出参数，由接口写入新创建的克隆ArkUI_UIInputEvent对象指针；使用完毕后应调用OH_ArkUI_PointerEvent_DestroyClonedEvent释放资源。 |
...
设置当前带有指向性的克隆输入事件的动作类型。只有通过[OH_ArkUI_PointerEvent_CreateClonedEvent](#oh_arkui_pointerevent_createclonedevent)接口创建的ArkUI_UIInputEvent克隆事件指针才能使用此接口。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| int32_t actionType | ArkUI_UIInputEvent克隆事件的动作类型。触摸事件根据按下、移动、抬起或取消场景选择UI_TOUCH_EVENT_ACTION_DOWN、UI_TOUCH_EVENT_ACTION_MOVE、UI_TOUCH_EVENT_ACTION_UP或UI_TOUCH_EVENT_ACTION_CANCEL；鼠标事件根据按下、松开、移动或取消场景选择UI_MOUSE_EVENT_ACTION_PRESS、UI_MOUSE_EVENT_ACTION_RELEASE、UI_MOUSE_EVENT_ACTION_MOVE或UI_MOUSE_EVENT_ACTION_CANCEL；轴事件根据开始、更新、结束或取消场景选择UI_AXIS_EVENT_ACTION_BEGIN、UI_AXIS_EVENT_ACTION_UPDATE、UI_AXIS_EVENT_ACTION_END或UI_AXIS_EVENT_ACTION_CANCEL。根据克隆事件的事件类型和动作阶段选择对应取值。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| [ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)** clonedEvent | 输出参数，用于接收创建成功后的ArkUI_UIInputEvent克隆事件指针；使用完毕后需调用OH_ArkUI_PointerEvent_DestroyClonedPointerEvent()释放该事件。 |
...
创建新事件，不克隆现有事件。该接口适用于触摸事件、鼠标事件和轴事件。调用成功后，请使用OH_ArkUI_ClonedEvent_SetXXX系列接口设置事件属性。事件使用完毕后，请调用OH_ArkUI_PointerEvent_DestroyClonedPointerEvent()销毁事件，避免资源未释放。
...
| 参数项 | 描述 |
| -- | -- |
| [ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)** event | 输出参数，用于接收新创建的ArkUI_UIInputEvent对象；使用完毕后需调用OH_ArkUI_PointerEvent_DestroyClonedPointerEvent()释放该事件。 |
| [ArkUI_UIInputEvent_Type](#arkui_uiinputevent_type) type | ArkUI_UIInputEvent的事件类型。创建触摸事件时选择ARKUI_UIINPUTEVENT_TYPE_TOUCH，适用于模拟或处理触摸输入；创建轴事件时选择ARKUI_UIINPUTEVENT_TYPE_AXIS，适用于模拟或处理鼠标滚轮、触控板滚动或捏合输入；创建鼠标事件时选择ARKUI_UIINPUTEVENT_TYPE_MOUSE，适用于模拟或处理鼠标按键或移动输入；根据需要创建的输入设备事件类型选择对应取值。 |
...
| 类型 | 说明 |
| -- | -- |
| [ArkUI_ErrorCode](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) | 返回结果代码。<br>         如果操作成功，则返回[ARKUI_ERROR_CODE_NO_ERROR](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果event无效，则返回[ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果输入的事件指针不是克隆事件指针，则返回[ARKUI_ERROR_CODE_NOT_CLONED_POINTER_EVENT](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。|
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| int32_t type | 克隆事件的动作类型，包括触摸事件的[UI_TOUCH_EVENT_ACTION](#anonymous1)，鼠标事件的[UI_MOUSE_EVENT_ACTION](#anonymous4)，轴事件的[UI_AXIS_EVENT_ACTION](#anonymous7)。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| int32_t sourceType | 克隆事件的源类型。取值为0、1、2、4或5，其中0表示未知，1表示鼠标，2表示触屏，4表示键盘，5表示手柄。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| int32_t toolType | 克隆事件的工具类型。手指操作选择UI_INPUT_EVENT_TOOL_TYPE_FINGER，触控笔操作选择UI_INPUT_EVENT_TOOL_TYPE_PEN，鼠标操作选择UI_INPUT_EVENT_TOOL_TYPE_MOUSE，触控板操作选择UI_INPUT_EVENT_TOOL_TYPE_TOUCHPAD，操纵杆操作选择UI_INPUT_EVENT_TOOL_TYPE_JOYSTICK，无法确定工具类型时选择UI_INPUT_EVENT_TOOL_TYPE_UNKNOWN。应根据实际产生事件的操作工具选择对应取值。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| float pressure | 应用于触摸屏的压力，通常取值范围为[0, 1]，包含0和1；部分设备可能支持大于1的值，具体范围以设备能力为准。传入不支持的值时返回[ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。 |

**返回：**

| 类型 | 说明 |
| -- | -- |
| [ArkUI_ErrorCode](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) | 返回结果代码。<br>         如果操作成功，则返回[ARKUI_ERROR_CODE_NO_ERROR](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果入参错误，例如event无效或pressure参数不符合要求，则返回[ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)；请检查事件指针和压力参数。<br>         如果输入的事件指针不是克隆事件指针，则返回[ARKUI_ERROR_CODE_NOT_CLONED_POINTER_EVENT](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果事件类型不支持，可能是传入了非触摸事件；请传入触摸事件后重试。|
...
| 类型 | 说明 |
| -- | -- |
| [ArkUI_ErrorCode](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) | 返回结果代码。<br>         如果操作成功，则返回[ARKUI_ERROR_CODE_NO_ERROR](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果入参错误，例如event或fingerId无效，则返回[ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果输入的事件指针不是克隆事件指针，则返回[ARKUI_ERROR_CODE_NOT_CLONED_POINTER_EVENT](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果事件类型不支持，可能是传入了非触摸事件；请传入触摸事件后重试。|
...
| 类型 | 说明 |
| -- | -- |
| [ArkUI_ErrorCode](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) | 返回结果代码。<br>         如果操作成功，则返回[ARKUI_ERROR_CODE_NO_ERROR](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果入参错误，例如event无效、pointerIndex超出有效范围或坐标参数不符合要求，则返回[ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)；请检查事件指针、触摸点索引和坐标参数。<br>         如果输入的事件指针不是克隆事件指针，则返回[ARKUI_ERROR_CODE_NOT_CLONED_POINTER_EVENT](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果事件类型不支持，可能是传入了非触摸事件；请传入触摸事件后重试。|
...
设置克隆事件相对于YZ平面和XZ平面的倾斜角度。tiltX和tiltY的取值范围均为[-90, 90]，包含-90和90；其中tiltX正值表示向右倾斜，tiltY正值表示向下倾斜。该接口适用于触摸事件。只有通过[OH_ArkUI_PointerEvent_CreateClonedPointerEvent](#oh_arkui_pointerevent_createclonedpointerevent)和[OH_ArkUI_PointerEvent_CreatePointerEvent](#oh_arkui_pointerevent_createpointerevent)接口创建的ArkUI_UIInputEvent对象才能使用此接口。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| int32_t* pressedKeyCodes | 所有按下按键的键值数组，键值取值为[ArkUI_KeyCode](capi-native-key-event-h.md#arkui_keycode)。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| int32_t hand | 触摸点的交互手类型。系统识别为左手触摸时选择ARKUI_EVENT_HAND_LEFT，系统识别为右手触摸时选择ARKUI_EVENT_HAND_RIGHT，无法确定交互手时选择ARKUI_EVENT_HAND_NONE；仅在需要模拟或修正克隆触摸事件的左右手属性时设置对应取值。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| double pinchAxisScaleValue | 设置克隆轴事件的捏合轴缩放值，用于控制该事件对应的捏合缩放状态；取值范围为[0, +∞)。 |
...
设置克隆事件水平滚动轴的缩放值。该接口仅适用于轴事件。只有通过[OH_ArkUI_PointerEvent_CreateClonedPointerEvent](#oh_arkui_pointerevent_createclonedpointerevent)和[OH_ArkUI_PointerEvent_CreatePointerEvent](#oh_arkui_pointerevent_createpointerevent)接口创建的ArkUI_UIInputEvent对象才能使用此接口。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| double horizontalAxisScaleValue | 水平滚动轴的值，用于控制该事件的水平滚动行为，单位为px。 |
...
设置克隆事件垂直滚动轴的缩放值。该接口仅适用于轴事件。只有通过[OH_ArkUI_PointerEvent_CreateClonedPointerEvent](#oh_arkui_pointerevent_createclonedpointerevent)和[OH_ArkUI_PointerEvent_CreatePointerEvent](#oh_arkui_pointerevent_createpointerevent)接口创建的ArkUI_UIInputEvent对象才能使用此接口。
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| double verticalAxisScaleValue | 垂直滚动轴的值，用于控制该事件的垂直滚动行为。单位根据事件来源确定：触控板双指滑动为px，鼠标滚轮为角度。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| int32_t scrollStep | 克隆事件的滚动步长系数，用于控制鼠标滚轮轴事件的滚动放大效果，取值为[0, 65535]的整数。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| int32_t button | 克隆事件的鼠标按钮类型。模拟左键点击时选择UI_MOUSE_EVENT_BUTTON_LEFT，模拟右键菜单操作时选择UI_MOUSE_EVENT_BUTTON_RIGHT，模拟中键操作时选择UI_MOUSE_EVENT_BUTTON_MIDDLE，模拟鼠标后退或前进操作时分别选择UI_MOUSE_EVENT_BUTTON_BACK或UI_MOUSE_EVENT_BUTTON_FORWARD，无按钮操作时选择UI_MOUSE_EVENT_BUTTON_NONE；根据需要模拟的鼠标操作选择对应取值。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| float rawDeltaX | 鼠标位置相对于之前上报的鼠标事件中位置的X轴偏移量，单位为鼠标硬件上报的距离单位。 |
...
| -- | -- |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| float rawDeltaY | 鼠标位置相对于之前上报的鼠标事件中位置的Y轴偏移量，单位为鼠标硬件上报的距离单位。 |
...
| [ArkUI_NodeHandle](capi-arkui-nativemodule-arkui-node8h.md) node | 目标节点。 |
| [const ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)* event | ArkUI_UIInputEvent事件指针。 |
| [ArkUI_CompetitionStrategy](#arkui_competitionstrategy) strategy | 竞争策略。注入事件需要与目标节点已有手势独立并行处理时选择ARKUI_COMPETITION_STRATEGY_DEFAULT；需要让注入方和被注入方手势互相竞争且只处理其中一方时选择ARKUI_COMPETITION_STRATEGY_COMPETITION；根据是否需要手势竞争选择对应策略。 |

**返回：**

| 类型 | 说明 |
| -- | -- |
| [ArkUI_ErrorCode](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode) | 返回结果代码。<br>         如果操作成功，则返回[ARKUI_ERROR_CODE_NO_ERROR](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果入参错误，例如node、event或strategy无效，则返回[ARKUI_ERROR_CODE_PARAM_INVALID](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)；请检查并传入有效节点、事件指针和竞争策略。<br>         如果输入的事件指针不是克隆事件指针，则返回[ARKUI_ERROR_CODE_NOT_CLONED_POINTER_EVENT](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果组件状态异常，则返回[ARKUI_ERROR_CODE_POST_CLONED_COMPONENT_STATUS_ABNORMAL](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。<br>         如果未命中可响应事件的组件，则返回[ARKUI_ERROR_CODE_POST_CLONED_NO_COMPONENT_HIT_TO_RESPOND_TO_THE_EVENT](capi-arkui-nativemodule-arkui-error-code-h.md#arkui_errorcode)。|
...
调用该方法获取最近一次UIInput相关方法的执行情况。仅在调用UIInput相关方法后需要确认该次调用是否执行异常时使用。
...
请在从[ArkUI_NodeEvent](capi-arkui-nativemodule-arkui-nodeevent.md)对象获取[ArkUI_UIInputEvent](capi-arkui-eventmodule-arkui-uiinputevent.md)对象后调用此方法，以获取惯性滚动轴事件。仅当用户在触控板上使用双指执行满足系统惯性滚动判定条件的抛滑操作，且指针位置下存在注册了[NODE_ON_COASTING_AXIS_EVENT](capi-native-node-h.md#arkui_nodeeventtype)事件的组件时，才能获取有效事件。
...
1. 手指触摸触控板。
2. 滚动鼠标滚轮。
3. 手指或鼠标点击注册了惯性滚动轴事件的节点。需要注意的是，点击未注册此事件的节点不会中断当前惯性滚动轴事件。例如，A节点注册了惯性滚动轴事件，当事件发生时，让B节点滚动，但点击B节点不会中断此事件。点击事件的中断行为受[OH_ArkUI_PointerEvent_SetInterceptHitTestMode](#oh_arkui_pointerevent_setintercepthittestmode)的影响。此外，如果用户点击区域存在已收集的可响应惯性滚动轴事件的节点，本次惯性滚动轴事件会被强制终止。
...
| 类型 | 说明 |
| -- | -- |
| int64_t | 返回惯性滚动轴事件发生的时间，单位为ns；如果发生任何参数错误，则返回0。|
...
获取惯性滚动轴事件发生时的滚动阶段。当返回[ARKUI_COASTING_AXIS_EVENT_PHASE_END](#arkui_coastingaxiseventphase)时，应立即停止惯性滚动效果。
...
| -- | -- |
| [ArkUI_TouchTestInfo](./capi-arkui-nativemodule-arkui-touchtestinfo.md)* info | 指向触摸测试信息的指针。 |
| [ArkUI_TouchTestStrategy](#arkui_touchteststrategy) strategy | 触摸测试策略。无需自定义分发、由系统按当前节点命中状态分发事件时选择ARKUI_TOUCH_TEST_STRATEGY_DEFAULT；需要将事件指定分发给某个子节点且允许系统决定其他兄弟节点是否分发时选择ARKUI_TOUCH_TEST_STRATEGY_FORWARD_COMPETITION；需要将事件指定分发给某个子节点并禁止分发给其他兄弟节点时选择ARKUI_TOUCH_TEST_STRATEGY_FORWARD；根据是否需要自定义分发及是否允许兄弟节点继续参与选择对应策略。 |
...
设置是否阻止事件冒泡，适用于当前组件已处理表冠事件且不希望该事件继续传递给父组件或其他祖先组件的场景。仅适用于入参event里存储的是表冠事件对象的场景。
