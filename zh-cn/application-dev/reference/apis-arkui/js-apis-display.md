...
| brightnessPosition          | number    | 是  | 是   | 当前屏幕亮度所对应的亮度条位置，该参数为浮点数，取值范围[0.0, 1.0]，默认值为0.0。0.0和1.0分别代表亮度条的起始位置和结束位置。该参数返回的亮度条位置与实际可能存在精度为0.01的误差。<br>**起始版本：** 26.0.0。<br>**原子化服务API：** 从API版本26.0.0开始，该接口支持在原子化服务中使用。 |
