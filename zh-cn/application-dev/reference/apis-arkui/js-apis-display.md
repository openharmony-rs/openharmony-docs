...
| callback | Callback&lt;boolean&gt; | 是 | 回调函数。表示当前设备的屏幕显示信息是否存在被获取的情况。true表示设备开始处于截屏、投屏、录屏状态，或创建了虚拟屏幕(虚拟屏幕可能被应用获取屏幕图像)；false表示获取结束。截屏仅返回一次true。|
...
| callback | Callback&lt;boolean&gt; | 否 | 需要取消注册的回调函数。表示当前设备的屏幕显示信息是否存在被获取的情况。true表示设备开始处于截屏、投屏、录屏状态，或创建了虚拟屏幕(虚拟屏幕可能被应用获取屏幕图像)；false表示获取结束；截屏仅返回一次true。若无此参数，则取消注册当前设备的屏幕显示信息是否存在被获取监听的所有回调函数。|
