...
| 名称   | 类型   | 只读 | 可选 | 说明                   |
| ------ | ------ | ---- | ---- | ---------------------- |
| width  | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 否   | 组件大小的宽度。<br>单位：vp<br>取值范围：[0, +∞)<br>负数按默认值处理。 |
| height | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 否   | 组件大小的高度。<br>单位：vp<br>取值范围：[0, +∞)<br>负数按默认值处理。 |
...
| width  | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 否   | 组件的宽度。<br>单位：vp<br>取值范围：[0, +∞)<br>负数按默认值处理。   |
| height | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 否   | 组件的高度。<br>单位：vp<br>取值范围：[0, +∞)<br>负数按默认值处理。   |
...
| 名称   | 类型 | 只读 | 可选 | 说明             |
| ------------ | ---------------------------------------- | ---- | ---- | ------ |
| value       | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 否   | 长度属性的值。<br>取值范围：(-∞, +∞)。<br>当unit为PERCENT时，value表示百分比（1表示100%），参考尺寸取决于具体使用场景；其余单位表示对应单位的绝对长度。   |
...
| 参数名 | 类型          | 必填 | 说明         |
| ------ | ------------- | ---- | ------------ |
| value   | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 长度属性的值。<br>取值范围：[0, 1]<br>超出范围时按边界值处理。 |
...
| 参数名 | 类型          | 必填 | 说明         |
| ------ | ------------- | ---- | ------------ |
| value   | ArkTS-Dyn: number<br>ArkTS-Sta: int | 是   | HEX格式颜色，支持RGB或者ARGB。<br>取值范围：[0, 0xffffffff]<br>超出范围时按边界值处理。 |
...
| 参数名 | 类型          | 必填 | 说明         |
| ------ | ------------- | ---- | ------------ |
| red   | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 颜色的R分量（红色），值是0~255的整数。超出范围时按边界值处理。 |
| green | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 颜色的G分量（绿色），值是0~255的整数。超出范围时按边界值处理。 |
| blue  | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 颜色的B分量（蓝色），值是0~255的整数。超出范围时按边界值处理。 |
...
| red   | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 颜色的R分量（红色），值是0~1的浮点数。超出范围时按边界值处理。 |
| green | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 颜色的G分量（绿色），值是0~1的浮点数。超出范围时按边界值处理。 |
| blue  | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 颜色的B分量（蓝色），值是0~1的浮点数。超出范围时按边界值处理。 |
| alpha | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 颜色的A分量（透明度），值是0.0~1.0的浮点数，默认值为1.0，不透明。<br> **说明：** alpha小于0为全透明，大于1为不透明。|
...
| 类型          | 说明             |
| ------------- | ---------------- |
| ArkTS-Dyn: number<br>ArkTS-Sta: int | 颜色的A分量（透明度），值是0~255的整数。通过rgba()或colorWithSpace()方法设置时alpha取值范围为0.0~1.0的浮点数，内部会转换为0~255的整数存储。|
...
| 类型                            | 说明               |
| ------------------------------- | ------------------ |
| [Corners](#cornerst12)\<double> | 四个角的圆角度数。 |
...
| 名称    | 类型   | 只读 | 可选 | 说明                      |
| ------- | ------ | ---- | ---- | ------------------------- |
| centerX | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 否   | 圆心x轴的位置，单位为px。<br>取值范围：(-∞, +∞) |
| centerY | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 否   | 圆心y轴的位置，单位为px。<br>取值范围：(-∞, +∞) |
| radius  | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 否   | 圆形的半径，单位为px。<br>取值范围：[0, +∞)<br>负数按默认值处理。   |
...
| 名称            | 类型    | 只读 | 可选 | 说明                                                |
| --------------- | ------ | ---- | ---- | -------------------------------------------------- |
| fillColor       | ArkTS-Dyn: number<br>ArkTS-Sta: int | 否   | 否   | 遮罩的填充颜色，使用ARGB格式。默认值为`0XFF000000`。<br>取值范围：[0, 0xffffffff]<br>超出范围时按默认值处理。<br> 通过fillColor的透明度和亮度生成一个仅含透明度的颜色。亮度越高，颜色越透明。然后，使用[BlendMode.SRC_IN](../apis-arkgraphics2d/arkts-apis-graphics-drawing-e.md#blendmode)方式与RenderNode本身的颜色混合，生成最终颜色。 |
| strokeColor     | ArkTS-Dyn: number<br>ArkTS-Sta: int | 否   | 否   | 遮罩的边框颜色，使用ARGB格式。默认值为`0XFF000000`。<br>取值范围：[0, 0xffffffff]<br>超出范围时按默认值处理。 <br>  通过strokeColor的透明度和亮度生成一个仅含透明度的颜色。亮度越高，颜色越透明。然后，使用[BlendMode.SRC_IN](../apis-arkgraphics2d/arkts-apis-graphics-drawing-e.md#blendmode)方式与RenderNode本身的颜色混合，生成最终颜色。 |
| strokeWidth     | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 否   | 遮罩的边框宽度，单位为px。默认值为0。<br>取值范围：[0, +∞)<br>负数按默认值处理。   |
...
| 参数名 | 类型   | 必填 | 说明                 |
| ------ | ------ | ---- | -------------------- |
| all    | ArkTS-Dyn: number<br>ArkTS-Sta: int | 是   | 边框颜色，ARGB格式，示例：0xffff00ff。<br>取值范围：[0, 0xffffffff]<br>超出范围时按边界值处理。 |

**返回值：**

| 类型                     | 说明                                   |
| ------------------------ | -------------------------------------- |
| ArkTS-Dyn: [Edges](#edgest12)\<number> <br> ArkTS-Sta: [NodeEdges](#nodeedgest20)\<int> | 边框颜色均设置为传入值的边框颜色对象。 |
...
| 参数名 | 类型   | 必填 | 说明                 |
| ------ | ------ | ---- | -------------------- |
| all    | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 边框宽度，单位为vp。<br>取值范围：[0, +∞)<br>负数按默认值处理。 |

**返回值：**

| 类型                     | 说明                                   |
| ------------------------ | -------------------------------------- |
| ArkTS-Dyn: [Edges](#edgest12)\<number> <br> ArkTS-Sta: [NodeEdges](#nodeedgest20)\<double> | 边框宽度均设置为传入值的边框宽度对象。 |
...
| 参数名 | 类型   | 必填 | 说明       |
| ------ | ------ | ---- | ---------- |
| all    | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 边框圆角。<br>单位：vp<br>取值范围：[0, +∞)<br>负数按默认值处理。 |
...
| 名称     | 类型     | 只读 | 可选 | 说明                                     |
| -------- | -------- | ---- | ---- | ---------------------------------------- |
| radius   | ArkTS-Dyn: number<br>ArkTS-Sta: double   | 否   | 否   | 模糊半径。<br>单位：px<br>取值范围为[0, +∞)，默认值为0，负数、NaN和Infinity按默认值处理。值越大背景模糊效果越明显，为0时不模糊。 |
| grayscale | ArkTS-Dyn: [number, number]<br>ArkTS-Sta: [int, int] | 否   | 是   | 灰阶模糊参数，两参数取值范围均为[0, 127]，默认值为[0, 0]，超出范围时按默认值处理。对图像中的黑白色进行色阶调整，使其趋于灰色更为柔和美观，对图像中的彩色调整没有效果。参数一表示对黑色的提亮程度，参数二表示对白色的压暗程度，参数值越大调整效果越明显（黑白色变得越灰）。例如：设置参数为（20，20），图片中的黑色像素RGB：[0, 0, 0]会调整为[20, 20, 20]（0+20），白色像素RGB：[255, 255, 255]会调整为[235, 235, 235]（255-20），图像中的彩色像素维持不变。      |
...
| 名称     | 类型     | 只读 | 可选 | 说明                                     |
| -------- | -------- | ---- | ---- | ---------------------------------------- |
| radius   | ArkTS-Dyn: number<br>ArkTS-Sta: double   | 否   | 否   | 模糊半径。<br>单位：px<br>取值范围为[0, +∞)，默认值为0，负数、NaN和Infinity按默认值处理。值越大模糊效果越明显，为0时不模糊。 |
| grayscale | ArkTS-Dyn: [number, number]<br>ArkTS-Sta: [int, int] | 否   | 是   | 灰阶模糊参数，两参数取值范围均为[0, 127]，默认值为[0, 0]，超出范围时按默认值处理。对图像中的黑白色进行色阶调整，使其趋于灰色更为柔和美观，对图像中的彩色调整没有效果。参数一表示对黑色的提亮程度，参数二表示对白色的压暗程度，参数值越大调整效果越明显（黑白色变得越灰）。例如：设置参数为（20，20），图片中的黑色像素RGB：[0, 0, 0]会调整为[20, 20, 20]（0+20），白色像素RGB：[255, 255, 255]会调整为[235, 235, 235]（255-20），图像中的彩色像素维持不变。      |
...
| 名称   | 类型   | 只读 | 可选 | 说明                                |
| ------ | ------ | ---- | ---- | ----------------------------------- |
| radius | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 否   | 模糊半径。<br>单位：px<br>取值范围为[0, +∞)，默认值为0，负数、NaN和Infinity按默认值处理。值越大前景模糊效果越明显，为0时不模糊。 |