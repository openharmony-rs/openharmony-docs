...
>
> 从API版本26.0.0开始，该回调会在NodeAdapter绑定到宿主节点时立即触发，而不是在宿主节点挂载到主节点树时触发。此时宿主节点可能尚未挂载到主节点树。如果回调逻辑依赖节点已挂载（例如访问布局信息或执行动画），建议在该回调中注册[onAppear](./arkui-ts/ts-universal-events-show-hide.md#onappear)，并将相关逻辑放入onAppear中执行。如果通过动态赋值方式设置该回调，请在调用[attachNodeAdapter](#attachnodeadapter12)前完成设置，否则回调可能无法触发。
