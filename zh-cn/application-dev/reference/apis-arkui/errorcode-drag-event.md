...
当开发者在非[onDrop](./arkui-ts/ts-universal-events-drag-drop.md#ondrop)阶段调用仅支持该阶段调用的接口时，会抛出此错误码。
...
当开发者在数据未加载或加载完成后调用[cancelDataLoading](./arkts-apis-uicontext-dragcontroller.md#canceldataloading15)接口时，会抛出此错误码。
