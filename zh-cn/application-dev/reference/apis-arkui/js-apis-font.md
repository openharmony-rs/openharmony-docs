...
| weight         | ArkTS-Dyn: number<br/>ArkTS-Sta: int  | 否 | 否 | 系统字体的字重。<br/>取值范围：[100,900]，取值间隔为100，分别对应[FontWeight](../apis-arkgraphics2d/js-apis-graphics-text.md#fontweight)枚举中的值。<br/>默认值：100        |
| width          | ArkTS-Dyn: number<br/>ArkTS-Sta: int  | 否 | 否 | 系统字体的宽度。<br/>取值范围：[1,9]，取值间隔为1，分别对应[FontWidth](../apis-arkgraphics2d/js-apis-graphics-text.md#fontwidth)枚举中的值。    |
...
| weight        | ArkTS-Dyn: number<br/>ArkTS-Sta: double  | 否 | 否 | 当weight>0时表示此字体集只包含所指定weight的字体，当weight=0时，表示此字体集包含所有字体。<br/>可返回的值有0、100、400、700、900。 |
...
| 名称            | 类型    | 只读 | 可选  | 说明                       |
| -------------- | ------- | ------- | ------------------------- | ------------------------- |
| weight        | ArkTS-Dyn: number<br/>ArkTS-Sta: double  | 否 | 否 | 字体原本的weight值。<br/>可返回的值有50、80、100、200。      |
| to            | ArkTS-Dyn: number<br/>ArkTS-Sta: int  | 否 | 否 | 字体在应用中显示的weight值。<br/>可返回的值有100、400、700、900。 |
