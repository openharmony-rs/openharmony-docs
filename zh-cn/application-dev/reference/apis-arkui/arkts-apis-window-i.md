...
| displayId  | number                     | 否 | 是 | 当前物理屏幕ID。不设置，则默认为-1，该参数应为整数。<br>设置后会对displayId校验，小于0或屏幕ID不存在时，返回401错误码。<br>扩展屏场景下，创建悬浮窗可通过设置屏幕ID显示在指定屏幕上。<br>创建模态窗设置屏幕ID无效，默认跟随父窗口。<br>**系统能力：** SystemCapability.WindowManager.WindowManager.Core     |
