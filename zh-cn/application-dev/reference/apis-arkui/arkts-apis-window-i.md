...
| 名称     | 类型      | 只读 | 可选 | 说明               |
| -------- | -------- | ---- | ---- | ------------------ |
| needFocused | boolean  | 否   | 是   | 表示开始拖拽移动时，窗口是否需要处于获焦状态。默认值为true。<br/>- true：窗口处于获焦状态才能开始拖拽移动。<br/>- false：窗口无需处于获焦状态即可开始拖拽移动。|
| avoidRect | [Rect](arkts-apis-window-i.md#rect7) | 否 | 是 | 表示窗口拖拽移动时的避让区域，以窗口左上角为原点。系统会根据窗口区域与避让区域共同形成的最小外接矩形对窗口位置进行约束。若该最小外接矩形大于对应屏幕的可用区域，则认为避让区域无效，不执行避让修正。否则，系统按以下规则修正窗口位置：<br/>- 单屏拖拽场景下，系统会修正窗口位置，使避让区域完全位于当前屏幕可用区域内；<br/>- 跨屏拖拽场景下，系统会先修正窗口位置以避免窗口显示到非目标屏幕，再修正窗口位置使避让区域完全位于目标屏幕（指针所在屏幕）可用区域内。|
