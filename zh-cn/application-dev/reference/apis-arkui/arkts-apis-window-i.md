...
| isFullScreen                          | boolean                   | 否   | 否   | 是否开启了[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)并且隐藏了状态栏。true表示开启了[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)并且隐藏了状态栏；false表示没有开启[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)或没有隐藏状态栏。<br> **原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br/>**系统能力：** SystemCapability.WindowManager.WindowManager.Core|
| isLayoutFullScreen<sup>7+</sup>       | boolean                   | 否   | 否   | 是否开启了[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)且处于全屏状态。true表示开启了[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)且处于全屏状态；false表示未开启[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)或未处于全屏状态。<br> **原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br/>**系统能力：** SystemCapability.WindowManager.WindowManager.Core|
...
| dimBehindValue<sup>(deprecated)</sup> | number                    | 否   | 否   | 层级靠后窗口的暗度值。该参数为浮点数，取值范围为[0.0, 1.0]，其取1.0表示最暗。<br>- **说明：** 从API version 9开始废弃，当前无可替代接口。<br>- 从API version 7开始支持。<br/>**系统能力：** SystemCapability.WindowManager.WindowManager.Core|
