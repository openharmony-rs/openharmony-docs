...
| 名称   | 类型   | 只读 | 可选 | 说明                                       |
| ------ | ------ | ---- | ---- |------------------------------------------ |
| duration |  number  |  否  |  是  | 截图淡出动画时长，单位为ms。不指定时使用系统默认值：进退沉浸式动画场景默认值为400，其他场景默认值为250。取值范围为[0, 400]，超出取值返回会报错。|
| delay |  number  |  否  |  是  | 截图淡出动画延迟，单位为ms。不指定时使用系统默认值：进退沉浸式动画场景默认值为350，其他场景默认值为50。取值范围为[0, 350]，超出取值返回会报错。|
...
| snapshotAnimationConfig | [WindowSnapshotAnimationConfig](#windowsnapshotanimationconfig)  |  否  |  是  | 截图动画配置。在窗口最大化和窗口恢复[自由窗口](../../windowmanager/window-terminology.md#自由窗口)过程中，系统会通过截图动画遮盖应用布局变化的跳变，可通过此属性指定截图淡出动画延迟和淡出动画时长。不指定时使用系统默认动画。当duration和delay均为0时，表示取消截图动画。|
