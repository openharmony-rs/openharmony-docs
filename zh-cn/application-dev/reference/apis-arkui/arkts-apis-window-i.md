...
| displayId  | number                     | 否 | 是 | 当前屏幕ID。不设置，则默认为-1，该参数应为整数。<br>设置后会对屏幕ID校验，小于0或屏幕ID不存在时，返回401错误码。<br>扩展屏、异源虚拟屏场景下，悬浮窗可通过设置屏幕ID显示在指定屏幕上。<br>模态窗、系统窗设置屏幕ID无效，默认跟随父窗口。<br>**系统能力：** SystemCapability.WindowManager.WindowManager.Core     |
