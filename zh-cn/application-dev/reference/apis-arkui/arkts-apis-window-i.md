...
| windowMinimizedWhenStart<sup>26+</sup> | boolean | 否 | 是 | 窗口是否以最小化状态启动。<br>传入true时，窗口将不会拉起到前台，只会在快捷栏上显示图标。传入false时，表示窗口正常启动。<br>此参数不填时，默认为undefined，窗口正常启动。|
| excludeFromDock<sup>26+</sup> | boolean | 否 | 是 | 是否屏蔽快捷栏图标及Hover缩略图。<br>传入true时，则当前窗口不会在快捷栏上显示图标及Hover缩略图。<br>此参数不填时，默认为undefined，窗口正常启动。<br> **注意**：该参数需要绑定托盘图标才生效。|
| excludeFromRecent<sup>26+</sup> | boolean | 否 | 是 | 是否屏蔽多任务视图。<br>传入true时，则当前窗口不会在多任务视图中显示。传入false时，表示窗口正常启动。<br>此参数不填时，默认为undefined，窗口正常启动。<br> **注意**：该参数需要绑定托盘图标才生效。|
