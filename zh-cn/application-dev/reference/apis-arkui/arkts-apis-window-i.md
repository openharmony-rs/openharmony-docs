...
| displayId | number | 否 | 是 | 窗口所在屏幕的ID。折叠屏半折场景下，返回窗口实际所属屏幕ID。默认返回主屏幕ID，该参数为整数。<br>**起始版本：** 26.0.0 +<br> **模型约束：** 此接口仅可在Stage模型下使用。 |
| globalRect | [Rect](arkts-apis-window-i.md#rect7) | 否 | 是 | 全局坐标系下窗口实际显示区域（位置和尺寸）。默认值：[0, 0, 0, 0]。<br>**起始版本：** 26.0.0 +<br> **模型约束：** 此接口仅可在Stage模型下使用。 |
