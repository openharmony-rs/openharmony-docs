...
| isFullScreen                          | boolean                   | 否   | 否   | 是否开启了[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)并且隐藏了状态栏，默认为false。true表示开启了[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)并且隐藏了状态栏；false表示没有开启[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)或没有隐藏状态栏。<br> **原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br/>**系统能力：** SystemCapability.WindowManager.WindowManager.Core|
| isLayoutFullScreen<sup>7+</sup>       | boolean                   | 否   | 否   | 是否开启了[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)，默认为false。true表示开启了[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)；false表示未开启[沉浸式布局](../../windowmanager/window-terminology.md#沉浸式布局)。<br> **原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br/>**系统能力：** SystemCapability.WindowManager.WindowManager.Core|
| focusable<sup>7+</sup>                | boolean                   | 否   | 否   | 窗口是否可获焦，默认为true。true表示可获焦；false表示不可获焦。 <br/>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。 <br/>**系统能力：** SystemCapability.WindowManager.WindowManager.Core|
...
| dimBehindValue<sup>(deprecated)</sup> | number                    | 否   | 否   | 层级靠后窗口的暗度值。该参数为浮点数，取值范围为[0.0, 1.0]，其取1.0表示最暗，默认返回0.0。<br>- **说明：** 从API version 9开始废弃，当前无可替代接口。<br>- 从API version 7开始支持。<br/>**系统能力：** SystemCapability.WindowManager.WindowManager.Core|
...
| isPrivacyMode<sup>7+</sup>            | boolean                   | 否   | 否   | 窗口是否为隐私模式，默认为false。true表示窗口为隐私模式；false表示窗口为非隐私模式。可通过[setWindowPrivacyMode()](arkts-apis-window-Window.md#setwindowprivacymode9)设置窗口的隐私模式。<br/>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br/>**系统能力：** SystemCapability.WindowManager.WindowManager.Core|
| isRoundCorner<sup>(deprecated)</sup>  | boolean                   | 否   | 否   | 窗口是否为圆角。默认为false。true表示窗口为圆角；false表示窗口为非圆角。<br>- **说明：** 从API version 9开始废弃，当前无可替代接口。<br/>- 从API version 7开始支持。<br/>**系统能力：** SystemCapability.WindowManager.WindowManager.Core|
...
| id<sup>9+</sup>                       | number                    | 否   | 否   | 窗口ID，默认值为0，该参数为整数。<br/>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br/>**系统能力：** SystemCapability.WindowManager.WindowManager.Core|
...
| globalDisplayRect<sup>20+</sup> | [Rect](arkts-apis-window-i.md#rect7)   | 否   | 是   | 全局坐标系下的窗口尺寸。扩展屏场景下以主屏左上角为坐标原点，虚拟屏场景下以所在屏幕组中的主屏左上角为坐标原点。默认值：[0, 0, 0, 0]。<br/>**系统能力：**  SystemCapability.Window.SessionManager |
