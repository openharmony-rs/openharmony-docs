...
| TYPE_NAVIGATION_INDICATOR<sup>11+</sup> | 4    | 表示底部导航区域<!--Del-->或工具栏区域。当前工具栏仅Car设备支持<!--DelEnd-->。当三键导航显示时，底部导航避让区域始终存在。 <br> **系统能力：** SystemCapability.WindowManager.WindowManager.Core <br> **原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。<!--Del-->*设备行为差异：** 在OpenHarmony 6.0.0之前，此接口在Car设备上调用不生效也不报错。从OpenHarmony 6.0.0开始，此接口在Car设备上可正常调用并生效。<!--DelEnd--> |
