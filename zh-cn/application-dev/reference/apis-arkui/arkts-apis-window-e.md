...
折叠屏的跨屏策略枚举，用于控制折叠2in1设备在半折叠状态下主窗口最大化时的瀑布流模式行为。
...
| 名称       | 值   | 说明                          |
| ---------- | ---- | ----------------------------- |
| FOLLOW_ACROSS_DISPLAY_SETTING  | 0    | 跟随当前跨屏策略设置。若未设置过跨屏策略，则使用系统默认策略：设备半折叠状态下，窗口进入单屏最大化（即窗口最大化时只在上半屏或下半屏显示）；展开态下，窗口最大化并在折回半折叠态时保持瀑布流模式（即窗口跨上下两半屏显示）。     |
| ENTER_ACROSS_DISPLAY_MODE | 1    | 设备半折叠状态下，窗口直接进入瀑布流模式；展开态下，窗口最大化并在折回半折叠态时保持瀑布流模式。             |
| EXIT_ACROSS_DISPLAY_MODE    | 2    | 设备半折叠状态下，窗口退出瀑布流模式，进入单屏最大化；展开态下，窗口最大化并在折回半折叠态时退出瀑布流模式。   |
