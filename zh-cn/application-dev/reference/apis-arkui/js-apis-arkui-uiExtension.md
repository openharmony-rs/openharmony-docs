...
**相关接口：** 该接口对应的ArkTS-Dyn接口是[on('rectChange')](#onrectchange14)。
...
| 参数名   | 类型                           | 必填 | 说明                                                     |
| -------- | ------------------------------ | ---- | -------------------------------------------------------- |
| reasons  | int                         | 是   | 触发组件（EmbeddedComponent或UIExtensionComponent）位置及尺寸变化的原因，具体取值可参考[RectChangeReason](#rectchangereason14)枚举值。 |
| callback | [Callback](../apis-basic-services-kit/js-apis-base.md#callback)<[RectChangeOptions](#rectchangeoptions14)> | 是 | 回调函数。返回当前组件（EmbeddedComponent或UIExtensionComponent）矩形变化值及变化原因。 |
...
**相关接口：** 该接口对应的ArkTS-Dyn接口是[off('rectChange')](#offrectchange14)。
...
ArkTS-Dyn: occupyEvents(eventFlags: number): Promise&lt;void&gt;

ArkTS-Sta: occupyEvents(eventFlags: int): Promise&lt;void&gt;
...
| 参数名 | 类型    | 必填 | 说明           |
| ------ | ------ | ---- | -------------- |
| eventFlags | ArkTS-Dyn: number<br>ArkTS-Sta: int | 是 | 占用的事件类型，具体取值可见[EventFlag](#eventflag18)枚举值。 |
...
struct Extension {
  ...
  aboutToAppear(): void {
    ...
    this.extensionWindow?.onRectChange(uiExtension.RectChangeReason.HOST_WINDOW_RECT_CHANGE,
      (data: uiExtension.RectChangeOptions) => {
        console.info('Succeeded window rect changes. Data: ' + JSON.stringify(data));
      });
