...
| gradient    | boolean                                  | false    | 否    | 仅线形图支持设置是否显示填充渐变颜色。<br/>默认值：false，不显示填充渐变颜色。    |
...
| display  | boolean       | false    | 否    | 是否显示轴。<br/>默认值：false，不显示轴。                                   |
...
| font-family<sup>5+</sup> | Array          | -                          | 否    | 表示绘制注释的字体族，支持自定义字体。 |
