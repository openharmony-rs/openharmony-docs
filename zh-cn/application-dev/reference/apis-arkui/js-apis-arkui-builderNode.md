...
| 名称          | 类型                                   | 只读 | 可选 | 说明                                                         |
| ------------- | -------------------------------------- | ---- | ---- | ------------------------------------------------------------ |
| selfIdealSize | [Size](js-apis-arkui-graphics.md#size) | 否   | 是 | 节点的理想大小。当将BuilderNode生成的内容嵌入到其它RenderNode中显示时，需要显式指定selfIdealSize，否则Builder内的节点默认父组件布局约束为[0, 0]。<br>默认值：{ width: 0, height: 0 } <br/>**ArkTS-Dyn起始版本：** 11<br/>**ArkTS-Sta起始版本：** 23 |
| type          | [NodeRenderType](#noderendertype)      | 否   | 是 | 节点的渲染类型。当取值为NodeRenderType.RENDER_TYPE_TEXTURE时，仅在BuilderNode持有组件树的根节点为自定义组件时设置生效。<br>默认值：NodeRenderType.RENDER_TYPE_DISPLAY <br/>**ArkTS-Dyn起始版本：** 11<br/>**ArkTS-Sta起始版本：** 23 |
...
| 名称          | 类型               | 只读 | 可选 | 说明                                                         |
| ------------- | ----------------- | ---- | ---- | ------------------------------------------------------------ |
| nestingBuilderSupported | boolean | 否   | 是   | 是否支持Builder嵌套Builder使用。其中，true表示支持，false表示不支持。<br>默认值：false <br>**模型约束：** 此接口仅可在Stage模型下使用。<br>**原子化服务API（仅ArkTS-Dyn）：** 从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS模式：** 该参数仅适用于ArkTS-Dyn。 |
