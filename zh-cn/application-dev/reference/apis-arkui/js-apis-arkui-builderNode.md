...
| 名称          | 类型                                   | 只读 | 可选 | 说明                                                         |
| ------------- | -------------------------------------- | ---- | ---- | ------------------------------------------------------------ |
| selfIdealSize | [Size](js-apis-arkui-graphics.md#size) | 否   | 是 | 节点的理想大小。<br/>默认值：{ width: 0, height: 0 } <br/>**ArkTS-Dyn起始版本：** 11<br/>**ArkTS-Sta起始版本：** 23 |
| type          | [NodeRenderType](#noderendertype)      | 否   | 是 | 节点的渲染类型。<br/>默认值：NodeRenderType.RENDER_TYPE_DISPLAY <br/>**ArkTS-Dyn起始版本：** 11<br/>**ArkTS-Sta起始版本：** 23 |
| surfaceId     | string                                 | 否   | 是 | 纹理接收方的surfaceId。纹理接收方一般为[OH_NativeImage](../apis-arkgraphics2d/capi-oh-nativeimage-oh-nativeimage.md)。<br/>surfaceId仅当type为NodeRenderType.RENDER_TYPE_TEXTURE时生效。<br/>默认值："" <br/>**ArkTS-Dyn起始版本：** 11<br/>**ArkTS-Sta起始版本：** 23 |
| enableMinimized     | boolean                                 | 否   | 是 | 控制BuilderNode持有的FrameNode的类型，当此开关设置为true时，BuilderNode持有的FrameNode为轻量化的FrameNode，内存更小，但是不支持FrameNode的部分接口，具体信息请参见[isMinimized](./js-apis-arkui-frameNode.md#isminimized)。<br/>默认值：false <br/>**ArkTS模式：** 该接口仅适用于ArkTS-Sta。<br/>**ArkTS-Sta起始版本：** 26.0.0 |
...
ArkTS-Dyn: class BuilderNode\<Args extends Object[]>

ArkTS-Sta: class BuilderNode\<T = undefined>
...
ArkTS-Dyn: build(builder: WrappedBuilder\<Args>, arg?: Object): void

ArkTS-Sta: build(builder: WrappedBuilder\<CustomBuilder> | WrappedBuilder\<CustomBuilderT\<T>>, arg?: T): void
...
| 参数名  | 类型                                                            | 必填 | 说明                                                                                   |
| ------- | --------------------------------------------------------------- | ---- | -------------------------------------------------------------------------------------- |
| builder | ArkTS-Dyn: [WrappedBuilder](../../ui/state-management/arkts-wrapBuilder.md)\<Args><br/>ArkTS-Sta: [WrappedBuilder](../../ui/state-management/arkts-wrapBuilder.md)\<[CustomBuilder](./arkui-ts/ts-types.md#custombuilder8)> 或 [WrappedBuilder](../../ui/state-management/arkts-wrapBuilder.md)\<[CustomBuilderT](./arkui-ts/ts-types.md#custombuildertt23)\<T>> | 是   | 创建对应节点树的时候所需的无状态UI方法[@Builder](../../ui/state-management/arkts-builder.md)。 |
| arg     | ArkTS-Dyn: Object<br/>ArkTS-Sta: T                              | 否   | builder有入参时当前仅支持一个入参，且入参对象类型与@Builder定义的入参类型保持一致。ArkTS-Sta下，当builder类型为WrappedBuilder\<CustomBuilder>时无需传入该参数。<br/>默认值：undefined |

### build<sup>12+</sup>

ArkTS-Dyn: build(builder: WrappedBuilder\<Args>, arg: Object, options: BuildOptions): void

ArkTS-Sta: build(builder: WrappedBuilder\<CustomBuilderT\<T>>, arg: T, options: BuildOptions): void

依照传入的对象创建组件树，并持有组件树的根节点。无状态的UI方法[@Builder](../../ui/state-management/arkts-builder.md)最多拥有一个根节点。

支持自定义组件。相比[build](#build)接口，本接口支持builder的配置参数。ArkTS-Dyn下可用于判断是否支持@Builder中嵌套@Builder，ArkTS-Sta下可用于配置是否开启并行构建。
...
| 参数名  | 类型                                                            | 必填 | 说明                                                                                    |
| ------- | --------------------------------------------------------------- | ---- | -------------------------------------------------------------------------------------- |
| builder | ArkTS-Dyn: [WrappedBuilder](../../ui/state-management/arkts-wrapBuilder.md)\<Args><br/>ArkTS-Sta: [WrappedBuilder](../../ui/state-management/arkts-wrapBuilder.md)\<[CustomBuilderT](./arkui-ts/ts-types.md#custombuildertt23)\<T>> | 是   | 创建对应节点树的时候所需的无状态UI方法[@Builder](../../ui/state-management/arkts-builder.md)。   |
| arg     | ArkTS-Dyn: Object<br/>ArkTS-Sta: T                              | 是   | builder的入参。当前仅支持一个入参，且入参对象类型与@Builder定义的入参类型保持一致。                                                            |
| options | [BuildOptions](#buildoptions12)                                 | 是   | builder的配置参数。ArkTS-Dyn下用于判断是否支持@Builder中嵌套@Builder，ArkTS-Sta下用于配置是否开启并行构建。                                         |
...
ArkTS-Dyn: update(arg: Object): void

ArkTS-Sta: update(arg: T): void
...
| 参数名 | 类型   | 必填 | 说明                                                                     |
| ------ | ------ | ---- | ------------------------------------------------------------------------ |
| arg    | ArkTS-Dyn: Object<br/>ArkTS-Sta: T | 是   | 用于更新BuilderNode的参数，和[build](#build)调用时传入的参数类型一致。 |
...
| 参数名 | 类型   | 必填 | 说明                                                                     |
| ------ | ------ | ---- | ------------------------------------------------------------------------ |
| param  | ArkTS-Dyn: Object<br/>ArkTS-Sta: RecordData | 否   | 用于复用BuilderNode的参数。该参数将直接用于BuilderNode中所有顶层自定义组件的复用，应该包含每个自定义组件的构造函数参数所需内容，否则，会导致未定义行为。调用此方法将同步触发内部自定义组件的[aboutToReuse](../../reference//apis-arkui/arkui-ts/ts-custom-component-lifecycle.md#abouttoreuse10)生命周期回调，并将该参数作为回调的入参。默认值为undefined，此时BuilderNode中的自定义组件将直接使用构造时的数据源。 |
...
>
> - 系统在处理鼠标左键点击事件时将转换为触摸事件，转发时应注意不在外层同时绑定触摸事件与鼠标事件，否则可能导致坐标偏移。这是由于在事件转换过程中，[TouchType](arkui-ts/ts-appendix-enums.md#touchtype)不会发生变化，规格可查看[onTouch](arkui-ts/ts-universal-events-touch.md#ontouch)。
>
...
| competitionStrategy  | [CompetitionStrategy](arkui-ts/ts-appendix-enums.md#competitionstrategy24) | 否   | 分发事件的手势是否为竞争场景，默认为非竞争。 |
...
依照传入的[CustomBuilder](./arkui-ts/ts-types.md#custombuilder8)创建组件树，并持有组件树的根节点。支持响应式状态管理。
...
| 参数名  | 类型                                                            | 必填 | 说明                                                                                    |
| ------- | --------------------------------------------------------------- | ---- | -------------------------------------------------------------------------------------- |
| builder | [CustomBuilder](./arkui-ts/ts-types.md#custombuilder8)         | 是   | 创建对应节点树的时候所需的无状态UI方法[@Builder](../../ui/state-management/arkts-builder.md)。   |
| options | [BuildOptions](#buildoptions12)                                   | 否   | build相关的配置项。<br/>默认值：参考[BuildOptions](./js-apis-arkui-builderNode.md#buildoptions12)各个成员的默认值。   |
...
> 传入的坐标值需要转换为px，坐标转换示例可以参考下面示例代码。
>
> 鼠标左键点击事件将转换为触摸事件，转发时应注意不在外层且绑定触摸事件与鼠标事件，否则可能导致坐标偏移。这是由于在事件转换过程中，事件的[SourceType](arkui-ts/ts-gesture-settings.md#sourcetype枚举说明8)不会发生变化，规格可查看[onTouch](arkui-ts/ts-universal-events-touch.md#ontouch)。
>
> 注入事件为轴事件[（AxisEvent）](arkui-ts/ts-universal-events-axis.md#axisevent)时，由于轴事件中缺少旋转轴信息，因此注入的事件无法触发[RotationGesture](arkui-ts/ts-basic-gestures-rotationgesture.md)。
>
