...
| 名称 | 说明 |
| --- | --- |
| flushCachedGnssLocations | 读取并清空GNSS芯片所有缓存位置。使用callback异步回调。 |
...
| getCachedGnssLocationsSize | 获取GNSS芯片缓存位置的个数。使用callback异步回调。 |
...
| isLocationEnabled | 判断位置服务是否已经打开，使用callback异步回调。 |
...
| requestEnableLocation | 请求打开位置服务，使用callback异步回调。 |
