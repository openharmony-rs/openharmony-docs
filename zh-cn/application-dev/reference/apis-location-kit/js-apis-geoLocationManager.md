...
获取当前位置附近的POI信息。使用Promise异步回调。当前该接口仅支持中国境内（香港特别行政区、澳门特别行政区、中国台湾除外）使用。
