...
函数指针类型，在调用OH_JSVM_CreateArrayBufferFromExternalMemory接口时，可传入该类型的函数调用。回调函数将会在关联的ArrayBuffer对象被回收时被调用，用以执行native的清理动作。使用`JSVM_FinalizeArrayBuffer`请遵循以下规则：
...
- 根据`copied`参数决定内存的释放策略，详见使用JSVM-API接口从外部内存创建ArrayBuffer。
