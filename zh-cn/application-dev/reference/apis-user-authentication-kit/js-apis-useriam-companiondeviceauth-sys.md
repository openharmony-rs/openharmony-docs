...
| deviceUserId | number | 否 | 否 | 设备用户ID。设备上的用户标识，为非负整数，用于区分设备上的不同用户。 |
...
| localUserId | number | 否 | 否 | 本地用户ID。主设备上与该模板关联的用户标识，为非负整数。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| ------------- | ---- | ---- | ------------------------------------------------------------ |
| selectPurpose | number | 是 | 选择目的。用于标识当前设备选择的意图，取值参见[SelectPurpose](#selectpurpose)。SELECT_ADD_DEVICE(1)表示选择添加模板的设备，SELECT_AUTH_DEVICE(2)表示选择认证设备。厂商可自定义扩展值（大于等于10000）。应用应根据selectPurpose返回包含对应设备信息的DeviceSelectResult。 |
