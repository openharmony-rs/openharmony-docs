...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| authType | \_\_\_MD\_LINK\_USD\_0\_\_\_ | 是 | 认证类型。用于指定查询的凭据类型，支持FACE（人脸）、FINGERPRINT（指纹）、PIN（密码）、COMPANION\_\_\_ESCAPED\_UNDERSCORE\_\_\_DEVICE（伴随设备）。查询PIN时返回的是密码的整体状态，而非单个密码的数量。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| \_\_\_MD\_LINK\_USD\_0\_\_\_ | 当查询成功时，返回值为用户注册凭据的状态。包含credentialDigest（凭据摘要）和credentialCount（凭据数量）。应用可保存 |
