...
取消认证。 > **说明：** > > 使用获取到的[AuthInstance]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_对象调用该接口进行取消认证，此[AuthInstance]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_需要是正 > 在进行认证的对象。
...
取消订阅特定类型的认证事件。 - **name**: 表示认证事件类型，取值为"result"时，取消订阅认证结果；取值为"tip"时，取消订阅认证过程中的提示信息，类型为 [AuthEventKey]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_。 > **说明：** > > 需要使用已经成功订阅事件的[AuthInstance]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_对象调用该接口进行取消订阅。
...
订阅指定类型的用户认证事件。 - **name**: 表示认证事件类型，取值为"result"时，回调函数返回认证结果；取值为"tip"时，回调函数返回认证过程中的提示信息，类型为 [AuthEventKey]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_。 - **callback**: 认证接口的回调函数，用于返回认证结果或认证过程中的提示信息，类型为[AuthEvent]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_。 > **说明：** > > 使用获取到的[AuthInstance]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_对象调用该接口进行订阅。
...
开始认证。 > **说明：** > > 使用获取到的[AuthInstance]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_对象调用该接口进行认证。
