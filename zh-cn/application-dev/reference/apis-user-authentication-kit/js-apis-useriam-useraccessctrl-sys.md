...
| userId | ArkTS-Dyn: number<br>ArkTS-Sta: int | 否 | 否  | 用户ID。表示完成认证的用户标识，为大于等于0的正整数。|
...
| allowableDuration  | ArkTS-Dyn: number<br>ArkTS-Sta: int  | 是   | 允许的认证有效时长。从AuthToken签发起允许使用的最大时间间隔，单位为毫秒。值需大于0且小于等于86400000（24小时）。用于校验令牌的时效性，防止过期令牌被使用。 |
