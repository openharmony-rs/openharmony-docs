...
**使用场景**：多设备协同场景下，需要暂时停止分布式硬件的同步（如暂停分布式相机或麦克风的连接） ；分布式硬件资源管理，需要在特定情况下释放或恢复分布式硬件资源 。

**功能特点**：支持多种分布式硬件类型的控制（相机、屏幕、麦克风、扬声器等），支持基于源端设备的精细化控制，提供异步操作接口，不影响主流程 。
...
使用场景：当需要临时禁用分布式硬件以节省设备资源时；当需要暂停分布式相机、屏幕等硬件数据传输时。
