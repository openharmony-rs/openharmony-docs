...
| pickerColorMode       | [PickerColorMode](arkts-apis-photoAccessHelper-e.md#pickercolormode)  | 否   | 是 | PhotoPicker的颜色模式。PhotoPicker内所有内容的深色/浅色模式。默认值为Auto，跟随系统的深色/浅色模式。 <br>**模型约束：** 此接口仅可在Stage模型下使用。<br>**原子化服务API：** 从API版本26.1.0开始，该接口支持在原子化服务中使用。<br>**起始版本：** 26.1.0  |
