...
| sortRule    | string   | 否   | 否 | 用户上次选择图片的宫格界面的排序规则，默认为空字符串。<br>**原子化服务API：** 从API version 24开始，该接口支持在原子化服务中使用。 |
| fileSize    | number   | 否   | 否 | 用户上次选择图片的宫格界面，左上角首张图片的文件大小，默认为0。<br>**原子化服务API：** 从API version 24开始，该接口支持在原子化服务中使用。|
