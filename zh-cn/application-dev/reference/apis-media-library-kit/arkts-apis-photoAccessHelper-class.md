...
| isMovingPhotoBadgeShown<sup>22+</sup> | boolean  | 否   | 是 | 是否在大图浏览模式下展示动态照片图标，true表示展示，false表示不展示，默认为false。<br>若设置为true，[Photoselectresult](#photoselectresult)返回movingPhotoBadgeStates数组，动态照片默认返回状态为MOVING_PHOTO_ENABLED。<br>**注意：** 建议isMovingPhotoBadgeShown与MovingPhotoBadgeStateType一起使用，不建议单独使用MovingPhotoBadgeStateType判断是否是动图。<br>**原子化服务API：** 从API version 22开始，该接口支持在原子化服务中使用。 |
