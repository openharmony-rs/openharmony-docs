...
| 名称                    | 类型                | 只读 | 可选 | 说明                          |
| ----------------------- | ------------------- | ---- | ---- | -------------------------------- |
| isEditSupported<sup>11+</sup>       | boolean | 否   | 是 | 是否支持编辑照片，true表示支持，false表示不支持，默认为true。<br>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。<br>**设备行为差异：**  从API version 26.0.1开始，select方法支持car设备，但是isEditSupported属性不支持car设备，如果在car设备上配置则不生效。     |
