...
| defaultRecommendationType<sup>24+</sup> | [RecommendationType](arkts-apis-photoAccessHelper-e.md#recommendationtype11)   | 否   | 是 | 需要配置recommendationTypeList后，该配置才生效。表示打开Picker直接显示的推荐标签。如果该标签存在，则默认显示该标签页；如果该标签不存在，则默认显示“全部”标签页。<br>**模型约束：** 此接口仅可在Stage模型下使用。<br>**系统接口**：此接口为系统接口。|
