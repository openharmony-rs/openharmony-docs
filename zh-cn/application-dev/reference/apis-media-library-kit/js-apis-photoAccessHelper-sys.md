...
| filter | string | 否 | 过滤选项，要求Json字符串。<br>目前只支持传递currentFileId，表示当前精选人像卡片展示图片的file_id。比如:'{"currentFileId":"123"}'。<br>不填时默认从头返回资产。<br>填写则根据currentFileId，内部计算评分，返回小于等于该评分的资产。 |
