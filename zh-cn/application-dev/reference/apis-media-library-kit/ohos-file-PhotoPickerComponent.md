...
| isSlidingSelectionSupported<sup>13+</sup>     | boolean                                 | 否  | 是 | 是否支持滑动多选，true表示支持，false表示不支持。当不设置该参数时，默认值为false。重复选择场景不支持滑动多选。<br>**原子化服务API：** 从API version 13开始，该接口支持在原子化服务中使用。<br>**设备行为差异：**  从API version 26.0.1开始，select方法支持car设备，但是isSlidingSelectionSupported属性不支持car设备，如果在car设备上配置则不生效。                                           |
