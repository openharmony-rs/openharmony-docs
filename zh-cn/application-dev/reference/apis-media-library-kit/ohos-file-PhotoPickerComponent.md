...
| duration   | number                | 否 | 是   | 视频的持续时间（单位：毫秒）<br>**注意**：在API version 24之前动图的duration返回0，异常情况返回-1；从API version 24开始，新动图的duration返回动图对应的视频时长，未处理的旧动图返回0，异常场景仍然返回-1。<br>当[ItemType](#itemtype)为THUMBNAIL时支持，否则为空。<br>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。 |
