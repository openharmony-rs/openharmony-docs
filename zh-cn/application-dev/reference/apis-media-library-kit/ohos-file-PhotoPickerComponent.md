...
| duration   | number                | 否 | 是   | 视频的持续时间，单位为毫秒（ms）。在API version 23之前，动态照片的duration将返回0；在API version 23及之后，返回动态照片附带视频片段的时长，异常场景返回-1。<br>当[ItemType](#itemtype)为THUMBNAIL时支持，否则为空。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。 |
