...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| ---- | ---- | ---- | ---- | ---- |
| offset | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否 | 是 | 3D模型数据在资源文件中的起始偏移量，单位为字节。系统将从资源文件的该偏移位置定位并读取glb模型数据。例如，当glb模型嵌在MP4容器文件中时，可将此参数设置为glb数据在MP4文件中的起始字节位置，使系统能够正确提取并加载模型。取值必须大于或等于0。默认值为0，表示模型数据从文件起始位置开始。 |
