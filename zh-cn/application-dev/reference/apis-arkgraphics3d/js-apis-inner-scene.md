...
| shadowSampleCount | number | 否 | 是 | 采样数量，决定了每个像素采样阴影图的次数，数量越多，阴影质量越高，但性能开销越大。默认值16<br>取值范围：0 ~ 64<br>- 超出此范围的值会被自动限制到最近的有效边界值（例如 65 实际按 64 处理）<br>- 设置为0时，将不进行PCF采样，无阴影效果。<br>- 设置为undefined时，恢复默认值16进行渲染。 |
