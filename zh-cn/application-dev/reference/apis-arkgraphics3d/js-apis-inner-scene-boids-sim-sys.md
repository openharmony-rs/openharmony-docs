...
| gravityWeight | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否 | 是 | 引力场权重。引力场对该个体的吸引强度。取值 >= 0。默认值为0.0。 |
| repulsionWeight | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否 | 是 | 斥力场权重。斥力场对该个体的排斥强度。取值 >= 0。默认值为0.0。 |
