...
ArkTS-Dyn: createMessage\(pdu: Array&lt;number&gt;, specification: string, callback: AsyncCallback\<ShortMessage\>\): void

ArkTS-Sta: createMessage\(pdu: Array&lt;int&gt;, specification: string, callback: AsyncCallback\<ShortMessage\>\): void
...
const specification: string = '3gpp';
const pdu: Array<int> = [0x01, 0x00, 0x05, 0x81, 0x01, 0x80, 0xF6, 0x00, 0x00, 0x05, 0xE8, 0x32, 0x9B, 0xFD, 0x06];
sms.createMessage(pdu, specification, (err: BusinessError | null, data: sms.ShortMessage | undefined) => {
    if (err?.code) {
        console.error(`sms.createMessage callback test failed, err->${JSON.stringify(err)}`);
    } else {
        console.info(`createMessage callback test success, data->${JSON.stringify(data)}`);
    }
});
```
...
ArkTS-Dyn: createMessage\(pdu: Array&lt;number&gt;, specification: string\): Promise\<ShortMessage\>

ArkTS-Sta: createMessage\(pdu: Array&lt;int&gt;, specification: string\): Promise\<ShortMessage\>
...
try {
    ...
    let data: sms.ShortMessage = await sms.createMessage(pdu, specification);
    console.info(`sms.createMessage promise success, data is:  ->${JSON.stringify(data)}`);
} catch (error: Error) {
...
let sendCallback: AsyncCallback<sms.ISendShortMessageCallback> = (err: BusinessError|null, data: sms.ISendShortMessageCallback|undefined) => {
    console.info(`sendCallback: err->${JSON.stringify(err)}, data->${JSON.stringify(data)}`);
};

let deliveryCallback: AsyncCallback<sms.IDeliveryShortMessageCallback> = (err: BusinessError|null, data: sms.IDeliveryShortMessageCallback|undefined) => {
    console.info(`deliveryCallback: err->${JSON.stringify(err)}, data->${JSON.stringify(data)}`);
};
let contentMms: string = '短信内容';
let options: sms.SendMessageOptions = {
    slotId: 0,
    content: contentMms, 
    destinationHost: '+861xxxxxxxxxx',
...
sms.sendShortMessage(options, (err: BusinessError | null, data: undefined) => {
    if (err?.code) {
        console.error(`sms.sendShortMessage callback test failed err->${JSON.stringify(err)}`);
    } else {
        console.info(`sms.sendShortMessage callback test success`);
    }
});
```
...
try {
    ...
    let options: sms.SendMessageOptions = {
        slotId: 0,
        content: contentMms,
        destinationHost: '+8619986929014',
        serviceCenter: '+8619986929014',
        destinationPort: 1000,
        sendCallback: sendCallback,
        deliveryCallback: deliveryCallback,
    };
    await sms.sendShortMessage(options);
    console.info(`sms.sendShortMessage promise success`);
} catch (error: Error) {
    let err = error as BusinessError;
    console.error(`sms.sendShortMessage promise err->${JSON.stringify(err)}`);
}
...
ArkTS-Dyn: getDefaultSmsSlotId\(callback: AsyncCallback&lt;number&gt;\): void

ArkTS-Sta: getDefaultSmsSlotId\(callback: AsyncCallback&lt;int&gt;\): void
...
| 参数名   | 类型                        | 必填 | 说明                                     |
| -------- | --------------------------- | ---- | ---------------------------------------- |
| callback | ArkTS-Dyn: AsyncCallback&lt;number&gt;<br/>ArkTS-Sta: AsyncCallback&lt;int&gt; | 是   | 获取发送短信的默认SIM卡槽ID的回调函数。<br/>- 0：卡槽1。<br/>- 1：卡槽2。 |
...
sms.getDefaultSmsSlotId((err: BusinessError | null, data: int | undefined) => {
    if (err?.code) {
        console.error(`sms.getDefaultSmsSlotId callback test failed err->${JSON.stringify(err)}`);
    } else {
        console.info(`sms.getDefaultSmsSlotId promise success, data is:  ->${JSON.stringify(data)}`);
    }
});
```
...
ArkTS-Dyn: getDefaultSmsSlotId\(\): Promise&lt;number&gt;

ArkTS-Sta: getDefaultSmsSlotId\(\): Promise&lt;int&gt;
...
| 类型            | 说明                                                         |
| --------------- | ------------------------------------------------------------ |
| ArkTS-Dyn: Promise&lt;number&gt;<br/>ArkTS-Sta: Promise&lt;int&gt; | 以Promise形式返回发送短信的默认SIM卡：<br/>- 0：卡槽1。<br/>- 1：卡槽2。 |
...
try {
    let data: int = await sms.getDefaultSmsSlotId();
    console.info(`sms.getDefaultSmsSlotId callback test success, data->${JSON.stringify(data)}`);
} catch (error: Error) {
    let err = error as BusinessError;
    console.error(`sms.getDefaultSmsSlotId promise err->${JSON.stringify(err)}`);
}
```
...
let data: boolean = sms.hasSmsCapability(); 
console.info(`hasSmsCapability: ${JSON.stringify(data)}`);
```

## sms.getDefaultSmsSimId<sup>10+</sup>

ArkTS-Dyn: getDefaultSmsSimId\(callback: AsyncCallback&lt;number&gt;\): void

ArkTS-Sta: getDefaultSmsSimId\(callback: AsyncCallback&lt;int&gt;\): void
...
| 参数名   | 类型                        | 必填 | 说明                                     |
| -------- | --------------------------- | ---- | ---------------------------------------- |
| callback | ArkTS-Dyn: AsyncCallback&lt;number&gt;<br/>ArkTS-Sta: AsyncCallback&lt;int&gt;  | 是   | 获取默认短信SIM的SIM ID的回调函数。<br/>与SIM卡绑定，从1开始递增。<br/>无卡时返回值为-1。 |
...
sms.getDefaultSmsSimId((err: BusinessError | null, data: int | undefined) => {
    if (err?.code) {
        console.error(`sms.getDefaultSmsSimId callback test failed err->${JSON.stringify(err)}`);       
    } else {
        console.info(`sms.getDefaultSmsSimId callback test success, data->${JSON.stringify(data)}`);
    }
});
```
...
ArkTS-Dyn: getDefaultSmsSimId\(\): Promise&lt;number&gt;

ArkTS-Sta: getDefaultSmsSimId\(\): Promise&lt;int&gt;
...
try {
    let data: int = await sms.getDefaultSmsSimId();
    console.info(`sms.getDefaultSmsSimId promise test success, data->${JSON.stringify(data)}`);
} catch (error: Error) {
    let err = error as BusinessError;
    console.error(`sms.getDefaultSmsSimId promise ERROR, err->${JSON.stringify(err)}`);
}
```
...
| pdu                      | ArkTS-Dyn: Array&lt;number&gt; <br/>ArkTS-Sta: Array&lt;int&gt;                      |  否  |  否  | SMS消息中的协议数据单元 (PDU)。                            |
| protocolId               | ArkTS-Dyn: number <br/>ArkTS-Sta: int                                  |  否  |  否  | 发送短信时使用的协议标识。                                   |
| scAddress                | string                                  |  否  |  否  | 短消息服务中心(SMSC)地址。                                 |
| scTimestamp              | ArkTS-Dyn: number <br/>ArkTS-Sta: long                                   |  否  |  否  | SMSC时间戳。                                                 |
| status                   | ArkTS-Dyn: number <br/>ArkTS-Sta: int                                   |  否  |  否  | SMS-STATUS-REPORT消息中的短信状态指示短信服务中心(SMSC)发送的短信状态。 |
...
|       名称       | 类型                                                         | 只读 | 可选 | 说明                                                         |
| ---------------- | ------------------------------------------------------------ | ---- | ---- | ------------------------------------------------------------ |
| slotId           | ArkTS-Dyn: number <br/>ArkTS-Sta: int                          |  否  |  否  | 用于发送短信的SIM卡槽ID：<br/>- 0：卡槽1。<br/>- 1：卡槽2。      |
| destinationHost  | string                                                       |  否  |  否  | 短信的发送地址。                                             |
| content          | string \| Array&lt;int&gt;                                   |  否  |  否  | 如果内容是字符串，则这是一条文本短信。如果内容是字节数组，则这是一条数据短信。 |
| serviceCenter    | string                                                       |  否  |  是  | 短信中心地址。默认使用SIM卡中的短信中心地址。                |
| destinationPort  | ArkTS-Dyn: number <br/>ArkTS-Sta: int                          |  否  |  是  | 如果发送数据消息，destinationPort 是必需的。否则是可选的。   |
...
| 名称 | 类型                | 只读 | 可选 | 说明           |
| ---- | ------------------- | ---- | ---- | -------------- |
| pdu  | ArkTS-Dyn: Array&lt;number&gt; <br/>ArkTS-Sta: Array&lt;int&gt;  | 否   | 否   | 短信送达报告。 |
