...
| dial | 拨打电话，可设置通话参数。使用callback异步回调。 |
...
| formatPhoneNumber | 格式化电话号码，可设置格式化参数。使用callback异步回调。 |
...
| formatPhoneNumberToE164 | 将电话号码格式化为E.164表示形式，使用callback异步回调。 |
...
| getCallState | 获取当前通话状态。使用callback异步回调。 |
...
| hasCall | 判断是否存在通话。使用callback异步回调。 |
...
| isEmergencyPhoneNumber | 根据电话号码参数，判断是否是紧急电话号码。使用callback异步回调。 |
...
| makeCall | 跳转到拨号界面，并显示待拨出的号码。使用callback异步回调。只支持在UIAbility中调用。 |
