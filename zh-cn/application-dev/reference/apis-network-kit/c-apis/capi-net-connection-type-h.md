...
| [NetConn_NetConnCallback](capi-netconnection-netconn-netconncallback.md) | NetConn_NetConnCallback | 网络状态监听回调集合，所有回调事件需全部注册，无需关注的回调可以设为空实现。 |
...
| [typedef void (\*OH_NetConn_GlobalHttpProxyRefreshCallback)(int32_t result, const NetConn_HttpProxy *proxy, void *userContext)](#oh_netconn_globalhttpproxyrefreshcallback) | OH_NetConn_GlobalHttpProxyRefreshCallback | 定义用于接收全局HTTP代理重新认证结果的一次性回调。每次成功调用OH_NetConn_RefreshGlobalHttpProxyWithCallback时，该回调最多会被调用一次。 |
...
定义用于接收全局HTTP代理重新认证结果的一次性回调。每次成功调用OH_NetConn_RefreshGlobalHttpProxyWithCallback时，该回调最多会被调用一次。
