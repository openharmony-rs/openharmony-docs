...
| uint32_t priority | 优先级（数值越小优先级越高，规则同报文过滤器）。<br>**起始版本：** 26.0.0 |
| [OH_TrafficFilter_HookPoint](capi-net-trafficfilter-type-h.md#oh_trafficfilter_hookpoint) hookPoint | 钩子点（仅支持PREROUTING和OUTPUT）。<br>**起始版本：** 26.0.0 |
...
| uint32_t uidStart | 应用UID范围起始值（UINT32_MAX表示任意）。<br>**起始版本：** 26.0.0 |
| uint32_t uidEnd | 应用UID范围结束值（UINT32_MAX表示任意）。<br>**起始版本：** 26.0.0 |
