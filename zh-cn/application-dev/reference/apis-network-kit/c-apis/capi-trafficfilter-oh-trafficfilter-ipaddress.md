...
| uint8_t addr[OH_TRAFFICFILTER_IP_ADDRLEN] | IP地址字节。<br> 字节必须以网络字节序存储。 对于IPv4，{@link addr}[0]到{@link addr}[3]存储IPv4地址，<br>{@link addr}[4]到{@link addr}[15]必须设置为0。<br>对于IPv6，{@link addr}[0]到{@link addr}[15]存储IPv6地址。<br>如果字节与{@link family}要求的地址布局不匹配，<br>使用该结构体的接口将返回{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM}。<br>**起始版本：** 26.0.0 |
