...
| [int32_t OH_TrafficFilter_AddRedirectRule(OH_TrafficFilter_Redirector* redirector, const OH_TrafficFilter_RedirectRule* rule)](#oh_trafficfilter_addredirectrule) | 添加重定向规则 添加TCP流量重定向规则，将匹配的流量重定向到指定的代理服务器 要清除重定向规则，需要调用[OH_TrafficFilter_ClearRedirectRule](capi-net-trafficfilter-h.md#oh_trafficfilter_clearredirectrule)。 |
...
| [int32_t OH_TrafficFilter_AddPacketRule(OH_TrafficFilter_PacketController* controller, const OH_TrafficFilter_FilterRule* rule)](#oh_trafficfilter_addpacketrule) | 设置报文过滤规则 向控制器链添加报文过滤规则。 仅匹配规则的报文会被拦截并发送到回调函数。 |
| [int32_t OH_TrafficFilter_ClearPacketRule(OH_TrafficFilter_PacketController* controller)](#oh_trafficfilter_clearpacketrule) | 清除报文过滤规则 清除控制器中的所有报文过滤规则。 |
| [int32_t OH_TrafficFilter_CreatePacketController(uint32_t groupId, uint32_t priority, const OH_TrafficFilter_Config* config, OH_TrafficFilter_PacketController** controller)](#oh_trafficfilter_createpacketcontroller) | 创建报文控制器实例。 创建用于拦截和过滤网络报文的报文控制器 资源管理：该实例占用系统资源。 必须调用[OH_TrafficFilter_DestroyPacketController](capi-net-trafficfilter-h.md#oh_trafficfilter_destroypacketcontroller)释放资源。 如果该函数失败，不会返回有效的控制器。 |
...
| 参数项 | 描述 |
| -- | -- |
| uint32_t group_id | 重定向链标识符。这是应用内的逻辑分组ID。同一应用内的多个重定向器可以使用不同的group_id。不同应用的相同group_id会自动隔离。有效范围为[<br>    {@link OH_TRAFFICFILTER_MIN_GROUP_ID}, {@link OH_TRAFFICFILTER_MAX_GROUP_ID}]，包含两个边界。如果group_id超出此范围，该函数返回 {@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM}。 |
| uint32_t priority | 优先级。决定不同group_id链之间的执行顺序，数值越小越先执行。注意：重定向器优先级高于报文过滤器优先级。有效范围为[{@link OH_TRAFFICFILTER_MIN_PRIORITY},<br>    {@link OH_TRAFFICFILTER_MAX_PRIORITY}]，包含两个边界。如果priority超出此范围，该函数返回{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM}。 |
| OH_TrafficFilter_Redirector** redirector | 出参，成功时为重定向句柄。 |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| int32_t | {@link OH_TRAFFICFILTER_OK} - 成功。<br>    <br>{@link OH_TRAFFICFILTER_ERROR_PERMISSION_DENIED} - 缺少权限。<br>    <br>{@link OH_TRAFFICFILTER_ERROR_GROUP_ID_IN_USE} - group_id已存在。<br>    <br>{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM} - 参数错误。 |
...
| 类型 | 说明 |
| -- | -- |
| int32_t | {@link OH_TRAFFICFILTER_OK} - 成功。<br>    <br>{@link OH_TRAFFICFILTER_ERROR_PERMISSION_DENIED} - 缺少权限。<br>    <br>{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM} - redirector为NULL。<br>    <br>{@link OH_TRAFFICFILTER_ERROR_NOT_FOUND} - 未找到指定的重定向器句柄。 |
...
添加重定向规则 添加TCP流量重定向规则，将匹配的流量重定向到指定的代理服务器 要清除重定向规则，需要调用[OH_TrafficFilter_ClearRedirectRule](capi-net-trafficfilter-h.md#oh_trafficfilter_clearredirectrule)。
...
| -- | -- |
| OH_TrafficFilter_Redirector* redirector | OH_TrafficFilter_Redirector句柄 |
| const OH_TrafficFilter_RedirectRule* rule | 重定向规则。不能为NULL。 |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| int32_t | <ul><li>{@link OH_TRAFFICFILTER_OK} - 成功。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_PERMISSION_DENIED} - 缺少权限。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM} - redirector或rule为NULL。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_TOO_MANY_RULES} - 规则数量过多。</li></ul> |
...
| 类型 | 说明 |
| -- | -- |
| int32_t | {@link OH_TRAFFICFILTER_OK} - 成功。<br>    <br>{@link OH_TRAFFICFILTER_ERROR_PERMISSION_DENIED} - 缺少权限。<br>    <br>{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM} - redirector为NULL。 |
...
| OH_TrafficFilter_ProcessInfo* process_info | 输出的进程信息。 |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| int32_t | {@link OH_TRAFFICFILTER_OK} - 成功。<br>    <br>{@link OH_TRAFFICFILTER_ERROR_PERMISSION_DENIED} - 缺少权限。<br>    <br>{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM} - 输入参数无效。<br>    <br>{@link OH_TRAFFICFILTER_ERROR_NOT_FOUND} - 未找到进程。 |
...
设置报文过滤规则 向控制器链添加报文过滤规则。 仅匹配规则的报文会被拦截并发送到回调函数。
...
| 参数项 | 描述 |
| -- | -- |
| OH_TrafficFilter_PacketController* controller | [in] OH_TrafficFilter_PacketController句柄 |
| const OH_TrafficFilter_FilterRule* rule | [in] 过滤规则。不能为NULL。 |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| int32_t | <ul><li>{@link OH_TRAFFICFILTER_OK} - 成功。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_PERMISSION_DENIED} - 缺少权限。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM} - controller或rule为NULL。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_TOO_MANY_RULES} - 规则数量过多。</li></ul> |
...
| 类型 | 说明 |
| -- | -- |
| int32_t | <ul><li>{@link OH_TRAFFICFILTER_OK} - 成功。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_PERMISSION_DENIED} - 缺少权限。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM} - controller为NULL。</li></ul> |
...
创建报文控制器实例。 创建用于拦截和过滤网络报文的报文控制器 资源管理：该实例占用系统资源。 必须调用[OH_TrafficFilter_DestroyPacketController](capi-net-trafficfilter-h.md#oh_trafficfilter_destroypacketcontroller)释放资源。 如果该函数失败，不会返回有效的控制器。
...
| const OH_TrafficFilter_Config* config | [in] 配置参数（可以为NULL以使用默认配置） |
| OH_TrafficFilter_PacketController** controller | [out] 出参，<ul><li>成功时为报文控制器句柄。</li></ul> |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| int32_t | <ul><li>{@link OH_TRAFFICFILTER_OK} - 成功。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_PERMISSION_DENIED} - 缺少权限。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_GROUP_ID_IN_USE} - group_id已存在。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM} - priority无效。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_NFQUEUE_ERROR} - NFQueue初始化失败。</li></ul> |
...
| 类型 | 说明 |
| -- | -- |
| int32_t | <ul><li>{@link OH_TRAFFICFILTER_OK} - 成功。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_PERMISSION_DENIED} - 缺少权限。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM} - controller为NULL。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_NOT_FOUND} - 未找到指定的控制器句柄。</li></ul> |
...
| 参数项 | 描述 |
| -- | -- |
| OH_TrafficFilter_PacketController* controller | [in] OH_TrafficFilter_PacketController句柄。不能为NULL。 |
| OH_TrafficFilter_PacketCallback callback | [in] 回调函数指针。不能为NULL。 |
...
| 类型 | 说明 |
| -- | -- |
| int32_t | <ul><li>{@link OH_TRAFFICFILTER_OK} - 成功。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_PERMISSION_DENIED} - 缺少权限。</li><br>    <li>{@link OH_TRAFFICFILTER_ERROR_INVALID_PARAM} - controller或callback为NULL。</li></ul> |
