...
获取指定应用从最近一次开机开始至接口调用时刻的上行流量总和（单位：字节）。使用callback异步回调。 > **说明：** > > 若重启后该应用未产生流量消耗，则会抛出2103005错误码。
...
statistics.getUidTxBytes(20010038, (error: BusinessError|null, stats: long|undefined) => {
  console.error(JSON.stringify(error));
  console.info(JSON.stringify(stats));
});
```
...
获取指定应用从最近一次开机开始至接口调用时刻的上行流量总和（单位：字节）。使用Promise异步回调。 > **说明：** > > 若重启后该应用未产生流量消耗，则会抛出2103005错误码。
...
statistics.getUidTxBytes(20010038).then((stats: long) => {
  console.info(JSON.stringify(stats));
});
```
