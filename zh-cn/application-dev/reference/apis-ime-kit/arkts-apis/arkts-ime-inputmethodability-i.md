...
[getInputMethodAbility](arkts-ime-getinputmethodability-f.md#getinputmethodability-1)获取该实例。

下列API均需使用[getInputMethodAbility](arkts-ime-getinputmethodability-f.md#getinputmethodability-1)获取到InputMethodAbility实例后，通过实例调用。
...
创建输入法面板，仅支持输入法应用在
[InputMethodExtensionAbility](arkts-ime-inputmethodextensionability-c.md)（输入法扩展能力）类中调用。使
用callback异步回调。
...
创建输入法面板，仅支持输入法应用在
[InputMethodExtensionAbility](arkts-ime-inputmethodextensionability-c.md)类中调用。使用promise异
步回调。
