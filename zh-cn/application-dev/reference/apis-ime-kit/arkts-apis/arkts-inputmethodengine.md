...
| [TextInputClient](arkts-ime-inputmethodengine-textinputclient-i.md) | 下列API示例中都需使用 on('inputStart') 回调获取到TextInputClient实例，再通过此实例调用对应方法。 |
...
| [CursorContextChangeCallback](arkts-ime-inputmethodengine-cursorcontextchangecallback-t.md) | 光标上下文变化（cursorContextChange）事件的回调函数类型，用于定义该事件触发时执行的回调函数格式。 |
| [IMAInputStartCallback](arkts-ime-inputmethodengine-imainputstartcallback-t.md) | 输入法绑定成功事件的回调函数类型，用于定义inputStart事件触发时执行的回调函数格式。 |
| [InputKeyEventCallback](arkts-ime-inputmethodengine-inputkeyeventcallback-t.md) | 按键事件（keyEvent）的回调函数类型，用于定义keyEvent事件触发时执行的回调函数格式。 |
| [KeyEventCallback](arkts-ime-inputmethodengine-keyeventcallback-t.md) | 按键按下（keyDown）或按键抬起（keyUp）事件的回调函数类型，用于定义这两类按键事件触发时执行的回调函数格式。 |
| [OnMessageCallback](arkts-ime-inputmethodengine-onmessagecallback-t.md) | 当输入法框架需要显示预览文本时触发的回调。 |
| [SelectionChangeCallback](arkts-ime-inputmethodengine-selectionchangecallback-t.md) | 文本选择范围变化（selectionChange）事件的回调函数类型，用于定义该事件触发时执行的回调函数格式。 |
