...
| isFontScaleFollowSystem<sup>26+</sup> | boolean  | 是    | 是     | 卡片的字体缩放是否跟随系统。<br/>-&nbsp;true：字体缩放跟随系统。<br/>-&nbsp;false：字体缩放不会跟随系统。<br>**模型约束：** 此接口仅可在Stage模型下使用。|
...
| FORM_FONT_SIZE_SCALE_KEY<sup>26+</sup>    | 'ohos.extra.param.key.form_font_size_scale'   | 卡片字体大小缩放键值。 <br>**系统接口：** 此接口为系统接口。<br>**模型约束：** 此接口仅可在Stage模型下使用。  |
| FORM_FONT_WEIGHT_SCALE_KEY<sup>26+</sup>    | 'ohos.extra.param.key.form_font_weight_scale'   | 卡片字重缩放键值。 <br>**系统接口：** 此接口为系统接口。<br>**模型约束：** 此接口仅可在Stage模型下使用。  |
