...
设置指定卡片的下一次刷新时间，使用callback异步回调。适用于需要精确控制卡片刷新时机的场景，例如实时数据更新、定时任务等。
...
| minute | number |  是   | 指定卡片在多少分钟之后更新,取值范围:大于等于5,单位:min。超出范围时返回错误码401。     |
...
获取设备上当前应用的卡片信息，使用callback异步回调。适用于卡片管理、调试、统计等场景，例如查看应用所有卡片配置信息、统计卡片数量等。
