...
请求发布一张卡片到使用方，使用callback异步回调。使用方通常为桌面。适用于系统应用需要主动将卡片添加到桌面的场景，例如系统设置推荐添加卡片、桌面助手快捷添加功能等。
...
| callback | [AsyncCallback&lt;string&gt;](../apis-basic-services-kit/js-apis-base.md#asynccallback)         | 是   |  回调函数，用于异步返回卡片标识。回调参数：error为错误对象（成功时为null），data为卡片标识（string类型）。成功时data返回卡片标识字符串，失败时error返回错误对象。 |
...
请求发布一张卡片到使用方，使用Promise异步回调。使用方通常为桌面。适用于系统应用需要主动将卡片添加到桌面的场景，例如系统设置推荐添加卡片、桌面助手快捷添加功能等。
