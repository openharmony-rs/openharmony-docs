...
打开卡片管理页是否只显示特定单张卡片 - true：表示只显示特定单张卡片。 - false：表示显示所有卡片。 **系统接口：** 此接口为系统接口。
