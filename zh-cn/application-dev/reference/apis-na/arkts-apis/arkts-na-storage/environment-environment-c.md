...
将\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_的内置环境变量key 存入\_\_\_MD\_LINK\_DESC\_USD\_1\_\_\_中。 如果系统中未查询到Environment环境变量key的值，则使用默认值value，存入成功，返回true。如果AppStorage中已经有对应的key，则返回false。 所以建议在程序启动的时候调用该接口。 在没有调用envProp的情况下，就使用AppStorage读取环境变量是错误的。
...
和[envProp]\_\_\_JSDOC\_LINK\_DESC\_USD\_3\_\_\_类似，不同点在于参数为数组，可以一次性初始化多个数据。 建议在应用启动时调用，将系统环境变量批量存入\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_中。 需注意的是，如果传入的dafultValue为 \_\_\_MD\_LINK\_DESC\_USD\_1\_\_\_、 \_\_\_MD\_LINK\_DESC\_USD\_2\_\_\_类型， 或是数字，则需额外指定其具体类型。
