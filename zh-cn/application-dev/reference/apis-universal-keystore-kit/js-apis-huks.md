...
updateSession(handle: long, options: HuksOptions, callback: AsyncCallback\<HuksReturnResult>) : void
...
updateSession(handle: long, options: HuksOptions, token: Uint8Array, callback: AsyncCallback\<HuksReturnResult>) : void
...
| 参数名   | 类型                                                 | 必填 | 说明                                         |
| -------- | ---------------------------------------------------- | ---- | -------------------------------------------- |
| handle   | long                                              | 是   | updateSession操作的uint64类型的handle值。需要使用[initSession](#huksinitsession9)接口返回的handle值，以确保操作归属同一个会话上下文。    |
