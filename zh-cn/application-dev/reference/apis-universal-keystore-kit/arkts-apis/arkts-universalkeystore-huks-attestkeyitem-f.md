...
获取密钥证书。使用callback异步回调。 \_\_\_MD\_COMMENT\_DESC\_USD\_0\_\_\_ > **说明：** > > 在使用非匿名证书密钥证明时生成的证书链可能包含设备标识符（具体实现需向厂商确认），如包含设备标识符，其使用、留存、销毁由开发者决定，建议开发者在其隐私声明中对其使用目的、留存策略和销毁方式进行说明。 \_\_\_MD\_COMMENT\_DESC\_USD\_1\_\_\_
...
| callback | \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;HuksReturnResult&gt; | 是 | 回调函数。当获取密钥证书成功时，err为undefined，data为获取到的HuksReturnResult；否则为错误对象。 |
...
获取密钥证书。使用Promise异步回调。 \_\_\_MD\_COMMENT\_DESC\_USD\_0\_\_\_ > **说明：** > > 在使用非匿名证书密钥证明时生成的证书链可能包含设备标识符（具体实现需向厂商确认），如包含设备标识符，其使用、留存、销毁由开发者决定，建议开发者在其隐私声明中对其使用目的、留存策略和销毁方式进行说明。 \_\_\_MD\_COMMENT\_DESC\_USD\_1\_\_\_
