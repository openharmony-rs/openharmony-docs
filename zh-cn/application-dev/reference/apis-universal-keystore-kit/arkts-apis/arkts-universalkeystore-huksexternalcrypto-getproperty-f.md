...
调用此接口获取属性值并返回结果。使用Promise异步回调。 propertyId表示查询属性的ID信息，当前仅支持GMT 0016-2023中定义的SKF接口名作为属性ID，支持的ID包括如下： - SKF\_EnumDev - SKF\_GetDevInfo - SKF\_EnumApplication - SKF\_EnumContainer
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;Array&lt;HuksExternalCryptoParam&gt;&gt; | Promise对象，返回调用接口的结果。当调用成功时，返回结果为HuksExternalCryptoParam类型的数组，包 |
