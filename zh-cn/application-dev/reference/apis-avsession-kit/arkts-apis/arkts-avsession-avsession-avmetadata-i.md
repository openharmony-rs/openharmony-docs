...
媒体ID。媒体信息的唯一标识，由应用自定义。 - 该属性发生变化则其他元数据属性都将被刷新。 - 若该属性维持不变，且不设置相应的媒体元数据信息，那么将不会更新对应的媒体元数据信息。 - 当该属性设为空值时，调用[setAVMetadata]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_方法将失败，返回错误码6600101。
...
快进支持的时间间隔。默认为SECONDS\_15，即15秒。 系统会使用此值作为快进操作的时间间隔，而非skipIntervals的值。 若未设置此参数，快进操作的时间间隔仍会沿用skipIntervals的值。 **起始版本**：26.0.0
...
快退支持的时间间隔。默认为SECONDS\_15，即15秒。 系统会使用此值作为快退操作的时间间隔，而非skipIntervals的值。 若未设置此参数，快退操作的时间间隔仍会沿用skipIntervals的值。 **起始版本**：26.0.0
