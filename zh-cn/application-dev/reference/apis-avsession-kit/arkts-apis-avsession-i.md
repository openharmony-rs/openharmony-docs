...
| mediaImage | [image.PixelMap](../apis-image-kit/arkts-apis-image-PixelMap.md) \| string  | 否   | 是   | 播放列表设置媒体图片像素数据，在音视频投播场景下，该属性暂不支持使用pixelMap类型展示图片。<br>该属性只支持设置网络Uri，不支持本地Uri。当入参为string类型时，该属性和albumCoverUri属性功能相同，优先级高于albumCoverUri。<br>**系统能力：** SystemCapability.Multimedia.AVSession.Core<br/>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。 |
