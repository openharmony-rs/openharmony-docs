...
| 名称 | 说明 |
| --- | --- |
| addContact | 添加联系人。使用callback异步回调。 |
...
| deleteContact | 删除联系人。使用callback异步回调。 |
...
| hasMatchedCallLog | 检查是否有符合条件的通话记录，默认查询6小时以内的通话记录，仅针对运营商通话。使用Promise异步回调。 |
...
| isLocalContact | 判断当前联系人id是否在电话簿中。使用callback异步回调。 |
...
| isMyCard | 判断是否为“我的名片”。使用callback异步回调。 |
...
| queryContact | 根据联系人唯一标识符key查询联系人。使用callback异步回调。 |
...
| queryContacts | 查询所有联系人。使用callback异步回调。 |
...
| queryContactsByEmail | 根据email查询联系人。使用callback异步回调。该接口返回的列表仅包含联系人信息中的id、key、Emails属性。如果要查询联系人的所有信息，建议使用[queryContact](arkts-contacts-contact-querycontact-f.md#querycontact-7)接口，根据该接口返回的属性key查询。 |
...
| queryContactsByPhoneNumber | 根据电话号码查询联系人。使用callback异步回调。该接口仅返回联系人信息中的id、key、phoneNumbers属性。如果要查询联系人的所有信息，建议使用[queryContact](arkts-contacts-contact-querycontact-f.md#querycontact-7)接口，根据该接口返回的属性key查询。应用在后台调用此接口获取联系人信息必须要申请对应的长时任务。 |
...
| queryGroups | 查询联系人的所有群组。使用callback异步回调。 |
...
| queryHolders | 查询所有创建联系人的应用信息类。使用callback异步回调。 |
...
| queryKey | 根据联系人的id查询联系人的唯一查询键key。使用callback异步回调。 |
...
| queryMyCard | 查询“我的名片”。使用callback异步回调。 |
...
| updateContact | 更新联系人。使用callback异步回调。 |
