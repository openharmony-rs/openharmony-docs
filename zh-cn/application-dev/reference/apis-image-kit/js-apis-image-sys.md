...
## HdrDecomposeOptions<sup>26+</sup>

HDR PixelMap分解为Picture的配置选项。
...
| 名称               | 类型              | 只读 | 可选 | 说明             |
| ----------------- | ----------------- | ---- | ---- | ---------------- |
| isFullSizeGainmap | boolean | 否   | 是   | 是否生成全尺寸增益图（gainmap）。默认值为false，即生成1/2缩小的增益图。 |
| desiredPixelFormat | [PixelMapFormat](arkts-apis-image-PixelMap.md#pixelmapformat7) | 否   | 是   | 分解后SDR PixelMap和增益图的像素格式。支持RGBA_8888、NV12、NV21。默认值为RGBA_8888。 |

## image.decomposeToPicture<sup>26+</sup>
...
| options | [HdrDecomposeOptions](#hdrdecomposeoptions26) | 否   | HDR分解配置选项，包含增益图尺寸和像素格式设置。 |
...
async function DecomposeToPictureTest(context: Context) {
  ...
  // 使用默认配置分解HDR PixelMap
  let picture: image.Picture | undefined = await image.decomposeToPicture(hdrPixelMap);
  if (picture != undefined) {
    console.info('Decompose to picture succeeded');
  } else {
    console.error('Decompose to picture failed');
  }
  ...
  let picture2: image.Picture | undefined = await image.decomposeToPicture(hdrPixelMap, options);
  if (picture2 != undefined) {
    console.info('Decompose to picture with options succeeded');
  } else {
