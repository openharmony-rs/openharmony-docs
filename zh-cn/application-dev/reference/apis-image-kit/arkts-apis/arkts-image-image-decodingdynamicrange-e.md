...
自适应，根据图片信息处理。即如果图片本身为HDR图片，则会按照HDR内容解码；反之按照SDR内容解码。通过[CreateIncrementalSource](arkts-image-image-createincrementalsource-f.md#createincrementalsource-1)创建的ImageSource会解码为SDR内容。
...
按照高动态范围处理图片。通过[CreateIncrementalSource](arkts-image-image-createincrementalsource-f.md#createincrementalsource-1)创建的ImageSource会解码为SDR内容。
