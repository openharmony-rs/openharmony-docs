...
ImageReceiver类，用于获取组件surface id、接收最新的图片和读取下一张图片以及释放ImageReceiver实例。ImageReceiver作为图片的接收方和消费者，其参数属性实际上不会对接收到的图片产生影响。 图片属性的配置应在发送方和生产者上进行，如相机预览流 [createPreviewOutput]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ 。 在调用以下方法前需要先通过[image.createImageReceiver]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_创建ImageReceiver实例。 从API version 23开始，更推荐使用[image.createImageReceiver]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_，通过传入 [ImageReceiverOptions]\_\_\_JSDOC\_LINK\_DESC\_USD\_3\_\_\_创建ImageReceiver实例。 由于图片占用内存较大，所以当ImageReceiver实例使用完成后，应主动调用[release]\_\_\_JSDOC\_LINK\_DESC\_USD\_4\_\_\_ 方法及时释放内存。释放时应确保该实例的所有异步方法均执行完成，且后续不再使用该实例。 > **说明：** > > - 本Interface首批接口从API version 9开始支持。
...
从ImageReceiver读取最新的图片。使用callback异步回调。 > **注意**： > > 此接口需要在[on]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_回调触发后调用，才能正常的接收到数 > 据。且此接口返回的[Image]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_对象使用完毕后需要调用 > [release]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_方法释放，释放后才可以继续接收新的数据。
...
从ImageReceiver读取最新的图片。使用Promise异步回调。 > **注意**： > > 此接口需要在[on]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_回调触发后调用，才能正常的接收到数 > 据。且此接口返回的[Image]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_对象使用完毕后需要调用 > [release]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_方法释放，释放后才可以继续接收新的数据。
...
从ImageReceiver读取下一张图片。使用callback异步回调。 > **注意**： > > 此接口需要在[on]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_回调触发后调用，才能正常的接收到数 > 据。且此接口返回的[Image]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_对象使用完毕后需要调用 > [release]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_方法释放，释放后才可以继续接收新的数据。
...
从ImageReceiver读取下一张图片。使用Promise异步回调。 > **注意**： > > 此接口需要在[on]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_回调触发后调用，才能正常的接收到数 > 据。且此接口返回的[Image]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_对象使用完毕后需要调用 > [release]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_方法释放，释放后才可以继续接收新的数据。
