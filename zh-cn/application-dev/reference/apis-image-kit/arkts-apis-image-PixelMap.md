...
> **说明：**
>
> - 平移后的图像尺寸将变为：宽度 = 原宽度 + x，高度 = 原高度 + y。当x为负数时，平移的效果等同于裁剪掉自左侧开始的x列像素。当y为负数时，平移的效果等同于裁剪掉自上方开始的y行像素。
> - 建议平移后的图像尺寸不要超过屏幕的宽高。
...
| 参数名   | 类型                 | 必填 | 说明                                           |
| -------- | -------------------- | ---- | --------------------------------------------------------------------------------------- |
| x        | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 横向平移的距离。方向为正数向右，负数向左。取值范围是(-图像宽度, +∞)。单位：像素（px）。         |
| y        | ArkTS-Dyn: number<br>ArkTS-Sta: double | 是   | 纵向平移的距离。方向为正数向下，负数向上。取值范围是(-图像高度, +∞)。单位：像素（px）。         |
