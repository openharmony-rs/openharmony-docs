...
图像像素类，用于读取或写入图像数据以及获取图像信息。在调用PixelMap的方法前，需要先通过[image.createPixelMapFromPixels](arkts-apis-image-f.md#imagecreatepixelmapfrompixels2600)创建一个PixelMap实例。目前PixelMap序列化大小最大128MB，超过会送显失败。大小计算方式为（宽\*高\*[每像素占用字节数](arkts-apis-image-e.md#pixelmapformat7)）。
...
在调用PixelMap的方法前，可以通过[image.createPixelMapFromPixels](arkts-apis-image-f.md#imagecreatepixelmapfrompixels2600)传入像素数据创建一个PixelMap对象，也可以通过[ImageSource](arkts-apis-image-ImageSource.md)进行图片解码创建PixelMap对象。
...
## readAllPixelsToBuffer<sup>26.0.0+</sup>
...
function DemoReadAllPixelsToBuffer(pixelMap: image.PixelMap) {
  ...
  pixelMap.readAllPixelsToBuffer(readBuffer);
    .then(() => {
      console.log('pixelMap中的像素数据已拷贝至readBuffer。');
    })
...
## readAllPixelsToBufferSync<sup>26.0.0+</sup>
...
建议使用[readAllPixelsToBuffer](#readallpixelstobuffer2600)。
...
建议使用[readAllPixelsToBufferSync](#readallpixelstobuffersync2600)。
...
## readPixelsToArea<sup>26.0.0+</sup>
...
| 参数名   | 类型                 | 必填 | 说明                                                                                                  |
| -------- | ------------------- | ---- | ----------------------------------------------------------------------------------------------------- |
| area     | [PositionArea](arkts-apis-image-i.md#positionarea7) | 是   | 读取的区域。<br>该区域由[PositionArea](arkts-apis-image-i.md#positionarea7).region指定，函数执行结束后获取的图像像素数据会被写入[PositionArea](arkts-apis-image-i.md#positionarea7).pixels缓冲区。 |
...
function DemoReadPixelsToAreaRGBA(pixelMap: image.PixelMap) {
  ...
  pixelMap.readPixelsToArea(area);
    .then(() => {
      console.log('pixelMap区域中的像素数据已拷贝至area.pixels。');
    })
    .catch((e: BusinessError) => {
      console.error(`读取像素失败。错误码：${e.code} 错误信息：${e.message}`);
    });
...
function DemoReadPixelsToAreaYUV(pixelMap: image.PixelMap) {
  ...
  pixelMap.readPixelsToArea(area);
    .then(() => {
      console.log('pixelMap指定区域中的像素数据已拷贝至area.pixels。');
    })
    .catch((e: BusinessError) => {
      console.error(`读取像素失败。错误码：${e.code} 错误信息：${e.message}`);
    });
...
## readPixelsToAreaSync<sup>26.0.0+</sup>
...
建议使用[readPixelsToArea](#readpixelstoarea2600)。
...
建议使用[readPixelsToAreaSync](#readpixelstoareasync2600)。
...
## writePixelsFromArea<sup>26.0.0+</sup>

writePixelsFromArea(area: PositionArea): Promise\<void\>;

读取缓冲区内的图像像素数据，并按照BGRA_8888格式将其写入PixelMap的指定区域。缓冲区内数据的像素格式必须是BGRA_8888。使用Promise异步回调。
...
| 参数名   | 类型                 | 必填 | 说明                                                                                                  |
| -------- | ------------------- | ---- | ----------------------------------------------------------------------------------------------------- |
| area     | [PositionArea](arkts-apis-image-i.md#positionarea7) | 是   | 写入的区域。<br>该区域由[PositionArea](arkts-apis-image-i.md#positionarea7).region指定，函数执行结束后[PositionArea](arkts-apis-image-i.md#positionarea7).pixels缓冲区内的图像像素数据会被写入PixelMap。 |
...
function DemoWritePixelsFromAreaRGBA(pixelMap: image.PixelMap) {
  ...
  pixelMap.writePixelsFromArea(area);
    .then(() => {
      console.log('area.pixels中的像素数据已拷贝至pixelMap指定区域。');
    })
    .catch((e: BusinessError) => {
...
function DemoWritePixelsFromAreaYUV(pixelMap: image.PixelMap) {
  ...
  for (let i = ySize; i < area.pixels.length; i++) { // UV交错平面。
    bufferArr[i] = 0x80;
  }
...
## writePixelsFromAreaSync<sup>26.0.0+</sup>

writePixelsFromAreaSync(area: PositionArea): void;

读取缓冲区内的图像像素数据，并按照BGRA_8888格式将其写入PixelMap的指定区域。缓冲区内数据的像素格式必须是BGRA_8888。
...
建议使用[writePixelsFromArea](#writepixelsfromarea2600)。
...
建议使用[writePixelsFromAreaSync](#writepixelsfromareasync2600)。
...
## writeAllPixelsFromBuffer<sup>26.0.0+</sup>
...
function DemoWriteAllPixelsFromBuffer(pixelMap: image.PixelMap) {
  ...
  pixelMap.writeAllPixelsFromBuffer(writeBuffer);
    .then(() => {
      console.log('writeBuffer中的像素数据已拷贝至pixelMap。');
    })
    .catch((e: BusinessError) => {
      console.error(`写入像素失败。错误码：${e.code} 错误信息：${e.message}`);
    });
...
## writeAllPixelsFromBufferSync<sup>26.0.0+</sup>

writeAllPixelsFromBufferSync(dst: ArrayBuffer): void
...
建议使用[writeAllPixelsFromBuffer](#writeallpixelsfrombuffer2600)。
...
建议使用[writeAllPixelsFromBufferSync](#writeallpixelsfrombuffersync2600)。
...
建议使用[setOpacity](#setopacity2600)。
...
建议使用[setOpacitySync](#setopacitysync2600)。
...
## extractAlphaPixelMap<sup>26.0.0+</sup>
...
function DemoExtractAlphaPixelMap(pixelMap: image.PixelMap) {
  pixelMap.extractAlphaPixelMap();
    .then((alphaMap: image.PixelMap) => {
      console.log('创建Alpha通道PixelMap成功。');
    })
    .catch((e: BusinessError) => {
...
| 类型           | 说明                                             |
| -------------- | ------------------------------------------------ |
| PixelMap       | 仅包含Alpha通道信息的ALPHA_U8格式的PixelMap。      |
...
建议使用[extractAlphaPixelMap](#extractalphapixelmap2600)。
...
建议使用[extractAlphaPixelMapSync](#extractalphapixelmapsync2600)。
...
> **说明：**
>
> 1. 建议宽高的缩放倍数取正数，否则会产生翻转效果。
> 2. 缩放倍数 = 缩放后的图像尺寸 / 缩放前的图像尺寸。
...
建议使用[applyScale](#applyscale2600)。
...
建议使用[applyScaleSync](#applyscalesync2600)。
...
| 参数名   | 类型                 | 必填 | 说明                                                                                      |
| -------- | -------------------- | ---- | --------------------------------------------------------------------------------------- |
| x        | number               | 是   | 横向平移的距离。单位：像素。                                                               |
| y        | number               | 是   | 纵向平移的距离。单位：像素。                                                               |
...
## applyTranslateSync<sup>26.0.0+</sup>
...
建议使用[applyTranslate](#applytranslate2600)。
...
建议使用[applyTranslateSync](#applytranslatesync2600)。
...
> **说明：**
>
> 1. 图像旋转角度的取值范围为：[0, 360]。超出取值范围时，将根据圆周360°自动矫正。例如，-100°与260°效果相同。
> 2. 当旋转角度不是90°的倍数时，图像的尺寸会扩大为旋转后的外接矩形尺寸，以容纳旋转后的图像内容。例如：旋转45°时，输出图像尺寸为原图像的√2倍。
...
## applyRotateSync<sup>26.0.0+</sup>
...
建议使用[applyRotate](#applyrotate2600)。
...
建议使用[applyRotateSync](#applyrotatesync2600)。
...
建议使用[applyFlipSync](#applyflipsync2600)。
...
| 参数名   | 类型                 | 必填 | 说明                                                                                      |
| -------- | -------------------- | ---- | --------------------------------------------------------------------------------------- |
| region   | [Region](arkts-apis-image-i.md#region8)   | 是   | 裁剪的区域。区域范围不能超过图片的宽高。                               |
...
建议使用[applyCropSync](#applycropsync2600)。
