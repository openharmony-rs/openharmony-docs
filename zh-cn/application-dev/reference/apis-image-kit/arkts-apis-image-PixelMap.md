...
图像像素类，用于读取或写入图像数据以及获取图像信息。在调用PixelMap的方法前，需要先通过[image.createPixelMapFromPixels](arkts-apis-image-f.md#imagecreatepixelmapfrompixels)创建一个PixelMap实例。目前PixelMap序列化大小最大128MB，超过会渲染失败。大小计算方式为（宽 \* 高 \* 每像素占用字节数（参考[PixelMapFormat](arkts-apis-image-e.md#pixelmapformat7)））。
...
| isStrideAlignment<sup>11+</sup> | boolean | 是   | 否   | 图像的行数据是否已进行内存对齐。true表示已进行内存对齐，每行数据的末尾可能有空白字节填充以满足对齐要求；false表示未进行内存对齐，每行数据紧密排列，末尾无空白字节填充。<br>**ArkTS-Dyn起始版本：** 11<br>**ArkTS-Sta起始版本：** 23 |
...
```ts
function GetBytesNumberPerRow(pixelMap: image.PixelMap) {
  let rowCount: int = pixelMap.getBytesNumberPerRow();
}
```
...
| 参数名   | 类型                 | 必填 | 说明                          |
| -------- | -------------------- | ---- | ----------------------------- |
| x        | ArkTS-Dyn: number<br>ArkTS-Sta: double               | 是   | 区域横坐标。单位：像素（px）。 |
| y        | ArkTS-Dyn: number<br>ArkTS-Sta: double               | 是   | 区域纵坐标。单位：像素（px）。 |
...
| 参数名   | 类型                 | 必填 | 说明                                                                                      |
| -------- | -------------------- | ---- | --------------------------------------------------------------------------------------- |
| region   | [Region](arkts-apis-image-i.md#region8)   | 是   | 裁剪的区域，区域范围不能超过图片的宽高。单位为像素（px）。              |
