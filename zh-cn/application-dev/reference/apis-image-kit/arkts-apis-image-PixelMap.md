...
图像像素类，用于读取或写入图像数据以及获取图像信息。在调用PixelMap的方法前，需要先通过[image.createPixelMapFromPixels](arkts-apis-image-f.md#imagecreatepixelmapfrompixels)创建一个PixelMap实例。目前PixelMap序列化大小最大128MB，超过会送显失败。大小计算方式为（宽 \* 高 \* [每像素占用字节数](arkts-apis-image-e.md#pixelmapformat7)）。
...
在调用PixelMap的方法前，可以通过[image.createPixelMapFromPixels](arkts-apis-image-f.md#imagecreatepixelmapfrompixels)传入像素数据创建一个PixelMap对象，也可以通过[ImageSource](arkts-apis-image-ImageSource.md)进行图片解码创建PixelMap对象。
...
建议使用[readAllPixelsToBuffer](#readallpixelstobuffer)代替。
...
建议使用[readAllPixelsToBufferSync](#readallpixelstobuffersync)代替。
...
建议使用[readPixelsToArea](#readpixelstoarea)代替。
...
建议使用[readPixelsToAreaSync](#readpixelstoareasync)代替。
...
建议使用[writePixelsFromArea](#writepixelsfromarea)代替。
...
建议使用[writePixelsFromAreaSync](#writepixelsfromareasync)代替。
...
建议使用[writeAllPixelsFromBuffer](#writeallpixelsfrombuffer)代替。
...
建议使用[writeAllPixelsFromBufferSync](#writeallpixelsfrombuffersync)代替。
...
建议使用[setOpacitySync](#setopacitysync)代替。
...
建议使用[extractAlphaPixelMap](#extractalphapixelmap)代替。
...
建议使用[extractAlphaPixelMapSync](#extractalphapixelmapsync)代替。
...
建议使用[applyScaleSync](#applyscalesync)代替。
...
建议使用[applyTranslate](#applytranslate)代替。
...
建议使用[applyTranslateSync](#applytranslatesync)代替。
...
建议使用[applyRotate](#applyrotate)代替。
...
建议使用[applyRotateSync](#applyrotatesync)代替。
...
建议使用[applyFlipSync](#applyflipsync)代替。
...
建议使用[applyCropSync](#applycropsync)代替。
