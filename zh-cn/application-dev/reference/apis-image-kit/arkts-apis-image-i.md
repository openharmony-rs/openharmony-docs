...
| bufferSize<sup>9+</sup> | number | 否   | 是   | 接收编码数据的缓冲区大小。单位：字节（Byte）。如果不设置大小，默认为25M。如果编码图片超过25M，需要指定大小。bufferSize需大于编码后图片大小。使用[packToFile](arkts-apis-image-ImagePacker.md#packtofile11)不受此参数限制。<br>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。 |
