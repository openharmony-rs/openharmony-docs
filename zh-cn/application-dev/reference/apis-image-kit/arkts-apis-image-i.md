...
| offset | ArkTS-DT: number<br>ArkTS-ST: int | 否 |   否  | 偏移量。单位：字节。                                                     |
| stride | ArkTS-DT: number<br>ArkTS-ST: int | 否 |   否  | 跨距，内存中每行像素所占的空间。stride >= region.size.width * 4。                   |
...
| density<sup>9+</sup> | ArkTS-DT: number<br>ArkTS-ST: int | 否  | 否 |像素密度，单位为ppi。<br>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。 <br>**卡片能力：** 从API version 12开始，该接口支持在ArkTS卡片中使用。 |
| stride<sup>11+</sup> | ArkTS-DT: number<br>ArkTS-ST: int | 否  | 否  | 跨距，内存中每行像素所占的空间。stride >= region.size.width * 4 <br>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。 <br>**卡片能力：** 从API version 12开始，该接口支持在ArkTS卡片中使用。 |
...
| 名称   | 类型   | 只读 |  可选  |说明           |
| ------ | ------ | -- |-----| -------------- |
| height | ArkTS-DT: number<br>ArkTS-ST: int | 否  |  否  |输出图片的高，单位：像素。 |
| width  | ArkTS-DT: number<br>ArkTS-ST: int | 否  |  否 | 输出图片的宽，单位：像素。 |
...
| x<sup>7+</sup>    | ArkTS-DT: number<br>ArkTS-ST: int | 否   | 否  | 区域左上角横坐标。单位：像素。 |
| y<sup>7+</sup>    | ArkTS-DT: number<br>ArkTS-ST: int | 否  | 否  | 区域左上角纵坐标。单位：像素。 |
...
| quality | number | 否   | 否   | 1. 编码中设定输出图片质量的参数，该参数仅对JPEG图片和HEIF图片生效。取值范围：[0, 100]。0质量最低，100质量最高，质量越高生成图片所占空间越大。WebP、PNG等图片均为无损编码。<br> 2.sdr_astc_4x4编码中，可以设定输出图片质量的参数，可选参数：92、85。<br>3. sut编码中，设定输出图片质量可选参数：92。<br>4. （API 20支持）hdr_astc_4x4编码中，可以设定输出图片质量的参数，可选参数：85。<br>**原子化服务API：** 从API version 11开始，该接口支持在原子化服务中使用。 |
