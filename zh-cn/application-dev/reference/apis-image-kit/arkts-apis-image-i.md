...
| desiredSizeForMainPixelMap<sup>24+</sup> | [Size](#size) | 否   | 是   | 期望输出主图大小（必须为正整数），默认为主图原始尺寸，单位为像素（px）。<br>若指定尺寸与主图原始尺寸不一致，则会进行拉伸/缩放到指定尺寸。<br>辅助图的宽度与高度均将与主图按照同比例进行相应拉伸/缩放。<br>**模型约束：** 此接口仅可在Stage模型下使用。|
