...
图像创建模块，用于请求图像原生数据区域，并开放给应用处理原生图像数据的能力。
在调用以下方法前需要先创建ImageCreator实例，ImageCreator不支持并发操作。
...
从空闲队列中获取缓冲区的图像数据，用于绘制UI内容。使用callback异步回调。
...
从空闲队列中获取缓冲区中的图像数据，用于绘制UI内容。使用Promise异步回调。
...
注册imageRelease事件的回调函数。使用callback异步回调。
