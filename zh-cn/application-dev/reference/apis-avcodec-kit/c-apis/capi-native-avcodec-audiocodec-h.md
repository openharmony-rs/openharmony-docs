...
| 名称 | typedef关键字 | 描述 |
| -- | -- | -- |
| [MediaKeySession](capi-audiocodec-mediakeysession.md) | MediaKeySession | MediaKeySession结构。 |
...
| [OH_AVCodec *OH_AudioCodec_CreateByName(const char *name)](#oh_audiocodec_createbyname) | 通过音频编解码器名称创建音频编解码器实例，使用此接口的前提是知道编解码器的确切名称，编解码器的名称可以通过{@link OH_AVCapability_GetName}获取。 |
...
| [OH_AVErrCode OH_AudioCodec_Flush(OH_AVCodec *codec)](#oh_audiocodec_flush) | 清除编解码器中缓存的输入和输出数据。调用此接口后，以前通过异步回调上报的所有缓冲区 索引都将失效，请确保不要访问这些索引对应的缓冲区。 |
...
| [OH_AVFormat *OH_AudioCodec_GetOutputDescription(OH_AVCodec *codec)](#oh_audiocodec_getoutputdescription) | 获取编解码器输出数据的OH_AVFormat信息。<br> 需要注意的是，返回值所指向的OH_AVFormat实例需要开发者调用{@link OH_AVFormat_Destroy}接口手动释放。 |
...
| [OH_AVErrCode OH_AudioCodec_PushInputBuffer(OH_AVCodec *codec, uint32_t index)](#oh_audiocodec_pushinputbuffer) | 通知音频编解码器已完成对index所对应缓冲区进行输入数据的填充。<br> {@link OH_AVCodecOnNeedInputBuffer}回调将报告可用的输入缓冲区和对应的索引值。<br>一旦具有指定索引的缓冲区被提交给音频编解码器，该缓冲区将无法再次访问，直到再次收到{@link OH_AVCodecOnNeedInputBuffer}回调，收到相同索引时此缓冲区才可使用。<br> 此外，对于某些编解码器，需要在开始时向编解码器输入编解码特定配置数据(Codec-Specific-Data)，以初始化编解码器的编解码过程。<br> 注意：当返回值为AV_ERR_UNKNOWN时此次调用不生效，输入缓冲区仍为未处理状态，需根据返回的特定错误代码处理后输入相同的index重新调用OH_AudioCodec_PushInputBuffer。 |
...
通过音频编解码器名称创建音频编解码器实例，使用此接口的前提是知道编解码器的确切名称，编解码器的名称可以通过{@link OH_AVCapability_GetName}获取。
...
| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | {@link AV_ERR_OK}：执行成功。<br>    <br>{@link AV_ERR_INVALID_VAL}：OH_AVCodec实例为nullptr或无效。<br>    <br>{@link AV_ERR_INVALID_STATE}：编解码器服务不可用。<br>    <br>{@link AV_ERR_NO_MEMORY}：内部资源已经释放。<br>    <br>{@link AV_ERR_UNKNOWN}：发生内部错误，建议检查日志。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | {@link AV_ERR_OK}：执行成功。<br>    <br>{@link AV_ERR_INVALID_VAL}：输入参数为nullptr或无效。<br>    <br>{@link AV_ERR_INVALID_STATE}：编解码器服务不可用。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | {@link AV_ERR_OK}：执行成功。<br>    <br>{@link AV_ERR_INVALID_VAL}：输入参数为nullptr或无效。<br>    <br>{@link AV_ERR_INVALID_STATE}：编解码器服务不可用。<br>    <br>{@link AV_ERR_OPERATE_NOT_PERMIT}：不允许操作，这可能是由于状态不正确或不支持的操作。<br>    <br>{@link AV_ERR_UNKNOWN}：发生内部错误，建议检查日志。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | {@link AV_ERR_OK}：执行成功。<br>    <br>{@link AV_ERR_INVALID_VAL}：OH_AVCodec实例为nullptr或无效。<br>    <br>{@link AV_ERR_INVALID_STATE}：编解码器服务不可用。<br>    <br>{@link AV_ERR_OPERATE_NOT_PERMIT}：不允许操作，这可能是由于状态不正确或不支持的操作。<br>    <br>{@link AV_ERR_UNKNOWN}：发生内部错误，建议检查日志。 |
...
清除编解码器中缓存的输入和输出数据。调用此接口后，以前通过异步回调上报的所有缓冲区 索引都将失效，请确保不要访问这些索引对应的缓冲区。
...
| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | {@link AV_ERR_OK}：执行成功。<br>    <br>{@link AV_ERR_INVALID_VAL}：OH_AVCodec实例为nullptr或无效。<br>    <br>{@link AV_ERR_INVALID_STATE}：编解码器服务不可用。 |
...
获取编解码器输出数据的OH_AVFormat信息。<br> 需要注意的是，返回值所指向的OH_AVFormat实例需要开发者调用{@link OH_AVFormat_Destroy}接口手动释放。
...
| 类型 | 说明 |
| -- | -- |
| OH_AVFormat * | 返回OH_AVFormat句柄指针，生命周期将使用下一个[OH_AudioCodec_GetOutputDescription](capi-native-avcodec-audiocodec-h.md#oh_audiocodec_getoutputdescription)刷新，或使用OH_AVCodec销毁。 |
...
| -- | -- |
| OH_AVCodec *codec | 指向OH_AVCodec实例的指针。 |
| uint32_t index | 输入回调{@link OH_AVCodecOnNeedInputBuffer}给出的索引值。 |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | {@link AV_ERR_OK}：执行成功。<br>    <br>{@link AV_ERR_INVALID_VAL}：输入的index已使用或无效，需使用其他{@link OH_AVCodecOnNeedInputBuffer}回调返回的index。<br>    <br>{@link AV_ERR_INVALID_STATE}：编解码器状态错误，调用OH_AudioCodec_PushInputBuffer前需确保按顺序成功调用[OH_AudioCodec_Configure](capi-native-avcodec-audiocodec-h.md#oh_audiocodec_configure)、<br>    [OH_AudioCodec_Prepare](capi-native-avcodec-audiocodec-h.md#oh_audiocodec_prepare)、[OH_AudioCodec_Start](capi-native-avcodec-audiocodec-h.md#oh_audiocodec_start)。<br>    <br>{@link AV_ERR_UNKNOWN}：输入buffer size无效，需确保buffer设置了正确的buffer size和flags。 |
...
| -- | -- |
| OH_AVCodec *codec | 指向OH_AVCodec实例的指针。 |
| uint32_t index | 输出{@link OH_AVCodecOnNewOutputBuffer}给出的索引值。 |

**返回值：**

| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | {@link AV_ERR_OK}：执行成功。<br>    <br>{@link AV_ERR_INVALID_VAL}：输入参数为nullptr或无效。缓冲区索引应该由{@link OH_AVCodecOnNewOutputBuffer}给出。<br>    <br>{@link AV_ERR_INVALID_STATE}：编解码器服务不可用。<br>    <br>{@link AV_ERR_OPERATE_NOT_PERMIT}：不允许操作，这可能是由于状态不正确或不支持的操作。<br>    <br>{@link AV_ERR_UNKNOWN}：发生内部错误，建议检查日志。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | {@link AV_ERR_OK}：执行成功。<br>    <br>{@link AV_ERR_INVALID_VAL}：输入参数为nullptr或无效。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | {@link AV_ERR_OK}：执行成功。<br>    <br>{@link AV_ERR_INVALID_VAL}：OH_AVCodec实例为nullptr或无效，mediaKeySystemInfo实例为nullptr或无效。<br>    <br>{@link AV_ERR_INVALID_STATE}：编解码器服务不可用。<br>    <br>{@link AV_ERR_NO_MEMORY}：请求内存失败。 |
...
在设置的超时时间内，尝试查询对应音频编解码器可用的输入缓冲区的索引值。<br> 获取到索引值后，使用[OH_AudioCodec_GetInputBuffer](capi-native-avcodec-audiocodec-h.md#oh_audiocodec_getinputbuffer)接口获取索引值对应的缓冲区。<br> 在缓冲区填充数据后，使用[OH_AudioCodec_PushInputBuffer](capi-native-avcodec-audiocodec-h.md#oh_audiocodec_pushinputbuffer)输入对应索引值，将缓冲区数据送到编解码器。<br> 注意：此接口仅可在音频编解码的同步模式中使用。
...
| 参数项 | 描述 |
| -- | -- |
| struct OH_AVCodec *codec | 指向OH_AVCodec实例的指针。 |
...
| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | {@link AV_ERR_OK}：执行成功。<br>    <br>{@link AV_ERR_INVALID_VAL}：执行失败，输入参数错误。<br>    <br>{@link AV_ERR_INVALID_STATE}：执行失败，状态非法，没有启动编解码器等。<br>    <br>{@link AV_ERR_OPERATE_NOT_PERMIT}：执行失败，不允许非同步模式下调用。<br>    <br>{@link AV_ERR_TRY_AGAIN_LATER}：执行失败，超时时间内获取不到可用的缓冲区。 |
...
输入索引值，获取对应音频编解码器中该索引值对应的输入缓冲区。<br> 注意：此接口仅可在音频编解码的同步模式中使用。
...
| 类型 | 说明 |
| -- | -- |
| OH_AVErrCode | {@link AV_ERR_OK}：执行成功。<br>    <br>{@link AV_ERR_INVALID_VAL}：执行失败，输入参数错误。<br>    <br>{@link AV_ERR_INVALID_STATE}：执行失败，状态非法，没有启动编解码器等。<br>    <br>{@link AV_ERR_OPERATE_NOT_PERMIT}：执行失败，不允许非同步模式下调用。<br>    <br>{@link AV_ERR_STREAM_CHANGED}：解码输出流格式发生变化, 可以通过调用[OH_AudioCodec_GetOutputDescription](capi-native-avcodec-audiocodec-h.md#oh_audiocodec_getoutputdescription)接口获取新的流信息。<br>    <br>{@link AV_ERR_TRY_AGAIN_LATER}：执行失败，超时时间内获取不到可用的缓冲区。 |
