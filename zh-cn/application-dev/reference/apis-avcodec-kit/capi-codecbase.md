...
| OH_MD_KEY_BUFFER_SKIP_SAMPLES_INFO | AVBuffer中携带的键，用于丢弃音频解码输出的数据，以音频样点为单位，值类型为uint8_t*，仅mp3、vorbis、opus解码器支持。一般只有音频的起始、末尾帧有该信息，该键是可选的。<br>使用方法一（解封装时获取该信息，设置到解码输入的OH_AVBuffer里）：<br>1. 从[OH_AVCodecCallback](capi-codecbase-oh-avcodeccallback.md#oh_avcodeccallback)的[OH_AVCodecOnNeedInputBuffer](capi-codecbase-oh-avcodeccallback.md#oh_avcodeconneedinputbuffer)回调函数里，获取一个解码用的OH_AVBuffer。<br>2. 调用[OH_AVDemuxer_ReadSampleBuffer](capi-native-avdemuxer-h.md#oh_avdemuxer_readsamplebuffer)接口，该接口读取音频数据的同时会设置OH_MD_KEY_BUFFER_SKIP_SAMPLES_INFO信息。<br>3. 调用[OH_AudioCodec_PushInputBuffer](capi-native-avcodec-audiocodec-h.md#oh_audiocodec_pushinputbuffer)输入OH_AVBuffer解码即可。<br>使用方法二（构造该键需要的数据，设置到解码输入的OH_AVBuffer里）：<br>1. 构造该键需要的数据。构造方法：<br>&emsp;1）申请10个字节的uint8_t类型内存空间。<br>&emsp;2）[0 - 3] 4个字节表示从当前帧第一个样点开始，往后丢弃的音频样点数，该值可以大于1帧的样点数。用小端序存4个字节的uint32_t数值。<br>&emsp;3）[4 - 7] 4个字节表示从当前帧最后一个样点开始，往前丢弃的音频样点数，该值不大于1帧的样点数，否则无效。用小端序存4个字节的uint32_t数值。<br>&emsp;4）[8 - 9] 2个字节填0，用作保留位。<br>2. 调用[OH_AVBuffer_GetParameter](capi-native-avbuffer-h.md#oh_avbuffer_getparameter)从音频解码输入的OH_AVBuffer里获取OH_AVFormat。<br>3. 调用[OH_AVFormat_SetBuffer](capi-native-avformat-h.md#oh_avformat_setbuffer)把构造的数据，设置给OH_AVFormat。<br>4. 调用[OH_AVFormat_Destroy](capi-native-avformat-h.md#oh_avformat_destroy)销毁OH_AVFormat。 |
