...
| OH_MD_KEY_VIDEO_ENCODER_PER_FRAME_MARK_LTR  | 标记当前帧为长期参考帧的键，值类型为int32_t，1表示被标记，0表示不使能。该键是可选的且只用于视频编码。 |
| OH_MD_KEY_VIDEO_ENCODER_PER_FRAME_USE_LTR    | 	描述当前帧参考的长期参考帧帧的POC号的键，值类型为int32_t。该键是可选的且只用于视频编码。 |
| OH_MD_KEY_VIDEO_PER_FRAME_IS_LTR      | 当前[OH_AVBuffer](capi-core-oh-avbuffer.md)中输出的码流对应的帧是否为长期参考帧的键，值类型为int32_t，1表示是LTR，0表示不使能。该键是可选的且只用于视频编码。 |
