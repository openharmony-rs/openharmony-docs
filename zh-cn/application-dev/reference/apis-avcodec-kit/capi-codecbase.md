...
#### Codec；编解码器
对音视频数据进行压缩编码或解压缩解码的组件，通过去除冗余信息减小数据量，便于存储和传输。

#### MIME (Multipurpose Internet Mail Extensions)；多用途互联网邮件扩展
一种标准，用于标识文档和内容的类型格式，在音视频编解码场景中用于标识编解码器的媒体格式类型。

#### Media Track；媒体轨道
媒体文件中的独立数据流，包括音频轨、视频轨、字幕轨等，每个轨道包含特定类型的多媒体数据。

#### Key Frame；关键帧
视频流中可以独立解码的帧，不依赖其他帧的信息，也称为I帧（Intra Frame），用于随机访问和错误恢复。
...
#### Codec Callback；编解码器回调
异步通知机制，当编解码器状态变化时触发的函数指针集合，包括错误通知、流变化通知、输入输出缓冲区通知等。

#### Custom Data Source；自定义数据源
用户实现的媒体数据读取接口，允许从非标准来源（如网络流、自定义容器）读取音视频数据供解码器使用。

#### UserData；用户数据
在回调函数中传递的自定义数据指针，用于在回调上下文中携带应用层的状态信息。
...
#### Temporal GOP (Temporal Group of Pictures)；时域图片组
视频编码中按时间顺序组织的一组帧序列，包含基础层和增强层，支持时域可分层编码。

#### Reference Mode；参考模式
视频编码中帧间预测的参考方式，决定帧之间的依赖关系，影响编码效率和错误恢复能力。

### 码率控制术语

#### Bitrate；比特率
单位时间内编码输出的数据量，单位为比特每秒（bps），影响视频质量和传输带宽需求。

#### Bitrate Mode；比特率模式
编码器控制输出码率的策略，包括恒定比特率、可变比特率、恒定质量等模式，影响编码质量和文件大小的平衡。

#### SQR (Stable Quality Rate)；质量稳定码控
一种码控模式，编码质量由质量稳定因子和最大码率共同决定，适用于对质量和码率同时有稳定要求的场景。

#### PTS (Presentation Time Stamp)；显示时间戳
音视频帧在播放时应该显示的时间标记，用于同步音视频流和维持正确的播放顺序。

### 颜色空间术语

#### Color Primary；色域
视频信号中定义的颜色范围，如BT.709、BT.2020等，决定了视频可以呈现的颜色种类和饱和度范围。

#### Transfer Characteristic；传递函数
视频信号中亮度值与实际亮度之间的转换关系，如Gamma曲线、PQ曲线、HLG曲线等，影响视频的动态范围表现。

#### Matrix Coefficient；矩阵系数
用于YUV与RGB颜色空间转换的矩阵参数，定义了亮度分量与色度分量的组合方式。

#### HDR Vivid
中国自主知识产权的高动态范围视频标准，通过动态元数据实现场景级亮度优化，提供更丰富的明暗细节和色彩表现。

### 解码模式术语

#### Sync Mode；同步模式
编解码器的阻塞式工作模式，应用线程直接等待编解码操作完成，适用于低延迟或简单场景。

#### Decoding Order Output；解码顺序输出
解码器按照解码顺序而非显示顺序输出帧的特性，适用于需要低延迟或特定处理流程的场景。

#### Frame Retention Mode；帧保留模式
视频解码器控制输出帧保留策略的模式，包括全保留、均匀保留、自适应保留等，用于控制内存占用和播放流畅度。

#### Low Latency；低时延
编解码器优化策略，通过减少缓冲和优化处理流程降低端到端延迟，适用于实时通信等对延迟敏感的场景。

### 前处理术语

#### Preprocessing；编码前处理
视频编码前对原始帧进行的处理操作，包括降采样、裁剪、丢帧等，用于降低编码复杂度或适应特定分辨率要求。

#### Downsampling；降采样
降低视频帧分辨率的处理过程，在编码前减小图像尺寸以降低编码计算量和输出码率。

#### Crop；裁剪
截取视频帧中特定区域的处理过程，去除不需要的部分以聚焦关键内容或适应特定画面比例。

#### ROI (Region of Interest) Encoding；感兴趣区域编码
对视频中特定区域采用不同量化参数的编码技术，关键区域高质量编码，非关键区域低质量编码，在有限码率下优化主观质量。

### 音频编码术语

#### Sample Format；采样格式
音频数据的存储格式，定义每个采样点的位数和数据类型，如8位无符号、16位有符号、32位浮点等。

#### PTS Mode；PTS模式
音频编码器生成显示时间戳的策略，包括基于输入时间戳、基于采样数等不同模式。

#### Soundbed；声床
Audio Vivid三维音频中的基础声道布局，提供空间音频的背景声场。

#### Audio Object；音频对象
Audio Vivid三维音频中的独立声音实体，具有确定的空间位置和运动轨迹，可在三维空间中自由移动。

### 其他术语

#### NativeWindow；原生窗口
图形系统的底层窗口对象，用于Surface模式下的视频数据传输，支持零拷贝的高效渲染。

#### Scaling Mode；缩放模式
视频显示时的缩放策略，包括自适应缩放、裁剪缩放等，影响视频画面在目标窗口中的显示效果。

#### Seek Mode；跳转模式
媒体播放时定位到指定时间点的策略，包括跳转到下一同步帧、上一同步帧、最近同步帧等模式。
