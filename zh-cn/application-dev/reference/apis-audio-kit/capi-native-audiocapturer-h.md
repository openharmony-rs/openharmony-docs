...
异步请求启动内录流。该函数是非阻塞的，意味着系统在接收到启动请求后，将继续处理用户授权和内录流的启动。<br>最终结果将通过回调函数返回。该API最初仅对特定系统应用可用，但从API版本26.0.0开始，支持任意应用使用。
