...
| MUTE_WHEN_INTERRUPTED<sup>24+</sup> | 0x00000002 | 当系统需要停止或暂停音频流时，执行强制静音替代。<br/>通过接口[setAudioSessionBehavior](./arkts-apis-audio-AudioSessionManager.md#setaudiosessionbehavior24)设置该行为的同时，也需要调用接口[setAudioSessionScene](./arkts-apis-audio-AudioSessionManager.md#setaudiosessionscene20)使其生效。<br/>在音频会话场景下，应用将收到[AudioSessionStateChangeHint](./arkts-apis-audio-e.md#audiosessionstatechangehint20).AUDIO_SESSION_STATE_CHANGE_HINT_MUTE通知（静音时）和[AudioSessionStateChangeHint](./arkts-apis-audio-e.md#audiosessionstatechangehint20).AUDIO_SESSION_STATE_CHANGE_HINT_UNMUTE通知（恢复时）。<br/>在AudioRenderer和AudioCapturer场景下，应用将收到[InterruptHint](#interrupthint).INTERRUPT_HINT_MUTE通知（静音时），和[InterruptHint](#interrupthint).INTERRUPT_HINT_UNMUTE通知（恢复时）。<br/>**注意：** 该标志不能与PAUSE_WHEN_INTERRUPTED共存，若同时设置，仅PAUSE_WHEN_INTERRUPTED生效。 |
| PAUSE_WHEN_INTERRUPTED | 0x00000004 | 当系统需要停止音频流时，执行暂停替代。<br/>通过接口[setAudioSessionBehavior](./arkts-apis-audio-AudioSessionManager.md#setaudiosessionbehavior24)设置该行为的同时，也需要调用接口[setAudioSessionScene](./arkts-apis-audio-AudioSessionManager.md#setaudiosessionscene20)使其生效。<br/>在音频会话场景下，应用将收到[AudioSessionStateChangeHint](./arkts-apis-audio-e.md#audiosessionstatechangehint20).AUDIO_SESSION_STATE_CHANGE_HINT_PAUSE通知（暂停时）和[AudioSessionStateChangeHint](./arkts-apis-audio-e.md#audiosessionstatechangehint20).AUDIO_SESSION_STATE_CHANGE_HINT_RESUME通知（恢复时）。<br/>在AudioRenderer和AudioCapturer场景下，应用将收到[InterruptHint](#interrupthint).INTERRUPT_HINT_PAUSE通知（暂停时），和[InterruptHint](#interrupthint).INTERRUPT_HINT_RESUME通知（恢复时）。<br/>**注意：** 该标志不能与MUTE_WHEN_INTERRUPTED共存，若同时设置，仅该标志生效。<br/>**起始版本：** 26.0.0 |