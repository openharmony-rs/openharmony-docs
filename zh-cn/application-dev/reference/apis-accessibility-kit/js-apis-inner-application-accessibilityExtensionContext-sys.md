...
| 类型                  | 说明               |
| ------------------- | ---------------- |
| Promise&lt;void&gt; | Promise对象，拉起前台页面成功时无返回结果，失败时抛出异常。 |
...
| 类型                                  | 说明                     |
| ----------------------------------- | ---------------------- |
| Promise<Array&lt;[AccessibilityElement](#accessibilityelement)&gt;> | Promise对象，查询成功时返回当前窗口或者当前节点下的所有子节点列表，失败时抛出错误对象。 |
...
查询应用自定义设置的默认焦点元素ID列表。默认焦点是指窗口打开时无障碍服务优先聚焦的元素。使用Promise异步回调。
...
获取当前获得无障碍焦点的元素。无障碍焦点是指无障碍服务当前聚焦的节点，与输入焦点不同。使用Promise异步回调。
...
获取当前活动窗口的无障碍节点树根元素。活动窗口是指当前获得焦点的前台应用窗口。使用Promise异步回调。
...
根据elementId查询当前活动窗口下的节点元素。与[findElementById](#findelementbyid20)均根据元素ID查找节点元素，功能等价，推荐优先使用findElementById。使用Promise异步回调。
...
| 参数名         | 类型                                     | 必填   | 说明             |
| ----------- | ---------------------------------------- | ---- | -------------- |
| callback | AsyncCallback&lt;number&gt; | 是    | 回调函数，用于接收文本组件中光标位置。回调参数为number类型，表示光标在文本中的位置索引。|
...
| 类型                  | 说明               |
| ------------------- | ---------------- |
| Promise&lt;void&gt; | Promise对象，操作执行成功时无返回结果，失败时抛出异常。 |
...
根据焦点方向查找元素。与[findElementsByCondition](#findelementsbycondition23)相比，本方法主要用于查找Web组件；findElementsByCondition主要用于查找UI组件。使用Promise异步回调。
...
根据元素 ID 查找当前活动窗口下的节点元素。与[findElement('elementId')](#findelementelementid)功能等价，推荐优先使用本方法。使用Promise异步回调。
...
查询满足条件的可聚焦节点。与[findElementByFocusDirection](#findelementbyfocusdirection20)相比，本方法主要用于查找UI组件；findElementByFocusDirection主要用于查找Web组件。使用Promise异步回调。
