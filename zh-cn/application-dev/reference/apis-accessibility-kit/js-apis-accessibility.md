...
| 参数名   | 类型                   | 必填 | 说明                                                         |
| -------- | ---------------------- | ---- | ------------------------------------------------------------ |
| callback | Callback&lt;boolean&gt; | 否   | 回调函数。取消指定callback对象的事件响应。需与[accessibility.onSeniorModeStateChange](#accessibilityonSeniorModeStateChange26)的callback一致。缺省时，表示注销所有已注册事件。 |
