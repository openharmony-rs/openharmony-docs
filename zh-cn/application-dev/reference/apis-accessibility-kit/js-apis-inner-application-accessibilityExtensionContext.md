...
| offset<sup>12+</sup>             | number              | 否   | 否   | 对于可滚动类控件，如List、Grid，内容区相对控件的顶部坐标滚动的像素偏移量，单位px。默认值为0。 |
...
| 名称     | 类型     | 只读   | 可选   | 说明        |
| ------ | ------ | ---- | ---- | --------- |
| left   | number | 否    | 否    | 矩形区域的左边界，单位px。 |
| top    | number | 否    | 否    | 矩形区域的上边界，单位px。 |
| width  | number | 否    | 否    | 矩形区域的宽度，单位px。  |
| height | number | 否    | 否    | 矩形区域的高度，单位px。  |
