...
| 名称 | 类型 | 只读 | 可选 | 说明 |
| ------ | ------ | ------ | ------ | ------ |
| connectionId | ArkTS-Dyn: number<br>ArkTS-Sta: int | 否 | 否 | 连接的唯一标识符。 |
| bundleName | string | 否 | 否 | 调用方的应用包名。 |
| extensionOrigin | string | 否 | 否 | 调用方扩展的原始URL。 |
| fdRead | ArkTS-Dyn: number<br>ArkTS-Sta: int | 否 | 否 | 用于读取数据的管道文件描述符。 |
| fdWrite | ArkTS-Dyn: number<br>ArkTS-Sta: int | 否 | 否 | 用于写入数据的管道文件描述符。 |