...
设置Web过滚动模式。当过滚动模式开启时，当用户在Web根页面上滑动到边缘时，Web会通过弹性动画弹回界面，根页面上的内部页面不会触发回弹。若未显式调用该属性，关闭过滚动模式。
...
设置是否支持手势进行缩放。若未显式调用该属性，设置支持手势进行缩放。
...
设置是否开启数据库存储API权限。若未显式调用该属性，设置为不开启数据库存储API权限。
...
设置是否显示横向滚动条，包括系统默认滚动条和用户自定义滚动条。若未显式调用该属性，设置显示横向滚动条。
...
设置是否显示纵向滚动条，包括系统默认滚动条和用户自定义滚动条。若未显式调用该属性，设置显示纵向滚动条。
...
设置页面的文本缩放百分比。若未显式调用该属性，默认页面缩放比为100%
...
设置网页逻辑字体大小最小值。若未显式调用该属性，默认允许设置网页逻辑字体大小为8。
...
设置网页的serif字体库。若未显式调用该属性，默认允许设置网页的serif字体库为serif。
...
设置网页的standard font字体库。若未显式调用该属性，默认使用sans-serif设置网页的standard font字体库。
...
设置网页是否开启强制深色模式。该属性仅在[darkMode](#darkmode9)开启深色模式时生效。若未显式调用该属性，默认关闭强制深色模式。
...
设置网页是否开启捏合流畅模式。若未显式调用该属性，默认不开启。
...
设置Web媒体播放的策略，其中包括：Web中的音频在重新获焦后能够自动续播的有效期、应用内多个Web实例的音频是否独占、应用中Web音频类型。若未显式调用该属性，默认Web中的音频在重新获焦后不自动续播、应用内多个Web实例的音频不独占。
...
设置Web布局模式。若未显式调用该属性，默认允许使用跟随系统或自适应布局。常见问题请参考[Web组件大小自适应页面内容布局](../../web/web-fit-content.md)。
...
设置是否开启同层渲染功能。若未显式调用该属性，默认关闭此功能。
...
设置滚动条是否常驻。在常驻状态下，当页面大小超过一页时，滚动条出现且不消失。若未显式调用该属性，默认滚动条不常驻。
...
设置meta标签的viewport属性是否可用。若未显式调用该属性，默认允许解析iewport属性，并根据其布局。
...
设置使能文本自动调整大小。若未显式调用该属性，默认设置文本不自动调整大小。
...
开启[应用接管网页媒体播放功能](../../web/app-takeovers-web-media.md)。若未显式调用该属性，默认不开启该功能。
...
Web组件自定义软件键盘避让模式。若未显式调用该属性，默认开启软件键盘避让行为。
...
设置Web组件长按文本选择是否开启振动。 需配置"ohos.permission.VIBRATE"。若未显式调用该属性，默认开启振动。
...
设置当软键盘收起时Web元素失焦模式。若未显式调用该属性，软键盘收起时Web组件失焦功能关闭。
...
设置Web组件是否开启字重跟随系统设置变化。若未显式调用该属性，默认字重不再跟随系统设置中的字体粗细变化，系统设置改变时维持当前字重不变。
...
| 参数名        | 类型    | 必填   | 说明                   |
| ---------- | ------- | ---- | ---------------------- |
| optimizeParserBudget | ArkTS-Dyn: boolean<br/>ArkTS-Sta: boolean \|  undefined | 是    | 设置开启分段解析HTML优化。<br>true表示使用解析个数代替解析时间作为HTML分段解析的分段点，并减少每段解析的个数上限。false表示使用解析时间作为HTML分段解析的分段点。|
