...
| 类型     | 说明                 |
| ------ | ------------------ |
| ArkTS-Dyn: number<br>ArkTS-Sta: int | 获取成功时返回非负整数，失败时返回-1。<br>单位：px（物理像素）。 |
...
| 类型     | 说明                                       |
| ------ | ---------------------------------------- |
| ArkTS-Dyn: number<br>ArkTS-Sta: int | 获取网页元素可编辑标识，参照[ContextMenuEditStateFlags](./arkts-basic-components-web-e.md#contextmenueditstateflags9)。 |
...
| 类型     | 说明       |
| ------ | ----------- |
| ArkTS-Dyn: number<br>ArkTS-Sta: int | 存在预览图时返回预览图的宽，否则返回-1。<br>单位：px（物理像素）。 |
...
| 类型     | 说明       |
| ------ | ----------  |
| ArkTS-Dyn: number<br>ArkTS-Sta: int | 存在预览图时返回预览图的高，否则返回-1。<br>单位：px（物理像素）。 |
