...
>
> 通过[errorPageEvent.request.isMainFrame()](./arkts-basic-components-web-WebResourceRequest.md#ismainframe)判断请求来源是mainframe还是subframe，以便分别设置对应的自定义错误展示页。
    ...
    build() {
      Column() {
        Web({ src: "www.error-test.com", controller: this.controller })
          .onControllerAttached(() => {
            this.controller.setErrorPageEnabled(true, true);  // 此重载方法从API 26开始支持，若兼容低版本请使用setErrorPageEnabled(true)
