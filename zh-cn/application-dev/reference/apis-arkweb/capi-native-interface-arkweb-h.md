...
| [void OH_ArkWebCookieManager_FetchCookieAsync(const char\* url, bool incognito, bool includeHttpOnly, bool includePartitionedCookies, OH_ArkWeb_OnCookieFetchCallback callback)](#oh_arkwebcookiemanager_fetchcookieasync) | - | 获取指定URL对应的cookies。使用异步回调。在不初始化CookieManager接口的情况下，此接口将在UI线程上自动执行。<br>**起始版本：** 26.0.0 |
...
获取指定URL对应的cookies。使用异步回调。在不初始化CookieManager接口的情况下，此接口将在UI线程上自动执行。
