...
获取指定URL对应的cookie值。用于用户登录状态维护、会话管理、个性化配置读取等场景。默认情况下，该方法需在UI线程调用，调用前建议校验函数指针的可用性。从API版本26.0.1开始，调用[OH_NativeArkWeb_LazyInitializeWebEngineInCookieManager](capi-native-interface-arkweb-h.md#oh_nativearkweb_lazyinitializewebengineincookiemanager)设置为true后，支持在非UI线程调用。
...
设置指定URL的cookie值。用于保存用户偏好设置、维持登录状态、会话信息保存等场景。默认情况下，该方法需在UI线程调用，调用前建议校验函数指针的可用性。从API版本26.0.1开始，调用[OH_NativeArkWeb_LazyInitializeWebEngineInCookieManager](capi-native-interface-arkweb-h.md#oh_nativearkweb_lazyinitializewebengineincookiemanager)设置为true后，支持在非UI线程调用。
...
检查cookie是否存在。用于判断用户是否已登录、检查会话是否有效、验证身份状态等场景。默认情况下，该方法需在UI线程调用，调用前建议校验函数指针的可用性。从API版本26.0.1开始，调用[OH_NativeArkWeb_LazyInitializeWebEngineInCookieManager](capi-native-interface-arkweb-h.md#oh_nativearkweb_lazyinitializewebengineincookiemanager)设置为true后，支持在非UI线程调用。
...
清除所有会话cookies。用于清除临时会话数据、关闭所有会话、会话超时清理等场景。默认情况下，该方法需在UI线程调用，调用前建议校验函数指针的可用性。从API版本26.0.1开始，调用[OH_NativeArkWeb_LazyInitializeWebEngineInCookieManager](capi-native-interface-arkweb-h.md#oh_nativearkweb_lazyinitializewebengineincookiemanager)设置为true后，支持在非UI线程调用。
