...
| similarity | ArkTS-Dyn: number<br>ArkTS-Sta: double | 否   | 否   | 首屏相似度，根据历史加载首屏内容计算相似度，范围为[0, 1.0]，1.0表示完全一致，数值越接近1，相似度越高。该值存在滞后性，本地加载的相似度将在下次加载时才可反映。建议应用根据自身业务需求确定相似度阈值，当相似度低于该阈值时不启用无白屏加载插帧方案。 |
