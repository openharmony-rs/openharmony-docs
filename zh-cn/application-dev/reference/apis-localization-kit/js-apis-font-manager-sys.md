...
| 名称     | 类型 | 只读 | 可选    | 说明  |
| -------- | ---------------|--------|---------|-------------------------------- |
| timeRemaining | ArkTS-Dyn: number<br>ArkTS-Sta: int  |是 | 否 | 预计剩余时间，可能因设备性能、文件大小、系统负载等因素而有所差异。<br>取值范围为非负整数，最小值为0。<br>单位为s。    |
| progressPercentage | ArkTS-Dyn: number<br>ArkTS-Sta: int |是 | 否 | 数据迁移百分比进度，进度值根据已迁移的字体文件数量或大小计算，可能不是均匀增长。当progressPercentage为100时，迁移任务即将完成，onResult回调即将被调用。<br>取值范围为[0, 100]。|