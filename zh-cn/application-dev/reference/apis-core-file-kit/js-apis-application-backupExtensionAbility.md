...
| 名称 | 类型   | 只读 | 可选 | 说明             |
| ---- | ------ | ---- | --- | ---------------- |
| code | ArkTS-Dyn: number<br>ArkTS-Sta: long | 否   | 否  | 应用的版本号。   |
...
ArkTS-Dyn: onRelease(scenario: number): Promise&lt;void&gt;

ArkTS-Sta: onRelease(scenario: int): Promise&lt;void&gt;
...
| 参数名        | 类型                            | 必填 | 说明                           |
| ------------- | ------------------------------- | ---- | ------------------------------ |
| scenario | ArkTS-Dyn: number<br>ArkTS-Sta: int | 是   | 表示处于备份或恢复场景。<br>scenario = 1表示当前为备份场景。<br>scenario = 2表示当前为恢复场景。|
...
const SCENARIO_BACKUP: int = 1;
const SCENARIO_RESTORE: int = 2;
// 需要清理的临时目录
let filePath: string = '/data/storage/el2/base/.temp/';

class BackupExt extends BackupExtensionAbility {
  async onRelease(scenario: int): Promise<void> {
    try {
      if (scenario == SCENARIO_BACKUP) {
        // 备份场景，应用自行实现处理，以清理备份产生的临时文件为例
        console.info(`onRelease begin`);
