...
定义带占位符支持的云盘同步文件夹。必须将版本字段设置为有效的版本宏(例如 [OH_CLOUD_DISK_SYNC_FOLDER_EX_VERSION_1](capi-oh-cloud-disk-manager-h.md#OH_CLOUD_DISK_SYNC_FOLDER_EX_VERSION_1))，然后才能传递结构到任何API。运行时使用版本来确定字段有效；当指定低版本时，在较高版本中引入的字段将被忽略。
...
| 名称 | 描述 |
| -- | -- |
| uint32_t version | 指示此结构体的版本。必须初始化为有效的版本宏，例如 [OH_CLOUD_DISK_SYNC_FOLDER_EX_VERSION_1](capi-oh-cloud-disk-manager-h.md#OH_CLOUD_DISK_SYNC_FOLDER_EX_VERSION_1)。<br>**起始版本：** 26.1.0 |
