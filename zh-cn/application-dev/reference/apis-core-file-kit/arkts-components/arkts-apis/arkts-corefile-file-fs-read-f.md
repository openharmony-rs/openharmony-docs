...
| options | [ReadOptions](arkts-corefile-file-fs-readoptions-i.md) | 否 | 支持如下选项：<br>- offset，number类型，表示期望读取文件的位置，单位为Byte。可选，默认从当前位置开始读。<br>- length，number类型，表示期望读取数据的长度，单位为Byte。可选，默认缓冲区长度。<br>**适用版本：** 11 |
...
| fd | number | 是 | 已打开的文件描述符fd。 |
| buffer | ArrayBuffer | 是 | 用于保存读取到的文件数据的缓冲区。 |
| options | [ReadOptions](arkts-corefile-file-fs-readoptions-i.md) | 是 | 支持如下选项：<br>- offset，number类型，表示期望读取文件的位置，单位为Byte。可选，默认从当前位置开始读。<br>- length，number类型，表示期望读取数据的长度，单位为Byte。可选，默认缓冲区长度。<br>**适用版本：** 11 |
