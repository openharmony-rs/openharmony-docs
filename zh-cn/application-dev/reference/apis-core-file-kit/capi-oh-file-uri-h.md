...
| 名称 | 描述 |
| -- | -- |
| [FileManagement_ErrCode OH_FileUri_GetUriFromPath(const char *path, unsigned int length, char **result)](#oh_fileuri_geturifrompath) | 通过传入的路径path生成应用自己的uri；<br> 将path转uri时，路径中的中文及非数字字母的特殊字符将会编码成对应的ASCII码，拼接在uri中。 |
...
通过传入的路径path生成应用自己的uri；<br> 将path转uri时，路径中的中文及非数字字母的特殊字符将会编码成对应的ASCII码，拼接在uri中。
