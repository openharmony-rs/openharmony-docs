...
| waitTime    | number |  否  |  否  | 触发类型为0时代表执行碎片清理功能最大允许时间，超过此时间认为任务超时。单位：s，取值范围：[0, 300]，超出范围返回参数错误。<br>触发类型为1、2时不关注该参数。<br>触发类型为3时代表IO存储优化的间隔查询时间，每次达到间隔时间后触发一次查询IO空间换性能策略。单位：ms，取值范围：[0, +∞）。||
