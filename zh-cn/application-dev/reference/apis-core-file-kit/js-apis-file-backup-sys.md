...
| 名称        | 类型   | 只读 | 可选 | 说明                                                   |
| ----------- | ------ | ---- | ---- | ------------------------------------------------------ |
| triggerType | number |  否  |  否  | API版本23仅支持触发类型0，指定碎片清理的触发类型，表示执行存储器件碎片清理功能，清理碎片空间以改善存储性能，其他取值返回参数错误。<br>API版本26.0.0开始，支持指定IO存储优化的触发类型1、2和3。<br>1代表打开IO空间换性能策略；<br>2代表关闭IO空间换性能策略；<br>3代表查询IO空间换性能策略。<br>其他取值返回参数错误。|
| writeSize   | number |  否  |  否  | 触发类型为0时代表碎片清理功能的清理目标，预期可清理出目标大小的可用存储单元。单位：MB，取值范围：0-2097152MB，超出范围返回参数错误。<br>触发类型为1、2和3时代表IO存储优化的目标，预期目标大小超过多少时进行存储优化。单位：MB，取值范围：0-2097152MB，超出范围返回参数错误。|
| waitTime    | number |  否  |  否  | 触发类型为0时代表执行碎片清理功能最大允许时间，超过此时间认为任务超时。单位：秒，取值范围：0-300秒，超出范围返回参数错误。<br>触发类型为1、2时不关注该参数，触发类型为3时代表IO存储优化的间隔查询时间，每次达到间隔时间后触发一次查询IO空间换性能策略。单位：毫秒，取值范围：无限制。|
