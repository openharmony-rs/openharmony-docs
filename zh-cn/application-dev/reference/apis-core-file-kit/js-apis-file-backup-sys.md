...
| 名称        | 类型   | 只读 | 可选 | 说明                                                   |
| ----------- | ------ | ---- | ---- | ------------------------------------------------------ |
| triggerType | number |  否  |  否  | API版本23仅支持触发类型0，指定碎片清理的触发类型，表示执行存储器件碎片清理功能，清理碎片空间以改善存储性能，其他取值返回参数错误。<br>API版本26.0.1开始，支持指定IO存储优化的触发类型1、2和3。<br>1代表打开IO空间换性能策略；<br>2代表关闭IO空间换性能策略；<br>3代表查询IO空间换性能策略；<br>其他取值返回参数错误。|
