...
该方法返回迭代器对象RootIterator，然后通过[next](arkts-corefile-fileaccess-fileiterator-i-sys.md#next-1)方法返回[RootInfo](arkts-corefile-fileaccess-rootinfo-i-sys.md#RootInfo)。
...
callback带回迭代器对象RootIterator，然后通过[next](arkts-corefile-fileaccess-fileiterator-i-sys.md#next-1)方法返回
...
| uri | string | 是 | Indicates the selected file or directory. |
| displayName | string | 是 | Indicates the new directory or file name. |
| callback | AsyncCallback&lt;string&gt; | 是 | The callback is used to return a URI representing the new file or<br/>directory. |
