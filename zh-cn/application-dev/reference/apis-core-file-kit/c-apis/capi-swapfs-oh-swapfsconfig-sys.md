...
| 名称 | 描述 |
| -- | -- |
| const char *swapRootPath | 存储换出数据的根路径。设置不同的根路径会影响swapfs数据的存储位置和访问性能，确保路径具有足够的存储空间和读写权限。 swapfs使用固定的 “swapfs” 子目录。当为空指针或空字符串时，使用应用沙箱下的默认临时根路径 “/data/storage/el2/base/temp/swapfs”。<br>**起始版本：** 26.0.0 |
