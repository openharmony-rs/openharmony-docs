...
| 名称 | 描述 |
| -- | -- |
| [OH_Crypto_ErrCode OH_CryptoKdfParams_Create(const char *algoName, OH_CryptoKdfParams **params)](#oh_cryptokdfparams_create) | 创建密钥派生函数（KDF）参数。<br> 注意：创建的资源必须通过[OH_CryptoKdfParams_Destroy](#oh_cryptokdfparams_destroy)销毁。 |
...
| [OH_Crypto_ErrCode OH_CryptoKdf_Create(const char *algoName, OH_CryptoKdf **ctx)](#oh_cryptokdf_create) | 创建密钥派生函数（KDF）实例。<br> 注意：创建的资源必须通过[OH_CryptoKdf_Destroy](#oh_cryptokdf_destroy)销毁。 |
...
创建密钥派生函数（KDF）参数。<br> 注意：创建的资源必须通过[OH_CryptoKdfParams_Destroy](#oh_cryptokdfparams_destroy)销毁。
...
创建密钥派生函数（KDF）实例。<br> 注意：创建的资源必须通过[OH_CryptoKdf_Destroy](#oh_cryptokdf_destroy)销毁。
