...
[createSign(algName: string): Sign](arkts-cryptoarchitecture-cryptoframework-createsign-f.md#createSign-1)方法构造此实例。按序调用本类中的init、update、sign
...
| callback | AsyncCallback&lt;DataBlob&gt; | 是 | 回调函数。当签名成功时，err为undefined，data为获取到的签名结果DataBlob；否则<br/>为错误对象。 |
...
必须在对[Sign](arkts-cryptoarchitecture-cryptoframework-sign-i.md#Sign)实例使用[init](arkts-cryptoarchitecture-cryptoframework-sign-i.md#init-1)或
[initSync](arkts-cryptoarchitecture-cryptoframework-sign-i.md#initSync-1)初始化后，才能使用本函数。

> **说明：**
>
> 根据数据量，可以不调用update（即[init](arkts-cryptoarchitecture-cryptoframework-sign-i.md#init-1)完成后直接调用
> [sign](arkts-cryptoarchitecture-cryptoframework-sign-i.md#Sign)）或多次调用update。
>
...
在使用本函数前，必须先使用[Sign](arkts-cryptoarchitecture-cryptoframework-sign-i.md#Sign)方法对
[init()](arkts-cryptoarchitecture-cryptoframework-sign-i.md#init-1)实例进行初始化。

> **说明：**
>
> 根据数据量，可以不调用update（即[init](arkts-cryptoarchitecture-cryptoframework-sign-i.md#init-1)
> 完成后直接调用[sign](arkts-cryptoarchitecture-cryptoframework-sign-i.md#sign-2)）
> 或多次调用update。
>
...
必须在对[Sign](arkts-cryptoarchitecture-cryptoframework-sign-i.md#Sign)实例使用[initSync()](arkts-cryptoarchitecture-cryptoframework-sign-i.md#initSync-1)初始化
后，才能使用本函数。
