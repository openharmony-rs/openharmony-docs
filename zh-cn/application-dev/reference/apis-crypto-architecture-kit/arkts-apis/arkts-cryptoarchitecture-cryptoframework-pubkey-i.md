...
公钥，是[Key](arkts-cryptoarchitecture-cryptoframework-key-i.md#Key)的子类，在非对称加解密、验签、密钥协商时需要将其对象作为输入使用。

公钥可以通过非对称密钥生成器[AsyKeyGenerator](arkts-cryptoarchitecture-cryptoframework-asykeygenerator-i.md#AsyKeyGenerator)、
...
| [801](../../errorcode-universal.md#801-该操作不支持) | 该操作不支持。&lt;br&gt;**适用版本：** 12+ |
...
> 1. 本接口可以指定获取密钥数据的格式。
> 2. [Key.getEncoded()](arkts-cryptoarchitecture-cryptoframework-key-i.md#getEncoded-1)不支持指定获取密钥数据的格式，需要与原始数据的格式一致，即与
> [convertKey](arkts-cryptoarchitecture-cryptoframework-asykeygenerator-i.md#convertKey-1)生成的密钥对象的格式一致。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| format | string | 是 | 用于指定当前密钥格式。<br/>在API版本12-24，取值仅支持"X509\|COMPRESSED"和"X509\|UNCOMPRESSED"。<br/><br/>从API版本26.0.0开始，RSA公钥格式取值支持"PKCS1"和"X509"。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| format | string | 是 | 指定的获取密钥字符串的编码格式。支持RSA密钥，格式可以是'X509'或'PKCS1'。自API版本26.0.0起，支持<br/>EC密钥，格式可以是'X509'。 |
