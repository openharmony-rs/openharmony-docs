...
| 名称                 | 类型   | 只读 | 可选   | 说明                                       |
| ------------------ | ------ | ---- | ---- | ---------------------------------------- |
| deviceId           | string | 否 | 否    | client端蓝牙设备地址。例如："XX:XX:XX:XX:XX:XX"。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
| transId            | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否 | 否    | client端读请求的标识符，server端回复时需填写相同的transId。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22       |
| offset             | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否 | 否    | client端读数据的偏移值。例如：k表示从第k个字节开始读。<br/>server端回复响应时需填写相同的offset。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22 |
| characteristicUuid | string | 否 | 否    | client端需要读取的特征值UUID。例如：00002a11-0000-1000-8000-00805f9b34fb。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
| serviceUuid        | string | 否 | 否    | 特征值所属的服务UUID。例如：00001888-0000-1000-8000-00805f9b34fb。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
...
| ------------------ | ------ | ---- | ---- | ---------------------------------------- |
| deviceId           | string | 否 | 否    | client端蓝牙设备地址。例如："XX:XX:XX:XX:XX:XX"。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22  |
| transId            | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否 | 否    | client端写请求的标识符，server端回复时需填写相同的transId。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22        |
| offset             | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否 | 否    | client端写数据的偏移值。例如：k表示从第k个字节开始写。<br/>server端回复时需填写相同的offset。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22  |
| isPrepared             | boolean | 否 | 否    | 收到client端写请求后，是否立即回复。<br/>true表示稍后回复，false表示立即回复。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22  |
| needRsp             | boolean | 否 | 否    | 是否需要回复client端。<br/>true表示需要回复，false表示不需要回复。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22  |
| value             | ArrayBuffer | 否 | 否    | client端需要给特征值写入的数据。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22  |
| characteristicUuid | string | 否 | 否    | client端需要写入的特征值UUID。例如：00002a11-0000-1000-8000-00805f9b34fb。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22  |
| serviceUuid        | string | 否 | 否    | 特征值所属的服务UUID。例如：00001888-0000-1000-8000-00805f9b34fb。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22  |
...
| transId            | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否 | 否    | client端读请求的标识符，server端回复时需填写相同的transId。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22       |
| offset             | ArkTS-Dyn: number<br/>ArkTS-Sta: int | 否 | 否    | client端读数据的偏移值。例如：k表示从第k个字节开始读。<br/>server端回复响应时需填写相同的offset。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22 |
| descriptorUuid     | string | 否 | 否    | client端需要读取的描述符UUID。例如：00002902-0000-1000-8000-00805f9b34fb。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
| characteristicUuid | string | 否 | 否    | 描述符所属的特征值UUID。例如：00002a11-0000-1000-8000-00805f9b34fb。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
| serviceUuid        | string | 否 | 否    | 特征值所属的服务UUID。例如：00001888-0000-1000-8000-00805f9b34fb。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
...
| isPrepared         | boolean     | 否 | 否    | 收到client端写请求后，是否立即回复。<br/>true表示稍后回复，false表示立即回复。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22                             |
| needRsp            | boolean     | 否 | 否    | 是否需要回复client端。<br/>true表示需要回复，false表示不需要回复。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22                       |
| value              | ArrayBuffer | 否 | 否    | client端需要给描述符写入的数据。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22                           |
| descriptorUuid     | string      | 否 | 否    | client端需要写入的描述符UUID。例如：00002902-0000-1000-8000-00805f9b34fb。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
| characteristicUuid | string      | 否 | 否    | 描述符所属的特征值UUID。例如：00002a11-0000-1000-8000-00805f9b34fb。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
| serviceUuid        | string      | 否 | 否    | 特征值所属的服务UUID。例如：00001888-0000-1000-8000-00805f9b34fb。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
...
| -------- | ----------- | ---- |  ---- | -------------------------------------- |
| deviceId | string      | 否 | 否    | client端蓝牙设备地址。例如："XX:XX:XX:XX:XX:XX"。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22       |
| transId  | ArkTS-Dyn: number<br/>ArkTS-Sta: int      | 否 | 否    | 收到client端请求的标识符，与订阅client端读或者写请求事件携带的transId保持一致。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22        |
| status   | ArkTS-Dyn: number<br/>ArkTS-Sta: int      | 否 | 否    | 响应的状态，设置为0即可，表示正常。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22                   |
| offset   | ArkTS-Dyn: number<br/>ArkTS-Sta: int      | 否 | 否    | client端读或者写请求的数据偏移值，与订阅client端读或者写请求事件携带的offset保持一致。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22 |
| value    | ArrayBuffer | 否 | 否    | 回复的数据。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22                          |
...
| 名称          | 类型    | 只读 | 可选   | 说明                                       |
| ----------- | ------- | ---- | ---- | ---------------------------------------- |
| interval    | ArkTS-Dyn: number<br/>ArkTS-Sta: int  | 否 | 是    | 广播发送间隔。<br/>取值范围：[32, 16777215]，单位：slot（时间槽），一个slot代表0.625毫秒，默认值为1600。<br/>其中传统广播的最大值是16384。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22 |
| txPower     | ArkTS-Dyn: number<br/>ArkTS-Sta: int  | 否 | 是    | 广播发送功率。取值范围：[-127, 1]，单位：dBm，默认值为-7。<br/>考虑到发送广播的性能和功耗，建议高档取值为1，中档取为-7，低档取值为-15。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22   |
| connectable | boolean | 否 | 是    | 是否是可连接广播。true表示发送可连接广播，false表示发送不可连接广播，默认值为true。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22                   |
...
| 名称              | 类型                                     | 只读 | 可选   | 说明                          |
| --------------- | ---------------------------------------- | ---- | ---- | --------------------------- |
| serviceUuids    | Array&lt;string&gt;                      | 否 | 否    | 要携带的服务UUID。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
| manufactureData | Array&lt;[ManufactureData](#manufacturedata)&gt; | 否 | 否    | 要携带的制造商数据内容。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22           |
| serviceData     | Array&lt;[ServiceData](#servicedata)&gt; | 否 | 否    | 要携带的服务数据内容。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22               |
| includeDeviceName | boolean     | 否 | 是    | 是否携带蓝牙设备名称。true表示携带，false表示不携带，默认值为false。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22        |
...
| 名称                | 类型                             | 只读 | 可选  | 说明                      |
| ------------------- | ------------------------------- | ----- | ----- | ------------------------ |
| advertisingSettings | [AdvertiseSetting](#advertisesetting) | 否 | 否    | 广播的发送参数。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22     |
| advertisingData    | [AdvertiseData](#advertisedata) | 否 | 否    | 需要发送的广播报文数据内容。 <br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22      |
| advertisingResponse | [AdvertiseData](#advertisedata) | 否 | 是    | 回复扫描请求的广播报文数据内容。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22  |
...
| 名称        | 类型                    | 只读 | 可选   | 说明                                     |
| --------- | ----------------------- | ---- | ---- | -------------------------------------- |
| interval  | ArkTS-Dyn: number<br/>ArkTS-Sta: int                  | 否 | 是    | 扫描结果上报的延迟时间，单位：ms，默认值为0。搭配 [ScanReportMode](#scanreportmode15)使用。<br/>- 该值在常规或围栏扫描上报模式下不生效，当扫描到符合过滤条件的广播报文后，立刻上报。<br/>- 该值在批量扫描上报模式下生效，当扫描到符合过滤条件的广播报文后，会存入缓存队列，延时上报。若不设置该值或该值在[0, 5000)范围内，蓝牙子系统会默认设置延时时间为5000。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22                    |
| dutyMode  | [ScanDuty](#scanduty)   | 否 | 是    | 扫描模式，默认值为SCAN_MODE_LOW_POWER。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22        |
| matchMode | [MatchMode](#matchmode) | 否 | 是    | 硬件的过滤匹配模式，默认值为MATCH_MODE_AGGRESSIVE。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22 |
| phyType<sup>12+</sup> | [PhyType](#phytype12) | 否 | 是    | 扫描中使用的物理通道类型，默认值为PHY_LE_1M。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
...
| 名称       | 类型  | 只读 | 可选   | 说明          |
| -------- | ------ |---- |---- | ----------- |
| write    | boolean | 否 | 是  | 该特征值是否支持写入操作。<br/>true表示支持，且被写入时需要回复对端设备，false表示不支持。默认值为true。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
| writeNoResponse | boolean | 否 | 是   | 该特征值是否支持写入操作。<br/>true表示支持，且被写入时无需回复对端设备，false表示不支持。默认值为true。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
| read | boolean   | 否 |  是    | 该特征值是否支持读取操作。<br/>true表示支持，false表示不支持。默认值为true。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
| notify | boolean   | 否 | 是    | 该特征值是否支持主动向对端设备通知特征值内容。<br/>true表示支持，且对端设备不需要回复确认，false表示不支持。默认值为false。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22 |
| indicate | boolean   | 否 | 是    | 该特征值是否支持向对端设备指示特征值内容。<br/>true表示支持，对端设备需要回复确认，false表示不支持。默认值为false。<br/>**原子化服务API**：从API version 12开始，该接口支持在原子化服务中使用。<br/>**ArkTS-Dyn起始版本：** 12<br/>**ArkTS-Sta起始版本：** 22 |
...
| 名称      | 值    | 说明                           |
| --------  | ---- | ------------------------------ |
| STARTED<sup>11+</sup>   | 1    | 调用[startAdvertising](#blestartadvertising11)方法后，广播首次启动成功，且会分配相关资源。 <br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22      |
...
| STOPPED<sup>11+</sup>    | 4    | 调用[stopAdvertising](#blestopadvertising11)方法后，广播停止成功，且会释放首次启动广播时分配的相关资源。<br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22       |
...
| 名称      | 值    | 说明                           |
| --------  | ---- | ------------------------------ |
| ON_FOUND  | 1    | 扫描到符合过滤条件的BLE广播报文时，触发上报，可搭配常规和围栏上报模式使用。 <br/> **原子化服务API**：从API version 15开始，该接口支持在原子化服务中使用。 <br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22     |
| ON_LOST | 2    | 当不再扫描到符合过滤条件的BLE广播报文时，触发上报，只搭配围栏上报模式使用。 <br/> **原子化服务API**：从API version 15开始，该接口支持在原子化服务中使用。 <br/>**ArkTS-Dyn起始版本：** 13<br/>**ArkTS-Sta起始版本：** 22    |
