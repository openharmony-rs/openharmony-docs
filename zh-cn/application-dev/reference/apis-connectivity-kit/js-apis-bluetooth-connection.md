...
ArkTS-Dyn: setBluetoothScanMode(mode: ScanMode, duration: number): void

ArkTS-Sta: setBluetoothScanMode(mode: ScanMode, duration: int): void
...
import { BusinessError } from '@ohos.base';
import connection from '@ohos.bluetooth.connection';
function BondStateParam(data: connection.BondStateParam) {
    console.info("bondStateChange on:" + JSON.stringify(data);
}
try {
...
function onReceiveEvent(data: Array<connection.DiscoveryResult>) {
    console.info('bluetooth discoveryResult result:'+ JSON.stringify(data));
}
try {
...
ArkTS-Sta: getLastConnectionTime(deviceId: string): Promise&lt;long&gt;
...
import connection from '@ohos.bluetooth.connection';
try {
    await connection.getLastConnectionTime("00:11:22:33:44:55").then((time: long) => {
