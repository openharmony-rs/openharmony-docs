...
适用于需要获知一组星闪设备（合作设备集合）的成员组成及连接状态变化并据此进行业务联动的场景。例如，手机与耳机配对后，手机可通过CDSM查询左右耳机信息并感知其连接状态变化。同一应用针对同一远端设备创建一个 [CdsmClient](arkts-connectivity-cdsm-cdsmclient-i.md) 实例即可，重复创建会增加不必要的资源开销。
