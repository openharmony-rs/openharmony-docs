...
getNfcA(tagInfo: TagInfo): [NfcATag](js-apis-nfctech.md#nfcatag)
...
ArkTS-Dyn: registerForegroundDispatch(elementName: ElementName, discTech: number[], callback: AsyncCallback&lt;TagInfo&gt;): void

ArkTS-Sta: registerForegroundDispatch(elementName: ElementName, discTech: int[], callback: AsyncCallback&lt;TagInfo&gt;): void
...
| discTech         |  ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]   | 是   | 前台应用指定的NFC读卡技术类型，不可以为空，至少指定一种读卡技术类型。每个int或number表示所支持技术类型的常量值型，根据int或number值设置NFC读卡轮询的Tag技术类型（仅包含[NFC_A](#常量), [NFC_B](#常量), [NFC_F](#常量), [NFC_V](#常量)中的一种或多种）。 |
...
ArkTS-Dyn示例：
请参见[tag.unregisterForegroundDispatch](#tagunregisterforegrounddispatch10)接口的示例。
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[onReaderMode](#tagonreadermode23)。
...
| elementName   |  [ElementName](../apis-ability-kit/js-apis-bundleManager-elementName.md)   | 是   | 所属应用读卡的页面信息（至少包含bundleName、abilityName这两项的赋值），不可以为空。          |
| discTech         |  number[]   | 是   | 前台应用指定的NFC读卡技术类型，不可以为空，至少指定一种读卡技术类型。每个int值表示所支持技术类型的常量值型，根据int值设置NFC读卡轮询的Tag技术类型（仅包含[NFC_A](#常量), [NFC_B](#常量), [NFC_F](#常量), [NFC_V](#常量)中的一种或多种）。 |
...
## tag.onReaderMode<sup>23+</sup>

onReaderMode(elementName: ElementName, discTech: int[], callback: AsyncCallback&lt;TagInfo&gt;): void
...
**相关接口：** 该接口对应的ArkTS-Dyn接口是[on](#tagon11)。
...
| ------------ | -------- | ---- | ------------------------------------------------------- |
| elementName   |  [ElementName](../apis-ability-kit/js-apis-bundleManager-elementName.md)   | 是   | 所属应用读卡的页面信息（至少包含bundleName、abilityName这两项的赋值），不可以为空。          |
| discTech         |  int[]   | 是   | 前台应用指定的NFC读卡技术类型，不可以为空，至少指定一种读卡技术类型。每个int值表示所支持技术类型的常量值型，根据int值设置NFC读卡轮询的Tag技术类型（仅包含[NFC_A](#常量), [NFC_B](#常量), [NFC_F](#常量), [NFC_V](#常量)中的一种或多种）。 |
| callback | AsyncCallback&lt;[TagInfo](#taginfo)&gt; | 是   | 读卡器模式监听回调函数，返回读到的Tag信息，不可以为空。 |
...
示例请参见[tag.offReaderMode](#tagoffReaderMode23)接口的示例。
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[offReaderMode](#tagoffreadermode23)。
...
## tag.offReaderMode<sup>23+</sup>

offReaderMode(elementName: ElementName, callback?: AsyncCallback&lt;TagInfo&gt;): void

取消订阅NFC Tag读卡事件。设备退出读卡模式，并恢复卡模拟。如果已通过[tag.onreadermode](#tagonreadermode23)设置NFC的读卡器模式，需要在页面退出前台或页面销毁时调用off进行取消。

**需要权限：** ohos.permission.NFC_TAG

**ArkTS模式：** 该接口仅适用于ArkTS-Sta。

**相关接口：** 该接口对应的ArkTS-Dyn接口是[off](#tagoff11)。
...
let discTech: Array<int> = [tag.NFC_A, tag.NFC_B]; // 用前台ability时所需要的技术代替
let elementName: bundleManager.ElementName;
...
on(type: 'readerModeWithInterval', elementName: ElementName, discTech: int[], callback: Callback&lt;TagInfo&gt;, interval: int): void
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[onreaderModeWithInterval](#tagonreadermodewithinterval23)。
...
| discTech         |  int[]   | 是   | 前台应用指定的NFC读卡技术类型，至少指定一种读卡技术类型。每个int值表示所支持技术类型的常量值型，根据number值设置NFC读卡轮询的Tag技术类型（仅包含[NFC_A](#常量), [NFC_B](#常量), [NFC_F](#常量), [NFC_V](#常量)中的一种或多种）。 |
...
## tag.onReaderModeWithInterval<sup>23+</sup>

onReaderModeWithInterval(elementName: ElementName, discTech: int[], callback: Callback&lt;TagInfo&gt;, interval: int): void
...
**相关接口：** 该接口对应的ArkTS-Dyn接口是[on](#tagon23)。
...
示例请参见[tag.offReaderModeWithInterval](#tagoffreadermodewithinterval23)接口的示例。
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[offReaderModeWithInterval](#tagoffreadermodewithinterval23)。
...
## tag.offReaderModeWithInterval<sup>23+</sup>

offReaderModeWithInterval(elementName: ElementName, callback?: Callback&lt;TagInfo&gt;): void
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[off](#tagoff23)。
...
export default class MainAbility extends UIAbility {
    ...
    onForeground() {
        ...
        try {
            tag.onReaderModeWithInterval(elementName, discTech, (tagInfo: tag.TagInfo) => {
                if (!tagInfo) {
                    console.info(`ReaderModeWithInterval receive tagInfo`);
                } else {
                    console.error(`tag.onReaderModeWithInterval tagInfo null`);
                }
            }, interval);
        } catch (e) {
            console.error(`tag.on error: ${(e as BusinessError).message}`);
    ...
    onBackground() {
        ...
        try {
            tag.offReaderModeWithInterval(elementName);
        } catch (e) {
            console.error(`tag.off error:${(e as BusinessError).code}, error message: ${(e as BusinessError).message}`);
        }
    }
...
ArkTS-Dyn: makeMimeRecord(mimeType: string, mimeData: number[]): NdefRecord

ArkTS-Sta: makeMimeRecord(mimeType: string, mimeData: int[]): NdefRecord
...
| mimeData | ArkTS-Dyn: number[]<br>ArkTS-Sta: int[] | 是   | MIME数据内容，每个int或number十六进制表示，范围是0x00~0xFF。 |
...
try {
    let mimeType: string = "text/plain";   // 修改为预期的符合规则的MIME类型
    let mimeData: Array<int> = [0x01, 0x02, 0x03, 0x04]; // 修改为预期的符合格式的数据
    let ndefRecord: tag.NdefRecord = tag.ndef.makeMimeRecord(mimeType, mimeData);
    if (ndefRecord != undefined) {
        console.log("ndefMessage makeMimeRecord rtdType: " + ndefRecord.rtdType);
        console.log("ndefMessage makeMimeRecord payload: " + ndefRecord.payload);
...
ArkTS-Dyn: makeExternalRecord(domainName: string, type: string, externalData: number[]): NdefRecord

ArkTS-Sta: makeExternalRecord(domainName: string, type: string, externalData: int[]): NdefRecord
...
| externalData | ArkTS-Dyn: number[]<br>ArkTS-Sta: int[] | 是   | 外部数据内容，每个int或number十六进制表示，范围是0x00~0xFF。 |
...
try {
    let domainName: string = "ohos.nfc.application"; // 修改为符合规范的包名
    let type: string = "test"; // 修改为正确的数据类型
    let externalData: Array<int> = [0x01, 0x02, 0x03, 0x04]; // 修改为正确的外部数据内容
    let ndefRecord: tag.NdefRecord = tag.ndef.makeExternalRecord(domainName, type, externalData);
    if (ndefRecord != undefined) {
        console.log("ndefMessage makeExternalRecord rtdType: " + ndefRecord.rtdType);
        console.log("ndefMessage makeExternalRecord payload: " + ndefRecord.payload);
...
ArkTS-Dyn: messageToBytes(ndefMessage: NdefMessage): number[]

ArkTS-Sta: messageToBytes(ndefMessage: NdefMessage): int[]
...
| 类型 | 说明                                                                              |
| -------- | ------------------------------------------------------------------------------------- |
| ArkTS-Dyn: number[]<br>ArkTS-Sta: int[] | NDEF消息数据对象，所转换成的字节格式的数据。每个number或int十六进制表示，范围是0x00~0xFF。 |
...
let rawData: Array<int> = [0xD1, 0x01, 0x03, 0x54, 0x4E, 0x46, 0x43]; // 必须符合NDEF格式的数据
try {
    let ndefMessage: tag.NdefMessage = tag.ndef.createNdefMessageByData(rawData);
    console.log("ndef createNdefMessageByData, ndefMessage: " + ndefMessage);
    let rawData2: Array<int> = tag.ndef.messageToBytes(ndefMessage);
    console.log("ndefMessage messageToBytes rawData2: " + rawData2);
} catch (businessError: BusinessError) {
    console.error("ndef createNdefMessageByData businessError: " + businessError);
}
```
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[createNdefMessageByData](#tagndefcreatendefmessagebydata23)。
...
## tag.ndef.createNdefMessageByData<sup>23+</sup>

createNdefMessageByData(data: int[]): NdefMessage

使用原始字节数据创建NDEF标签的Message。该数据必须符合NDEF Record数据格式，如果不符合格式，则返回的NdeMessage数据对象，所包含的NDE Record列表会为空。

**ArkTS模式：** 该接口仅适用于ArkTS-Sta。

**相关接口：** 该接口对应的ArkTS-Dyn接口是[createNdefMessage](#tagndefcreatendefmessage9)。
...
| 参数名 | 类型 | 必填 | 说明                                                                           |
| ---------- | -------- | -------- | ---------------------------------------------------------------------------------- |
| data       | int[] | 是       | 原始字节，每个int十六进制表示，范围是0x00~0xFF。要求必须满足NDEF Record的格式。 |
...
let rawData: Array<int> = [0xD1, 0x01, 0x03, 0x54, 0x4E, 0x46, 0x43];  //必须是可以被解析的NDEF记录
try {
    let ndefMessage: tag.NdefMessage = tag.ndef.createNdefMessageByData(rawData);
    console.log("ndef createNdefMessageByData, ndefMessage: " + ndefMessage);
} catch (businessError: BusinessError) {
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[createNdefMessageByRecords](#tagndefcreatendefmessagebyrecords23)。
...
## tag.ndef.createNdefMessageByRecords<sup>23+</sup>

createNdefMessageByRecords(ndefRecords: NdefRecord[]): NdefMessage

使用NDEF Records列表，创建NDEF Message。

**ArkTS模式：** 该接口仅适用于ArkTS-Sta。

**相关接口：** 该接口对应的ArkTS-Sta接口是[createNdefMessage](#tagndefcreatendefmessage9-1)。
...
let uriRecord: tag.NdefRecord = tag.ndef.makeUriRecord("https://www.example.com");
let textRecord: tag.NdefRecord = tag.ndef.makeTextRecord("Hello World", "en");
let ndefRecords: Array<tag.NdefRecord> = [uriRecord, textRecord];
try {
    let ndefMessage: tag.NdefMessage = tag.ndef.createNdefMessageByRecords(ndefRecords);
    console.log("ndef createNdefMessageByRecords ndefMessage: " + ndefMessage);
} catch (businessError: BusinessError) {
    console.error("ndef createNdefMessageByRecords businessError: " + businessError);
}
```
...
| **名称**                      | **类型**                                                      | **只读** | **可选** | **说明**                                                                                     |
| ----------------------------- | ------------------------------------------------------------- | -------- | -------- | -------------------------------------------------------------------------------------------- |
| uid<sup>9+</sup>              | ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]                   | 否       | 否       | 标签的uid，每个number或int值是十六进制表示，范围是0x00~0xFF。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23       |
| technology<sup>9+</sup>       | ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]                   | 否       | 否       | 支持的技术类型，每个number或int值表示所支持技术类型的常量值。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。 <br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23     |
| supportedProfiles<sup>(deprecated)</sup>             | number[]                                                      | 否       | 否       | 支持的技术类型。<br>**说明：** 从API7开始支持，从API9开始废弃，使用[tag.TagInfo#technology](#taginfo)替代。<br>**ArkTS模式：** 该接口仅适用于ArkTS-Dyn。<br>**ArkTS-Dyn起始版本：** 7            |
...
| **名称** | **类型** | **只读** | **可选** | **说明**                               |
| -------- | -------- | -------- | -------- | ----------------------------------------------------------------------------------------- |
| tnf      | ArkTS-Dyn: number<br>ArkTS-Sta: int  | 是       | 否       | NDEF Record的TNF(Type Name Field)。     |
| rtdType  | ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]  | 是       | 否       | NDEF Record的RTD(Record Type Definition)类型值，每个number或int的值十六进制表示，范围是0x00~0xFF。  |
| id       | ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]  | 是       | 否       | NDEF Record的ID，每个number或int值用十六进制表示，范围是0x00~0xFF。     |
| payload  | ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]  | 是       | 否       | NDEF Record的PAYLOAD，每个number或int的值十六进制表示，范围是0x00~0xFF。    |
...
| **名称**                     |**类型**| **值** | **说明**                    |
| ---------------------------- | ------ | ------ | --------------------------- |
| NFC_A<sup>7+</sup>                        |  ArkTS-Dyn: number<br>ArkTS-Sta: int | 1      | NFC-A (ISO 14443-3A)技术。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 7<br>**ArkTS-Sta起始版本：** 23  |
| NFC_B<sup>7+</sup>                        |  ArkTS-Dyn: number<br>ArkTS-Sta: int | 2      | NFC-B (ISO 14443-3B)技术。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 7<br>**ArkTS-Sta起始版本：** 23  |
| ISO_DEP<sup>7+</sup>                      |  ArkTS-Dyn: number<br>ArkTS-Sta: int | 3      | ISO-DEP (ISO 14443-4)技术。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 7<br>**ArkTS-Sta起始版本：** 23 |
| NFC_F<sup>7+</sup>                        |  ArkTS-Dyn: number<br>ArkTS-Sta: int | 4      | NFC-F (JIS 6319-4)技术。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 7<br>**ArkTS-Sta起始版本：** 23    |
| NFC_V<sup>7+</sup>                        |  ArkTS-Dyn: number<br>ArkTS-Sta: int | 5      | NFC-V (ISO 15693)技术。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 7<br>**ArkTS-Sta起始版本：** 23     |
| NDEF<sup>7+</sup>                         |  ArkTS-Dyn: number<br>ArkTS-Sta: int | 6      | NDEF技术。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 7<br>**ArkTS-Sta起始版本：** 23                  |
| NDEF_FORMATABLE<sup>9+</sup> |  ArkTS-Dyn: number<br>ArkTS-Sta: int | 7      | 可以格式化的NDEF技术。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 7<br>**ArkTS-Sta起始版本：** 23       |
| MIFARE_CLASSIC<sup>7+</sup>               |  ArkTS-Dyn: number<br>ArkTS-Sta: int | 8      | MIFARE Classic技术。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 7<br>**ArkTS-Sta起始版本：** 23        |
| MIFARE_ULTRALIGHT<sup>7+</sup>            |  ArkTS-Dyn: number<br>ArkTS-Sta: int | 9      | MIFARE Utralight技术。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 7<br>**ArkTS-Sta起始版本：** 23      |
| NFC_BARCODE<sup>18+</sup>    |  ArkTS-Dyn: number<br>ArkTS-Sta: int | 10     | BARCODE技术。<br>**原子化服务API：** 从API version 18开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 18<br>**ArkTS-Sta起始版本：** 23               |
| RTD_TEXT<sup>9+</sup> |ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]| [0x54] | 文本类型的NDEF Record，参考NDEF标签技术规范《NFCForum-TS-NDEF_1.0》的定义细节。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23 |
| RTD_URI<sup>9+</sup>  |ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]| [0x55] | URI类型的NDEF Record，参考NDEF标签技术规范《NFCForum-TS-NDEF_1.0》的定义细节。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 23  |
...
| **名称**              |**类型**| **值** | **说明**                |
| --------------------- | ------ | ------ | ----------------------- |
| RTD_TEXT<sup>9+</sup> |ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]| [0x54] | 文本类型的NDEF Record。 |
| RTD_URI<sup>9+</sup>  |ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]| [0x55] | URI类型的NDEF Record。  |
