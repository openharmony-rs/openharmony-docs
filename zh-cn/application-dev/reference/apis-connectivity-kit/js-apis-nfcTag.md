...
**相关接口：** 该接口对应的ArkTS-Sta接口是[onReaderMode](#tagonreadermode22)。
...
示例请参见[tag.offReaderMode](#tagoffReaderMode22)接口的示例。
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[offReaderMode](#tagoffreadermode22)。
...
## tag.offReaderMode<sup>22+</sup>

offReaderMode(elementName: ElementName, callback?: AsyncCallback&lt;TagInfo&gt;): void

取消订阅NFC Tag读卡事件。设备退出读卡模式，并恢复卡模拟。如果已通过[tag.onreadermode](#tagonreadermode22)设置NFC的读卡器模式，需要在页面退出前台或页面销毁时调用off进行取消。
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[createNdefMessageByData](#tagndefcreatendefmessagebydata22)。
...
## tag.ndef.createNdefMessageByData<sup>22+</sup>

createNdefMessageByData(data: int[]): [NdefMessage](js-apis-nfctech.md#ndefmessage9)
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[createNdefMessageByRecords](#tagndefcreatendefmessagebyrecords22)。
...
## tag.ndef.createNdefMessageByRecords<sup>22+</sup>
...
| **名称**                      | 类型                                                      | **可读** | **可写** | 说明                                                                                     |
| ----------------------------- | ------------------------------------------------------------- | -------- | -------- | -------------------------------------------------------------------------------------------- |
| uid<sup>9+</sup>              | ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]                   | 是       | 否       | 标签的uid，每个number或int值是十六进制表示，范围是0x00~0xFF。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。<br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 22       |
| technology<sup>9+</sup>       | ArkTS-Dyn: number[]<br>ArkTS-Sta: int[]                   | 是       | 否       | 支持的技术类型，每个number或int值表示所支持技术类型的常量值。<br>**原子化服务API：** 从API version 12开始，该接口支持在原子化服务中使用。 <br>**ArkTS-Dyn起始版本：** 9<br>**ArkTS-Sta起始版本：** 22     |
