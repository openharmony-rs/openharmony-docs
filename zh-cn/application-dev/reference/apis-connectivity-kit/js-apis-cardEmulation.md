...
**相关接口：** 该接口对应的ArkTS-Dyn接口是[on](#on8)。
...
| 参数名   | 类型                    | 必填 | 说明                                         |
| -------- | ----------------------- | ---- | -------------------------------------------- |
| callback | AsyncCallback\<int[]> | 是   | 订阅的事件回调，入参是符合APDU协议的数据，每个int十六进制表示，范围是0x00~0xFF。 |
...
| 参数名   | 类型                    | 必填 | 说明                                         |
| -------- | ----------------------- | ---- | -------------------------------------------- |
| callback | AsyncCallback\<int[]> | 否   | 订阅的事件回调，每个int十六进制表示，范围是0x00~0xFF。 |
...
let hceService: cardEmulation.HceService = new cardEmulation.HceService();
let element: ElementName;
const apduCallback: AsyncCallback<int[]> = (err: BusinessError | null, data: Arry<int> | undefined): void => {
  // 处理数据和异常
  console.log("AsyncCallback got apdu data");
};
...
export default class EntryAbility extends UIAbility {
  onCreate(want: Want, param: AbilityConstant.LaunchParam) {
    ...
    hceService.onHceCmd(apduCallback);
  }
  onDestroy(): Promise<void> | undefined {
    hilog.info(0x0000, 'testHce', '%{public}s', 'Ability onDestroy');
    hceService.offHceCmd(apduCallback);
    hceService.stop(element);
    return undefined;
  }
  // 生命周期内的其它功能
...
| 参数名       | 类型     | 必填 | 说明                                               |
| ------------ | -------- | ---- | -------------------------------------------------- |
| response | ArkTS-Dyn: number[]<br>ArkTS-Sta: int[] | 是   | 发送到对端读卡设备的符合APDU协议的数据，十六进制表示，范围是0x00~0xFF。 |
...
// 应用程序实际想要发送的数据， 此处仅做为示例
const responseData: Array<int> = [0x1, 0x2];
hceService.transmit(responseData).then(() => {
  // 处理 promise 的回调
  console.log("transmit Promise success.");
...
try {
  ...
  hceService.transmit(responseData, (err: BusinessError | null): void => {
    if (err) {
      console.error(`transmit AsyncCallback err Code: ${err.code}, message: ${err.message}`);
    } else {
      console.log("transmit AsyncCallback success.");
