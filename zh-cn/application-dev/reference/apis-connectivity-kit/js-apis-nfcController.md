...
**相关接口：** 该接口对应的ArkTS-Dyn接口是[on](#nfccontrolleronnfcstatechange)。
...
**相关接口：** 该接口对应的ArkTS-Dyn接口是[off](#nfccontrolleroffnfcstatechange)。
...
// 注册回调以接收nfc状态更改通知
nfcController.onNfcStateChange((nfcState): void => {
  console.log("nfcController onNfcStateChange callback nfcState: " + nfcState);
});
...
// 取消注册回调
nfcController.offNfcStateChange();
```