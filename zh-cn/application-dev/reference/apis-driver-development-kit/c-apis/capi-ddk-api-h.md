...
| 名称 | 描述 |
| -- | -- |
| [DDK_RetCode OH_DDK_CreateAshmem(const uint8_t *name, uint32_t size, DDK_Ashmem **ashmem)](#oh_ddk_createashmem) | 创建共享内存。为了防止资源泄漏，通过调用[OH_DDK_DestroyAshmem](capi-ddk-api-h.md#oh_ddk_destroyashmem)接口来销毁不再需要的共享内存。 |
| [DDK_RetCode OH_DDK_MapAshmem(DDK_Ashmem *ashmem, const uint8_t ashmemMapType)](#oh_ddk_mapashmem) | 映射创建的共享内存到用户空间。通过调用[OH_DDK_UnmapAshmem](capi-ddk-api-h.md#oh_ddk_unmapashmem)接口取消映射不需要的共享内存。 |
...
创建共享内存。为了防止资源泄漏，通过调用[OH_DDK_DestroyAshmem](capi-ddk-api-h.md#oh_ddk_destroyashmem)接口来销毁不再需要的共享内存。
...
| [DDK_Ashmem](capi-ddk-ddk-ashmem.md) **ashmem | 指向创建的共享内存的指针。 |
...
映射创建的共享内存到用户空间。通过调用[OH_DDK_UnmapAshmem](capi-ddk-api-h.md#oh_ddk_unmapashmem)接口取消映射不需要的共享内存。
...
| 参数项 | 描述 |
| -- | -- |
| [DDK_Ashmem](capi-ddk-ddk-ashmem.md) *ashmem | 要映射的共享内存指针。 |
...
| 参数项 | 描述 |
| -- | -- |
| [DDK_Ashmem](capi-ddk-ddk-ashmem.md) *ashmem | 要取消映射的共享内存指针。 |
...
| 参数项 | 描述 |
| -- | -- |
| [DDK_Ashmem](capi-ddk-ddk-ashmem.md) *ashmem | 要销毁的共享内存指针。 |
