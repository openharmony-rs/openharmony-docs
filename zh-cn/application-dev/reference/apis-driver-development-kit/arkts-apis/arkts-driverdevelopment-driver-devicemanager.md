...
| 名称 | 说明 |
| --- | --- |
| bindDevice | 根据queryDevices()返回的设备信息绑定设备。必须和unbindDevice接口成对使用。 |
...
| bindDeviceDriver | 根据queryDevices()返回的设备信息绑定设备。必须与unbindDevice接口成对使用。 |
...
| unbindDevice | 解除设备绑定。必须先通过bindDevice接口绑定设备。 |
