...
Extension生命周期回调，会在[onCreate](../../apis-ability-kit/arkts-apis/arkts-ability-abilitystage-c.md#onCreate-1)之后回调。返回一个
[RemoteObject](@ohos.rpc:rpc.RemoteObject)对象，用于客户端和服务端进行通信。
...
| 类型 | 说明 |
| --- | --- |
| rpc.RemoteObject | 一个RemoteObject对象，用于客户端和服务端进行通信；或一个Promise对象，返回用于通信的<br/>RemoteObject对象。 |
