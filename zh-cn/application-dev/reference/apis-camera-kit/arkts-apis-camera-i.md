...
实际预览流的宽高指的是相机输出流的分辨率，请参考[profile](arkts-apis-camera-i.md#profile)中的[size](arkts-apis-camera-i.md#size)。
预览流的数据获取请参考[双路预览](../../media/camera/camera-dual-channel-preview.md)。
...
| timestamp   | number                                      |  是  |  否  | 当前时间戳，单位纳秒（ns）。|
