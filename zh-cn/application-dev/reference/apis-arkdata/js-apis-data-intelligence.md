...
| networkPolicy    | [NetworkPolicy](#networkpolicy)           | 否 | 是   |下载云侧模型时使用的网络策略，默认值为WIFI_ONLY。此次参数仅在使用文本嵌入模型时生效，在使用图像嵌入模型场景此参数不生效。<br/>**ArkTS-Dyn起始版本：** 26.0.0<br/>**ArkTS-Sta起始版本：** 26.0.0**模型约束：** 此接口仅可在Stage模型下使用。 |
