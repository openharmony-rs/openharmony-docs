...
API版本26.0.0之前，数组中每个元素为应用的[appIdentifier](../../../../quick-start/common-problem-of-application.md#什么是appidentifier)
...
表示是否是多值类型的共享配置，默认为false，表示非多值类型。如果为true，则表示本次发布的数据是多值类型，则 [value](arkts-arkdata-proxydata-i.md#value) 参数将被忽略。
默认值： false。
...
数组中每个元素为某个应用的[appIdentifier]（../../../../quick-start/common-problem-of-application.md#什么是appIdentifier）)。
...
[DataProxyConfig](arkts-arkdata-dataproxyconfig-i.md)中配置maxValueLength将最大长度扩展到102400字节。
...
lue的总长度受参数值[maxValueLength](arkts-arkdata-dataproxyconfig-i.md#maxvaluelength)]限制。该参数仅在[isMultiValues](arkts-arkdata-proxydata-i.md#ismultivalues)}设置为true时生效。
