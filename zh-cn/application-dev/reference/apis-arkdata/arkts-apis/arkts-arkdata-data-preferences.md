...
| 名称 | 说明 |
| --- | --- |
| deletePreferences | 从缓存中删除指定的Preferences实例，若Preferences实例有对应的持久化文件，则同时删除其持久化文件。通过name进行参数设置，使用callback异步回调。 |
...
| getPreferences | 获取Preferences实例，通过name进行参数设置，使用callback异步回调。 |
...
| removePreferencesFromCache | 从缓存中移除指定的Preferences实例，通过name进行参数设置，使用callback异步回调。 |
...
| removePreferencesFromCacheSync | 从缓存中移除指定的Preferences实例，通过name进行参数设置，此为同步接口。 |
