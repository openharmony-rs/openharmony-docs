...
> **说明：** 
> 
> - 从API version 9开始，该接口不再维护，推荐使用新接口[@ohos.data.relationalStore](arkts-arkdata-data-relationalstore.md)。
...
| 名称 | 说明 |
| --- | --- |
| deleteRdbStore | 删除数据库，使用callback异步回调。 |
| [deleteRdbStore](arkts-arkdata-rdb-deleterdbstore-f.md#deleterdbstore-1) | 使用指定的数据库文件配置删除数据库，使用Promise异步回调。 |
| getRdbStore | 获得一个相关的RdbStore，操作关系型数据库，用户可以根据自己的需求配置RdbStore的参数，然后通过RdbStore调用相关接口可以执行相关的数据操作，使用callback异步回调。 |
