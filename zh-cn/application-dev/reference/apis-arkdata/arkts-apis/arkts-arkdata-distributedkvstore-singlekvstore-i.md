...
以指定名称备份数据库到默认路径（context.databaseDir），使用callback异步回调。如需备份到自定义路径，请使用 [backupEx]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口。
...
以指定名称备份数据库到默认路径（context.databaseDir），使用Promise异步回调。如需备份到自定义路径，请使用 [backupEx]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口。
...
关闭由[SingleKVStore.getResultSet]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_返回的 KVStoreResultSet对象，使用callback异步回调。关闭结果集后，该结果集对象将不可再用，相关数据库资源被释放。
...
关闭由[SingleKVStore.getResultSet]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_返回的 KVStoreResultSet对象，使用Promise异步回调。关闭结果集后，该结果集对象将不可再用，相关数据库资源被释放。
...
提交SingleKVStore数据库中的事务，使用callback异步回调。需先调用 [startTransaction]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_启动事务后再调 用本接口提交事务。提交成功后，事务期间的所有数据变更将永久生效并写入数据库。
...
提交SingleKVStore数据库中的事务，使用Promise异步回调。需先调用 [startTransaction]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_启动事务后再调 用本接口提交事务。提交成功后，事务期间的所有数据变更将永久生效并写入数据库。
...
根据指定名称从默认路径（context.databaseDir）删除备份文件，使用callback异步回调。删除备份文件后，将无法再通过 [restore]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口恢复该备份文件中的数据。如需从自定义路径删除备份，请使用 [deleteBackupEx]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_接口。
...
根据指定名称从默认路径（context.databaseDir）删除备份文件，使用Promise异步回调。删除备份文件后，将无法再通过 [restore]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口恢复该备份文件中的 数据。如需从自定义路径删除备份，请使用[deleteBackupEx]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_接口。
...
设定是否开启端端同步，使用callback异步回调。开启端端同步后，数据库中的数据可在多设备间自动同步；关闭后则不会自动同步，需要手动调用 [sync]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口触发同步。
...
设定是否开启端端同步，使用Promise异步回调。开启端端同步后，数据库中的数据可在多设备间自动同步；关闭后则不会自动同步，需要手动调用 [sync]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口触发同步。
...
从SingleKVStore数据库中获取具有指定前缀的结果集，使用callback异步回调。获取结果集后，在使用完毕时需调用 [closeResultSet]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ 关闭结果集释放资源。
...
从SingleKVStore数据库中获取具有指定前缀的结果集，使用Promise异步回调。获取结果集后，在使用完毕时需调用 [closeResultSet]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ 关闭结果集释放资源。
...
获取与指定Query对象匹配的KVStoreResultSet对象，使用callback异步回调。获取结果集后，在使用完毕时需调用 [closeResultSet]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ 关闭结果集释放资源。
...
获取与指定Query对象匹配的KVStoreResultSet对象，使用Promise异步回调。获取结果集后，在使用完毕时需调用 [closeResultSet]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ 关闭结果集释放资源。
...
取消订阅数据变更通知。必须先调用 [on('dataChange')]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ 订阅后，才能调用off取消订阅。
...
取消订阅端端同步完成事件回调通知。必须先调用 [on('syncComplete')]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ 订阅后，才能调用off取消订阅。
...
| syncCallback | \_\_\_MD\_LINK\_USD\_0\_\_\_&lt;Array&lt;[string, number]&gt;&gt; | 否 | 取消订阅的同步完成回调函数。如果该参数不填，则取消所有已订阅的同步完成回调函数。需要注意的是：如果同一个数据库存在多个ArkTS实例（通过[getKVStore]\_\_\_ESCAPED\_UNDERSCORE\_\_\_\_\_\_ESCAPED\_UNDERSCORE\_\_\_\_\_\_ESCAPED\_UNDERSCORE\_\_\_JSDOC\_\_\_ESCAPED\_UNDERSCORE\_\_\_LINK\_\_\_ESCAPED\_UNDERSCORE\_\_\_DESC\_\_\_ESCAPED\_UNDERSCORE\_\_\_USD\_\_\_ESCAPED\_UNDERSCORE\_\_\_0\_\_\_ESCAPED\_UNDERSCORE\_\_\_\_\_\_ESCAPED\_UNDERSCORE\_\_\_\_\_\_ESCAPED\_UNDERSCORE\_\_\_接口获取），且这些实例分别注册了同步完成事件回调，那么当任意一个实例调用off('syncComplete')且不传入syncCallback参数（即取消该实例的所有回调）时，其他实例已订阅的同步完成回调函数也会被一并取消。 |
...
订阅指定类型的数据变更通知。调用on订阅后，在不需要监听时必须调用 [off('dataChange')]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ 取消订阅。
...
订阅端端同步完成事件回调通知。调用on订阅后，在不需要监听时必须调用 [off('syncComplete')]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_ 取消订阅。
...
删除指定设备的数据，使用callback异步回调。删除成功后，指定设备的所有数据将从本地数据库中永久移除，无法再通过get等方法查询该设备的数据。 > **说明：** > > 其中deviceId为[DeviceBasicInfo]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_中的 > networkId，通过调用 > [deviceManager.getAvailableDeviceListSync]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ > 方法得到。 > > deviceId具体获取方式请参考 > [sync接口示例]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_。
...
删除指定设备的数据，使用Promise异步回调。删除成功后，指定设备的所有数据将从本地数据库中永久移除，无法再通过get等方法查询该设备的数据。 > **说明：** > > 其中deviceId为[DeviceBasicInfo]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_中的 > networkId，通过调用 > [deviceManager.getAvailableDeviceListSync]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ > 方法得到。 > > deviceId具体获取方式请参考 > [sync接口示例]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_。
...
从数据库默认路径（context.databaseDir）下指定名称的备份文件恢复数据库，使用callback异步回调。恢复成功后，当前数据库中的数据将被替换为备份文件中的数据，原有的未备份数据将丢失。如需从自定义路径恢复，请 使用[restoreEx]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口。
...
从数据库默认路径（context.databaseDir）下指定名称的备份文件恢复数据库，使用Promise异步回调。恢复成功后，当前数据库中的数据将被替换为备份文件中的数据，原有的未备份数据将丢失。如需从自定义路径恢复，请使 用[restoreEx]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口。
...
在SingleKVStore数据库中回滚事务，使用callback异步回调。需先调用 [startTransaction]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_启动事务后再调 用本接口回滚事务。回滚成功后，事务期间的所有数据变更将被丢弃，不会写入数据库。
...
在SingleKVStore数据库中回滚事务，使用Promise异步回调。需先调用 [startTransaction]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_启动事务后再调 用本接口回滚事务。回滚成功后，事务期间的所有数据变更将被丢弃，不会写入数据库。
...
设置数据库端端同步允许的默认延时，使用callback异步回调。 > **说明：** > > 设置默认延时后，调用 > [sync]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口不会立即触发 > 端端同步，而是等待指定的延时时间后再执行。
...
设置数据库端端同步允许的默认延时，使用Promise异步回调。 > **说明：** > > 设置默认延时后，调用 > [sync]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_接口不会立即触发 > 端端同步，而是等待指定的延时时间后再执行。
...
启动SingleKVStore数据库中的事务，使用callback异步回调。启动事务后，后续的数据库操作将纳入此事务范围，直到调用 [commit]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_提交或 [rollback]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_回滚才会结束事务。
...
启动SingleKVStore数据库中的事务，使用Promise异步回调。启动事务后，后续的数据库操作将纳入此事务范围，直到调用 [commit]\_\_\_JSDOC\_LINK\_DESC\_USD\_0\_\_\_提交或 [rollback]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_回滚才会结束事务。
...
在手动同步方式下，触发数据库端端同步。同步结果可通过订阅 [on('syncComplete')]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ 事件获取。关于键值型数据库的端端同步方式说明，请见\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_。 > **说明：** > > 其中deviceIds为[DeviceBasicInfo]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_中的 > networkId, 通过调用 > [deviceManager.getAvailableDeviceListSync]\_\_\_JSDOC\_LINK\_DESC\_USD\_3\_\_\_ > 方法得到。
...
| delayMs | ArkTS-Dyn: number  \_\_\_HTML\_TAG\_USD\_0\_\_\_ArkTS-Sta：int | 否 | 可选参数，允许延时时间，单位：ms（毫秒），默认为0。设置delayMs后，调用sync接口时延时时间为delayMs。未设置时以[setSyncParam]\_\_\_ESCAPED\_UNDERSCORE\_\_\_\_\_\_ESCAPED\_UNDERSCORE\_\_\_\_\_\_ESCAPED\_UNDERSCORE\_\_\_JSDOC\_\_\_ESCAPED\_UNDERSCORE\_\_\_LINK\_\_\_ESCAPED\_UNDERSCORE\_\_\_DESC\_\_\_ESCAPED\_UNDERSCORE\_\_\_USD\_\_\_ESCAPED\_UNDERSCORE\_\_\_0\_\_\_ESCAPED\_UNDERSCORE\_\_\_\_\_\_ESCAPED\_UNDERSCORE\_\_\_\_\_\_ESCAPED\_UNDERSCORE\_\_\_设置的时长为准。 |
...
在手动同步方式下，触发数据库端端同步，支持按查询条件过滤同步数据。同步结果可通过订阅 [on('syncComplete')]\_\_\_JSDOC\_LINK\_DESC\_USD\_1\_\_\_ 事件获取。关于键值型数据库的端端同步方式说明，请见\_\_\_MD\_LINK\_DESC\_USD\_0\_\_\_。 > **说明：** > > 其中deviceIds为[DeviceBasicInfo]\_\_\_JSDOC\_LINK\_DESC\_USD\_2\_\_\_中的 > networkId, 通过调用 > [deviceManager.getAvailableDeviceListSync]\_\_\_JSDOC\_LINK\_DESC\_USD\_3\_\_\_ > 方法得到。
