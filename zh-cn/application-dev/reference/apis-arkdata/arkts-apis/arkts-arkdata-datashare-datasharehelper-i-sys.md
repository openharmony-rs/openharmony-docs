...
| --- | --- | --- | --- |
| uri | string | 是 | 要插入的数据的路径。 |
| values | [Array](../../apis-arkts/arkts-apis/arkts-arkts-collections-array-c.md)<ValuesBucket> | 是 | 要插入的数据。 |
...
取消订阅指定URI下指定callback对应的数据资源的变更通知。与订阅接口[on](arkts-arkdata-datashare-datasharehelper-i-sys.md#on-1)相对应。
...
取消订阅指定URI下指定callback对应的数据资源的变更通知。与订阅接口[on](arkts-arkdata-datashare-datasharehelper-i-sys.md#on-2)相对应。
...
| uris | [Array](../../apis-arkts/arkts-apis/arkts-arkts-collections-array-c.md)<string> | 是 | 要操作的数据的路径。 |
...
| 类型 | 说明 |
| --- | --- |
| [Array](../../apis-arkts/arkts-apis/arkts-arkts-collections-array-c.md)<OperationResult> | 返回操作结果。 |
...
触发通知：非静默场景下，调用[notifyChange](arkts-arkdata-datashare-datasharehelper-i-sys.md#notifychange-2)方法，就会触发对指定URI订阅者的通知；或者静默场景下，使用指定URI的静默访问修改了数据，也会自动触发通知。
...
触发通知：非静默场景下，调用[notifyChange](arkts-arkdata-datashare-datasharehelper-i-sys.md#notifychange-3)方法，就会触发对指定URI订阅者的通知；或者静默场景下，使用指定URI的静默访问修改了数据，也会自动触发通知, 但此时callback通知中的changeInfo无效。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| data | [Array](../../apis-arkts/arkts-apis/arkts-arkts-collections-array-c.md)<PublishedItem> | 是 | 要发布的数据。 |
...
| columns | [Array](../../apis-arkts/arkts-apis/arkts-arkts-collections-array-c.md)<string> | 是 | 要查询的列。如果此参数为空，则查询所有列。 |
