...
| 类型 | 说明 |
| -- | -- |
| [OH_Cursor](capi-rdb-oh-cursor.md) * | 如果查询成功则返回一个指向OH_Cursor结构体实例的指针。如果获取store失败或结果集为空，则返回nullptr。 |
...
| 类型 | 说明 |
| -- | -- |
| [OH_Cursor](capi-rdb-oh-cursor.md) * | 如果查询成功则返回一个指向OH_Cursor结构体实例的指针。如果SQL语句无效或内存分配失败，则返回nullptr。 |
