...
| MAX_STORE_ID_LENGTH   |  128  | 值为128，表示数据库标识符允许的最大长度，单位：字节（Byte）。        |
| MAX_KEY_LENGTH_DEVICE |  896  | 值为896，表示设备协同数据库中Key允许的最大长度，单位：字节（Byte）。 |
| MAX_KEY_LENGTH        |  1024 | 值为1024，表示数据库中Key允许的最大长度，单位：字节（Byte）。        |
| MAX_QUERY_LENGTH      |  512000  | 值为512000，表示最大查询长度，单位：字节（Byte）。                   |
| MAX_VALUE_LENGTH      |  4194303  | 值为4194303，表示数据库中Value允许的最大长度，单位：字节（Byte）。   |
...
| BYTE_ARRAY | 3 | 表示值类型为字节（Byte）数组。<br/>**ArkTS-Dyn起始版本：** 9<br/>**ArkTS-Sta起始版本：** 23|
...
| rootDir<sup>24+</sup> | string                         | 否    | 是  | 设置数据库文件存储路径，不设置即为默认路径（context.databaseDir）。不能设置空字符串，创建数据库和删除数据库时目录必须有访问权限且存在，关闭数据库不校验此参数。<br> ArkTS-Dyn：设置undefined时，异常将报错。<br> ArkTS-Sta：设置undefined时，数据库文件存储路径为默认路径。<br> **ArkTS-Dyn起始版本：** 24 <br> **ArkTS-Sta起始版本：** 24 <br>**系统能力：** SystemCapability.DistributedDataManager.KVStore.DistributedKVStore |
...
| skip    | ArkTS-Dyn: number<br/>ArkTS-Sta: int                  | 否  | 否  | 支持在检查Value时，跳过skip指定的字节（Byte）数，取值范围为[0, 4 * 1024 * 1024 - 2]字节（Byte）。<br/>**模型约束：** 此接口仅可在Stage模型下使用。        |
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[ondistributedDataServiceDie](#ondistributeddataservicedie23)。
...
### onDistributedDataServiceDie<sup>23+</sup>
...
**相关接口：** 该接口对应的ArkTS-Dyn接口是[on('distributedDataServiceDie')](#ondistributeddataservicedie)。
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[offDistributedDataServiceDie](#offdistributeddataservicedie23)。
...
### offDistributedDataServiceDie<sup>23+</sup>
...
**相关接口：** 该接口对应的ArkTS-Dyn接口是[off('distributedDataServiceDie')](#offdistributeddataservicedie)。
...
ArkTS-Dyn: move(offset: number): boolean

ArkTS-Sta: move(offset: int): boolean
...
ArkTS-Dyn: moveToPosition(position: number): boolean

ArkTS-Sta: moveToPosition(position: int): boolean
...
| -----  |---------------------------------------------------------| ----  | ----------------------- |
| field  | string                                                  | 是    |表示指定字段，不能包含'^'。包含'^'将导致谓词失效，查询结果会返回数据库中的所有数据。  |
| value  | ArkTS-Dyn: number \| string<br/>ArkTS-Sta: long \| double \| string | 是    | 表示指定字段要匹配的值，值的类型应与Schema中定义的字段类型一致。|
...
ArkTS-Dyn: inNumber(field: string, valueList: number[]): Query

ArkTS-Sta: inNumber(field: string, valueList: long[] | double[]): Query
...
ArkTS-Dyn: notInNumber(field: string, valueList: number[]): Query

ArkTS-Sta: notInNumber(field: string, valueList: long[] | double[]): Query
...
ArkTS-Dyn: limit(total: number, offset: number): Query

ArkTS-Sta: limit(total: int, offset: int): Query
...
| 参数名 | 类型 | 必填 | 说明               |
| ------ | -------- | ---- | ------------------ |
| total  | ArkTS-Dyn: number<br/>ArkTS-Sta: int   | 是   | 表示最大数据记录数。<br/>取值为非负整数时表示指定的最大记录数。<br/>取值为负数时，表示查询整个结果集。|
| offset | ArkTS-Dyn: number<br/>ArkTS-Sta: int   | 是   | 指定查询结果的起始位置。取值为非负整数时表示指定的起始位置；取值为负数时，表示查询整个结果集。当offset超出结果集最后位置时，查询结果为空。|
...
ArkTS-Dyn: put(key: string, value: Uint8Array | string | number | boolean, callback: AsyncCallback&lt;void&gt;): void

ArkTS-Sta: put(key: string, value: Uint8Array | string | long | double | boolean, callback: AsyncCallback\<void\>): void
...
| value  | ArkTS-Dyn: Uint8Array \| string \| number \| boolean<br/>ArkTS-Sta: Uint8Array \| string \| long \| double \| boolean | 是    |要添加数据的value，支持Uint8Array、string、number、boolean，Uint8Array、string的长度范围为0-[MAX_VALUE_LENGTH](#constants)。   |
...
| callback  | ArkTS-Dyn: AsyncCallback&lt;boolean \| string \| number \| Uint8Array&gt;<br/>ArkTS-Sta: AsyncCallback&lt;boolean \| string \| long \| double \| Uint8Array&gt; | 是    |回调函数。返回获取查询的值，值的类型取决于存储时的数据类型。  |
...
| 类型    | 说明       |
| ------  | -------   |
|ArkTS-Dyn: Promise&lt;boolean \| string \| number \| Uint8Array&gt;<br/>ArkTS-Sta: Promise&lt;boolean \| string \| long \| double \| Uint8Array&gt; |Promise对象。返回指定键对应的值，值的类型取决于存储时的数据类型。|
...
订阅指定类型的数据变更通知。调用onDataChange订阅后，在不需要监听时必须调用[offDataChange](#offdatachange23)取消订阅。
...
**相关接口：** 该接口对应的ArkTS-Sta接口是[onSyncComplete](#onsynccomplete23)。
...
### onSyncComplete<sup>23+</sup>

onSyncComplete(syncCallback: Callback&lt;Array&lt;[string, int]&gt;&gt;): void

订阅端端同步完成事件回调通知。调用onSyncComplete订阅后，在不需要监听时必须调用[offSyncComplete](#offsynccomplete23)取消订阅。

**ArkTS模式：** 该接口仅适用于ArkTS-Sta。

**相关接口：** 该接口对应的ArkTS-Dyn接口是[on('syncComplete')](#onsynccomplete)。
...
| 参数名       | 类型                                      | 必填 | 说明                                                   |
| ------------ | --------------------------------------------- | ---- | ------------------------------------------------------ |
| syncCallback | Callback&lt;Array&lt;[string, int]&gt;&gt; | 是   | 回调函数。用于向调用方发送同步结果的回调。             |
...
取消订阅数据变更通知。必须先调用[onDataChange](#ondatachange23)订阅后，才能调用offDataChange取消订阅。
...
取消订阅端端同步完成事件回调通知。必须先调用[onSyncComplete](#onsynccomplete23)订阅后，才能调用offSyncComplete取消订阅。
...
| 参数名       | 类型                                      | 必填 | 说明                                                                                                                                                                                                         |
| ------------ | --------------------------------------------- | ---- |------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| syncCallback | Callback&lt;Array&lt;[string, int]&gt;&gt; | 否   | 取消订阅的同步完成回调函数。如果该参数不填，则取消所有已订阅的同步完成回调函数。需要注意的是：如果同一个数据库存在多个ArkTS实例（通过[getKVStore](#getkvstore)接口获取），且这些实例分别注册了同步完成事件回调，那么当任意一个实例调用offSyncComplete且不传入syncCallback参数（即取消该实例的所有回调）时，其他实例已订阅的同步完成回调函数也会被一并取消。 |
...
| 类型                                                     | 说明                            |
| -------------------------------------------------------- | ------------------------------- |
| ArkTS-Dyn: Promise&lt;boolean \| string \| number \| Uint8Array&gt;<br>ArkTS-Sta: Promise&lt;boolean \| string \| long \| double \| Uint8Array&gt; | Promise对象。返回获取查询的值，值的类型取决于存储时的数据类型。 |
...
| callback  | ArkTS-Dyn: AsyncCallback&lt;boolean \| string \| number \| Uint8Array&gt;<br>ArkTS-Sta: AsyncCallback&lt;boolean \| string \| long \| double \| Uint8Array&gt; | 是    |回调函数。成功时返回匹配给定条件的字符串值（值的类型取决于存储时的数据类型），失败时返回错误对象。    |
...
| 类型                                                                           | 说明       |
|------------------------------------------------------------------------------| -------   |
| ArkTS-Dyn: Promise&lt;boolean \|string \|number \| Uint8Array&gt;<br>ArkTS-Sta: Promise&lt;boolean \| string \| long \| double \| Uint8Array&gt; |Promise对象。返回匹配给定条件的字符串值，值的类型取决于存储时的数据类型。|
