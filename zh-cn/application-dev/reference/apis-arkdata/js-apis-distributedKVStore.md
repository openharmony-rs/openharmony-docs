...
| bundleName | string          | 否   | 否   | 调用方的包名，不可为空且长度范围为1-256 byte（字节）。                                               |
...
| 名称                  | 类型   | 只读 | 可选 | 说明                                                       |
| --------------------- | ------ | ---- | ---- | ---------------------------------------------------------- |
| MAX_KEY_LENGTH        | number | 是   | 否   | 值为1024，表示数据库中Key允许的最大长度，单位：byte（字节）。        |
| MAX_VALUE_LENGTH      | number | 是   | 否   | 值为4194303，表示数据库中Value允许的最大长度，单位：byte（字节）。   |
| MAX_KEY_LENGTH_DEVICE | number | 是   | 否   | 值为896，表示设备协同数据库中Key允许的最大长度，单位：byte（字节）。 |
| MAX_STORE_ID_LENGTH   | number | 是   | 否   | 值为128，表示数据库标识符允许的最大长度，单位：byte（字节）。        |
| MAX_QUERY_LENGTH      | number | 是   | 否   | 值为512000，表示最大查询长度，单位：byte（字节）。                   |
...
| skip    | number                  | 否  | 否  | 支持在检查Value时，跳过skip指定的byte（字节）数，取值范围为[0, 4 * 1024 * 1024 - 2] byte（字节）。|
...
| 参数名   | 类型                  | 必填 | 说明                                                         |
| -------- | ------------------------- | ---- | ------------------------------------------------------------ |
| appId    | string                    | 是   | 应用的BundleName，不可为空且长度范围为1-256 byte（字节）。                                      |
