...
| tableType<sup>23+</sup> |  [DistributedTableType](arkts-apis-data-relationalStore-e.md#distributedtabletype23)  | 否 | 是 | 表示当前端端分布式表的分布式表类型。DEVICE_COLLABORATION表示设备协作表；SINGLE_VERSION表示单版本表。端端分布式表默认值为DEVICE_COLLABORATION，端云分布式表默认SINGLE_VERSION。 |
