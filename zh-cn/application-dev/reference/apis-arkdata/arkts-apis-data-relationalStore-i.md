...
| encrypt | boolean | 否 | 是 | 指定数据库是否加密，默认不加密。数据库创建后，禁止对该参数进行直接修改。如果想修改数据库加密状态需使用[rekeyEx](arkts-apis-data-relationalStore-RdbStore.md#rekeyex22)<br/> true：加密。<br/> false：非加密。<br/>**系统能力：** SystemCapability.DistributedDataManager.RelationalStore.Core |
