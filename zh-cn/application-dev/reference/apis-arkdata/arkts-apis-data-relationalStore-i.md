...
| assetTempPath | string | 否 | 是 | 资产临时路径。当assetConflictPolicy值为CONFLICT_POLICY_TEMP_PATH时，此参数有效，需填写为[distributedfiles](../../file-management/app-sandbox-directory.md#应用文件目录与应用文件路径)下的临时路径，否则使用异常抛出401错误码。默认值为空。<br/>**起始版本**：26.0.0<br/>**模型约束：** 此接口仅可在Stage模型下使用。 |
...
| message | string | 否 | 是   | 同步状态的详情消息。通过message信息查看详细的失败原因。默认值为空。<br/>**起始版本**：26.0.0<br/>**模型约束：** 此接口仅可在Stage模型下使用。 |
