...
try {
  ...
  let error = err as BusinessError;
  console.error(`Failed to do putValuesBuckets error. Code: ${error.code}, message: ${error.message}`);
}
```
...
try {
  ...
  kvStore!.putValuesBuckets(people).then(() => {
    ...
    console.error(`Failed to do putValuesBuckets error. Code: ${err.code}, message: ${err.message}`);
  });
} catch (err) {
  let error = err as BusinessError;
  console.error(`Failed to do putValuesBuckets error. Code: ${error.code}, message: ${error.message}`);
...
| 类型                                                 | 说明                      |
| ---------------------------------------------------- | ------------------------- |
| Promise&lt;[KVStoreResultSet](js-apis-distributedKVStore.md#kvstoreresultset)&gt; | Promise对象。返回KVStoreResultSet对象。 |
...
获取与本设备指定Predicates对象匹配的KVStoreResultSet对象，使用callback异步回调。获取结果集后，需要及时调用[closeResultSet](js-apis-distributedKVStore.md#closeresultset)方法关闭结果集以释放资源。
...
获取与本设备指定Predicates对象匹配的KVStoreResultSet对象，使用Promise异步回调。获取结果集后，需要及时调用[closeResultSet](js-apis-distributedKVStore.md#closeresultset)方法关闭结果集以释放资源。
