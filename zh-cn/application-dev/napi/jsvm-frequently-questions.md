...
6. Q: 在调用JSVM-API时出现如下两种报错：
   ...
   A: 这两种报错均指向同一问题，即应用侧执行完`OH_JSVM_DestoryEnv()`(释放JSVM环境)后，仍在执行业务逻辑，尝试调用JSVM-API，触发报错。该报错可能由两种情况产生：
      ...
      a) 回调函数中含有对JSVM-API的调用，在被触发时应用侧已经执行完`OH_JSVM_DestoryEnv()`，此时直接在回调函数内尝试调用JSVM-API，则可能会产生该错误。开发者应当保证所有JSVM实例运行在同一个js线程上，并在此线程上为每个JSVM实例添加对应标记，在释放JSVM环境后将对应标记置为true。回调函数中调用JSVM-API时，应当先判断当前是否在上述js线程上，若是，则直接根据flag判断是否能够调用API，若不是，则把该任务抛到上述js线程上再进行判断和执行。若使用任务队列来进行js任务执行，需要注意当flag为true时，队列中剩余的任务需要全部跳过。
