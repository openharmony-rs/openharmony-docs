...
## 详细迁移指导
### 声明文件迁移
在ArkTS中，您通常会有一个.d.ts（声明文件）来描述Node-API模块的接口。这个文件提供了其他ets文件使用该Node-API模块接口的类型声明，是您编写ANI类声明的参考。开发者可以参考它编写ANI类声明。
...
* 自动注册特性
    ...
    使用__attribute__((constructor))修饰的注册函数会在so加载时自动执行，无需显式调用注册逻辑。

* 模块描述规范

    napi_module结构体定义了模块元信息，nm_register_func是核心回调函数，负责导出native方法。
...
3. 方法绑定
    ...
    在ANI_Constructor中，使用以下API进行方法绑定：
...
在Node-API中，通常通过在C++代码中调用Node-API接口来创建类或对象，常见的创建接口和类对象的方法包括：
...
2. 使用`napi_define_class`定义类，然后通过`napi_new_instance`创建该类的实例对象。
    ...
    napi_property_descriptor properties[] = {
        ...
        {"plugOne", nullptr, PlugOne, nullptr, nullptr, nullptr, napi_default, nullptr}
    };
    ...
    // ...
    napi_value nWindow;
    napi_new_instance(env, cons, 0, nullptr, &nWindow);
    ```
**ANI类型定义代码示例**
...
2. 在ets中声明后，还需要在C++侧的ANI_Constructor中调用ANI接口，完成方法的显式绑定。
    ...
    ani_status ANI_Constructor(ani_vm* vm, uint32_t* result) {
        ...
        env->FindClass("C{MyModule.MyObject}", &cls);

        std::array<ani_native_function, 1> methods = {{
            {"plugOne", "i:i", reinterpret_cast<void*>(nativePlugOne)}
        }};
