...
OpenHarmony提供Purgeable Memory管理机制，开发者可以使用相关接口创建PurgeableMemory对象，从而管理Purgeable Memory。


开发者可以通过本指导了解在OpenHarmony应用中，如何使用Native层相关接口操作Purgeable Memory。功能包括Purgeable Memory的申请、释放等。
...
| bool OH_PurgeableMemory_Destroy(OH_PurgeableMemory *purgObj) | 销毁PurgeableMemory对象。 |
| bool OH_PurgeableMemory_BeginRead(OH_PurgeableMemory *purgObj) | 对PurgeableMemory对象进行读访问，将读引用计数加1。 |
| void OH_PurgeableMemory_EndRead(OH_PurgeableMemory *purgObj) | 读操作结束，将PurgeableMemory对象的读引用计数减1。 |
| bool OH_PurgeableMemory_BeginWrite(OH_PurgeableMemory *purgObj) | 对PurgeableMemory对象进行写访问，将写引用计数加1。 |
| void OH_PurgeableMemory_EndWrite(OH_PurgeableMemory *purgObj) | 写操作结束，将PurgeableMemory对象的写引用计数减1。 |
| void *OH_PurgeableMemory_GetContent(OH_PurgeableMemory *purgObj) | 获取PurgeableMemory对象内存数据指针，返回void*类型，使用时需根据实际数据类型进行转换。 |
...
以下步骤描述了在**OpenHarmony**中如何使用`Purgeable Memory`提供的Native层相关接口，申请PurgeableMemory对象，并将内容写入PurgeableMemory对象后，对该对象进行读写访问。
