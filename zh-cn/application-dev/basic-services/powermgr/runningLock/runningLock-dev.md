...
当计算机在一段时间内没有检测到用户活动（如键盘或鼠标输入）时，通常系统会自动尝试进入睡眠，使用[RunningLockType.BACKGROUND_USER_IDLE](../../../reference/apis-basic-services-kit/js-apis-runninglock.md#runninglocktype)运行锁，保证在持锁过程中系统不会进入自动睡眠，保证业务正常运行。
