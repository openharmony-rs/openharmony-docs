...
| 接口名                                                                          | 描述                       |
|------------------------------------------------------------------------------|--------------------------|
| getPortList                                       | 获取串口设备列表。                |
| hasSerialRight                                      | 检查应用程序是否具有访问串口设备的权限。     |
| requestSerialRight                        | 请求对串口设备的访问权限。            |
| open                                                   | 打开串口设备。                  |
| close                                                  | 关闭串口设备。                  |
| read| 从串口设备读取数据，使用Promise异步返回。 |
| readSync       | 以同步方法从串口设备读取数据。          |
| write | 往串口设备写入数据，使用Promise异步返回。 |
| writeSync      | 以同步方法往串口设备写入数据。          |
...
5. 通过串口读取数据。
   ...
   ArkTs-Sta示例：
    ...
    // 异步读取 
    let readBuffer: Uint8Array = new Uint8Array(64);
    serial.read(portId, readBuffer, 2000).then((size: int) => {
      console.info(`read usbSerial success, readBuffer: ${readBuffer}`);
    }).catch((error: Error) => {
      console.error(`read usbSerial error: ${error}`);
    })
...
6. 通过串口写入数据。
   ...
   ArkTs-Sta示例：
    ...
    // 异步写入
    let writeBuffer: Uint8Array = new Uint8Array(buffer.from('Hello World', 'utf-8').buffer)
    serial.write(portId, writeBuffer, 2000).then((size: int) => {
      console.info(`write usbSerial success, writeBuffer: ${writeBuffer}`);
    }).catch((error: Error) => {
      console.error(`write usbSerial error: ${error}`);
    })
