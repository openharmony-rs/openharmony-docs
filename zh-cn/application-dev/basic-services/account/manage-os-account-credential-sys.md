...
录入生物识别凭据后，用户可以进行生物识别认证。类似PIN码认证，开发者可以使用auth接口完成此操作。
...
2. 调用getAuthInfo接口，获取指定类型的凭据信息（以指纹凭据为例）。
