...
2. 指定类型和域账号信息，调用createOsAccountForDomain接口在设备上创建一个域账号。
...
1. 调用getOsAccountLocalIdForDomain方法，根据域账号信息获取系统账号ID。
...
1. 定义查询选项，可以指定需要查询的域名和账号名。选项的类型为GetDomainAccountInfoOptions。
