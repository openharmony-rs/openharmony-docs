...
[方式一](#步骤3-通过接口拉起系统打印预览界面下发任务)：应用通过接口拉起系统打印预览界面，传输渲染好的PDF文件进行打印任务下发。这种方式适用于需具有要打印功能但不想自行开发打印预览界面的应用。

[方式二](#步骤4-通过打印接口自主进行打印设备的发现连接和打印任务的下发)：应用通过打印接口自主发现、连接打印设备并下发任务。此方式适合具有打印预览界面的应用，使用接口发现打印设备、获取设备能力，构建预览界面信息。
