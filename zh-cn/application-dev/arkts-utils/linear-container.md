...
| 增加元素 | ArkTS-Dyn: insert(element: T, index: number) <br/> ArkTS-Sta: insert(element: T, index: int) | 在指定位置插入一个元素。 |
| 访问元素 | ArkTS-Dyn: arr[index: number] <br/> ArkTS-Sta: [index: int] | 获取指定index对应的value值。 |
| 访问元素 | ArkTS-Dyn: forEach(callbackFn: (value: T, index?: number, arrlist?: ArrayList&lt;T&gt;) =&gt; void, thisArg?: Object) <br/> ArkTS-Sta: forEach(callbackFn: ArrayListForEachCb&lt;T&gt;) | 访问整个ArrayList容器的元素，其中`callbackFn`是`forEach`方法中用于处理每个元素的回调函数，它接收当前元素、索引和原列表作为参数。 |
...
| 删除元素 | ArkTS-Dyn: removeByRange(fromIndex: number, toIndex:number) <br/> ArkTS-Sta: removeByRange(fromIndex: int, toIndex:int) | 删除指定范围内的元素。 |
...
| --------- | ------- | ------- |
| 增加元素 | add(element: T) | 在数组尾部增加一个元素。 |
| 增加元素 | ArkTS-Dyn: insert(element: T, index: number) <br/> ArkTS-Sta: insert(element: T, index: int) | 在指定位置增加一个元素。 |
| 访问元素 | ArkTS-Dyn: get(index: number) <br/> ArkTS-Sta: get(index: int) | 获取指定index位置对应的元素。 |
| 访问元素 | ArkTS-Dyn: list[index: number] <br/> ArkTS-Sta: [index: int] | 获取指定索引位置的元素。若索引超出数组范围（index < 0 或 index >= list.length），或者数组是稀疏数组（存在未赋值的索引），则返回undefined。 |
...
| 访问元素 | ArkTS-Dyn: forEach(callbackfn: (value:T, index?: number, list?: List&lt;T&gt;)=&gt; void,thisArg?: Object) <br/> ArkTS-Sta: forEach(callbackfn: (value:T, index?: int, list?: List&lt;T&gt;)=&gt; void,thisArg?: Object) | 遍历访问整个List容器中的每个元素，并执行指定的回调函数。 |
| 访问元素 | ArkTS-Dyn: \[Symbol.iterator]():IterableIterator&lt;T&gt; <br/> ArkTS-Sta: $_iterator(): IterableIterator&lt;T&gt; | 创建迭代器以进行数据访问。 |
| 修改元素 | ArkTS-Dyn: set(index:number, element: T) <br/> ArkTS-Sta: set(index:int, element: T) | 修改指定index位置的元素值为element。 |
| 修改元素 | list[index] = element | 修改指定index位置的元素值为element时，不会对链表中的实际节点进行任何更改，仅会在对象上添加一个属性，可能导致程序状态与链表内部数据不符，引发难以预测的行为。 |
| 修改元素 | ArkTS-Dyn: replaceAllElements(callbackFn:(value: T,index?: number,list?: List&lt;T&gt;)=&gt;T,thisArg?: Object) <br/> ArkTS-Sta: replaceAllElements(callbackFn:(value: T,index?: int,list?: List&lt;T&gt;)=&gt;T,thisArg?: Object) | 对List内元素进行逐个替换。 |
| 删除元素 | remove(element: T) | 通过 === 运算符逐个比对链表中的元素，删除第一个匹配成功的节点。对于对象类型，只有当传入的对象与链表中某节点的引用完全一致时才会被删除。 |
| 删除元素 | ArkTS-Dyn: removeByIndex(index:number) <br/> ArkTS-Sta: removeByIndex(index:int) | 删除index位置对应的元素，如果index超出范围，则会报out of range错误。 |
...
| 增加元素 | ArkTS-Dyn: insert(element: T, index: number) <br/> ArkTS-Sta: insert(element: T, index: int) | 在指定位置插入一个元素。 |
| 访问元素 | ArkTS-Dyn: get(index: number) <br/> ArkTS-Sta: get(index: int) | 获取指定index位置对应的元素。 |
| 访问元素 | ArkTS-Dyn: list[index: number] <br/> ArkTS-Sta: [index: int] | 获取指定index位置对应的元素，若索引超出数组范围（index < 0 或 index >= list.length），或者数组是稀疏数组（存在未赋值的索引），则返回undefined。 |
| 访问元素 | getFirst() | 获取第一个元素。 |
| 访问元素 | getLast() | 获取最后一个元素。 |
| 访问元素 | getIndexOf(element: T) | 获取第一个匹配指定元素的位置。 |
| 访问元素 | getLastIndexOf(element: T) | 获取最后一个匹配指定元素的位置。 |
| 访问元素 | ArkTS-Dyn: forEach(callbackFn: (value: T, index?: number, list?: LinkedList&lt;T&gt;) =&gt; void, thisArg?: Object) <br/> ArkTS-Sta: forEach(callbackFn: (value: T, index?: int, list?: LinkedList&lt;T&gt;) =&gt; void, thisArg?: Object) | 遍历访问整个LinkedList容器的每个元素，并执行指定的回调函数。 |
| 访问元素 | ArkTS-Dyn: \[Symbol.iterator]():IterableIterator&lt;T&gt; <br/> ArkTS-Sta: $_iterator(): IterableIterator&lt;T&gt; | 创建迭代器以进行数据访问。 |
| 修改元素 | ArkTS-Dyn: set(index:number, element: T) <br/> ArkTS-Sta: set(index:int, element: T) | 修改指定index位置的元素值为element。 |
| 修改元素 | list[index] = element | 修改指定index位置的元素值为element，若索引超出数组范围（index < 0 或 index >= list.length），或者数组是稀疏数组（存在未赋值的索引），则可能导致未定义行为。 |
| 删除元素 | remove(element: T) | 删除第一个匹配到的元素。 |
| 删除元素 | ArkTS-Dyn: removeByIndex(index:number) <br/> ArkTS-Sta: removeByIndex(index:int) | 删除index位置对应的元素。 |
...
| 访问元素 | ArkTS-Dyn: forEach(callbackFn:(value: T, index?: number, deque?: Deque&lt;T&gt;) =&gt; void, thisArg?: Object) <br/> ArkTS-Sta: forEach(callbackFn:(value: T, index?: int, deque?: Deque&lt;T&gt;) =&gt; void, thisArg?: Object) | 遍历访问整个Deque容器的每个元素，并执行指定的回调函数。 |
| 访问元素 | ArkTS-Dyn: \[Symbol.iterator]():IterableIterator&lt;T&gt; <br/> ArkTS-Sta: $_iterator(): IterableIterator&lt;T&gt; | 创建迭代器以进行数据访问。 |
...
| 访问元素 | ArkTS-Dyn: forEach(callbackFn: (value: T, index?: number, queue?: Queue&lt;T&gt;) =&gt; void,thisArg?: Object) <br/> ArkTS-Sta: forEach(callbackFn: (value: T, index?: int, queue?: Queue&lt;T&gt;) =&gt; void,thisArg?: Object) | 遍历访问整个Queue容器的每个元素，并执行指定的回调函数。 |
| 访问元素 | ArkTS-Dyn: \[Symbol.iterator]():IterableIterator&lt;T&gt; <br/> ArkTS-Sta: $_iterator(): IterableIterator&lt;T&gt; | 创建迭代器以进行数据访问。 |
...
<!-- @[linear_container](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/ArkTS-Sta/ArkTSRuntimeCoreLibrary/ArkTSContainerLibrary/LinearContainers/entry/src/main/ets/pages/Index.ets) --> 
...
import { ArrayList, Deque, Stack, List } from '@kit.ArkTS'; // 导入ArrayList，Deque，Stack，List模块
  ...
  arrayList1.add('a'); // 增加一个值为'a'的元素
  let arrayList2: ArrayList<int> = new ArrayList<int>();
  arrayList2.add(1); // 增加一个值为1的元素
  console.info(`result: ${arrayList2[0]}`); // 访问索引为0的元素。输出：result: 1
  // ...
  arrayList1[0] = 'one'; // 修改索引为0的元素
  ...
  deque1.insertFront('a'); // 头部增加一个值为'a'的元素
  let deque2: Deque<int> = new Deque<int>();
  deque2.insertFront(1); // 头部增加一个值为1的元素
  console.info(`result: ${deque2.getFirst()}`); // 访问队列首部的元素。输出：result: 1
  // ...
  deque1.insertEnd('one'); // 尾部增加一个值为'one'的元素
  ...
  stack1.push('a'); // 向栈里增加一个值为'a'的元素
  let stack2: Stack<int> = new Stack<int>();
  stack2.push(1); // 向栈里增加一个值为1的元素
  console.info(`result: ${stack1.peek()}`); // 访问栈顶元素。输出：result: a
  // ...
  let popValue = stack2.pop()
  console.info(`result: ${popValue}`); // 删除栈顶元素并返回该删除元素。输出：result: 1
  // ...
  console.info(`result: ${stack2.length}`); // 输出：result: 0
  // ...
  // List
  ...
  list1.add('a'); // 增加一个值为'a'的元素
  let list2: List<int> = new List<int>();
  list2.insert(0, 0); // 在0号位置插入（增加）一个值为0的元素
  let list3: List<Array<int>> = new List<Array<int>>();
  let b2 = [1, 2, 3];
  list3.add(b2); // 增加一个Array类型的元素
  console.info(`result: ${list1[0]}`); // 访问索引为0的元素。输出：result: a
  // ...
