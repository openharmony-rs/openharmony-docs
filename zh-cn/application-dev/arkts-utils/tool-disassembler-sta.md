...
Disassembler是ArkTS反汇编工具，用于将方舟字节码文件（`.abc`）反汇编为可读的文本。
...
| --with_separators | 否 | 否 | 在输出中添加段落分隔注释（# LITERALS、# RECORDS、# METHODS、# STRING）。 |

## 输出格式与使用说明

本章介绍ArkTS-Sta版本Disassembler的输出格式与使用方法。
...
> **说明** :
>
> ArkTS-Sta版本默认不输出段落分隔注释，如需添加可使用`--with_separators`选项。
