...
6. 了解需要配置白名单的场景，将白名单配置到obfuscation-rules.txt文件中。
...
1. HAR包的开发者需充分了解三种混淆配置文件以及混淆规则的合并策略，以及在被HAP包使用时的HAP包混淆建议中的注意事项，确保被应用依赖时本模块所有功能正常。
...
1. HSP包的开发者需充分了解三种混淆配置文件以及混淆规则的合并策略，以及在被HAP包使用时的HAP包混淆建议中的注意事项，确保被应用依赖时本模块所有功能正常。
