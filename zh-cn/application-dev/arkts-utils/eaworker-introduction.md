...
EAWorker的创建和销毁耗费性能，建议开发者合理管理已创建的EAWorker并重复使用。EAWorker空闲时也会一直运行，因此当不需要EAWorker时，可以调用[quit()](../reference/apis-arkts/js-apis-worker.md#terminate9)接口方法主动销毁EAWorker。调用`quit()`后，EAWorker会在处理完消息队列中的剩余任务后才退出。若EAWorker处于已销毁或正在销毁等非运行状态时，调用其功能接口，会抛出相应的错误。
