...
ArkTS-Dyn 已经提供了Sendable对象来实现跨线程共享能力，但也是基于内存隔离的基础上，实现的部分数据共享。不能实现主流编程语言并发的内存共享模型。
...
| 能力 | 说明 | 适用场景 |
| -------- | -------- | -------- |
| Promise和async/await | 用Promise表示异步结果，通过await等待异步操作完成。 | I/O等待、定时器、异步流程编排。 |
| TaskPool | 将短时或可拆分任务提交到运行时管理的任务池执行，并通过Promise返回结果。 | CPU密集型计算、后台数据处理、批量任务。 |
| EAWorker | 创建独占工作线程，并在该线程上提交任务。 | 固定线程、长期驻留、独立事件循环、与JS互操作。 |
| 线程间通信 | 通过共享内存、TaskPool结果和进度通知、EAWorker消息等方式在线程间协作。 | 数据共享、任务通知、主线程与工作线程协同。 |
| 并发数据安全| 使用锁、原子类型、并发容器或WorkerLocal管理共享数据。 | 多线程读写共享对象、生产者/消费者、线程本地状态。 |
...
- 使用ConcurrentHashMap、ConcurrentSet、BlockingQueue等并发容器管理集合数据。
