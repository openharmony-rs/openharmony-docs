...
* `obfuscation-rules.txt`
    ...
    > **注意：**
    > 
    > ArkTS-Sta的字节码混淆目前只支持 `obfuscation-rules.txt`配置文件。
...
2. 待上述选项开启成功后，开启`-enable-property-obfuscation`。
    ...
    4. 验证应用功能，排查遗漏的场景。若应用出现功能异常，依据混淆后的报错栈从对应的[中间产物](#查看字节码混淆效果)中找到报错行的代码，排查需要配置的白名单并使用`-keep-property-name`进行保留。
...
3. 待上述选项应用适配成功后，开启`-enable-export-obfuscation`选项。此选项开启后以下场景需要适配：
    ...
    4. 验证应用功能以及模块被依赖时的接口调用功能，排查遗漏的场景。若应用出现功能异常，依据混淆后的报错栈从对应的[中间产物](#查看字节码混淆效果)中找到报错行的代码，排查需要配置的白名单并进行保留。
...
* 混淆名称映射表及系统API白名单目录：`build/default/[...]/obfuscation`。
* 名称映射表文件：`nameCache.json`，该文件记录了字节码名称混淆的映射关系。
* obf目录：混淆后的modules_static.abc文件和modules_static.pa文件（开启-enable-bytecode-obfuscation-debugging后，生成pa文件）。
* origin目录：混淆前的modules_static.abc文件。
* 配置信息文件：`config.json`，该文件记录了混淆的配置项和keep配置列表
* debug.txt：开启-enable-bytecode-obfuscation-debugging后，生成混淆过程日志。

![bytecode-static-build-product](figures/bytecode-static-build-product.png)
