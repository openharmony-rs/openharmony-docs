...
**静态字节码混淆**
相比之下，ArkGuard静态字节码混淆主要支持名称混淆，默认混淆函数，类，方法，属性，目录，文件等，但不支持main函数，构造函数和系统函数混淆。
...
静态字节码混淆开启后，函数，类，方法，属性，目录，文件等都将被混淆，如果开发者需要保留部分函数，类，方法，属性，目录或文件，可以通过keep等选项进行配置。
...
控制字节码混淆是否输出调试信息，默认不开启。开启后会生成混淆日志，请参考[混淆效果](bytecode-obfuscation-guide.md#查看字节码混淆效果)。
...
> **注意：**
> 
> 联合类型在使用时，keep中的类型需要使用{}括起来，同时类型需要按照首字母排序，第一个类型需要加上大写U。
...
3.在使用[跨包路由](../ui/arkts-navigation-cross-package.md)进行路由跳转时，传递给动态路由的路径应该被保留。动态路由提供系统路由表和自定义路由表两种方式。

若采用自定义路由表进行跳转，配置白名单的方式与上述第二种动态引用场景一致。

若采用系统路由表进行跳转，则需要将模块下`resources/base/profile/route_map.json`文件中`pageSourceFile`字段对应的路径添加到白名单中。
...
2.对于间接导出的场景，例如`export MyClass`和`let a = MyClass; export {a};`，如果不想混淆它们的属性名，那么需要使用[保留选项](#保留选项)来保留这些属性名。

另外，对于直接导出的类或对象的属性的属性名，例如下面例子中的`firstName`和`personAge`，如果不想混淆它们，那么也需要使用[保留选项](#保留选项)来保留这些属性名。
