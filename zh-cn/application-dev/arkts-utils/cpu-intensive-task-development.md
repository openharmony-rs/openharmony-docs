...
2. 对数据进行分段，并通过任务组发起关联任务调度。
   ...
   创建[TaskGroup/apis-arkts/js-apis-taskpool.md#taskgroup10)，通过[addTask()/apis-arkts/js-apis-taskpool.md#addtask10)添加对应的任务，然后通过[execute()/apis-arkts/js-apis-taskpool.md#taskpoolexecute10)执行任务组，并指定为[HIGH/apis-arkts/js-apis-taskpool.md#priority)优先级。在当前任务组所有任务结束后，会将直方图处理结果同时返回。
...
2. 在宿主线程中首先调用ThreadWorker的[constructor()/apis-arkts/js-apis-worker.md#constructor9)方法创建Worker对象；然后通过注册[onmessage()/apis-arkts/js-apis-worker.md#属性-1)回调接收Worker线程发送过来的消息；最后通过调用[postMessage()/apis-arkts/js-apis-worker.md#postmessage9)方法向Worker线程发送消息。
...
3. 在MyWorker.ets文件中绑定Worker对象，当前线程即为Worker线程。在Worker线程中通过注册[onmessage()/apis-arkts/js-apis-worker.md#属性-2)回调接收宿主线程发送的消息，并通过调用[postMessage()/apis-arkts/js-apis-worker.md#postmessage9-2)方法向宿主线程发送消息。
...
4. 在Worker线程中完成任务后，可以执行销毁操作。销毁方式有两种：一是在宿主线程中销毁Worker线程；二是在Worker线程中主动销毁。
    ...
    在宿主线程中通过调用[onexit()/apis-arkts/js-apis-worker.md#属性)回调定义Worker线程销毁后的处理逻辑。
    ...
    方式一：在宿主线程中通过调用[terminate()/apis-arkts/js-apis-worker.md#terminate9)方法销毁Worker线程，并终止Worker接收消息。
    ...
    方式二：在Worker线程中通过调用[close()/apis-arkts/js-apis-worker.md#close9)方法主动销毁Worker线程，并终止Worker接收消息。
