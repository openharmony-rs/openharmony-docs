...
- **超时：** 短时任务即将超时时（一般在超时前6秒，需在6秒内完成清理并调用cancelSuspendDelay），系统会回调应用，应用需要取消短时任务。如果超时不取消，系统会对应用进行管控，包括进程挂起和进程终止。
