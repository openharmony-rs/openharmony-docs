...
2. 为保证在星闪断连之后能及时恢复连接，本端设备应用在星闪连接之后通过[scan.onDeviceFound](../reference/apis-connectivity-kit/js-apis-nearlink-scan.md#scanondevicefound)订阅星闪扫描结果，断连之后，本端设备通过[startScan](../reference/apis-connectivity-kit/js-apis-nearlink-scan.md#scanstartscan)主动发起星闪扫描，对端设备发送星闪广播。 
3. 本端设备成功扫描到对端设备之后，需要通过[connect](../reference/apis-connectivity-kit/js-apis-nearlink-ssap.md#connect)向对端设备发起连接。如无其他业务扫描需求，本端设备可停止星闪扫描。
