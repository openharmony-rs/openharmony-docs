...
从API版本26.2.0开始，系统提供后台加载任务能力，适用于期望通过后台预先加载应用数据以优化应用启动体验的场景（如资讯刷新、消息获取、视频缓存等），不适用于需要定时或条件触发的通用后台任务及长时间后台运行场景。如需定时或条件触发的后台任务，请使用[延迟任务](work-scheduler.md)；如需长时间后台运行，请使用[长时任务](continuous-task.md)。
