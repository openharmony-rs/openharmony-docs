...
* **自定义弹出框：** 开发者需要根据使用场景，传入自定义组件填充在弹出框中实现自定义的弹出框内容。主要包括基础自定义弹出框 (CustomDialog)、不依赖UI组件的全局自定义弹出框 (openCustomDialog)。
...
| 对话框 (showDialog) | 固定样式，当用户需要处理弹出框响应后的异步返回结果时调用。 |
| 操作菜单 (showActionMenu) | 固定样式，当用户需要处理操作菜单响应后的异步返回结果时调用。 |
| 页面级弹出框 | 页面级弹出框，当用户期望弹出框跟随导航页面切换时使用。 |
| 弹出框层级管理 | 从API version 18开始，可以通过设置levelOrder参数来管理弹出框的显示顺序。 |
...
* 建议<!--Del-->在除ServiceExtensionAbility等无UI界面的场景外，均<!--DelEnd-->使用UIContext中的弹出框方法。
