...
2. 创建一个设置了[NODE_ROTATE_TRANSITION](xx), [NODE_SCALE_TRANSITION](xx), [NODE_TRANSLATE_TRANSITION](xx)属性的节点，当目标节点上下树时会播放转场动画。
...
帧动画具有逐帧回调的特性，便于开发者在每一帧中调整所需属性。通过提供[onFrame](xx)逐帧回调，帧动画允许开发者在应用的每一帧设置属性值，从而实现组件属性值变化的自然过渡，营造出流畅的动画效果。帧动画接口可参考[createAnimator](../reference/apis-arkui/capi-arkui-nativemodule-arkui-nativeanimateapi-1.md#createanimator)。
