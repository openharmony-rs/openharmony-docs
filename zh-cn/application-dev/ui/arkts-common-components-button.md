...
在ArkTS1.1上Button通过调用接口来创建，接口调用通常使用以下两种形式：
...
在ArkTS1.2上使用[ButtonOptions](../reference/apis-arkui/arkui-ts/ts-basic-components-button.md#buttonoptions对象说明)创建该组件时，需要先导入Button以及用来指定参数类型的ButtonOptions：

```ts
import { Button, ButtonOptions, ButtonType } from '@ohos.arkui.component';
Button(label?: string, options?: { type?: ButtonType, stateEffect?: boolean } as ButtonOptions)
```

## 设置按钮类型

Button有四种可选类型，分别为胶囊类型（Capsule）、圆形按钮（Circle）、普通按钮（Normal）和圆角矩形按钮（ROUNDED_RECTANGLE）。

在ArkTS1.1上通过type进行设置。
...
在ArkTS1.2上设置按钮类型时，需先导入ButtonType：

```ts
import { Button, ButtonType } from '@ohos.arkui.component';
```
