...
```ts
import { Entry, Component, Text, $r, Flex, TouchEvent, FlexDirection, FlexDirection, FlexAlign, FontWeight, FontStyle, Image,
  TouchType, ImageFit, BlurOptions, SystemAdaptiveOptions, ItemAlign
  ...
  build() {
    Flex({ direction: FlexDirection.Column, alignItems: ItemAlign.Center, justifyContent: FlexAlign.SpaceBetween }) {
      Text(this.text)
        ...
        .fontStyle(FontStyle.Italic)
      Image($r("app.media.wall"))
        .blur(this.radius, {grayscale: [1, 1]} as BlurOptions, {disableSystemAdaptation: false} as SystemAdaptiveOptions)// 使用blur接口为照片组件添加内容模糊效果
        .height('100%')
        .width("100%")
        .objectFit(ImageFit.Cover)
    }.height('100%')
...
```ts
import { Entry, Component, Text, $r, Column, Image, Grid, GridItem, Margin, Color, TextAlign, FlexAlign, BlurStyle, ThemeColorMode,
  AdaptiveColor, ImageRepeat
} from '@ohos.arkui.component';
import { State } from '@ohos.arkui.stateManagement';
    ...
    Grid() {
      GridItem() {
        Column() {
          ...
          .aspectRatio(1)
          .borderRadius(10)
          .backgroundImage($r('app.media.share'), ImageRepeat.NoRepeat)
...
```ts
import { Entry, Component, $r, Column, Image, Margin, ClickEvent } from '@ohos.arkui.component';
import { State } from '@ohos.arkui.stateManagement';
