...
**动画接口选型建议：**
- 当需要对多个属性做相同参数的动画时，优先选择animateTo。
- 当需要对不同属性做不同参数的动画时，选择animation。
- 当需要对同一属性做分段的连续动画时，选择keyframeAnimateTo。
- animateTo和animation可以配合[转场动画](arkts-transition-overview.md)实现组件出现和消失的动画效果。
