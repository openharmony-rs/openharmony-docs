...
自定义组合提供组件粒度的自定义能力，开发者可通过自定义组件组合已有组件来构建特定的符合业务逻辑的高级组件。组件组合的过程中可以使用布局容器和通用布局属性来对子组件进行自定义的位置排布，可以使用[Canvas](arkts-drawing-customization-on-canvas.md)组件完成纯自绘，也可以使用[Shape/apis-arkui/arkui-ts/ts-drawing-components-shape.md)类组件，通过形状的组合，构建几何图形，实现带交互的图形动画。动画模块可针对不同类型的属性，实现灵活的动画效果。
...
| 组合基础图形实现自定义绘制效果|如果高级组件的UI效果是较为规则化的，例如：矩形、圆形、多边形等基础的图形，以及基础图形组合成的自定义图形，可通过基础的图形类组件（[Rect/apis-arkui/arkui-ts/ts-drawing-components-rect.md)、[Path/apis-arkui/arkui-ts/ts-drawing-components-path.md)、[Circle/apis-arkui/arkui-ts/ts-drawing-components-circle.md)、[Ellipse/apis-arkui/arkui-ts/ts-drawing-components-ellipse.md)、[Polyline/apis-arkui/arkui-ts/ts-drawing-components-polyline.md)、[Polygon/apis-arkui/arkui-ts/ts-drawing-components-polygon.md)）组合完成。 |
...
| 通过@ohos.animator动画接口实现动画效果|更加灵活的是帧动画，通过[@ohos.animator/apis-arkui/js-apis-animator.md)动画接口实现逐帧方法回调，在回调中对节点的属性进行修改，或者结合自定义绘制的能力来调整绘制内容，可以实现帧动画的效果。 |
