...
控制组件缩放、旋转、平移的一类属性。改变仿射属性只让组件"看起来"变形或移动，不影响组件在页面中的实际占位大小和位置。

### Animatable Property；可动画属性

一类组件属性，值发生变化时能自动产生平滑过渡效果，而不是瞬间跳到新值。比如位置、颜色等属性，旧值会逐渐过渡到新值，形成自然的渐变。
...
弹簧动画主体运动结束后，逐渐衰减直到完全静止的微弱抖动阶段。此时运动看起来快停了但还没真正停下来。
