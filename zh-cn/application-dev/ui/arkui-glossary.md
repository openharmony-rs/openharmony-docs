...
控制组件缩放、旋转、平移的一类属性。改变它只让组件"看起来"变形，不影响实际占位。单独修改不产生动画，放在动画闭包（如animateTo）里才能让变化平滑过渡。

### Animatable Property；可动画属性

一类组件属性，单独修改时瞬间生效不产生动画；放在动画闭包（如animateTo）里修改时，值的变化会被自动接管，从旧值平滑过渡到新值，而非瞬间跳变。
