...
用来控制组件缩放、旋转、平移的一类属性。改变仿射属性只会让组件"看起来"移动或变形，不会真正影响组件在页面中的占位和布局，所以运行开销比直接改宽高之类的布局属性更低。

### Animatable Property；可动画属性

一类组件属性，当它的值发生变化时，不会瞬间跳到新值，而是会自动产生一段平滑的过渡动画。比如把一个组件的可动画位置属性从0改到100，组件会缓缓移动过去，而不是一下子跳到100的位置。
...
弹簧动画的主体运动结束之后，还有一小段肉眼可见的微弱抖动，这段逐渐减弱的抖动阶段就叫长尾状态。此时动画看起来已经快停了，但还没有完全静止。
