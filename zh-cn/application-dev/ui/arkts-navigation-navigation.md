...
- 自适应模式
  ...
  在ArkTS-Sta上Navigation组件通过mode属性设置页面的显示模式，需先导入Navigation和NavigationMode。
 ...
 在ArkTS-Sta上Navigation组件通过mode属性设置页面的显示模式，需先导入Navigation和NavigationMode。
  ...
  export struct pageOneTmp {
    build() {
      ...
      .onBackPressed(() => {
        const popDestinationInfo = pageInfos.pop(new Object(), true); // 弹出路由栈栈顶元素
        console.log('pop' + '返回值' + JSON.stringify(popDestinationInfo));
        return true;
      })
    }
...
- Mini模式
  ...
  **图3** Mini模式标题栏  
  ...
  在ArkTS-Sta上Navigation组件通过mode属性设置标题栏模式，需先导入Navigation和NavigationTitleMode。
...
ArkTS-Sta上Navigation组件通过mode属性设置菜单栏模式，需先导入Navigation和NavigationMenuItem。
...
ArkTS-Sta上Navigation组件通过mode属性设置工具栏模式，需先导入Navigation和ToolbarItem。
...
ArkTS-Sta上Navigation路由相关的操作，需先导入Navigation和NavPathStack。
...
在ArkTS-Dyn上NavPathStack通过Push相关的接口去实现页面跳转的功能，主要分为以下三类：
...
在ArkTS-Sta上NavPathStack需要补充完整的Push相关的接口参数，然后导入NavigationOptions，NavPathInfo和PopInfo等模块，去实现页面跳转的功能，主要分为以下三类：
...
在ArkTS-Dyn上NavPathStack通过Pop相关接口去实现页面返回功能。
...
在ArkTS-Sta上NavPathStack需要补充完整Pop相关接口参数，然后导入相关模块去实现页面返回功能。
...
在ArkTS-Dyn上NavPathStack通过Replace相关接口去实现页面替换功能。
...
在ArkTS-Sta上NavPathStack需要补充完整Replace相关接口参数，然后导入NavigationOptions和NavPathInfo等相关模块去实现页面替换功能。
...
在ArkTS-Dyn上NavPathStack通过Move相关接口去实现移动路由栈中特定页面到栈顶的功能。
...
在ArkTS-Sta上NavPathStack需要补充完整Move相关接口参数，然后导入相关模块去实现移动路由栈中特定页面到栈顶的功能。
...
在ArkTS-Sta上NavDestination子页第一次创建时会触发[onReady](../reference/apis-arkui/arkui-ts/ts-basic-components-navdestination.md#onready11)回调，需先导入NavDestination，NavPathStack和NavDestinationContext等模块。
...
- 弹窗类型
  ...
  在ArkTS-Sta上NavDestination设置mode为NavDestinationMode.DIALOG弹窗类型时，需先导入NavDestinationMode。
...
- 页面信息查询
  ...
  在ArkTS-Sta上NavDestination子页面内部的自定义组件通过全局方法监听或查询页面时，需先导入uiObserver。
