...
- 自适应模式
  ...
  在ArkTS1.2上Navigation组件通过mode属性设置页面的显示模式，需先导入Navigation，NavigationMode：

  ```ts
  import { Navigation, NavigationMode } from '@ohos.arkui.component';
  Navigation() {
    // ...
  }
  .mode(NavigationMode.Auto)
...
- 分栏模式
 ...
 在ArkTS1.2上Navigation组件通过mode属性设置页面的显示模式，需先导入Navigation，NavigationMode：
  ...
  import { Provide, State, Consume } from '@ohos.arkui.stateManagement';
  ...
  struct NavigationExample {
    ...
    @Provide({ alias: "pageInfos" }) pageInfos: NavPathStack = new NavPathStack();
    private arr: number[] = [1.0, 2.0, 3.0];
              ...
              ListItem() {
                Text("Page" + item)
                  ...
                  .onClick((e: ClickEvent) => {
                    let info: NavPathInfo = new NavPathInfo("NavDestinationTile" + item, undefined)
                    this.pageInfos.pushPath(info);
                  })
              }
            }, (item: number) => item.toString())
          }
...
- Mini模式
  ...
  **图3** Mini模式标题栏  
  ...
  在ArkTS1.2上Navigation组件通过mode属性设置标题栏模式，需先导入Navigation，NavigationTitleMode：

  ```ts
  import { Navigation, NavigationTitleMode } from '@ohos.arkui.component';
  Navigation() {
    // ...
  }
  .titleMode(NavigationTitleMode.Mini)
...
ArkTS1.2上Navigation组件通过mode属性设置菜单栏模式，需先导入Navigation，NavigationMenuItem：

```ts
import { Navigation, NavigationMenuItem } from '@ohos.arkui.component';
...
ArkTS1.2上Navigation组件通过mode属性设置工具栏模式，需先导入Navigation，ToolbarItem：

```ts
import { Navigation, ToolbarItem } from '@ohos.arkui.component';
...
ArkTS1.2上Navigation路由相关的操作，需先导入Navigation，NavPathStack：

```ts
import { Entry, Component, Navigation, NavPathStack } from '@ohos.arkui.component';
@Entry
@Component
struct Index {
  // 创建一个导航控制器对象并传入Navigation
...
在ArkTS1.1上NavPathStack通过Push相关的接口去实现页面跳转的功能，主要分为以下三类：
...
在ArkTS1.2上NavPathStack需要补充完整的Push相关的接口参数，然后导入NavigationOptions，NavPathInfo，PopInfo等模块，去实现页面跳转的功能，主要分为以下三类：

1. 普通跳转，通过页面的name去跳转，并可以携带param。

    ```ts
    import { NavigationOptions, NavPathInfo } from '@ohos.arkui.component';
    let info: NavPathInfo = new NavPathInfo("PageOne", "PageOne Param");
    this.pageStack.pushPath(info, {} as NavigationOptions);
    this.pageStack.pushPathByName("PageOne", new Object(), true);
    ```

2. 带返回回调的跳转，跳转时添加onPop回调，能在页面出栈时获取返回信息，并进行处理。

    ```ts
    import { NavigationOptions, NavPathInfo, PopInfo } from '@ohos.arkui.component';
    let info: NavPathInfo = new NavPathInfo("PageOne", "PageOne Param");
    this.pageStack.pushPathByName("PageOne", new Object(), (info：PopInfo) => {
      console.log('Pop page name is: ' + PopInfo.info.name + ', result: ' + JSON.stringify(PopInfo.result));
    }, true);
    ```
...
3. 带错误码的跳转，跳转结束会触发异步回调，返回错误码信息。
    ...
    import { NavigationOptions, NavPathInfo } from '@ohos.arkui.component';
    let info: NavPathInfo = new NavPathInfo("PageOne", "PageOne Param");
    this.pageStack.pushDestination(info, {} as NavigationOptions)
      .catch((error: BusinessError) => {
        console.error(`Push destination failed, error code = ${error.code}, error.message = ${error.message}.`);
      }).then(() => {
        console.info('Push destination succeed.');
      });
    this.pageStack.pushDestinationByName("PageOne", new Object(), true)
      .catch((error: BusinessError) => {
        console.error(`Push destination failed, error code = ${error.code}, error.message = ${error.message}.`);
      }).then(() => {
        console.info('Push destination succeed.');
...
在ArkTS1.1上NavPathStack通过Pop相关接口去实现页面返回功能。
...
在ArkTS1.2上NavPathStack需要补充完整Pop相关接口参数，然后导入相关模块去实现页面返回功能。
...
在ArkTS1.1上NavPathStack通过Replace相关接口去实现页面替换功能。
...
在ArkTS1.2上NavPathStack需要补充完整Replace相关接口参数，然后导入NavigationOptions，NavPathInfo等相关模块去实现页面替换功能。
...
this.pageStack.replacePathByName("PageOne", new Object(), true);
// 带错误码的替换，跳转结束会触发异步回调，返回错误码信息
this.pageStack.replaceDestination(info, {} as NavigationOptions)
  .catch((error: BusinessError) => {
    console.error(`Replace destination failed, error code = ${error.code}, error.message = ${error.message}.`);
  }).then(() => {
    console.info('Replace destination succeed.');
...
在ArkTS1.1上NavPathStack通过Move相关接口去实现移动路由栈中特定页面到栈顶的功能。
...
在ArkTS1.2上NavPathStack需要补充完整Move相关接口参数，然后导入相关模块去实现移动路由栈中特定页面到栈顶的功能。

```ts
// 移动栈中name为PageOne的页面到栈顶
this.pageStack.moveToTop("PageOne", true);
// 移动栈中索引为1的页面到栈顶
this.pageStack.moveIndexToTop(1, true);
```
...
在ArkTS1.2上NavDestination子页第一次创建时会触发[onReady](../reference/apis-arkui/arkui-ts/ts-basic-components-navdestination.md#onready11)回调，需先导入NavDestination，NavPathStack，NavDestinationContext等模块。

```ts
import { Component, NavDestination, NavPathStack, NavDestinationContext } from '@ohos.arkui.component';
...
- 弹窗类型
  ...
  在ArkTS1.2上NavDestination设置mode为NavDestinationMode.DIALOG弹窗类型时，需先导入NavDestinationMode。
  ...
  @Component
  struct Index {
    @Provide({ alias: 'NavPathStack' }) pageStack: NavPathStack = new NavPathStack();
    ...
    build() {
      Navigation(this.pageStack) {
        Button('Push DialogPage').margin(20).width('80%').onClick((e: ClickEvent) => {
          this.pageStack.pushPathByName('DialogPage', new Object(), (info: PopInfo) => {
          }, true)
        })
...
- 页面信息查询
  ...
  在ArkTS1.2上NavDestination子页面内部的自定义组件通过全局方法监听或查询页面时，需先导入uiObserver。

  ```ts
  import uiObserver from '@ohos.arkui.observer';
  import { Component, NavDestination, NavDestinationInfo } from '@ohos.arkui.component';
...
- 全局关闭
  ...
  ```ts
  import { NavPathStack } from '@ohos.arkui.component';
  pageStack: NavPathStack = new NavPathStack();
...
- 单次关闭
  ...
  this.pageStack.pushPath({ name: "PageOne" }, false);
  this.pageStack.pop(new Object(), false);
  ```
...
1. 为需要实现共享元素转场的组件添加geometryTransition属性，id参数必须在两个NavDestination之间保持一致。
   ...
   ArkTS1.1示例：
    ...
    ```ts
    // 起始页配置共享元素id
    import { NavDestination, Column, $r, Image } from '@ohos.arkui.component';
    NavDestination() {
      Column() {
        // ...
        Image($r('app.media.startIcon'))
...
2. 将页面路由的操作，放到animateTo动画闭包中，配置对应的动画参数以及关闭系统默认的转场。
    ...
    ```ts
    import { NavDestination, Column, Button, ClickEvent } from '@ohos.arkui.component';
    NavDestination() {
      Column() {
        Button('跳转目的页')
        .width('80%')
...
4. 通过pushPathByName等路由接口进行页面跳转。(注意：此时Navigation中可以不用配置navDestination属性。)
   ...
   ```ts
    import { Entry, Component, NavPathStack, Navigation } from '@ohos.arkui.component';
...
@Component
struct NavigationDemo {
  @Provide({ alias: 'pathInfos' }) pathInfos: NavPathStack = new NavPathStack();
  private listArray: Array<string> = ['WLAN', 'Bluetooth', 'Personal Hotspot', 'Connect & Share'];
  ...
  build() {
    Column() {
          ...
          ForEach(this.listArray, (item: string, index: number) => {
            ListItem() {
              Row() {
                Row() {
                  Text(`${item.slice(0, 1)}`)
                    .fontColor(Color.White).fontSize(14).fontWeight(FontWeight.Bold)
                }
                .width(30)
                .height(30)
                .backgroundColor('#a8a8a8')
                ...
                Column() {
                  Text(item).fontSize(16).margin({ bottom: 5 } as Margin)
                }.alignItems(HorizontalAlign.Start)
                ...
                Row()
                  ...
                  .border({ width: { top: 2, right: 2 }, color: 0xcccccc })
                  .rotate({ angle: 45 })
              }
              .borderRadius(15)
              ...
              this.pathInfos.pushPathByName(`${item}`, {} as Object, (info: PopInfo)=>{}, true); // 将name指定的NaviDestination页面信息入栈,传递的参数为param
            })
      ...
      .mode(NavigationMode.Auto)
      .title('设置') // 设置标题文字
    }.size({ width: '100%', height: '100%' }).backgroundColor(0xf4f4f5)
  }
}
```
        ...
        List({ space: 12, initialIndex: 0 }) {
          ForEach(this.listArray, (item: string) => {
            ListItem() {
              Row() {
                Row() {
                  Text(`${item.slice(0, 1)}`).fontColor(Color.White).fontSize(14).fontWeight(FontWeight.Bold)
                }
                .width(30)
                .height(30)
                .backgroundColor('#a8a8a8')
              ...
              this.pathInfos.pushPathByName(`${item}`, {} as Object, (info: PopInfo)=>{}, true);
            })
          }, (item: string): string => item)
        }.listDirection(Axis.Vertical).edgeEffect(EdgeEffect.Spring).sticky(StickyStyle.Header).width('100%')
      }.size({ width: '100%', height: '100%' })
    }.title(`${this.name}`)
    .onReady((context: NavDestinationContext) => {
      hilog.info(0x0000, 'testTag', 'enter onReady' + JSON.stringify(context));
