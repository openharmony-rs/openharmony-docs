...
在ArkTS1.1上，通过配置菜单项图标和文本的数组，来弹出菜单：
...
在ArkTS1.2上，通过配置菜单项图标和文本的数组弹出菜单时，需要先构造类型为MenuElement的数组：
...
在ArkTS1.2上使用@Builder自定义菜单内容时，需先导入Builder：
...
struct menuExample {
  ...
  MyMenu() {
    Menu() {
      ...
      MenuItem({ startIcon: '', content: "菜单选项" } as MenuItemOptions).enabled(false)
      MenuItem({
        startIcon: this.iconStr,
        content: "菜单选项",
        endIcon: '',
      ...
      MenuItemGroup({ header: '小标题' } as MenuItemGroupOptions) {
...
- 确认菜单的弹出方式，并使用bindContextMenu属性绑定组件。示例中为右键弹出菜单。
  ...
  ArkTs1.2上使用右键弹出菜单时，需先导入ResponseType：
