...
当前的系统支持两种[应用模型](../application-models/stage-model-development-overview.md)——[FA模型](../application-models/ability-terminology.md#fa模型)和Stage模型。在FA模型中，每个UI实例拥有独立的ArkTS引擎，全局接口可以通过ArkTS引擎跟踪到对应的UI实例上，因此不存在UI上下文不明确的问题。
...
|         [animateToImmediately](../reference/apis-arkui/arkts-apis-uicontext-uicontext.md#animatetoimmediately23)          |                不支持                 |        显式立即动画        |
...
@Component
struct Index {
  @State pxValue: number = 0;

  build() {
    RelativeContainer() {
      Text(this.pxValue > 0 ? `20vp = ${this.pxValue}px` : 'Calculate 20vp to px')
        .fontWeight(FontWeight.Bold)
        .alignRules({
          center: { anchor: '__container__', align: VerticalAlign.Center },
          middle: { anchor: '__container__', align: HorizontalAlign.Center }
        ...
        .onClick(() => {
          this.pxValue = this.getUIContext().vp2px(20);
        })
    }
    .height('100%')
    .width('100%')
...
>**说明：**
>
>1. 必须在UI实例创建完成后，才可以通过窗口对象的getUIContext方法获取UIContext。建议在[loadContent](../reference/apis-arkui/arkts-apis-window-Window.md#loadcontent9)的成功回调中调用，以确保UI实例准备就绪。
  ...
  build() {
    RelativeContainer() {
      Text('Calculate 20vp to px')
        ...
        .onClick(() => {
          // 在有UI实例且上下文明确时调用，此时会根据此时UI上下文对应的实例的像素密度返回计算结果。
          let pxValue = this.getUIContext().vp2px(20);
        })
    }
    .height('100%')
    .width('100%')
...
export default class EntryAbility extends UIAbility {
  ...
  onWindowStageCreate(windowStage: window.WindowStage): void {
    ...
    // loadContent是异步接口，在loadContent之后并不能保证UI实例已经创建并初始化。此时调用例如vp2px等依赖UI实例的函数，可能因实例不正确导致结果不符合预期。
  }
...
class PixelUtils {
  static vp2px(vpValue: number): number {
    return UIContext.resolveUIContext().vp2px(vpValue);
  }

  static fp2px(fpValue: number): number | undefined {
    return UIContext.resolveUIContext().fp2px(fpValue);
  }

  static lpx2px(lpxValue: number): number | undefined {
    return UIContext.resolveUIContext().lpx2px(lpxValue);
  }
}
...
对于UIContext中没有提供替代的接口（例如[CalendarPickerDialog](../reference/apis-arkui/arkui-ts/ts-methods-calendarpicker-dialog.md)），或者开发者自定义实现的业务行为与多实例相关，需要和实例绑定时（例如，一个代码段），可以使用UIContext对象[runScopedTask](../reference/apis-arkui/arkts-apis-uicontext-uicontext.md#runscopedtask)方法执行闭包。
...
export class PixelUtils {
  ...
  static vp2px(vpValue: number): number {
    return PixelUtils.uiContext?.vp2px(vpValue) ?? 0;
  }

  static fp2px(fpValue: number): number | undefined {
    return PixelUtils.uiContext?.fp2px(fpValue);
  }

  static lpx2px(lpxValue: number): number | undefined {
    return PixelUtils.uiContext?.lpx2px(lpxValue);
  }
}
```
