...
- 如果要在滑动过程中定制动效，可在滑动回调[onDidScroll](../reference/apis-arkui/arkui-ts/ts-container-grid.md#ondidscroll)中监控滑动距离，并计算每个组件的仿射属性。也可以自己定义手势，通过手势监控位置，手动调用ScrollTo改变滑动位置。
