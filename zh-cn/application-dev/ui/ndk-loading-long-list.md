...
参考添加事件监听章节实现列表滚动事件监听。 

## 使用懒加载 

### NodeAdapter介绍 
NDK提供了NodeAdapter对象替代ArkTS侧的LazyForEach功能，用于按需生成子组件，NodeAdapter支持在List/ListItemGroup、Grid、WaterFlow、Swiper组件中使用。

- 设置了NodeAdapter属性的节点，不支持直接通过addChild等接口添加子组件。子组件完全由NodeAdapter管理，使用属性方法设置NodeAdapter时，会判断父组件是否已经存在子节点，如果父组件已经存在子节点，则设置NodeAdapter操作失败，返回错误码。
