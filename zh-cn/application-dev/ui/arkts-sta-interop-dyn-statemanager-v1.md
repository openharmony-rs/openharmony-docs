...
ArkTS-Sta使用ArkTS-Dyn管理组件拥有的状态，涉及状态管理V1交互的场景主要包括：
...
struct Index { // 使用包含@State成员属性的子组件
  ...
  build() {
    Column() {
      // 支持初始化子组件的@State成员属性
      // 子组件@State状态的变化不会同步给ArkTS-Sta父组件
...
export struct Child { // 定义包含@Prop成员属性的子组件
  ...
  build() {
    Column() {
      Button(`click times: ${this.count}`)
        .onClick(() => {
          // 子组件@Prop状态的变化不会同步给ArkTS-Sta父组件
          this.count += 1;
        })
    }
  }
...
export struct Child { // 包含@Link成员属性的子组件
  ...
  build() {
    Column() {
      Button(`click times: ${this.count}`)
        .onClick(() => {
          // 子组件@Link状态的变化会同步给ArkTS-Sta父组件，并刷新UI
          this.count += 1;
        })
    }
  }
...
struct Index { // 使用包含@Link成员属性的子组件
  ...
  build() {
    Column() {
      ...
      Button('change value')
        .onClick((value: ClickEvent) => {
          // 父组件状态的变化会同步给ArkTS-Dyn子组件，并刷新UI
          this.count += 2;
        })
    }
  }
...
@Component
export struct Child { // 包含@Provide和@Consume成员属性的子组件
  ...
  build() {
    Column() {
      ...
      Button(`click times: ${this.count}`)
        .onClick(() => {
          // 子组件的状态变化会同步给ArkTS-Sta父组件，并刷新UI
          this.count += 1;
        })
    }
  }
...
struct Index { // 使用包含@Provide成员属性的子组件
  ...
  build() {
    Column() {
      ...
      Button('change value')
        .onClick((value: ClickEvent) => {
          // 祖先组件的状态变化会同步给ArkTS-Dyn子组件，并刷新UI
          this.count += 2;
        })
    }
  }
