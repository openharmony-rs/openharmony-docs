...
RichEditor是支持图文混排和文本交互式编辑的组件，通常用于响应用户对图文混排内容的输入操作，例如可以输入图文的评论区。具体用法参考RichEditor组件的API文档。
...
可通过addSymbolSpan添加Symbol内容。此接口可用于Symbol矢量图标的添加，例如在文本中插入图标、标志等可视化符号元素。
...
从API version 20开始，支持通过textVerticalAlign属性设置文本段落在垂直方向的对齐方式。
...
从API version 20开始，支持通过enableAutoSpacing属性设置是否开启中文与西文的自动间距。
...
从API version 18开始，支持通过maxLines属性设置富文本组件内可显示文本的最大行数。
