...
| `poolAccepts` | **ArkTS-Dyn:** Function[]<br>**ArkTS-Sta:** string[] | 否 | 可复用组件名称的数组，可以同时包含@Reusable和@ReusableV2装饰的自定义组件。 |
...
| IReusableInfo（动态ArkTS接口[IReusableInfo](../../reference/apis-arkui/js-apis-stateManagement.md#ireusableinfo)，静态ArkTS接口[IReusableInfo](../../reference/apis-arkui/js-apis-stateManagement-static.md#ireusableinfo)）.maxCount | 允许的最大回收组件数。设置此项可控制缓存大小。 |
|IReusableInfo（动态ArkTS接口[IReusableInfo](../../reference/apis-arkui/js-apis-stateManagement.md#ireusableinfo)，静态ArkTS接口[IReusableInfo](../../reference/apis-arkui/js-apis-stateManagement-static.md#ireusableinfo)）.reuseId | 全局复用池基于reuseId分区存放复用组件，该分区对应的reuseId（只读）。 |
