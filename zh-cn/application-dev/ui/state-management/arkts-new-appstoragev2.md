...
AppStorageV2是在应用UI启动时会被创建的单例。它用于提供应用状态数据的中心存储，这些状态数据在应用级别都是可访问的。AppStorageV2将在应用运行过程保留其数据。数据通过唯一的键字符串值访问。需要注意的是，[AppStorage](./arkts-appstorage.md)与AppStorageV2之间的数据互不共享。

AppStorageV2可以修改[connect](../../reference/apis-arkui/js-apis-stateManagement.md#connect)的返回值，实现与UI组件的同步。
