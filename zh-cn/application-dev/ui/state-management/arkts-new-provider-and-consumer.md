...
| alias和属性名         | alias是唯一匹配的key，缺省时默认alias为属性名。 | alias和属性名都为key，优先匹配alias，匹配不到可以匹配属性名。|
...
3. 建立双向同步的关系后，如果\@Provider装饰变量的值和\@Consumer的默认值不同，则会回调\@Consumer的[\@Monitor](./arkts-new-monitor.md)方法，以及与\@Consumer有同步关系的变量的\@Monitor方法，例如：\@Consumer通知其子组件中的\@Param触发\@Monitor方法。
