...
| storage | [LocalStorage](arkts-localstorage.md) | 否 | 是 | 页面级的UI状态存储。 |
| useSharedStorage<sup>12+</sup> | boolean | 否 | 是 | 是否使用LocalStorage.getShared()接口返回的共享的[LocalStorage](arkts-localstorage.md)实例对象。默认值false。<br>true表示使用共享的[LocalStorage](arkts-localstorage.md)实例对象。<br>false表示不使用共享的[LocalStorage](arkts-localstorage.md)实例对象。 |
