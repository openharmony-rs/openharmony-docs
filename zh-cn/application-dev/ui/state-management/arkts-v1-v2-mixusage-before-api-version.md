...
在V1中使用V2的自定义组件时，如果不存在变量传递，则不会产生影响。以下示例代码中，Child是不接受参数的V2自定义组件，Index可直接使用Child。
...
以下代码示例中，定义了Child为V2组件，组件接受message、undefinedVal、info等参数。Child中用\@Param接收的简单类型message和undefinedVal，能观测到变化；Class类型变量info未被\@ObservedV2和\@Trace修饰，无法观测到类属性变化。
  ...
  build() {
    Column() {
      ...
      Text(`undefinedVal:${this.undefinedVal}`) // 显示 undefinedVal
        ...
        .fontWeight(FontWeight.Bold)
        .onClick(() => {
          this.undefinedVal = 'change to define'; // 刷新当前组件UI
        })
      Divider()
        .color(Color.Blue)
...
- 最外层MessageInfoNested类被\@Observed修饰，在V1组件Index中可以被\@State修饰。数据源\@State的第二层的改变（info和messageId属性），虽不能触发本层的刷新，但会被\@ObjectLink和\@Param观察到，并触发它们关联组件的刷新。

- messageInfo属性传递给V1组件，V1组件Child要用\@ObjectLink接收，而传递给V2组件GrandSon的info属性的class类用\@ObservedV2修饰。

- \@Track防止MessageInfo类中的info因messageId改变而连带刷新，开发者去掉\@Track可观测到，当messageId改变时，info的连带刷新，但这并非\@ObjectLink的观测能力。
...
struct Child {
  ...
  build() {
    Column() {
      ...
      GrandSon({info1: this.messageInfo.info}); // 继续拆解一层子组件
    }
  }
}
...
struct GrandSon {
  ...
  build() {
    Column() {
      Text(`ObjectLink info info.myId:${this.info1.myId}`) // myId属性被@Trace装饰，可以观测变化
        .fontSize(30)
        .onClick(() => {
          this.info1.myId++; // 当前组件和父组件Child1都刷新
        })
    }
  }
}
...
V2向V1自定义组件传递数据时，支持普通class类。在以下示例代码中，Info类未被\@ObservedV2修饰，传递给V1组件Child时，可以使用\@State接收。修改V1组件中的info变量，依赖\@State的观测能力刷新UI。
...
@ComponentV2
struct Index {
  @Provider() info: Info = new Info(); // Class类型，支持传递
...
@Component
struct Child {
  info: Info = new Info(); // V1装饰器不能和@ObservedV2一起使用
...
在以下示例代码中，V2向V1自定义组件传递set变量，V1组件使用\@Provide接收。因此，在V2组件Index中定义set变量时，为避免编译错误，set变量不能用\@Local修饰。
