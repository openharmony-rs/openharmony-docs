...
为了动态添加或删除状态管理V2的状态变量的监听函数，开发者可以使用[addMonitor/apis-arkui/js-apis-stateManagement.md#addmonitor20)或[clearMonitor/apis-arkui/js-apis-stateManagement.md#clearmonitor20)。
...
如果开发者希望动态给\@ObservedV2和\@ComponentV2实例添加或者删除监听函数，则可以使用[addMonitor/apis-arkui/js-apis-stateManagement.md#addmonitor20)和[clearMonitor/apis-arkui/js-apis-stateManagement.md#clearmonitor20)接口。
...
```
- addMonitor设置[isSynchronous/apis-arkui/js-apis-stateManagement.md#monitoroptions20)仅第一次有效，即其不能被更改，如果开发者更改`isSynchronous`，则会打印错误日志。
```ts
import { UIUtils } from '@kit.ArkUI';
