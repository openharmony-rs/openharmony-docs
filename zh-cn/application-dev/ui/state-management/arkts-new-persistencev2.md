...
@ObservedV2
class SampleChild {
  // 应用持久化点击的数量，默认为0，如果点击后退出再次进入，则默认为持久化的数量
  @Trace public childId: number = 0;
  // 应用持久化点击的数量，默认为1，如果点击后退出再次进入，则默认为持久化的数量
...
@ComponentV2
struct Page1 {
  // 进入当前页面后点击了Persist keys:及之后的内容多少次
...
@ObservedV2
class SampleChild {
  // 应用持久化Key connect2:点击的数量，默认为0，如果点击后退出再次进入，则默认为持久化的数量
  @Trace public childId: number = 0;
  // 应用持久化save key Sample:点击的数量，默认为1，如果点击后退出再次进入，则默认为持久化的数量
  public groupId: number = 1;
}
