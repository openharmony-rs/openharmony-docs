...
- 状态管理框架已提供[@ObservedV2/@Trace](./arkts-new-observedV2-and-trace.md)用于观察类属性变化，makeObserved接口提供主要应用于@ObservedV2/@Trace无法涵盖的场景：
  ...
  - interface或者[JSON.parse](../../reference/apis-arkts/js-apis-json.md#jsonparse)返回的匿名对象：这类场景往往没有明确的class声明，开发者无法使用@Trace标记当前属性可以被观察，此时可以使用makeObserved。


- 使用makeObserved接口需要导入[UIUtils](../../reference/apis-arkui/js-apis-stateManagement.md#uiutils)。
  ```ts
