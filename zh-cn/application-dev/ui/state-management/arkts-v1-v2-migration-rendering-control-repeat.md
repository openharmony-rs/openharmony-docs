...
1. 使用状态管理V2装饰器。
   ...
   Repeat推荐和状态管理V2装饰器配合使用。（懒加载模式下只支持和状态管理V2装饰器配合使用，详情见[循环渲染能力说明](../rendering-control/arkts-new-rendering-control-repeat.md#循环渲染能力说明)）。如果之前使用的是状态管理V1装饰器，需要修改为状态管理V2装饰器。
...
4. 配置懒加载功能。
  ...
  - 全量加载模式渲染所有子节点（对标[ForEach](../rendering-control/arkts-rendering-control-foreach.md)），详情见[关闭懒加载](../rendering-control/arkts-new-rendering-control-repeat.md#关闭懒加载)。

  - 懒加载模式动态渲染屏幕区域和预加载区域内的子节点（需要与容器组件配合使用，对标LazyForEach），详情见[循环渲染能力说明](../rendering-control/arkts-new-rendering-control-repeat.md#循环渲染能力说明)。
