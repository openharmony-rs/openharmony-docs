...
export struct DocSampleNestedClass {
  ...
  @Monitor({}, 'class0.class1.person.*') // 使能通配符，监听嵌套对象
...
@Entry
@ComponentV2 struct DocMonitorDate {
...
struct DocMonitorMap {
  ...
  @Monitor({ enableWildcard: true }, 'map.*') // 使能通配符
...
@Entry
@ComponentV2 struct DocMonitorSet {
  @Local set: Set<string> = new Set<string>();
  cnt: number = 0;

  @Monitor({ enableWildcard: true }, 'set.*') // 使能通配符
