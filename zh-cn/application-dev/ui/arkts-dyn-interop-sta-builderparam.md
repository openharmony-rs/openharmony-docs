...
从API version 22开始，支持在ArkTS-Dyn中使用[\@Builder](./state-management/arkts-builder.md)函数初始化ArkTS-Sta自定义组件的\@BuilderParam成员属性。
...
- 创建ArkTS-Sta子模块`static_module`，在`static_module/src/main/ets/components`目录创建并导出自定义组件以及`Person`类。如何创建子模块参考共享包（[HAR](../quick-start/har-package.md)）说明。
...
@Observed
export class Person { // ArkTS-Sta侧的对象字面量类
  name: string = '';
  age: number = 0;
}
...
import { Person, Child } from 'static_module'; // 引入ArkTS-Sta侧的Person类与自定义组件
