...
- 完全自定义节点：提供完整的自定义能力，包括自定义测量、布局以及绘制，支持节点的动态增、删，设置通用属性，设置事件回调。适用于不自带渲染引擎，需要依赖系统的布局、事件、动画、渲染等能力的场景。通过FrameNode的构造函数constructor创建的节点或者通过typeNode的节点创建方法（例如：createNode('Text')、createNode('Column')、createNode('Row')等）创建出来的节点属于完全自定义节点。

- 系统组件代理节点：提供系统组件的代理能力，提供遍历节点树的能力，通过组件树上的FrameNode可以遍历整个组件树，并通过节点访问组件的信息或者注册额外的事件监听回调。适用于结合无感监听的接口实现打点、广告SDK、中台DFX等业务。通过FrameNode相关的查询接口（例如：getChild、getParent、getNextSibling、getAttachedFrameNodeById等）返回的FrameNode对象，若不属于完全自定义节点则为系统组件代理节点。
