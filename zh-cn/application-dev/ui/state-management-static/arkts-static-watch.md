...
| 装饰器的顺序         | 装饰器顺序不影响实际功能，开发者可以根据自己的需要决定装饰器顺序的先后。建议[\@State](./arkts-static-state.md)、 [\@PropRef](./arkts-static-propref.md)、 [\@Link](./arkts-static-link.md)等装饰器在\@Watch装饰器之前，以保持整体风格的一致。 |
