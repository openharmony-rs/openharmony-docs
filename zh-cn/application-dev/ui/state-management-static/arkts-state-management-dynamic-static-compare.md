...
| 嵌套类型标记 | class中使用[\@Type](../state-management/arkts-new-type.md)装饰器标记嵌套类型属性 | 不使用\@Type，统一通过[StorageDefaultSubCreators](../../reference/apis-arkui/js-apis-stateManagement-static.md#connectoptions)批量注册 |
| 子元素构造器 | [defaultSubCreator](../../reference/apis-arkui/js-apis-stateManagement.md#connectoptions)：传入单个工厂函数<br>`defaultSubCreator: () => new Person()` | [defaultSubCreators](../../reference/apis-arkui/js-apis-stateManagement-static.md#connectoptions)：传入StorageDefaultSubCreators构造器集合<br>`defaultSubCreators: creators` |
...
2. **使用[StorageDefaultSubCreators](../../reference/apis-arkui/js-apis-stateManagement-static.md#connectoptions)批量注册嵌套类型**：将动态中\@Type装饰器 + [defaultSubCreator](../../reference/apis-arkui/js-apis-stateManagement.md#connectoptions)的方式，替换为通过StorageDefaultSubCreators一次性注册所有嵌套类型（包括Person、Info、Inner）的构造器，每个构造器使用 `Class.from<>()` 指定类型。
