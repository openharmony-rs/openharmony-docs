...
> **说明：**
>
> 从API version 23开始，开发者可以在静态语言上下文中使用\@Computed装饰器。
   ...
   struct Page {
     ...
     build() {
       Column() {
         ...
         Button('get fullName').onClick((e: ClickEvent) => {
...
- 在\@Computed装饰的getter方法中，不能改变状态变量的值，否则会抛出运行时异常。API version 26.0.0以后，在@Computed方法中修改状态变量会编译报错，报错消息是“State variables cannot be modified within a getter function decorated with '@Computed'.”。
...
- \@Computed装饰的是getter访问器，不会被子组件同步，也不能被赋值。开发者自己实现的计算属性的setter不生效，且产生编译时报错。
   ...
   @ComponentV2
   struct Child {
     @Param aDouble: number = 100;
     @Event changeDouble: (val: number) => void;
   
     build() {
       Button('ChildChange')
         .onClick((e: ClickEvent) => {
           this.changeDouble(200);
         })
     }
   }
     ...
     build() {
       Scroll() {
         Column({ space: 3 } as ColumnOptions) {
           ...
           Child({ aDouble: this.aDouble, changeDouble: (val: number) => { this.aDouble = val; } })
         }
       }
     }
   }
...
- 点击“+”，celsius++ -> fahrenheit -> kelvin --> kelvin变化时调用onKelvinMonitor。
   ...
   struct MyView {
     ...
     onKelvinMonitor(mon: IMonitor) {
       console.info('kelvin changed from ' + mon.value<Any>()?.before + ' to ' + mon.value<Any>()?.now);
     }
