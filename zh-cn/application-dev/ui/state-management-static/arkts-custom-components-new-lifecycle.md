...
2. 自定义组件和它的变量将被删除，如果组件有同步的变量（如[@Link](../state-management/arkts-link.md)、[@Prop](../state-management/arkts-prop.md)、[@StorageLink](../state-management/arkts-appstorage.md#storagelink)），将从[同步源](../state-management/arkts-state-management-overview.md#基本概念)上取消注册。
