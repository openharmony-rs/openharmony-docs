...
在ArkUI中，UI显示的内容均为组件，框架直接提供的称为系统组件，开发者定义的称为自定义组件。进行UI界面开发时，需组合使用系统组件，考虑代码的可复用性、业务逻辑与UI的分离，以及后续版本的演进。因此，将UI和部分业务逻辑封装成自定义组件是必需的。此外，相比动态自定义组件，自定义组件需要通过import才能使用。
...
> **说明：**
>
> 如果在其他文件中引用自定义组件，需要使用`export`关键字导出组件，并在页面中`import`该自定义组件。

通过在其他自定义组件的`build()`函数中多次创建`HelloComponent`，可以实现自定义组件的重用。
...
自定义组件通过 `struct` 实现，`struct` 加上自定义组件名及大括号内的内容构成自定义组件，不允许有继承关系。实例化 `struct` 时，可以省略 `new`。
...
[@ComponentV2](./arkts-static-componentv2.md)主要配合状态管理V2使用。除非特别说明，自定义组件使用@ComponentV2装饰后将与@Component装饰的组件保持相同的行为。
 ...
 一个简单的\@ComponentV2装饰的自定义组件应该包含以下部分：
  ```ts
...
\@Entry 装饰的自定义组件作为 UI 页面的入口。单个 UI 页面中，仅允许一个 @Entry 装饰的自定义组件作为入口。@Entry 可接受一个可选的 [LocalStorage](arkts-static-localstorage.md) 参数。
...
在 `build()` 函数中声明的所有语句统称为 UI 描述，UI 描述需遵循以下规则：
...
build() 函数支持写非UI的逻辑，包括变量声明，switch/case, 打印日志等， 但是开发者不能写太耗时的操作，否则会阻塞UI主线程，影响应用界面的渲染性能。
...
struct MyStateSample {
  ...
  build() {
    hilog.info(0X0000, 'testTag', `${this.stateVar}`); // 在build中打印日志
    Column() {
      Text('show text1')
...
**在 build() 函数中调用耗时的同步接口**

在build()函数中，不建议编写非UI逻辑，如调用剪切板接口[getDataSync](../../../application-dev/reference/apis-basic-services-kit/js-apis-pasteboard.md)获取剪切板数据、执行for循环等，这些操作会增加构建时间，影响UI性能。
