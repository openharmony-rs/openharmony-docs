...
<!-- @[lazy_custom_layout_algorithm_class](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/LazyColumnLayoutAlgorithm.ets) -->
...
export class LazyColumnLayoutAlgorithm extends LazyCustomLayoutAlgorithm {
  // === 核心布局数据 ===
  private itemArr: Array<ItemPos> = new Array<ItemPos>(); // 已测量元素的位置数组
  private itemArrStartIndex: int = -1;       // itemArr[0] 对应的实际元素索引
  private estimateItemHeight: double = -1;   // 估算的平均元素高度（用于未知区域的测量）
  private totalHeight: double = 0;           // 总高度（包含未测量区域的估算高度）

  // === 元素信息 ===
  private childCnt: int = 0;                // 子元素总数
  private startIndex: int = -1;             // 当前可视范围的第一个元素索引
  private endIndex: int = -1;               // 当前可视范围的最后一个元素索引

  // === 布局参数 ===
  private rowGap: double = 0;               // 元素间距
  private leftPadding: double = 0;          // 左内边距
  private rightPadding: double = 0;         // 右内边距
  private topPadding: double = 0;           // 上内边距
  private bottomPadding: double = 0;        // 下内边距

  // === 状态追踪 ===
  private prevRowGap: double = 0;           // 上一次的间距（用于检测变化）
  private selfNode?: FrameNode;             // 节点引用（用于触发重新布局）
  private prevStartIndex: int = -1;         // 上一次布局完成的起始索引号
  private prevEndIndex: int = -1;           // 上一次布局完成的末尾索引号

  // === 锚点信息（滚动时保持稳定） ===
  private anchorChildIndex: int = -1;                 // 锚点元素的索引
  private anchorChildRelativePos: double = 0;         // 锚点位置（正向距内容顶部，反向距内容底部）
```
...
private convertToPx(lengthMetrics: LengthMetrics | undefined, referenceSize?: double): double {
  if (!lengthMetrics) {
    return 0;
  }

  const value = lengthMetrics.value;
  const unit = lengthMetrics.unit;
  ...
  switch (unit) {
    case LengthUnit.PX:
      return value;
    case LengthUnit.VP:
      return context ? context.vp2px(value) : value;
    case LengthUnit.PERCENT:
      return referenceSize ? (value) * referenceSize : 0;
    case LengthUnit.FP:
      return context ? context.fp2px(value) : value;
    case LengthUnit.LPX:
      return context ? context.lpx2px(value) : value;
    default:
      return value;
  }
}
...
<!-- @[lazy_custom_layout_algorithm_set_row_gap](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/LazyColumnLayoutAlgorithm.ets) -->

``` TypeScript
setRowGap(value: double): void {
  if (this.rowGap === value) {
    return;
  }
  this.prevRowGap = this.rowGap;
...
<!-- @[lazy_custom_layout_algorithm_on_measure](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/LazyColumnLayoutAlgorithm.ets) -->
          ...
          if (this.anchorChildIndex >= 0) {
            if (helper.getLazyLayoutDirection() === LazyLayoutDirection.FORWARD) {
              ...
              if (arrayIndex >= 0 && arrayIndex < this.itemArr.length) {
                let newPos = this.itemArr[arrayIndex].start; // 锚点子组件重新测量后距离总高度顶部的位置
                if (newPos !== this.anchorChildRelativePos) {
                  helper.setAdjustedOffset(Math.round(this.anchorChildRelativePos - newPos).toInt());
                }
              }
            } else {
              const arrayIndex = this.anchorChildIndex - this.itemArrStartIndex;
              if (arrayIndex >= 0 && arrayIndex < this.itemArr.length) {
                let newPos = this.totalHeight - this.itemArr[arrayIndex].end;
                if (newPos !== this.anchorChildRelativePos) {
                  helper.setAdjustedOffset(Math.round(newPos - this.anchorChildRelativePos).toInt());
                }
              }
            }
          }
...
4. 测量可视范围内的元素。
    ...
    - 获取子组件。
        ...
        通过[getChild](../reference/apis-arkui/js-apis-arkui-frameNode.md#getchild12)方法获取指定索引的子组件[FrameNode](../reference/apis-arkui/js-apis-arkui-frameNode.md#framenode-1)。ArkTS-Dyn示例传入[ExpandMode.LAZY_NOT_EXPAND](../reference/apis-arkui/js-apis-arkui-frameNode.md#expandmode15)，ArkTS-Sta示例传入[ExpandMode.LAZY_EXPAND](../reference/apis-arkui/js-apis-arkui-frameNode.md#expandmode15)，避免全量加载导致懒加载失效。
    ...
    **完整测量流程示例**
    ...
    private measureForward(
      ...
      startIndex: int,
      startPos: double,
      tmpItemArr: Array<ItemPos>,
      tmpItemArrStartIndex: int
    ): int {
      ...
      for (; currIndex < this.childCnt; currIndex++) {
        ...
        tmpItemArrStartIndex = Math.min(currIndex, tmpItemArrStartIndex).toInt();
        tmpItemArr.push(new ItemPos(currPos, currPos + itemHeight));
        currPos += itemHeight;
        if (currIndex < this.childCnt - 1) {
          currPos += this.rowGap;
        }
...
5. 回收离开可视区域的子组件。
     ...
     */
    private recycleChildren(helper: LazyLayoutHelper): void {
      let recycleList: Array<int> = new Array<int>();
      if (this.prevStartIndex >= 0 && this.prevStartIndex < this.startIndex) {
        for (let i = this.prevStartIndex; i < this.startIndex; i++) {
          recycleList.push(i);
        }
...
6. 设置布局容器自身大小。
    ...
    **关键接口说明**
    ...
    | [getChild](../reference/apis-arkui/js-apis-arkui-frameNode.md#getchild12) | 获取指定索引的子组件[FrameNode](../reference/apis-arkui/js-apis-arkui-frameNode.md#framenode-1)。ArkTS-Dyn示例使用[ExpandMode.LAZY_NOT_EXPAND](../reference/apis-arkui/js-apis-arkui-frameNode.md#expandmode15)，ArkTS-Sta示例使用[ExpandMode.LAZY_EXPAND](../reference/apis-arkui/js-apis-arkui-frameNode.md#expandmode15)，避免获取子组件时全量加载导致懒加载失效。 |
...
<!-- @[lazy_custom_layout_algorithm_on_layout](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/LazyColumnLayoutAlgorithm.ets) -->
...
onLayout(self: FrameNode, position: NodePosition): void {
  ...
  for (let i = this.startIndex; i <= this.endIndex; i++) {
    let child = self.getChild(i, ExpandMode.LAZY_EXPAND);
...
使用LazyDynamicLayout组件时，需要先导入组件和布局算法类。ArkTS-Dyn从`@kit.ArkUI`导入LazyDynamicLayout和布局算法类型；ArkTS-Sta从`@ohos.arkui.components.ArkLazyDynamicLayout`导入LazyDynamicLayout，并从`@ohos.arkui.lazyLayoutAlgorithm`导入布局算法类型。完成导入后，在构造函数中传入布局算法实例。
...
<!-- @[lazy_custom_layout_create](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/CustomLazyColumnLayoutSample.ets) -->
...
<!-- @[lazy_custom_layout_dynamic_param](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/CustomLazyColumnLayoutSample.ets) -->
...
<!-- @[lazy_custom_layout_unit_convert](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/CustomLazyColumnLayoutSample.ets) -->
...
<!-- @[lazy_custom_layout_on_visible_indexes_change](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/CustomLazyColumnLayoutSample.ets) -->
...
<!--RP1-->
ArkTS-Dyn完整示例请参考[自定义懒加载单列布局示例](https://gitcode.com/openharmony/applications_app_samples/blob/master/code/DocsSample/ArkUISample/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/CustomLazyColumnLayoutSample.ets)。

ArkTS-Sta完整示例请参考[自定义懒加载单列布局示例](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/CustomLazyColumnLayoutSample.ets)。
<!--RP1End-->
...
- [自定义懒加载单列布局示例](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/CustomLazyColumnLayoutSample.ets)：展示如何使用LazyDynamicLayout配合LazyColumnLayoutAlgorithm实现懒加载单列列表。
- [自定义懒加载网格布局示例](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/CustomLazyGridLayoutSample.ets)：展示如何使用LazyDynamicLayout配合LazyGridLayoutAlgorithm实现懒加载网格布局。
- [自定义懒加载瀑布流布局示例](https://gitcode.com/openharmony/applications_app_samples/blob/OpenHarmony_feature_sta_20260331/code/DocsSample/ArkUISample-Sta/ScrollableComponent/entry/src/main/ets/pages/lazyCustomLayout/CustomLazyWaterFlowLayoutSample.ets)：展示如何使用LazyDynamicLayout配合LazyWaterFlowLayoutAlgorithm实现懒加载瀑布流布局。
<!--RP2End-->
