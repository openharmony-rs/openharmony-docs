...
在ArkTS1.2上Swiper组件设置loop属性时，需先导入Swiper模块。

```ts
import { Swiper, Text, Color, TextAlign } from '@ohos.arkui.component';
...
```ts
import { Swiper } from '@ohos.arkui.component';
...
在ArkTS1.2上Swiper组件设置autoPlay属性时，需先导入Swiper模块。
...
在ArkTS1.2上Swiper组件设置Indicator属性时，需先导入Swiper， indicator。

```ts
import { Swiper, Color, indicator } from '@ohos.arkui.component';
...
```ts
import { Swiper, Color } from '@ohos.arkui.component';
...
在ArkTS1.2上Swiper支持手指滑动、点击导航点和通过控制器三种方式切换页面，通过控制器切换页面时需先导入SwiperController， SwiperAnimationMode。
...
在ArkTS1.2上设置轮播方向时，需先导入Swiper组件。
...
在ArkTS1.2上设置每页显示多个子页面时，需先导入Swiper组件。
...
在ArkTS1.2上设置自定义切换动画时，需先导入SwiperAnimationMode组件。
...
struct SwiperCustomAnimationExample {
  ...
  @State zIndexList: Array<number> = new Array<number>();
...
在ArkTS1.2上设置设置圆点导航点间距时，需先导入Swiper，LengthMetrics，DotIndicator。

```ts
import { Swiper, LengthMetrics, DotIndicator } from '@ohos.arkui.component';
...
在ArkTS1.2上设置导航点忽略组件大小时，需先导入Swiper，LengthMetrics，DotIndicator。
...
在ArkTS1.2上设置数字导航点忽略组件大小时，需先导入Swiper，LengthMetrics，DotIndicator。
...
class MyDataSource implements IDataSource<number> {
  totalCount(): number {
    return this.num
  }
...
struct SwiperExample {
  ...
  build() {
    Column({ space: 5 } as ColumnOptions) {
      Swiper() {
        LazyForEach<Number>(this.data, (item: Number): void=> {
            // ...
          })
      }
      .onChange((index) => {
      ...
      Column({ space: 12 } as ColumnOptions) {
        ...
        Row() {
          // 在LazyForEach索引为0的位置添加数据
          Button('header data add').height(30).onClick((e: ClickEvent) => {
            this.data.addData(0, 'header Data');
          })
          // 删除LazyForEach索引为0的位置数据
          Button('header data delete').height(30).onClick((e: ClickEvent) => {
            this.data.deleteData(0);
          })
        }
      }.margin(5)
