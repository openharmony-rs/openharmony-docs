...
替代的ArkTS-Sta接口声明：[animateTo(value: AnimateParam, event: () => void): void](../reference/apis-arkui/arkts-apis-uicontext-uicontext.md#animateto)
...
animatorResult.onrepeat = ()=>{
    hilog.info(DOMAIN, TAG, "onrepeat");
}
```
...
animatorResult.oncancel = ()=>{
    hilog.info(DOMAIN, TAG, "oncancel");
}
```
...
animatorResult.onfinish = ()=>{
    hilog.info(DOMAIN, TAG, "onfinish");
}
```
