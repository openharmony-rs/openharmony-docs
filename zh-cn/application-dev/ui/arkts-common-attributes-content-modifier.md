...
type CheckboxBuilder = @Builder (config: CheckBoxConfiguration) => void
  ...
  class MyCheckboxStyle implements ContentModifier<CheckBoxConfiguration> {
    ...
    // 此处返回类型相比ArkTS 1.1有改变，使用新声明的类型作为返回值
    applyContent(): WrappedBuilder<CheckboxBuilder> {
      return wrapBuilder(buildCheckbox);
    }
  }
...
struct Test {
  build() {
    Row() {
      Column() {
        Checkbox({ name: 'checkbox status', group: 'checkboxGroup' } as CheckboxOptions)
          .select(true)
          .contentModifier(new MyCheckboxStyle(Color.Red))
          .onChange((value: boolean) => {
            console.info('Checkbox change is' + value);
...
![ContentModifier1](figures/ContentModifier1.gif)