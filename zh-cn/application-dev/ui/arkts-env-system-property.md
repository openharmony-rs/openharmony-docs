...
- 根据入参读取相应的环境变量信息。目前仅支持[SystemProperties.BREAK_POINT](../reference/apis-arkui/arkui-ts/ts-env-system-property.md#systemproperties)，用于获取[窗口](../reference/apis-arkui/js-apis-window-sys.md#window)不同宽高阈值下对应的断点值信息。详情见[\@Env支持参数](#env支持参数)。
...
\@Env用于展示\@Component/\@ComponentV2所在[窗口](../reference/apis-arkui/js-apis-window-sys.md#window)的环境变量信息。开发者通过BuilderNode切换@Component\@ComponentV2所在的窗口实例时，\@Env会根据新的窗口获取对应的环境变量信息，并触发关联的UI组件刷新。以`SystemProperties.BREAK_POINT`为例。
在下面的示例中：
