# UI互操作声明文件规范
## 概述
UI互操作依赖语言编译构建。在ArkTS-Dyn与ArkTS-Sta的互操作工程中，由于语言编译器一次仅接收一种语法文件，需要借助编译时生成的声明文件和胶水代码完成语法编译检查。声明文件用于向调用方暴露类型、装饰器等信息，胶水代码用于将被调用侧代码翻译为调用侧可识别、可调用的形式。工程编译构建时自动生成这两类代码文件，生成位置位于各模块的build目录下。考虑到互操作场景的复杂性，部分场景下自动生成的文件可能不完整，因此支持开发者在生成产物基础上手写修改声明文件和胶水代码，修改完成后通过增量编译使其生效。

UI互操作无需对胶水层做额外处理，可直接使用自动生成结果。本文重点给出声明文件的手写参考规范。

## 声明文件转换规格
### ArkTS-Dyn调用ArkTS-Sta
ArkTS-Dyn主模块调用ArkTS-Sta模块时，declgen为ArkTS-Sta的文件生成符合ArkTS-Dyn语法的声明文件，并与ArkTS-Dyn主模块源文件混合编译。声明文件产物路径为：`build/default/intermediates/declgen/default/declgenV1/.../xxx.d.ets`。下面分别给出声明文件全局修改原则，状态管理V1装饰器转换规格，状态管理V2装饰器转换规格，\@Builder/WrappedBuilder/\@BuilderParam转换规格。建议结合[使用场景](#arkts-dyn调用arkts-sta-1)章节，通过完整示例了解这些规则在互操作工程中的用法。
...
ArkTS-Sta主模块调用ArkTS-Dyn模块时，declgen为ArkTS-Dyn的文件生成符合ArkTS-Sta语法的声明文件，并与ArkTS-Sta主模块源文件混合编译。声明文件产物路径为：`build/default/intermediates/declgen/default/declgenV2/.../xxx.d.ets`。下面分别给出声明文件全局修改原则，状态管理V1装饰器转换规格，状态管理V2装饰器转换规格，\@Builder/WrappedBuilder/\@BuilderParam转换规格。建议结合[使用场景](#arkts-sta调用arkts-dyn-1)章节，通过完整示例了解这些规则在互操作工程中的用法。
...
## 使用场景
### ArkTS-Dyn调用ArkTS-Sta
结合[声明文件转换规格](#arkts-dyn调用arkts-sta)，下面分别给出状态管理V1装饰器，状态管理V2装饰器，\@Builder/WrappedBuilder/\@BuilderParam的使用场景。
...
下面的代码示例展示了ArkTS-Dyn调用ArkTS-Sta自定义组件并使用状态管理V1装饰器的场景下，手写声明文件的规格。
...
下面的代码示例展示了ArkTS-Dyn调用ArkTS-Sta自定义组件并使用状态管理V2装饰器的场景下，手写声明文件的规格。
...
下面的代码示例展示了ArkTS-Dyn调用ArkTS-Sta自定义组件并使用\@Builder，WrappedBuilder，\@BuilderParam，手写声明文件的规格。
...
### ArkTS-Sta调用ArkTS-Dyn
结合[声明文件转换规格](#arkts-sta调用arkts-dyn)，下面分别给出状态管理V1装饰器，状态管理V2装饰器，\@Builder/WrappedBuilder/\@BuilderParam的使用场景。
...
下面的代码示例展示了ArkTS-Sta调用ArkTS-Dyn自定义组件并使用状态管理V1装饰器的场景下，手写声明文件的规格。
...
下面的代码示例展示了ArkTS-Sta调用ArkTS-Dyn自定义组件并使用状态管理V2装饰器的场景下，手写声明文件的规格。
...
下面的代码示例展示了ArkTS-Sta调用ArkTS-Dyn自定义组件并使用\@Builder，WrappedBuilder，\@BuilderParam的场景下，手写声明文件的规格。
