...
以下步骤以SURFACE类型为例，描述了如何在ArkTS静态语法环境下使用`XComponent组件`在ArkTS侧传入SurfaceId，在Native侧创建NativeWindow实例，然后创建`EGL/GLES`环境，实现在主页面绘制图形。
...
1.  在界面中定义XComponent。
    ...
    class NativeRender {
        static {
            loadLibrary('nativerender');
        }
        native SetSurfaceId(id: long): void;
        native ChangeSurface(id: long, w: double, h: double): void;
        native DrawPattern(id: long): void;
        native DestroySurface(id: long): void;
    };
    ...
    class MyXComponentController extends XComponentController {
        ...
        onSurfaceCreated(surfaceId: string): void {
            console.log(`onSurfaceCreated surfaceId: ${surfaceId}`)
            // 设置surfaceId
            this.nativeRender.SetSurfaceId(bigint(surfaceId).getLong());
        }
    
        onSurfaceChanged(surfaceId: string, rect: SurfaceRect): void {
            console.log(`onSurfaceChanged surfaceId: ${surfaceId}, rect: ${JSON.stringify(rect)}}`)
            // 在onSurfaceChanged中调用ChangeSurface更新绘制内容的大小信息
            this.nativeRender.ChangeSurface(bigint(surfaceId).getLong(), rect.surfaceWidth, rect.surfaceHeight)
        }
    
        onSurfaceDestroyed(surfaceId: string): void {
            console.log(`onSurfaceDestroyed surfaceId: ${surfaceId}`)
            this.nativeRender.DestroySurface(bigint(surfaceId).getLong())
        }
    }
            ...
            Column() {
                ...
                Row() {
                    Button('Draw Star')
                        .fontSize('16fp')
                        .fontWeight(500)
                        .margin({ bottom: 24 } as Margin)
                        .onClick((e: ClickEvent) => {
                            let surfaceId = this.xComponentController.getXComponentSurfaceId()
                            this.xComponentController.nativeRender.DrawPattern(bigint(surfaceId).getLong())
                            this.currentStatus = "draw star"
                        })
                        .width('53.6%')
                        .height(40)
...
2.  ANI模块注册，具体使用请参考[Ark Native Interface应用实践手册](../ani/ani-usage-scenarios.md)。

    ```cpp
    #include "ani/ani.h"
    #include "common/common.h"
    #include "manager/plugin_manager.h"
    // 在ani_init.cpp文件中，ANI_Constructor方法注册接口函数，从而将封装的C++方法传递出来，供ArkTS侧调用
    #define ANI_METHODS_COUNT 4
    ANI_EXPORT ani_status ANI_Constructor(ani_vm *vm, uint32_t *result)
    {
        ani_env* env;
        if (ANI_OK != vm->GetEnv(ANI_VERSION_1, &env)) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ANI_Constructor", "Get ANI env failed.");
            return ANI_ERROR;
        }
        // 找到ArkTS侧定义的NativeRender类
        const char* clsName = "Lentry/src/main/ets/pages/Index/NativeRender;";
        ani_class cls {};
        if (ANI_OK != env->FindClass(clsName, &cls)) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ANI_Constructor", "FindClass failed.");
            return ANI_ERROR;
        }
        // 绑定NativeRender类的SetSurfaceId(),ChangeSurface(),DestroySurface()和
        // DrawPattern()方法对应的C++侧函数
        ani_native_function methods[] = {
            ani_native_function { "SetSurfaceId", "J:V", reinterpret_cast<void*>(PluginManager::AniSetSurfaceId) },
            ani_native_function { "ChangeSurface", "JDD:V", reinterpret_cast<void*>(PluginManager::AniChangeSurface) },
            ani_native_function { "DrawPattern", "J:V", reinterpret_cast<void*>(PluginManager::AniDrawPattern) },
            ani_native_function { "DestroySurface", "J:V", reinterpret_cast<void*>(PluginManager::AniDestroySurface) }
        };
        if (ANI_OK != env->Class_BindNativeMethods(cls, methods, ANI_METHODS_COUNT)) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ANI_Constructor", "BindNativeMethods failed.");
            return ANI_ERROR;
        }
...
3.  上述注册的六个函数在Native侧具体实现。
    ...
    class PluginManager {
    public:
        static PluginRender *GetPluginRender(uint64_t &id);
        static void AniSetSurfaceId(ani_env *env, [[maybe_unused]] ani_object object, ani_long id);
        static void AniDrawPattern(ani_env *env, [[maybe_unused]] ani_object object, ani_long id);
        static void AniChangeSurface(ani_env *env, [[maybe_unused]] ani_object object, ani_long id, ani_double w, ani_double h);
        static void AniDestroySurface(ani_env *env, [[maybe_unused]] ani_object object, ani_long id);
        static std::unordered_map<uint64_t, PluginRender *> pluginRenderMap_;
        static std::unordered_map<uint64_t, OHNativeWindow *> windowMap_;
    };
    ```

    ```cpp
    // PluginManager类相关方法的实现
    // 设置SurfaceId，基于SurfaceId完成对NativeWindow的初始化
    void PluginManager::AniSetSurfaceId(ani_env *env, [[maybe_unused]] ani_object object, ani_long id) {
        uint64_t surfaceId = static_cast<uint64_t>(id);
        OHNativeWindow *nativeWindow;
        if (windowMap_.find(surfaceId) == windowMap_.end()) {
            OH_NativeWindow_CreateNativeWindowFromSurfaceId(surfaceId, &nativeWindow);
            windowMap_[surfaceId] = nativeWindow;
        ...
        if (pluginRenderMap_.find(surfaceId) == pluginRenderMap_.end()) {
            PluginRender *pluginRender = new PluginRender(surfaceId);
            pluginRenderMap_[surfaceId] = pluginRender;
            pluginRender->InitNativeWindow(nativeWindow);
        }
    }
    ...
    // 根据传入的surfaceId、width、height实现Surface大小的变动
    void PluginManager::AniChangeSurface(ani_env *env, [[maybe_unused]] ani_object object, ani_long id,
        ani_double w, ani_double h) {
        if ((env == nullptr)) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "PluginManager",
                         "ChangeSurface: OnLoad env or info is null");
            return;
    ...
    // 销毁Surface
    void PluginManager::AniDestroySurface(ani_env *env, [[maybe_unused]] ani_object object, ani_long id) {
        uint64_t surfaceId = static_cast<uint64_t>(id);
        auto pluginRenderMapIter = pluginRenderMap_.find(surfaceId);
        if (pluginRenderMapIter != pluginRenderMap_.end()) {
            delete pluginRenderMapIter->second;
    ...
    // 实现EGL绘画逻辑
    void PluginManager::AniDrawPattern(ani_env *env, [[maybe_unused]] ani_object object, ani_long id) {
        uint64_t surfaceId = static_cast<uint64_t>(id);
        auto pluginRender = GetPluginRender(surfaceId);
        if (pluginRender == nullptr) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "PluginManager", "DrawPattern: Get pluginRender failed");
    ...
    PluginRender *PluginManager::GetPluginRender(uint64_t &id) {
        if (pluginRenderMap_.find(id) != pluginRenderMap_.end()) {
            return pluginRenderMap_[id];
        }
        return nullptr;
...
4.  配置具体的CMakeLists，使用CMake工具链将C++源代码编译成动态链接库文件。
    ...
    # 添加构建需要链接的库
    target_link_libraries(nativerender PUBLIC
        ${EGL-lib} ${GLES-lib} ${hilog-lib} ${libace-lib} libnative_window.so)
    ```
...
以下步骤以SURFACE类型为例，描述了如何在ArkTS静态语法环境下创建SURFACE类型的XComponent节点，并调用OH_ArkUI_SurfaceHolder相关接口管理Surface生命周期。创建`EGL/GLES`环境的逻辑与ArkTS动态语法环境下一致，可以参考[开发步骤（ArkTS-Dyn）](#开发步骤arkts-dyn-1)章节的创建`EGL/GLES`环境示例。
...
1.  在界面中定义XComponent。
    ...
    class NativeRender {
        ...
        native bindNode(id: string, node: FrameNode): void;
        native unbindNode(id: string): void;
    };

    class MyNodeController extends NodeController {
        public root: FrameNode | null = null;
        public xcomponentNode: typeNode.XComponent | null = null;
        nodeId: string = 'nodeId_' + Math.random()
        nativeRender: NativeRender = new NativeRender()
        makeNode(uiContext: UIContext): FrameNode | null {
            ...
            // 通过NativeXComponentParameters创建生命周期归一的XComponent节点
            this.xcomponentNode = typeNode.createNode(uiContext, 'XComponent', params);
            this.xcomponentNode!
                .initialize(params)
                .width(200)
                .height(200)
            this.nativeRender.bindNode(this.nodeId, this.xcomponentNode! as typeNode.XComponent);
            this.root?.appendChild(this.xcomponentNode!);
            return this.root;
        }
        aboutToDisappear() {
            this.nativeRender.unbindNode(this.nodeId);
        }
    }
...
2.  ANI模块注册，具体使用请参考[Ark Native Interface应用实践手册](../ani/ani-usage-scenarios.md)。
    ...
    #include "common/common.h"
    #include "manager/plugin_manager.h"
    #define ANI_METHODS_COUNT 2
    // ani_init.cpp文件中，ANI_Constructor方法注册接口函数，从而将封装的C++方法传递出来，供ArkTS侧调用
    ANI_EXPORT ani_status ANI_Constructor(ani_vm *vm, uint32_t *result)
    {
        ani_env* env;
        if (ANI_OK != vm->GetEnv(ANI_VERSION_1, &env)) {
        ...
        // 绑定NativeRender类的bindNode(),unbindNode()方法对应的C++侧函数
        ani_native_function methods[] = {
            ani_native_function { "bindNode", nullptr, reinterpret_cast<void*>(PluginManager::AniBindNode) },
            ani_native_function { "unbindNode", nullptr, reinterpret_cast<void*>(PluginManager::AniUnbindNode) },
        }
        if (ANI_OK != env->Class_BindNativeMethods(cls, methods, ANI_METHODS_COUNT)) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ANI_Constructor", "BindNativeMethods failed.");
            return ANI_ERROR;
...
3.  注册XComponent生命周期回调，使用CAPI实现往XComponent注册回调函数。

    定义BindNode、UnbindNode方法，暴露到ArkTS侧的bindNode、unbindNode方法会执行该方法。

    ```c++
    // plugin_manager.h
    class PluginManager {
    public:
        static void AniBindNode(ani_env *env, [[maybe_unused]] ani_object object, ani_string id, ani_object node);
        static void AniUnbindNode(ani_env *env, [[maybe_unused]] ani_object object, ani_string id);
        static std::unordered_map<std::string, ArkUI_NodeHandle> nodeHandleMap_;
        static std::unordered_map<uint64_t, EGLRender *> renderMap_;
        static std::unordered_map<void *, OH_ArkUI_SurfaceCallback*> callbackMap_;
    };
    ```
    ...
    // plugin_manager.cpp
    std::unordered_map<std::string, ArkUI_NodeHandle> PluginManager::nodeHandleMap_;
    std::unordered_map<uint64_t, EGLRender *> PluginManager::renderMap_;
    std::unordered_map<void *, OH_ArkUI_SurfaceCallback*> PluginManager::callbackMap_;
    ArkUI_NativeNodeAPI_1 *nodeAPI = reinterpret_cast<ArkUI_NativeNodeAPI_1 *>(
        OH_ArkUI_QueryModuleInterfaceByName(ARKUI_NATIVE_NODE, "ArkUI_NativeNodeAPI_1"));

    void ANIStringToStdString(ani_env *env, ani_string src, std::string& dst) { // 将ani_string转化为std::string类型的变量
        if (env == nullptr) {
            return "";
        }
        ani_size strSize;
        env->String_GetUTF8(ani_str, &strSize);
        ...
        dst = std::string(utf8_buffer);
    }

    void PluginManager::AniBindNode(ani_env *env, [[maybe_unused]] ani_object object, ani_string id, ani_object node) {
        ArkUI_NodeHandle handle;
        int32_t code = OH_ArkUI_NativeModule_GetNodeHandleFromAniValue(env, node, &handle);
        if (code != 0) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, 0xff00, "bindNode", "GetNodeHandleFromAniValue failed.");
            return;
        }
        std::string nodeId;
        ANIStringToStdString(env, id, nodeId);
        nodeHandleMap_[nodeId] = handle;
        // 创建SurfaceHolder
    ...
    void PluginManager::AniUnbindNode(ani_env *env, [[maybe_unused]] ani_object object, ani_string id) {
        std::string nodeId;
        ANIStringToStdString(env, id, nodeId);
        auto node = nodeHandleMap_[nodeId];
        nodeAPI->disposeNode(node); // 销毁nodeHandle
    ...
    定义Surface创建成功，发生改变，销毁和事件。

    ```c++
    void OnSurfaceCreated(OH_ArkUI_SurfaceHolder *holder) {
        auto window = OH_ArkUI_XComponent_GetNativeWindow(holder); // 获取native window
        uint64_t surfaceId;
        OH_NativeWindow_GetSurfaceId((OHNativeWindow*)window, &surfaceId);
        auto render = new EGLRender();
        PluginManager::renderMap_[surfaceId] = render;
        render->SetUpEGLContext(window);
    }
    ...
    void OnSurfaceChanged(OH_ArkUI_SurfaceHolder *holder, uint64_t width, uint64_t height) {
        ...
        uint64_t surfaceId;
        OH_NativeWindow_GetSurfaceId((OHNativeWindow*)window, &surfaceId);
        auto it = PluginManager::renderMap_.find(surfaceId);
        if (it != PluginManager::renderMap_.end()) {
    ...
    void OnSurfaceDestroyed(OH_ArkUI_SurfaceHolder *holder) {
        ...
        uint64_t surfaceId;
        OH_NativeWindow_GetSurfaceId((OHNativeWindow*)window, &surfaceId);
        if (PluginManager::renderMap_.count(surfaceId)) { // 销毁render对象
            auto render = PluginManager::renderMap_[surfaceId];
            delete render;
            PluginManager::renderMap_.erase(surfaceId);
        }
        if (PluginManager::callbackMap_.count(holder)) {
            auto callback = PluginManager::callbackMap_[holder];
            OH_ArkUI_SurfaceHolder_RemoveSurfaceCallback(holder, callback); // 移除SurfaceCallback
...
4.  CMakeLists文件。使用CMake工具链将C++源代码编译成动态链接库文件。
...
以下步骤以SURFACE类型为例，描述了如何在ArkTS静态语法环境下使用NativeXComponent管理XComponent生命周期，实现在主页面绘制图形。创建`EGL/GLES`环境的逻辑与ArkTS动态语法环境下一致，可以参考[开发步骤（ArkTS-Dyn）](#开发步骤arkts-dyn-2)章节的创建`EGL/GLES`环境示例。
...
1.  在界面中定义XComponent。
    ...
    class NativeRender {
        ...
        native DrawPattern(id: long): void;
        native HandleNativeXComponent(ptr: NativeXComponentPointer): void;
    };
    ...
    struct Index {
        @State currentStatus: string = "index";
        xcomponentId: string = "xcomponent_" + Math.random();
            ...
            Column() {
                ...
                Column({ space: 10 } as ColumnOptions) {
                    XComponent({
                        id: this.xcomponentId,
                        type: XComponentType.SURFACE,
                        // 使用nativeXComponentHandler获取NativeXComponent实例
                        nativeXComponentHandler: this.nativeRender.HandleNativeXComponent as Callback<NativeXComponentPointer>,
                        controller: this.xComponentController
                ...
                Row() {
                    Button('Draw Star')
                        ...
                        .onClick((e: ClickEvent) => {
                            let surfaceId = this.xComponentController.getXComponentSurfaceId()
                            this.nativeRender.DrawPattern(bigint(surfaceId).getLong())
                            this.currentStatus = "draw star"
                        })
                        .width('53.6%')
                        .height(40)
...
2.  ANI模块注册，具体使用请参考[Ark Native Interface应用实践手册](../ani/ani-usage-scenarios.md)。
    ...
    ANI_EXPORT ani_status ANI_Constructor(ani_vm *vm, uint32_t *result)
    {
        ...
        // 绑定NativeRender类的DrawPattern(),HandleNativeXComponent()方法对应的native函数
        ani_native_function methods[] = {
            ani_native_function { "DrawPattern", "J:V", reinterpret_cast<void*>(PluginManager::AniDrawPattern) },
            ani_native_function { "HandleNativeXComponent", "J:V", reinterpret_cast<void*>(PluginManager::AniHandleNativeXComponent) }
        };
        if (ANI_OK != env->Class_BindNativeMethods(cls, methods, ANI_METHODS_COUNT)) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ANI_Constructor", "BindNativeMethods failed.");
            return ANI_ERROR;
...
3.  注册XComponent事件回调，使用Node-API实现XComponent事件回调函数。
    ...
    定义Surface创建成功，发生改变，销毁和XComponent的touch事件回调接口。
    ...
    public:
        static PluginRender *GetPluginRender(uint64_t &id);
        static void AniDrawPattern(ani_env *env, [[maybe_unused]] ani_object object, ani_long id);
        static void AniHandleNativeXComponent(ani_env *env, [[maybe_unused]] ani_object object, ani_long ptr);
        static std::unordered_map<uint64_t, PluginRender *> pluginRenderMap_;
        // OH_NativeXComponent_Callback对象
        static OH_NativeXComponent_Callback callback_;
    };
    ```
    ...
    // 定义一个函数DispatchTouchEventCB()，响应触摸事件时触发该回调
    void DispatchTouchEventCB(OH_NativeXComponent *component, void *window) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "PluginManager", "Handle TouchEvent");
    }
   
    // 定义OH_NativeXComponent_Callback对象
    ...
    定义AniHandleNativeXComponent方法，暴露到ArkTS侧的HandleNativeXComponent()方法会执行该方法。

    ```c++
    void PluginManager::AniHandleNativeXComponent(ani_env *env, [[maybe_unused]] ani_object object, ani_long ptr)
    {
        if (env == nullptr) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "PluginManager", "HandleNativeXComponent env is null");
            return;
        }
        OH_NativeXComponent* component = reinterpret_cast<OH_NativeXComponent*>(ptr);
        if (component == nullptr) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "PluginManager", "HandleNativeXComponent component is null");
            return;
        }
        // 注册NativeXComponent回调函数
        OH_NativeXComponent_RegisterCallback(component, &PluginManager::callback_);
    }
    ```

    定义AniDrawPattern方法，暴露到ArkTS侧的DrawPattern()方法会执行该方法。
...
针对Native XComponent的使用，有以下相关示例可供参考：
...
针对ArkTS XComponent的使用，有以下相关示例可供参考：
