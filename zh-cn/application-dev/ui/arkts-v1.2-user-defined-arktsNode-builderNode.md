...
> - 本模块仅适用于ArkTS-Sta。
>
> - BuilderNode只支持一个由wrapBuilder包装的[全局自定义构建函数](../ui/state-management/arkts-builder.md#全局自定义构建函数)@Builder。
>
...
>
> - 如果传入的Builder的根节点为未设置通用样式的[自定义组件](./state-management/arkts-create-custom-components.md)、[ContentSlot](../../application-dev/ui/rendering-control/arkts-rendering-control-contentslot.md)、LazyForEach、Repeat、ForEach、[if/else](../../application-dev/ui/rendering-control/arkts-rendering-control-ifelse.md)，需要额外生成一个FrameNode。在节点树中显示为“BuilderProxyNode”。
>
> - 不支持根节点为懒加载模式的repeat节点。
>
