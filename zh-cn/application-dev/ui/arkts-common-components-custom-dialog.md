...
1. 使用\@CustomDialog装饰器装饰自定义弹出框，可在此装饰器内自定义弹出框内容。CustomDialogController需在@Component内定义。
   ...
   ```ts
   import { CustomDialog, CustomDialogController, Column, Text, FlexAlign} from '@ohos.arkui.component';
...
3. 点击与onClick事件绑定的组件使弹出框弹出。
   ...
   ```ts
   import { CustomDialogController, Button, Margin } from '@ohos.arkui.component';
...
1. 在\@CustomDialog装饰器内添加按钮和数据函数。
   ...
   struct CustomDialogExample {
     ...
     build() {
       Column() {
         Text('我是内容').fontSize(20).margin({ top: 10, bottom: 10 } as Margin)
...
struct CustomDialogExample {
  ...
  build() {
    Column() {
      Text('我是内容').fontSize(16).margin({ bottom: 10 } as Margin)
    }
  }
}
...
struct CustomDialogUser {
  ...
  dialogController: CustomDialogController | null = new CustomDialogController({
    ...
    borderStyle: BorderStyle.Dashed,// 使用borderStyle属性，需要和borderWidth属性一起使用
    borderColor: Color.Blue,// 使用borderColor属性，需要和borderWidth属性一起使用
    shadow: ({ radius: 20, color: Color.Grey, offsetX: 50, offsetY: 0} as ShadowOptions),
  });
