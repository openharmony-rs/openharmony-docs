...
### Fun-based Widget
  ...
  趣味交互类型互动卡片是互动卡片的一种类型，提供卡片小游戏功能，当用户点击卡片时开始体验对应卡片小游戏。当前仅支持基于快游戏开发，适用于提升卡片趣味性和用户粘性的场景。
...
### Scene-based Widget
  ...
  场景动效类型互动卡片是互动卡片的一种类型，支持在特定场景下（如数据定时或定点刷新、用户点击等）触发互动卡片的特有效果。包含激活态和非激活态两种状态，激活态时卡片UI由LiveFormExtensionAbility对应page页面完成渲染，支持实现破框动效，适用于需要丰富信息提醒和浅层交互的场景。
