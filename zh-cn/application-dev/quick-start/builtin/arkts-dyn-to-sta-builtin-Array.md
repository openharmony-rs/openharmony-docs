...
**ArkTS-Sta版本签名：**
  ...
  ```typescript
  Array.create<T>(arrayLength: int, initialValue: T): Array<T>
  ```
...
predicate函数返回值说明：
  | 类型 | 说明 |
  | -------- | -------- |
  | unknown | 返回值会被隐式转换为boolean。true表示保留元素，false表示过滤元素。 |
...
predicate函数返回值说明：
  | 类型 | 说明 |
  | -------- | -------- |
  | unknown | 返回值会被隐式转换为boolean。true表示找到匹配项，false表示未找到。 |
...
**适配建议：**
  ...
  删除最后一个参数，避免直接修改this的指向。若必须修改this指向，可使用闭包实现。
...
**参数：**
  ...
  | -------- | -------- | -------- | -------- |
  | predicate | function | 是 | 测试函数 |
  | thisArg | any | 否 | 在执行callback函数时作为this的值，默认值为undefined。 |
...
**ArkTS-Sta版本签名：**
  ...
  ```typescript
  static from<T, U>(arrayLike: ArrayLike<T>, mapfn: (v: T, k: int) => U): U[]
  ```

  `static from<T, U>(iterable: Iterable<T>, mapfn: (v: T, k: int) => U): U[]`
...
**返回值：**
  ...
  | 类型 | 说明 |
  | -------- | -------- |
  | boolean | 表示所有元素是否都满足测试条件，true表示全部满足，false表示全部不满足。 |
...
**适配建议：**
  ...
  ArkTS-Sta相比ArkTS-Dyn接口签名有变更，但开发者调用的接口行为和执行结果保持不变。
