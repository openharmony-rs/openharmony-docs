...
```typescript
  ...
  console.info(instance.getSecret()); // "my secret"
```
**ArkTS-Sta版本签名：**
...
console.info(result); // "kiwi, orange, kiwi, banana"

const str = "apple, orange, apple, banana";
const result1 = str.replace("apple", "kiwi");
console.info(result1); // "kiwi, orange, apple, banana"

const result2 = str.replace(/apple/g, "kiwi");
console.info(result2); // "kiwi, orange, kiwi, banana"
```
...
**适配建议：**
  ...
  ArkTS-Sta版本any类型变为(Object | null | undefined)，使用需指定类型。
...
**适配建议：**
  ...
  ArkTS-Dyn的JSON.parse返回一个any对象，该any对象可以为任何值，但ArkTS-Sta无法生成一个any对象，并向里面动态添加属性。
...
replacer函数参数说明：
  | 参数名 | 类型 | 必填 | 说明 |
  | -------- | -------- | -------- | -------- |
  | key | string | 是 | 正在处理的当前上下文对象属性的键名。 |
...
### ArkTS-Sta部分builtin class标注final，无法被继承使用。
