...
- **Library类型的Module：** 用于实现代码和资源的共享。同一个Library类型的Module可以被其他的Module多次引用，合理地使用该类型的Module，能够降低开发和维护成本。Library类型的Module分为Static和Shared两种类型，编译后生成共享包。
  ...
  | --------  | ---- | --- |
  | HAR | HAR中的代码和资源跟随使用方编译，如果有多个使用方，它们的编译产物中会存在多份相同拷贝。<br/>注意：构建HAR时，建议开启混淆能力，保护代码资产。 | HAR除了支持应用内引用，还可以独立打包发布到OHPM中心仓或者OHPM私仓，供其他应用引用。 | 
...
| Static Library | HAR | 静态共享包，编译态复用。<br/> - 支持应用内共享，也可以作为二方库（SDK）、三方库（SDK）发布后供其他应用使用。<br/> &ensp; - 作为二方库（SDK），发布到OHPM私仓，供公司内部其他应用使用。<br/> &ensp; - 作为三方库（SDK），发布到OHPM中心仓，供其他应用使用。<br/> - 多包（HAP/HSP）同时引用相同的HAR时，会造成多包间代码和资源的重复拷贝，从而导致应用包增大。<br/> - 注意：构建HAR时，建议开启混淆能力，保护代码资产。 |
