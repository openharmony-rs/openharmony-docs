...
## 不支持`Function('return this')`

**规则：** `arkts-no-function-return-this`

**规则解释：**

在ArkTS-Dyn中，`new Function('return this')`返回globalThis。

ArkTS-Sta上不支持此写法。

**变更原因：**

ArkTS-Sta上不支持globalThis和运行时动态执行代码。
...
```typescript
let t: object = Function('return this')(); // 返回globalThis
```
...
```typescript
  ...
  console.info(String(a instanceof Record)) // 报错，在ArkTS-Dyn中，Record不具有运行时类型
...
```typescript
let a: Record<string, number> = { 'v': 123 };
console.info(String(a instanceof Record)) // 输出：true，在ArkTS-Sta中，Record具有运行时类型
console.info(a['v'] + '') // 输出：123
```
...
class A implements I { // ArkTS-Sta中报错，需要同时实现get/set方法
  get v(): number {
    return 0;
  }
}
...
```typescript
console.info(typeof (1 && 1)); // 输出：number
console.info(typeof (1 || 1)); // 输出：number
```
...
```typescript
console.info(typeof (1 && 1)); // 输出：int
console.info(typeof (1 || 1)); // 输出：int
```
...
console.info(a.age + ''); // 输出：undefined
```
...
```typescript
export type OptionalTuples = [number, boolean, string?]; // 报错

export type VariableTuples = [string, ...string[]]; // 报错
```
...
// ArkTS-Dyn
arg_rest_int_function(1, 2, 3) === 1; // 报错
...
console.info(AAA.default + ''); // 输出：111
```
...
| Package | 类名 |
|---------|---------|
| escompat | ErrorOptionsImpl, Error, EvalError, NonIntegralIndexError, TypeError, ReferenceError, URIError, AggregateError, Int8Array, Int16Array, Int32Array, BigInt64Array, Float32Array, Float64Array, Uint8ClampedArray, Uint8Array, Uint16Array, Uint32Array, BigUint64Array, WeakMap, WeakSet |
...
| std.containers | ConcurrentQueue, ListInt, ListObject, ListString |
...
在ArkTS-Sta中，类对象字面量可以省略有默认值的字段。

**适配建议：**

在ArkTS-Sta中，如果类的字段有默认值，则在对象字面量实例化时可以省略该字段。
...
let p1: Person = { name: "", age: 0 }; // 不可省略字段
```
...
class Son extends Father {
    override func1(a: A | B) { } // 逆变，参数类型比父型更宽泛
    override func2(a: A) { } // 协变，参数类型比父型更具体，在ArkTS-Sta中报错
}
```
