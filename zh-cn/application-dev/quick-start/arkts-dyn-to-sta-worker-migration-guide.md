...
为了兼容ArkTS-Dyn基于事件循环的异步能力（如[setTimeout](../reference/common/js-apis-timer.md#settimeout)、[setInterval](../reference/common/js-apis-timer.md#setinterval)），EAWorker API必须同时包含线程、事件循环和消息发送处理的API。与Worker API类似，EAWorker API中的线程API语言类似于Java Thread。在典型场景下，开发者创建一个EAWorker实例，并在创建时给构造器传递一个EAWorker需要执行的线程任务。EAWorker实例启动后，必须显式调用 [start](../reference/apis-arkts/eaworker_managed.md#start) 启动事件循环；停止时需通过 [quit](../reference/apis-arkts/eaworker_managed.md#quit)来结束事件循环。开发者通过[MessageHandler](../reference/apis-arkts/message_handler.md)定义消息[Message](../reference/apis-arkts/message.md)（消息队列中的一个消息，消息可以携带数据和回调函数）的处理逻辑，一个事件循环可以绑定多个`MessageHandler`来处理不同类型和来源的消息。消息的发送也通过调用`MessageHandler`的API来完成，因此不同的消息可以绑定不同的消息处理逻辑。
...
回调函数。表示Worker在执行过程中发生异常被调用的事件处理程序，处理程序在宿主线程中执行。其中回调函数中err类型为[ErrorEvent](../reference/apis-arkts/js-apis-worker.md#errorevent)，表示收到的异常数据。
...
回调函数。表示宿主线程接收到来自其创建的Worker通过workerPort.postMessage接口发送的消息时被调用的事件处理程序，处理程序在宿主线程中执行。其中回调函数中event类型为[MessageEvents](../reference/apis-arkts/js-apis-worker.md#messageevents9)，表示收到的Worker消息数据。
...
回调函数。表示当Worker对象接收到一条无法被序列化的消息时被调用的事件处理程序，处理程序在宿主线程中执行。其中回调函数中event类型为[MessageEvents](../reference/apis-arkts/js-apis-worker.md#messageevents9)，表示收到的Worker消息数据。
...
Worker线程用于与宿主线程通信的类，通过postMessage接口发送消息给宿主线程、close接口销毁Worker线程。ThreadWorkerGlobalScope类继承[GlobalScope](../reference/apis-arkts/js-apis-worker.md#globalscope9)。
...
回调函数。ThreadWorkerGlobalScope的onmessage属性表示Worker线程收到来自其宿主线程通过postMessage接口发送的消息时被调用的事件处理程序，处理程序在Worker线程中执行。其中this指调用者对象本身[ThreadWorkerGlobalScope](../reference/apis-arkts/js-apis-worker.md#threadworkerglobalscope9)，ev类型为[MessageEvents](../reference/apis-arkts/js-apis-worker.md#messageevents9)，表示收到的Worker消息数据。
...
回调函数。ThreadWorkerGlobalScope的onmessageerror属性表示当Worker对象接收到一条无法被反序列化的消息时被调用的事件处理程序，处理程序在Worker线程中执行。其中this指调用者对象本身[ThreadWorkerGlobalScope](../reference/apis-arkts/js-apis-worker.md#threadworkerglobalscope9)，ev类型为[MessageEvents](../reference/apis-arkts/js-apis-worker.md#messageevents9)，表示收到的Worker消息数据。
...
Worker线程自身的运行环境，GlobalScope类继承[WorkerEventTarget](../reference/apis-arkts/js-apis-worker.md#workereventtarget9)。

### onerror

onerror?: (ev: ErrorEvent) =&gt; void

回调函数。GlobalScope的onerror属性表示Worker在执行过程中发生异常被调用的事件处理程序，处理程序在Worker线程中执行。其中回调函数中ev类型为[ErrorEvent](../reference/apis-arkts/js-apis-worker.md#errorevent)，表示收到的异常数据。
...
### 如何进行异常处理
ArkTS-Dyn中由于内存隔离的限制，当Worker发生未捕获异常时，会向宿主线程发送一个异常信息的消息，并触发jscrash从而导致程序终止退出。对于这种情况，需要在宿主线程对worker注册[ErrorCallback](../reference/apis-arkts/js-apis-worker.md#errorcallback18)来捕获worker上的未捕获异常，而不能在宿主线程通过运行时定义的try/catch机制进行异常处理。ArkTS-Sta中没有了内存隔离的限制，统一了所有线程的语义模型，可以由开发者来选择在合适的线程，通过try/catch机制来处理其他线程运行任务发生的未捕获异常。

对于通过ArkTS-Sta的[EAWorker.run](../reference/apis-arkts/eaworker_managed.md#run)接口运行的可等待线程任务，ArkTS-Dyn设计范式中可以通过[Await](../reference/apis-arkts/job.md#await)来将其他线程任务中未捕获的异常转移到当前线程中，通过try/catch机制来处理。
...
const workerInstance = new EAWorker("Worker");
workerInstance.start();
let job = workerInstance.run<void>(() =>{
    throw new Error("error")
})
try{
    job.Await();
...
workerInstance.quit();
```
由于ArkTS-Sta中的[MessageHandler](../reference/apis-arkts/message_handler.md)本质是通过[EAWorker.run](../reference/apis-arkts/eaworker_managed.md#run)去执行线程任务，所以在以下消息处理场景中，对于未捕获的异常，ArkTS-Dyn会发生jscrash并终止程序，而ArkTS-Sta则会在线程任务运行结束后，将未捕获的异常重定向于console中输出并清除未捕获的异常。
...
```ts
const currentworker = EAWorker.main();
const workerInstance = new EAWorker("Worker");
workerInstance.start();

const workerCB = (msg: concurrency.Message) =>{
    // console:[nodict]coroutines: Coroutine "XXX" completed with an exception: escompat.Error
    throw new Error("error");
}
let workerHandler = new concurrency.MessageHandler(workerCB, workerInstance);
let workerMessage = new concurrency.Message(1, "test", workerHandler);
workerMessage.sendToTarget();
```

对于不能由开发者主动等待获得返回值的线程任务，例如在ArkTS-Sta中通过[setTimeout()](../reference/common/js-apis-timer.md#settimeout)触发的任务，如果任务运行时产生未捕获异常，其行为与ArkTS-Dyn一致，都会导致crash，并终止程序。
