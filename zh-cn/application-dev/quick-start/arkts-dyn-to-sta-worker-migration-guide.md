...
### 如何进行异常处理
ArkTS-Dyn中由于内存隔离的限制，当Worker发生未捕获异常时，会向宿主线程发送一个异常信息的消息，并触发jscrash从而导致程序终止退出。对于这种情况，需要在宿主线程对worker注册[onAllErrors](../reference/apis-arkts/js-apis-worker.md#onallerrors18)来捕获worker上的未捕获异常，而不能在宿主线程通过运行时定义的try/catch机制进行异常处理。ArkTS-Sta中没有了内存隔离的限制，统一了所有线程的语义模型，可以由开发者来选择在合适的线程，通过try/catch机制来处理其他线程运行任务发生的未捕获异常。

对于通过ArkTS-Sta的[EAWorker.run](../reference/native-lib/eaworker_managed.md#run)接口运行的可等待线程任务，ArkTS-Dyn设计范式中可以通过[Await](../reference/native-lib/job.md#await)来将其他线程任务中未捕获的异常转移到当前线程中，通过try/catch机制来处理。
...
workerInstance.quit();
```
由于ArkTS-Sta中的[MessageHandler](../reference/native-lib/message_handler.md)本质是通过[EAWorker.run](../reference/native-lib/eaworker_managed.md#run)去执行线程任务，所以在以下消息处理场景中，对于未捕获的异常，ArkTS-Dyn会发生jscrash并终止程序，而ArkTS-Sta则会在线程任务运行结束后，将未捕获的异常重定向于console中输出并清除未捕获的异常。
