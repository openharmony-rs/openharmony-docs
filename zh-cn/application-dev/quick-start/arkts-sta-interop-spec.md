...
### ArkTS-Dyn导入ArkTS-Sta

ArkTS-Dyn可以通过虚拟模块导入ArkTS-Sta类型：

```typescript
import {UserService} from "staticApp.services.UserService";
import {Instant, ArrayList } from "staticApp.library.UtilLibrary";
```
...
### ArkTS-Sta导入ArkTS-Dyn

ArkTS-Sta必须可以加载ArkTS-Dyn模块：

```typescript
import {UserService} from "dynApp.services.UserService";
import {Instant, ArrayList} from "dynApp.library.UtilLibrary";
```

规范要求：

- ArkTS-Sta必须可以访问默认导出、命名导出和模块namespace。
- 绑定失败时必须抛出标准编译错误。
...
| 场景 | 方向 | 调用方式 | 被调用实体类型 |
|------|------|----------|---------------|
| S1 | ArkTS-Sta调用ArkTS-Dyn | 直接模块导入 | - Promise 实例对象<br>- 返回 Promise 的函数 / lambda<br>- 类的异步公共实例方法<br>- 类的异步公共静态方法 |
| S2 | ArkTS-Sta调用ArkTS-Dyn | 通过ESValue | 同S1场景 |
| S3 | ArkTS-Dyn调用ArkTS-Sta | 直接模块导入 | 同S1场景 |

**支持的Promise API规格**

以下API在S1、S2、S3场景下均按所述行为支持，除非特别说明。

- Promise实例方法:

    | API | 支持情况 |
    |-----|---------|
    | promise.then(onFulfilled, onRejected) 双参数 | onFulfilled 接收参数为value的回调函数<br>onRejected可选、只接收参数为Error类型的回调函数 |
    | promise.then(onFulfilled) 单参数 | onFulfilled接收无参回调函数、undefined或省略 |
    | promise.catch(onRejected) | onRejected可选、只接收参数为Error类型的回调函数<br>onRejected接收无参回调函数 |
    | promise.finally(onFinally) | onFinally可选、接收无参回调函数 |

- Promise静态方法:
    | API | 支持情况 |
    |-----|---------|
    | Promise.reject() | 参数只支持Error类型 |
    | Promise.resolve() | 支持静态以动态传入的promise对象作为参数<br>支持动态以静态传入的promise对象作为参数 |
    | Promise.all() | 同Promise.resolve() |
    | Promise.any() | 同Promise.resolve() |
    | Promise.race() | 同Promise.resolve() |
    | Promise.allSettled() | 不支持 |

- 其它:
    | API | 支持情况 |
    |-----|---------|
    | **async/await** | 支持在动态async函数中await规格内的静态promise对象<br>支持在静态async函数中await规格内的动态promise对象 |
    | ESValue.toPromise()（仅S2场景下） | 支持 |
    | **Promise\<T\>** | Promise\<T\>中泛型T支持类型：<br>Interop支持的基本类型、Array、Map、Set |
    | 静态子线程中调用动态promise | 只支持S2场景：通过ESValue在子线程中调用动态的promise对象 |
    | 动态子线程中调用静态promise | 不支持跨线程调用，不支持主线程和子线程间跨线程调用promise |
...
ArkTS-Dyn与ArkTS-Sta支持双向的重载方法，调用规则如下：

| 调用方向 | 支持范围与能力 | 限制说明 |
|----------|---------------|----------|
| ArkTS-Sta To ArkTS-Dyn | 1. 自动识别基础类型重载（数值、字符串、布尔）。<br>2. 支持数值精细匹配（由窄到宽优先级，无损优先）。<br>3. 支持基于继承关系的“遮蔽”规则（子类优先）。<br>4. 支持可选参数及变长参数的动态数量匹配。 | 1.暂时不支持 Any, Array, Promise, Function等参与重载匹配。<br>2.基本类型不匹配 Object 签名。<br>3.不支持联合类型的重载方法调用。 |
| ArkTS-Dyn To ArkTS-Sta | 1. 支持ArkTS-Sta重载签名映射到同一ArkTS-Dyn函数。<br>2. 支持变长参数的展开传递。<br>3. 支持通过特殊语义方法（Getter/Setter/Index）访问ArkTS-Dyn属性。 | 1. 调用决策完全在编译期完成，ArkTS-Dyn侧仅执行单一函数逻辑分发。 |

> **说明：**
> 
> 此处重载分析的对象包含，普通函数、构造函数、实例方法、静态方法等。
