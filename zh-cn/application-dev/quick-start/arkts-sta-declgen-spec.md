...
- 工具类型
    ...
    **完全不兼容**的场景：
    ...
    `Record<K, V>`类型尽管在`ArkTS-Sta`和`ArkTS-Dyn`中同时存在，由于一些历史原因，Declgen不保证对其转换的正确性。此外，Awaited<T\>、Pick<T, K\>、Omit<Type, Keys\> 等其它工具类完全不兼容。
...
ArkTS-Sta的普通函数重载目前在动态侧不可用，Declgen目前会删除普通函数重载。
...
```typescript
export declare function foo(p: string, q: number): void;
```
...
将被标记的`class`/`interface`/`enum`转换为`ESObject`。
