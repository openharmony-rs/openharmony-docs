...
新增对象天然共享特性，不再依赖Sendable特性。可直接使用ArkTS-Sta原生容器进行跨线程安全访问。
...
ArkTS演进为taskpool、process、ArkTSUtils提供原生支持，无需import和ArkTSUtils.locks.即可直接使用。

**适配建议：**

删除taskpool、process、ArkTSUtils、AsyncLock前的import，改为直接使用，并且process更名为StdProcess；同时删除AsyncLock的ArkTSUtils.locks.前缀。
...
- ArkTS-Sta中不支持以泛型数组的形式对回调参数类型进行校验。

- ArkTS-Sta中不支持将非可变参数类型回调赋值给可变参数函数。 
...
ArkTS-Sta中void不能作为变量类型，Promise\<void>在实例化时转换为Promise\<undefined>，因此需使用resolve(undefined)确保类型一致。
