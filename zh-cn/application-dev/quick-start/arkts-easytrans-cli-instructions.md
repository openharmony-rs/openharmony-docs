...
从GitCode仓库的[每日构建](https://dcp.openharmony.cn/workbench/cicd/dailybuild/dailylist)中下载ohos-sdk-full_Dyn_Sta构建的SDK并解压。

**图1** 每日构建页面

![mig001](figures/mig001.png)

目录`ohos-sdk\windows\ets\static\build-tools\easytrans`内包含迁移工具。
...
| --sdk-default-api-path     | OpenHarmony的SDK路径，以openharmony/ets结尾。                          |
...
2. 推荐先基于ArkTS-Dyn编译工程。否则，可能会缺失工程的模块间依赖，导致部分规则漏报或误报。
...
> **说明：** 
> 
>- `dist/tslinter.js`在下载的每日构建`easytrans`目录下，需要先将终端定位到该目录，或补全路径。
