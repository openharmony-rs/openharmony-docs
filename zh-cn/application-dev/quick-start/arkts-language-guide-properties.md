...
静态属性与实例属性的区别见实例属性与静态属性的区别章节。以下通过对比示例进一步展示两者在共享和独立方面的差异。
...
ArkTS不支持索引签名（`arkts-no-indexed-signatures`）和索引访问字段（`arkts-no-props-by-index`），详见从TypeScript到ArkTS的适配规则，使用`Map`替代。
