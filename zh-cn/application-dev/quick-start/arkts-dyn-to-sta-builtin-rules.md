...
迁移检查工具会基于工程目标ES版本选择ArkTS-Dyn builtin声明，并与ArkTS-Sta builtin声明进行签名匹配。若同一builtin API在ArkTS-Dyn中为`number`，在ArkTS-Sta中变更为`int`或`long`，工具会报告`builtin-api-num2int`问题。
