...
`for...in`遍历对象的**属性键名**（包括继承的可枚举属性），与`for...of`遍历**值**不同。`for...in`的遍历顺序不保证固定，且遍历数组时返回的是字符串索引（如`"0"`而非`0`），容易导致类型和逻辑错误。ArkTS不支持`for...in`（`arkts-no-for-in`），详见从TypeScript到ArkTS的适配规则，遍历数组应使用`for...of`或`forEach`，遍历对象键名应使用`Object.keys()`。
