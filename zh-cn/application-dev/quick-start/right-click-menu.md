...
| 属性名称 | 含义 | 数据类型 | 是否可缺省 |
| -------- | -------- | -------- | -------- |
| abilityName | 表示当前右键菜单对应的需要拉起的Ability名称。<br/>**说明：**<br/> 当配置了menuSubItems时，该标签需配置为空字符串。 | 字符串 | 不可缺省。 |
...
| menuSubItems | 定义该菜单项的子菜单列表，用于实现子菜单。当配置了该标签时，当前菜单项仅作为父节点展示，不再读取该菜单项的abilityName、menuHandler和menuContext，而是由子菜单项各自携带这些标签来定义响应行为和展示条件。<br/>**说明：**<br/> 从API版本26.1.0开始，支持该标签。<br/>根据子菜单项的menuContext进行过滤，仅当至少一个子菜单项满足当前上下文条件时，才会展示父菜单项及其子菜单。 | 对象数组 | 可缺省，缺省值为空。 |
| menuHandler | 一个Ability可以创建多个右键菜单， 该标签与右键菜单显示项一一对应，用于区分用户拉起的不同右键菜单项。开发者可自定义该标签取值，确保该标签在整个Ability中唯一。在用户点击右键菜单拉起应用时，会作为参数传递给应用。<br/>**说明：**<br/> 当配置了menuSubItems时，该标签需配置为空字符串。 | 字符串 | 不可缺省。 |
| menuContext | 定义展示该菜单项需要的上下文，包括触发场景（空白处、文件、文件夹等）、选择方式（单选、多选）和文件类型过滤等。<br/>**说明：**<br/> 当配置了menuSubItems时，该标签需配置为空数组。 | 对象数组 | 不可缺省。 |

**表2** menuSubItems标签配置说明

| 属性名称 | 含义 | 数据类型 | 是否可缺省 |
| -------- | -------- | -------- | -------- |
| abilityName | 子菜单项对应的需要拉起的Ability名称。 | 字符串 | 不可缺省。 |
| menuItem | 子菜单项显示的文本信息。 | 资源id | 不可缺省。 |
| menuIcon | 子菜单项显示的自定义图标资源。取值为长度不超过255字节的字符串。图标规格为不超过5MB的PNG格式图片，图片设计应遵循应用图标设计规范。<br/>**说明：**<br/> 使用自定义图标需要申请`ohos.permission.CUSTOMIZE_MENU_ICON`权限，未授权或资源格式不符合要求时，菜单项将不显示自定义图标。 | 字符串 | 可缺省，缺省值为空。 |
