...
ArkTS-Dyn中定义一个异常的实例，可以在ArkTS-Sta中捕获这个异常。
...
TS中定义一个异常的实例，可以在ArkTS-Sta中捕获这个异常。
...
在JS中定义一个异常的实例，可以在ArkTS-Sta中ESValue来获取这个异常。
...
### 在ArkTS-Sta中使用ArkTS-Dyn

#### ArkTS-Dyn给ArkTS-Sta传参或赋值给Object类型
...
#### ArkTS-Dyn的Object内置方法作用在ArkTS-Sta对象上
...
#### ArkTS-Dyn的Reflect内置方法作用在ArkTS-Sta对象上
...
#### ArkTS-Sta的Object内置方法作用在ArkTS-Dyn对象上
...
#### ArkTS-Sta Reflect内置方法作用在ArkTS-Dyn对象
...
#### ArkTS-Dyn Object内置方法作用在ArkTS-Sta对象
...
- 变更理由：
  ArkTS-Sta对象在动态上下文中没有own property，表现为sealed，不能修改对象布局。

---

#### ArkTS-Dyn Reflect内置方法作用在ArkTS-Sta对象
...
- 变更理由：
  ArkTS-Sta和TS的可迭代的定义不同。ArkTS-Sta中没有Symbol.iterator方法，无法识别自定义的ts可迭代对象

---

#### Object内置方法作用在ArkTS-Sta对象
...
- 编译报错信息：
  ArkTS-Sta only catch a Error instance
- 运行时报错信息：
  不涉及
- 适配建议：
  ArkTS-Sta只能catch Error实例，针对非常规的ts异常对象，交互是会被包装到ESError中，通过getValue()方法可以获取包装了原始异常对象的ESValue实例
- 变更理由：
  ArkTS-Sta中throw和catch的对象只能是Error的实例

---

#### ArkTS-Sta判断TS boxed type类型
...
- 变更理由：
  ArkTS-Sta不区分基本类型和boxed类型，因此js的boxed对象在interop时会被unboxed成基本类型的值

---

#### ArkTS-Sta动态import TS
...
- 编译报错信息：
  ArkTS-Sta cannot export entities from js
- 运行时报错信息：
  不涉及
- 适配建议：
  直接从js文件导入使用
...
- 编译报错信息：
  - ArkTS-Sta cannot import js files directly
  - foo is not callable
- 运行时报错信息：
  不涉及
- 适配建议：
...
- 运行时报错信息
  不涉及
- 适配建议
  ArkTS-Sta只能catch Error实例，针对非常规的js异常对象，交互是会被包装到ESError中，通过getValue()方法可以获取包装了原始异常对象的ESValue实例
- 变更理由
  ArkTS-Sta中throw和catch的对象只能是Error的实例

---

#### ArkTS-Sta访问js的boxed对象
...
- 变更理由
  ArkTS-Sta的对象布局在编译时就确定了，不能动态修改

---

#### js Object内置方法作用在ArkTS-Sta对象
...
- 变更理由
  ArkTS-Sta对象在动态上下文中没有own property，相关解构操作会失效。

---

#### ArkTS-Sta动态import JS
...
并发执行ArkTS-Dyn&ArkTS-Sta混合代码的主要方式通过ArkTS-Sta的上下文；ArkTS-Dyn中需要执行ArkTS-Dyn&ArkTS-Sta混合代码的场景需要迁移到ArkTS-Sta来保证易用性和性能。

### ArkTS-Sta使用ArkTS-Dyn的场景

#### ArkTS-Sta与ArkTS-Dyn的Promise互操作
...
#### ArkTS-Sta与ArkTS-Dyn的async function互操作
...
// OK, use the return value as a ArkTS-Sta Promise
const p = foo();
```

#### ArkTS-Sta单线程异步上下文与ArkTS-Dyn对象互操作
...
#### ArkTS-Sta多线程异步上下文与ArkTS-Dyn对象互操作

- 跨线程通信时，适用ArkTS-Dyn线程间通信的规格，但sendData / onReceiveData不再支持，用ArkTS-Sta线程间通信代替
- 使用当前线程的动态上下文，规格同主线程规格
...
function baz() {
  ...
  // NO. not allowed to call ArkTS-Dyn non-concurrenct function
...
### ArkTS-Dyn使用ArkTS-Sta的场景

#### ArkTS-Dyn与ArkTS-Sta的Promise互操作
...
#### ArkTS-Dyn与ArkTS-Sta的async function互操作
...
// OK, use the return value as a ArkTS-Dyn Promise
const p = foo();
```

#### ArkTS-Dyn单线程异步上下文与ArkTS-Sta对象互操作
