...
在ArkTS-Sta中使用`import`函数导入ArkTS-Dyn的相关文件内容，包含函数，类，对象等。
...
在ArkTS-Dyn中可以直接访问ArkTS-Sta中导入的对象方法。
...
export class C extends B {
  override foo() : void { console.info('D.foo') }
}
...
在ArkTS-Dyn中，可以将参数传递给导入的函数，从而将数据回传至ArkTS-Sta。
...
在ArkTS-Sta中使用`import`函数可以导入ArkTS-Dyn文件中的函数、类和对象等内容。
...
在ArkTS-Sta中，可以调用从TS导入的对象，并调用该对象的方法。
...
在ArkTS-Sta中可以通过调用从TS中导入的函数，将参数传递至TS。
...
在ArkTS-Sta中，可以使用ESValue调用JS对象的方法。
...
在ArkTS-Sta中，可以使用ESValue调用JS函数，将参数传递给JS。
