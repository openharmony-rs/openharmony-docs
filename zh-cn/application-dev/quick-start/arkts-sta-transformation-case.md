...
在静态化改造过程中，常会遇到将动态类型对象传入静态环境使用，或在静态环境中执行动态类型操作的情况。针对这类ArkTS-Sta调用ArkTS-Dyn的互操作场景，建议开发者显式使用ESValue的方式。
...
对于更具体的ESValue接口使用方法以及示例见：[ESValue接口文档](../reference/apis-arkts/arkts-sta-interop-esvalue.md)。
...
在ArkTS应用的静态化改造中，同样会面临将静态对象跨域传递至动态环境，或在ArkTS-Dyn中执行静态操作的需求。面对此类ArkTS-Dyn调用ArkTS-Sta的互操作场景，建议开发者采用显式引入STValue的调用方式。
...
| **对静态对象的解包** | 提供`unwrap`接口将STValue解包为ArkTS-Dyn值。支持解包获取 `boolean`、`number`、`string`、`bigint` 基本类型值及引用类型。 |
...
对于更具体的STValue接口使用方法以及示例见：[STValue接口文档](../reference/apis-arkts/arkts-sta-interop-stvalue.md)。
...
[EAWorker的基本使用](https://gitcode.com/openharmony/docs/blob/OpenHarmony_feature_20250702/zh-cn/application-dev/reference/native-lib/eaworker_managed.md)。
