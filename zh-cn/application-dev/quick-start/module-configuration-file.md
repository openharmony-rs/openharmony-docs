...
| extensionProcessMode | 标识当前ExtensionAbility组件的多进程实例模型，根据ExtensionAbility的类型不同，支持的配置项有所区别：<br/>对于 UIExtensionAbility 及其子类，支持以下三种模式：<br/>-&nbsp;instance：表示该ExtensionAbility每个实例一个进程。<br/>-&nbsp;type：表示该ExtensionAbility实例都运行在同一个进程里，与其他ExtensionAbility分离进程。<br/>-&nbsp;bundle：表示该ExtensionAbility实例都运行在应用统一进程里，与其他配置了bundle模型的ExtensionAbility共进程。<br>对于类型为 appService 的ExtensionAbility，支持以下两种模式：<br/>-&nbsp;type：表示该类型的所有实例运行在同一个进程中，与其他类型进程隔离。<br/>-&nbsp;bundle：表示实例运行在应用的统一进程中，与配置为bundle模型的其他ExtensionAbility共进程。默认值为 bundle。<!--RP10--><!--RP10End--> | 字符串 | 该标签可缺省，缺省值为空。 |
