...
| extensionProcessMode | 标识当前ExtensionAbility组件的进程模型，根据ExtensionAbility的类型不同，支持的配置项有所区别。支持的取值范围如下，默认值为bundle。<br/>-&nbsp;instance：进程名格式：进程名格式：[bundleName]（./app-configuration-file.md#配置文件标签）:[name]（#extensionabilities标签）:实例ID，表示该ExtensionAbility每个实例都各自运行在单独进程。<br/>-&nbsp;type：进程名格式：进程名格式：[bundleName]（./app-configuration-file.md#配置文件标签）:[name]（#extensionabilities标签），表示该ExtensionAbility的所有实例都运行在同一独立进程，与其他name的ExtensionAbility组件实例运行在不同进程。<br/>-&nbsp;bundle：表示该ExtensionAbility的实例与同包名下相同extensionType的ExtensionAbility实例运行在同一进程。<!--Del--><br/>-&nbsp;callerInstance：进程名格式：[bundleName]（./app-configuration-file.md#配置文件标签）:[name]（#extensionabilities标签）:[type]（#extensionabilities标签），表示该ExtensionAbility实例运行在单独进程，进程的隔离粒度为调用方。同一调用方拉起的ExtensionAbility的实例运行在同一进程，不同调用方拉起的ExtensionAbility的实例运行在不同进程。<!--DelEnd--><br/>对于UIExtensionAbility及其子类，支持instance、type、bundle三种进程模型。<br/>对于类型为appService的ExtensionAbility，支持type和bundle两种进程模型。<!--Del--><br/>对于类型为sysPicker/mediaControl的ExtensionAbility，支持bundle、type、instance、caller_instance四种进程模型。<!--DelEnd--><!--RP10--><!--RP10End--> | 字符串 | 该标签可缺省，缺省值为bundle。 |
