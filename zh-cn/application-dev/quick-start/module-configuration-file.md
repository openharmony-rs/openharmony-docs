...
| extensionProcessMode | 标识当前ExtensionAbility组件的多进程实例模型，根据ExtensionAbility的类型不同，支持的配置项有所区别，默认值为bundle：<br/>对于 UIExtensionAbility 及其子类，支持以下三种模式：<br/>-&nbsp;instance：多进程多实例，表示该ExtensionAbility每个实例一个进程。<br/>-&nbsp;type：指定多进程多实例，相同name的ExtensionAbility是一个进程。<br/>-&nbsp;bundle：单进程多实例，相同bundlename下，相同type类型的ExtensionAbility是一个进程。<br>对于类型为appService的ExtensionAbility，支持以下两种模式：<br/>-&nbsp;type：指定多进程多实例，相同name的ExtensionAbility是一个进程。<br/>-&nbsp;bundle：单进程多实例，相同bundlename下，相同type类型的ExtensionAbility是一个进程。<!--RP10--><!--RP10End--> | 字符串 | 该标签可缺省，缺省值为空。 |
