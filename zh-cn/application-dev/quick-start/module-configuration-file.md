...
| grantMode | 标识权限的授予方式，支持如下三种授予模式：<br/>-&nbsp;system_grant：安装后系统自动授予该权限。<br/>-&nbsp;user_grant：使用时动态申请，用户授权后才可使用。<br/>-&nbsp;manual_settings：需用户手动到系统设置页面授权后才可使用。<br/>从API version 21开始，权限的授予方式支持配置成manual_settings。 | 字符串 | 可缺省，缺省值为system_grant。 |
