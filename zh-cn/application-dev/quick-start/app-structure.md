...
| releaseType | 用于标识应用运行时SDK的状态。<br/>canaryN：面向特定开发者早期预览版本，不承诺质量，不承诺API稳定，N代表大于零的整数。。<br/>betaN：公开发布的Beta版本，早期Beta版本不承诺API稳定，经历若干次发布后，通过Release&nbsp;Notes对开发者声明该Beta版本为API稳定里程碑，后续版本的API冻结，N代表大于零的整数。。<br/>release：正式发布版本，承诺质量，API不可变更。当版本处于此状态时版本号中不呈现Stage字段。 | 字符串 | 配置在build.profile中，打包时由DevEco Studio填充到config.json中。 |
