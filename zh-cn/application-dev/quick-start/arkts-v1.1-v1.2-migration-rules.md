...
在ArkTS1.2中，用new创建的Number、Boolean和String对象不再是object类型，进行判断、比较等操作时，与ArkTS1.1的表现不同。
...
// ArkTS1.1结果：true（这里if语句判断的是Boolean对象是否为空，而不是拆箱后的结果，所以结果为true）
// ArkTS1.2结果：false
...
```typescript
// ArkTS1.1&ArkTS1.2结果均为："number"
typeof 1;

// ArkTS1.1&ArkTS1.2结果均为：true
1 == 1;

// ArkTS1.1&ArkTS1.2结果均为：false
if (false) {}
```
...
```typescript
let s = 1000000000000000000000000000000000000; // 在ArkTS1.1中，该值会被转换为浮点形式数据，而ArkTS1.2中则会导致编译错误。
let t = 1E+309;                               // 在ArkTS1.1中，该值会被转换为Infinity，而在ArkTS1.2中则会导致编译错误。
```
...
在ArkTS1.2中，属性和方法有本质区别，函数属性类型不再与方法类型兼容，因此不支持这种写法。
