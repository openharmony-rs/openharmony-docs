...
## 编译态包结构
不同类型的Module编译后会生成对应的HAP、HAR、HSP等文件，编译后可通过DevEco Studio或打包工具打包成APP包，用于上架应用市场。编译HAP和HSP时，会把它们所依赖的HAR直接编译到HAP和HSP中，因此打包成APP包后，只有HAP和HSP包，没有HAR包。开发态视图与编译态视图的对照关系如下：
