...
在`Stage`模型下， 允许系统应用创建和管理系统窗口，包括音量条、壁纸、通知栏、状态栏、导航栏等。具体支持的系统窗口类型见WindowType。
...
> **说明：**
>
> 本文档涉及系统接口的使用，请使用full-SDK进行开发。<!--Del-->具体使用可见full-SDK替换指南。<!--DelEnd-->


## 接口说明

更多API说明请参见@ohos.window (窗口)(系统接口)。
...
| TransitionContext | completeTransition(isCompleted: boolean): void               | 设置属性转换的最终完成状态。该函数需要在动画函数animateTo()执行后设置。此接口为系统接口。 |
...
1. 创建系统窗口。
   ...
   在ServiceExtensionContext下，使用`window.createWindow`接口创建音量条系统窗口。
