...
| Window         | setWindowSystemBarProperties(systemBarProperties: SystemBarProperties): Promise&lt;void&gt; | 设置主窗口状态栏<!--RP5-->、工具栏或三键导航栏的属性。当前工具栏的属性设置仅Car设备支持。<!--RP5End-->使用Promise异步回调。<br>`systemBarProperties`：状态栏<!--Del-->、工具栏或三键导航栏<!--DelEnd-->的属性集合。 |
