...
| Window         | setWindowSystemBarEnable(names: Array&lt;'status'\|'navigation'&gt;): Promise&lt;void&gt; |设置主窗口状态栏、<!--RP4-->工具栏或三键导航栏的显示或隐藏，状态栏通过status控制、工具栏或三键导航栏通过navigation控制。当前工具栏的显示或隐藏设置仅Car设备支持。<!--RP4End-->使用Promise异步回调。<br>例如，该参数设置为['status',&nbsp;'navigation']，则全部显示；设置为[]，则不显示。|
| Window         | setWindowSystemBarProperties(systemBarProperties: SystemBarProperties): Promise&lt;void&gt; | 设置主窗口状态栏、<!--RP5-->工具栏或三键导航栏的属性。当前工具栏的属性设置仅Car设备支持。<!--RP5End-->使用Promise异步回调。<br>`systemBarProperties`：状态栏、<!--Del-->工具栏或三键导航栏<!--DelEnd-->的属性集合。<br/>`systemBarProperties`：导航栏、状态栏的属性集合。 |
