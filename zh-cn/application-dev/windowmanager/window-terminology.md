...
全局坐标系是指在设备连接[扩展屏](../displaymanager/display-terminology.md#扩展屏)（多物理屏幕）的场景下，以主屏幕左上角为原点(0, 0)，对窗口、指针等对象的位置进行统一描述的坐标体系。
...
- 窗口跨屏移动：调用基于全局坐标系的窗口移动API，无需传递具体屏幕ID参数，即可实现窗口在多屏之间移动。
