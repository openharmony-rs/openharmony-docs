...
智慧多窗悬浮窗是指设备屏幕上悬浮的、非全屏的应用窗口。一般用于在已有全屏任务运行的基础上，临时处理另一个任务，或短时间多任务并行使用。如浏览网页的同时回复消息。智慧多窗悬浮窗是一种窗口模式，只有主窗口支持。详细说明请参考[智慧多窗](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-window-practice)。

全局悬浮窗是指一种特殊的应用窗口，具备在应用主窗口和对应Ability退至后台后仍然可以在前台显示的能力。全局悬浮窗可以用于应用退至后台后，使用小窗继续播放视频，或者为特定的应用创建悬浮球等快速入口。应用在创建全局悬浮窗前，需要申请对应的权限。全局悬浮窗是一种窗口类型。详细说明请参考[设置全局悬浮窗](application-window-stage.md#设置全局悬浮窗)。
