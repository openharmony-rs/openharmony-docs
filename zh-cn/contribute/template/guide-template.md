...
_必选。阐述本Kit是否支持模拟器、有哪些能力差异。_

_**【开发者关注点】**_

_我要使用该Kit/方案/特性/功能/模块模拟器开发，是否支持？有哪些能力差异？_

**_【写作要点】_**

- _根据Kit的实际模拟器支持情况，参考如下句式，清晰、准确阐述模拟器规格。_
- _其中，模拟器的“通用差异”如性能差异、图形显示差异等，统一在IDE文档的“模拟器与真机的差异”中阐述，各Kit按需提供链接。_


_句式1：适用于完全不支持模拟器的Kit_

本Kit暂不支持模拟器。

_句式2：适用于完全支持模拟器、且仅存在“通用差异”的Kit_

本Kit支持模拟器，但与真机存在差异，详情请参见“模拟器与真机的差异（链接到IDE的对应页面）”。

_句式3：适用于模拟器与真机存在“通用差异”及其他能力差异的Kit_

本Kit支持模拟器，但与真机存在部分能力差异，具体差异如下。</br>
- 通用差异：请参见“模拟器与真机的差异（链接到IDE的对应页面）”。
- xxx（客观描述“通用差异”之外的其他能力差异）。
...
- _规范性要求_
   - _保证示例代码的逻辑/语法正确性以及书写规范性（例如为示例代码添加语言属性等），详情请参见[示例代码风格](https://gitcode.com/openharmony/docs/blob/master/zh-cn/contribute/style-guide/style-guide-example-code-style.md)。_
  - _涉及用户手机号码、身份证、帐号名等敏感信息均需要打码处理，如186\*\*\*\*\*\*\*\*；涉及IP地址、域名等，需要使用私网IP或相应格式代替，如xx.xx.xx.xx、www.example.com，禁止使用真实IP地址、域名。_
