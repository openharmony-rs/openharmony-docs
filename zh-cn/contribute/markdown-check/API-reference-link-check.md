...
其他检查项请参见 [门禁检查规则说明文档](./md-style-check.md)。
...
> **注意：**
> 跳过条件 5 至 7 意味着：本项检查 **不会** 报出断链问题。若链接本身失效，会先由 [相对链接检查](./md-style-check.md#相对链接检查) 报错。请先修复断链，再处理名称准确性问题。
...
英文 API 文档的命名与中文文档存在体系差异，当前版本对 `/en/` 路径下的文档跳过本项检查。英文文档的链接问题由 [相对链接检查](./md-style-check.md#相对链接检查) 与 [HTTP 链接检查](./md-style-check.md#http-链接检查) 覆盖。
