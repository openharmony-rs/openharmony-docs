...
> **说明：**
> API 链接准确性检查的规则较为独立，已拆分为单独文档，请参见 [API链接准确性检查规则说明](./API链接准确性检查规则说明.md)。
...
| [API 链接准确性检查](./API链接准确性检查规则说明.md) | `链接名称不准确`、`链接名称在标题及标题下表格均不存在`、`检测到链接名称与目标页面标题不一致` | `application-dev/` 下中文文档 |
...
error_info: 错误类型：代码格式异常，多行注释中，多行注释中*(星号)未对齐；异常代码起始位置：第 45行。请处理！
```
...
| 链接名称不准确 | API 链接准确性检查 | [查看](./API链接准确性检查规则说明.md) |
| 链接名称在标题及标题下表格均不存在 | API 链接准确性检查 | [查看](./API链接准确性检查规则说明.md) |
| 检测到链接名称与目标页面标题不一致 | API 链接准确性检查 | [查看](./API链接准确性检查规则说明.md) |
...
`error_type` 为 `检测到链接名称与目标页面标题不一致`，属于 API 链接准确性检查，参见 [API链接准确性检查规则说明](./API链接准确性检查规则说明.md)。
...
> **注意：**
> 中英文括号形态、括号前后有无空格，都属于必须与目标标题一致的细节。从其他文档复制链接时，这类差异是「检测到链接名称与目标页面标题不一致」最常见的诱因。名称预处理规则详见 [链接名称的预处理](./API链接准确性检查规则说明.md#链接名称的预处理)。
