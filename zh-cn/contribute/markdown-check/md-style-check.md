...
Readme中，节点必须有urlpath
可参考website或已有kit的Readme
...
[示例代码风格-【规则】使用console.info进行正常日志打印](https://gitcode.com/openharmony/docs/blob/master/zh-cn/contribute/style-guide/style-guide-example-code-style.md#%E8%A7%84%E5%88%99%E4%BD%BF%E7%94%A8consoleinfo%E8%BF%9B%E8%A1%8C%E6%AD%A3%E5%B8%B8%E6%97%A5%E5%BF%97%E6%89%93%E5%8D%B0)
...
[json标准格式](http://www.json.org/json-zh.html)
...
| 注释 | 不支持 | 支持`//` 和 `/* */` |
| 尾随逗号 | 不允许 | 允许 |
| 对象键引号 | 必须使用双引号 | 可省略（支持单引号） |
| 字符串 | 仅双引号 | 可用单引号 |
| 数字 | 十进制 | 支持十六进制、Infinity、NaN、-0 |
| 关键字 | 小写 true/false/null | 大小写不敏感 |

> **说明：**
>
> 当前已对代码片段进行适配，适配方式为，在代码片段外加一对大括号\{\}。

### 报错信息

json代码块中格式存在异常，请将代码块放置在IDE中进行确认处理。
json5代码块中格式存在异常，请将代码块放置在IDE中进行确认处理。

### 问题原因

1. json中存在注释。
2. json中存在多余/缺失符号，如逗号，大括号，引号等。
3. 代码块中存放多个json对象
...
1. 建议在IDE中创建对应类型文档，如json、json5，然后将问题代码放入文件中，依据IDE的提示处理对应问题。
2. json代码块中使用了json5的格式，将代码块格式由json替换为json5。
