...
| **词语** | **说明** |
| -------- | -------- |
| 泄漏 | **定义**：因程序逻辑缺陷，导致已分配的资源（内存、句柄、连接、线程等）无法被系统回收或重用。<br>**适用对象**：内存、句柄、连接池、线程池、文件描述符、GPU显存、ClassLoader、缓存引用。<br>**例子**：内存泄漏、资源泄漏 |
| 泄露 | **定义**：数据、信息或物理介质穿透既定安全边界，流向非授权环境。<br>**适用对象**：用户信息、密钥凭证、日志敏感字段、API过度返回字段、物理液体/信号。<br>**例子**：信息泄露、数据泄露|
