...
当前Linux内核基线是基于Linux社区 5.10 LTS版本演进，合入CVE及bugfix补丁。具体信息参考[代码库](https://gitcode.com/openharmony/kernel_linux_5.10)，对应repo工程代码路径为`kernel/linux/linux-5.10`。
...
1. 准备内核config（特别是芯片相关的config）。
   ...
   当前内核config采用分层配置结构。以hi3516dv300开发板（设备名hispark_taurus）为例，其config由以下分层配置组合生成：

   - `base_defconfig`:内核必选模块及安全红线特性配置（不可覆盖）
   - `type/small_defconfig`:小型系统通用配置
   - `hispark_taurus/arch/arm_defconfig`:芯片平台相关配置

   新增芯片时，需在`kernel/linux/config/linux-5.10/`目录下新建以设备名命名的目录（如&lt;YOUR_DEVICE&gt;），在其中放置芯片平台对应的arch配置文件。
...
2. 准备芯片补丁。
   ...
   以hi3516dv300开发板（设备名hispark_taurus）为例，参考已有的patch目录`hispark_taurus_patch`，新建&lt;YOUR_DEVICE&gt;_patch目录，放置芯片补丁。HDF驱动补丁统一放在`common_patch/hdf.patch`中

3. 编译。

   具体内核编译入口脚本位于工程目录`kernel/linux/build/`下面，版本级整编命令会通过BUILD.gn进入`kernel_module_build.sh`和`kernel.mk`，需要在这2个文件中针对性进行patch及defconfig文件路径、编译器、芯片架构、内核Image格式等的适配。
   ...
   >
   > - 参考`kernel/linux/build/kernel.mk`，在OpenHarmony工程的编译构建流程中会拷贝kernel/linux/linux-5.10的代码环境后进行打补丁动作，在使用版本级编译命令前，需要kernel/linux/linux-5.10保持原代码环境。
   > 
   > - 对应拷贝后的目录位于：`out/<***>/kernel/linux/linux-5.10`，可以在该目录下进行补丁的修改适配。
...
1. 制作根文件系统镜像。
   ...
   - 启动配置
     ...
     启动配置在`vendor/{company}/{product}/init_configs/etc`目录下，包括fstab、rcS和Sxxx文件，请按开发板实际情况配置。
