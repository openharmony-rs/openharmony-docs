...
本文章基于意法半导体`STM32MP157`芯片的小熊派[BearPi-HM Micro开发板](https://gitcode.com/openharmony/device_board_bearpi)，进行小型带屏开发板的移植，实现了`ace_engine_lite`、`arkui_ui_lite`、`aafwk_lite`、`appexecfwk_lite`、`HDF`等部件基于`OpenHarmony LiteOS-A`内核的适配。移植架构上采用`Board`与`SoC`分离的方案。

## 编译构建

### 目录规划

本方案的目录结构使用[Board和SoC解耦的设计思路](https://gitcode.com/openharmony-sig/sig-content/blob/master/devboard/docs/board-soc-arch-design.md)，并把芯片适配目录规划为：
...
2. 在`//device/soc/st/common/platform/Kconfig`中添加驱动相关配置。
    ...
    ```
3. 在`//device/board/bearpi/bearpi_hm_micro/liteos_a/drivers/Kconfig`中引用SoC平台驱动配置。
    ```text
    source "../../device/soc/st/common/platform/Kconfig"
    ```
4. 在`//vendor/bearpi/bearpi_hm_micro/kernel_configs/debug_tee.config`中使能相关配置。
    ```.config
    ...
    ...
    ```
### gn编译适配
1. 在`//device/board/bearpi/bearpi_hm_micro/liteos_a`中新建BUILD.gn，添加代码如下，此模块依赖board、drivers、hdf_config。
    ```gn
    ...
    group("liteos_a") {
      deps = [
        "board",
        "drivers",
        "//device/soc/st/stm32mp1xx/mpp/module_init",
      ]
    ...
    ```
2. 在`//device/board/bearpi/bearpi_hm_micro/liteos_a/board`中新建BUILD.gn，添加代码如下。将os_adapt.c内核启动相关代码编译进系统。
    ```gn
    ...
    ```
3. 在`//device/board/bearpi/bearpi_hm_micro/liteos_a/drivers`中新建BUILD.gn，添加代码如下，将device/soc/st/common/platform路径下的HDF驱动编译进系统。
    ```gn
    ...
    ```
4. 在`//vendor/bearpi/bearpi_hm_micro/hdf_config`中新建BUILD.gn，添加代码如下，将HCS配置文件编译进系统。
    ```gn
    import("//drivers/hdf_core/adapter/khdf/liteos/hdf.gni")
...
3. 在`//device/soc/st/stm32mp1xx/sdk_liteos/hdf_config/gpio`中添加gpio硬件描述信息文件gpio_config.hcs，在该文件中添加驱动私有配置信息。
...
启动恢复子系统需要适配`bootstrap_lite`、`init`、`appspawn`三个部件。请在`vendor/bearpi/bearpi_hm_micro/config.json`中新增对应的配置选项。
...
进行`DFX`子系统适配需要添加`hilog_lite`、`hidumper_lite`部件，直接在`config.json`文件配置即可。
...
进行系统服务管理子系统适配需要添加`samgr_lite`、`safwk_lite`两个部件，直接在`config.json`配置即可。
...
进行安全子系统适配需要添加`permission_lite`、`appverify`、`device_auth`、`device_security_level`、`huks`五个部件，直接在`config.json`配置即可。
...
```json
    ...
    "components": [
        ...
        { "component": "device_security_level", "features": [] },
...
进行图形子系统适配需要添加`graphic_utils_lite`部件，直接在`config.json`配置即可。
...
进行包管理子系统适配需要添加`bundle_framework_lite`部件，直接在`config.json`配置即可。
