...
OpenHarmony提供了两种驱动适配方式：使用外设驱动子系统、使用HDF驱动框架。由于轻量级系统的资源有限，这里建议使用外设驱动子系统（IOT子系统）方式。
...
2. 添加适配文件。
   ...
   static_library("hal_iothardware") {   #目标名。
       ...
       include_dirs = [
        "//base/iothardware/peripheral/interfaces/inner_api",     #iot子系统的头文件路径
        "//device/soc/MyDeviceCompany/MyBoard/sdk_liteos/include" #厂商SDK头文件路径（不同厂商有差异）
       ]
   }
   ```
