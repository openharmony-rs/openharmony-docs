...
通过OpenHarmony SDK获取hdc工具，无需额外下载配置。hdc默认安装在DevEco Studio/sdk/default/openharmony/toolchains路径下，MacOS系统的sdk位于DevEco Studio/Contents目录下。

hdc支持USB，TCP无线和UART串口三种连接调试方式。USB和无线调试方式可以在设备的设置>系统>开发者选项中开启或关闭调试开关 ，无需重启设备即可生效。UART串口方式需调试设备支持通过转接器连接串口进行调试。具体调试连接方式及操作步骤请参考[USB连接场景](#usb连接场景)，[TCP连接场景](#tcp连接场景)，[UART串口连接场景](#uart串口连接场景)。

### （可选）命令行直接执行hdc程序

**安装目录下的调试**

1. 开发者可以在OpenHarmony SDK的toolchains子目录下执行hdc命令进行设备调试。

2. hdc支持独立运行。将toolchains子目录下的hdc可执行文件和libusb_shared依赖文件集中存放至自定义目录中，在自定义目录下直接执行hdc命令，实现独立调试。
...
> **说明：**
>
> 环境变量配置完成后，重启命令行窗口或其他使用到OpenHarmony SDK的软件，以生效新配置的环境变量。
...
| [target mount](#以读写模式挂载系统分区) | 以读写模式挂载系统分区。 |
| [smode](#授予设备端hdc后台服务进程root权限) | 授予设备端hdc后台服务进程root权限，使用-r参数取消授权。 |
| [keygen](#安全相关命令) | 生成一个新的密钥对。 |
| [version](#查询hdc版本号) | 打印hdc版本信息，也可使用hdc -v打印版本信息。 |
| [checkserver](#查询客户端和服务器进程版本) | 获取客户进程与服务进程版本信息。 |
| [checkdevice](#检查串口连接状态) | 检查给定串口是否可用，串口是否有设备连接。（UART串口连接场景） |
| [any](#连接首个可连接串口设备) | 尝试连接首个就绪串口设备。（UART串口连接场景） |
| [bugreport](#导出系统信息) | 导出系统信息。 |
| [update](#升级整包) | 升级整包。 |
| [flash](#刷写镜像) | 刷写镜像。 |
| [erase](#擦除镜像) | 擦除镜像。 |
| [format](#格式化镜像userdatacache) | 格式化镜像。 |
...
| 确认项 | 正常 | 异常处理 |
| -------- | -------- | -------- |
| UART串口调试系统参数 | 开启。 | 调试系统参数默认关闭，需通过手动执行命令方式开启，执行后需重启调试设备。命令如下：<br/>1. hdc shell param set persist.hdc.mode all<br/>2. hdc shell param set persist.hdc.mode.uart enable  |
| USB信号及转接器引脚说明 | - | TXD：接 MCU.RXD（信号流向：MCU.RX << FT232 << PC.TX）<br/>RXD：接 MCU.TXD（信号流向：MCU.TX >> FT232 >> PC.RX） |
| hdc环境变量 | 终端命令行输入hdc -h有回显帮助信息内容。 | 参见[环境准备](#环境准备)章节。 |
| 驱动 | 连接hdc设备后，设备管理器端口（COM和LPT）存在设备“USB Serial Port（串口号）”。 | 参见[设备无法识别](#设备无法识别)章节。 |

**连接步骤**

1. 电脑端通过USB连接设备，同时使用UART串口转接器连接设备。

2. 使用以下命令通过UART串口连接设备：

   ```shell
   hdc tconn serial-port
   hdc tconn serial-port,1500000 # 1500000为波特率。
   ```
...
| 参数 | 说明 |
| -------- | -------- |
| -b bundlename | 3.1.0e版本新增参数。指定可调试应用包名。<br/>- 指定command参数时：在该可调试应用数据目录内以非交互式模式执行命令。<br/>[命令行方式访问应用沙箱](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-device-file-explorer#section48216711204)。<br/>- 缺省command参数时：3.2.0e及以上版本支持进入该可调试应用数据目录的交互式shell会话，默认工作目录为可调试应用数据目录根路径。<br/>未配置此参数时，默认执行路径为系统根目录。 |
...
> **说明：**
>
> 使用参数[-b bundlename]指定包名，该包名对应的已安装应用必须满足以下条件：使用调试证书签名，并且已在设备上启动。
>
...
| -------- | -------- |
| SOURCE | 本地待传输的文件路径。 |
| DEST | 远程目标文件路径。<br>从API version 21开始，媒体库文件支持通过hdc进行部分操作（低版本使用会提示[Fail]Error opening file: ...）。<br>媒体库文件路径：/mnt/data/\<uid\>/media_fuse/Photo/目录及其子目录，\<uid\>为当前用户的id。|
| -a | 保留文件修改时间戳。 |
...
| -------- | -------- |
| SOURCE | 本地待接收的文件路径。 |
| DEST | 远程待传输的文件路径。 <br>从API version 21开始，媒体库文件支持通过hdc进行部分操作（低版本使用会提示[Fail]Error opening file: ...）。<br>媒体库文件路径：/mnt/data/\<uid\>/media_fuse/Photo/目录及其子目录，\<uid\>为当前用户的id。 |
| -a | 保留文件修改时间戳。 |
| -sync | 只传输文件mtime有更新的文件。<br/>mtime（modified timestamp）：修改后的时间戳。 |
| -z | 通过LZ4格式压缩传输，此功能未开放，请勿使用。 |
| -m | 文件传输时同步文件DAC权限，uid，gid，MAC权限。<br/>DAC（Discretionary Access Control）：自主访问控制，<br/>uid（User identifier）：用户标识符（或用户ID），<br/>gid（Group identifier）：组标识符（或组ID），<br/>MAC（Mandatory Access Control）：强制访问控制（或非自主访问控制）。 |
...
> **说明：**
>
> 使用参数[-b bundlename]指定包名，应满足条件：指定包名的已安装应用为“使用调试证书签名的应用”且在设备上已启动。
>
...
| track-jpid [-a\|-p] | 实时显示设备上已打开应用的进程pid和应用包名，其中只有debug标签的应用可以被调试。不加参数时只显示已打开debug版本应用的进程pid和应用包名，使用-a参数会显示debug和release应用的进程标签，使用-p参数不显示debug和release的进程标签。 |
...
| smode [-r] | 授予设备端hdc后台服务进程root权限，使用-r参数取消授权。 |
...
| -------- | -------- |
| - | 串口空闲可连接调试设备。 |
| [Info]Target is connected, repeat operation | 调试设备已连接。 |
| [Fail]CreateConnect failed | 创建串口连接异常。 |
| [Fail]Connect failed | 串口连接失败。 |

**使用方法**：

```shell
# 串口空闲可连接调试设备，打印空行。
$ hdc checkdevice COM4
...
| Connect OK. | 连接成功。 |
| [Fail]Connect failed | 串口连接失败。 |
| [Fail]No target available | 无可用串口设备。 |
...
flash命令对接设备端的flashd模块，flashd模块在root设备且升级模式才存在。flashd适用于不支持fastboot刷机的设备快速刷写镜像。进入flashd命令：
...
```shell
hdc flash <image name> <image path>
```

**使用方法**：

```shell
$ hdc flash updater_ramdisk updater_ramdisk.img
```

### 擦除镜像

```shell
hdc erase <image name>
```

**使用方法**：

```shell
$ hdc erase updater_ramdisk
```

### 格式化镜像（userdata、cache）
...
```shell
hdc update <package path>
```

**使用方法**：

```shell
$ hdc update update.zip
```
...
| 操作系统 | 配置方法 |
| -------- | -------- |
| Windows | 在电脑设置中搜索“查看高级系统设置”，在环境变量>系统变量>Path>编辑中，添加环境变量名称为OHOS_HDC_LOG_LEVEL，变量值为5。配置完毕后点击确认。环境变量配置完成后，关闭并重启命令行或其他使用到OpenHarmony SDK的软件，以生效新配置的环境变量。 |
...
| 3.2.0e | 26及以上版本 | - shell命令支持-b参数缺省command进入交互式shell会话：详细说明参见[执行交互命令](#执行交互命令)。 |
> **注意：**
>
