...
| 名称                                             | 类型      | 值         | 说明                                                   |
| ------------------------------------------------ | -------- | -----------| ------------------------------------------------------ |
| RULE_CAUTION_PRINT_LOG                           | uint64_t   | 1ULL << 63 | 告警规则，当有告警时记录日志。                            |
| RULE_CAUTION_TRIGGER_CRASH                       | uint64_t   | 1ULL << 62 | 告警规则，当有告警时让应用退出。                          |
| RULE_THREAD_CHECK_SLOW_PROCESS                   | uint64_t   | 1ULL       | 检测规则，检测是否有耗时函数被调用。                      |
| RULE_THREAD_CHECK_NETWORK_USAGE                  | uint64_t   | 1ULL << 1  | 检测规则，检测线程是否调用网络耗时接口。<br>**起始版本**: 26.0.0  |
| RULE_CHECK_ABILITY_CONNECTION_LEAK               | uint64_t   | 1ULL << 33 | 检测规则，检测是否发生ability泄露。                      |
| RULE_CHECK_ARKUI_PERFORMANCE<sup>11+</sup>       | uint64_t   | 1ULL << 34 | 检测规则，检测arkui性能。                               |
...
| -------- | -------- | 
| AddRule(uint_64_t&nbsp;rule)&nbsp;:&nbsp;void | 添加一条或者多条[规则](#规则说明)到系统，系统根据添加的规则进行检测或反馈。 | 
...
| NotifyNetworkUsage()&nbsp;:&nbsp;void | 通知线程有调用网络耗时接口。 | 

  **表2** Caution类的函数接口

| **方法（包括返回值、方法名、参数列表）** | **描述** | 
| -------- | -------- | 
| GetTriggerRule()&nbsp;:&nbsp;uint_64_t  | 获取触发当前告警的检测规则。 | 
...
1. 在使用hichecker功能相关的代码文件中，包含hichecker头文件：
   ...
   - NotifyNetworkUsage() 通知线程有调用网络耗时接口。
