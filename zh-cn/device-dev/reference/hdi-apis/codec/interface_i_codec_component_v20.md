...
| [SendCommand](#sendcommand) ([in] enum [OMX_COMMANDTYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h) cmd, [in] unsigned int param, [in] byte[] cmdData) | 发送命令给组件。 | 
...
| [GetState](#getstate) ([out] enum [OMX_STATETYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h) state) | 获取组件的当前状态。 | 
...
其他值表示底层返回失败，具体错误码详见OpenMAX IL定义的[OMX_ERRORTYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。
...
调用此接口使组件去初始化，当组件处于OMX_StateLoaded状态时，将直接关闭组件，更多组件状态的说明请详见[OMX_STATETYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。
...
当组件处于OMX_StateLoaded状态时（表示组件已加载），用户通过调用此接口确定组件是否可以进行Tunnel传输，如果可以则设置组件的Tunnel传输。 更多组件状态的说明请详见[OMX_STATETYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。
...
此接口在组件处于OMX_StateExecuting或者OMX_StatePause状态时调用，更多组件状态的说明请详见[OMX_STATETYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。
...
- 在去使能端口上，组件处于OMX_StateExecuting、OMX_StatePause或OMX_StateIdle时调用。 更多组件状态的说明请详见[OMX_STATETYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。
...
| 名称 | 描述 | 
| -------- | -------- |
| index | 待填充结构的索引，详见[OMX_INDEXTYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。 | 
...
| indexType | 由paramName转换的配置索引，详见[OMX_INDEXTYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。 | 
...
当组件处于除了OMX_StateInvalid（组件状态异常）之外的其他状态，用户可通过此接口获取组件参数，组件状态详见[OMX_STATETYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。
...
| 名称 | 描述 | 
| -------- | -------- |
| index | 待填充结构的索引，详见OMX IL定义的[OMX_INDEXTYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。 | 
...
```
ICodecComponent::GetState ([out] enum [OMX_STATETYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h) state)
```
...
| 名称 | 描述 | 
| -------- | -------- |
| state | 指向获取到的状态指针，组件状态详见[OMX_STATETYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。 | 
...
```
ICodecComponent::SendCommand ([in] enum [OMX_COMMANDTYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h) cmd, [in] unsigned int param, [in] byte[] cmdData )
```
...
| 名称 | 描述 | 
| -------- | -------- |
| cmd | 组件要执行的命令，详见[OMX_COMMANDTYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。 | 
| param | 组件要执行的命令携带的参数。<br/>- 当cmd为OMX_CommandStateSet时，param的值详见[OMX_STATETYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。<br/>- 当cmd为OMX_CommandFlush、OMX_CommandPortDisable、OMX_CommandPortEnable、OMX_CommandMarkBuffer时，param为目标端口。 | 
...
当组件处于OMX_StateLoaded状态时，使用此回调函数向上通知事件以及上报可用的输入输出信息。更多组件状态的说明请详见[OMX_STATETYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。
...
| 名称 | 描述 | 
| -------- | -------- |
| index | 要设置的结构索引，详见[OMX_INDEXTYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。 | 
...
更多组件状态的说明请详见[OMX_STATETYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。
...
| 名称 | 描述 | 
| -------- | -------- |
| index | 要设置的结构索引，详见OMX IL定义的[OMX_INDEXTYPE](https://gitcode.com/openharmony/third_party_openmax/blob/master/api/1.1.2/OMX_Core.h)。 | 
