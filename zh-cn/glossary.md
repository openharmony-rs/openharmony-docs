...
OpenHarmony无需安装的轻量级服务范式，用户可直接访问功能而无需下载完整应用包。标注为原子化服务API的接口保证在此受限运行时环境中可用。
...
基于ArkTS声明式语法的轻量级服务卡片，运行在设备桌面或服务中心，无需安装完整应用。区别于JS卡片，支持动画、交互和数据刷新，但受限于受限生命周期和有限API集合。
...
ArkUI机制，通过实现AttributeModifier\<T\>类在运行时动态修改组件属性，支持条件化、样式化的属性应用，区别于静态声明式属性链。
...
ArkUI离线节点创建机制，在活跃渲染树之外构建组件节点，支持跨容器节点迁移和重新绑定。
...
ArkUI节点动态管理控制器，支持跨容器组件迁移和重新绑定。

### NodeContent；节点内容插槽

将Native（C/C++）开发的UI组件传入ArkTS页面进行挂载和显示的内容插槽机制。
...
封装给定窗口/页面UI执行上下文的作用域对象，提供实例绑定的API（animateTo、createAnimator、getRouter等），避免全局调用歧义。通过getUIContext()获取，多窗口可各拥有独立实例。
...
能力声明标签，表示该接口经认证可在原子化服务受限运行时环境中正常工作，该环境具有更严格的沙箱和资源约束。
