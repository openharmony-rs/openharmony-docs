...
## cl.bundlemanager.1 module.json的skillProfiles标签新增version必填字段
...
[module.json5配置文件](../../../application-dev/quick-start/module-configuration-file.md)中[skillProfiles标签](../../../application-dev/quick-start/module-configuration-file.md#skillprofiles标签)是模块的技能配置信息，通过定义技能，应用可以将AI代理的能力暴露给系统或其他应用。为了支持技能的版本管理，`skillProfiles`标签新增`version`必填字段，用于标识技能的语义化版本号。
...
- 变更后：`skillProfiles`中的每个技能配置项必须包含`version`字段。
...
1. 检索`module.json`文件，确认是否存在`skillProfiles`配置。
2. 如果配置了`skillProfiles`，检查每个`skillProfiles`配置项是否配置了`version`字段；如果未配置，则需要添加符合规范的版本号，格式为`主版本号.次版本号.补丁版本号`。
...
检查应用`module.json`中`skillProfiles`的`name`字段值，确认名称中不包含连续的连字符。
