...
|新增API|NA|类名：certificateManager；<br>API声明：function getAllUserTrustedCertificates(scope: CertScope): Promise\<CMResult>;<br>差异内容：function getAllUserTrustedCertificates(scope: CertScope): Promise\<CMResult>;|api/@ohos.security.certManager.d.ts|
